...
def test_hyp_java_instancespecification_is_not_abstract():
    assert not inspect.isabstract(java_InstanceSpecification)


def test_hyp_java_instancespecification_constructor_exists():
    assert callable(java_InstanceSpecification.__init__)


def test_hyp_java_instancespecification_constructor_args():
...
def test_java_InstanceSpecification_isa_NamedElement():
    instance = java_InstanceSpecification()
    assert isinstance(instance, NamedElement)
...
def test_assoc_annotations363_link_reassign_clear():
    ...
    if hasattr(b1, 'java_Annotation364'):
        assert _is_linked(b1, 'java_Annotation364', a)
    _safe_set(a, 'java_VariableDeclarationStatement', {b2})
    assert _is_linked(a, 'java_VariableDeclarationStatement', b2)
    if hasattr(b1, 'java_Annotation364'):
        assert not _is_linked(b1, 'java_Annotation364', a)
    if hasattr(b2, 'java_Annotation364'):
        assert _is_linked(b2, 'java_Annotation364', a)
    _safe_set(a, 'java_VariableDeclarationStatement', set())
    assert not _is_linked(a, 'java_VariableDeclarationStatement', b2)
    if hasattr(b2, 'java_Annotation364'):
        assert not _is_linked(b2, 'java_Annotation364', a)
...
def test_assoc_bound365_link_reassign_clear():
    a = java_WildCardType(upperBound=True)
    b1 = java_TypeAccess()
    b2 = java_TypeAccess()
    _safe_set(a, 'java_WildCardType', b1)
    assert _is_linked(a, 'java_WildCardType', b1)
    if hasattr(b1, 'java_TypeAccess366'):
        assert _is_linked(b1, 'java_TypeAccess366', a)
    _safe_set(a, 'java_WildCardType', b2)
    assert _is_linked(a, 'java_WildCardType', b2)
    if hasattr(b1, 'java_TypeAccess366'):
        assert not _is_linked(b1, 'java_TypeAccess366', a)
    if hasattr(b2, 'java_TypeAccess366'):
        assert _is_linked(b2, 'java_TypeAccess366', a)
    _safe_set(a, 'java_WildCardType', None)
    assert not _is_linked(a, 'java_WildCardType', b2)
    if hasattr(b2, 'java_TypeAccess366'):
        assert not _is_linked(b2, 'java_TypeAccess366', a)
...
def test_assoc_initializer353_link_reassign_clear():
    ...
    if hasattr(b1, 'java_Expression354'):
        assert _is_linked(b1, 'java_Expression354', a)
    _safe_set(a, 'java_VariableDeclaration', b2)
    assert _is_linked(a, 'java_VariableDeclaration', b2)
    if hasattr(b1, 'java_Expression354'):
        assert not _is_linked(b1, 'java_Expression354', a)
    if hasattr(b2, 'java_Expression354'):
        assert _is_linked(b2, 'java_Expression354', a)
    _safe_set(a, 'java_VariableDeclaration', None)
    assert not _is_linked(a, 'java_VariableDeclaration', b2)
    if hasattr(b2, 'java_Expression354'):
        assert not _is_linked(b2, 'java_Expression354', a)
...
def test_assoc_usageInVariableAccess355_link_reassign_clear():
    a = java_VariableDeclaration(extraArrayDimensions=7)
...
java_InstanceSpecification_strategy = st.builds(java_InstanceSpecification)
