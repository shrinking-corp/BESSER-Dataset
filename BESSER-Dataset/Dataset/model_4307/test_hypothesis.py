...
def test_metrics_metric_has_value():
    assert hasattr(metrics_Metric, "value")
    descriptor = None
...
def test_metrics_metricsset_is_not_abstract():
    assert not inspect.isabstract(metrics_MetricsSet)


def test_metrics_metricsset_constructor_exists():
    assert callable(metrics_MetricsSet.__init__)


def test_metrics_metricsset_constructor_args():
    sig = inspect.signature(metrics_MetricsSet.__init__)
    params = list(sig.parameters.keys())
    assert "name" in params, "Missing parameter 'name'"

def test_metrics_metricsset_has_name():
    assert hasattr(metrics_MetricsSet, "name")
    descriptor = None
    for klass in metrics_MetricsSet.__mro__:
        if "name" in klass.__dict__:
            descriptor = klass.__dict__["name"]
            break
    assert isinstance(descriptor, property)
...
@given(instance=metrics_Metric_strategy)
def test_metrics_metric_value_setter(instance):
...
@given(instance=metrics_MetricsSet_strategy)
def test_metrics_metricsset_name_setter(instance):
    original = instance.name
    instance.name = original
    assert instance.name == original
