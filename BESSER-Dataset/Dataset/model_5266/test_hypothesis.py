...
def test_typeb_a_has_isNameABC():
    assert hasattr(TypeB_A, "isNameABC")
    descriptor = None
    for klass in TypeB_A.__mro__:
        if "isNameABC" in klass.__dict__:
            descriptor = klass.__dict__["isNameABC"]
            break
    assert isinstance(descriptor, property)
...
@given(instance=TypeB_A_strategy)
def test_typeb_a_isNameABC_setter(instance):
