...
class student:
    ...
    def __init__(self, s_id: int, s_name: str, tescher7: "Tescher" = None, courses11: set["courses"] = None):
        self.s_id = s_id
        self.s_name = s_name
        self.tescher7 = tescher7
        self.courses11 = courses11 if courses11 is not None else set()
    ...
    @s_name.setter
    def s_name(self, s_name: str):
        self.__s_name = s_name

    @property
    def tescher7(self):
        return self.__tescher7
    @tescher7.setter
    def tescher7(self, value):
        # Bidirectional consistency
        old_value = getattr(self, f"_student__tescher7", None)
        self.__tescher7 = value
    ...
    @property
    def courses11(self):
        return self.__courses11
    @courses11.setter
    def courses11(self, value):
        # Bidirectional consistency
        old_value = getattr(self, f"_student__courses11", None)
        self.__courses11 = value if value is not None else set()
...
class courses:
    ...
    def __init__(self, c_name: str, cid: int, tescher9: set["Tescher"] = None, student10: set["student"] = None):
        self.c_name = c_name
        self.cid = cid
        self.tescher9 = tescher9 if tescher9 is not None else set()
    ...
    def student10(self, value):
        # Bidirectional consistency
        old_value = getattr(self, f"_courses__student10", None)
        self.__student10 = value if value is not None else set()
        
        # Remove self from old opposite end
        if old_value is not None:
            for item in old_value:
                if hasattr(item, "courses11"):
                    opp_val = getattr(item, "courses11", None)
        ...
        if value is not None:
            for item in value:
                if hasattr(item, "courses11"):
                    ...
                    if opp_val is None:
                        setattr(item, "courses11", set([self]))
                    elif isinstance(opp_val, set):
                        opp_val.add(self)
    ...
    @property
    def tescher9(self):
        return self.__tescher9
    @tescher9.setter
    def tescher9(self, value):
        # Bidirectional consistency
        old_value = getattr(self, f"_courses__tescher9", None)
        self.__tescher9 = value if value is not None else set()
        
        # Remove self from old opposite end
        if old_value is not None:
            for item in old_value:
                if hasattr(item, "courses8"):
                    opp_val = getattr(item, "courses8", None)
        ...
        if value is not None:
            for item in value:
                if hasattr(item, "courses8"):
                    ...
                    if opp_val is None:
                        setattr(item, "courses8", set([self]))
                    elif isinstance(opp_val, set):
                        opp_val.add(self)
...
class Tescher:
    ...
    def __init__(self, t_id: int, Name: str, City: str, student6: set["student"] = None, courses8: set["courses"] = None):
        self.t_id = t_id
        self.Name = Name
        self.City = City
        self.student6 = student6 if student6 is not None else set()
        self.courses8 = courses8 if courses8 is not None else set()
    ...
    @property
    def courses8(self):
        return self.__courses8
    @courses8.setter
    def courses8(self, value):
        # Bidirectional consistency
        old_value = getattr(self, f"_Tescher__courses8", None)
        self.__courses8 = value if value is not None else set()
        
        # Remove self from old opposite end
        if old_value is not None:
            for item in old_value:
                if hasattr(item, "tescher9"):
                    opp_val = getattr(item, "tescher9", None)
        ...
        if value is not None:
            for item in value:
                if hasattr(item, "tescher9"):
                    ...
                    if opp_val is None:
                        setattr(item, "tescher9", set([self]))
                    elif isinstance(opp_val, set):
                        opp_val.add(self)
    ...
    @property
    def student6(self):
        return self.__student6
    @student6.setter
    def student6(self, value):
        # Bidirectional consistency
        old_value = getattr(self, f"_Tescher__student6", None)
        self.__student6 = value if value is not None else set()
        
        # Remove self from old opposite end
        if old_value is not None:
            for item in old_value:
                if hasattr(item, "tescher7"):
                    opp_val = getattr(item, "tescher7", None)
                    
                    if opp_val == self:
                        setattr(item, "tescher7", None)
