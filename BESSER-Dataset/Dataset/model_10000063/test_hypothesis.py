...
def test_utilisateur_has_nom3():
    assert hasattr(Utilisateur, "nom3")
    descriptor = None
    for klass in Utilisateur.__mro__:
        if "nom3" in klass.__dict__:
            descriptor = klass.__dict__["nom3"]
            break
    assert isinstance(descriptor, property)

def test_utilisateur_has_nom4():
    assert hasattr(Utilisateur, "nom4")
    descriptor = None
    for klass in Utilisateur.__mro__:
        if "nom4" in klass.__dict__:
            descriptor = klass.__dict__["nom4"]
            break
    assert isinstance(descriptor, property)
...
@given(instance=Utilisateur_strategy)
def test_utilisateur_nom3_setter(instance):
...
@given(instance=Utilisateur_strategy)
def test_utilisateur_nom4_setter(instance):
