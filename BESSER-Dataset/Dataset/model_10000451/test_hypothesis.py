...
def test_asteroidspawner_is_not_abstract():
    assert not inspect.isabstract(AsteroidSpawner)


def test_asteroidspawner_constructor_exists():
...
def test_asteroidspawner_has_asteroids():
    assert hasattr(AsteroidSpawner, "asteroids")
    descriptor = None
    for klass in AsteroidSpawner.__mro__:
...
def test_asteroidspawner_has_packageAttribute():
    assert hasattr(AsteroidSpawner, "packageAttribute")
    descriptor = None
    for klass in AsteroidSpawner.__mro__:
        if "packageAttribute" in klass.__dict__:
            descriptor = klass.__dict__["packageAttribute"]
...
@given(instance=AsteroidSpawner_strategy)
def test_asteroidspawner_privateAttribute_setter(instance):
    original = instance.privateAttribute
    instance.privateAttribute = original
    assert instance.privateAttribute == original
...
@given(instance=AsteroidSpawner_strategy)
def test_asteroidspawner_asteroids_setter(instance):
...
@given(instance=AsteroidSpawner_strategy)
def test_asteroidspawner_packageAttribute_setter(instance):
    original = instance.packageAttribute
    instance.packageAttribute = original
    assert instance.packageAttribute == original
