...
def test_library_library_has_ages():
    assert hasattr(library_Library, "ages")
    descriptor = None
    for klass in library_Library.__mro__:
        if "ages" in klass.__dict__:
            descriptor = klass.__dict__["ages"]
            break
    assert isinstance(descriptor, property)
...
@given(instance=library_Library_strategy)
def test_library_library_ages_setter(instance):
