...
def test_hyp_boo_is_not_abstract():
    assert not inspect.isabstract(boo)


def test_hyp_boo_constructor_exists():
    assert callable(boo.__init__)


def test_hyp_boo_constructor_args():
    sig = inspect.signature(boo.__init__)
...
def test_assoc_Class5_C_link_reassign_clear():
    a = C(attC1=7, attC2=True)
    b1 = B(attB=7)
    b2 = B(attB=13)
    _safe_set(a, 'class51', b1)
    assert _is_linked(a, 'class51', b1)
    if hasattr(b1, 'c0'):
        assert _is_linked(b1, 'c0', a)
    _safe_set(a, 'class51', b2)
    assert _is_linked(a, 'class51', b2)
    if hasattr(b1, 'c0'):
        assert not _is_linked(b1, 'c0', a)
    if hasattr(b2, 'c0'):
        assert _is_linked(b2, 'c0', a)
    _safe_set(a, 'class51', None)
    assert not _is_linked(a, 'class51', b2)
    if hasattr(b2, 'c0'):
...
boo_strategy = st.builds(boo)
@given(instance=boo_strategy)
@settings(max_examples=25)
def test_boo_instantiation(instance):
    assert isinstance(instance, boo)
