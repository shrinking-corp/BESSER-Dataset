...
def test_hyp_minifsm_fsm_currentEvent_setter(instance):
...
def test_minifsm_FSM_currentEvent_value_roundtrip():
    instance = minifsm_FSM(currentEvent="sample_text")
    assert instance.currentEvent == "sample_text"
    instance.currentEvent = "sample_text_2"
    assert instance.currentEvent == "sample_text_2"
...
def test_assoc_currentState2_link_reassign_clear():
    ...
    b1 = minifsm_FSM(currentEvent="sample_text")
    b2 = minifsm_FSM(currentEvent="sample_text_2")
    _safe_set(a, 'minifsm_State4', b1)
    assert _is_linked(a, 'minifsm_State4', b1)
    if hasattr(b1, 'minifsm_FSM3'):
        assert _is_linked(b1, 'minifsm_FSM3', a)
    _safe_set(a, 'minifsm_State4', b2)
    assert _is_linked(a, 'minifsm_State4', b2)
    if hasattr(b1, 'minifsm_FSM3'):
        assert not _is_linked(b1, 'minifsm_FSM3', a)
    if hasattr(b2, 'minifsm_FSM3'):
        assert _is_linked(b2, 'minifsm_FSM3', a)
    _safe_set(a, 'minifsm_State4', None)
    assert not _is_linked(a, 'minifsm_State4', b2)
    if hasattr(b2, 'minifsm_FSM3'):
        assert not _is_linked(b2, 'minifsm_FSM3', a)
...
minifsm_FSM_strategy = st.builds(minifsm_FSM, currentEvent=safe_text)
@given(instance=minifsm_FSM_strategy)
@settings(max_examples=25)
