...
def test_hyp_sgraph_junction_is_not_abstract():
    assert not inspect.isabstract(sgraph_Junction)


def test_hyp_sgraph_junction_constructor_exists():
    assert callable(sgraph_Junction.__init__)


def test_hyp_sgraph_junction_constructor_args():
    sig = inspect.signature(sgraph_Junction.__init__)
    params = list(sig.parameters.keys())
...
sgraph_Junction_strategy = st.builds(
    sgraph_Junction,
)
...
sgraph_ScopedElement_strategy = st.builds(sgraph_ScopedElement)
