...
def test_hyp_uma_fulfillableelement_is_not_abstract():
    assert not inspect.isabstract(uma_FulfillableElement)


def test_hyp_uma_fulfillableelement_constructor_exists():
    assert callable(uma_FulfillableElement.__init__)


def test_hyp_uma_fulfillableelement_constructor_args():
...
def test_hyp_fulfillableelement_is_not_abstract():
    assert not inspect.isabstract(FulfillableElement)


def test_hyp_fulfillableelement_constructor_exists():
    assert callable(FulfillableElement.__init__)


def test_hyp_fulfillableelement_constructor_args():
...
@given(instance=uma_ContentDescription_strategy)
def test_hyp_uma_contentdescription_longPresentationName_setter(instance):
...
def test_hyp_uma_classifier_isAbstract_setter(instance):
...
def test_assoc_bases277_link_reassign_clear():
    ...
    _safe_set(a, 'uma_MethodPlugin276', {b1})
    assert _is_linked(a, 'uma_MethodPlugin276', b1)
    if hasattr(b1, 'uma_MethodPlugin278'):
        assert _is_linked(b1, 'uma_MethodPlugin278', a)
    _safe_set(a, 'uma_MethodPlugin276', {b2})
    assert _is_linked(a, 'uma_MethodPlugin276', b2)
    if hasattr(b1, 'uma_MethodPlugin278'):
        assert not _is_linked(b1, 'uma_MethodPlugin278', a)
    if hasattr(b2, 'uma_MethodPlugin278'):
        assert _is_linked(b2, 'uma_MethodPlugin278', a)
    _safe_set(a, 'uma_MethodPlugin276', set())
    assert not _is_linked(a, 'uma_MethodPlugin276', b2)
    if hasattr(b2, 'uma_MethodPlugin278'):
        assert not _is_linked(b2, 'uma_MethodPlugin278', a)
...
def test_assoc_center346_link_reassign_clear():
    ...
    _safe_set(a, 'uma_Point347', b1)
    assert _is_linked(a, 'uma_Point347', b1)
    if hasattr(b1, 'uma_Ellipse'):
        assert _is_linked(b1, 'uma_Ellipse', a)
    _safe_set(a, 'uma_Point347', b2)
    assert _is_linked(a, 'uma_Point347', b2)
    if hasattr(b1, 'uma_Ellipse'):
        assert not _is_linked(b1, 'uma_Ellipse', a)
    if hasattr(b2, 'uma_Ellipse'):
        assert _is_linked(b2, 'uma_Ellipse', a)
    _safe_set(a, 'uma_Point347', None)
    assert not _is_linked(a, 'uma_Point347', b2)
    if hasattr(b2, 'uma_Ellipse'):
...
def test_assoc_contained304_link_reassign_clear():
...
def test_assoc_container314_link_reassign_clear():
    a = uma_DiagramElement(isVisible="sample_text")
    b1 = uma_GraphElement()
    b2 = uma_GraphElement()
...
def test_assoc_deliverablePartsExclude171_link_reassign_clear():
    a = uma_WorkProductDescriptor(activityEntryState="sample_text", activityExitState="sample_text")
    b1 = uma_WorkProduct()
    b2 = uma_WorkProduct()
    _safe_set(a, 'uma_WorkProductDescriptor172', {b1})
    assert _is_linked(a, 'uma_WorkProductDescriptor172', b1)
    if hasattr(b1, 'uma_WorkProduct173'):
        assert _is_linked(b1, 'uma_WorkProduct173', a)
    _safe_set(a, 'uma_WorkProductDescriptor172', {b2})
    assert _is_linked(a, 'uma_WorkProductDescriptor172', b2)
    if hasattr(b1, 'uma_WorkProduct173'):
        assert not _is_linked(b1, 'uma_WorkProduct173', a)
    if hasattr(b2, 'uma_WorkProduct173'):
        assert _is_linked(b2, 'uma_WorkProduct173', a)
    _safe_set(a, 'uma_WorkProductDescriptor172', set())
    assert not _is_linked(a, 'uma_WorkProductDescriptor172', b2)
    if hasattr(b2, 'uma_WorkProduct173'):
        assert not _is_linked(b2, 'uma_WorkProduct173', a)


def test_assoc_diagram321_link_reassign_clear():
...
def test_assoc_diagram331_link_reassign_clear():
    a = uma_SemanticModelBridge(presentation="sample_text")
    b1 = uma_Diagram(zoom="sample_text")
    b2 = uma_Diagram(zoom="sample_text_2")
    _safe_set(a, 'namespace', b1)
    assert _is_linked(a, 'namespace', b1)
    if hasattr(b1, 'Diagram332'):
        assert _is_linked(b1, 'Diagram332', a)
    _safe_set(a, 'namespace', b2)
    assert _is_linked(a, 'namespace', b2)
    if hasattr(b1, 'Diagram332'):
        assert not _is_linked(b1, 'Diagram332', a)
    if hasattr(b2, 'Diagram332'):
        assert _is_linked(b2, 'Diagram332', a)
    _safe_set(a, 'namespace', None)
    assert not _is_linked(a, 'namespace', b2)
    if hasattr(b2, 'Diagram332'):
        assert not _is_linked(b2, 'Diagram332', a)


def test_assoc_diagramLink298_link_reassign_clear():
    a = uma_DiagramLink(zoom="sample_text")
    b1 = uma_Diagram(zoom="sample_text")
    b2 = uma_Diagram(zoom="sample_text_2")
    _safe_set(a, 'DiagramLink', b1)
...
def test_assoc_diagrams296_link_reassign_clear():
    ...
    if hasattr(b1, 'uma_ProcessPackage297'):
        assert _is_linked(b1, 'uma_ProcessPackage297', a)
    _safe_set(a, 'uma_Diagram', b2)
    assert _is_linked(a, 'uma_Diagram', b2)
    if hasattr(b1, 'uma_ProcessPackage297'):
        assert not _is_linked(b1, 'uma_ProcessPackage297', a)
    if hasattr(b2, 'uma_ProcessPackage297'):
        assert _is_linked(b2, 'uma_ProcessPackage297', a)
    _safe_set(a, 'uma_Diagram', None)
    assert not _is_linked(a, 'uma_Diagram', b2)
    if hasattr(b2, 'uma_ProcessPackage297'):
        assert not _is_linked(b2, 'uma_ProcessPackage297', a)


def test_assoc_element341_link_reassign_clear():
    ...
    _safe_set(a, 'uma_MethodElement342', b1)
    assert _is_linked(a, 'uma_MethodElement342', b1)
    if hasattr(b1, 'uma_UMASemanticModelBridge'):
        assert _is_linked(b1, 'uma_UMASemanticModelBridge', a)
    _safe_set(a, 'uma_MethodElement342', b2)
    assert _is_linked(a, 'uma_MethodElement342', b2)
    if hasattr(b1, 'uma_UMASemanticModelBridge'):
    ...
    _safe_set(a, 'uma_MethodElement342', None)
    assert not _is_linked(a, 'uma_MethodElement342', b2)
    if hasattr(b2, 'uma_UMASemanticModelBridge'):
...
def test_assoc_externalInput204_link_reassign_clear():
    a = uma_WorkProductDescriptor(activityEntryState="sample_text", activityExitState="sample_text")
    b1 = uma_TaskDescriptor()
    b2 = uma_TaskDescriptor()
    _safe_set(a, 'uma_WorkProductDescriptor206', b1)
    assert _is_linked(a, 'uma_WorkProductDescriptor206', b1)
    if hasattr(b1, 'uma_TaskDescriptor205'):
        assert _is_linked(b1, 'uma_TaskDescriptor205', a)
    _safe_set(a, 'uma_WorkProductDescriptor206', b2)
    assert _is_linked(a, 'uma_WorkProductDescriptor206', b2)
    if hasattr(b1, 'uma_TaskDescriptor205'):
        assert not _is_linked(b1, 'uma_TaskDescriptor205', a)
    if hasattr(b2, 'uma_TaskDescriptor205'):
        assert _is_linked(b2, 'uma_TaskDescriptor205', a)
    _safe_set(a, 'uma_WorkProductDescriptor206', None)
    assert not _is_linked(a, 'uma_WorkProductDescriptor206', b2)
    if hasattr(b2, 'uma_TaskDescriptor205'):
        assert not _is_linked(b2, 'uma_TaskDescriptor205', a)


def test_assoc_graphElement322_link_reassign_clear():
    a = uma_DiagramLink(zoom="sample_text")
    b1 = uma_GraphElement()
    b2 = uma_GraphElement()
    _safe_set(a, 'link', b1)
    assert _is_linked(a, 'link', b1)
    if hasattr(b1, 'GraphElement323'):
        assert _is_linked(b1, 'GraphElement323', a)
    _safe_set(a, 'link', b2)
    assert _is_linked(a, 'link', b2)
    if hasattr(b1, 'GraphElement323'):
        assert not _is_linked(b1, 'GraphElement323', a)
    if hasattr(b2, 'GraphElement323'):
        assert _is_linked(b2, 'GraphElement323', a)
    _safe_set(a, 'link', None)
    assert not _is_linked(a, 'link', b2)
    if hasattr(b2, 'GraphElement323'):
        assert not _is_linked(b2, 'GraphElement323', a)


def test_assoc_graphElement333_link_reassign_clear():
    a = uma_SemanticModelBridge(presentation="sample_text")
    b1 = uma_GraphElement()
    b2 = uma_GraphElement()
    _safe_set(a, 'semanticModel', b1)
    assert _is_linked(a, 'semanticModel', b1)
    if hasattr(b1, 'GraphElement334'):
        assert _is_linked(b1, 'GraphElement334', a)
    _safe_set(a, 'semanticModel', b2)
    assert _is_linked(a, 'semanticModel', b2)
    if hasattr(b1, 'GraphElement334'):
        assert not _is_linked(b1, 'GraphElement334', a)
    if hasattr(b2, 'GraphElement334'):
        assert _is_linked(b2, 'GraphElement334', a)
    _safe_set(a, 'semanticModel', None)
    assert not _is_linked(a, 'semanticModel', b2)
    if hasattr(b2, 'GraphElement334'):
        assert not _is_linked(b2, 'GraphElement334', a)


def test_assoc_guidanceAdditional175_link_reassign_clear():
    a = uma_Descriptor(isSynchronizedWithSource="sample_text")
    b1 = uma_Guidance()
    b2 = uma_Guidance()
    _safe_set(a, 'uma_Descriptor176', {b1})
    assert _is_linked(a, 'uma_Descriptor176', b1)
    if hasattr(b1, 'uma_Guidance177'):
        assert _is_linked(b1, 'uma_Guidance177', a)
    _safe_set(a, 'uma_Descriptor176', {b2})
    assert _is_linked(a, 'uma_Descriptor176', b2)
    if hasattr(b1, 'uma_Guidance177'):
        assert not _is_linked(b1, 'uma_Guidance177', a)
    if hasattr(b2, 'uma_Guidance177'):
        assert _is_linked(b2, 'uma_Guidance177', a)
    _safe_set(a, 'uma_Descriptor176', set())
    assert not _is_linked(a, 'uma_Descriptor176', b2)
    if hasattr(b2, 'uma_Guidance177'):
        assert not _is_linked(b2, 'uma_Guidance177', a)


def test_assoc_guidanceExclude174_link_reassign_clear():
    a = uma_Descriptor(isSynchronizedWithSource="sample_text")
    b1 = uma_Guidance()
    b2 = uma_Guidance()
    _safe_set(a, 'uma_Descriptor', {b1})
    assert _is_linked(a, 'uma_Descriptor', b1)
    if hasattr(b1, 'uma_Guidance'):
        assert _is_linked(b1, 'uma_Guidance', a)
    _safe_set(a, 'uma_Descriptor', {b2})
    assert _is_linked(a, 'uma_Descriptor', b2)
    if hasattr(b1, 'uma_Guidance'):
        assert not _is_linked(b1, 'uma_Guidance', a)
    if hasattr(b2, 'uma_Guidance'):
        assert _is_linked(b2, 'uma_Guidance', a)
    _safe_set(a, 'uma_Descriptor', set())
    assert not _is_linked(a, 'uma_Descriptor', b2)
    if hasattr(b2, 'uma_Guidance'):
        assert not _is_linked(b2, 'uma_Guidance', a)
...
def test_assoc_interfaceIO338_link_reassign_clear():
    a = uma_WorkProductDescriptor(activityEntryState="sample_text", activityExitState="sample_text")
    b1 = uma_ProcessComponentInterface()
    b2 = uma_ProcessComponentInterface()
    _safe_set(a, 'uma_WorkProductDescriptor340', b1)
    assert _is_linked(a, 'uma_WorkProductDescriptor340', b1)
    if hasattr(b1, 'uma_ProcessComponentInterface339'):
        assert _is_linked(b1, 'uma_ProcessComponentInterface339', a)
    _safe_set(a, 'uma_WorkProductDescriptor340', b2)
    assert _is_linked(a, 'uma_WorkProductDescriptor340', b2)
    if hasattr(b1, 'uma_ProcessComponentInterface339'):
        assert not _is_linked(b1, 'uma_ProcessComponentInterface339', a)
    if hasattr(b2, 'uma_ProcessComponentInterface339'):
        assert _is_linked(b2, 'uma_ProcessComponentInterface339', a)
    _safe_set(a, 'uma_WorkProductDescriptor340', None)
    assert not _is_linked(a, 'uma_WorkProductDescriptor340', b2)
    if hasattr(b2, 'uma_ProcessComponentInterface339'):
        assert not _is_linked(b2, 'uma_ProcessComponentInterface339', a)
...
def test_assoc_link307_link_reassign_clear():
    a = uma_DiagramLink(zoom="sample_text")
    b1 = uma_GraphElement()
    b2 = uma_GraphElement()
    _safe_set(a, 'DiagramLink308', b1)
    assert _is_linked(a, 'DiagramLink308', b1)
    if hasattr(b1, 'graphElement'):
        assert _is_linked(b1, 'graphElement', a)
    _safe_set(a, 'DiagramLink308', b2)
    assert _is_linked(a, 'DiagramLink308', b2)
    if hasattr(b1, 'graphElement'):
        assert not _is_linked(b1, 'graphElement', a)
    if hasattr(b2, 'graphElement'):
        assert _is_linked(b2, 'graphElement', a)
    _safe_set(a, 'DiagramLink308', None)
    assert not _is_linked(a, 'DiagramLink308', b2)
    if hasattr(b2, 'graphElement'):
        assert not _is_linked(b2, 'graphElement', a)
...
def test_assoc_mandatoryInput207_link_reassign_clear():
    a = uma_WorkProductDescriptor(activityEntryState="sample_text", activityExitState="sample_text")
    b1 = uma_TaskDescriptor()
    b2 = uma_TaskDescriptor()
    _safe_set(a, 'uma_WorkProductDescriptor209', b1)
    assert _is_linked(a, 'uma_WorkProductDescriptor209', b1)
    if hasattr(b1, 'uma_TaskDescriptor208'):
        assert _is_linked(b1, 'uma_TaskDescriptor208', a)
    _safe_set(a, 'uma_WorkProductDescriptor209', b2)
    assert _is_linked(a, 'uma_WorkProductDescriptor209', b2)
    if hasattr(b1, 'uma_TaskDescriptor208'):
        assert not _is_linked(b1, 'uma_TaskDescriptor208', a)
    if hasattr(b2, 'uma_TaskDescriptor208'):
        assert _is_linked(b2, 'uma_TaskDescriptor208', a)
    _safe_set(a, 'uma_WorkProductDescriptor209', None)
    assert not _is_linked(a, 'uma_WorkProductDescriptor209', b2)
    if hasattr(b2, 'uma_TaskDescriptor208'):
        assert not _is_linked(b2, 'uma_TaskDescriptor208', a)
...
def test_assoc_methodPackageSelection255_link_reassign_clear():
    a = uma_MethodPackage(global_="sample_text")
    b1 = uma_MethodConfiguration()
    b2 = uma_MethodConfiguration()
    _safe_set(a, 'uma_MethodPackage257', b1)
    assert _is_linked(a, 'uma_MethodPackage257', b1)
    if hasattr(b1, 'uma_MethodConfiguration256'):
        assert _is_linked(b1, 'uma_MethodConfiguration256', a)
    _safe_set(a, 'uma_MethodPackage257', b2)
    assert _is_linked(a, 'uma_MethodPackage257', b2)
    if hasattr(b1, 'uma_MethodConfiguration256'):
        assert not _is_linked(b1, 'uma_MethodConfiguration256', a)
    if hasattr(b2, 'uma_MethodConfiguration256'):
        assert _is_linked(b2, 'uma_MethodConfiguration256', a)
    _safe_set(a, 'uma_MethodPackage257', None)
    assert not _is_linked(a, 'uma_MethodPackage257', b2)
    if hasattr(b2, 'uma_MethodConfiguration256'):
        assert not _is_linked(b2, 'uma_MethodConfiguration256', a)


def test_assoc_methodPackages273_link_reassign_clear():
    a = uma_MethodPlugin(supporting=True, userChangeable="sample_text")
    b1 = uma_MethodPackage(global_="sample_text")
    b2 = uma_MethodPackage(global_="sample_text_2")
    _safe_set(a, 'uma_MethodPlugin274', {b1})
    assert _is_linked(a, 'uma_MethodPlugin274', b1)
    if hasattr(b1, 'uma_MethodPackage275'):
        assert _is_linked(b1, 'uma_MethodPackage275', a)
    _safe_set(a, 'uma_MethodPlugin274', {b2})
    assert _is_linked(a, 'uma_MethodPlugin274', b2)
    if hasattr(b1, 'uma_MethodPackage275'):
        assert not _is_linked(b1, 'uma_MethodPackage275', a)
    if hasattr(b2, 'uma_MethodPackage275'):
        assert _is_linked(b2, 'uma_MethodPackage275', a)
    _safe_set(a, 'uma_MethodPlugin274', set())
    assert not _is_linked(a, 'uma_MethodPlugin274', b2)
    if hasattr(b2, 'uma_MethodPackage275'):
        assert not _is_linked(b2, 'uma_MethodPackage275', a)


def test_assoc_methodPluginSelection253_link_reassign_clear():
    a = uma_MethodPlugin(supporting=True, userChangeable="sample_text")
    b1 = uma_MethodConfiguration()
    b2 = uma_MethodConfiguration()
    _safe_set(a, 'uma_MethodPlugin', b1)
    assert _is_linked(a, 'uma_MethodPlugin', b1)
    if hasattr(b1, 'uma_MethodConfiguration254'):
        assert _is_linked(b1, 'uma_MethodConfiguration254', a)
    _safe_set(a, 'uma_MethodPlugin', b2)
    assert _is_linked(a, 'uma_MethodPlugin', b2)
    if hasattr(b1, 'uma_MethodConfiguration254'):
        assert not _is_linked(b1, 'uma_MethodConfiguration254', a)
    if hasattr(b2, 'uma_MethodConfiguration254'):
        assert _is_linked(b2, 'uma_MethodConfiguration254', a)
    _safe_set(a, 'uma_MethodPlugin', None)
    assert not _is_linked(a, 'uma_MethodPlugin', b2)
    if hasattr(b2, 'uma_MethodConfiguration254'):
        assert not _is_linked(b2, 'uma_MethodConfiguration254', a)


def test_assoc_methodPlugins350_link_reassign_clear():
    a = uma_MethodPlugin(supporting=True, userChangeable="sample_text")
    b1 = uma_MethodLibrary()
    b2 = uma_MethodLibrary()
    _safe_set(a, 'uma_MethodPlugin351', b1)
    assert _is_linked(a, 'uma_MethodPlugin351', b1)
    if hasattr(b1, 'uma_MethodLibrary'):
        assert _is_linked(b1, 'uma_MethodLibrary', a)
    _safe_set(a, 'uma_MethodPlugin351', b2)
    assert _is_linked(a, 'uma_MethodPlugin351', b2)
    if hasattr(b1, 'uma_MethodLibrary'):
        assert not _is_linked(b1, 'uma_MethodLibrary', a)
    if hasattr(b2, 'uma_MethodLibrary'):
        assert _is_linked(b2, 'uma_MethodLibrary', a)
    _safe_set(a, 'uma_MethodPlugin351', None)
    assert not _is_linked(a, 'uma_MethodPlugin351', b2)
    if hasattr(b2, 'uma_MethodLibrary'):
        assert not _is_linked(b2, 'uma_MethodLibrary', a)


def test_assoc_modifies187_link_reassign_clear():
    a = uma_WorkProductDescriptor(activityEntryState="sample_text", activityExitState="sample_text")
    b1 = uma_RoleDescriptor()
    b2 = uma_RoleDescriptor()
    _safe_set(a, 'uma_WorkProductDescriptor189', b1)
    assert _is_linked(a, 'uma_WorkProductDescriptor189', b1)
    if hasattr(b1, 'uma_RoleDescriptor188'):
        assert _is_linked(b1, 'uma_RoleDescriptor188', a)
    _safe_set(a, 'uma_WorkProductDescriptor189', b2)
    assert _is_linked(a, 'uma_WorkProductDescriptor189', b2)
    if hasattr(b1, 'uma_RoleDescriptor188'):
        assert not _is_linked(b1, 'uma_RoleDescriptor188', a)
    if hasattr(b2, 'uma_RoleDescriptor188'):
        assert _is_linked(b2, 'uma_RoleDescriptor188', a)
    _safe_set(a, 'uma_WorkProductDescriptor189', None)
    assert not _is_linked(a, 'uma_WorkProductDescriptor189', b2)
    if hasattr(b2, 'uma_RoleDescriptor188'):
        assert not _is_linked(b2, 'uma_RoleDescriptor188', a)


def test_assoc_namespace299_link_reassign_clear():
    a = uma_SemanticModelBridge(presentation="sample_text")
    b1 = uma_Diagram(zoom="sample_text")
    b2 = uma_Diagram(zoom="sample_text_2")
    _safe_set(a, 'SemanticModelBridge', b1)
    assert _is_linked(a, 'SemanticModelBridge', b1)
    if hasattr(b1, 'diagram300'):
        assert _is_linked(b1, 'diagram300', a)
    _safe_set(a, 'SemanticModelBridge', b2)
    assert _is_linked(a, 'SemanticModelBridge', b2)
    if hasattr(b1, 'diagram300'):
        assert not _is_linked(b1, 'diagram300', a)
    if hasattr(b2, 'diagram300'):
        assert _is_linked(b2, 'diagram300', a)
    _safe_set(a, 'SemanticModelBridge', None)
    assert not _is_linked(a, 'SemanticModelBridge', b2)
    if hasattr(b2, 'diagram300'):
        assert not _is_linked(b2, 'diagram300', a)


def test_assoc_optionalInput210_link_reassign_clear():
    a = uma_WorkProductDescriptor(activityEntryState="sample_text", activityExitState="sample_text")
    b1 = uma_TaskDescriptor()
    b2 = uma_TaskDescriptor()
    _safe_set(a, 'uma_WorkProductDescriptor212', b1)
    assert _is_linked(a, 'uma_WorkProductDescriptor212', b1)
    if hasattr(b1, 'uma_TaskDescriptor211'):
        assert _is_linked(b1, 'uma_TaskDescriptor211', a)
    _safe_set(a, 'uma_WorkProductDescriptor212', b2)
    assert _is_linked(a, 'uma_WorkProductDescriptor212', b2)
    if hasattr(b1, 'uma_TaskDescriptor211'):
        assert not _is_linked(b1, 'uma_TaskDescriptor211', a)
    if hasattr(b2, 'uma_TaskDescriptor211'):
        assert _is_linked(b2, 'uma_TaskDescriptor211', a)
    _safe_set(a, 'uma_WorkProductDescriptor212', None)
    assert not _is_linked(a, 'uma_WorkProductDescriptor212', b2)
    if hasattr(b2, 'uma_TaskDescriptor211'):
        assert not _is_linked(b2, 'uma_TaskDescriptor211', a)


def test_assoc_output213_link_reassign_clear():
    a = uma_WorkProductDescriptor(activityEntryState="sample_text", activityExitState="sample_text")
    b1 = uma_TaskDescriptor()
    b2 = uma_TaskDescriptor()
    _safe_set(a, 'uma_WorkProductDescriptor215', b1)
    assert _is_linked(a, 'uma_WorkProductDescriptor215', b1)
    if hasattr(b1, 'uma_TaskDescriptor214'):
        assert _is_linked(b1, 'uma_TaskDescriptor214', a)
    _safe_set(a, 'uma_WorkProductDescriptor215', b2)
    assert _is_linked(a, 'uma_WorkProductDescriptor215', b2)
    if hasattr(b1, 'uma_TaskDescriptor214'):
        assert not _is_linked(b1, 'uma_TaskDescriptor214', a)
    if hasattr(b2, 'uma_TaskDescriptor214'):
        assert _is_linked(b2, 'uma_TaskDescriptor214', a)
    _safe_set(a, 'uma_WorkProductDescriptor215', None)
    assert not _is_linked(a, 'uma_WorkProductDescriptor215', b2)
    if hasattr(b2, 'uma_TaskDescriptor214'):
        assert not _is_linked(b2, 'uma_TaskDescriptor214', a)
...
def test_assoc_position305_link_reassign_clear():
    a = uma_Point(x="sample_text", y="sample_text")
    b1 = uma_GraphElement()
    b2 = uma_GraphElement()
    _safe_set(a, 'uma_Point306', b1)
    assert _is_linked(a, 'uma_Point306', b1)
    if hasattr(b1, 'uma_GraphElement'):
        assert _is_linked(b1, 'uma_GraphElement', a)
    _safe_set(a, 'uma_Point306', b2)
    assert _is_linked(a, 'uma_Point306', b2)
    if hasattr(b1, 'uma_GraphElement'):
        assert not _is_linked(b1, 'uma_GraphElement', a)
    if hasattr(b2, 'uma_GraphElement'):
        assert _is_linked(b2, 'uma_GraphElement', a)
    _safe_set(a, 'uma_Point306', None)
    assert not _is_linked(a, 'uma_Point306', b2)
    if hasattr(b2, 'uma_GraphElement'):
...
def test_assoc_property316_link_reassign_clear():
...
def test_assoc_reference315_link_reassign_clear():
...
def test_assoc_referenced317_link_reassign_clear():
    a = uma_Reference(isIndividualRepresentation="sample_text")
    b1 = uma_DiagramElement(isVisible="sample_text")
    b2 = uma_DiagramElement(isVisible="sample_text_2")
    _safe_set(a, 'reference', b1)
    assert _is_linked(a, 'reference', b1)
    if hasattr(b1, 'DiagramElement318'):
        assert _is_linked(b1, 'DiagramElement318', a)
    _safe_set(a, 'reference', b2)
    assert _is_linked(a, 'reference', b2)
    if hasattr(b1, 'DiagramElement318'):
        assert not _is_linked(b1, 'DiagramElement318', a)
    if hasattr(b2, 'DiagramElement318'):
        assert _is_linked(b2, 'DiagramElement318', a)
    _safe_set(a, 'reference', None)
    assert not _is_linked(a, 'reference', b2)
    if hasattr(b2, 'DiagramElement318'):
        assert not _is_linked(b2, 'DiagramElement318', a)
...
def test_assoc_responsibleFor190_link_reassign_clear():
    a = uma_WorkProductDescriptor(activityEntryState="sample_text", activityExitState="sample_text")
    b1 = uma_RoleDescriptor()
    b2 = uma_RoleDescriptor()
    _safe_set(a, 'uma_WorkProductDescriptor192', b1)
    assert _is_linked(a, 'uma_WorkProductDescriptor192', b1)
    if hasattr(b1, 'uma_RoleDescriptor191'):
        assert _is_linked(b1, 'uma_RoleDescriptor191', a)
    _safe_set(a, 'uma_WorkProductDescriptor192', b2)
    assert _is_linked(a, 'uma_WorkProductDescriptor192', b2)
    if hasattr(b1, 'uma_RoleDescriptor191'):
        assert not _is_linked(b1, 'uma_RoleDescriptor191', a)
    if hasattr(b2, 'uma_RoleDescriptor191'):
        assert _is_linked(b2, 'uma_RoleDescriptor191', a)
    _safe_set(a, 'uma_WorkProductDescriptor192', None)
    assert not _is_linked(a, 'uma_WorkProductDescriptor192', b2)
    if hasattr(b2, 'uma_RoleDescriptor191'):
        assert not _is_linked(b2, 'uma_RoleDescriptor191', a)
...
def test_assoc_selectedSteps219_link_reassign_clear():
    a = uma_Section(sectionDescription="sample_text", sectionName="sample_text")
    b1 = uma_TaskDescriptor()
    b2 = uma_TaskDescriptor()
    _safe_set(a, 'uma_Section221', b1)
    assert _is_linked(a, 'uma_Section221', b1)
    if hasattr(b1, 'uma_TaskDescriptor220'):
        assert _is_linked(b1, 'uma_TaskDescriptor220', a)
    _safe_set(a, 'uma_Section221', b2)
    assert _is_linked(a, 'uma_Section221', b2)
    if hasattr(b1, 'uma_TaskDescriptor220'):
        assert not _is_linked(b1, 'uma_TaskDescriptor220', a)
    if hasattr(b2, 'uma_TaskDescriptor220'):
        assert _is_linked(b2, 'uma_TaskDescriptor220', a)
    _safe_set(a, 'uma_Section221', None)
    assert not _is_linked(a, 'uma_Section221', b2)
    if hasattr(b2, 'uma_TaskDescriptor220'):
        assert not _is_linked(b2, 'uma_TaskDescriptor220', a)


def test_assoc_selectedStepsExclude237_link_reassign_clear():
    a = uma_Section(sectionDescription="sample_text", sectionName="sample_text")
    b1 = uma_TaskDescriptor()
    b2 = uma_TaskDescriptor()
    _safe_set(a, 'uma_Section239', b1)
    assert _is_linked(a, 'uma_Section239', b1)
    if hasattr(b1, 'uma_TaskDescriptor238'):
        assert _is_linked(b1, 'uma_TaskDescriptor238', a)
    _safe_set(a, 'uma_Section239', b2)
    assert _is_linked(a, 'uma_Section239', b2)
    if hasattr(b1, 'uma_TaskDescriptor238'):
        assert not _is_linked(b1, 'uma_TaskDescriptor238', a)
    if hasattr(b2, 'uma_TaskDescriptor238'):
        assert _is_linked(b2, 'uma_TaskDescriptor238', a)
    _safe_set(a, 'uma_Section239', None)
    assert not _is_linked(a, 'uma_Section239', b2)
    if hasattr(b2, 'uma_TaskDescriptor238'):
        assert not _is_linked(b2, 'uma_TaskDescriptor238', a)


def test_assoc_semanticModel311_link_reassign_clear():
    a = uma_SemanticModelBridge(presentation="sample_text")
    b1 = uma_GraphElement()
    b2 = uma_GraphElement()
    _safe_set(a, 'SemanticModelBridge313', b1)
    assert _is_linked(a, 'SemanticModelBridge313', b1)
    if hasattr(b1, 'graphElement312'):
        assert _is_linked(b1, 'graphElement312', a)
    _safe_set(a, 'SemanticModelBridge313', b2)
    assert _is_linked(a, 'SemanticModelBridge313', b2)
    if hasattr(b1, 'graphElement312'):
        assert not _is_linked(b1, 'graphElement312', a)
    if hasattr(b2, 'graphElement312'):
        assert _is_linked(b2, 'graphElement312', a)
    _safe_set(a, 'SemanticModelBridge313', None)
    assert not _is_linked(a, 'SemanticModelBridge313', b2)
    if hasattr(b2, 'graphElement312'):
        assert not _is_linked(b2, 'graphElement312', a)


def test_assoc_size303_link_reassign_clear():
...
def test_assoc_viewpoint301_link_reassign_clear():
    a = uma_Point(x="sample_text", y="sample_text")
    b1 = uma_Diagram(zoom="sample_text")
    b2 = uma_Diagram(zoom="sample_text_2")
    _safe_set(a, 'uma_Point', b1)
    assert _is_linked(a, 'uma_Point', b1)
    if hasattr(b1, 'uma_Diagram302'):
        assert _is_linked(b1, 'uma_Diagram302', a)
    _safe_set(a, 'uma_Point', b2)
    assert _is_linked(a, 'uma_Point', b2)
    if hasattr(b1, 'uma_Diagram302'):
        assert not _is_linked(b1, 'uma_Diagram302', a)
    if hasattr(b2, 'uma_Diagram302'):
        assert _is_linked(b2, 'uma_Diagram302', a)
    _safe_set(a, 'uma_Point', None)
    assert not _is_linked(a, 'uma_Point', b2)
    if hasattr(b2, 'uma_Diagram302'):
        assert not _is_linked(b2, 'uma_Diagram302', a)


def test_assoc_viewport319_link_reassign_clear():
    a = uma_Point(x="sample_text", y="sample_text")
    b1 = uma_DiagramLink(zoom="sample_text")
    b2 = uma_DiagramLink(zoom="sample_text_2")
    _safe_set(a, 'uma_Point320', b1)
    assert _is_linked(a, 'uma_Point320', b1)
    if hasattr(b1, 'uma_DiagramLink'):
        assert _is_linked(b1, 'uma_DiagramLink', a)
    _safe_set(a, 'uma_Point320', b2)
    assert _is_linked(a, 'uma_Point320', b2)
    if hasattr(b1, 'uma_DiagramLink'):
        assert not _is_linked(b1, 'uma_DiagramLink', a)
    if hasattr(b2, 'uma_DiagramLink'):
        assert _is_linked(b2, 'uma_DiagramLink', a)
    _safe_set(a, 'uma_Point320', None)
    assert not _is_linked(a, 'uma_Point320', b2)
    if hasattr(b2, 'uma_DiagramLink'):
        assert not _is_linked(b2, 'uma_DiagramLink', a)


def test_assoc_waypoints327_link_reassign_clear():
    a = uma_Point(x="sample_text", y="sample_text")
    b1 = uma_GraphEdge()
    b2 = uma_GraphEdge()
    _safe_set(a, 'uma_Point328', b1)
    assert _is_linked(a, 'uma_Point328', b1)
    if hasattr(b1, 'uma_GraphEdge'):
        assert _is_linked(b1, 'uma_GraphEdge', a)
    _safe_set(a, 'uma_Point328', b2)
    assert _is_linked(a, 'uma_Point328', b2)
    if hasattr(b1, 'uma_GraphEdge'):
        assert not _is_linked(b1, 'uma_GraphEdge', a)
    if hasattr(b2, 'uma_GraphEdge'):
        assert _is_linked(b2, 'uma_GraphEdge', a)
    _safe_set(a, 'uma_Point328', None)
    assert not _is_linked(a, 'uma_Point328', b2)
    if hasattr(b2, 'uma_GraphEdge'):
        assert not _is_linked(b2, 'uma_GraphEdge', a)


def test_assoc_waypoints344_link_reassign_clear():
    ...
    if hasattr(b1, 'uma_Point345'):
        assert _is_linked(b1, 'uma_Point345', a)
    _safe_set(a, 'uma_Polyline', {b2})
    assert _is_linked(a, 'uma_Polyline', b2)
    if hasattr(b1, 'uma_Point345'):
        assert not _is_linked(b1, 'uma_Point345', a)
    if hasattr(b2, 'uma_Point345'):
        assert _is_linked(b2, 'uma_Point345', a)
    _safe_set(a, 'uma_Polyline', set())
    assert not _is_linked(a, 'uma_Polyline', b2)
    if hasattr(b2, 'uma_Point345'):
        assert not _is_linked(b2, 'uma_Point345', a)
