...
def test_component_constructor_args():
    ...
    assert "beschikbaarheidspercentage" in params, "Missing parameter 'beschikbaarheidspercentage'"
...
def test_component_has_prijs():
    assert hasattr(Component, "prijs")
    descriptor = None
    for klass in Component.__mro__:
...
def test_component_has_beschikbaarheidspercentage():
    assert hasattr(Component, "beschikbaarheidspercentage")
    descriptor = None
    for klass in Component.__mro__:
        if "beschikbaarheidspercentage" in klass.__dict__:
            descriptor = klass.__dict__["beschikbaarheidspercentage"]
            break
    assert isinstance(descriptor, property)



def test_componentenlijst_is_not_abstract():
    assert not inspect.isabstract(ComponentenLijst)


def test_componentenlijst_constructor_exists():
    assert callable(ComponentenLijst.__init__)


def test_componentenlijst_constructor_args():
    sig = inspect.signature(ComponentenLijst.__init__)
    params = list(sig.parameters.keys())
    assert "componentenlijst" in params, "Missing parameter 'componentenlijst'"

def test_componentenlijst_has_componentenlijst():
    assert hasattr(ComponentenLijst, "componentenlijst")
    descriptor = None
    for klass in ComponentenLijst.__mro__:
        if "componentenlijst" in klass.__dict__:
            descriptor = klass.__dict__["componentenlijst"]
            break
    assert isinstance(descriptor, property)
...
ComponentenLijst_strategy = st.builds(
    ComponentenLijst,
    componentenlijst=
        safe_text
)
...
@given(instance=Component_strategy)
def test_component_model_setter(instance):
...
@given(instance=Component_strategy)
def test_component_prijs_setter(instance):
...
@given(instance=Component_strategy)
def test_component_beschikbaarheidspercentage_setter(instance):
    original = instance.beschikbaarheidspercentage
    instance.beschikbaarheidspercentage = original
    assert instance.beschikbaarheidspercentage == original
...
@given(instance=ComponentenLijst_strategy)
def test_componentenlijst_componentenlijst_setter(instance):
    original = instance.componentenlijst
    instance.componentenlijst = original
    assert instance.componentenlijst == original
