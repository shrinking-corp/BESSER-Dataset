...
def test_assoc_block2_link_reassign_clear():
    ...
    if hasattr(b1, 'BlockB'):
        assert _is_linked(b1, 'BlockB', a)
    _safe_set(a, 'outputPorts', b2)
    assert _is_linked(a, 'outputPorts', b2)
    if hasattr(b1, 'BlockB'):
        assert not _is_linked(b1, 'BlockB', a)
    if hasattr(b2, 'BlockB'):
        assert _is_linked(b2, 'BlockB', a)
    _safe_set(a, 'outputPorts', None)
    assert not _is_linked(a, 'outputPorts', b2)
    if hasattr(b2, 'BlockB'):
        assert not _is_linked(b2, 'BlockB', a)
...
def test_assoc_outputPorts1_link_reassign_clear():
    ...
    b1 = typeB_BlockB()
    b2 = typeB_BlockB()
    _safe_set(a, 'OutPortB', b1)
    assert _is_linked(a, 'OutPortB', b1)
    if hasattr(b1, 'block'):
        assert _is_linked(b1, 'block', a)
    _safe_set(a, 'OutPortB', b2)
    assert _is_linked(a, 'OutPortB', b2)
    if hasattr(b1, 'block'):
        assert not _is_linked(b1, 'block', a)
    if hasattr(b2, 'block'):
        assert _is_linked(b2, 'block', a)
    _safe_set(a, 'OutPortB', None)
    assert not _is_linked(a, 'OutPortB', b2)
    if hasattr(b2, 'block'):
