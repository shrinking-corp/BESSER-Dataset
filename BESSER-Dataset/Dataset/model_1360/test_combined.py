...
def test_hyp_statemachine_need_is_not_abstract():
    assert not inspect.isabstract(statemachine_Need)


def test_hyp_statemachine_need_constructor_exists():
    assert callable(statemachine_Need.__init__)


def test_hyp_statemachine_need_constructor_args():
...
def test_assoc_source_need5_link_reassign_clear():
...
def test_assoc_target_need10_link_reassign_clear():
    a = statemachine_Resource(name="sample_text")
    b1 = statemachine_Need()
    b2 = statemachine_Need()
    _safe_set(a, 'statemachine_Resource12', b1)
    assert _is_linked(a, 'statemachine_Resource12', b1)
    if hasattr(b1, 'statemachine_Need11'):
        assert _is_linked(b1, 'statemachine_Need11', a)
    _safe_set(a, 'statemachine_Resource12', b2)
    assert _is_linked(a, 'statemachine_Resource12', b2)
    if hasattr(b1, 'statemachine_Need11'):
        assert not _is_linked(b1, 'statemachine_Need11', a)
    if hasattr(b2, 'statemachine_Need11'):
        assert _is_linked(b2, 'statemachine_Need11', a)
    _safe_set(a, 'statemachine_Resource12', None)
    assert not _is_linked(a, 'statemachine_Resource12', b2)
    if hasattr(b2, 'statemachine_Need11'):
        assert not _is_linked(b2, 'statemachine_Need11', a)
