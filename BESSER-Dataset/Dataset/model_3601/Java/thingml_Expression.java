





import java.util.List;
import java.util.ArrayList;

public class thingml_Expression  {






    private thingml_LocalVariable thingml_localvariable;




    private thingml_ConfigPropertyAssign thingml_configpropertyassign;




    private thingml_PropertyAssign thingml_propertyassign;




    private thingml_Handler thingml_handler;




    private thingml_PropertyAssign thingml_propertyassign;




    private thingml_Property thingml_property;




    private thingml_ConfigPropertyAssign thingml_configpropertyassign;




    private thingml_TypedElement thingml_typedelement;


    public thingml_Expression(
    ) {
    }



    public thingml_LocalVariable getThingml_localvariable() {
        return thingml_localvariable;
    }

    public void setThingml_localvariable(thingml_LocalVariable thingml_localvariable) {
        this.thingml_localvariable = thingml_localvariable;
    }
    public thingml_ConfigPropertyAssign getThingml_configpropertyassign() {
        return thingml_configpropertyassign;
    }

    public void setThingml_configpropertyassign(thingml_ConfigPropertyAssign thingml_configpropertyassign) {
        this.thingml_configpropertyassign = thingml_configpropertyassign;
    }
    public thingml_PropertyAssign getThingml_propertyassign() {
        return thingml_propertyassign;
    }

    public void setThingml_propertyassign(thingml_PropertyAssign thingml_propertyassign) {
        this.thingml_propertyassign = thingml_propertyassign;
    }
    public thingml_Handler getThingml_handler() {
        return thingml_handler;
    }

    public void setThingml_handler(thingml_Handler thingml_handler) {
        this.thingml_handler = thingml_handler;
    }
    public thingml_PropertyAssign getThingml_propertyassign() {
        return thingml_propertyassign;
    }

    public void setThingml_propertyassign(thingml_PropertyAssign thingml_propertyassign) {
        this.thingml_propertyassign = thingml_propertyassign;
    }
    public thingml_Property getThingml_property() {
        return thingml_property;
    }

    public void setThingml_property(thingml_Property thingml_property) {
        this.thingml_property = thingml_property;
    }
    public thingml_ConfigPropertyAssign getThingml_configpropertyassign() {
        return thingml_configpropertyassign;
    }

    public void setThingml_configpropertyassign(thingml_ConfigPropertyAssign thingml_configpropertyassign) {
        this.thingml_configpropertyassign = thingml_configpropertyassign;
    }
    public thingml_TypedElement getThingml_typedelement() {
        return thingml_typedelement;
    }

    public void setThingml_typedelement(thingml_TypedElement thingml_typedelement) {
        this.thingml_typedelement = thingml_typedelement;
    }

}