...
def test_column_testschema_is_not_abstract():
    assert not inspect.isabstract(column_TestSchema)


def test_column_testschema_constructor_exists():
    assert callable(column_TestSchema.__init__)


def test_column_testschema_constructor_args():
    sig = inspect.signature(column_TestSchema.__init__)
    params = list(sig.parameters.keys())
    assert "name" in params, "Missing parameter 'name'"

def test_column_testschema_has_name():
    assert hasattr(column_TestSchema, "name")
    descriptor = None
    for klass in column_TestSchema.__mro__:
...
@given(instance=column_TestSchema_strategy)
def test_column_testschema_name_setter(instance):
