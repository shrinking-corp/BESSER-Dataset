...
def test_model1_product1_constructor_args():
    ...
    assert "otherVATs" in params, "Missing parameter 'otherVATs'"
...
def test_model1_product1_has_otherVATs():
    assert hasattr(model1_Product1, "otherVATs")
    descriptor = None
    for klass in model1_Product1.__mro__:
        if "otherVATs" in klass.__dict__:
            descriptor = klass.__dict__["otherVATs"]
            break
    assert isinstance(descriptor, property)
...
@given(instance=model1_Product1_strategy)
def test_model1_product1_otherVATs_setter(instance):
    original = instance.otherVATs
    instance.otherVATs = original
    assert instance.otherVATs == original
