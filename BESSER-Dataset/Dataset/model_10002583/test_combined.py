...
def test_hyp_carro_is_not_abstract():
    assert not inspect.isabstract(Carro)


def test_hyp_carro_constructor_exists():
    assert callable(Carro.__init__)


def test_hyp_carro_constructor_args():
    sig = inspect.signature(Carro.__init__)
    params = list(sig.parameters.keys())
    assert "velocidade" in params, "Missing parameter 'velocidade'"
...
@given(instance=Carro_strategy)
def test_hyp_carro_velocidade_setter(instance):
    original = instance.velocidade
    instance.velocidade = original
    assert instance.velocidade == original
...
def test_Carro_velocidade_value_roundtrip():
    instance = Carro(velocidade=3.14)
    assert instance.velocidade == 3.14
    instance.velocidade = 9.99
    assert instance.velocidade == 9.99
...
Carro_strategy = st.builds(Carro, velocidade=st.floats(allow_nan=False, allow_infinity=False))
@given(instance=Carro_strategy)
@settings(max_examples=25)
def test_Carro_instantiation(instance):
    assert isinstance(instance, Carro)
