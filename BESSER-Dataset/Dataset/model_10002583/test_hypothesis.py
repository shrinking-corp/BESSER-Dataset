...
def test_carro_is_not_abstract():
    assert not inspect.isabstract(Carro)


def test_carro_constructor_exists():
    assert callable(Carro.__init__)


def test_carro_constructor_args():
    sig = inspect.signature(Carro.__init__)
    params = list(sig.parameters.keys())
    assert "velocidade" in params, "Missing parameter 'velocidade'"

def test_carro_has_velocidade():
    assert hasattr(Carro, "velocidade")
    descriptor = None
    for klass in Carro.__mro__:
        if "velocidade" in klass.__dict__:
            descriptor = klass.__dict__["velocidade"]
            break
...
@given(instance=Carro_strategy)
def test_carro_velocidade_setter(instance):
