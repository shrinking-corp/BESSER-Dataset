...
def test_hyp_gmfgraph_layoutref_is_not_abstract():
    assert not inspect.isabstract(gmfgraph_LayoutRef)


def test_hyp_gmfgraph_layoutref_constructor_exists():
    assert callable(gmfgraph_LayoutRef.__init__)


def test_hyp_gmfgraph_layoutref_constructor_args():
    sig = inspect.signature(gmfgraph_LayoutRef.__init__)
    params = list(sig.parameters.keys())
...
def test_hyp_gmfgraph_borderref_is_not_abstract():
    assert not inspect.isabstract(gmfgraph_BorderRef)


def test_hyp_gmfgraph_borderref_constructor_exists():
    assert callable(gmfgraph_BorderRef.__init__)


def test_hyp_gmfgraph_borderref_constructor_args():
    sig = inspect.signature(gmfgraph_BorderRef.__init__)
    params = list(sig.parameters.keys())
...
gmfgraph_LayoutRef_strategy = st.builds(
    gmfgraph_LayoutRef,
)
...
gmfgraph_BorderRef_strategy = st.builds(
    gmfgraph_BorderRef,
)
...
def test_assoc_accessors109_link_reassign_clear():
    ...
    if hasattr(b1, 'owner110'):
        assert _is_linked(b1, 'owner110', a)
    _safe_set(a, 'ChildAccess', b2)
    assert _is_linked(a, 'ChildAccess', b2)
    if hasattr(b1, 'owner110'):
        assert not _is_linked(b1, 'owner110', a)
    if hasattr(b2, 'owner110'):
        assert _is_linked(b2, 'owner110', a)
    _safe_set(a, 'ChildAccess', None)
    assert not _is_linked(a, 'ChildAccess', b2)
    if hasattr(b2, 'owner110'):
        assert not _is_linked(b2, 'owner110', a)


def test_assoc_attributes69_link_reassign_clear():
...
def test_assoc_children103_link_reassign_clear():
    a = gmfgraph_RealFigure(name="sample_text")
    b1 = gmfgraph_Figure()
    b2 = gmfgraph_Figure()
    _safe_set(a, 'gmfgraph_RealFigure104', {b1})
    assert _is_linked(a, 'gmfgraph_RealFigure104', b1)
    if hasattr(b1, 'gmfgraph_Figure105'):
        assert _is_linked(b1, 'gmfgraph_Figure105', a)
    _safe_set(a, 'gmfgraph_RealFigure104', {b2})
    assert _is_linked(a, 'gmfgraph_RealFigure104', b2)
    if hasattr(b1, 'gmfgraph_Figure105'):
        assert not _is_linked(b1, 'gmfgraph_Figure105', a)
    if hasattr(b2, 'gmfgraph_Figure105'):
        assert _is_linked(b2, 'gmfgraph_Figure105', a)
    _safe_set(a, 'gmfgraph_RealFigure104', set())
    assert not _is_linked(a, 'gmfgraph_RealFigure104', b2)
    if hasattr(b2, 'gmfgraph_Figure105'):
        assert not _is_linked(b2, 'gmfgraph_Figure105', a)


def test_assoc_color74_link_reassign_clear():
    a = gmfgraph_LineBorder(width=7)
    b1 = gmfgraph_Color()
    b2 = gmfgraph_Color()
    _safe_set(a, 'gmfgraph_LineBorder', b1)
    assert _is_linked(a, 'gmfgraph_LineBorder', b1)
    if hasattr(b1, 'gmfgraph_Color75'):
        assert _is_linked(b1, 'gmfgraph_Color75', a)
    _safe_set(a, 'gmfgraph_LineBorder', b2)
    assert _is_linked(a, 'gmfgraph_LineBorder', b2)
    if hasattr(b1, 'gmfgraph_Color75'):
        assert not _is_linked(b1, 'gmfgraph_Color75', a)
    if hasattr(b2, 'gmfgraph_Color75'):
        assert _is_linked(b2, 'gmfgraph_Color75', a)
    _safe_set(a, 'gmfgraph_LineBorder', None)
    assert not _is_linked(a, 'gmfgraph_LineBorder', b2)
    if hasattr(b2, 'gmfgraph_Color75'):
        assert not _is_linked(b2, 'gmfgraph_Color75', a)
...
def test_assoc_customChildren72_link_reassign_clear():
    a = gmfgraph_FigureAccessor(accessor="sample_text")
    b1 = gmfgraph_CustomFigure()
    b2 = gmfgraph_CustomFigure()
    _safe_set(a, 'gmfgraph_FigureAccessor73', b1)
    assert _is_linked(a, 'gmfgraph_FigureAccessor73', b1)
    if hasattr(b1, 'gmfgraph_CustomFigure'):
        assert _is_linked(b1, 'gmfgraph_CustomFigure', a)
    _safe_set(a, 'gmfgraph_FigureAccessor73', b2)
    assert _is_linked(a, 'gmfgraph_FigureAccessor73', b2)
    if hasattr(b1, 'gmfgraph_CustomFigure'):
    ...
    _safe_set(a, 'gmfgraph_FigureAccessor73', None)
    assert not _is_linked(a, 'gmfgraph_FigureAccessor73', b2)
    if hasattr(b2, 'gmfgraph_CustomFigure'):
        assert not _is_linked(b2, 'gmfgraph_CustomFigure', a)


def test_assoc_defaultSize101_link_reassign_clear():
    a = gmfgraph_Dimension(dx=7, dy=7)
    b1 = gmfgraph_DefaultSizeFacet()
    b2 = gmfgraph_DefaultSizeFacet()
    _safe_set(a, 'gmfgraph_Dimension102', b1)
    assert _is_linked(a, 'gmfgraph_Dimension102', b1)
    if hasattr(b1, 'gmfgraph_DefaultSizeFacet'):
        assert _is_linked(b1, 'gmfgraph_DefaultSizeFacet', a)
    _safe_set(a, 'gmfgraph_Dimension102', b2)
    assert _is_linked(a, 'gmfgraph_Dimension102', b2)
    if hasattr(b1, 'gmfgraph_DefaultSizeFacet'):
    ...
    _safe_set(a, 'gmfgraph_Dimension102', None)
    assert not _is_linked(a, 'gmfgraph_Dimension102', b2)
    if hasattr(b2, 'gmfgraph_DefaultSizeFacet'):
...
def test_assoc_figure112_link_reassign_clear():
    a = gmfgraph_ChildAccess(accessor="sample_text")
    b1 = gmfgraph_Figure()
    b2 = gmfgraph_Figure()
    _safe_set(a, 'gmfgraph_ChildAccess113', b1)
    assert _is_linked(a, 'gmfgraph_ChildAccess113', b1)
    if hasattr(b1, 'gmfgraph_Figure114'):
        assert _is_linked(b1, 'gmfgraph_Figure114', a)
    _safe_set(a, 'gmfgraph_ChildAccess113', b2)
    assert _is_linked(a, 'gmfgraph_ChildAccess113', b2)
    if hasattr(b1, 'gmfgraph_Figure114'):
        assert not _is_linked(b1, 'gmfgraph_Figure114', a)
    if hasattr(b2, 'gmfgraph_Figure114'):
        assert _is_linked(b2, 'gmfgraph_Figure114', a)
    _safe_set(a, 'gmfgraph_ChildAccess113', None)
    assert not _is_linked(a, 'gmfgraph_ChildAccess113', b2)
    if hasattr(b2, 'gmfgraph_Figure114'):
        assert not _is_linked(b2, 'gmfgraph_Figure114', a)
...
def test_assoc_figure59_link_reassign_clear():
    ...
    _safe_set(a, 'gmfgraph_RealFigure60', b1)
    assert _is_linked(a, 'gmfgraph_RealFigure60', b1)
    if hasattr(b1, 'gmfgraph_FigureRef'):
        assert _is_linked(b1, 'gmfgraph_FigureRef', a)
    _safe_set(a, 'gmfgraph_RealFigure60', b2)
    assert _is_linked(a, 'gmfgraph_RealFigure60', b2)
    if hasattr(b1, 'gmfgraph_FigureRef'):
        assert not _is_linked(b1, 'gmfgraph_FigureRef', a)
    if hasattr(b2, 'gmfgraph_FigureRef'):
        assert _is_linked(b2, 'gmfgraph_FigureRef', a)
    _safe_set(a, 'gmfgraph_RealFigure60', None)
    assert not _is_linked(a, 'gmfgraph_RealFigure60', b2)
    if hasattr(b2, 'gmfgraph_FigureRef'):
...
def test_assoc_insets46_link_reassign_clear():
...
def test_assoc_insets76_link_reassign_clear():
    a = gmfgraph_Insets(bottom=7, left=7, right=7, top=7)
    b1 = gmfgraph_MarginBorder()
    b2 = gmfgraph_MarginBorder()
    _safe_set(a, 'gmfgraph_Insets77', b1)
    assert _is_linked(a, 'gmfgraph_Insets77', b1)
    if hasattr(b1, 'gmfgraph_MarginBorder'):
        assert _is_linked(b1, 'gmfgraph_MarginBorder', a)
    _safe_set(a, 'gmfgraph_Insets77', b2)
    assert _is_linked(a, 'gmfgraph_Insets77', b2)
    if hasattr(b1, 'gmfgraph_MarginBorder'):
    ...
    _safe_set(a, 'gmfgraph_Insets77', None)
    assert not _is_linked(a, 'gmfgraph_Insets77', b2)
    if hasattr(b2, 'gmfgraph_MarginBorder'):
...
def test_assoc_margins89_link_reassign_clear():
    ...
    if hasattr(b1, 'gmfgraph_Dimension90'):
        assert _is_linked(b1, 'gmfgraph_Dimension90', a)
    _safe_set(a, 'gmfgraph_GridLayout', b2)
    assert _is_linked(a, 'gmfgraph_GridLayout', b2)
    if hasattr(b1, 'gmfgraph_Dimension90'):
        assert not _is_linked(b1, 'gmfgraph_Dimension90', a)
    if hasattr(b2, 'gmfgraph_Dimension90'):
        assert _is_linked(b2, 'gmfgraph_Dimension90', a)
    _safe_set(a, 'gmfgraph_GridLayout', None)
    assert not _is_linked(a, 'gmfgraph_GridLayout', b2)
    if hasattr(b2, 'gmfgraph_Dimension90'):
        assert not _is_linked(b2, 'gmfgraph_Dimension90', a)
...
def test_assoc_minimumSize38_link_reassign_clear():
    a = gmfgraph_Dimension(dx=7, dy=7)
    b1 = gmfgraph_Figure()
    b2 = gmfgraph_Figure()
    _safe_set(a, 'gmfgraph_Dimension40', b1)
    assert _is_linked(a, 'gmfgraph_Dimension40', b1)
    if hasattr(b1, 'gmfgraph_Figure39'):
        assert _is_linked(b1, 'gmfgraph_Figure39', a)
    _safe_set(a, 'gmfgraph_Dimension40', b2)
    assert _is_linked(a, 'gmfgraph_Dimension40', b2)
    if hasattr(b1, 'gmfgraph_Figure39'):
        assert not _is_linked(b1, 'gmfgraph_Figure39', a)
    if hasattr(b2, 'gmfgraph_Figure39'):
        assert _is_linked(b2, 'gmfgraph_Figure39', a)
    _safe_set(a, 'gmfgraph_Dimension40', None)
    assert not _is_linked(a, 'gmfgraph_Dimension40', b2)
    if hasattr(b2, 'gmfgraph_Figure39'):
        assert not _is_linked(b2, 'gmfgraph_Figure39', a)
...
def test_assoc_preferredSize41_link_reassign_clear():
    a = gmfgraph_Dimension(dx=7, dy=7)
    b1 = gmfgraph_Figure()
    b2 = gmfgraph_Figure()
    _safe_set(a, 'gmfgraph_Dimension43', b1)
    assert _is_linked(a, 'gmfgraph_Dimension43', b1)
    if hasattr(b1, 'gmfgraph_Figure42'):
        assert _is_linked(b1, 'gmfgraph_Figure42', a)
    _safe_set(a, 'gmfgraph_Dimension43', b2)
    assert _is_linked(a, 'gmfgraph_Dimension43', b2)
    if hasattr(b1, 'gmfgraph_Figure42'):
        assert not _is_linked(b1, 'gmfgraph_Figure42', a)
    if hasattr(b2, 'gmfgraph_Figure42'):
        assert _is_linked(b2, 'gmfgraph_Figure42', a)
    _safe_set(a, 'gmfgraph_Dimension43', None)
    assert not _is_linked(a, 'gmfgraph_Dimension43', b2)
    if hasattr(b2, 'gmfgraph_Figure42'):
        assert not _is_linked(b2, 'gmfgraph_Figure42', a)


def test_assoc_resolvedChildren61_link_reassign_clear():
    ...
    if hasattr(b1, 'gmfgraph_Figure62'):
        assert _is_linked(b1, 'gmfgraph_Figure62', a)
    _safe_set(a, 'gmfgraph_Shape', {b2})
    assert _is_linked(a, 'gmfgraph_Shape', b2)
    if hasattr(b1, 'gmfgraph_Figure62'):
        assert not _is_linked(b1, 'gmfgraph_Figure62', a)
    if hasattr(b2, 'gmfgraph_Figure62'):
        assert _is_linked(b2, 'gmfgraph_Figure62', a)
    _safe_set(a, 'gmfgraph_Shape', set())
    assert not _is_linked(a, 'gmfgraph_Shape', b2)
    if hasattr(b2, 'gmfgraph_Figure62'):
        assert not _is_linked(b2, 'gmfgraph_Figure62', a)
...
def test_assoc_size98_link_reassign_clear():
    a = gmfgraph_Dimension(dx=7, dy=7)
    b1 = gmfgraph_XYLayoutData()
    b2 = gmfgraph_XYLayoutData()
    _safe_set(a, 'gmfgraph_Dimension100', b1)
    assert _is_linked(a, 'gmfgraph_Dimension100', b1)
    if hasattr(b1, 'gmfgraph_XYLayoutData99'):
        assert _is_linked(b1, 'gmfgraph_XYLayoutData99', a)
    _safe_set(a, 'gmfgraph_Dimension100', b2)
    assert _is_linked(a, 'gmfgraph_Dimension100', b2)
    if hasattr(b1, 'gmfgraph_XYLayoutData99'):
        assert not _is_linked(b1, 'gmfgraph_XYLayoutData99', a)
    if hasattr(b2, 'gmfgraph_XYLayoutData99'):
        assert _is_linked(b2, 'gmfgraph_XYLayoutData99', a)
    _safe_set(a, 'gmfgraph_Dimension100', None)
    assert not _is_linked(a, 'gmfgraph_Dimension100', b2)
    if hasattr(b2, 'gmfgraph_XYLayoutData99'):
        assert not _is_linked(b2, 'gmfgraph_XYLayoutData99', a)


def test_assoc_sizeHint84_link_reassign_clear():
    ...
    if hasattr(b1, 'gmfgraph_Dimension85'):
        assert _is_linked(b1, 'gmfgraph_Dimension85', a)
    _safe_set(a, 'gmfgraph_GridLayoutData', b2)
    assert _is_linked(a, 'gmfgraph_GridLayoutData', b2)
    if hasattr(b1, 'gmfgraph_Dimension85'):
        assert not _is_linked(b1, 'gmfgraph_Dimension85', a)
    if hasattr(b2, 'gmfgraph_Dimension85'):
        assert _is_linked(b2, 'gmfgraph_Dimension85', a)
    _safe_set(a, 'gmfgraph_GridLayoutData', None)
    assert not _is_linked(a, 'gmfgraph_GridLayoutData', b2)
    if hasattr(b2, 'gmfgraph_Dimension85'):
        assert not _is_linked(b2, 'gmfgraph_Dimension85', a)


def test_assoc_spacing91_link_reassign_clear():
    a = gmfgraph_GridLayout(equalWidth=True, numColumns=7)
    b1 = gmfgraph_Dimension(dx=7, dy=7)
    b2 = gmfgraph_Dimension(dx=13, dy=13)
    _safe_set(a, 'gmfgraph_GridLayout92', b1)
    assert _is_linked(a, 'gmfgraph_GridLayout92', b1)
    if hasattr(b1, 'gmfgraph_Dimension93'):
        assert _is_linked(b1, 'gmfgraph_Dimension93', a)
    _safe_set(a, 'gmfgraph_GridLayout92', b2)
    assert _is_linked(a, 'gmfgraph_GridLayout92', b2)
    if hasattr(b1, 'gmfgraph_Dimension93'):
    ...
    _safe_set(a, 'gmfgraph_GridLayout92', None)
    assert not _is_linked(a, 'gmfgraph_GridLayout92', b2)
    if hasattr(b2, 'gmfgraph_Dimension93'):
        assert not _is_linked(b2, 'gmfgraph_Dimension93', a)


def test_assoc_spacing94_link_reassign_clear():
    a = gmfgraph_Dimension(dx=7, dy=7)
    b1 = gmfgraph_BorderLayout()
    b2 = gmfgraph_BorderLayout()
    _safe_set(a, 'gmfgraph_Dimension95', b1)
    assert _is_linked(a, 'gmfgraph_Dimension95', b1)
    if hasattr(b1, 'gmfgraph_BorderLayout'):
        assert _is_linked(b1, 'gmfgraph_BorderLayout', a)
    _safe_set(a, 'gmfgraph_Dimension95', b2)
    assert _is_linked(a, 'gmfgraph_Dimension95', b2)
    if hasattr(b1, 'gmfgraph_BorderLayout'):
    ...
    _safe_set(a, 'gmfgraph_Dimension95', None)
    assert not _is_linked(a, 'gmfgraph_Dimension95', b2)
    if hasattr(b2, 'gmfgraph_BorderLayout'):
        assert not _is_linked(b2, 'gmfgraph_BorderLayout', a)


def test_assoc_template63_link_reassign_clear():
    a = gmfgraph_Point(x=7, y=7)
    b1 = gmfgraph_Polyline()
    b2 = gmfgraph_Polyline()
    _safe_set(a, 'gmfgraph_Point64', b1)
    assert _is_linked(a, 'gmfgraph_Point64', b1)
    if hasattr(b1, 'gmfgraph_Polyline'):
        assert _is_linked(b1, 'gmfgraph_Polyline', a)
    _safe_set(a, 'gmfgraph_Point64', b2)
    assert _is_linked(a, 'gmfgraph_Point64', b2)
    if hasattr(b1, 'gmfgraph_Polyline'):
        assert not _is_linked(b1, 'gmfgraph_Polyline', a)
    if hasattr(b2, 'gmfgraph_Polyline'):
        assert _is_linked(b2, 'gmfgraph_Polyline', a)
    _safe_set(a, 'gmfgraph_Point64', None)
    assert not _is_linked(a, 'gmfgraph_Point64', b2)
    if hasattr(b2, 'gmfgraph_Polyline'):
        assert not _is_linked(b2, 'gmfgraph_Polyline', a)


def test_assoc_topLeft96_link_reassign_clear():
    a = gmfgraph_Point(x=7, y=7)
    b1 = gmfgraph_XYLayoutData()
    b2 = gmfgraph_XYLayoutData()
    _safe_set(a, 'gmfgraph_Point97', b1)
    assert _is_linked(a, 'gmfgraph_Point97', b1)
    if hasattr(b1, 'gmfgraph_XYLayoutData'):
        assert _is_linked(b1, 'gmfgraph_XYLayoutData', a)
    _safe_set(a, 'gmfgraph_Point97', b2)
    assert _is_linked(a, 'gmfgraph_Point97', b2)
    if hasattr(b1, 'gmfgraph_XYLayoutData'):
    ...
    _safe_set(a, 'gmfgraph_Point97', None)
    assert not _is_linked(a, 'gmfgraph_Point97', b2)
    if hasattr(b2, 'gmfgraph_XYLayoutData'):
        assert not _is_linked(b2, 'gmfgraph_XYLayoutData', a)


def test_assoc_typedFigure70_link_reassign_clear():
    a = gmfgraph_RealFigure(name="sample_text")
    b1 = gmfgraph_FigureAccessor(accessor="sample_text")
    b2 = gmfgraph_FigureAccessor(accessor="sample_text_2")
    _safe_set(a, 'gmfgraph_RealFigure71', b1)
    assert _is_linked(a, 'gmfgraph_RealFigure71', b1)
    if hasattr(b1, 'gmfgraph_FigureAccessor'):
        assert _is_linked(b1, 'gmfgraph_FigureAccessor', a)
    _safe_set(a, 'gmfgraph_RealFigure71', b2)
    assert _is_linked(a, 'gmfgraph_RealFigure71', b2)
    if hasattr(b1, 'gmfgraph_FigureAccessor'):
    ...
    _safe_set(a, 'gmfgraph_RealFigure71', None)
    assert not _is_linked(a, 'gmfgraph_RealFigure71', b2)
    if hasattr(b2, 'gmfgraph_FigureAccessor'):
