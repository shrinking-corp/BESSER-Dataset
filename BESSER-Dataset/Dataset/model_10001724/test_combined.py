...
def test_assoc_ShoppinCart_ShoppinCart_link_reassign_clear():
    a = ShoppinCart(creationDate=date(2024, 1, 1))
    b1 = ShoppinCart(creationDate=date(2024, 1, 1))
    b2 = ShoppinCart(creationDate=date(2025, 6, 15))
    _safe_set(a, 'shoppinCart20', b1)
    assert _is_linked(a, 'shoppinCart20', b1)
    if hasattr(b1, 'shoppinCart21'):
        assert _is_linked(b1, 'shoppinCart21', a)
    _safe_set(a, 'shoppinCart20', b2)
    assert _is_linked(a, 'shoppinCart20', b2)
    if hasattr(b1, 'shoppinCart21'):
        assert not _is_linked(b1, 'shoppinCart21', a)
    if hasattr(b2, 'shoppinCart21'):
        assert _is_linked(b2, 'shoppinCart21', a)
    _safe_set(a, 'shoppinCart20', None)
    assert not _is_linked(a, 'shoppinCart20', b2)
    if hasattr(b2, 'shoppinCart21'):
        assert not _is_linked(b2, 'shoppinCart21', a)
