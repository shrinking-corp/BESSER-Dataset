...
def test_assoc_association3_link_reassign_clear():
    ...
    b1 = UML2_Association()
    b2 = UML2_Association()
    _safe_set(a, 'UML2_Property4', b1)
    assert _is_linked(a, 'UML2_Property4', b1)
    if hasattr(b1, 'UML2_Association'):
        assert _is_linked(b1, 'UML2_Association', a)
    _safe_set(a, 'UML2_Property4', b2)
    assert _is_linked(a, 'UML2_Property4', b2)
    if hasattr(b1, 'UML2_Association'):
        assert not _is_linked(b1, 'UML2_Association', a)
    if hasattr(b2, 'UML2_Association'):
        assert _is_linked(b2, 'UML2_Association', a)
    _safe_set(a, 'UML2_Property4', None)
    assert not _is_linked(a, 'UML2_Property4', b2)
    if hasattr(b2, 'UML2_Association'):
        assert not _is_linked(b2, 'UML2_Association', a)


def test_assoc_behavior42_link_reassign_clear():
    ...
    if hasattr(b1, 'UML2_Behavior43'):
        assert _is_linked(b1, 'UML2_Behavior43', a)
    _safe_set(a, 'UML2_OpaqueExpression', b2)
    assert _is_linked(a, 'UML2_OpaqueExpression', b2)
    if hasattr(b1, 'UML2_Behavior43'):
        assert not _is_linked(b1, 'UML2_Behavior43', a)
    if hasattr(b2, 'UML2_Behavior43'):
        assert _is_linked(b2, 'UML2_Behavior43', a)
    _safe_set(a, 'UML2_OpaqueExpression', None)
    assert not _is_linked(a, 'UML2_OpaqueExpression', b2)
    if hasattr(b2, 'UML2_Behavior43'):
        assert not _is_linked(b2, 'UML2_Behavior43', a)


def test_assoc_bodyCondition32_link_reassign_clear():
    a = UML2_Operation(isQuery=True)
    b1 = UML2_Constraint()
    b2 = UML2_Constraint()
    _safe_set(a, 'UML2_Operation33', b1)
    assert _is_linked(a, 'UML2_Operation33', b1)
    if hasattr(b1, 'UML2_Constraint'):
        assert _is_linked(b1, 'UML2_Constraint', a)
    _safe_set(a, 'UML2_Operation33', b2)
    assert _is_linked(a, 'UML2_Operation33', b2)
    if hasattr(b1, 'UML2_Constraint'):
        assert not _is_linked(b1, 'UML2_Constraint', a)
    if hasattr(b2, 'UML2_Constraint'):
        assert _is_linked(b2, 'UML2_Constraint', a)
    _safe_set(a, 'UML2_Operation33', None)
    assert not _is_linked(a, 'UML2_Operation33', b2)
    if hasattr(b2, 'UML2_Constraint'):
        assert not _is_linked(b2, 'UML2_Constraint', a)


def test_assoc_classifier45_link_reassign_clear():
    a = UML2_Classifier(isAbstract=True)
    b1 = UML2_InstanceSpecification()
    b2 = UML2_InstanceSpecification()
    _safe_set(a, 'UML2_Classifier47', b1)
    assert _is_linked(a, 'UML2_Classifier47', b1)
    if hasattr(b1, 'UML2_InstanceSpecification46'):
        assert _is_linked(b1, 'UML2_InstanceSpecification46', a)
    _safe_set(a, 'UML2_Classifier47', b2)
    assert _is_linked(a, 'UML2_Classifier47', b2)
    if hasattr(b1, 'UML2_InstanceSpecification46'):
        assert not _is_linked(b1, 'UML2_InstanceSpecification46', a)
    if hasattr(b2, 'UML2_InstanceSpecification46'):
        assert _is_linked(b2, 'UML2_InstanceSpecification46', a)
    _safe_set(a, 'UML2_Classifier47', None)
    assert not _is_linked(a, 'UML2_Classifier47', b2)
    if hasattr(b2, 'UML2_InstanceSpecification46'):
        assert not _is_linked(b2, 'UML2_InstanceSpecification46', a)


def test_assoc_classifier57_link_reassign_clear():
    a = UML2_Classifier(isAbstract=True)
    b1 = UML2_CreateObjectAction()
    b2 = UML2_CreateObjectAction()
    _safe_set(a, 'UML2_Classifier58', b1)
    assert _is_linked(a, 'UML2_Classifier58', b1)
    if hasattr(b1, 'UML2_CreateObjectAction'):
        assert _is_linked(b1, 'UML2_CreateObjectAction', a)
    _safe_set(a, 'UML2_Classifier58', b2)
    assert _is_linked(a, 'UML2_Classifier58', b2)
    if hasattr(b1, 'UML2_CreateObjectAction'):
    ...
    _safe_set(a, 'UML2_Classifier58', None)
    assert not _is_linked(a, 'UML2_Classifier58', b2)
    if hasattr(b2, 'UML2_CreateObjectAction'):
        assert not _is_linked(b2, 'UML2_CreateObjectAction', a)


def test_assoc_context_81_link_reassign_clear():
    a = UML2_Classifier(isAbstract=True)
    b1 = UML2_Action()
    b2 = UML2_Action()
    _safe_set(a, 'UML2_Classifier82', b1)
    assert _is_linked(a, 'UML2_Classifier82', b1)
    if hasattr(b1, 'UML2_Action'):
        assert _is_linked(b1, 'UML2_Action', a)
    _safe_set(a, 'UML2_Classifier82', b2)
    assert _is_linked(a, 'UML2_Classifier82', b2)
    if hasattr(b1, 'UML2_Action'):
        assert not _is_linked(b1, 'UML2_Action', a)
    if hasattr(b2, 'UML2_Action'):
        assert _is_linked(b2, 'UML2_Action', a)
    _safe_set(a, 'UML2_Classifier82', None)
    assert not _is_linked(a, 'UML2_Classifier82', b2)
    if hasattr(b2, 'UML2_Action'):
        assert not _is_linked(b2, 'UML2_Action', a)


def test_assoc_definingFeature48_link_reassign_clear():
    ...
    if hasattr(b1, 'UML2_Slot49'):
        assert _is_linked(b1, 'UML2_Slot49', a)
    _safe_set(a, 'UML2_StructuralFeature', b2)
    assert _is_linked(a, 'UML2_StructuralFeature', b2)
    if hasattr(b1, 'UML2_Slot49'):
        assert not _is_linked(b1, 'UML2_Slot49', a)
    if hasattr(b2, 'UML2_Slot49'):
        assert _is_linked(b2, 'UML2_Slot49', a)
    _safe_set(a, 'UML2_StructuralFeature', None)
    assert not _is_linked(a, 'UML2_StructuralFeature', b2)
    if hasattr(b2, 'UML2_Slot49'):
        assert not _is_linked(b2, 'UML2_Slot49', a)
...
def test_assoc_end20_link_reassign_clear():
    ...
    b1 = UML2_LinkEndData()
    b2 = UML2_LinkEndData()
    _safe_set(a, 'UML2_Property22', b1)
    assert _is_linked(a, 'UML2_Property22', b1)
    if hasattr(b1, 'UML2_LinkEndData21'):
        assert _is_linked(b1, 'UML2_LinkEndData21', a)
    _safe_set(a, 'UML2_Property22', b2)
    assert _is_linked(a, 'UML2_Property22', b2)
    if hasattr(b1, 'UML2_LinkEndData21'):
        assert not _is_linked(b1, 'UML2_LinkEndData21', a)
    if hasattr(b2, 'UML2_LinkEndData21'):
        assert _is_linked(b2, 'UML2_LinkEndData21', a)
    _safe_set(a, 'UML2_Property22', None)
    assert not _is_linked(a, 'UML2_Property22', b2)
    if hasattr(b2, 'UML2_LinkEndData21'):
        assert not _is_linked(b2, 'UML2_LinkEndData21', a)
...
def test_assoc_feature68_link_reassign_clear():
    ...
    _safe_set(a, 'UML2_Feature70', b1)
    assert _is_linked(a, 'UML2_Feature70', b1)
    if hasattr(b1, 'UML2_Classifier69'):
        assert _is_linked(b1, 'UML2_Classifier69', a)
    _safe_set(a, 'UML2_Feature70', b2)
    assert _is_linked(a, 'UML2_Feature70', b2)
    if hasattr(b1, 'UML2_Classifier69'):
        assert not _is_linked(b1, 'UML2_Classifier69', a)
    if hasattr(b2, 'UML2_Classifier69'):
        assert _is_linked(b2, 'UML2_Classifier69', a)
    _safe_set(a, 'UML2_Feature70', None)
    assert not _is_linked(a, 'UML2_Feature70', b2)
    if hasattr(b2, 'UML2_Classifier69'):
        assert not _is_linked(b2, 'UML2_Classifier69', a)
...
def test_assoc_featuringClassifier65_link_reassign_clear():
    ...
    if hasattr(b1, 'UML2_Classifier66'):
        assert _is_linked(b1, 'UML2_Classifier66', a)
    _safe_set(a, 'UML2_Feature', {b2})
    assert _is_linked(a, 'UML2_Feature', b2)
    if hasattr(b1, 'UML2_Classifier66'):
        assert not _is_linked(b1, 'UML2_Classifier66', a)
    if hasattr(b2, 'UML2_Classifier66'):
        assert _is_linked(b2, 'UML2_Classifier66', a)
    _safe_set(a, 'UML2_Feature', set())
    assert not _is_linked(a, 'UML2_Feature', b2)
    if hasattr(b2, 'UML2_Classifier66'):
        assert not _is_linked(b2, 'UML2_Classifier66', a)


def test_assoc_general29_link_reassign_clear():
    a = UML2_Classifier(isAbstract=True)
    b1 = UML2_Generalization()
    b2 = UML2_Generalization()
    _safe_set(a, 'UML2_Classifier', b1)
    assert _is_linked(a, 'UML2_Classifier', b1)
    if hasattr(b1, 'UML2_Generalization30'):
        assert _is_linked(b1, 'UML2_Generalization30', a)
    _safe_set(a, 'UML2_Classifier', b2)
    assert _is_linked(a, 'UML2_Classifier', b2)
    if hasattr(b1, 'UML2_Generalization30'):
        assert not _is_linked(b1, 'UML2_Generalization30', a)
    if hasattr(b2, 'UML2_Generalization30'):
        assert _is_linked(b2, 'UML2_Generalization30', a)
    _safe_set(a, 'UML2_Classifier', None)
    assert not _is_linked(a, 'UML2_Classifier', b2)
    if hasattr(b2, 'UML2_Generalization30'):
        assert not _is_linked(b2, 'UML2_Generalization30', a)


def test_assoc_generalization74_link_reassign_clear():
    a = UML2_Classifier(isAbstract=True)
    b1 = UML2_Generalization()
    b2 = UML2_Generalization()
    _safe_set(a, 'UML2_Classifier75', {b1})
    assert _is_linked(a, 'UML2_Classifier75', b1)
    if hasattr(b1, 'UML2_Generalization76'):
        assert _is_linked(b1, 'UML2_Generalization76', a)
    _safe_set(a, 'UML2_Classifier75', {b2})
    assert _is_linked(a, 'UML2_Classifier75', b2)
    if hasattr(b1, 'UML2_Generalization76'):
        assert not _is_linked(b1, 'UML2_Generalization76', a)
    if hasattr(b2, 'UML2_Generalization76'):
        assert _is_linked(b2, 'UML2_Generalization76', a)
    _safe_set(a, 'UML2_Classifier75', set())
    assert not _is_linked(a, 'UML2_Classifier75', b2)
    if hasattr(b2, 'UML2_Generalization76'):
        assert not _is_linked(b2, 'UML2_Generalization76', a)


def test_assoc_importedElement83_link_reassign_clear():
...
def test_assoc_inheritedMember71_link_reassign_clear():
    a = UML2_NamedElement(name="sample_text", visibility="sample_text")
    b1 = UML2_Classifier(isAbstract=True)
    b2 = UML2_Classifier(isAbstract=False)
    _safe_set(a, 'UML2_NamedElement73', b1)
    assert _is_linked(a, 'UML2_NamedElement73', b1)
    if hasattr(b1, 'UML2_Classifier72'):
        assert _is_linked(b1, 'UML2_Classifier72', a)
    _safe_set(a, 'UML2_NamedElement73', b2)
    assert _is_linked(a, 'UML2_NamedElement73', b2)
    if hasattr(b1, 'UML2_Classifier72'):
        assert not _is_linked(b1, 'UML2_Classifier72', a)
    if hasattr(b2, 'UML2_Classifier72'):
        assert _is_linked(b2, 'UML2_Classifier72', a)
    _safe_set(a, 'UML2_NamedElement73', None)
    assert not _is_linked(a, 'UML2_NamedElement73', b2)
    if hasattr(b2, 'UML2_Classifier72'):
        assert not _is_linked(b2, 'UML2_Classifier72', a)
...
def test_assoc_memberEnd39_link_reassign_clear():
    a = UML2_Property(aggregation="sample_text", isComposite=True, isDerived=True, isDerivedUnion=True)
    b1 = UML2_Association()
    b2 = UML2_Association()
    _safe_set(a, 'UML2_Property41', b1)
    assert _is_linked(a, 'UML2_Property41', b1)
    if hasattr(b1, 'UML2_Association40'):
        assert _is_linked(b1, 'UML2_Association40', a)
    _safe_set(a, 'UML2_Property41', b2)
    assert _is_linked(a, 'UML2_Property41', b2)
    if hasattr(b1, 'UML2_Association40'):
        assert not _is_linked(b1, 'UML2_Association40', a)
    if hasattr(b2, 'UML2_Association40'):
        assert _is_linked(b2, 'UML2_Association40', a)
    _safe_set(a, 'UML2_Property41', None)
    assert not _is_linked(a, 'UML2_Property41', b2)
    if hasattr(b2, 'UML2_Association40'):
        assert not _is_linked(b2, 'UML2_Association40', a)


def test_assoc_nestedClassifier77_link_reassign_clear():
    a = UML2_Classifier(isAbstract=True)
    b1 = UML2_Class(isActive=True)
    b2 = UML2_Class(isActive=False)
    _safe_set(a, 'UML2_Classifier78', b1)
    assert _is_linked(a, 'UML2_Classifier78', b1)
    if hasattr(b1, 'UML2_Class'):
        assert _is_linked(b1, 'UML2_Class', a)
    _safe_set(a, 'UML2_Classifier78', b2)
    assert _is_linked(a, 'UML2_Classifier78', b2)
    if hasattr(b1, 'UML2_Class'):
        assert not _is_linked(b1, 'UML2_Class', a)
    if hasattr(b2, 'UML2_Class'):
        assert _is_linked(b2, 'UML2_Class', a)
    _safe_set(a, 'UML2_Classifier78', None)
    assert not _is_linked(a, 'UML2_Classifier78', b2)
    if hasattr(b2, 'UML2_Class'):
        assert not _is_linked(b2, 'UML2_Class', a)


def test_assoc_ownedEnd36_link_reassign_clear():
    a = UML2_Property(aggregation="sample_text", isComposite=True, isDerived=True, isDerivedUnion=True)
    b1 = UML2_Association()
    b2 = UML2_Association()
    _safe_set(a, 'UML2_Property38', b1)
    assert _is_linked(a, 'UML2_Property38', b1)
    if hasattr(b1, 'UML2_Association37'):
        assert _is_linked(b1, 'UML2_Association37', a)
    _safe_set(a, 'UML2_Property38', b2)
    assert _is_linked(a, 'UML2_Property38', b2)
    if hasattr(b1, 'UML2_Association37'):
        assert not _is_linked(b1, 'UML2_Association37', a)
    if hasattr(b2, 'UML2_Association37'):
        assert _is_linked(b2, 'UML2_Association37', a)
    _safe_set(a, 'UML2_Property38', None)
    assert not _is_linked(a, 'UML2_Property38', b2)
    if hasattr(b2, 'UML2_Association37'):
        assert not _is_linked(b2, 'UML2_Association37', a)


def test_assoc_ownedParameter31_link_reassign_clear():
...
def test_assoc_ownedReception79_link_reassign_clear():
    a = UML2_Class(isActive=True)
    b1 = UML2_Reception()
    b2 = UML2_Reception()
    _safe_set(a, 'UML2_Class80', {b1})
    assert _is_linked(a, 'UML2_Class80', b1)
    if hasattr(b1, 'UML2_Reception'):
        assert _is_linked(b1, 'UML2_Reception', a)
    _safe_set(a, 'UML2_Class80', {b2})
    assert _is_linked(a, 'UML2_Class80', b2)
    if hasattr(b1, 'UML2_Reception'):
        assert not _is_linked(b1, 'UML2_Reception', a)
    if hasattr(b2, 'UML2_Reception'):
        assert _is_linked(b2, 'UML2_Reception', a)
    _safe_set(a, 'UML2_Class80', set())
    assert not _is_linked(a, 'UML2_Class80', b2)
    if hasattr(b2, 'UML2_Reception'):
...
def test_assoc_qualifier6_link_reassign_clear():
    a = UML2_Property(aggregation="sample_text", isComposite=True, isDerived=True, isDerivedUnion=True)
    ...
    _safe_set(a, 'UML2_Property5', {b1})
    assert _is_linked(a, 'UML2_Property5', b1)
    if hasattr(b1, 'UML2_Property7'):
        assert _is_linked(b1, 'UML2_Property7', a)
    _safe_set(a, 'UML2_Property5', {b2})
    assert _is_linked(a, 'UML2_Property5', b2)
    if hasattr(b1, 'UML2_Property7'):
        assert not _is_linked(b1, 'UML2_Property7', a)
    if hasattr(b2, 'UML2_Property7'):
        assert _is_linked(b2, 'UML2_Property7', a)
    _safe_set(a, 'UML2_Property5', set())
    assert not _is_linked(a, 'UML2_Property5', b2)
    if hasattr(b2, 'UML2_Property7'):
        assert not _is_linked(b2, 'UML2_Property7', a)


def test_assoc_structuralFeature84_link_reassign_clear():
    a = UML2_StructuralFeature(isReadOnly=True)
    b1 = UML2_StructuralFeatureAction()
    b2 = UML2_StructuralFeatureAction()
    _safe_set(a, 'UML2_StructuralFeature85', b1)
    assert _is_linked(a, 'UML2_StructuralFeature85', b1)
    if hasattr(b1, 'UML2_StructuralFeatureAction'):
        assert _is_linked(b1, 'UML2_StructuralFeatureAction', a)
    _safe_set(a, 'UML2_StructuralFeature85', b2)
    assert _is_linked(a, 'UML2_StructuralFeature85', b2)
    if hasattr(b1, 'UML2_StructuralFeatureAction'):
    ...
    _safe_set(a, 'UML2_StructuralFeature85', None)
    assert not _is_linked(a, 'UML2_StructuralFeature85', b2)
    if hasattr(b2, 'UML2_StructuralFeatureAction'):
        assert not _is_linked(b2, 'UML2_StructuralFeatureAction', a)


def test_assoc_subsettedProperty2_link_reassign_clear():
    a = UML2_Property(aggregation="sample_text", isComposite=True, isDerived=True, isDerivedUnion=True)
    b1 = UML2_Property(aggregation="sample_text", isComposite=True, isDerived=True, isDerivedUnion=True)
    b2 = UML2_Property(aggregation="sample_text_2", isComposite=False, isDerived=False, isDerivedUnion=False)
    _safe_set(a, 'UML2_Property', b1)
    assert _is_linked(a, 'UML2_Property', b1)
    if hasattr(b1, 'UML2_Property1'):
        assert _is_linked(b1, 'UML2_Property1', a)
    _safe_set(a, 'UML2_Property', b2)
    assert _is_linked(a, 'UML2_Property', b2)
    if hasattr(b1, 'UML2_Property1'):
        assert not _is_linked(b1, 'UML2_Property1', a)
    if hasattr(b2, 'UML2_Property1'):
        assert _is_linked(b2, 'UML2_Property1', a)
    _safe_set(a, 'UML2_Property', None)
    assert not _is_linked(a, 'UML2_Property', b2)
    if hasattr(b2, 'UML2_Property1'):
        assert not _is_linked(b2, 'UML2_Property1', a)
