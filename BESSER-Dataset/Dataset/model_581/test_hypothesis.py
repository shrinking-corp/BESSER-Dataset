...
def test_assistantmvc_style_is_not_abstract():
    assert not inspect.isabstract(AssistantMVC_Style)


def test_assistantmvc_style_constructor_exists():
    assert callable(AssistantMVC_Style.__init__)


def test_assistantmvc_style_constructor_args():
    sig = inspect.signature(AssistantMVC_Style.__init__)
    params = list(sig.parameters.keys())
    assert "type" in params, "Missing parameter 'type'"
    assert "age" in params, "Missing parameter 'age'"

def test_assistantmvc_style_has_type():
    assert hasattr(AssistantMVC_Style, "type")
    descriptor = None
    for klass in AssistantMVC_Style.__mro__:
        if "type" in klass.__dict__:
            descriptor = klass.__dict__["type"]
            break
    assert isinstance(descriptor, property)

def test_assistantmvc_style_has_age():
    assert hasattr(AssistantMVC_Style, "age")
    descriptor = None
    for klass in AssistantMVC_Style.__mro__:
...
@given(instance=AssistantMVC_Style_strategy)
def test_assistantmvc_style_type_setter(instance):
...
@given(instance=AssistantMVC_Style_strategy)
def test_assistantmvc_style_age_setter(instance):
