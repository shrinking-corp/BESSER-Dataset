...
def test_sport_is_not_abstract():
    assert not inspect.isabstract(Sport)


def test_sport_constructor_exists():
    assert callable(Sport.__init__)


def test_sport_constructor_args():
    sig = inspect.signature(Sport.__init__)
    params = list(sig.parameters.keys())
    assert "id" in params, "Missing parameter 'id'"
    assert "nom" in params, "Missing parameter 'nom'"

def test_sport_has_id():
    assert hasattr(Sport, "id")
    descriptor = None
    for klass in Sport.__mro__:
        if "id" in klass.__dict__:
            descriptor = klass.__dict__["id"]
            break
    assert isinstance(descriptor, property)

def test_sport_has_nom():
    assert hasattr(Sport, "nom")
    descriptor = None
    for klass in Sport.__mro__:
...
@given(instance=Sport_strategy)
def test_sport_id_setter(instance):
...
@given(instance=Sport_strategy)
def test_sport_nom_setter(instance):
