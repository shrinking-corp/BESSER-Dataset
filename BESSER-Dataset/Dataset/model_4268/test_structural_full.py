...
def test_assoc_games8_link_reassign_clear():
...
def test_assoc_matchup9_link_reassign_clear():
    a = bowling_Game(frames=7)
    b1 = bowling_Matchup()
    b2 = bowling_Matchup()
...
def test_assoc_matchups6_link_reassign_clear():
    a = bowling_Tournament(type="sample_text")
    b1 = bowling_Matchup()
    b2 = bowling_Matchup()
    _safe_set(a, 'bowling_Tournament7', {b1})
    assert _is_linked(a, 'bowling_Tournament7', b1)
    if hasattr(b1, 'bowling_Matchup'):
        assert _is_linked(b1, 'bowling_Matchup', a)
    _safe_set(a, 'bowling_Tournament7', {b2})
    assert _is_linked(a, 'bowling_Tournament7', b2)
    if hasattr(b1, 'bowling_Matchup'):
        assert not _is_linked(b1, 'bowling_Matchup', a)
    if hasattr(b2, 'bowling_Matchup'):
        assert _is_linked(b2, 'bowling_Matchup', a)
    _safe_set(a, 'bowling_Tournament7', set())
    assert not _is_linked(a, 'bowling_Tournament7', b2)
    if hasattr(b2, 'bowling_Matchup'):
        assert not _is_linked(b2, 'bowling_Matchup', a)


def test_assoc_player10_link_reassign_clear():
    ...
    _safe_set(a, 'bowling_Player11', b1)
    assert _is_linked(a, 'bowling_Player11', b1)
    if hasattr(b1, 'bowling_Game'):
        assert _is_linked(b1, 'bowling_Game', a)
    _safe_set(a, 'bowling_Player11', b2)
    assert _is_linked(a, 'bowling_Player11', b2)
    if hasattr(b1, 'bowling_Game'):
        assert not _is_linked(b1, 'bowling_Game', a)
    if hasattr(b2, 'bowling_Game'):
        assert _is_linked(b2, 'bowling_Game', a)
    _safe_set(a, 'bowling_Player11', None)
    assert not _is_linked(a, 'bowling_Player11', b2)
    if hasattr(b2, 'bowling_Game'):
...
def test_assoc_subleagues2_link_reassign_clear():
    a = bowling_League(name="sample_text")
    b1 = bowling_League(name="sample_text")
    b2 = bowling_League(name="sample_text_2")
    _safe_set(a, 'bowling_League1', {b1})
    assert _is_linked(a, 'bowling_League1', b1)
    if hasattr(b1, 'bowling_League3'):
        assert _is_linked(b1, 'bowling_League3', a)
    _safe_set(a, 'bowling_League1', {b2})
    assert _is_linked(a, 'bowling_League1', b2)
    if hasattr(b1, 'bowling_League3'):
        assert not _is_linked(b1, 'bowling_League3', a)
    if hasattr(b2, 'bowling_League3'):
        assert _is_linked(b2, 'bowling_League3', a)
    _safe_set(a, 'bowling_League1', set())
    assert not _is_linked(a, 'bowling_League1', b2)
    if hasattr(b2, 'bowling_League3'):
        assert not _is_linked(b2, 'bowling_League3', a)


def test_assoc_tournaments4_link_reassign_clear():
    a = bowling_Tournament(type="sample_text")
    b1 = bowling_League(name="sample_text")
    b2 = bowling_League(name="sample_text_2")
