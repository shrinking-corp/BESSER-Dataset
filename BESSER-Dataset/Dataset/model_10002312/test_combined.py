...
def test_hyp_ncs_is_not_abstract():
    assert not inspect.isabstract(NCS)


def test_hyp_ncs_constructor_exists():
    assert callable(NCS.__init__)


def test_hyp_ncs_constructor_args():
    sig = inspect.signature(NCS.__init__)
...
NCS_strategy = st.builds(NCS)
@given(instance=NCS_strategy)
@settings(max_examples=25)
def test_NCS_instantiation(instance):
    assert isinstance(instance, NCS)
