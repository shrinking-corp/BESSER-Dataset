...
def test_hyp_b_y_is_not_abstract():
    assert not inspect.isabstract(b_Y)


def test_hyp_b_y_constructor_exists():
    assert callable(b_Y.__init__)


def test_hyp_b_y_constructor_args():
    sig = inspect.signature(b_Y.__init__)
...
@given(instance=b_Y_strategy)
def test_hyp_b_y_info_setter(instance):
...
@given(instance=b_Y_strategy)
def test_hyp_b_y_label_setter(instance):
...
def test_b_Y_info_value_roundtrip():
    instance = b_Y(info="sample_text", label="sample_text")
    assert instance.info == "sample_text"
    instance.info = "sample_text_2"
    assert instance.info == "sample_text_2"


def test_b_Y_label_value_roundtrip():
    instance = b_Y(info="sample_text", label="sample_text")
...
def test_assoc_y1_link_reassign_clear():
    a = b_Y(info="sample_text", label="sample_text")
    b1 = b_Model()
    b2 = b_Model()
    _safe_set(a, 'b_Y', b1)
    assert _is_linked(a, 'b_Y', b1)
    if hasattr(b1, 'b_Model2'):
        assert _is_linked(b1, 'b_Model2', a)
    _safe_set(a, 'b_Y', b2)
    assert _is_linked(a, 'b_Y', b2)
    if hasattr(b1, 'b_Model2'):
        assert not _is_linked(b1, 'b_Model2', a)
    if hasattr(b2, 'b_Model2'):
        assert _is_linked(b2, 'b_Model2', a)
    _safe_set(a, 'b_Y', None)
    assert not _is_linked(a, 'b_Y', b2)
    if hasattr(b2, 'b_Model2'):
...
b_Y_strategy = st.builds(b_Y, info=safe_text, label=safe_text)
@given(instance=b_Y_strategy)
@settings(max_examples=25)
def test_b_Y_instantiation(instance):
    assert isinstance(instance, b_Y)
