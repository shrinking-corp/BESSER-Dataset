...
@given(instance=DBDetails_strategy)
def test_hyp_dbdetails_isloggedin_setter(instance):
...
def test_DBDetails_iIssessionout_value_roundtrip():
    instance = DBDetails(iIssessionout=True, isloggedin=True)
    assert instance.iIssessionout == True
    instance.iIssessionout = False
    assert instance.iIssessionout == False


def test_DBDetails_isloggedin_value_roundtrip():
    instance = DBDetails(iIssessionout=True, isloggedin=True)
...
DBDetails_strategy = st.builds(DBDetails, iIssessionout=st.booleans(), isloggedin=st.booleans())
@given(instance=DBDetails_strategy)
@settings(max_examples=25)
