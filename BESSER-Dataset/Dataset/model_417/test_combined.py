...
def test_assoc_classifier52_link_reassign_clear():
    ...
    _safe_set(a, 'UML2_Classifier53', b1)
    assert _is_linked(a, 'UML2_Classifier53', b1)
    if hasattr(b1, 'UML2_CreateObjectAction'):
        assert _is_linked(b1, 'UML2_CreateObjectAction', a)
    _safe_set(a, 'UML2_Classifier53', b2)
    assert _is_linked(a, 'UML2_Classifier53', b2)
    if hasattr(b1, 'UML2_CreateObjectAction'):
    ...
    _safe_set(a, 'UML2_Classifier53', None)
    assert not _is_linked(a, 'UML2_Classifier53', b2)
    if hasattr(b2, 'UML2_CreateObjectAction'):
        assert not _is_linked(b2, 'UML2_CreateObjectAction', a)


def test_assoc_context_75_link_reassign_clear():
    a = UML2_Classifier(isAbstract=True)
    b1 = UML2_Action()
    b2 = UML2_Action()
    _safe_set(a, 'UML2_Classifier76', b1)
    assert _is_linked(a, 'UML2_Classifier76', b1)
    if hasattr(b1, 'UML2_Action'):
        assert _is_linked(b1, 'UML2_Action', a)
    _safe_set(a, 'UML2_Classifier76', b2)
    assert _is_linked(a, 'UML2_Classifier76', b2)
    if hasattr(b1, 'UML2_Action'):
        assert not _is_linked(b1, 'UML2_Action', a)
    if hasattr(b2, 'UML2_Action'):
        assert _is_linked(b2, 'UML2_Action', a)
    _safe_set(a, 'UML2_Classifier76', None)
    assert not _is_linked(a, 'UML2_Classifier76', b2)
    if hasattr(b2, 'UML2_Action'):
        assert not _is_linked(b2, 'UML2_Action', a)


def test_assoc_definingFeature33_link_reassign_clear():
    a = UML2_StructuralFeature(isReadOnly=True)
    b1 = UML2_Slot()
    b2 = UML2_Slot()
    _safe_set(a, 'UML2_StructuralFeature35', b1)
    assert _is_linked(a, 'UML2_StructuralFeature35', b1)
    if hasattr(b1, 'UML2_Slot34'):
        assert _is_linked(b1, 'UML2_Slot34', a)
    _safe_set(a, 'UML2_StructuralFeature35', b2)
    assert _is_linked(a, 'UML2_StructuralFeature35', b2)
    if hasattr(b1, 'UML2_Slot34'):
        assert not _is_linked(b1, 'UML2_Slot34', a)
    if hasattr(b2, 'UML2_Slot34'):
        assert _is_linked(b2, 'UML2_Slot34', a)
    _safe_set(a, 'UML2_StructuralFeature35', None)
    assert not _is_linked(a, 'UML2_StructuralFeature35', b2)
    if hasattr(b2, 'UML2_Slot34'):
        assert not _is_linked(b2, 'UML2_Slot34', a)


def test_assoc_end17_link_reassign_clear():
    a = UML2_Property(aggregation="sample_text", isComposite=True, isDerived=True, isDerivedUnion=True)
    b1 = UML2_LinkEndData()
    b2 = UML2_LinkEndData()
    _safe_set(a, 'UML2_Property', b1)
    assert _is_linked(a, 'UML2_Property', b1)
    if hasattr(b1, 'UML2_LinkEndData18'):
        assert _is_linked(b1, 'UML2_LinkEndData18', a)
    _safe_set(a, 'UML2_Property', b2)
    assert _is_linked(a, 'UML2_Property', b2)
    if hasattr(b1, 'UML2_LinkEndData18'):
        assert not _is_linked(b1, 'UML2_LinkEndData18', a)
    if hasattr(b2, 'UML2_LinkEndData18'):
        assert _is_linked(b2, 'UML2_LinkEndData18', a)
    _safe_set(a, 'UML2_Property', None)
    assert not _is_linked(a, 'UML2_Property', b2)
    if hasattr(b2, 'UML2_LinkEndData18'):
        assert not _is_linked(b2, 'UML2_LinkEndData18', a)


def test_assoc_feature4_link_reassign_clear():
    ...
    if hasattr(b1, 'UML2_Classifier5'):
        assert _is_linked(b1, 'UML2_Classifier5', a)
    _safe_set(a, 'UML2_Feature', b2)
    assert _is_linked(a, 'UML2_Feature', b2)
    if hasattr(b1, 'UML2_Classifier5'):
        assert not _is_linked(b1, 'UML2_Classifier5', a)
    if hasattr(b2, 'UML2_Classifier5'):
        assert _is_linked(b2, 'UML2_Classifier5', a)
    _safe_set(a, 'UML2_Feature', None)
    assert not _is_linked(a, 'UML2_Feature', b2)
    if hasattr(b2, 'UML2_Classifier5'):
        assert not _is_linked(b2, 'UML2_Classifier5', a)


def test_assoc_featuringClassifier12_link_reassign_clear():
    a = UML2_Feature(isStatic=True)
    b1 = UML2_Classifier(isAbstract=True)
    b2 = UML2_Classifier(isAbstract=False)
    _safe_set(a, 'UML2_Feature13', {b1})
    assert _is_linked(a, 'UML2_Feature13', b1)
    if hasattr(b1, 'UML2_Classifier14'):
        assert _is_linked(b1, 'UML2_Classifier14', a)
    _safe_set(a, 'UML2_Feature13', {b2})
    assert _is_linked(a, 'UML2_Feature13', b2)
    if hasattr(b1, 'UML2_Classifier14'):
        assert not _is_linked(b1, 'UML2_Classifier14', a)
    if hasattr(b2, 'UML2_Classifier14'):
        assert _is_linked(b2, 'UML2_Classifier14', a)
    _safe_set(a, 'UML2_Feature13', set())
    assert not _is_linked(a, 'UML2_Feature13', b2)
    if hasattr(b2, 'UML2_Classifier14'):
        assert not _is_linked(b2, 'UML2_Classifier14', a)


def test_assoc_general62_link_reassign_clear():
    a = UML2_Classifier(isAbstract=True)
    b1 = UML2_Generalization()
    b2 = UML2_Generalization()
    _safe_set(a, 'UML2_Classifier64', b1)
    assert _is_linked(a, 'UML2_Classifier64', b1)
    if hasattr(b1, 'UML2_Generalization63'):
        assert _is_linked(b1, 'UML2_Generalization63', a)
    _safe_set(a, 'UML2_Classifier64', b2)
    assert _is_linked(a, 'UML2_Classifier64', b2)
    if hasattr(b1, 'UML2_Generalization63'):
        assert not _is_linked(b1, 'UML2_Generalization63', a)
    if hasattr(b2, 'UML2_Generalization63'):
        assert _is_linked(b2, 'UML2_Generalization63', a)
    _safe_set(a, 'UML2_Classifier64', None)
    assert not _is_linked(a, 'UML2_Classifier64', b2)
    if hasattr(b2, 'UML2_Generalization63'):
        assert not _is_linked(b2, 'UML2_Generalization63', a)
...
def test_assoc_inheritedMember6_link_reassign_clear():
    ...
    if hasattr(b1, 'UML2_Classifier7'):
        assert _is_linked(b1, 'UML2_Classifier7', a)
    _safe_set(a, 'UML2_NamedElement', b2)
    assert _is_linked(a, 'UML2_NamedElement', b2)
    if hasattr(b1, 'UML2_Classifier7'):
        assert not _is_linked(b1, 'UML2_Classifier7', a)
    if hasattr(b2, 'UML2_Classifier7'):
        assert _is_linked(b2, 'UML2_Classifier7', a)
    _safe_set(a, 'UML2_NamedElement', None)
    assert not _is_linked(a, 'UML2_NamedElement', b2)
    if hasattr(b2, 'UML2_Classifier7'):
        assert not _is_linked(b2, 'UML2_Classifier7', a)


def test_assoc_member77_link_reassign_clear():
    a = UML2_NamedElement(name="sample_text", visibility="sample_text")
    b1 = UML2_Namespace()
    b2 = UML2_Namespace()
    _safe_set(a, 'UML2_NamedElement78', b1)
    assert _is_linked(a, 'UML2_NamedElement78', b1)
    if hasattr(b1, 'UML2_Namespace'):
        assert _is_linked(b1, 'UML2_Namespace', a)
    _safe_set(a, 'UML2_NamedElement78', b2)
    assert _is_linked(a, 'UML2_NamedElement78', b2)
    if hasattr(b1, 'UML2_Namespace'):
        assert not _is_linked(b1, 'UML2_Namespace', a)
    if hasattr(b2, 'UML2_Namespace'):
        assert _is_linked(b2, 'UML2_Namespace', a)
    _safe_set(a, 'UML2_NamedElement78', None)
    assert not _is_linked(a, 'UML2_NamedElement78', b2)
    if hasattr(b2, 'UML2_Namespace'):
        assert not _is_linked(b2, 'UML2_Namespace', a)


def test_assoc_memberEnd49_link_reassign_clear():
    a = UML2_Property(aggregation="sample_text", isComposite=True, isDerived=True, isDerivedUnion=True)
    b1 = UML2_Association()
    b2 = UML2_Association()
...
def test_assoc_nestedClassifier65_link_reassign_clear():
    a = UML2_Classifier(isAbstract=True)
    b1 = UML2_Class(isActive=True)
    b2 = UML2_Class(isActive=False)
    _safe_set(a, 'UML2_Classifier66', b1)
    assert _is_linked(a, 'UML2_Classifier66', b1)
    if hasattr(b1, 'UML2_Class'):
        assert _is_linked(b1, 'UML2_Class', a)
    _safe_set(a, 'UML2_Classifier66', b2)
    assert _is_linked(a, 'UML2_Classifier66', b2)
    if hasattr(b1, 'UML2_Class'):
        assert not _is_linked(b1, 'UML2_Class', a)
    if hasattr(b2, 'UML2_Class'):
        assert _is_linked(b2, 'UML2_Class', a)
    _safe_set(a, 'UML2_Classifier66', None)
    assert not _is_linked(a, 'UML2_Classifier66', b2)
    if hasattr(b2, 'UML2_Class'):
        assert not _is_linked(b2, 'UML2_Class', a)


def test_assoc_ownedEnd46_link_reassign_clear():
    a = UML2_Property(aggregation="sample_text", isComposite=True, isDerived=True, isDerivedUnion=True)
    b1 = UML2_Association()
    b2 = UML2_Association()
    _safe_set(a, 'UML2_Property48', b1)
    assert _is_linked(a, 'UML2_Property48', b1)
    if hasattr(b1, 'UML2_Association47'):
        assert _is_linked(b1, 'UML2_Association47', a)
    _safe_set(a, 'UML2_Property48', b2)
    assert _is_linked(a, 'UML2_Property48', b2)
    if hasattr(b1, 'UML2_Association47'):
        assert not _is_linked(b1, 'UML2_Association47', a)
    if hasattr(b2, 'UML2_Association47'):
        assert _is_linked(b2, 'UML2_Association47', a)
    _safe_set(a, 'UML2_Property48', None)
    assert not _is_linked(a, 'UML2_Property48', b2)
    if hasattr(b2, 'UML2_Association47'):
        assert not _is_linked(b2, 'UML2_Association47', a)


def test_assoc_ownedParameter11_link_reassign_clear():
...
def test_assoc_ownedReception67_link_reassign_clear():
    a = UML2_Class(isActive=True)
    b1 = UML2_Reception()
    b2 = UML2_Reception()
    _safe_set(a, 'UML2_Class68', {b1})
    assert _is_linked(a, 'UML2_Class68', b1)
    if hasattr(b1, 'UML2_Reception'):
        assert _is_linked(b1, 'UML2_Reception', a)
    _safe_set(a, 'UML2_Class68', {b2})
    assert _is_linked(a, 'UML2_Class68', b2)
    if hasattr(b1, 'UML2_Reception'):
        assert not _is_linked(b1, 'UML2_Reception', a)
    if hasattr(b2, 'UML2_Reception'):
        assert _is_linked(b2, 'UML2_Reception', a)
    _safe_set(a, 'UML2_Class68', set())
    assert not _is_linked(a, 'UML2_Class68', b2)
    if hasattr(b2, 'UML2_Reception'):
...
def test_assoc_qualifier23_link_reassign_clear():
    ...
    _safe_set(a, 'UML2_Property25', b1)
    assert _is_linked(a, 'UML2_Property25', b1)
    if hasattr(b1, 'UML2_QualifierValue24'):
        assert _is_linked(b1, 'UML2_QualifierValue24', a)
    _safe_set(a, 'UML2_Property25', b2)
    assert _is_linked(a, 'UML2_Property25', b2)
    if hasattr(b1, 'UML2_QualifierValue24'):
        assert not _is_linked(b1, 'UML2_QualifierValue24', a)
    if hasattr(b2, 'UML2_QualifierValue24'):
        assert _is_linked(b2, 'UML2_QualifierValue24', a)
    _safe_set(a, 'UML2_Property25', None)
    assert not _is_linked(a, 'UML2_Property25', b2)
    if hasattr(b2, 'UML2_QualifierValue24'):
        assert not _is_linked(b2, 'UML2_QualifierValue24', a)


def test_assoc_qualifier43_link_reassign_clear():
    a = UML2_Property(aggregation="sample_text", isComposite=True, isDerived=True, isDerivedUnion=True)
    ...
    _safe_set(a, 'UML2_Property42', {b1})
    assert _is_linked(a, 'UML2_Property42', b1)
    if hasattr(b1, 'UML2_Property44'):
        assert _is_linked(b1, 'UML2_Property44', a)
    _safe_set(a, 'UML2_Property42', {b2})
    assert _is_linked(a, 'UML2_Property42', b2)
    if hasattr(b1, 'UML2_Property44'):
        assert not _is_linked(b1, 'UML2_Property44', a)
    if hasattr(b2, 'UML2_Property44'):
        assert _is_linked(b2, 'UML2_Property44', a)
    _safe_set(a, 'UML2_Property42', set())
    assert not _is_linked(a, 'UML2_Property42', b2)
    if hasattr(b2, 'UML2_Property44'):
        assert not _is_linked(b2, 'UML2_Property44', a)


def test_assoc_structuralFeature21_link_reassign_clear():
    a = UML2_StructuralFeature(isReadOnly=True)
...
def test_assoc_subsettedProperty38_link_reassign_clear():
    a = UML2_Property(aggregation="sample_text", isComposite=True, isDerived=True, isDerivedUnion=True)
    b1 = UML2_Property(aggregation="sample_text", isComposite=True, isDerived=True, isDerivedUnion=True)
    b2 = UML2_Property(aggregation="sample_text_2", isComposite=False, isDerived=False, isDerivedUnion=False)
    _safe_set(a, 'UML2_Property37', {b1})
    assert _is_linked(a, 'UML2_Property37', b1)
    if hasattr(b1, 'UML2_Property39'):
        assert _is_linked(b1, 'UML2_Property39', a)
    _safe_set(a, 'UML2_Property37', {b2})
    assert _is_linked(a, 'UML2_Property37', b2)
    if hasattr(b1, 'UML2_Property39'):
        assert not _is_linked(b1, 'UML2_Property39', a)
    if hasattr(b2, 'UML2_Property39'):
        assert _is_linked(b2, 'UML2_Property39', a)
    _safe_set(a, 'UML2_Property37', set())
    assert not _is_linked(a, 'UML2_Property37', b2)
    if hasattr(b2, 'UML2_Property39'):
        assert not _is_linked(b2, 'UML2_Property39', a)
