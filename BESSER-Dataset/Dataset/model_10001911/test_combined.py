...
def test_hyp_machine_attribute2_setter(instance):
...
@given(instance=Machine_strategy)
def test_hyp_machine_attribute_setter(instance):
...
def test_Machine_attribute_value_roundtrip():
    instance = Machine(attribute="sample_text", attribute2="sample_text")
    assert instance.attribute == "sample_text"
    instance.attribute = "sample_text_2"
    assert instance.attribute == "sample_text_2"


def test_Machine_attribute2_value_roundtrip():
    instance = Machine(attribute="sample_text", attribute2="sample_text")
...
def test_assoc_Machine_Card_link_reassign_clear():
    a = Machine(attribute="sample_text", attribute2="sample_text")
    b1 = Card()
    b2 = Card()
    _safe_set(a, 'cards19', {b1})
    assert _is_linked(a, 'cards19', b1)
    if hasattr(b1, 'machine18'):
        assert _is_linked(b1, 'machine18', a)
    _safe_set(a, 'cards19', {b2})
    assert _is_linked(a, 'cards19', b2)
    if hasattr(b1, 'machine18'):
        assert not _is_linked(b1, 'machine18', a)
    if hasattr(b2, 'machine18'):
        assert _is_linked(b2, 'machine18', a)
    _safe_set(a, 'cards19', set())
    assert not _is_linked(a, 'cards19', b2)
    if hasattr(b2, 'machine18'):
        assert not _is_linked(b2, 'machine18', a)


def test_assoc_Machine_Memory_link_reassign_clear():
    a = Machine(attribute="sample_text", attribute2="sample_text")
    b1 = Memory_Interface()
    b2 = Memory_Interface()
    _safe_set(a, 'memory1', {b1})
    assert _is_linked(a, 'memory1', b1)
    if hasattr(b1, 'machine0'):
        assert _is_linked(b1, 'machine0', a)
    _safe_set(a, 'memory1', {b2})
    assert _is_linked(a, 'memory1', b2)
    if hasattr(b1, 'machine0'):
        assert not _is_linked(b1, 'machine0', a)
    if hasattr(b2, 'machine0'):
        assert _is_linked(b2, 'machine0', a)
    _safe_set(a, 'memory1', set())
    assert not _is_linked(a, 'memory1', b2)
    if hasattr(b2, 'machine0'):
        assert not _is_linked(b2, 'machine0', a)


def test_assoc_Machine_Processor_link_reassign_clear():
    a = Machine(attribute="sample_text", attribute2="sample_text")
    b1 = Processor()
    b2 = Processor()
    _safe_set(a, 'processor7', {b1})
    assert _is_linked(a, 'processor7', b1)
    if hasattr(b1, 'machine6'):
        assert _is_linked(b1, 'machine6', a)
    _safe_set(a, 'processor7', {b2})
    assert _is_linked(a, 'processor7', b2)
    if hasattr(b1, 'machine6'):
        assert not _is_linked(b1, 'machine6', a)
    if hasattr(b2, 'machine6'):
        assert _is_linked(b2, 'machine6', a)
    _safe_set(a, 'processor7', set())
    assert not _is_linked(a, 'processor7', b2)
    if hasattr(b2, 'machine6'):
        assert not _is_linked(b2, 'machine6', a)
...
Machine_strategy = st.builds(Machine, attribute=safe_text, attribute2=safe_text)
@given(instance=Machine_strategy)
@settings(max_examples=25)
