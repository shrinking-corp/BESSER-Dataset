...
def test_mid_operator_NestingOperator_nestedMIDPath_value_roundtrip():
    instance = mid_operator_NestingOperator(nestedMIDPath="sample_text")
    assert instance.nestedMIDPath == "sample_text"
    instance.nestedMIDPath = "sample_text_2"
    assert instance.nestedMIDPath == "sample_text_2"


def test_mid_operator_Operator_commutative_value_roundtrip():
    instance = mid_operator_Operator(commutative=True, executionTime="sample_text", workingPath="sample_text")
...
def test_mid_operator_Operator_workingPath_value_roundtrip():
    instance = mid_operator_Operator(commutative=True, executionTime="sample_text", workingPath="sample_text")
    assert instance.workingPath == "sample_text"
    instance.workingPath = "sample_text_2"
    assert instance.workingPath == "sample_text_2"
...
def test_mid_operator_WorkflowOperator_isa_NestingOperator():
    instance = mid_operator_WorkflowOperator()
...
def test_mid_operator_NestingOperator_isa_Operator():
    instance = mid_operator_NestingOperator(nestedMIDPath="sample_text")
    assert isinstance(instance, Operator)
...
def test_assoc_EMFInstanceRoot23_link_reassign_clear():
    a = mid_Model(fileExtension="sample_text", origin="sample_text")
    b1 = mid_EObject()
    b2 = mid_EObject()
    _safe_set(a, 'mid_Model24', b1)
    assert _is_linked(a, 'mid_Model24', b1)
    if hasattr(b1, 'mid_EObject'):
        assert _is_linked(b1, 'mid_EObject', a)
    _safe_set(a, 'mid_Model24', b2)
    assert _is_linked(a, 'mid_Model24', b2)
    if hasattr(b1, 'mid_EObject'):
        assert not _is_linked(b1, 'mid_EObject', a)
    if hasattr(b2, 'mid_EObject'):
        assert _is_linked(b2, 'mid_EObject', a)
    _safe_set(a, 'mid_Model24', None)
    assert not _is_linked(a, 'mid_Model24', b2)
    if hasattr(b2, 'mid_EObject'):
        assert not _is_linked(b2, 'mid_EObject', a)
...
def test_assoc_containedObject39_link_reassign_clear():
    ...
    _safe_set(a, 'mid_relationship_ExtendibleElementReference40', b1)
    assert _is_linked(a, 'mid_relationship_ExtendibleElementReference40', b1)
    if hasattr(b1, 'relationship_mid_ExtendibleElement41'):
        assert _is_linked(b1, 'relationship_mid_ExtendibleElement41', a)
    _safe_set(a, 'mid_relationship_ExtendibleElementReference40', b2)
    assert _is_linked(a, 'mid_relationship_ExtendibleElementReference40', b2)
    if hasattr(b1, 'relationship_mid_ExtendibleElement41'):
        assert not _is_linked(b1, 'relationship_mid_ExtendibleElement41', a)
    if hasattr(b2, 'relationship_mid_ExtendibleElement41'):
        assert _is_linked(b2, 'relationship_mid_ExtendibleElement41', a)
    _safe_set(a, 'mid_relationship_ExtendibleElementReference40', None)
    assert not _is_linked(a, 'mid_relationship_ExtendibleElementReference40', b2)
    if hasattr(b2, 'relationship_mid_ExtendibleElement41'):
        assert not _is_linked(b2, 'relationship_mid_ExtendibleElement41', a)
...
def test_assoc_eInfo25_link_reassign_clear():
    a = mid_ModelElement()
    ...
    _safe_set(a, 'mid_ModelElement26', b1)
    assert _is_linked(a, 'mid_ModelElement26', b1)
    if hasattr(b1, 'mid_EMFInfo'):
        assert _is_linked(b1, 'mid_EMFInfo', a)
    _safe_set(a, 'mid_ModelElement26', b2)
    assert _is_linked(a, 'mid_ModelElement26', b2)
    if hasattr(b1, 'mid_EMFInfo'):
        assert not _is_linked(b1, 'mid_EMFInfo', a)
    if hasattr(b2, 'mid_EMFInfo'):
        assert _is_linked(b2, 'mid_EMFInfo', a)
    _safe_set(a, 'mid_ModelElement26', None)
    assert not _is_linked(a, 'mid_ModelElement26', b2)
    if hasattr(b2, 'mid_EMFInfo'):
...
def test_assoc_generics63_link_reassign_clear():
    a = mid_operator_Operator(commutative=True, executionTime="sample_text", workingPath="sample_text")
    b1 = GenericEndpoint()
    b2 = GenericEndpoint()
...
def test_assoc_mappingRefs32_link_reassign_clear():
    a = mid_relationship_ModelRel()
    b1 = MappingReference()
    b2 = MappingReference()
    _safe_set(a, 'mid_relationship_ModelRel33', {b1})
    assert _is_linked(a, 'mid_relationship_ModelRel33', b1)
    if hasattr(b1, 'MappingReference'):
        assert _is_linked(b1, 'MappingReference', a)
    _safe_set(a, 'mid_relationship_ModelRel33', {b2})
    assert _is_linked(a, 'mid_relationship_ModelRel33', b2)
    if hasattr(b1, 'MappingReference'):
        assert not _is_linked(b1, 'MappingReference', a)
    if hasattr(b2, 'MappingReference'):
        assert _is_linked(b2, 'MappingReference', a)
    _safe_set(a, 'mid_relationship_ModelRel33', set())
    assert not _is_linked(a, 'mid_relationship_ModelRel33', b2)
    if hasattr(b2, 'MappingReference'):
...
def test_assoc_modelElemEndpointRefs47_link_reassign_clear():
    a = mid_relationship_Mapping()
    b1 = ModelElementEndpointReference()
    b2 = ModelElementEndpointReference()
    _safe_set(a, 'mid_relationship_Mapping48', {b1})
    assert _is_linked(a, 'mid_relationship_Mapping48', b1)
    if hasattr(b1, 'ModelElementEndpointReference49'):
        assert _is_linked(b1, 'ModelElementEndpointReference49', a)
    _safe_set(a, 'mid_relationship_Mapping48', {b2})
    assert _is_linked(a, 'mid_relationship_Mapping48', b2)
    if hasattr(b1, 'ModelElementEndpointReference49'):
    ...
    _safe_set(a, 'mid_relationship_Mapping48', set())
    assert not _is_linked(a, 'mid_relationship_Mapping48', b2)
    if hasattr(b2, 'ModelElementEndpointReference49'):
        assert not _is_linked(b2, 'ModelElementEndpointReference49', a)


def test_assoc_modelElemEndpointRefs50_link_reassign_clear():
    ...
    if hasattr(b1, 'ModelElementEndpointReference51'):
        assert _is_linked(b1, 'ModelElementEndpointReference51', a)
    _safe_set(a, 'mid_relationship_MappingReference', {b2})
    assert _is_linked(a, 'mid_relationship_MappingReference', b2)
    if hasattr(b1, 'ModelElementEndpointReference51'):
        assert not _is_linked(b1, 'ModelElementEndpointReference51', a)
    if hasattr(b2, 'ModelElementEndpointReference51'):
        assert _is_linked(b2, 'ModelElementEndpointReference51', a)
    _safe_set(a, 'mid_relationship_MappingReference', set())
    assert not _is_linked(a, 'mid_relationship_MappingReference', b2)
    if hasattr(b2, 'ModelElementEndpointReference51'):
        assert not _is_linked(b2, 'ModelElementEndpointReference51', a)


def test_assoc_modelElemEndpoints46_link_reassign_clear():
    a = mid_relationship_Mapping()
...
def test_assoc_modelElemRef57_link_reassign_clear():
    ...
    if hasattr(b1, 'ModelElementReference58'):
        assert _is_linked(b1, 'ModelElementReference58', a)
    _safe_set(a, 'modelElemEndpointRefs', b2)
    assert _is_linked(a, 'modelElemEndpointRefs', b2)
    if hasattr(b1, 'ModelElementReference58'):
        assert not _is_linked(b1, 'ModelElementReference58', a)
    if hasattr(b2, 'ModelElementReference58'):
        assert _is_linked(b2, 'ModelElementReference58', a)
    _safe_set(a, 'modelElemEndpointRefs', None)
    assert not _is_linked(a, 'modelElemEndpointRefs', b2)
    if hasattr(b2, 'ModelElementReference58'):
        assert not _is_linked(b2, 'ModelElementReference58', a)


def test_assoc_modelElemRefs44_link_reassign_clear():
...
def test_assoc_modelEndpointRefs30_link_reassign_clear():
    a = mid_relationship_ModelRel()
...
def test_assoc_modelEndpoints27_link_reassign_clear():
    a = mid_relationship_ModelRel()
...
def test_assoc_outputs60_link_reassign_clear():
    a = mid_operator_Operator(commutative=True, executionTime="sample_text", workingPath="sample_text")
    b1 = operator_mid_ModelEndpoint()
    b2 = operator_mid_ModelEndpoint()
    _safe_set(a, 'mid_operator_Operator61', {b1})
    assert _is_linked(a, 'mid_operator_Operator61', b1)
    if hasattr(b1, 'operator_mid_ModelEndpoint62'):
        assert _is_linked(b1, 'operator_mid_ModelEndpoint62', a)
    _safe_set(a, 'mid_operator_Operator61', {b2})
    assert _is_linked(a, 'mid_operator_Operator61', b2)
    if hasattr(b1, 'operator_mid_ModelEndpoint62'):
        assert not _is_linked(b1, 'operator_mid_ModelEndpoint62', a)
    if hasattr(b2, 'operator_mid_ModelEndpoint62'):
        assert _is_linked(b2, 'operator_mid_ModelEndpoint62', a)
    _safe_set(a, 'mid_operator_Operator61', set())
    assert not _is_linked(a, 'mid_operator_Operator61', b2)
    if hasattr(b2, 'operator_mid_ModelEndpoint62'):
        assert not _is_linked(b2, 'operator_mid_ModelEndpoint62', a)


def test_assoc_referencedObject38_link_reassign_clear():
    a = mid_relationship_ExtendibleElementReference(modifiable=True)
    b1 = relationship_mid_ExtendibleElement()
    b2 = relationship_mid_ExtendibleElement()
...
def test_assoc_sourceModel34_link_reassign_clear():
...
def test_assoc_sourceModelElemRef52_link_reassign_clear():
    ...
    if hasattr(b1, 'ModelElementReference53'):
        assert _is_linked(b1, 'ModelElementReference53', a)
    _safe_set(a, 'mid_relationship_BinaryMappingReference', b2)
    assert _is_linked(a, 'mid_relationship_BinaryMappingReference', b2)
    if hasattr(b1, 'ModelElementReference53'):
        assert not _is_linked(b1, 'ModelElementReference53', a)
    if hasattr(b2, 'ModelElementReference53'):
        assert _is_linked(b2, 'ModelElementReference53', a)
    ...
    if hasattr(b2, 'ModelElementReference53'):
        assert not _is_linked(b2, 'ModelElementReference53', a)
...
def test_assoc_supertypeRef42_link_reassign_clear():
    a = mid_relationship_ExtendibleElementReference(modifiable=True)
    b1 = ExtendibleElementReference()
    b2 = ExtendibleElementReference()
    _safe_set(a, 'mid_relationship_ExtendibleElementReference43', b1)
    assert _is_linked(a, 'mid_relationship_ExtendibleElementReference43', b1)
    if hasattr(b1, 'ExtendibleElementReference'):
        assert _is_linked(b1, 'ExtendibleElementReference', a)
    _safe_set(a, 'mid_relationship_ExtendibleElementReference43', b2)
    assert _is_linked(a, 'mid_relationship_ExtendibleElementReference43', b2)
    if hasattr(b1, 'ExtendibleElementReference'):
    ...
    _safe_set(a, 'mid_relationship_ExtendibleElementReference43', None)
    assert not _is_linked(a, 'mid_relationship_ExtendibleElementReference43', b2)
    if hasattr(b2, 'ExtendibleElementReference'):
...
def test_assoc_targetModel35_link_reassign_clear():
    a = mid_relationship_BinaryModelRel()
    b1 = relationship_mid_Model()
    b2 = relationship_mid_Model()
    _safe_set(a, 'mid_relationship_BinaryModelRel36', b1)
    assert _is_linked(a, 'mid_relationship_BinaryModelRel36', b1)
    if hasattr(b1, 'relationship_mid_Model37'):
        assert _is_linked(b1, 'relationship_mid_Model37', a)
    _safe_set(a, 'mid_relationship_BinaryModelRel36', b2)
    assert _is_linked(a, 'mid_relationship_BinaryModelRel36', b2)
    if hasattr(b1, 'relationship_mid_Model37'):
        assert not _is_linked(b1, 'relationship_mid_Model37', a)
    if hasattr(b2, 'relationship_mid_Model37'):
        assert _is_linked(b2, 'relationship_mid_Model37', a)
    _safe_set(a, 'mid_relationship_BinaryModelRel36', None)
    assert not _is_linked(a, 'mid_relationship_BinaryModelRel36', b2)
    if hasattr(b2, 'relationship_mid_Model37'):
        assert not _is_linked(b2, 'relationship_mid_Model37', a)


def test_assoc_targetModelElemRef54_link_reassign_clear():
    a = mid_relationship_BinaryMappingReference()
    b1 = ModelElementReference()
    b2 = ModelElementReference()
    _safe_set(a, 'mid_relationship_BinaryMappingReference55', b1)
    assert _is_linked(a, 'mid_relationship_BinaryMappingReference55', b1)
    if hasattr(b1, 'ModelElementReference56'):
        assert _is_linked(b1, 'ModelElementReference56', a)
    _safe_set(a, 'mid_relationship_BinaryMappingReference55', b2)
    assert _is_linked(a, 'mid_relationship_BinaryMappingReference55', b2)
    if hasattr(b1, 'ModelElementReference56'):
    ...
    _safe_set(a, 'mid_relationship_BinaryMappingReference55', None)
    assert not _is_linked(a, 'mid_relationship_BinaryMappingReference55', b2)
    if hasattr(b2, 'ModelElementReference56'):
...
NestingOperator_strategy = st.builds(NestingOperator)
...
mid_EObject_strategy = st.builds(mid_EObject)
...
mid_operator_NestingOperator_strategy = st.builds(mid_operator_NestingOperator, nestedMIDPath=safe_text)
...
mid_operator_Operator_strategy = st.builds(mid_operator_Operator, commutative=st.booleans(), executionTime=safe_text, workingPath=safe_text)
...
mid_operator_WorkflowOperator_strategy = st.builds(mid_operator_WorkflowOperator)
