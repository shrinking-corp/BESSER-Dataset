...
def test_assoc_Payment_Property_link_reassign_clear():
    ...
    if hasattr(b1, 'property14'):
        assert _is_linked(b1, 'property14', a)
    _safe_set(a, 'payment15', b2)
    assert _is_linked(a, 'payment15', b2)
    if hasattr(b1, 'property14'):
        assert not _is_linked(b1, 'property14', a)
    if hasattr(b2, 'property14'):
        assert _is_linked(b2, 'property14', a)
    _safe_set(a, 'payment15', None)
    assert not _is_linked(a, 'payment15', b2)
    if hasattr(b2, 'property14'):
        assert not _is_linked(b2, 'property14', a)
...
def test_assoc_Payment_Reg_User_link_reassign_clear():
    ...
    if hasattr(b1, 'reg_User12'):
        assert _is_linked(b1, 'reg_User12', a)
    _safe_set(a, 'payment13', b2)
    assert _is_linked(a, 'payment13', b2)
    if hasattr(b1, 'reg_User12'):
        assert not _is_linked(b1, 'reg_User12', a)
    if hasattr(b2, 'reg_User12'):
        assert _is_linked(b2, 'reg_User12', a)
    _safe_set(a, 'payment13', None)
    assert not _is_linked(a, 'payment13', b2)
    if hasattr(b2, 'reg_User12'):
        assert not _is_linked(b2, 'reg_User12', a)
...
def test_assoc_Property_Management_link_reassign_clear():
    ...
    _safe_set(a, 'management10', {b1})
    assert _is_linked(a, 'management10', b1)
    if hasattr(b1, 'property11'):
        assert _is_linked(b1, 'property11', a)
    _safe_set(a, 'management10', {b2})
    assert _is_linked(a, 'management10', b2)
    if hasattr(b1, 'property11'):
        assert not _is_linked(b1, 'property11', a)
    if hasattr(b2, 'property11'):
        assert _is_linked(b2, 'property11', a)
    _safe_set(a, 'management10', set())
    assert not _is_linked(a, 'management10', b2)
    if hasattr(b2, 'property11'):
        assert not _is_linked(b2, 'property11', a)
