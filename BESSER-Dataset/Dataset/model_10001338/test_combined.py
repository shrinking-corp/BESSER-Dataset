...
def test_hyp_c_attC_setter(instance):
...
def test_B1_attB_value_roundtrip():
    instance = B1(attB="sample_text")
    assert instance.attB == "sample_text"
    instance.attB = "sample_text_2"
    assert instance.attB == "sample_text_2"


def test_C_attC_value_roundtrip():
    instance = C(attC="sample_text")
    assert instance.attC == "sample_text"
    instance.attC = "sample_text_2"
    assert instance.attC == "sample_text_2"
...
def test_assoc_B_C2_link_reassign_clear():
    ...
    b1 = B1(attB="sample_text")
    b2 = B1(attB="sample_text_2")
    _safe_set(a, 'b9', b1)
...
def test_assoc_Class3_Class2_link_reassign_clear():
    a = B1(attB="sample_text")
...
def test_assoc_aR_link_reassign_clear():
    a = A1(attA="sample_text")
    b1 = R()
    b2 = R()
    _safe_set(a, 'class77', b1)
    assert _is_linked(a, 'class77', b1)
    if hasattr(b1, 'class36'):
        assert _is_linked(b1, 'class36', a)
    _safe_set(a, 'class77', b2)
    assert _is_linked(a, 'class77', b2)
    if hasattr(b1, 'class36'):
        assert not _is_linked(b1, 'class36', a)
    if hasattr(b2, 'class36'):
        assert _is_linked(b2, 'class36', a)
    _safe_set(a, 'class77', None)
    assert not _is_linked(a, 'class77', b2)
    if hasattr(b2, 'class36'):
        assert not _is_linked(b2, 'class36', a)
...
B1_strategy = st.builds(B1, attB=safe_text)
@given(instance=B1_strategy)
@settings(max_examples=25)
...
C_strategy = st.builds(C, attC=safe_text)
@given(instance=C_strategy)
@settings(max_examples=25)
