...
def test_connection_Concept_LoopExpression_value_roundtrip():
    instance = connection_Concept(LoopExpression="sample_text", LoopLimit="sample_text", inputModel=True)
...
def test_connection_XmlFileConnection_Encoding_value_roundtrip():
    instance = connection_XmlFileConnection(Encoding="sample_text", Guess=True, MaskXPattern="sample_text", XmlFilePath="sample_text", XsdFilePath="sample_text", inputModel=True, outputFilePath="sample_text")
    assert instance.Encoding == "sample_text"
    instance.Encoding = "sample_text_2"
    assert instance.Encoding == "sample_text_2"
...
def test_assoc_cdcConnection62_link_reassign_clear():
    a = connection_CDCType(journalName="sample_text", linkDB="sample_text")
    b1 = connection_CDCConnection()
    b2 = connection_CDCConnection()
    _safe_set(a, 'connection_CDCType63', b1)
    assert _is_linked(a, 'connection_CDCType63', b1)
    if hasattr(b1, 'connection_CDCConnection64'):
        assert _is_linked(b1, 'connection_CDCConnection64', a)
    _safe_set(a, 'connection_CDCType63', b2)
    assert _is_linked(a, 'connection_CDCType63', b2)
    if hasattr(b1, 'connection_CDCConnection64'):
        assert not _is_linked(b1, 'connection_CDCConnection64', a)
    if hasattr(b2, 'connection_CDCConnection64'):
        assert _is_linked(b2, 'connection_CDCConnection64', a)
    _safe_set(a, 'connection_CDCType63', None)
    assert not _is_linked(a, 'connection_CDCType63', b2)
    if hasattr(b2, 'connection_CDCConnection64'):
        assert not _is_linked(b2, 'connection_CDCConnection64', a)
...
def test_assoc_cdcTypes59_link_reassign_clear():
    a = connection_CDCType(journalName="sample_text", linkDB="sample_text")
    b1 = connection_CDCConnection()
    b2 = connection_CDCConnection()
...
def test_assoc_conceptTargets67_link_reassign_clear():
    a = connection_ConceptTarget(RelativeLoopExpression="sample_text", targetName="sample_text")
    b1 = connection_Concept(LoopExpression="sample_text", LoopLimit="sample_text", inputModel=True)
    b2 = connection_Concept(LoopExpression="sample_text_2", LoopLimit="sample_text_2", inputModel=False)
    _safe_set(a, 'ConceptTarget', b1)
    assert _is_linked(a, 'ConceptTarget', b1)
    if hasattr(b1, 'schema68'):
        assert _is_linked(b1, 'schema68', a)
    _safe_set(a, 'ConceptTarget', b2)
    assert _is_linked(a, 'ConceptTarget', b2)
    if hasattr(b1, 'schema68'):
        assert not _is_linked(b1, 'schema68', a)
    if hasattr(b2, 'schema68'):
        assert _is_linked(b2, 'schema68', a)
    _safe_set(a, 'ConceptTarget', None)
    assert not _is_linked(a, 'ConceptTarget', b2)
    if hasattr(b2, 'schema68'):
        assert not _is_linked(b2, 'schema68', a)
...
def test_assoc_connection51_link_reassign_clear():
    ...
    b1 = connection_XmlFileConnection(Encoding="sample_text", Guess=True, MaskXPattern="sample_text", XmlFilePath="sample_text", XsdFilePath="sample_text", inputModel=True, outputFilePath="sample_text")
    b2 = connection_XmlFileConnection(Encoding="sample_text_2", Guess=False, MaskXPattern="sample_text_2", XmlFilePath="sample_text_2", XsdFilePath="sample_text_2", inputModel=False, outputFilePath="sample_text_2")
...
def test_assoc_connection58_link_reassign_clear():
    a = connection_DatabaseConnection(AdditionalParams="sample_text", DBRootPath="sample_text", DatabaseType="sample_text", DatasourceName="sample_text", DbmsId="sample_text", DriverClass="sample_text", DriverJarPath="sample_text", FileFieldName="sample_text", NullChar="sample_text", Password="sample_text", Port="sample_text", ProductId="sample_text", SID="sample_text", SQLMode=True, ServerName="sample_text", SqlSynthax="sample_text", StandardSQL=True, StringQuote="sample_text", SystemSQL=True, URL="sample_text", UiSchema="sample_text", Username="sample_text", cdcTypeMode="sample_text", dbVersionString="sample_text")
    b1 = connection_CDCConnection()
    b2 = connection_CDCConnection()
...
def test_assoc_functionUnit65_link_reassign_clear():
    a = connection_SAPFunctionUnit(OutputTableName="sample_text", OutputType="sample_text")
    b1 = connection_SAPTestInputParameterTable()
    b2 = connection_SAPTestInputParameterTable()
    _safe_set(a, 'SAPFunctionUnit66', b1)
    assert _is_linked(a, 'SAPFunctionUnit66', b1)
    if hasattr(b1, 'TestInputParameterTable'):
        assert _is_linked(b1, 'TestInputParameterTable', a)
    _safe_set(a, 'SAPFunctionUnit66', b2)
    assert _is_linked(a, 'SAPFunctionUnit66', b2)
    if hasattr(b1, 'TestInputParameterTable'):
    ...
    _safe_set(a, 'SAPFunctionUnit66', None)
    assert not _is_linked(a, 'SAPFunctionUnit66', b2)
    if hasattr(b2, 'TestInputParameterTable'):
        assert not _is_linked(b2, 'TestInputParameterTable', a)


def test_assoc_group37_link_reassign_clear():
    a = connection_XmlFileConnection(Encoding="sample_text", Guess=True, MaskXPattern="sample_text", XmlFilePath="sample_text", XsdFilePath="sample_text", inputModel=True, outputFilePath="sample_text")
...
def test_assoc_group69_link_reassign_clear():
    ...
    b1 = connection_Concept(LoopExpression="sample_text", LoopLimit="sample_text", inputModel=True)
    b2 = connection_Concept(LoopExpression="sample_text_2", LoopLimit="sample_text_2", inputModel=False)
    _safe_set(a, 'connection_XMLFileNode71', b1)
    assert _is_linked(a, 'connection_XMLFileNode71', b1)
    if hasattr(b1, 'connection_Concept70'):
        assert _is_linked(b1, 'connection_Concept70', a)
    _safe_set(a, 'connection_XMLFileNode71', b2)
    assert _is_linked(a, 'connection_XMLFileNode71', b2)
    if hasattr(b1, 'connection_Concept70'):
        assert not _is_linked(b1, 'connection_Concept70', a)
    if hasattr(b2, 'connection_Concept70'):
        assert _is_linked(b2, 'connection_Concept70', a)
    _safe_set(a, 'connection_XMLFileNode71', None)
    assert not _is_linked(a, 'connection_XMLFileNode71', b2)
    if hasattr(b2, 'connection_Concept70'):
        assert not _is_linked(b2, 'connection_Concept70', a)
...
def test_assoc_loop75_link_reassign_clear():
    a = connection_XMLFileNode(Attribute="sample_text", DefaultValue="sample_text", Order=7, RelatedColumn="sample_text", Type="sample_text", XMLPath="sample_text")
    b1 = connection_Concept(LoopExpression="sample_text", LoopLimit="sample_text", inputModel=True)
    b2 = connection_Concept(LoopExpression="sample_text_2", LoopLimit="sample_text_2", inputModel=False)
    _safe_set(a, 'connection_XMLFileNode77', b1)
    assert _is_linked(a, 'connection_XMLFileNode77', b1)
    if hasattr(b1, 'connection_Concept76'):
        assert _is_linked(b1, 'connection_Concept76', a)
    _safe_set(a, 'connection_XMLFileNode77', b2)
    assert _is_linked(a, 'connection_XMLFileNode77', b2)
    if hasattr(b1, 'connection_Concept76'):
        assert not _is_linked(b1, 'connection_Concept76', a)
    if hasattr(b2, 'connection_Concept76'):
        assert _is_linked(b2, 'connection_Concept76', a)
    _safe_set(a, 'connection_XMLFileNode77', None)
    assert not _is_linked(a, 'connection_XMLFileNode77', b2)
    if hasattr(b2, 'connection_Concept76'):
        assert not _is_linked(b2, 'connection_Concept76', a)
...
def test_assoc_ownedDocument82_link_reassign_clear():
    a = connection_xml_TdXmlElementType(javaType="sample_text")
    b1 = xml_TdXmlSchema()
    b2 = xml_TdXmlSchema()
    _safe_set(a, 'connection_xml_TdXmlElementType83', b1)
    assert _is_linked(a, 'connection_xml_TdXmlElementType83', b1)
    if hasattr(b1, 'xml_TdXmlSchema'):
        assert _is_linked(b1, 'xml_TdXmlSchema', a)
    _safe_set(a, 'connection_xml_TdXmlElementType83', b2)
    assert _is_linked(a, 'connection_xml_TdXmlElementType83', b2)
    if hasattr(b1, 'xml_TdXmlSchema'):
        assert not _is_linked(b1, 'xml_TdXmlSchema', a)
    if hasattr(b2, 'xml_TdXmlSchema'):
        assert _is_linked(b2, 'xml_TdXmlSchema', a)
    _safe_set(a, 'connection_xml_TdXmlElementType83', None)
    assert not _is_linked(a, 'connection_xml_TdXmlElementType83', b2)
    if hasattr(b2, 'xml_TdXmlSchema'):
...
def test_assoc_root72_link_reassign_clear():
    a = connection_XMLFileNode(Attribute="sample_text", DefaultValue="sample_text", Order=7, RelatedColumn="sample_text", Type="sample_text", XMLPath="sample_text")
    b1 = connection_Concept(LoopExpression="sample_text", LoopLimit="sample_text", inputModel=True)
    b2 = connection_Concept(LoopExpression="sample_text_2", LoopLimit="sample_text_2", inputModel=False)
    _safe_set(a, 'connection_XMLFileNode74', b1)
    assert _is_linked(a, 'connection_XMLFileNode74', b1)
    if hasattr(b1, 'connection_Concept73'):
        assert _is_linked(b1, 'connection_Concept73', a)
    _safe_set(a, 'connection_XMLFileNode74', b2)
    assert _is_linked(a, 'connection_XMLFileNode74', b2)
    if hasattr(b1, 'connection_Concept73'):
        assert not _is_linked(b1, 'connection_Concept73', a)
    if hasattr(b2, 'connection_Concept73'):
        assert _is_linked(b2, 'connection_Concept73', a)
    _safe_set(a, 'connection_XMLFileNode74', None)
    assert not _is_linked(a, 'connection_XMLFileNode74', b2)
    if hasattr(b2, 'connection_Concept73'):
        assert not _is_linked(b2, 'connection_Concept73', a)


def test_assoc_root79_link_reassign_clear():
...
def test_assoc_schema78_link_reassign_clear():
    a = connection_ConceptTarget(RelativeLoopExpression="sample_text", targetName="sample_text")
    b1 = connection_Concept(LoopExpression="sample_text", LoopLimit="sample_text", inputModel=True)
    b2 = connection_Concept(LoopExpression="sample_text_2", LoopLimit="sample_text_2", inputModel=False)
...
def test_assoc_sqlDataType80_link_reassign_clear():
...
def test_assoc_subscribers60_link_reassign_clear():
    ...
    if hasattr(b1, 'connection_CDCType61'):
        assert _is_linked(b1, 'connection_CDCType61', a)
    _safe_set(a, 'connection_SubscriberTable', b2)
    assert _is_linked(a, 'connection_SubscriberTable', b2)
    if hasattr(b1, 'connection_CDCType61'):
        assert not _is_linked(b1, 'connection_CDCType61', a)
    if hasattr(b2, 'connection_CDCType61'):
        assert _is_linked(b2, 'connection_CDCType61', a)
    _safe_set(a, 'connection_SubscriberTable', None)
    assert not _is_linked(a, 'connection_SubscriberTable', b2)
    if hasattr(b2, 'connection_CDCType61'):
        assert not _is_linked(b2, 'connection_CDCType61', a)
...
def test_assoc_xmlContent84_link_reassign_clear():
    a = connection_xml_TdXmlElementType(javaType="sample_text")
    b1 = xml_TdXmlContent()
    b2 = xml_TdXmlContent()
    _safe_set(a, 'connection_xml_TdXmlElementType85', b1)
    assert _is_linked(a, 'connection_xml_TdXmlElementType85', b1)
    if hasattr(b1, 'xml_TdXmlContent'):
        assert _is_linked(b1, 'xml_TdXmlContent', a)
    _safe_set(a, 'connection_xml_TdXmlElementType85', b2)
    assert _is_linked(a, 'connection_xml_TdXmlElementType85', b2)
    if hasattr(b1, 'xml_TdXmlContent'):
        assert not _is_linked(b1, 'xml_TdXmlContent', a)
    if hasattr(b2, 'xml_TdXmlContent'):
        assert _is_linked(b2, 'xml_TdXmlContent', a)
    _safe_set(a, 'connection_xml_TdXmlElementType85', None)
    assert not _is_linked(a, 'connection_xml_TdXmlElementType85', b2)
    if hasattr(b2, 'xml_TdXmlContent'):
        assert not _is_linked(b2, 'xml_TdXmlContent', a)


def test_assoc_xsdElementDeclaration81_link_reassign_clear():
    a = connection_xml_TdXmlElementType(javaType="sample_text")
...
connection_Concept_strategy = st.builds(connection_Concept, LoopExpression=safe_text, LoopLimit=safe_text, inputModel=st.booleans())
@given(instance=connection_Concept_strategy)
@settings(max_examples=25)
...
connection_XmlFileConnection_strategy = st.builds(connection_XmlFileConnection, Encoding=safe_text, Guess=st.booleans(), MaskXPattern=safe_text, XmlFilePath=safe_text, XsdFilePath=safe_text, inputModel=st.booleans(), outputFilePath=safe_text)
@given(instance=connection_XmlFileConnection_strategy)
@settings(max_examples=25)
