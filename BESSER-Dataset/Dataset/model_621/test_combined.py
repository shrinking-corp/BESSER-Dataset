...
def test_assoc_book7_link_reassign_clear():
    ...
    _safe_set(a, 'tutorial_Loan8', b1)
    assert _is_linked(a, 'tutorial_Loan8', b1)
    if hasattr(b1, 'tutorial_Book'):
        assert _is_linked(b1, 'tutorial_Book', a)
    _safe_set(a, 'tutorial_Loan8', b2)
    assert _is_linked(a, 'tutorial_Loan8', b2)
    if hasattr(b1, 'tutorial_Book'):
        assert not _is_linked(b1, 'tutorial_Book', a)
    if hasattr(b2, 'tutorial_Book'):
        assert _is_linked(b2, 'tutorial_Book', a)
    _safe_set(a, 'tutorial_Loan8', None)
    assert not _is_linked(a, 'tutorial_Loan8', b2)
    if hasattr(b2, 'tutorial_Book'):
