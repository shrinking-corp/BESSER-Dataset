...
def test_ecore_ememoclass_is_not_abstract():
    assert not inspect.isabstract(ecore_EMemoClass)


def test_ecore_ememoclass_constructor_exists():
    assert callable(ecore_EMemoClass.__init__)


def test_ecore_ememoclass_constructor_args():
    sig = inspect.signature(ecore_EMemoClass.__init__)
    params = list(sig.parameters.keys())
    assert "instances" in params, "Missing parameter 'instances'"

def test_ecore_ememoclass_has_instances():
    assert hasattr(ecore_EMemoClass, "instances")
    descriptor = None
    for klass in ecore_EMemoClass.__mro__:
...
@given(instance=ecore_EMemoClass_strategy)
def test_ecore_ememoclass_instances_setter(instance):
