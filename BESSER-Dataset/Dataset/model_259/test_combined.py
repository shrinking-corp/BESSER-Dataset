...
def test_ecore_EClassifier_defaultValue_value_roundtrip():
    instance = ecore_EClassifier(defaultValue="sample_text", instanceClass="sample_text", instanceClassName="sample_text")
...
def test_ecore_EEnumLiteral_instance_value_roundtrip():
    instance = ecore_EEnumLiteral(instance="sample_text", value=7)
...
def test_assoc_eClassifiers48_link_reassign_clear():
    ...
    b1 = ecore_EClassifier(defaultValue="sample_text", instanceClass="sample_text", instanceClassName="sample_text")
    b2 = ecore_EClassifier(defaultValue="sample_text_2", instanceClass="sample_text_2", instanceClassName="sample_text_2")
...
def test_assoc_eEnum39_link_reassign_clear():
    a = ecore_EEnumLiteral(instance="sample_text", value=7)
    b1 = ecore_EEnum()
    b2 = ecore_EEnum()
...
def test_assoc_eExceptions45_link_reassign_clear():
    a = ecore_EClassifier(defaultValue="sample_text", instanceClass="sample_text", instanceClassName="sample_text")
    b1 = ecore_EOperation()
    b2 = ecore_EOperation()
    _safe_set(a, 'ecore_EClassifier', b1)
    assert _is_linked(a, 'ecore_EClassifier', b1)
    if hasattr(b1, 'ecore_EOperation46'):
        assert _is_linked(b1, 'ecore_EOperation46', a)
    _safe_set(a, 'ecore_EClassifier', b2)
    assert _is_linked(a, 'ecore_EClassifier', b2)
    if hasattr(b1, 'ecore_EOperation46'):
        assert not _is_linked(b1, 'ecore_EOperation46', a)
    if hasattr(b2, 'ecore_EOperation46'):
        assert _is_linked(b2, 'ecore_EOperation46', a)
    _safe_set(a, 'ecore_EClassifier', None)
    assert not _is_linked(a, 'ecore_EClassifier', b2)
    if hasattr(b2, 'ecore_EOperation46'):
        assert not _is_linked(b2, 'ecore_EOperation46', a)
...
def test_assoc_eOpposite59_link_reassign_clear():
    ...
    _safe_set(a, 'ecore_EReference58', b1)
    assert _is_linked(a, 'ecore_EReference58', b1)
    if hasattr(b1, 'ecore_EReference60'):
        assert _is_linked(b1, 'ecore_EReference60', a)
    _safe_set(a, 'ecore_EReference58', b2)
    assert _is_linked(a, 'ecore_EReference58', b2)
    if hasattr(b1, 'ecore_EReference60'):
        assert not _is_linked(b1, 'ecore_EReference60', a)
    if hasattr(b2, 'ecore_EReference60'):
        assert _is_linked(b2, 'ecore_EReference60', a)
    _safe_set(a, 'ecore_EReference58', None)
    assert not _is_linked(a, 'ecore_EReference58', b2)
    if hasattr(b2, 'ecore_EReference60'):
        assert not _is_linked(b2, 'ecore_EReference60', a)
...
def test_assoc_eReferenceType61_link_reassign_clear():
    ...
    b1 = ecore_EClass(abstract=True, interface=True)
    b2 = ecore_EClass(abstract=False, interface=False)
    _safe_set(a, 'ecore_EReference62', b1)
    assert _is_linked(a, 'ecore_EReference62', b1)
    if hasattr(b1, 'ecore_EClass63'):
        assert _is_linked(b1, 'ecore_EClass63', a)
    _safe_set(a, 'ecore_EReference62', b2)
    assert _is_linked(a, 'ecore_EReference62', b2)
    if hasattr(b1, 'ecore_EClass63'):
        assert not _is_linked(b1, 'ecore_EClass63', a)
    if hasattr(b2, 'ecore_EClass63'):
        assert _is_linked(b2, 'ecore_EClass63', a)
    _safe_set(a, 'ecore_EReference62', None)
    assert not _is_linked(a, 'ecore_EReference62', b2)
    if hasattr(b2, 'ecore_EClass63'):
        assert not _is_linked(b2, 'ecore_EClass63', a)
...
def test_assoc_eType66_link_reassign_clear():
    ...
    if hasattr(b1, 'ecore_EClassifier67'):
        assert _is_linked(b1, 'ecore_EClassifier67', a)
    _safe_set(a, 'ecore_ETypedElement', b2)
    assert _is_linked(a, 'ecore_ETypedElement', b2)
    if hasattr(b1, 'ecore_EClassifier67'):
        assert not _is_linked(b1, 'ecore_EClassifier67', a)
    if hasattr(b2, 'ecore_EClassifier67'):
        assert _is_linked(b2, 'ecore_EClassifier67', a)
    _safe_set(a, 'ecore_ETypedElement', None)
    assert not _is_linked(a, 'ecore_ETypedElement', b2)
    if hasattr(b2, 'ecore_EClassifier67'):
        assert not _is_linked(b2, 'ecore_EClassifier67', a)
...
ecore_EClassifier_strategy = st.builds(ecore_EClassifier, defaultValue=safe_text, instanceClass=safe_text, instanceClassName=safe_text)
@given(instance=ecore_EClassifier_strategy)
@settings(max_examples=25)
...
ecore_EEnumLiteral_strategy = st.builds(ecore_EEnumLiteral, instance=safe_text, value=st.integers())
@given(instance=ecore_EEnumLiteral_strategy)
@settings(max_examples=25)
