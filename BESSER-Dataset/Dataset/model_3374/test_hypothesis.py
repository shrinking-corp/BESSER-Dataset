...
def test_ddl_number_has_scale():
    assert hasattr(DDL_Number, "scale")
    descriptor = None
    for klass in DDL_Number.__mro__:
        if "scale" in klass.__dict__:
            descriptor = klass.__dict__["scale"]
            break
    assert isinstance(descriptor, property)

def test_ddl_number_has_precision():
    assert hasattr(DDL_Number, "precision")
    descriptor = None
    for klass in DDL_Number.__mro__:
        if "precision" in klass.__dict__:
            descriptor = klass.__dict__["precision"]
...
@given(instance=DDL_Number_strategy)
def test_ddl_number_scale_setter(instance):
...
@given(instance=DDL_Number_strategy)
def test_ddl_number_precision_setter(instance):
    original = instance.precision
    instance.precision = original
    assert instance.precision == original
