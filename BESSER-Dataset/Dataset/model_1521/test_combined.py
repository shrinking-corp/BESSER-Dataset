...
def test_hyp_sm6_state_is_not_abstract():
    assert not inspect.isabstract(sm6_State)


def test_hyp_sm6_state_constructor_exists():
    assert callable(sm6_State.__init__)


def test_hyp_sm6_state_constructor_args():
...
def test_hyp_sm6_statemachine_is_not_abstract():
    assert not inspect.isabstract(sm6_StateMachine)


def test_hyp_sm6_statemachine_constructor_exists():
    assert callable(sm6_StateMachine.__init__)


def test_hyp_sm6_statemachine_constructor_args():
    sig = inspect.signature(sm6_StateMachine.__init__)
    params = list(sig.parameters.keys())



def test_hyp_sm6_transition_is_not_abstract():
    assert not inspect.isabstract(sm6_Transition)


def test_hyp_sm6_transition_constructor_exists():
    assert callable(sm6_Transition.__init__)


def test_hyp_sm6_transition_constructor_args():
...
def test_hyp_sm6_state_isFinal_setter(instance):
...
@given(instance=sm6_State_strategy)
def test_hyp_sm6_state_name_setter(instance):
...
def test_hyp_sm6_transition_event_setter(instance):
...
def test_sm6_State_isFinal_value_roundtrip():
    instance = sm6_State(isFinal="sample_text", name="sample_text")
    assert instance.isFinal == "sample_text"
    instance.isFinal = "sample_text_2"
    assert instance.isFinal == "sample_text_2"


def test_sm6_State_name_value_roundtrip():
    instance = sm6_State(isFinal="sample_text", name="sample_text")
...
def test_sm6_Transition_event_value_roundtrip():
    instance = sm6_Transition(event="sample_text")
...
def test_assoc_edges2_link_reassign_clear():
    a = sm6_Transition(event="sample_text")
    b1 = sm6_StateMachine()
    b2 = sm6_StateMachine()
    _safe_set(a, 'sm6_Transition', b1)
    assert _is_linked(a, 'sm6_Transition', b1)
    if hasattr(b1, 'sm6_StateMachine3'):
        assert _is_linked(b1, 'sm6_StateMachine3', a)
    _safe_set(a, 'sm6_Transition', b2)
    assert _is_linked(a, 'sm6_Transition', b2)
    if hasattr(b1, 'sm6_StateMachine3'):
        assert not _is_linked(b1, 'sm6_StateMachine3', a)
    if hasattr(b2, 'sm6_StateMachine3'):
        assert _is_linked(b2, 'sm6_StateMachine3', a)
    _safe_set(a, 'sm6_Transition', None)
    assert not _is_linked(a, 'sm6_Transition', b2)
    if hasattr(b2, 'sm6_StateMachine3'):
        assert not _is_linked(b2, 'sm6_StateMachine3', a)


def test_assoc_incoming6_link_reassign_clear():
    a = sm6_Transition(event="sample_text")
    b1 = sm6_State(isFinal="sample_text", name="sample_text")
    b2 = sm6_State(isFinal="sample_text_2", name="sample_text_2")
...
def test_assoc_initialState0_link_reassign_clear():
    a = sm6_State(isFinal="sample_text", name="sample_text")
    b1 = sm6_StateMachine()
    b2 = sm6_StateMachine()
    _safe_set(a, 'sm6_State', b1)
    assert _is_linked(a, 'sm6_State', b1)
    if hasattr(b1, 'sm6_StateMachine'):
        assert _is_linked(b1, 'sm6_StateMachine', a)
    _safe_set(a, 'sm6_State', b2)
    assert _is_linked(a, 'sm6_State', b2)
    if hasattr(b1, 'sm6_StateMachine'):
        assert not _is_linked(b1, 'sm6_StateMachine', a)
    if hasattr(b2, 'sm6_StateMachine'):
        assert _is_linked(b2, 'sm6_StateMachine', a)
    _safe_set(a, 'sm6_State', None)
    assert not _is_linked(a, 'sm6_State', b2)
    if hasattr(b2, 'sm6_StateMachine'):
        assert not _is_linked(b2, 'sm6_StateMachine', a)
...
sm6_State_strategy = st.builds(sm6_State, isFinal=safe_text, name=safe_text)
@given(instance=sm6_State_strategy)
...
sm6_StateMachine_strategy = st.builds(sm6_StateMachine)
...
sm6_Transition_strategy = st.builds(sm6_Transition, event=safe_text)
@given(instance=sm6_Transition_strategy)
@settings(max_examples=25)
