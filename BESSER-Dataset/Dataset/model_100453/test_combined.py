...
def test_hyp_stext_parenthesizedexpression_is_not_abstract():
    assert not inspect.isabstract(stext_ParenthesizedExpression)


def test_hyp_stext_parenthesizedexpression_constructor_exists():
    assert callable(stext_ParenthesizedExpression.__init__)


def test_hyp_stext_parenthesizedexpression_constructor_args():
...
def test_stext_ParenthesizedExpression_isa_Expression():
    instance = stext_ParenthesizedExpression()
    assert isinstance(instance, Expression)
...
def test_assoc_args91_link_reassign_clear():
    a = stext_FeatureCall(operationCall=True)
    b1 = stext_Expression()
    b2 = stext_Expression()
    _safe_set(a, 'stext_FeatureCall92', {b1})
    assert _is_linked(a, 'stext_FeatureCall92', b1)
    if hasattr(b1, 'stext_Expression93'):
        assert _is_linked(b1, 'stext_Expression93', a)
    _safe_set(a, 'stext_FeatureCall92', {b2})
    assert _is_linked(a, 'stext_FeatureCall92', b2)
    if hasattr(b1, 'stext_Expression93'):
        assert not _is_linked(b1, 'stext_Expression93', a)
    if hasattr(b2, 'stext_Expression93'):
        assert _is_linked(b2, 'stext_Expression93', a)
    _safe_set(a, 'stext_FeatureCall92', set())
    assert not _is_linked(a, 'stext_FeatureCall92', b2)
    if hasattr(b2, 'stext_Expression93'):
...
def test_assoc_args96_link_reassign_clear():
    ...
    b1 = stext_Expression()
    b2 = stext_Expression()
    _safe_set(a, 'stext_ElementReferenceExpression97', {b1})
    assert _is_linked(a, 'stext_ElementReferenceExpression97', b1)
    if hasattr(b1, 'stext_Expression98'):
        assert _is_linked(b1, 'stext_Expression98', a)
    _safe_set(a, 'stext_ElementReferenceExpression97', {b2})
    assert _is_linked(a, 'stext_ElementReferenceExpression97', b2)
    if hasattr(b1, 'stext_Expression98'):
        assert not _is_linked(b1, 'stext_Expression98', a)
    if hasattr(b2, 'stext_Expression98'):
        assert _is_linked(b2, 'stext_Expression98', a)
    _safe_set(a, 'stext_ElementReferenceExpression97', set())
    assert not _is_linked(a, 'stext_ElementReferenceExpression97', b2)
    if hasattr(b2, 'stext_Expression98'):
        assert not _is_linked(b2, 'stext_Expression98', a)


def test_assoc_expression26_link_reassign_clear():
    a = stext_AssignmentExpression(operator="sample_text")
    b1 = stext_Expression()
    b2 = stext_Expression()
    _safe_set(a, 'stext_AssignmentExpression27', b1)
    assert _is_linked(a, 'stext_AssignmentExpression27', b1)
    if hasattr(b1, 'stext_Expression28'):
        assert _is_linked(b1, 'stext_Expression28', a)
    _safe_set(a, 'stext_AssignmentExpression27', b2)
    assert _is_linked(a, 'stext_AssignmentExpression27', b2)
    if hasattr(b1, 'stext_Expression28'):
        assert not _is_linked(b1, 'stext_Expression28', a)
    if hasattr(b2, 'stext_Expression28'):
        assert _is_linked(b2, 'stext_Expression28', a)
    _safe_set(a, 'stext_AssignmentExpression27', None)
    assert not _is_linked(a, 'stext_AssignmentExpression27', b2)
    if hasattr(b2, 'stext_Expression28'):
        assert not _is_linked(b2, 'stext_Expression28', a)


def test_assoc_feature89_link_reassign_clear():
    a = stext_FeatureCall(operationCall=True)
    b1 = stext_EObject()
    b2 = stext_EObject()
    _safe_set(a, 'stext_FeatureCall90', b1)
    assert _is_linked(a, 'stext_FeatureCall90', b1)
    if hasattr(b1, 'stext_EObject'):
        assert _is_linked(b1, 'stext_EObject', a)
    _safe_set(a, 'stext_FeatureCall90', b2)
    assert _is_linked(a, 'stext_FeatureCall90', b2)
    if hasattr(b1, 'stext_EObject'):
        assert not _is_linked(b1, 'stext_EObject', a)
    if hasattr(b2, 'stext_EObject'):
        assert _is_linked(b2, 'stext_EObject', a)
    _safe_set(a, 'stext_FeatureCall90', None)
    assert not _is_linked(a, 'stext_FeatureCall90', b2)
    if hasattr(b2, 'stext_EObject'):
...
def test_assoc_leftOperand64_link_reassign_clear():
    ...
    if hasattr(b1, 'stext_Expression65'):
        assert _is_linked(b1, 'stext_Expression65', a)
    _safe_set(a, 'stext_LogicalRelationExpression', b2)
    assert _is_linked(a, 'stext_LogicalRelationExpression', b2)
    if hasattr(b1, 'stext_Expression65'):
        assert not _is_linked(b1, 'stext_Expression65', a)
    if hasattr(b2, 'stext_Expression65'):
        assert _is_linked(b2, 'stext_Expression65', a)
    _safe_set(a, 'stext_LogicalRelationExpression', None)
    assert not _is_linked(a, 'stext_LogicalRelationExpression', b2)
    if hasattr(b2, 'stext_Expression65'):
        assert not _is_linked(b2, 'stext_Expression65', a)


def test_assoc_leftOperand69_link_reassign_clear():
    ...
    if hasattr(b1, 'stext_Expression70'):
        assert _is_linked(b1, 'stext_Expression70', a)
    _safe_set(a, 'stext_ShiftExpression', b2)
    assert _is_linked(a, 'stext_ShiftExpression', b2)
    if hasattr(b1, 'stext_Expression70'):
        assert not _is_linked(b1, 'stext_Expression70', a)
    if hasattr(b2, 'stext_Expression70'):
        assert _is_linked(b2, 'stext_Expression70', a)
    _safe_set(a, 'stext_ShiftExpression', None)
    assert not _is_linked(a, 'stext_ShiftExpression', b2)
    if hasattr(b2, 'stext_Expression70'):
        assert not _is_linked(b2, 'stext_Expression70', a)


def test_assoc_leftOperand74_link_reassign_clear():
    ...
    if hasattr(b1, 'stext_Expression75'):
        assert _is_linked(b1, 'stext_Expression75', a)
    _safe_set(a, 'stext_NumericalAddSubtractExpression', b2)
    assert _is_linked(a, 'stext_NumericalAddSubtractExpression', b2)
    if hasattr(b1, 'stext_Expression75'):
        assert not _is_linked(b1, 'stext_Expression75', a)
    if hasattr(b2, 'stext_Expression75'):
        assert _is_linked(b2, 'stext_Expression75', a)
    _safe_set(a, 'stext_NumericalAddSubtractExpression', None)
    assert not _is_linked(a, 'stext_NumericalAddSubtractExpression', b2)
    if hasattr(b2, 'stext_Expression75'):
        assert not _is_linked(b2, 'stext_Expression75', a)


def test_assoc_leftOperand79_link_reassign_clear():
    ...
    if hasattr(b1, 'stext_Expression80'):
        assert _is_linked(b1, 'stext_Expression80', a)
    _safe_set(a, 'stext_NumericalMultiplyDivideExpression', b2)
    assert _is_linked(a, 'stext_NumericalMultiplyDivideExpression', b2)
    if hasattr(b1, 'stext_Expression80'):
        assert not _is_linked(b1, 'stext_Expression80', a)
    if hasattr(b2, 'stext_Expression80'):
        assert _is_linked(b2, 'stext_Expression80', a)
    ...
    if hasattr(b2, 'stext_Expression80'):
        assert not _is_linked(b2, 'stext_Expression80', a)


def test_assoc_operand84_link_reassign_clear():
    ...
    if hasattr(b1, 'stext_Expression85'):
        assert _is_linked(b1, 'stext_Expression85', a)
    _safe_set(a, 'stext_NumericalUnaryExpression', b2)
    assert _is_linked(a, 'stext_NumericalUnaryExpression', b2)
    if hasattr(b1, 'stext_Expression85'):
        assert not _is_linked(b1, 'stext_Expression85', a)
    if hasattr(b2, 'stext_Expression85'):
        assert _is_linked(b2, 'stext_Expression85', a)
    _safe_set(a, 'stext_NumericalUnaryExpression', None)
    assert not _is_linked(a, 'stext_NumericalUnaryExpression', b2)
    if hasattr(b2, 'stext_Expression85'):
        assert not _is_linked(b2, 'stext_Expression85', a)
...
def test_assoc_reference94_link_reassign_clear():
    a = stext_ElementReferenceExpression(operationCall=True)
    b1 = stext_EObject()
    b2 = stext_EObject()
    _safe_set(a, 'stext_ElementReferenceExpression', b1)
    assert _is_linked(a, 'stext_ElementReferenceExpression', b1)
    if hasattr(b1, 'stext_EObject95'):
        assert _is_linked(b1, 'stext_EObject95', a)
    _safe_set(a, 'stext_ElementReferenceExpression', b2)
    assert _is_linked(a, 'stext_ElementReferenceExpression', b2)
    if hasattr(b1, 'stext_EObject95'):
        assert not _is_linked(b1, 'stext_EObject95', a)
    if hasattr(b2, 'stext_EObject95'):
        assert _is_linked(b2, 'stext_EObject95', a)
    _safe_set(a, 'stext_ElementReferenceExpression', None)
    assert not _is_linked(a, 'stext_ElementReferenceExpression', b2)
    if hasattr(b2, 'stext_EObject95'):
        assert not _is_linked(b2, 'stext_EObject95', a)


def test_assoc_rightOperand66_link_reassign_clear():
    a = stext_LogicalRelationExpression(operator="sample_text")
    b1 = stext_Expression()
    b2 = stext_Expression()
    _safe_set(a, 'stext_LogicalRelationExpression67', b1)
    assert _is_linked(a, 'stext_LogicalRelationExpression67', b1)
    if hasattr(b1, 'stext_Expression68'):
        assert _is_linked(b1, 'stext_Expression68', a)
    _safe_set(a, 'stext_LogicalRelationExpression67', b2)
    assert _is_linked(a, 'stext_LogicalRelationExpression67', b2)
    if hasattr(b1, 'stext_Expression68'):
        assert not _is_linked(b1, 'stext_Expression68', a)
    if hasattr(b2, 'stext_Expression68'):
        assert _is_linked(b2, 'stext_Expression68', a)
    _safe_set(a, 'stext_LogicalRelationExpression67', None)
    assert not _is_linked(a, 'stext_LogicalRelationExpression67', b2)
    if hasattr(b2, 'stext_Expression68'):
        assert not _is_linked(b2, 'stext_Expression68', a)


def test_assoc_rightOperand71_link_reassign_clear():
    a = stext_ShiftExpression(operator="sample_text")
    b1 = stext_Expression()
    b2 = stext_Expression()
    _safe_set(a, 'stext_ShiftExpression72', b1)
    assert _is_linked(a, 'stext_ShiftExpression72', b1)
    if hasattr(b1, 'stext_Expression73'):
        assert _is_linked(b1, 'stext_Expression73', a)
    _safe_set(a, 'stext_ShiftExpression72', b2)
    assert _is_linked(a, 'stext_ShiftExpression72', b2)
    if hasattr(b1, 'stext_Expression73'):
        assert not _is_linked(b1, 'stext_Expression73', a)
    if hasattr(b2, 'stext_Expression73'):
        assert _is_linked(b2, 'stext_Expression73', a)
    _safe_set(a, 'stext_ShiftExpression72', None)
    assert not _is_linked(a, 'stext_ShiftExpression72', b2)
    if hasattr(b2, 'stext_Expression73'):
        assert not _is_linked(b2, 'stext_Expression73', a)


def test_assoc_rightOperand76_link_reassign_clear():
    a = stext_NumericalAddSubtractExpression(operator="sample_text")
    b1 = stext_Expression()
    b2 = stext_Expression()
    _safe_set(a, 'stext_NumericalAddSubtractExpression77', b1)
    assert _is_linked(a, 'stext_NumericalAddSubtractExpression77', b1)
    if hasattr(b1, 'stext_Expression78'):
        assert _is_linked(b1, 'stext_Expression78', a)
    _safe_set(a, 'stext_NumericalAddSubtractExpression77', b2)
    assert _is_linked(a, 'stext_NumericalAddSubtractExpression77', b2)
    if hasattr(b1, 'stext_Expression78'):
        assert not _is_linked(b1, 'stext_Expression78', a)
    if hasattr(b2, 'stext_Expression78'):
        assert _is_linked(b2, 'stext_Expression78', a)
    _safe_set(a, 'stext_NumericalAddSubtractExpression77', None)
    assert not _is_linked(a, 'stext_NumericalAddSubtractExpression77', b2)
    if hasattr(b2, 'stext_Expression78'):
        assert not _is_linked(b2, 'stext_Expression78', a)


def test_assoc_rightOperand81_link_reassign_clear():
    a = stext_NumericalMultiplyDivideExpression(operator="sample_text")
    b1 = stext_Expression()
    b2 = stext_Expression()
    _safe_set(a, 'stext_NumericalMultiplyDivideExpression82', b1)
    assert _is_linked(a, 'stext_NumericalMultiplyDivideExpression82', b1)
    if hasattr(b1, 'stext_Expression83'):
        assert _is_linked(b1, 'stext_Expression83', a)
    _safe_set(a, 'stext_NumericalMultiplyDivideExpression82', b2)
    assert _is_linked(a, 'stext_NumericalMultiplyDivideExpression82', b2)
    if hasattr(b1, 'stext_Expression83'):
        assert not _is_linked(b1, 'stext_Expression83', a)
    if hasattr(b2, 'stext_Expression83'):
        assert _is_linked(b2, 'stext_Expression83', a)
    _safe_set(a, 'stext_NumericalMultiplyDivideExpression82', None)
    assert not _is_linked(a, 'stext_NumericalMultiplyDivideExpression82', b2)
    if hasattr(b2, 'stext_Expression83'):
...
def test_assoc_value11_link_reassign_clear():
    ...
    if hasattr(b1, 'stext_Expression12'):
        assert _is_linked(b1, 'stext_Expression12', a)
    _safe_set(a, 'stext_TimeEventSpec', b2)
    assert _is_linked(a, 'stext_TimeEventSpec', b2)
    if hasattr(b1, 'stext_Expression12'):
        assert not _is_linked(b1, 'stext_Expression12', a)
    if hasattr(b2, 'stext_Expression12'):
        assert _is_linked(b2, 'stext_Expression12', a)
    _safe_set(a, 'stext_TimeEventSpec', None)
    assert not _is_linked(a, 'stext_TimeEventSpec', b2)
    if hasattr(b2, 'stext_Expression12'):
        assert not _is_linked(b2, 'stext_Expression12', a)
...
def test_assoc_varRef24_link_reassign_clear():
    ...
    if hasattr(b1, 'stext_Expression25'):
        assert _is_linked(b1, 'stext_Expression25', a)
    _safe_set(a, 'stext_AssignmentExpression', b2)
    assert _is_linked(a, 'stext_AssignmentExpression', b2)
    if hasattr(b1, 'stext_Expression25'):
        assert not _is_linked(b1, 'stext_Expression25', a)
    if hasattr(b2, 'stext_Expression25'):
        assert _is_linked(b2, 'stext_Expression25', a)
    _safe_set(a, 'stext_AssignmentExpression', None)
    assert not _is_linked(a, 'stext_AssignmentExpression', b2)
    if hasattr(b2, 'stext_Expression25'):
        assert not _is_linked(b2, 'stext_Expression25', a)
...
stext_ParenthesizedExpression_strategy = st.builds(stext_ParenthesizedExpression)
