...
def test_STUDENT_id_value_roundtrip():
    instance = STUDENT(id="sample_text")
    assert instance.id == "sample_text"
    instance.id = "sample_text_2"
    assert instance.id == "sample_text_2"
...
def test_assoc_FACULTY_STUDENT_link_reassign_clear():
    a = STUDENT(id="sample_text")
    b1 = FACULTY(id="sample_text", password="sample_text")
    b2 = FACULTY(id="sample_text_2", password="sample_text_2")
    _safe_set(a, 'fACULTY31', {b1})
    assert _is_linked(a, 'fACULTY31', b1)
    if hasattr(b1, 'sTUDENT30'):
        assert _is_linked(b1, 'sTUDENT30', a)
    _safe_set(a, 'fACULTY31', {b2})
    assert _is_linked(a, 'fACULTY31', b2)
    if hasattr(b1, 'sTUDENT30'):
        assert not _is_linked(b1, 'sTUDENT30', a)
    if hasattr(b2, 'sTUDENT30'):
        assert _is_linked(b2, 'sTUDENT30', a)
    _safe_set(a, 'fACULTY31', set())
    assert not _is_linked(a, 'fACULTY31', b2)
    if hasattr(b2, 'sTUDENT30'):
        assert not _is_linked(b2, 'sTUDENT30', a)
...
STUDENT_strategy = st.builds(STUDENT, id=safe_text)
