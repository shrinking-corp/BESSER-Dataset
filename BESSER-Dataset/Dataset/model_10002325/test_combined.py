...
def test_assoc_FACULTY_ADMIN_link_reassign_clear():
    ...
    if hasattr(b1, 'fACULTY29'):
        assert _is_linked(b1, 'fACULTY29', a)
    _safe_set(a, 'aDMIN28', b2)
    assert _is_linked(a, 'aDMIN28', b2)
    if hasattr(b1, 'fACULTY29'):
        assert not _is_linked(b1, 'fACULTY29', a)
    if hasattr(b2, 'fACULTY29'):
        assert _is_linked(b2, 'fACULTY29', a)
    _safe_set(a, 'aDMIN28', None)
    assert not _is_linked(a, 'aDMIN28', b2)
    if hasattr(b2, 'fACULTY29'):
        assert not _is_linked(b2, 'fACULTY29', a)
...
def test_assoc_FACULTY_STUDENT_link_reassign_clear():
    ...
    _safe_set(a, 'fACULTY27', {b1})
    assert _is_linked(a, 'fACULTY27', b1)
    if hasattr(b1, 'sTUDENT26'):
        assert _is_linked(b1, 'sTUDENT26', a)
    _safe_set(a, 'fACULTY27', {b2})
    assert _is_linked(a, 'fACULTY27', b2)
    if hasattr(b1, 'sTUDENT26'):
        assert not _is_linked(b1, 'sTUDENT26', a)
    if hasattr(b2, 'sTUDENT26'):
        assert _is_linked(b2, 'sTUDENT26', a)
    _safe_set(a, 'fACULTY27', set())
    assert not _is_linked(a, 'fACULTY27', b2)
    if hasattr(b2, 'sTUDENT26'):
...
def test_assoc_PARENT_ADMIN_link_reassign_clear():
    ...
    if hasattr(b1, 'pARENT33'):
        assert _is_linked(b1, 'pARENT33', a)
    _safe_set(a, 'aDMIN32', b2)
    assert _is_linked(a, 'aDMIN32', b2)
    if hasattr(b1, 'pARENT33'):
        assert not _is_linked(b1, 'pARENT33', a)
    if hasattr(b2, 'pARENT33'):
        assert _is_linked(b2, 'pARENT33', a)
    _safe_set(a, 'aDMIN32', None)
    assert not _is_linked(a, 'aDMIN32', b2)
    if hasattr(b2, 'pARENT33'):
        assert not _is_linked(b2, 'pARENT33', a)
