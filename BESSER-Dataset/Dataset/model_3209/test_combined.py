...
def test_hyp_llp_block_is_not_abstract():
    assert not inspect.isabstract(llp_Block)


def test_hyp_llp_block_constructor_exists():
    assert callable(llp_Block.__init__)


def test_hyp_llp_block_constructor_args():
    sig = inspect.signature(llp_Block.__init__)
    params = list(sig.parameters.keys())



def test_hyp_controlflowinstruction_is_not_abstract():
    assert not inspect.isabstract(ControlFlowInstruction)


def test_hyp_controlflowinstruction_constructor_exists():
    assert callable(ControlFlowInstruction.__init__)


def test_hyp_controlflowinstruction_constructor_args():
    sig = inspect.signature(ControlFlowInstruction.__init__)
    params = list(sig.parameters.keys())



def test_hyp_llp_skipinstruction_is_not_abstract():
    assert not inspect.isabstract(llp_SkipInstruction)


def test_hyp_llp_skipinstruction_constructor_exists():
    assert callable(llp_SkipInstruction.__init__)


def test_hyp_llp_skipinstruction_constructor_args():
    sig = inspect.signature(llp_SkipInstruction.__init__)
    params = list(sig.parameters.keys())



def test_hyp_llp_repetitioninstruction_is_not_abstract():
    assert not inspect.isabstract(llp_RepetitionInstruction)


def test_hyp_llp_repetitioninstruction_constructor_exists():
    assert callable(llp_RepetitionInstruction.__init__)


def test_hyp_llp_repetitioninstruction_constructor_args():
...
def test_hyp_llp_parenthesisinstruction_is_not_abstract():
    assert not inspect.isabstract(llp_ParenthesisInstruction)


def test_hyp_llp_parenthesisinstruction_constructor_exists():
    assert callable(llp_ParenthesisInstruction.__init__)


def test_hyp_llp_parenthesisinstruction_constructor_args():
    sig = inspect.signature(llp_ParenthesisInstruction.__init__)
    params = list(sig.parameters.keys())



def test_hyp_llp_controlflowbranchinginstruction_is_not_abstract():
    assert not inspect.isabstract(llp_ControlFlowBranchingInstruction)


def test_hyp_llp_controlflowbranchinginstruction_constructor_exists():
    assert callable(llp_ControlFlowBranchingInstruction.__init__)


def test_hyp_llp_controlflowbranchinginstruction_constructor_args():
    sig = inspect.signature(llp_ControlFlowBranchingInstruction.__init__)
    params = list(sig.parameters.keys())



def test_hyp_synchronisationinstruction_is_not_abstract():
    assert not inspect.isabstract(SynchronisationInstruction)


def test_hyp_synchronisationinstruction_constructor_exists():
    assert callable(SynchronisationInstruction.__init__)


def test_hyp_synchronisationinstruction_constructor_args():
    sig = inspect.signature(SynchronisationInstruction.__init__)
    params = list(sig.parameters.keys())



def test_hyp_llp_unlockinstruction_is_not_abstract():
    assert not inspect.isabstract(llp_UnlockInstruction)


def test_hyp_llp_unlockinstruction_constructor_exists():
    assert callable(llp_UnlockInstruction.__init__)


def test_hyp_llp_unlockinstruction_constructor_args():
    sig = inspect.signature(llp_UnlockInstruction.__init__)
    params = list(sig.parameters.keys())



def test_hyp_llp_lockinstruction_is_not_abstract():
    assert not inspect.isabstract(llp_LockInstruction)


def test_hyp_llp_lockinstruction_constructor_exists():
    assert callable(llp_LockInstruction.__init__)


def test_hyp_llp_lockinstruction_constructor_args():
    sig = inspect.signature(llp_LockInstruction.__init__)
    params = list(sig.parameters.keys())



def test_hyp_cacheinstruction_is_not_abstract():
    assert not inspect.isabstract(CacheInstruction)


def test_hyp_cacheinstruction_constructor_exists():
    assert callable(CacheInstruction.__init__)


def test_hyp_cacheinstruction_constructor_args():
    sig = inspect.signature(CacheInstruction.__init__)
    params = list(sig.parameters.keys())



def test_hyp_llp_commitinstruction_is_not_abstract():
    assert not inspect.isabstract(llp_CommitInstruction)


def test_hyp_llp_commitinstruction_constructor_exists():
    assert callable(llp_CommitInstruction.__init__)


def test_hyp_llp_commitinstruction_constructor_args():
    sig = inspect.signature(llp_CommitInstruction.__init__)
    params = list(sig.parameters.keys())



def test_hyp_llp_memoryreference_is_not_abstract():
    assert not inspect.isabstract(llp_MemoryReference)


def test_hyp_llp_memoryreference_constructor_exists():
    assert callable(llp_MemoryReference.__init__)


def test_hyp_llp_memoryreference_constructor_args():
...
def test_hyp_dataaccesspattern_is_not_abstract():
    assert not inspect.isabstract(DataAccessPattern)


def test_hyp_dataaccesspattern_constructor_exists():
    assert callable(DataAccessPattern.__init__)


def test_hyp_dataaccesspattern_constructor_args():
    sig = inspect.signature(DataAccessPattern.__init__)
    params = list(sig.parameters.keys())



def test_hyp_llp_cacheinstruction_is_not_abstract():
    assert not inspect.isabstract(llp_CacheInstruction)


def test_hyp_llp_cacheinstruction_constructor_exists():
    assert callable(llp_CacheInstruction.__init__)


def test_hyp_llp_cacheinstruction_constructor_args():
    sig = inspect.signature(llp_CacheInstruction.__init__)
    params = list(sig.parameters.keys())



def test_hyp_llp_spawninstruction_is_not_abstract():
    assert not inspect.isabstract(llp_SpawnInstruction)


def test_hyp_llp_spawninstruction_constructor_exists():
    assert callable(llp_SpawnInstruction.__init__)


def test_hyp_llp_spawninstruction_constructor_args():
    sig = inspect.signature(llp_SpawnInstruction.__init__)
    params = list(sig.parameters.keys())



def test_hyp_llp_controlflowinstruction_is_not_abstract():
    assert not inspect.isabstract(llp_ControlFlowInstruction)


def test_hyp_llp_controlflowinstruction_constructor_exists():
    assert callable(llp_ControlFlowInstruction.__init__)


def test_hyp_llp_controlflowinstruction_constructor_args():
    sig = inspect.signature(llp_ControlFlowInstruction.__init__)
    params = list(sig.parameters.keys())



def test_hyp_llp_synchronisationinstruction_is_not_abstract():
    assert not inspect.isabstract(llp_SynchronisationInstruction)


def test_hyp_llp_synchronisationinstruction_constructor_exists():
    assert callable(llp_SynchronisationInstruction.__init__)


def test_hyp_llp_synchronisationinstruction_constructor_args():
    sig = inspect.signature(llp_SynchronisationInstruction.__init__)
    params = list(sig.parameters.keys())



def test_hyp_llp_ioinstruction_is_not_abstract():
    assert not inspect.isabstract(llp_IOInstruction)


def test_hyp_llp_ioinstruction_constructor_exists():
    assert callable(llp_IOInstruction.__init__)


def test_hyp_llp_ioinstruction_constructor_args():
    sig = inspect.signature(llp_IOInstruction.__init__)
    params = list(sig.parameters.keys())



def test_hyp_ioinstruction_is_not_abstract():
    assert not inspect.isabstract(IOInstruction)


def test_hyp_ioinstruction_constructor_exists():
    assert callable(IOInstruction.__init__)


def test_hyp_ioinstruction_constructor_args():
    sig = inspect.signature(IOInstruction.__init__)
    params = list(sig.parameters.keys())



def test_hyp_llp_writeinstruction_is_not_abstract():
    assert not inspect.isabstract(llp_WriteInstruction)


def test_hyp_llp_writeinstruction_constructor_exists():
    assert callable(llp_WriteInstruction.__init__)


def test_hyp_llp_writeinstruction_constructor_args():
    sig = inspect.signature(llp_WriteInstruction.__init__)
    params = list(sig.parameters.keys())



def test_hyp_llp_readinstruction_is_not_abstract():
    assert not inspect.isabstract(llp_ReadInstruction)


def test_hyp_llp_readinstruction_constructor_exists():
    assert callable(llp_ReadInstruction.__init__)


def test_hyp_llp_readinstruction_constructor_args():
    sig = inspect.signature(llp_ReadInstruction.__init__)
    params = list(sig.parameters.keys())



def test_hyp_llp_dataaccesspattern_is_not_abstract():
    assert not inspect.isabstract(llp_DataAccessPattern)


def test_hyp_llp_dataaccesspattern_constructor_exists():
    assert callable(llp_DataAccessPattern.__init__)


def test_hyp_llp_dataaccesspattern_constructor_args():
    sig = inspect.signature(llp_DataAccessPattern.__init__)
    params = list(sig.parameters.keys())



def test_hyp_llp_task_is_not_abstract():
    assert not inspect.isabstract(llp_Task)


def test_hyp_llp_task_constructor_exists():
    assert callable(llp_Task.__init__)


def test_hyp_llp_task_constructor_args():
...
def test_hyp_llp_lowlevelprogram_is_not_abstract():
    assert not inspect.isabstract(llp_LowLevelProgram)


def test_hyp_llp_lowlevelprogram_constructor_exists():
    assert callable(llp_LowLevelProgram.__init__)


def test_hyp_llp_lowlevelprogram_constructor_args():
...
def test_hyp_llp_repetitioninstruction_numberOfRepetitions_setter(instance):
...
def test_hyp_llp_memoryreference_address_setter(instance):
...
def test_hyp_llp_task_name_setter(instance):
