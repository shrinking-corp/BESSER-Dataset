...
def test_wt_port_has_isPublic():
    assert hasattr(WT_Port, "isPublic")
    descriptor = None
...
def test_wt_statemachine_has_isPublic():
    assert hasattr(WT_StateMachine, "isPublic")
    descriptor = None
...
def test_wt_connector_is_not_abstract():
    assert not inspect.isabstract(WT_Connector)


def test_wt_connector_constructor_exists():
    assert callable(WT_Connector.__init__)


def test_wt_connector_constructor_args():
    sig = inspect.signature(WT_Connector.__init__)
    params = list(sig.parameters.keys())
...
WT_Connector_strategy = st.builds(
    WT_Connector,
)
...
@given(instance=WT_Port_strategy)
def test_wt_port_isPublic_setter(instance):
...
@given(instance=WT_StateMachine_strategy)
def test_wt_statemachine_isPublic_setter(instance):
    original = instance.isPublic
    instance.isPublic = original
    assert instance.isPublic == original
