...
def test_hyp_graph_stringtoobjectmapentry_is_not_abstract():
    assert not inspect.isabstract(graph_StringToObjectMapEntry)


def test_hyp_graph_stringtoobjectmapentry_constructor_exists():
    assert callable(graph_StringToObjectMapEntry.__init__)


def test_hyp_graph_stringtoobjectmapentry_constructor_args():
...
def test_hyp_graph_stringtoobjectmapentry_key_setter(instance):
...
@given(instance=graph_StringToObjectMapEntry_strategy)
def test_hyp_graph_stringtoobjectmapentry_value_setter(instance):
...
def test_graph_StringToObjectMapEntry_key_value_roundtrip():
    instance = graph_StringToObjectMapEntry(key="sample_text", value="sample_text")
    assert instance.key == "sample_text"
    instance.key = "sample_text_2"
    assert instance.key == "sample_text_2"


def test_graph_StringToObjectMapEntry_value_value_roundtrip():
    instance = graph_StringToObjectMapEntry(key="sample_text", value="sample_text")
...
def test_assoc_layoutOptions19_link_reassign_clear():
    a = graph_StringToObjectMapEntry(key="sample_text", value="sample_text")
    b1 = graph_GLayouting(layout="sample_text")
    b2 = graph_GLayouting(layout="sample_text_2")
    _safe_set(a, 'graph_StringToObjectMapEntry20', b1)
    assert _is_linked(a, 'graph_StringToObjectMapEntry20', b1)
    if hasattr(b1, 'graph_GLayouting'):
        assert _is_linked(b1, 'graph_GLayouting', a)
    _safe_set(a, 'graph_StringToObjectMapEntry20', b2)
    assert _is_linked(a, 'graph_StringToObjectMapEntry20', b2)
    if hasattr(b1, 'graph_GLayouting'):
        assert not _is_linked(b1, 'graph_GLayouting', a)
    if hasattr(b2, 'graph_GLayouting'):
        assert _is_linked(b2, 'graph_GLayouting', a)
    _safe_set(a, 'graph_StringToObjectMapEntry20', None)
    assert not _is_linked(a, 'graph_StringToObjectMapEntry20', b2)
    if hasattr(b2, 'graph_GLayouting'):
        assert not _is_linked(b2, 'graph_GLayouting', a)
...
def test_assoc_layoutOptions5_link_reassign_clear():
    ...
    b1 = graph_GGraph()
    b2 = graph_GGraph()
    _safe_set(a, 'graph_StringToObjectMapEntry', b1)
    assert _is_linked(a, 'graph_StringToObjectMapEntry', b1)
    if hasattr(b1, 'graph_GGraph'):
        assert _is_linked(b1, 'graph_GGraph', a)
    _safe_set(a, 'graph_StringToObjectMapEntry', b2)
    assert _is_linked(a, 'graph_StringToObjectMapEntry', b2)
    if hasattr(b1, 'graph_GGraph'):
        assert not _is_linked(b1, 'graph_GGraph', a)
    if hasattr(b2, 'graph_GGraph'):
        assert _is_linked(b2, 'graph_GGraph', a)
    _safe_set(a, 'graph_StringToObjectMapEntry', None)
    assert not _is_linked(a, 'graph_StringToObjectMapEntry', b2)
    if hasattr(b2, 'graph_GGraph'):
...
graph_StringToObjectMapEntry_strategy = st.builds(graph_StringToObjectMapEntry, key=safe_text, value=safe_text)
@given(instance=graph_StringToObjectMapEntry_strategy)
@settings(max_examples=25)
