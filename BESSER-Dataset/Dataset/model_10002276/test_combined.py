...
def test_hyp_projectcontroller_is_not_abstract():
    assert not inspect.isabstract(ProjectController)


def test_hyp_projectcontroller_constructor_exists():
    assert callable(ProjectController.__init__)


def test_hyp_projectcontroller_constructor_args():
    sig = inspect.signature(ProjectController.__init__)
    params = list(sig.parameters.keys())



def test_hyp_logon_is_not_abstract():
    assert not inspect.isabstract(LogOn)


def test_hyp_logon_constructor_exists():
    assert callable(LogOn.__init__)


def test_hyp_logon_constructor_args():
    sig = inspect.signature(LogOn.__init__)
    params = list(sig.parameters.keys())
...
ProjectController_strategy = st.builds(
    ProjectController,
)
LogOn_strategy = st.builds(
    LogOn,
)
LogON_strategy = st.builds(
    LogON,
...
LogON_strategy = st.builds(LogON)
@given(instance=LogON_strategy)
@settings(max_examples=25)
def test_LogON_instantiation(instance):
    assert isinstance(instance, LogON)
...
ProjectController_strategy = st.builds(ProjectController)
@given(instance=ProjectController_strategy)
@settings(max_examples=25)
def test_ProjectController_instantiation(instance):
    assert isinstance(instance, ProjectController)
