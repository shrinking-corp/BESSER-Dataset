





import java.util.List;
import java.util.ArrayList;

public class db_DBConnection extends DBResource {

    private String properties;
    private String transactionMode;
    private String password;
    private int loginTimeout;
    private int maxPoolSize;
    private String url;
    private int acquireIncrement;
    private int minPoolSize;
    private String user;
    private int maxIdleTime;





    private db_DBDriver db_dbdriver;




    private db_Query db_query;




    private List<db_Query> db_querys;




    private db_DBDriver db_dbdriver;


    public db_DBConnection(
        String properties,        String transactionMode,        String password,        int loginTimeout,        int maxPoolSize,        String url,        int acquireIncrement,        int minPoolSize,        String user,        int maxIdleTime    ) {
        super(
        );
        this.properties = properties;
        this.transactionMode = transactionMode;
        this.password = password;
        this.loginTimeout = loginTimeout;
        this.maxPoolSize = maxPoolSize;
        this.url = url;
        this.acquireIncrement = acquireIncrement;
        this.minPoolSize = minPoolSize;
        this.user = user;
        this.maxIdleTime = maxIdleTime;
        this.db_querys = new ArrayList<>();
    }

    public db_DBConnection(
        String properties,        String transactionMode,        String password,        int loginTimeout,        int maxPoolSize,        String url,        int acquireIncrement,        int minPoolSize,        String user,        int maxIdleTime        ArrayList<db_Query> db_querys    ) {
        this.properties = properties;
        this.transactionMode = transactionMode;
        this.password = password;
        this.loginTimeout = loginTimeout;
        this.maxPoolSize = maxPoolSize;
        this.url = url;
        this.acquireIncrement = acquireIncrement;
        this.minPoolSize = minPoolSize;
        this.user = user;
        this.maxIdleTime = maxIdleTime;
        this.db_querys = db_querys;
    }

    public String getProperties() {
        return properties;
    }

    public void setProperties(String properties) {
        this.properties = properties;
    }
    public String getTransactionmode() {
        return transactionMode;
    }

    public void setTransactionmode(String transactionMode) {
        this.transactionMode = transactionMode;
    }
    public String getPassword() {
        return password;
    }

    public void setPassword(String password) {
        this.password = password;
    }
    public int getLogintimeout() {
        return loginTimeout;
    }

    public void setLogintimeout(int loginTimeout) {
        this.loginTimeout = loginTimeout;
    }
    public int getMaxpoolsize() {
        return maxPoolSize;
    }

    public void setMaxpoolsize(int maxPoolSize) {
        this.maxPoolSize = maxPoolSize;
    }
    public String getUrl() {
        return url;
    }

    public void setUrl(String url) {
        this.url = url;
    }
    public int getAcquireincrement() {
        return acquireIncrement;
    }

    public void setAcquireincrement(int acquireIncrement) {
        this.acquireIncrement = acquireIncrement;
    }
    public int getMinpoolsize() {
        return minPoolSize;
    }

    public void setMinpoolsize(int minPoolSize) {
        this.minPoolSize = minPoolSize;
    }
    public String getUser() {
        return user;
    }

    public void setUser(String user) {
        this.user = user;
    }
    public int getMaxidletime() {
        return maxIdleTime;
    }

    public void setMaxidletime(int maxIdleTime) {
        this.maxIdleTime = maxIdleTime;
    }

    public db_DBDriver getDb_dbdriver() {
        return db_dbdriver;
    }

    public void setDb_dbdriver(db_DBDriver db_dbdriver) {
        this.db_dbdriver = db_dbdriver;
    }
    public db_Query getDb_query() {
        return db_query;
    }

    public void setDb_query(db_Query db_query) {
        this.db_query = db_query;
    }
    public List<db_Query> getDb_querys() {
        return db_querys;
    }

    public void addDb_query(Db_query db_query) {
        this.db_querys.add(db_query);
    }
    public db_DBDriver getDb_dbdriver() {
        return db_dbdriver;
    }

    public void setDb_dbdriver(db_DBDriver db_dbdriver) {
        this.db_dbdriver = db_dbdriver;
    }

}