...
def test_units_Quantity_value_value_roundtrip():
    instance = units_Quantity(value=3.14)
...
def test_assoc_lhs6_link_reassign_clear():
    ...
    b1 = units_Quantity(value=3.14)
    b2 = units_Quantity(value=9.99)
...
def test_assoc_unit0_link_reassign_clear():
    ...
    b1 = units_Unit()
    b2 = units_Unit()
    _safe_set(a, 'units_Quantity', b1)
    assert _is_linked(a, 'units_Quantity', b1)
    if hasattr(b1, 'units_Unit'):
        assert _is_linked(b1, 'units_Unit', a)
    _safe_set(a, 'units_Quantity', b2)
    assert _is_linked(a, 'units_Quantity', b2)
    if hasattr(b1, 'units_Unit'):
        assert not _is_linked(b1, 'units_Unit', a)
    if hasattr(b2, 'units_Unit'):
        assert _is_linked(b2, 'units_Unit', a)
    _safe_set(a, 'units_Quantity', None)
    assert not _is_linked(a, 'units_Quantity', b2)
    if hasattr(b2, 'units_Unit'):
        assert not _is_linked(b2, 'units_Unit', a)
...
units_Quantity_strategy = st.builds(units_Quantity, value=st.floats(allow_nan=False, allow_infinity=False))
