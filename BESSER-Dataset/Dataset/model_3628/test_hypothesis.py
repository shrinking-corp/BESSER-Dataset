...
def test_minuml1_rootactivitygraph_is_not_abstract():
    assert not inspect.isabstract(minuml1_RootActivityGraph)


def test_minuml1_rootactivitygraph_constructor_exists():
    assert callable(minuml1_RootActivityGraph.__init__)


def test_minuml1_rootactivitygraph_constructor_args():
    sig = inspect.signature(minuml1_RootActivityGraph.__init__)
    params = list(sig.parameters.keys())
    assert "name" in params, "Missing parameter 'name'"

def test_minuml1_rootactivitygraph_has_name():
    assert hasattr(minuml1_RootActivityGraph, "name")
    descriptor = None
    for klass in minuml1_RootActivityGraph.__mro__:
...
@given(instance=minuml1_RootActivityGraph_strategy)
def test_minuml1_rootactivitygraph_name_setter(instance):
