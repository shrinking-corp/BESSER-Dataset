...
def test_hyp_minuml1_rootactivitygraph_is_not_abstract():
    assert not inspect.isabstract(minuml1_RootActivityGraph)


def test_hyp_minuml1_rootactivitygraph_constructor_exists():
    assert callable(minuml1_RootActivityGraph.__init__)


def test_hyp_minuml1_rootactivitygraph_constructor_args():
    sig = inspect.signature(minuml1_RootActivityGraph.__init__)
...
minuml1_RootActivityGraph_strategy = st.builds(
    minuml1_RootActivityGraph,
    name=
        safe_text
)
...
@given(instance=minuml1_RootActivityGraph_strategy)
def test_hyp_minuml1_rootactivitygraph_name_setter(instance):
...
def test_minuml1_RootActivityGraph_name_value_roundtrip():
    instance = minuml1_RootActivityGraph(name="sample_text")
    assert instance.name == "sample_text"
    instance.name = "sample_text_2"
    assert instance.name == "sample_text_2"
...
def test_assoc_partition24_link_reassign_clear():
    a = minuml1_RootActivityGraph(name="sample_text")
    b1 = minuml1_Partition()
    b2 = minuml1_Partition()
    _safe_set(a, 'minuml1_RootActivityGraph25', {b1})
    assert _is_linked(a, 'minuml1_RootActivityGraph25', b1)
    if hasattr(b1, 'minuml1_Partition26'):
        assert _is_linked(b1, 'minuml1_Partition26', a)
    _safe_set(a, 'minuml1_RootActivityGraph25', {b2})
    assert _is_linked(a, 'minuml1_RootActivityGraph25', b2)
    if hasattr(b1, 'minuml1_Partition26'):
        assert not _is_linked(b1, 'minuml1_Partition26', a)
    if hasattr(b2, 'minuml1_Partition26'):
        assert _is_linked(b2, 'minuml1_Partition26', a)
    _safe_set(a, 'minuml1_RootActivityGraph25', set())
    assert not _is_linked(a, 'minuml1_RootActivityGraph25', b2)
    if hasattr(b2, 'minuml1_Partition26'):
        assert not _is_linked(b2, 'minuml1_Partition26', a)


def test_assoc_top21_link_reassign_clear():
    a = minuml1_RootActivityGraph(name="sample_text")
    b1 = minuml1_State()
    b2 = minuml1_State()
    _safe_set(a, 'minuml1_RootActivityGraph22', b1)
    assert _is_linked(a, 'minuml1_RootActivityGraph22', b1)
    if hasattr(b1, 'minuml1_State23'):
        assert _is_linked(b1, 'minuml1_State23', a)
    _safe_set(a, 'minuml1_RootActivityGraph22', b2)
    assert _is_linked(a, 'minuml1_RootActivityGraph22', b2)
    if hasattr(b1, 'minuml1_State23'):
        assert not _is_linked(b1, 'minuml1_State23', a)
    if hasattr(b2, 'minuml1_State23'):
        assert _is_linked(b2, 'minuml1_State23', a)
    _safe_set(a, 'minuml1_RootActivityGraph22', None)
    assert not _is_linked(a, 'minuml1_RootActivityGraph22', b2)
    if hasattr(b2, 'minuml1_State23'):
        assert not _is_linked(b2, 'minuml1_State23', a)


def test_assoc_transitions19_link_reassign_clear():
    a = minuml1_RootActivityGraph(name="sample_text")
    b1 = minuml1_Transition()
    b2 = minuml1_Transition()
    _safe_set(a, 'minuml1_RootActivityGraph', {b1})
    assert _is_linked(a, 'minuml1_RootActivityGraph', b1)
    if hasattr(b1, 'minuml1_Transition20'):
        assert _is_linked(b1, 'minuml1_Transition20', a)
    _safe_set(a, 'minuml1_RootActivityGraph', {b2})
    assert _is_linked(a, 'minuml1_RootActivityGraph', b2)
    if hasattr(b1, 'minuml1_Transition20'):
        assert not _is_linked(b1, 'minuml1_Transition20', a)
    if hasattr(b2, 'minuml1_Transition20'):
        assert _is_linked(b2, 'minuml1_Transition20', a)
    _safe_set(a, 'minuml1_RootActivityGraph', set())
    assert not _is_linked(a, 'minuml1_RootActivityGraph', b2)
    if hasattr(b2, 'minuml1_Transition20'):
        assert not _is_linked(b2, 'minuml1_Transition20', a)
...
minuml1_RootActivityGraph_strategy = st.builds(minuml1_RootActivityGraph, name=safe_text)
@given(instance=minuml1_RootActivityGraph_strategy)
@settings(max_examples=25)
def test_minuml1_RootActivityGraph_instantiation(instance):
    assert isinstance(instance, minuml1_RootActivityGraph)
