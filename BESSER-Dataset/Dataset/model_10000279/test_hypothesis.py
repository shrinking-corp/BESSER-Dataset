...
def test_waiter1_is_not_abstract():
    assert not inspect.isabstract(Waiter1)


def test_waiter1_constructor_exists():
    assert callable(Waiter1.__init__)


def test_waiter1_constructor_args():
...
def test_employee_has_Contact_():
    assert hasattr(Employee, "Contact_")
    descriptor = None
    for klass in Employee.__mro__:
        if "Contact_" in klass.__dict__:
            descriptor = klass.__dict__["Contact_"]
            break
    assert isinstance(descriptor, property)

def test_employee_has_jopType():
    assert hasattr(Employee, "jopType")
    descriptor = None
    for klass in Employee.__mro__:
        if "jopType" in klass.__dict__:
            descriptor = klass.__dict__["jopType"]
            break
    assert isinstance(descriptor, property)
...
@given(instance=Employee_strategy)
def test_employee_Contact__setter(instance):
...
@given(instance=Employee_strategy)
def test_employee_jopType_setter(instance):
