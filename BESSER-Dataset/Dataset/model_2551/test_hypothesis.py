...
def test_test2_e_is_not_abstract():
    assert not inspect.isabstract(test2_E)


def test_test2_e_constructor_exists():
    assert callable(test2_E.__init__)


def test_test2_e_constructor_args():
    sig = inspect.signature(test2_E.__init__)
...
def test_test2_d2_is_not_abstract():
    assert not inspect.isabstract(test2_D2)


def test_test2_d2_constructor_exists():
    assert callable(test2_D2.__init__)


def test_test2_d2_constructor_args():
    sig = inspect.signature(test2_D2.__init__)
    params = list(sig.parameters.keys())



def test_test2_d_is_not_abstract():
    assert not inspect.isabstract(test2_D)


def test_test2_d_constructor_exists():
    assert callable(test2_D.__init__)


def test_test2_d_constructor_args():
    sig = inspect.signature(test2_D.__init__)
    params = list(sig.parameters.keys())
...
test2_E_strategy = st.builds(
    test2_E,
)
D_strategy = st.builds(
    D,
)
test2_D2_strategy = st.builds(
    test2_D2,
)
test2_D_strategy = st.builds(
    test2_D,
)
