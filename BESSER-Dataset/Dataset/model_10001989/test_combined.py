...
def test_assoc_Customer_Shopping_Cart_link_reassign_clear():
    ...
    if hasattr(b1, 'add_to_Cart4'):
        assert _is_linked(b1, 'add_to_Cart4', a)
    _safe_set(a, 'customer5', {b2})
    assert _is_linked(a, 'customer5', b2)
    if hasattr(b1, 'add_to_Cart4'):
        assert not _is_linked(b1, 'add_to_Cart4', a)
    if hasattr(b2, 'add_to_Cart4'):
        assert _is_linked(b2, 'add_to_Cart4', a)
    _safe_set(a, 'customer5', set())
    assert not _is_linked(a, 'customer5', b2)
    if hasattr(b2, 'add_to_Cart4'):
        assert not _is_linked(b2, 'add_to_Cart4', a)
