...
def test_hyp_shoppingbasket_is_not_abstract():
    assert not inspect.isabstract(ShoppingBAsket)


def test_hyp_shoppingbasket_constructor_exists():
    assert callable(ShoppingBAsket.__init__)


def test_hyp_shoppingbasket_constructor_args():
    sig = inspect.signature(ShoppingBAsket.__init__)
    params = list(sig.parameters.keys())
    assert "id" in params, "Missing parameter 'id'"
...
ShoppingBAsket_strategy = st.builds(
    ShoppingBAsket,
    id=
        st.integers(),
    creationDate=
...
@given(instance=ShoppingBAsket_strategy)
def test_hyp_shoppingbasket_id_setter(instance):
    original = instance.id
    instance.id = original
    assert instance.id == original
...
@given(instance=ShoppingBAsket_strategy)
def test_hyp_shoppingbasket_creationDate_setter(instance):
...
def test_ShoppingBAsket_creationDate_value_roundtrip():
    instance = ShoppingBAsket(creationDate=date(2024, 1, 1), id=7)
...
def test_ShoppingBAsket_id_value_roundtrip():
    instance = ShoppingBAsket(creationDate=date(2024, 1, 1), id=7)
    assert instance.id == 7
    instance.id = 13
    assert instance.id == 13
...
def test_assoc_association2_link_reassign_clear():
    a = ShoppingBAsket(creationDate=date(2024, 1, 1), id=7)
...
ShoppingBAsket_strategy = st.builds(ShoppingBAsket, creationDate=st.dates(), id=st.integers())
@given(instance=ShoppingBAsket_strategy)
@settings(max_examples=25)
def test_ShoppingBAsket_instantiation(instance):
    assert isinstance(instance, ShoppingBAsket)
