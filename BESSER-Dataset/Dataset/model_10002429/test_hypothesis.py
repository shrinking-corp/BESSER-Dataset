...
def test_shoppingbasket_is_not_abstract():
    assert not inspect.isabstract(ShoppingBAsket)


def test_shoppingbasket_constructor_exists():
    assert callable(ShoppingBAsket.__init__)


def test_shoppingbasket_constructor_args():
    sig = inspect.signature(ShoppingBAsket.__init__)
    params = list(sig.parameters.keys())
    assert "id" in params, "Missing parameter 'id'"
    assert "creationDate" in params, "Missing parameter 'creationDate'"

def test_shoppingbasket_has_id():
    assert hasattr(ShoppingBAsket, "id")
    descriptor = None
    for klass in ShoppingBAsket.__mro__:
        if "id" in klass.__dict__:
            descriptor = klass.__dict__["id"]
            break
    assert isinstance(descriptor, property)

def test_shoppingbasket_has_creationDate():
    assert hasattr(ShoppingBAsket, "creationDate")
    descriptor = None
    for klass in ShoppingBAsket.__mro__:
...
@given(instance=ShoppingBAsket_strategy)
def test_shoppingbasket_id_setter(instance):
    original = instance.id
    instance.id = original
    assert instance.id == original
...
@given(instance=ShoppingBAsket_strategy)
def test_shoppingbasket_creationDate_setter(instance):
