...
def test_hyp_library_manuscript_is_not_abstract():
    assert not inspect.isabstract(library_Manuscript)


def test_hyp_library_manuscript_constructor_exists():
    assert callable(library_Manuscript.__init__)


def test_hyp_library_manuscript_constructor_args():
    sig = inspect.signature(library_Manuscript.__init__)
    params = list(sig.parameters.keys())
...
library_Manuscript_strategy = st.builds(
    library_Manuscript,
    state=
        st.integers()
)
...
@given(instance=library_Manuscript_strategy)
def test_hyp_library_manuscript_state_setter(instance):
...
def test_library_Manuscript_state_value_roundtrip():
    instance = library_Manuscript(state=7)
    assert instance.state == 7
    instance.state = 13
    assert instance.state == 13
...
def test_library_Manuscript_isa_Book():
    instance = library_Manuscript(state=7)
...
def test_assoc_citation6_link_reassign_clear():
    a = library_Book(category="sample_text", pages=7, title="sample_text")
    b1 = library_Book(category="sample_text", pages=7, title="sample_text")
    b2 = library_Book(category="sample_text_2", pages=13, title="sample_text_2")
    _safe_set(a, 'library_Book5', {b1})
    assert _is_linked(a, 'library_Book5', b1)
    if hasattr(b1, 'library_Book7'):
        assert _is_linked(b1, 'library_Book7', a)
    _safe_set(a, 'library_Book5', {b2})
    assert _is_linked(a, 'library_Book5', b2)
    if hasattr(b1, 'library_Book7'):
        assert not _is_linked(b1, 'library_Book7', a)
    if hasattr(b2, 'library_Book7'):
        assert _is_linked(b2, 'library_Book7', a)
    _safe_set(a, 'library_Book5', set())
    assert not _is_linked(a, 'library_Book5', b2)
    if hasattr(b2, 'library_Book7'):
...
library_Manuscript_strategy = st.builds(library_Manuscript, state=st.integers())
@given(instance=library_Manuscript_strategy)
@settings(max_examples=25)
def test_library_Manuscript_instantiation(instance):
    assert isinstance(instance, library_Manuscript)
