...
def test_core_requirementscontainer_is_not_abstract():
    assert not inspect.isabstract(core_RequirementsContainer)


def test_core_requirementscontainer_constructor_exists():
    assert callable(core_RequirementsContainer.__init__)


def test_core_requirementscontainer_constructor_args():
    sig = inspect.signature(core_RequirementsContainer.__init__)
    params = list(sig.parameters.keys())
    assert "type" in params, "Missing parameter 'type'"

def test_core_requirementscontainer_has_type():
    assert hasattr(core_RequirementsContainer, "type")
    descriptor = None
    for klass in core_RequirementsContainer.__mro__:
        if "type" in klass.__dict__:
            descriptor = klass.__dict__["type"]
            break
    assert isinstance(descriptor, property)
...
@given(instance=core_RequirementsContainer_strategy)
def test_core_requirementscontainer_type_setter(instance):
    original = instance.type
    instance.type = original
    assert instance.type == original
