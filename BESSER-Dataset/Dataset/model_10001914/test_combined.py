...
def test_assoc_B_C_link_reassign_clear():
    ...
    b1 = B1(attB=7)
    b2 = B1(attB=13)
    _safe_set(a, 'test3', b1)
    assert _is_linked(a, 'test3', b1)
    if hasattr(b1, 'c2'):
        assert _is_linked(b1, 'c2', a)
    _safe_set(a, 'test3', b2)
    assert _is_linked(a, 'test3', b2)
    if hasattr(b1, 'c2'):
        assert not _is_linked(b1, 'c2', a)
    if hasattr(b2, 'c2'):
        assert _is_linked(b2, 'c2', a)
    _safe_set(a, 'test3', None)
    assert not _is_linked(a, 'test3', b2)
    if hasattr(b2, 'c2'):
