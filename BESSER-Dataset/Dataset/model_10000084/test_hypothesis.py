...
def test_elevator1_is_not_abstract():
    assert not inspect.isabstract(Elevator1)


def test_elevator1_constructor_exists():
    assert callable(Elevator1.__init__)


def test_elevator1_constructor_args():
    sig = inspect.signature(Elevator1.__init__)
    params = list(sig.parameters.keys())
    assert "id" in params, "Missing parameter 'id'"

def test_elevator1_has_id():
    assert hasattr(Elevator1, "id")
    descriptor = None
    for klass in Elevator1.__mro__:
...
def test_elevator_request_is_not_abstract():
    assert not inspect.isabstract(Elevator_Request)


def test_elevator_request_constructor_exists():
    assert callable(Elevator_Request.__init__)


def test_elevator_request_constructor_args():
...
@given(instance=Elevator1_strategy)
def test_elevator1_id_setter(instance):
...
@given(instance=Floor_strategy)
def test_floor_Id_setter(instance):
    original = instance.Id
    instance.Id = original
    assert instance.Id == original
