...
def test_assoc_annotations296_link_reassign_clear():
    a = java_SingleVariableDeclaration(varargs=True)
    b1 = java_Annotation()
    b2 = java_Annotation()
    _safe_set(a, 'java_SingleVariableDeclaration297', {b1})
    assert _is_linked(a, 'java_SingleVariableDeclaration297', b1)
    if hasattr(b1, 'java_Annotation298'):
        assert _is_linked(b1, 'java_Annotation298', a)
    _safe_set(a, 'java_SingleVariableDeclaration297', {b2})
    assert _is_linked(a, 'java_SingleVariableDeclaration297', b2)
    if hasattr(b1, 'java_Annotation298'):
        assert not _is_linked(b1, 'java_Annotation298', a)
    if hasattr(b2, 'java_Annotation298'):
        assert _is_linked(b2, 'java_Annotation298', a)
    _safe_set(a, 'java_SingleVariableDeclaration297', set())
    assert not _is_linked(a, 'java_SingleVariableDeclaration297', b2)
    if hasattr(b2, 'java_Annotation298'):
        assert not _is_linked(b2, 'java_Annotation298', a)


def test_assoc_annotations360_link_reassign_clear():
    ...
    if hasattr(b1, 'java_Annotation361'):
        assert _is_linked(b1, 'java_Annotation361', a)
    _safe_set(a, 'java_VariableDeclarationStatement', {b2})
    assert _is_linked(a, 'java_VariableDeclarationStatement', b2)
    if hasattr(b1, 'java_Annotation361'):
        assert not _is_linked(b1, 'java_Annotation361', a)
    if hasattr(b2, 'java_Annotation361'):
        assert _is_linked(b2, 'java_Annotation361', a)
    _safe_set(a, 'java_VariableDeclarationStatement', set())
    assert not _is_linked(a, 'java_VariableDeclarationStatement', b2)
    if hasattr(b2, 'java_Annotation361'):
        assert not _is_linked(b2, 'java_Annotation361', a)
...
def test_assoc_attachedSource105_link_reassign_clear():
    ...
    _safe_set(a, 'java_CompilationUnit107', b1)
    assert _is_linked(a, 'java_CompilationUnit107', b1)
    if hasattr(b1, 'java_ClassFile106'):
        assert _is_linked(b1, 'java_ClassFile106', a)
    _safe_set(a, 'java_CompilationUnit107', b2)
    assert _is_linked(a, 'java_CompilationUnit107', b2)
    if hasattr(b1, 'java_ClassFile106'):
        assert not _is_linked(b1, 'java_ClassFile106', a)
    if hasattr(b2, 'java_ClassFile106'):
        assert _is_linked(b2, 'java_ClassFile106', a)
    _safe_set(a, 'java_CompilationUnit107', None)
    assert not _is_linked(a, 'java_CompilationUnit107', b2)
    if hasattr(b2, 'java_ClassFile106'):
        assert not _is_linked(b2, 'java_ClassFile106', a)


def test_assoc_attributes208_link_reassign_clear():
    ...
    _safe_set(a, 'java_ManifestEntry209', {b1})
    assert _is_linked(a, 'java_ManifestEntry209', b1)
    if hasattr(b1, 'java_ManifestAttribute210'):
        assert _is_linked(b1, 'java_ManifestAttribute210', a)
    _safe_set(a, 'java_ManifestEntry209', {b2})
    assert _is_linked(a, 'java_ManifestEntry209', b2)
    if hasattr(b1, 'java_ManifestAttribute210'):
        assert not _is_linked(b1, 'java_ManifestAttribute210', a)
    if hasattr(b2, 'java_ManifestAttribute210'):
        assert _is_linked(b2, 'java_ManifestAttribute210', a)
    _safe_set(a, 'java_ManifestEntry209', set())
    assert not _is_linked(a, 'java_ManifestEntry209', b2)
    if hasattr(b2, 'java_ManifestAttribute210'):
        assert not _is_linked(b2, 'java_ManifestAttribute210', a)
...
def test_assoc_bound362_link_reassign_clear():
    a = java_WildCardType(upperBound=True)
    b1 = java_TypeAccess()
    b2 = java_TypeAccess()
    _safe_set(a, 'java_WildCardType', b1)
    assert _is_linked(a, 'java_WildCardType', b1)
    if hasattr(b1, 'java_TypeAccess363'):
        assert _is_linked(b1, 'java_TypeAccess363', a)
    _safe_set(a, 'java_WildCardType', b2)
    assert _is_linked(a, 'java_WildCardType', b2)
    if hasattr(b1, 'java_TypeAccess363'):
        assert not _is_linked(b1, 'java_TypeAccess363', a)
    if hasattr(b2, 'java_TypeAccess363'):
        assert _is_linked(b2, 'java_TypeAccess363', a)
    _safe_set(a, 'java_WildCardType', None)
    assert not _is_linked(a, 'java_WildCardType', b2)
    if hasattr(b2, 'java_TypeAccess363'):
        assert not _is_linked(b2, 'java_TypeAccess363', a)


def test_assoc_catchClause300_link_reassign_clear():
    a = java_SingleVariableDeclaration(varargs=True)
    b1 = java_CatchClause()
    b2 = java_CatchClause()
...
def test_assoc_classFiles31_link_reassign_clear():
...
def test_assoc_commentList127_link_reassign_clear():
    a = java_CompilationUnit(originalFilePath="sample_text")
    ...
    _safe_set(a, 'java_CompilationUnit128', {b1})
    assert _is_linked(a, 'java_CompilationUnit128', b1)
    if hasattr(b1, 'java_Comment129'):
        assert _is_linked(b1, 'java_Comment129', a)
    _safe_set(a, 'java_CompilationUnit128', {b2})
    assert _is_linked(a, 'java_CompilationUnit128', b2)
    if hasattr(b1, 'java_Comment129'):
        assert not _is_linked(b1, 'java_Comment129', a)
    if hasattr(b2, 'java_Comment129'):
        assert _is_linked(b2, 'java_Comment129', a)
    _safe_set(a, 'java_CompilationUnit128', set())
    assert not _is_linked(a, 'java_CompilationUnit128', b2)
    if hasattr(b2, 'java_Comment129'):
        assert not _is_linked(b2, 'java_Comment129', a)


def test_assoc_comments39_link_reassign_clear():
    a = java_Comment(content="sample_text", enclosedByParent=True, prefixOfParent=True)
    b1 = java_ASTNode()
    b2 = java_ASTNode()
    _safe_set(a, 'java_Comment40', b1)
    assert _is_linked(a, 'java_Comment40', b1)
    if hasattr(b1, 'java_ASTNode'):
        assert _is_linked(b1, 'java_ASTNode', a)
    _safe_set(a, 'java_Comment40', b2)
    assert _is_linked(a, 'java_Comment40', b2)
    if hasattr(b1, 'java_ASTNode'):
        assert not _is_linked(b1, 'java_ASTNode', a)
    if hasattr(b2, 'java_ASTNode'):
        assert _is_linked(b2, 'java_ASTNode', a)
    _safe_set(a, 'java_Comment40', None)
    assert not _is_linked(a, 'java_Comment40', b2)
    if hasattr(b2, 'java_ASTNode'):
        assert not _is_linked(b2, 'java_ASTNode', a)


def test_assoc_commentsAfterBody15_link_reassign_clear():
    a = java_Comment(content="sample_text", enclosedByParent=True, prefixOfParent=True)
    b1 = java_AbstractTypeDeclaration()
    b2 = java_AbstractTypeDeclaration()
    _safe_set(a, 'java_Comment17', b1)
    assert _is_linked(a, 'java_Comment17', b1)
    if hasattr(b1, 'java_AbstractTypeDeclaration16'):
        assert _is_linked(b1, 'java_AbstractTypeDeclaration16', a)
    _safe_set(a, 'java_Comment17', b2)
    assert _is_linked(a, 'java_Comment17', b2)
    if hasattr(b1, 'java_AbstractTypeDeclaration16'):
        assert not _is_linked(b1, 'java_AbstractTypeDeclaration16', a)
    if hasattr(b2, 'java_AbstractTypeDeclaration16'):
        assert _is_linked(b2, 'java_AbstractTypeDeclaration16', a)
    _safe_set(a, 'java_Comment17', None)
    assert not _is_linked(a, 'java_Comment17', b2)
    if hasattr(b2, 'java_AbstractTypeDeclaration16'):
        assert not _is_linked(b2, 'java_AbstractTypeDeclaration16', a)


def test_assoc_commentsBeforeBody14_link_reassign_clear():
    a = java_Comment(content="sample_text", enclosedByParent=True, prefixOfParent=True)
    b1 = java_AbstractTypeDeclaration()
    b2 = java_AbstractTypeDeclaration()
...
def test_assoc_elementType77_link_reassign_clear():
    a = java_ArrayType(dimensions=7)
    b1 = java_TypeAccess()
    b2 = java_TypeAccess()
    _safe_set(a, 'java_ArrayType', b1)
    assert _is_linked(a, 'java_ArrayType', b1)
    if hasattr(b1, 'java_TypeAccess78'):
        assert _is_linked(b1, 'java_TypeAccess78', a)
    _safe_set(a, 'java_ArrayType', b2)
    assert _is_linked(a, 'java_ArrayType', b2)
    if hasattr(b1, 'java_TypeAccess78'):
        assert not _is_linked(b1, 'java_TypeAccess78', a)
    if hasattr(b2, 'java_TypeAccess78'):
        assert _is_linked(b2, 'java_TypeAccess78', a)
    _safe_set(a, 'java_ArrayType', None)
    assert not _is_linked(a, 'java_ArrayType', b2)
    if hasattr(b2, 'java_TypeAccess78'):
        assert not _is_linked(b2, 'java_TypeAccess78', a)


def test_assoc_enhancedForStatement301_link_reassign_clear():
    a = java_SingleVariableDeclaration(varargs=True)
...
def test_assoc_entryAttributes206_link_reassign_clear():
    a = java_ManifestEntry(name="sample_text")
    b1 = java_Manifest()
    b2 = java_Manifest()
...
def test_assoc_exception98_link_reassign_clear():
    a = java_SingleVariableDeclaration(varargs=True)
    b1 = java_CatchClause()
    b2 = java_CatchClause()
    _safe_set(a, 'SingleVariableDeclaration99', b1)
    assert _is_linked(a, 'SingleVariableDeclaration99', b1)
    if hasattr(b1, 'catchClause'):
        assert _is_linked(b1, 'catchClause', a)
    _safe_set(a, 'SingleVariableDeclaration99', b2)
    assert _is_linked(a, 'SingleVariableDeclaration99', b2)
    if hasattr(b1, 'catchClause'):
        assert not _is_linked(b1, 'catchClause', a)
    if hasattr(b2, 'catchClause'):
        assert _is_linked(b2, 'catchClause', a)
    _safe_set(a, 'SingleVariableDeclaration99', None)
    assert not _is_linked(a, 'SingleVariableDeclaration99', b2)
    if hasattr(b2, 'catchClause'):
        assert not _is_linked(b2, 'catchClause', a)


def test_assoc_expression306_link_reassign_clear():
    a = java_SwitchCase(default=True)
    b1 = java_Expression()
    b2 = java_Expression()
    _safe_set(a, 'java_SwitchCase', b1)
    assert _is_linked(a, 'java_SwitchCase', b1)
    if hasattr(b1, 'java_Expression307'):
        assert _is_linked(b1, 'java_Expression307', a)
    _safe_set(a, 'java_SwitchCase', b2)
    assert _is_linked(a, 'java_SwitchCase', b2)
    if hasattr(b1, 'java_Expression307'):
        assert not _is_linked(b1, 'java_Expression307', a)
    if hasattr(b2, 'java_Expression307'):
        assert _is_linked(b2, 'java_Expression307', a)
    _safe_set(a, 'java_SwitchCase', None)
    assert not _is_linked(a, 'java_SwitchCase', b2)
    if hasattr(b2, 'java_Expression307'):
        assert not _is_linked(b2, 'java_Expression307', a)
...
def test_assoc_fragments318_link_reassign_clear():
    a = java_TagElement(tagName="sample_text")
    b1 = java_ASTNode()
    b2 = java_ASTNode()
    _safe_set(a, 'java_TagElement319', {b1})
    assert _is_linked(a, 'java_TagElement319', b1)
    if hasattr(b1, 'java_ASTNode320'):
        assert _is_linked(b1, 'java_ASTNode320', a)
    _safe_set(a, 'java_TagElement319', {b2})
    assert _is_linked(a, 'java_TagElement319', b2)
    if hasattr(b1, 'java_ASTNode320'):
        assert not _is_linked(b1, 'java_ASTNode320', a)
    if hasattr(b2, 'java_ASTNode320'):
        assert _is_linked(b2, 'java_ASTNode320', a)
    _safe_set(a, 'java_TagElement319', set())
    assert not _is_linked(a, 'java_TagElement319', b2)
    if hasattr(b2, 'java_ASTNode320'):
        assert not _is_linked(b2, 'java_ASTNode320', a)


def test_assoc_importedElement183_link_reassign_clear():
    ...
    if hasattr(b1, 'java_ImportDeclaration184'):
        assert _is_linked(b1, 'java_ImportDeclaration184', a)
    _safe_set(a, 'java_NamedElement', b2)
    assert _is_linked(a, 'java_NamedElement', b2)
    if hasattr(b1, 'java_ImportDeclaration184'):
        assert not _is_linked(b1, 'java_ImportDeclaration184', a)
    if hasattr(b2, 'java_ImportDeclaration184'):
        assert _is_linked(b2, 'java_ImportDeclaration184', a)
    _safe_set(a, 'java_NamedElement', None)
    assert not _is_linked(a, 'java_NamedElement', b2)
    if hasattr(b2, 'java_ImportDeclaration184'):
        assert not _is_linked(b2, 'java_ImportDeclaration184', a)


def test_assoc_imports130_link_reassign_clear():
    a = java_ImportDeclaration(static=True)
    b1 = java_CompilationUnit(originalFilePath="sample_text")
    b2 = java_CompilationUnit(originalFilePath="sample_text_2")
    _safe_set(a, 'java_ImportDeclaration', b1)
    assert _is_linked(a, 'java_ImportDeclaration', b1)
    if hasattr(b1, 'java_CompilationUnit131'):
        assert _is_linked(b1, 'java_CompilationUnit131', a)
    _safe_set(a, 'java_ImportDeclaration', b2)
    assert _is_linked(a, 'java_ImportDeclaration', b2)
    if hasattr(b1, 'java_CompilationUnit131'):
        assert not _is_linked(b1, 'java_CompilationUnit131', a)
    if hasattr(b2, 'java_CompilationUnit131'):
        assert _is_linked(b2, 'java_CompilationUnit131', a)
    _safe_set(a, 'java_ImportDeclaration', None)
    assert not _is_linked(a, 'java_ImportDeclaration', b2)
    if hasattr(b2, 'java_CompilationUnit131'):
        assert not _is_linked(b2, 'java_CompilationUnit131', a)


def test_assoc_initializer350_link_reassign_clear():
    a = java_VariableDeclaration(extraArrayDimensions=7)
    b1 = java_Expression()
    b2 = java_Expression()
    _safe_set(a, 'java_VariableDeclaration351', b1)
    assert _is_linked(a, 'java_VariableDeclaration351', b1)
    if hasattr(b1, 'java_Expression352'):
        assert _is_linked(b1, 'java_Expression352', a)
    _safe_set(a, 'java_VariableDeclaration351', b2)
    assert _is_linked(a, 'java_VariableDeclaration351', b2)
    if hasattr(b1, 'java_Expression352'):
        assert not _is_linked(b1, 'java_Expression352', a)
    if hasattr(b2, 'java_Expression352'):
        assert _is_linked(b2, 'java_Expression352', a)
    _safe_set(a, 'java_VariableDeclaration351', None)
    assert not _is_linked(a, 'java_VariableDeclaration351', b2)
    if hasattr(b2, 'java_Expression352'):
        assert not _is_linked(b2, 'java_Expression352', a)


def test_assoc_leftHandSide79_link_reassign_clear():
    a = java_Assignment(operator="sample_text")
    b1 = java_Expression()
    b2 = java_Expression()
    _safe_set(a, 'java_Assignment', b1)
    assert _is_linked(a, 'java_Assignment', b1)
    if hasattr(b1, 'java_Expression80'):
        assert _is_linked(b1, 'java_Expression80', a)
    _safe_set(a, 'java_Assignment', b2)
    assert _is_linked(a, 'java_Assignment', b2)
    if hasattr(b1, 'java_Expression80'):
        assert not _is_linked(b1, 'java_Expression80', a)
    if hasattr(b2, 'java_Expression80'):
        assert _is_linked(b2, 'java_Expression80', a)
    _safe_set(a, 'java_Assignment', None)
    assert not _is_linked(a, 'java_Assignment', b2)
    if hasattr(b2, 'java_Expression80'):
        assert not _is_linked(b2, 'java_Expression80', a)
...
def test_assoc_mainAttributes204_link_reassign_clear():
    ...
    if hasattr(b1, 'java_Manifest205'):
        assert _is_linked(b1, 'java_Manifest205', a)
    _safe_set(a, 'java_ManifestAttribute', b2)
    assert _is_linked(a, 'java_ManifestAttribute', b2)
    if hasattr(b1, 'java_Manifest205'):
        assert not _is_linked(b1, 'java_Manifest205', a)
    if hasattr(b2, 'java_Manifest205'):
        assert _is_linked(b2, 'java_Manifest205', a)
    _safe_set(a, 'java_ManifestAttribute', None)
    assert not _is_linked(a, 'java_ManifestAttribute', b2)
    if hasattr(b2, 'java_Manifest205'):
        assert not _is_linked(b2, 'java_Manifest205', a)
...
def test_assoc_manifest32_link_reassign_clear():
    ...
    b1 = java_Manifest()
    b2 = java_Manifest()
    _safe_set(a, 'java_Archive33', b1)
    assert _is_linked(a, 'java_Archive33', b1)
    if hasattr(b1, 'java_Manifest'):
        assert _is_linked(b1, 'java_Manifest', a)
    _safe_set(a, 'java_Archive33', b2)
    assert _is_linked(a, 'java_Archive33', b2)
    if hasattr(b1, 'java_Manifest'):
        assert not _is_linked(b1, 'java_Manifest', a)
    if hasattr(b2, 'java_Manifest'):
        assert _is_linked(b2, 'java_Manifest', a)
    _safe_set(a, 'java_Archive33', None)
    assert not _is_linked(a, 'java_Archive33', b2)
    if hasattr(b2, 'java_Manifest'):
        assert not _is_linked(b2, 'java_Manifest', a)


def test_assoc_member211_link_reassign_clear():
    a = java_NamedElement(name="sample_text", proxy=True)
    b1 = java_MemberRef()
    b2 = java_MemberRef()
    _safe_set(a, 'java_NamedElement212', b1)
    assert _is_linked(a, 'java_NamedElement212', b1)
    if hasattr(b1, 'java_MemberRef'):
        assert _is_linked(b1, 'java_MemberRef', a)
    _safe_set(a, 'java_NamedElement212', b2)
    assert _is_linked(a, 'java_NamedElement212', b2)
    if hasattr(b1, 'java_MemberRef'):
        assert not _is_linked(b1, 'java_MemberRef', a)
    if hasattr(b2, 'java_MemberRef'):
        assert _is_linked(b2, 'java_MemberRef', a)
    _safe_set(a, 'java_NamedElement212', None)
    assert not _is_linked(a, 'java_NamedElement212', b2)
    if hasattr(b2, 'java_MemberRef'):
        assert not _is_linked(b2, 'java_MemberRef', a)


def test_assoc_methodDeclaration299_link_reassign_clear():
    a = java_SingleVariableDeclaration(varargs=True)
    ...
    if hasattr(b1, 'AbstractMethodDeclaration'):
        assert _is_linked(b1, 'AbstractMethodDeclaration', a)
    _safe_set(a, 'parameters', b2)
    assert _is_linked(a, 'parameters', b2)
    if hasattr(b1, 'AbstractMethodDeclaration'):
        assert not _is_linked(b1, 'AbstractMethodDeclaration', a)
    if hasattr(b2, 'AbstractMethodDeclaration'):
        assert _is_linked(b2, 'AbstractMethodDeclaration', a)
    _safe_set(a, 'parameters', None)
    assert not _is_linked(a, 'parameters', b2)
    if hasattr(b2, 'AbstractMethodDeclaration'):
        assert not _is_linked(b2, 'AbstractMethodDeclaration', a)


def test_assoc_model260_link_reassign_clear():
    a = java_Model(name="sample_text")
    b1 = java_Package()
    b2 = java_Package()
    _safe_set(a, 'Model', b1)
    assert _is_linked(a, 'Model', b1)
    if hasattr(b1, 'ownedElements261'):
        assert _is_linked(b1, 'ownedElements261', a)
    _safe_set(a, 'Model', b2)
    assert _is_linked(a, 'Model', b2)
    if hasattr(b1, 'ownedElements261'):
        assert not _is_linked(b1, 'ownedElements261', a)
    if hasattr(b2, 'ownedElements261'):
        assert _is_linked(b2, 'ownedElements261', a)
    _safe_set(a, 'Model', None)
    assert not _is_linked(a, 'Model', b2)
    if hasattr(b2, 'ownedElements261'):
        assert not _is_linked(b2, 'ownedElements261', a)


def test_assoc_modifier292_link_reassign_clear():
    a = java_SingleVariableDeclaration(varargs=True)
    ...
    if hasattr(b1, 'Modifier293'):
        assert _is_linked(b1, 'Modifier293', a)
    _safe_set(a, 'singleVariableDeclaration', b2)
    assert _is_linked(a, 'singleVariableDeclaration', b2)
    if hasattr(b1, 'Modifier293'):
        assert not _is_linked(b1, 'Modifier293', a)
    if hasattr(b2, 'Modifier293'):
        assert _is_linked(b2, 'Modifier293', a)
    _safe_set(a, 'singleVariableDeclaration', None)
    assert not _is_linked(a, 'singleVariableDeclaration', b2)
    if hasattr(b2, 'Modifier293'):
        assert not _is_linked(b2, 'Modifier293', a)


def test_assoc_modifier353_link_reassign_clear():
    a = java_Modifier(inheritance="sample_text", native=True, static=True, strictfp=True, synchronized=True, transient=True, visibility="sample_text", volatile=True)
    b1 = java_VariableDeclarationExpression()
    b2 = java_VariableDeclarationExpression()
    _safe_set(a, 'Modifier354', b1)
    assert _is_linked(a, 'Modifier354', b1)
    if hasattr(b1, 'variableDeclarationExpression'):
        assert _is_linked(b1, 'variableDeclarationExpression', a)
    _safe_set(a, 'Modifier354', b2)
    assert _is_linked(a, 'Modifier354', b2)
    if hasattr(b1, 'variableDeclarationExpression'):
    ...
    _safe_set(a, 'Modifier354', None)
    assert not _is_linked(a, 'Modifier354', b2)
    if hasattr(b2, 'variableDeclarationExpression'):
        assert not _is_linked(b2, 'variableDeclarationExpression', a)


def test_assoc_modifier358_link_reassign_clear():
    a = java_VariableDeclarationStatement(extraArrayDimensions=7)
    b1 = java_Modifier(inheritance="sample_text", native=True, static=True, strictfp=True, synchronized=True, transient=True, visibility="sample_text", volatile=True)
    b2 = java_Modifier(inheritance="sample_text_2", native=False, static=False, strictfp=False, synchronized=False, transient=False, visibility="sample_text_2", volatile=False)
    _safe_set(a, 'variableDeclarationStatement', b1)
    assert _is_linked(a, 'variableDeclarationStatement', b1)
    if hasattr(b1, 'Modifier359'):
        assert _is_linked(b1, 'Modifier359', a)
    _safe_set(a, 'variableDeclarationStatement', b2)
    assert _is_linked(a, 'variableDeclarationStatement', b2)
    if hasattr(b1, 'Modifier359'):
        assert not _is_linked(b1, 'Modifier359', a)
    if hasattr(b2, 'Modifier359'):
        assert _is_linked(b2, 'Modifier359', a)
    _safe_set(a, 'variableDeclarationStatement', None)
    assert not _is_linked(a, 'variableDeclarationStatement', b2)
    if hasattr(b2, 'Modifier359'):
        assert not _is_linked(b2, 'Modifier359', a)


def test_assoc_modifier89_link_reassign_clear():
    a = java_Modifier(inheritance="sample_text", native=True, static=True, strictfp=True, synchronized=True, transient=True, visibility="sample_text", volatile=True)
    b1 = java_BodyDeclaration()
    b2 = java_BodyDeclaration()
    _safe_set(a, 'Modifier', b1)
...
def test_assoc_operand281_link_reassign_clear():
    ...
    if hasattr(b1, 'java_Expression282'):
        assert _is_linked(b1, 'java_Expression282', a)
    _safe_set(a, 'java_PostfixExpression', b2)
    assert _is_linked(a, 'java_PostfixExpression', b2)
    if hasattr(b1, 'java_Expression282'):
        assert not _is_linked(b1, 'java_Expression282', a)
    if hasattr(b2, 'java_Expression282'):
        assert _is_linked(b2, 'java_Expression282', a)
    _safe_set(a, 'java_PostfixExpression', None)
    assert not _is_linked(a, 'java_PostfixExpression', b2)
    if hasattr(b2, 'java_Expression282'):
        assert not _is_linked(b2, 'java_Expression282', a)
...
def test_assoc_originalClassFile43_link_reassign_clear():
    a = java_ClassFile(originalFilePath="sample_text")
    b1 = java_ASTNode()
    b2 = java_ASTNode()
    _safe_set(a, 'java_ClassFile45', b1)
    assert _is_linked(a, 'java_ClassFile45', b1)
    if hasattr(b1, 'java_ASTNode44'):
        assert _is_linked(b1, 'java_ASTNode44', a)
    _safe_set(a, 'java_ClassFile45', b2)
    assert _is_linked(a, 'java_ClassFile45', b2)
    if hasattr(b1, 'java_ASTNode44'):
        assert not _is_linked(b1, 'java_ASTNode44', a)
    if hasattr(b2, 'java_ASTNode44'):
        assert _is_linked(b2, 'java_ASTNode44', a)
    _safe_set(a, 'java_ClassFile45', None)
    assert not _is_linked(a, 'java_ClassFile45', b2)
    if hasattr(b2, 'java_ASTNode44'):
        assert not _is_linked(b2, 'java_ASTNode44', a)


def test_assoc_originalCompilationUnit41_link_reassign_clear():
    a = java_CompilationUnit(originalFilePath="sample_text")
    b1 = java_ASTNode()
    b2 = java_ASTNode()
    _safe_set(a, 'java_CompilationUnit', b1)
    assert _is_linked(a, 'java_CompilationUnit', b1)
    if hasattr(b1, 'java_ASTNode42'):
        assert _is_linked(b1, 'java_ASTNode42', a)
    _safe_set(a, 'java_CompilationUnit', b2)
    assert _is_linked(a, 'java_CompilationUnit', b2)
    if hasattr(b1, 'java_ASTNode42'):
        assert not _is_linked(b1, 'java_ASTNode42', a)
    if hasattr(b2, 'java_ASTNode42'):
        assert _is_linked(b2, 'java_ASTNode42', a)
    _safe_set(a, 'java_CompilationUnit', None)
    assert not _is_linked(a, 'java_CompilationUnit', b2)
    if hasattr(b2, 'java_ASTNode42'):
        assert not _is_linked(b2, 'java_ASTNode42', a)
...
def test_assoc_package108_link_reassign_clear():
    a = java_ClassFile(originalFilePath="sample_text")
    b1 = java_Package()
    b2 = java_Package()
    _safe_set(a, 'java_ClassFile109', b1)
    assert _is_linked(a, 'java_ClassFile109', b1)
    if hasattr(b1, 'java_Package'):
        assert _is_linked(b1, 'java_Package', a)
    _safe_set(a, 'java_ClassFile109', b2)
    assert _is_linked(a, 'java_ClassFile109', b2)
    if hasattr(b1, 'java_Package'):
        assert not _is_linked(b1, 'java_Package', a)
    if hasattr(b2, 'java_Package'):
        assert _is_linked(b2, 'java_Package', a)
    _safe_set(a, 'java_ClassFile109', None)
    assert not _is_linked(a, 'java_ClassFile109', b2)
    if hasattr(b2, 'java_Package'):
        assert not _is_linked(b2, 'java_Package', a)


def test_assoc_package132_link_reassign_clear():
    a = java_CompilationUnit(originalFilePath="sample_text")
    b1 = java_Package()
    b2 = java_Package()
    _safe_set(a, 'java_CompilationUnit133', b1)
    assert _is_linked(a, 'java_CompilationUnit133', b1)
    if hasattr(b1, 'java_Package134'):
        assert _is_linked(b1, 'java_Package134', a)
    _safe_set(a, 'java_CompilationUnit133', b2)
    assert _is_linked(a, 'java_CompilationUnit133', b2)
    if hasattr(b1, 'java_Package134'):
        assert not _is_linked(b1, 'java_Package134', a)
    if hasattr(b2, 'java_Package134'):
        assert _is_linked(b2, 'java_Package134', a)
    _safe_set(a, 'java_CompilationUnit133', None)
    assert not _is_linked(a, 'java_CompilationUnit133', b2)
    if hasattr(b2, 'java_Package134'):
        assert not _is_linked(b2, 'java_Package134', a)


def test_assoc_parameter150_link_reassign_clear():
    a = java_SingleVariableDeclaration(varargs=True)
    b1 = java_EnhancedForStatement()
    b2 = java_EnhancedForStatement()
    _safe_set(a, 'SingleVariableDeclaration151', b1)
    assert _is_linked(a, 'SingleVariableDeclaration151', b1)
    if hasattr(b1, 'enhancedForStatement'):
        assert _is_linked(b1, 'enhancedForStatement', a)
    _safe_set(a, 'SingleVariableDeclaration151', b2)
    assert _is_linked(a, 'SingleVariableDeclaration151', b2)
    if hasattr(b1, 'enhancedForStatement'):
        assert not _is_linked(b1, 'enhancedForStatement', a)
    if hasattr(b2, 'enhancedForStatement'):
        assert _is_linked(b2, 'enhancedForStatement', a)
    _safe_set(a, 'SingleVariableDeclaration151', None)
    assert not _is_linked(a, 'SingleVariableDeclaration151', b2)
    if hasattr(b2, 'enhancedForStatement'):
        assert not _is_linked(b2, 'enhancedForStatement', a)
...
def test_assoc_redefinedMethodDeclaration219_link_reassign_clear():
...
def test_assoc_redefinitions221_link_reassign_clear():
    a = java_MethodDeclaration(extraArrayDimensions=7)
    b1 = java_MethodDeclaration(extraArrayDimensions=7)
    b2 = java_MethodDeclaration(extraArrayDimensions=13)
    _safe_set(a, 'MethodDeclaration222', b1)
    assert _is_linked(a, 'MethodDeclaration222', b1)
    if hasattr(b1, 'redefinedMethodDeclaration'):
        assert _is_linked(b1, 'redefinedMethodDeclaration', a)
    _safe_set(a, 'MethodDeclaration222', b2)
    assert _is_linked(a, 'MethodDeclaration222', b2)
    if hasattr(b1, 'redefinedMethodDeclaration'):
    ...
    _safe_set(a, 'MethodDeclaration222', None)
    assert not _is_linked(a, 'MethodDeclaration222', b2)
    if hasattr(b2, 'redefinedMethodDeclaration'):
...
def test_assoc_returnType216_link_reassign_clear():
    ...
    if hasattr(b1, 'java_TypeAccess217'):
        assert _is_linked(b1, 'java_TypeAccess217', a)
    _safe_set(a, 'java_MethodDeclaration', b2)
    assert _is_linked(a, 'java_MethodDeclaration', b2)
    if hasattr(b1, 'java_TypeAccess217'):
        assert not _is_linked(b1, 'java_TypeAccess217', a)
    if hasattr(b2, 'java_TypeAccess217'):
        assert _is_linked(b2, 'java_TypeAccess217', a)
    _safe_set(a, 'java_MethodDeclaration', None)
    assert not _is_linked(a, 'java_MethodDeclaration', b2)
    if hasattr(b2, 'java_TypeAccess217'):
        assert not _is_linked(b2, 'java_TypeAccess217', a)


def test_assoc_rightHandSide81_link_reassign_clear():
    a = java_Assignment(operator="sample_text")
    b1 = java_Expression()
    b2 = java_Expression()
    _safe_set(a, 'java_Assignment82', b1)
    assert _is_linked(a, 'java_Assignment82', b1)
    if hasattr(b1, 'java_Expression83'):
        assert _is_linked(b1, 'java_Expression83', a)
    _safe_set(a, 'java_Assignment82', b2)
    assert _is_linked(a, 'java_Assignment82', b2)
    if hasattr(b1, 'java_Expression83'):
        assert not _is_linked(b1, 'java_Expression83', a)
    if hasattr(b2, 'java_Expression83'):
        assert _is_linked(b2, 'java_Expression83', a)
    _safe_set(a, 'java_Assignment82', None)
    assert not _is_linked(a, 'java_Assignment82', b2)
    if hasattr(b2, 'java_Expression83'):
...
def test_assoc_type102_link_reassign_clear():
    ...
    b1 = java_AbstractTypeDeclaration()
    b2 = java_AbstractTypeDeclaration()
    _safe_set(a, 'java_ClassFile103', b1)
    assert _is_linked(a, 'java_ClassFile103', b1)
    if hasattr(b1, 'java_AbstractTypeDeclaration104'):
        assert _is_linked(b1, 'java_AbstractTypeDeclaration104', a)
    _safe_set(a, 'java_ClassFile103', b2)
    assert _is_linked(a, 'java_ClassFile103', b2)
    if hasattr(b1, 'java_AbstractTypeDeclaration104'):
        assert not _is_linked(b1, 'java_AbstractTypeDeclaration104', a)
    if hasattr(b2, 'java_AbstractTypeDeclaration104'):
        assert _is_linked(b2, 'java_AbstractTypeDeclaration104', a)
    _safe_set(a, 'java_ClassFile103', None)
    assert not _is_linked(a, 'java_ClassFile103', b2)
    if hasattr(b2, 'java_AbstractTypeDeclaration104'):
        assert not _is_linked(b2, 'java_AbstractTypeDeclaration104', a)
...
def test_assoc_type294_link_reassign_clear():
    a = java_SingleVariableDeclaration(varargs=True)
    b1 = java_TypeAccess()
    b2 = java_TypeAccess()
    _safe_set(a, 'java_SingleVariableDeclaration', b1)
    assert _is_linked(a, 'java_SingleVariableDeclaration', b1)
    if hasattr(b1, 'java_TypeAccess295'):
        assert _is_linked(b1, 'java_TypeAccess295', a)
    _safe_set(a, 'java_SingleVariableDeclaration', b2)
    assert _is_linked(a, 'java_SingleVariableDeclaration', b2)
    if hasattr(b1, 'java_TypeAccess295'):
        assert not _is_linked(b1, 'java_TypeAccess295', a)
    if hasattr(b2, 'java_TypeAccess295'):
        assert _is_linked(b2, 'java_TypeAccess295', a)
    _safe_set(a, 'java_SingleVariableDeclaration', None)
    assert not _is_linked(a, 'java_SingleVariableDeclaration', b2)
    if hasattr(b2, 'java_TypeAccess295'):
        assert not _is_linked(b2, 'java_TypeAccess295', a)


def test_assoc_types135_link_reassign_clear():
    a = java_CompilationUnit(originalFilePath="sample_text")
    b1 = java_AbstractTypeDeclaration()
    b2 = java_AbstractTypeDeclaration()
    _safe_set(a, 'java_CompilationUnit136', {b1})
    assert _is_linked(a, 'java_CompilationUnit136', b1)
    if hasattr(b1, 'java_AbstractTypeDeclaration137'):
        assert _is_linked(b1, 'java_AbstractTypeDeclaration137', a)
    _safe_set(a, 'java_CompilationUnit136', {b2})
    assert _is_linked(a, 'java_CompilationUnit136', b2)
    if hasattr(b1, 'java_AbstractTypeDeclaration137'):
        assert not _is_linked(b1, 'java_AbstractTypeDeclaration137', a)
    if hasattr(b2, 'java_AbstractTypeDeclaration137'):
        assert _is_linked(b2, 'java_AbstractTypeDeclaration137', a)
    _safe_set(a, 'java_CompilationUnit136', set())
    assert not _is_linked(a, 'java_CompilationUnit136', b2)
    if hasattr(b2, 'java_AbstractTypeDeclaration137'):
        assert not _is_linked(b2, 'java_AbstractTypeDeclaration137', a)
...
def test_assoc_variable287_link_reassign_clear():
    a = java_VariableDeclaration(extraArrayDimensions=7)
    ...
    if hasattr(b1, 'java_SingleVariableAccess288'):
        assert _is_linked(b1, 'java_SingleVariableAccess288', a)
    _safe_set(a, 'java_VariableDeclaration', b2)
    assert _is_linked(a, 'java_VariableDeclaration', b2)
    if hasattr(b1, 'java_SingleVariableAccess288'):
        assert not _is_linked(b1, 'java_SingleVariableAccess288', a)
    if hasattr(b2, 'java_SingleVariableAccess288'):
        assert _is_linked(b2, 'java_SingleVariableAccess288', a)
    _safe_set(a, 'java_VariableDeclaration', None)
    assert not _is_linked(a, 'java_VariableDeclaration', b2)
    if hasattr(b2, 'java_SingleVariableAccess288'):
        assert not _is_linked(b2, 'java_SingleVariableAccess288', a)
