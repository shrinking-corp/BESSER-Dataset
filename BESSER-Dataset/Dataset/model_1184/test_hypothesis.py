...
def test_gbind_dsl_bindingmodel_constructor_args():
    ...
    params = list(sig.parameters.keys())
    assert "name" in params, "Missing parameter 'name'"
    assert "targetBinding" in params, "Missing parameter 'targetBinding'"
...
def test_gbind_dsl_bindingmodel_has_targetBinding():
    assert hasattr(gbind_dsl_BindingModel, "targetBinding")
    descriptor = None
    for klass in gbind_dsl_BindingModel.__mro__:
        if "targetBinding" in klass.__dict__:
            descriptor = klass.__dict__["targetBinding"]
            break
    assert isinstance(descriptor, property)
...
def test_gbind_dsl_virtualtupleexp_is_not_abstract():
    assert not inspect.isabstract(gbind_dsl_VirtualTupleExp)


def test_gbind_dsl_virtualtupleexp_constructor_exists():
    assert callable(gbind_dsl_VirtualTupleExp.__init__)


def test_gbind_dsl_virtualtupleexp_constructor_args():
    sig = inspect.signature(gbind_dsl_VirtualTupleExp.__init__)
    params = list(sig.parameters.keys())
    assert "typeName" in params, "Missing parameter 'typeName'"

def test_gbind_dsl_virtualtupleexp_has_typeName():
    assert hasattr(gbind_dsl_VirtualTupleExp, "typeName")
    descriptor = None
    for klass in gbind_dsl_VirtualTupleExp.__mro__:
...
def test_gbind_dsl_metamodeldeclaration_has_resource():
    assert hasattr(gbind_dsl_MetamodelDeclaration, "resource")
    descriptor = None
    for klass in gbind_dsl_MetamodelDeclaration.__mro__:
...
gbind_dsl_VirtualTupleExp_strategy = st.builds(
    gbind_dsl_VirtualTupleExp,
    typeName=
        safe_text
)
...
@given(instance=gbind_dsl_BindingModel_strategy)
def test_gbind_dsl_bindingmodel_targetBinding_setter(instance):
    original = instance.targetBinding
    instance.targetBinding = original
    assert instance.targetBinding == original
...
@given(instance=gbind_dsl_VirtualTupleExp_strategy)
def test_gbind_dsl_virtualtupleexp_typeName_setter(instance):
...
@given(instance=gbind_dsl_MetamodelDeclaration_strategy)
def test_gbind_dsl_metamodeldeclaration_resource_setter(instance):
