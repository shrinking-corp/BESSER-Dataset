...
def test_hyp_walid_is_not_abstract():
    assert not inspect.isabstract(walid)


def test_hyp_walid_constructor_exists():
    assert callable(walid.__init__)


def test_hyp_walid_constructor_args():
    sig = inspect.signature(walid.__init__)
    params = list(sig.parameters.keys())
...
walid_strategy = st.builds(walid)
@given(instance=walid_strategy)
@settings(max_examples=25)
def test_walid_instantiation(instance):
    assert isinstance(instance, walid)
