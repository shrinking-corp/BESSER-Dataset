...
def test_assoc_a113_link_reassign_clear():
    ...
    if hasattr(b1, 'Pointer114'):
        assert _is_linked(b1, 'Pointer114', a)
    _safe_set(a, 'model_values_Connection', b2)
    assert _is_linked(a, 'model_values_Connection', b2)
    if hasattr(b1, 'Pointer114'):
        assert not _is_linked(b1, 'Pointer114', a)
    if hasattr(b2, 'Pointer114'):
        assert _is_linked(b2, 'Pointer114', a)
    _safe_set(a, 'model_values_Connection', None)
    assert not _is_linked(a, 'model_values_Connection', b2)
    if hasattr(b2, 'Pointer114'):
        assert not _is_linked(b2, 'Pointer114', a)


def test_assoc_anonymousTypes128_link_reassign_clear():
    ...
    if hasattr(b1, 'Type129'):
        assert _is_linked(b1, 'Type129', a)
    _safe_set(a, 'model_variables_Variable', {b2})
    assert _is_linked(a, 'model_variables_Variable', b2)
    if hasattr(b1, 'Type129'):
        assert not _is_linked(b1, 'Type129', a)
    if hasattr(b2, 'Type129'):
        assert _is_linked(b2, 'Type129', a)
    _safe_set(a, 'model_variables_Variable', set())
    assert not _is_linked(a, 'model_variables_Variable', b2)
    if hasattr(b2, 'Type129'):
        assert not _is_linked(b2, 'Type129', a)


def test_assoc_arrayType58_link_reassign_clear():
...
def test_assoc_b115_link_reassign_clear():
    a = model_values_Connection(connectivity="sample_text")
    b1 = Pointer()
    b2 = Pointer()
    _safe_set(a, 'model_values_Connection116', b1)
    assert _is_linked(a, 'model_values_Connection116', b1)
    if hasattr(b1, 'Pointer117'):
        assert _is_linked(b1, 'Pointer117', a)
    _safe_set(a, 'model_values_Connection116', b2)
    assert _is_linked(a, 'model_values_Connection116', b2)
    if hasattr(b1, 'Pointer117'):
        assert not _is_linked(b1, 'Pointer117', a)
    if hasattr(b2, 'Pointer117'):
        assert _is_linked(b2, 'Pointer117', a)
    _safe_set(a, 'model_values_Connection116', None)
    assert not _is_linked(a, 'model_values_Connection116', b2)
    if hasattr(b2, 'Pointer117'):
        assert not _is_linked(b2, 'Pointer117', a)
...
def test_assoc_defaultValue60_link_reassign_clear():
    ...
    b1 = ArrayValue()
    b2 = ArrayValue()
    _safe_set(a, 'model_types_ArrayType61', b1)
    assert _is_linked(a, 'model_types_ArrayType61', b1)
    if hasattr(b1, 'ArrayValue'):
        assert _is_linked(b1, 'ArrayValue', a)
    _safe_set(a, 'model_types_ArrayType61', b2)
    assert _is_linked(a, 'model_types_ArrayType61', b2)
    if hasattr(b1, 'ArrayValue'):
        assert not _is_linked(b1, 'ArrayValue', a)
    if hasattr(b2, 'ArrayValue'):
        assert _is_linked(b2, 'ArrayValue', a)
    _safe_set(a, 'model_types_ArrayType61', None)
    assert not _is_linked(a, 'model_types_ArrayType61', b2)
    if hasattr(b2, 'ArrayValue'):
        assert not _is_linked(b2, 'ArrayValue', a)


def test_assoc_dependenciesLibrary146_link_reassign_clear():
    ...
    _safe_set(a, 'model_datasources_DataSource147', {b1})
    assert _is_linked(a, 'model_datasources_DataSource147', b1)
    if hasattr(b1, 'datasources_model_GeppettoLibrary'):
        assert _is_linked(b1, 'datasources_model_GeppettoLibrary', a)
    _safe_set(a, 'model_datasources_DataSource147', {b2})
    assert _is_linked(a, 'model_datasources_DataSource147', b2)
    if hasattr(b1, 'datasources_model_GeppettoLibrary'):
    ...
    _safe_set(a, 'model_datasources_DataSource147', set())
    assert not _is_linked(a, 'model_datasources_DataSource147', b2)
    if hasattr(b2, 'datasources_model_GeppettoLibrary'):
        assert not _is_linked(b2, 'datasources_model_GeppettoLibrary', a)


def test_assoc_distal104_link_reassign_clear():
    ...
    if hasattr(b1, 'Point105'):
        assert _is_linked(b1, 'Point105', a)
    _safe_set(a, 'model_values_Cylinder', b2)
    assert _is_linked(a, 'model_values_Cylinder', b2)
    if hasattr(b1, 'Point105'):
        assert not _is_linked(b1, 'Point105', a)
    if hasattr(b2, 'Point105'):
        assert _is_linked(b2, 'Point105', a)
    _safe_set(a, 'model_values_Cylinder', None)
    assert not _is_linked(a, 'model_values_Cylinder', b2)
    if hasattr(b2, 'Point105'):
        assert not _is_linked(b2, 'Point105', a)
...
def test_assoc_elements81_link_reassign_clear():
...
def test_assoc_fetchVariableQuery151_link_reassign_clear():
    a = model_datasources_DataSource(dataSourceService="sample_text", url="sample_text")
    b1 = Query()
    b2 = Query()
    _safe_set(a, 'model_datasources_DataSource152', b1)
    assert _is_linked(a, 'model_datasources_DataSource152', b1)
    if hasattr(b1, 'Query153'):
        assert _is_linked(b1, 'Query153', a)
    _safe_set(a, 'model_datasources_DataSource152', b2)
    assert _is_linked(a, 'model_datasources_DataSource152', b2)
    if hasattr(b1, 'Query153'):
        assert not _is_linked(b1, 'Query153', a)
    if hasattr(b2, 'Query153'):
        assert _is_linked(b2, 'Query153', a)
    _safe_set(a, 'model_datasources_DataSource152', None)
    assert not _is_linked(a, 'model_datasources_DataSource152', b2)
    if hasattr(b2, 'Query153'):
        assert not _is_linked(b2, 'Query153', a)
...
def test_assoc_initialValue120_link_reassign_clear():
    a = model_values_ArrayElement(index="sample_text")
    b1 = Value()
    b2 = Value()
    _safe_set(a, 'model_values_ArrayElement121', b1)
    assert _is_linked(a, 'model_values_ArrayElement121', b1)
    if hasattr(b1, 'Value122'):
        assert _is_linked(b1, 'Value122', a)
    _safe_set(a, 'model_values_ArrayElement121', b2)
    assert _is_linked(a, 'model_values_ArrayElement121', b2)
    if hasattr(b1, 'Value122'):
        assert not _is_linked(b1, 'Value122', a)
    if hasattr(b2, 'Value122'):
        assert _is_linked(b2, 'Value122', a)
    _safe_set(a, 'model_values_ArrayElement121', None)
    assert not _is_linked(a, 'model_values_ArrayElement121', b2)
    if hasattr(b2, 'Value122'):
        assert not _is_linked(b2, 'Value122', a)


def test_assoc_initialValues132_link_reassign_clear():
    a = model_variables_Variable(static="sample_text")
    b1 = TypeToValueMap()
    b2 = TypeToValueMap()
    _safe_set(a, 'model_variables_Variable133', {b1})
    assert _is_linked(a, 'model_variables_Variable133', b1)
    if hasattr(b1, 'TypeToValueMap'):
        assert _is_linked(b1, 'TypeToValueMap', a)
    _safe_set(a, 'model_variables_Variable133', {b2})
    assert _is_linked(a, 'model_variables_Variable133', b2)
    if hasattr(b1, 'TypeToValueMap'):
        assert not _is_linked(b1, 'TypeToValueMap', a)
    if hasattr(b2, 'TypeToValueMap'):
        assert _is_linked(b2, 'TypeToValueMap', a)
    _safe_set(a, 'model_variables_Variable133', set())
    assert not _is_linked(a, 'model_variables_Variable133', b2)
    if hasattr(b2, 'TypeToValueMap'):
...
def test_assoc_library154_link_reassign_clear():
    ...
    if hasattr(b1, 'datasources_model_GeppettoLibrary155'):
        assert _is_linked(b1, 'datasources_model_GeppettoLibrary155', a)
    ...
    if hasattr(b1, 'datasources_model_GeppettoLibrary155'):
        assert not _is_linked(b1, 'datasources_model_GeppettoLibrary155', a)
    if hasattr(b2, 'datasources_model_GeppettoLibrary155'):
        assert _is_linked(b2, 'datasources_model_GeppettoLibrary155', a)
    ...
    if hasattr(b2, 'datasources_model_GeppettoLibrary155'):
        assert not _is_linked(b2, 'datasources_model_GeppettoLibrary155', a)


def test_assoc_libraryConfigurations142_link_reassign_clear():
    a = model_datasources_DataSource(dataSourceService="sample_text", url="sample_text")
...
def test_assoc_matchingCriteria156_link_reassign_clear():
...
def test_assoc_parameter109_link_reassign_clear():
    ...
    if hasattr(b1, 'Quantity110'):
        assert _is_linked(b1, 'Quantity110', a)
    _safe_set(a, 'model_values_VisualGroupElement', b2)
    assert _is_linked(a, 'model_values_VisualGroupElement', b2)
    if hasattr(b1, 'Quantity110'):
        assert not _is_linked(b1, 'Quantity110', a)
    if hasattr(b2, 'Quantity110'):
        assert _is_linked(b2, 'Quantity110', a)
    _safe_set(a, 'model_values_VisualGroupElement', None)
    assert not _is_linked(a, 'model_values_VisualGroupElement', b2)
    if hasattr(b2, 'Quantity110'):
        assert not _is_linked(b2, 'Quantity110', a)


def test_assoc_parameters160_link_reassign_clear():
...
def test_assoc_point82_link_reassign_clear():
    a = model_values_Pointer(path="sample_text")
    b1 = Point()
    b2 = Point()
    _safe_set(a, 'model_values_Pointer83', b1)
    assert _is_linked(a, 'model_values_Pointer83', b1)
    if hasattr(b1, 'Point84'):
        assert _is_linked(b1, 'Point84', a)
    _safe_set(a, 'model_values_Pointer83', b2)
    assert _is_linked(a, 'model_values_Pointer83', b2)
    if hasattr(b1, 'Point84'):
        assert not _is_linked(b1, 'Point84', a)
    if hasattr(b2, 'Point84'):
        assert _is_linked(b2, 'Point84', a)
    _safe_set(a, 'model_values_Pointer83', None)
    assert not _is_linked(a, 'model_values_Pointer83', b2)
    if hasattr(b2, 'Point84'):
        assert not _is_linked(b2, 'Point84', a)


def test_assoc_position118_link_reassign_clear():
    a = model_values_ArrayElement(index="sample_text")
    b1 = Point()
    b2 = Point()
    _safe_set(a, 'model_values_ArrayElement', b1)
    assert _is_linked(a, 'model_values_ArrayElement', b1)
    if hasattr(b1, 'Point119'):
        assert _is_linked(b1, 'Point119', a)
    _safe_set(a, 'model_values_ArrayElement', b2)
    assert _is_linked(a, 'model_values_ArrayElement', b2)
    if hasattr(b1, 'Point119'):
        assert not _is_linked(b1, 'Point119', a)
    if hasattr(b2, 'Point119'):
        assert _is_linked(b2, 'Point119', a)
    _safe_set(a, 'model_values_ArrayElement', None)
    assert not _is_linked(a, 'model_values_ArrayElement', b2)
    if hasattr(b2, 'Point119'):
        assert not _is_linked(b2, 'Point119', a)


def test_assoc_position134_link_reassign_clear():
    a = model_variables_Variable(static="sample_text")
    b1 = Point()
    b2 = Point()
    _safe_set(a, 'model_variables_Variable135', b1)
    assert _is_linked(a, 'model_variables_Variable135', b1)
    if hasattr(b1, 'Point136'):
        assert _is_linked(b1, 'Point136', a)
    _safe_set(a, 'model_variables_Variable135', b2)
    assert _is_linked(a, 'model_variables_Variable135', b2)
    if hasattr(b1, 'Point136'):
        assert not _is_linked(b1, 'Point136', a)
    if hasattr(b2, 'Point136'):
        assert _is_linked(b2, 'Point136', a)
    _safe_set(a, 'model_variables_Variable135', None)
    assert not _is_linked(a, 'model_variables_Variable135', b2)
    if hasattr(b2, 'Point136'):
        assert not _is_linked(b2, 'Point136', a)


def test_assoc_queries143_link_reassign_clear():
    a = model_datasources_DataSource(dataSourceService="sample_text", url="sample_text")
    b1 = Query()
    b2 = Query()
    _safe_set(a, 'model_datasources_DataSource144', {b1})
    assert _is_linked(a, 'model_datasources_DataSource144', b1)
    if hasattr(b1, 'Query145'):
        assert _is_linked(b1, 'Query145', a)
    _safe_set(a, 'model_datasources_DataSource144', {b2})
    assert _is_linked(a, 'model_datasources_DataSource144', b2)
    if hasattr(b1, 'Query145'):
        assert not _is_linked(b1, 'Query145', a)
    if hasattr(b2, 'Query145'):
        assert _is_linked(b2, 'Query145', a)
    _safe_set(a, 'model_datasources_DataSource144', set())
    assert not _is_linked(a, 'model_datasources_DataSource144', b2)
    if hasattr(b2, 'Query145'):
        assert not _is_linked(b2, 'Query145', a)
...
def test_assoc_results165_link_reassign_clear():
...
def test_assoc_returnType157_link_reassign_clear():
    a = model_datasources_Query(description="sample_text", runForCount="sample_text")
    b1 = Type()
    b2 = Type()
    _safe_set(a, 'model_datasources_Query158', b1)
    assert _is_linked(a, 'model_datasources_Query158', b1)
    if hasattr(b1, 'Type159'):
        assert _is_linked(b1, 'Type159', a)
    _safe_set(a, 'model_datasources_Query158', b2)
    assert _is_linked(a, 'model_datasources_Query158', b2)
    if hasattr(b1, 'Type159'):
        assert not _is_linked(b1, 'Type159', a)
    if hasattr(b2, 'Type159'):
        assert _is_linked(b2, 'Type159', a)
    _safe_set(a, 'model_datasources_Query158', None)
    assert not _is_linked(a, 'model_datasources_Query158', b2)
    if hasattr(b2, 'Type159'):
        assert not _is_linked(b2, 'Type159', a)
...
def test_assoc_targetLibrary148_link_reassign_clear():
    a = model_datasources_DataSource(dataSourceService="sample_text", url="sample_text")
    b1 = datasources_model_GeppettoLibrary()
    b2 = datasources_model_GeppettoLibrary()
    _safe_set(a, 'model_datasources_DataSource149', b1)
    assert _is_linked(a, 'model_datasources_DataSource149', b1)
    if hasattr(b1, 'datasources_model_GeppettoLibrary150'):
        assert _is_linked(b1, 'datasources_model_GeppettoLibrary150', a)
    _safe_set(a, 'model_datasources_DataSource149', b2)
    assert _is_linked(a, 'model_datasources_DataSource149', b2)
    if hasattr(b1, 'datasources_model_GeppettoLibrary150'):
        assert not _is_linked(b1, 'datasources_model_GeppettoLibrary150', a)
    if hasattr(b2, 'datasources_model_GeppettoLibrary150'):
        assert _is_linked(b2, 'datasources_model_GeppettoLibrary150', a)
    _safe_set(a, 'model_datasources_DataSource149', None)
    assert not _is_linked(a, 'model_datasources_DataSource149', b2)
    if hasattr(b2, 'datasources_model_GeppettoLibrary150'):
        assert not _is_linked(b2, 'datasources_model_GeppettoLibrary150', a)
...
def test_assoc_type87_link_reassign_clear():
    ...
    b1 = Type()
    b2 = Type()
    _safe_set(a, 'model_values_PointerElement88', b1)
    assert _is_linked(a, 'model_values_PointerElement88', b1)
    if hasattr(b1, 'Type89'):
        assert _is_linked(b1, 'Type89', a)
    _safe_set(a, 'model_values_PointerElement88', b2)
    assert _is_linked(a, 'model_values_PointerElement88', b2)
    if hasattr(b1, 'Type89'):
        assert not _is_linked(b1, 'Type89', a)
    if hasattr(b2, 'Type89'):
        assert _is_linked(b2, 'Type89', a)
    _safe_set(a, 'model_values_PointerElement88', None)
    assert not _is_linked(a, 'model_values_PointerElement88', b2)
    if hasattr(b2, 'Type89'):
...
def test_assoc_types130_link_reassign_clear():
    ...
    if hasattr(b1, 'Type131'):
        assert _is_linked(b1, 'Type131', a)
    _safe_set(a, 'referencedVariables', {b2})
    assert _is_linked(a, 'referencedVariables', b2)
    if hasattr(b1, 'Type131'):
        assert not _is_linked(b1, 'Type131', a)
    if hasattr(b2, 'Type131'):
        assert _is_linked(b2, 'Type131', a)
    _safe_set(a, 'referencedVariables', set())
    assert not _is_linked(a, 'referencedVariables', b2)
    if hasattr(b2, 'Type131'):
        assert not _is_linked(b2, 'Type131', a)
...
def test_assoc_visualGroupElements111_link_reassign_clear():
    ...
    if hasattr(b1, 'VisualGroupElement112'):
        assert _is_linked(b1, 'VisualGroupElement112', a)
    _safe_set(a, 'model_values_VisualGroup', {b2})
    assert _is_linked(a, 'model_values_VisualGroup', b2)
    if hasattr(b1, 'VisualGroupElement112'):
        assert not _is_linked(b1, 'VisualGroupElement112', a)
    if hasattr(b2, 'VisualGroupElement112'):
        assert _is_linked(b2, 'VisualGroupElement112', a)
    _safe_set(a, 'model_values_VisualGroup', set())
    assert not _is_linked(a, 'model_values_VisualGroup', b2)
    if hasattr(b2, 'VisualGroupElement112'):
        assert not _is_linked(b2, 'VisualGroupElement112', a)
