





import java.util.List;
import java.util.ArrayList;

public class sql_Operand  {






    private sql_OpFunction sql_opfunction;




    private sql_Operands sql_operands;




    private sql_FunctionExtract sql_functionextract;




    private sql_ExpOperand sql_expoperand;




    private sql_Operands sql_operands;




    private sql_POperand sql_poperand;




    private sql_SQLCaseOperand sql_sqlcaseoperand;




    private sql_ColumnOperand sql_columnoperand;




    private sql_OpFunctionCast sql_opfunctioncast;




    private sql_ScalarOperand sql_scalaroperand;




    private sql_SubQueryOperand sql_subqueryoperand;




    private sql_Operand sql_operand;


    public sql_Operand(
    ) {
    }



    public sql_OpFunction getSql_opfunction() {
        return sql_opfunction;
    }

    public void setSql_opfunction(sql_OpFunction sql_opfunction) {
        this.sql_opfunction = sql_opfunction;
    }
    public sql_Operands getSql_operands() {
        return sql_operands;
    }

    public void setSql_operands(sql_Operands sql_operands) {
        this.sql_operands = sql_operands;
    }
    public sql_FunctionExtract getSql_functionextract() {
        return sql_functionextract;
    }

    public void setSql_functionextract(sql_FunctionExtract sql_functionextract) {
        this.sql_functionextract = sql_functionextract;
    }
    public sql_ExpOperand getSql_expoperand() {
        return sql_expoperand;
    }

    public void setSql_expoperand(sql_ExpOperand sql_expoperand) {
        this.sql_expoperand = sql_expoperand;
    }
    public sql_Operands getSql_operands() {
        return sql_operands;
    }

    public void setSql_operands(sql_Operands sql_operands) {
        this.sql_operands = sql_operands;
    }
    public sql_POperand getSql_poperand() {
        return sql_poperand;
    }

    public void setSql_poperand(sql_POperand sql_poperand) {
        this.sql_poperand = sql_poperand;
    }
    public sql_SQLCaseOperand getSql_sqlcaseoperand() {
        return sql_sqlcaseoperand;
    }

    public void setSql_sqlcaseoperand(sql_SQLCaseOperand sql_sqlcaseoperand) {
        this.sql_sqlcaseoperand = sql_sqlcaseoperand;
    }
    public sql_ColumnOperand getSql_columnoperand() {
        return sql_columnoperand;
    }

    public void setSql_columnoperand(sql_ColumnOperand sql_columnoperand) {
        this.sql_columnoperand = sql_columnoperand;
    }
    public sql_OpFunctionCast getSql_opfunctioncast() {
        return sql_opfunctioncast;
    }

    public void setSql_opfunctioncast(sql_OpFunctionCast sql_opfunctioncast) {
        this.sql_opfunctioncast = sql_opfunctioncast;
    }
    public sql_ScalarOperand getSql_scalaroperand() {
        return sql_scalaroperand;
    }

    public void setSql_scalaroperand(sql_ScalarOperand sql_scalaroperand) {
        this.sql_scalaroperand = sql_scalaroperand;
    }
    public sql_SubQueryOperand getSql_subqueryoperand() {
        return sql_subqueryoperand;
    }

    public void setSql_subqueryoperand(sql_SubQueryOperand sql_subqueryoperand) {
        this.sql_subqueryoperand = sql_subqueryoperand;
    }
    public sql_Operand getSql_operand() {
        return sql_operand;
    }

    public void setSql_operand(sql_Operand sql_operand) {
        this.sql_operand = sql_operand;
    }

}