...
def test_hyp_model_roundedrectangle_is_not_abstract():
    assert not inspect.isabstract(model_RoundedRectangle)


def test_hyp_model_roundedrectangle_constructor_exists():
    assert callable(model_RoundedRectangle.__init__)


def test_hyp_model_roundedrectangle_constructor_args():
...
def test_hyp_model_polygon_is_not_abstract():
    assert not inspect.isabstract(model_Polygon)


def test_hyp_model_polygon_constructor_exists():
    assert callable(model_Polygon.__init__)


def test_hyp_model_polygon_constructor_args():
...
def test_hyp_model_timetrigger_is_not_abstract():
    assert not inspect.isabstract(model_TimeTrigger)


def test_hyp_model_timetrigger_constructor_exists():
    assert callable(model_TimeTrigger.__init__)


def test_hyp_model_timetrigger_constructor_args():
...
@given(instance=model_Text_strategy)
def test_hyp_model_text_alpha_setter(instance):
...
def test_hyp_model_timetrigger_onTrigger_setter(instance):
...
@given(instance=model_TimeTrigger_strategy)
def test_hyp_model_timetrigger_period_setter(instance):
...
@given(instance=model_Symbol_strategy)
def test_hyp_model_symbol_backgroundImage_setter(instance):
...
def test_model_Symbol_backgroundColor_value_roundtrip():
    instance = model_Symbol(backgroundColor="sample_text", backgroundImage="sample_text", onDispose="sample_text", onInit="sample_text", onUpdate="sample_text", scriptModules="sample_text")
    assert instance.backgroundColor == "sample_text"
    instance.backgroundColor = "sample_text_2"
    assert instance.backgroundColor == "sample_text_2"


def test_model_Symbol_backgroundImage_value_roundtrip():
    instance = model_Symbol(backgroundColor="sample_text", backgroundImage="sample_text", onDispose="sample_text", onInit="sample_text", onUpdate="sample_text", scriptModules="sample_text")
...
def test_model_Text_alpha_value_roundtrip():
    instance = model_Text(alpha="sample_text", fontBold=True, fontItalic=True, fontName="sample_text", fontSize=7, iconAlignment="sample_text", labelAlignment="sample_text", text="sample_text", textAlignment="sample_text", textPlacement="sample_text")
    assert instance.alpha == "sample_text"
    instance.alpha = "sample_text_2"
    assert instance.alpha == "sample_text_2"
...
def test_model_TimeTrigger_onTrigger_value_roundtrip():
    instance = model_TimeTrigger(onTrigger="sample_text", period="sample_text")
    assert instance.onTrigger == "sample_text"
    instance.onTrigger = "sample_text_2"
    assert instance.onTrigger == "sample_text_2"


def test_model_TimeTrigger_period_value_roundtrip():
    instance = model_TimeTrigger(onTrigger="sample_text", period="sample_text")
...
def test_model_Polygon_isa_Shape():
    instance = model_Polygon()
    assert isinstance(instance, Shape)
...
def test_model_RoundedRectangle_isa_Shape():
    instance = model_RoundedRectangle()
    assert isinstance(instance, Shape)


def test_assoc_children28_link_reassign_clear():
...
def test_assoc_children29_link_reassign_clear():
...
def test_assoc_children38_link_reassign_clear():
    a = model_Primitive(name="sample_text")
    b1 = model_StackContainer()
    b2 = model_StackContainer()
    _safe_set(a, 'model_Primitive39', b1)
    assert _is_linked(a, 'model_Primitive39', b1)
    if hasattr(b1, 'model_StackContainer'):
        assert _is_linked(b1, 'model_StackContainer', a)
    _safe_set(a, 'model_Primitive39', b2)
    assert _is_linked(a, 'model_Primitive39', b2)
    if hasattr(b1, 'model_StackContainer'):
    ...
    _safe_set(a, 'model_Primitive39', None)
    assert not _is_linked(a, 'model_Primitive39', b2)
    if hasattr(b2, 'model_StackContainer'):
        assert not _is_linked(b2, 'model_StackContainer', a)


def test_assoc_connections7_link_reassign_clear():
    a = model_Symbol(backgroundColor="sample_text", backgroundImage="sample_text", onDispose="sample_text", onInit="sample_text", onUpdate="sample_text", scriptModules="sample_text")
    b1 = model_Connection()
    b2 = model_Connection()
...
def test_assoc_content30_link_reassign_clear():
    a = model_Primitive(name="sample_text")
    b1 = model_FigureContainer()
    b2 = model_FigureContainer()
    _safe_set(a, 'model_Primitive31', b1)
    assert _is_linked(a, 'model_Primitive31', b1)
    if hasattr(b1, 'model_FigureContainer'):
        assert _is_linked(b1, 'model_FigureContainer', a)
    _safe_set(a, 'model_Primitive31', b2)
    assert _is_linked(a, 'model_Primitive31', b2)
    if hasattr(b1, 'model_FigureContainer'):
    ...
    _safe_set(a, 'model_Primitive31', None)
    assert not _is_linked(a, 'model_Primitive31', b2)
    if hasattr(b2, 'model_FigureContainer'):
        assert not _is_linked(b2, 'model_FigureContainer', a)


def test_assoc_corner42_link_reassign_clear():
    a = model_Dimension(height=3.14, width=3.14)
    b1 = model_RoundedRectangle()
    b2 = model_RoundedRectangle()
    _safe_set(a, 'model_Dimension43', b1)
    assert _is_linked(a, 'model_Dimension43', b1)
    if hasattr(b1, 'model_RoundedRectangle'):
        assert _is_linked(b1, 'model_RoundedRectangle', a)
    _safe_set(a, 'model_Dimension43', b2)
    assert _is_linked(a, 'model_Dimension43', b2)
    if hasattr(b1, 'model_RoundedRectangle'):
        assert not _is_linked(b1, 'model_RoundedRectangle', a)
    if hasattr(b2, 'model_RoundedRectangle'):
        assert _is_linked(b2, 'model_RoundedRectangle', a)
    _safe_set(a, 'model_Dimension43', None)
    assert not _is_linked(a, 'model_Dimension43', b2)
    if hasattr(b2, 'model_RoundedRectangle'):
        assert not _is_linked(b2, 'model_RoundedRectangle', a)


def test_assoc_cursor23_link_reassign_clear():
    a = model_Figure(backgroundColor="sample_text", border="sample_text", foregroundColor="sample_text", onClick="sample_text", onDoubleClick="sample_text", onMouseDrag="sample_text", onMouseHover="sample_text", onMouseIn="sample_text", onMouseMove="sample_text", onMouseOut="sample_text", opaque="sample_text", toolTip="sample_text", visible=True)
    b1 = model_Cursor()
    b2 = model_Cursor()
    _safe_set(a, 'model_Figure24', b1)
    assert _is_linked(a, 'model_Figure24', b1)
    if hasattr(b1, 'model_Cursor25'):
        assert _is_linked(b1, 'model_Cursor25', a)
    _safe_set(a, 'model_Figure24', b2)
    assert _is_linked(a, 'model_Figure24', b2)
    if hasattr(b1, 'model_Cursor25'):
        assert not _is_linked(b1, 'model_Cursor25', a)
    if hasattr(b2, 'model_Cursor25'):
        assert _is_linked(b2, 'model_Cursor25', a)
    _safe_set(a, 'model_Figure24', None)
    assert not _is_linked(a, 'model_Figure24', b2)
    if hasattr(b2, 'model_Cursor25'):
        assert not _is_linked(b2, 'model_Cursor25', a)
...
def test_assoc_dimension14_link_reassign_clear():
    a = model_Dimension(height=3.14, width=3.14)
    b1 = model_XYChild()
    b2 = model_XYChild()
    _safe_set(a, 'model_Dimension16', b1)
    assert _is_linked(a, 'model_Dimension16', b1)
    if hasattr(b1, 'model_XYChild15'):
        assert _is_linked(b1, 'model_XYChild15', a)
    _safe_set(a, 'model_Dimension16', b2)
    assert _is_linked(a, 'model_Dimension16', b2)
    if hasattr(b1, 'model_XYChild15'):
        assert not _is_linked(b1, 'model_XYChild15', a)
    if hasattr(b2, 'model_XYChild15'):
        assert _is_linked(b2, 'model_XYChild15', a)
    _safe_set(a, 'model_Dimension16', None)
    assert not _is_linked(a, 'model_Dimension16', b2)
    if hasattr(b2, 'model_XYChild15'):
        assert not _is_linked(b2, 'model_XYChild15', a)
...
def test_assoc_element11_link_reassign_clear():
    ...
    _safe_set(a, 'model_Primitive12', b1)
    assert _is_linked(a, 'model_Primitive12', b1)
    if hasattr(b1, 'model_Child'):
        assert _is_linked(b1, 'model_Child', a)
    _safe_set(a, 'model_Primitive12', b2)
    assert _is_linked(a, 'model_Primitive12', b2)
    if hasattr(b1, 'model_Child'):
        assert not _is_linked(b1, 'model_Child', a)
    if hasattr(b2, 'model_Child'):
        assert _is_linked(b2, 'model_Child', a)
    _safe_set(a, 'model_Primitive12', None)
    assert not _is_linked(a, 'model_Primitive12', b2)
    if hasattr(b2, 'model_Child'):
        assert not _is_linked(b2, 'model_Child', a)


def test_assoc_end35_link_reassign_clear():
    a = model_Primitive(name="sample_text")
    b1 = model_Connection()
    b2 = model_Connection()
    _safe_set(a, 'model_Primitive37', b1)
    assert _is_linked(a, 'model_Primitive37', b1)
    if hasattr(b1, 'model_Connection36'):
        assert _is_linked(b1, 'model_Connection36', a)
    _safe_set(a, 'model_Primitive37', b2)
    assert _is_linked(a, 'model_Primitive37', b2)
    if hasattr(b1, 'model_Connection36'):
        assert not _is_linked(b1, 'model_Connection36', a)
    if hasattr(b2, 'model_Connection36'):
        assert _is_linked(b2, 'model_Connection36', a)
    _safe_set(a, 'model_Primitive37', None)
    assert not _is_linked(a, 'model_Primitive37', b2)
    if hasattr(b2, 'model_Connection36'):
        assert not _is_linked(b2, 'model_Connection36', a)


def test_assoc_points19_link_reassign_clear():
    a = model_Position(x=3.14, y=3.14)
    b1 = model_Line()
    b2 = model_Line()
    _safe_set(a, 'model_Position20', b1)
    assert _is_linked(a, 'model_Position20', b1)
    if hasattr(b1, 'model_Line'):
        assert _is_linked(b1, 'model_Line', a)
    _safe_set(a, 'model_Position20', b2)
    assert _is_linked(a, 'model_Position20', b2)
    if hasattr(b1, 'model_Line'):
        assert not _is_linked(b1, 'model_Line', a)
    if hasattr(b2, 'model_Line'):
        assert _is_linked(b2, 'model_Line', a)
    _safe_set(a, 'model_Position20', None)
    assert not _is_linked(a, 'model_Position20', b2)
    if hasattr(b2, 'model_Line'):
        assert not _is_linked(b2, 'model_Line', a)


def test_assoc_points40_link_reassign_clear():
    a = model_Position(x=3.14, y=3.14)
    b1 = model_Polygon()
    b2 = model_Polygon()
    _safe_set(a, 'model_Position41', b1)
    assert _is_linked(a, 'model_Position41', b1)
    if hasattr(b1, 'model_Polygon'):
        assert _is_linked(b1, 'model_Polygon', a)
    _safe_set(a, 'model_Position41', b2)
    assert _is_linked(a, 'model_Position41', b2)
    if hasattr(b1, 'model_Polygon'):
        assert not _is_linked(b1, 'model_Polygon', a)
    if hasattr(b2, 'model_Polygon'):
        assert _is_linked(b2, 'model_Polygon', a)
    _safe_set(a, 'model_Position41', None)
    assert not _is_linked(a, 'model_Position41', b2)
    if hasattr(b2, 'model_Polygon'):
        assert not _is_linked(b2, 'model_Polygon', a)
...
def test_assoc_properties26_link_reassign_clear():
    ...
    if hasattr(b1, 'model_StringToStringMap27'):
        assert _is_linked(b1, 'model_StringToStringMap27', a)
    _safe_set(a, 'model_SymbolReference', {b2})
    assert _is_linked(a, 'model_SymbolReference', b2)
    if hasattr(b1, 'model_StringToStringMap27'):
        assert not _is_linked(b1, 'model_StringToStringMap27', a)
    if hasattr(b2, 'model_StringToStringMap27'):
        assert _is_linked(b2, 'model_StringToStringMap27', a)
    _safe_set(a, 'model_SymbolReference', set())
    assert not _is_linked(a, 'model_SymbolReference', b2)
    if hasattr(b2, 'model_StringToStringMap27'):
        assert not _is_linked(b2, 'model_StringToStringMap27', a)
...
def test_assoc_size21_link_reassign_clear():
    a = model_Figure(backgroundColor="sample_text", border="sample_text", foregroundColor="sample_text", onClick="sample_text", onDoubleClick="sample_text", onMouseDrag="sample_text", onMouseHover="sample_text", onMouseIn="sample_text", onMouseMove="sample_text", onMouseOut="sample_text", opaque="sample_text", toolTip="sample_text", visible=True)
    b1 = model_Dimension(height=3.14, width=3.14)
    b2 = model_Dimension(height=9.99, width=9.99)
    _safe_set(a, 'model_Figure', b1)
    assert _is_linked(a, 'model_Figure', b1)
    if hasattr(b1, 'model_Dimension22'):
        assert _is_linked(b1, 'model_Dimension22', a)
    _safe_set(a, 'model_Figure', b2)
    assert _is_linked(a, 'model_Figure', b2)
    if hasattr(b1, 'model_Dimension22'):
        assert not _is_linked(b1, 'model_Dimension22', a)
    if hasattr(b2, 'model_Dimension22'):
        assert _is_linked(b2, 'model_Dimension22', a)
    _safe_set(a, 'model_Figure', None)
    assert not _is_linked(a, 'model_Figure', b2)
    if hasattr(b2, 'model_Dimension22'):
        assert not _is_linked(b2, 'model_Dimension22', a)


def test_assoc_start32_link_reassign_clear():
    a = model_Primitive(name="sample_text")
    b1 = model_Connection()
    b2 = model_Connection()
    _safe_set(a, 'model_Primitive34', b1)
    assert _is_linked(a, 'model_Primitive34', b1)
    if hasattr(b1, 'model_Connection33'):
        assert _is_linked(b1, 'model_Connection33', a)
    _safe_set(a, 'model_Primitive34', b2)
    assert _is_linked(a, 'model_Primitive34', b2)
    if hasattr(b1, 'model_Connection33'):
        assert not _is_linked(b1, 'model_Connection33', a)
    if hasattr(b2, 'model_Connection33'):
        assert _is_linked(b2, 'model_Connection33', a)
    _safe_set(a, 'model_Primitive34', None)
    assert not _is_linked(a, 'model_Primitive34', b2)
    if hasattr(b2, 'model_Connection33'):
        assert not _is_linked(b2, 'model_Connection33', a)


def test_assoc_timeTriggers9_link_reassign_clear():
    a = model_TimeTrigger(onTrigger="sample_text", period="sample_text")
    b1 = model_Symbol(backgroundColor="sample_text", backgroundImage="sample_text", onDispose="sample_text", onInit="sample_text", onUpdate="sample_text", scriptModules="sample_text")
    b2 = model_Symbol(backgroundColor="sample_text_2", backgroundImage="sample_text_2", onDispose="sample_text_2", onInit="sample_text_2", onUpdate="sample_text_2", scriptModules="sample_text_2")
    _safe_set(a, 'model_TimeTrigger', b1)
    assert _is_linked(a, 'model_TimeTrigger', b1)
    if hasattr(b1, 'model_Symbol10'):
        assert _is_linked(b1, 'model_Symbol10', a)
    _safe_set(a, 'model_TimeTrigger', b2)
    assert _is_linked(a, 'model_TimeTrigger', b2)
    if hasattr(b1, 'model_Symbol10'):
        assert not _is_linked(b1, 'model_Symbol10', a)
    if hasattr(b2, 'model_Symbol10'):
        assert _is_linked(b2, 'model_Symbol10', a)
    _safe_set(a, 'model_TimeTrigger', None)
    assert not _is_linked(a, 'model_TimeTrigger', b2)
    if hasattr(b2, 'model_Symbol10'):
        assert not _is_linked(b2, 'model_Symbol10', a)
...
model_Polygon_strategy = st.builds(model_Polygon)
...
model_RoundedRectangle_strategy = st.builds(model_RoundedRectangle)
...
model_Symbol_strategy = st.builds(model_Symbol, backgroundColor=safe_text, backgroundImage=safe_text, onDispose=safe_text, onInit=safe_text, onUpdate=safe_text, scriptModules=safe_text)
@given(instance=model_Symbol_strategy)
@settings(max_examples=25)
...
model_Text_strategy = st.builds(model_Text, alpha=safe_text, fontBold=st.booleans(), fontItalic=st.booleans(), fontName=safe_text, fontSize=st.integers(), iconAlignment=safe_text, labelAlignment=safe_text, text=safe_text, textAlignment=safe_text, textPlacement=safe_text)
@given(instance=model_Text_strategy)
@settings(max_examples=25)
...
model_TimeTrigger_strategy = st.builds(model_TimeTrigger, onTrigger=safe_text, period=safe_text)
@given(instance=model_TimeTrigger_strategy)
@settings(max_examples=25)
