...
def test_product_has_defined():
    assert hasattr(Product, "defined")
    descriptor = None
    for klass in Product.__mro__:
        if "defined" in klass.__dict__:
            descriptor = klass.__dict__["defined"]
            break
    assert isinstance(descriptor, property)
...
@given(instance=Product_strategy)
def test_product_defined_setter(instance):
