...
def test_ffddsff_is_not_abstract():
    assert not inspect.isabstract(ffddsff)


def test_ffddsff_constructor_exists():
    assert callable(ffddsff.__init__)


def test_ffddsff_constructor_args():
...
def test_ffddsff_has_packageAttribute():
    assert hasattr(ffddsff, "packageAttribute")
    descriptor = None
    for klass in ffddsff.__mro__:
        if "packageAttribute" in klass.__dict__:
            descriptor = klass.__dict__["packageAttribute"]
            break
    assert isinstance(descriptor, property)

def test_ffddsff_has_publicAttribute():
    assert hasattr(ffddsff, "publicAttribute")
    descriptor = None
    for klass in ffddsff.__mro__:
        if "publicAttribute" in klass.__dict__:
            descriptor = klass.__dict__["publicAttribute"]
...
def test_ffddsff_has_privateAttribute():
    assert hasattr(ffddsff, "privateAttribute")
    descriptor = None
    for klass in ffddsff.__mro__:
        if "privateAttribute" in klass.__dict__:
            descriptor = klass.__dict__["privateAttribute"]
...
def test_ffddsff_has_attribute():
    assert hasattr(ffddsff, "attribute")
    descriptor = None
    for klass in ffddsff.__mro__:
...
def test_ffddsff_has_protectedAttribute():
    assert hasattr(ffddsff, "protectedAttribute")
    descriptor = None
    for klass in ffddsff.__mro__:
        if "protectedAttribute" in klass.__dict__:
            descriptor = klass.__dict__["protectedAttribute"]
...
@given(instance=ffddsff_strategy)
def test_ffddsff_packageAttribute_setter(instance):
    original = instance.packageAttribute
    instance.packageAttribute = original
    assert instance.packageAttribute == original
...
@given(instance=ffddsff_strategy)
def test_ffddsff_publicAttribute_setter(instance):
    original = instance.publicAttribute
    instance.publicAttribute = original
    assert instance.publicAttribute == original
...
@given(instance=ffddsff_strategy)
def test_ffddsff_privateAttribute_setter(instance):
    original = instance.privateAttribute
    instance.privateAttribute = original
    assert instance.privateAttribute == original
...
@given(instance=ffddsff_strategy)
def test_ffddsff_attribute_setter(instance):
...
@given(instance=ffddsff_strategy)
def test_ffddsff_protectedAttribute_setter(instance):
    original = instance.protectedAttribute
    instance.protectedAttribute = original
    assert instance.protectedAttribute == original
