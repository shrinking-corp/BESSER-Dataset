...
def test_assoc_AfterTrans0_link_reassign_clear():
    a = SBCS_Snapshot()
    b1 = SBCS_Transition()
    b2 = SBCS_Transition()
...
def test_assoc_AfterTrans12_link_reassign_clear():
    a = SBCS_Snapshot()
    b1 = SBCS_Transition()
    b2 = SBCS_Transition()
    _safe_set(a, 'SBCS_Snapshot13', b1)
    assert _is_linked(a, 'SBCS_Snapshot13', b1)
    if hasattr(b1, 'SBCS_Transition14'):
        assert _is_linked(b1, 'SBCS_Transition14', a)
    _safe_set(a, 'SBCS_Snapshot13', b2)
    assert _is_linked(a, 'SBCS_Snapshot13', b2)
    if hasattr(b1, 'SBCS_Transition14'):
        assert not _is_linked(b1, 'SBCS_Transition14', a)
    if hasattr(b2, 'SBCS_Transition14'):
        assert _is_linked(b2, 'SBCS_Transition14', a)
    _safe_set(a, 'SBCS_Snapshot13', None)
    assert not _is_linked(a, 'SBCS_Snapshot13', b2)
    if hasattr(b2, 'SBCS_Transition14'):
        assert not _is_linked(b2, 'SBCS_Transition14', a)


def test_assoc_CPPost6_link_reassign_clear():
...
def test_assoc_ControlProgramSnapshot9_link_reassign_clear():
    a = SBCS_Snapshot()
    b1 = SBCS_ControlProgram(mode="sample_text", wlmdFailure=True)
    b2 = SBCS_ControlProgram(mode="sample_text_2", wlmdFailure=False)
    _safe_set(a, 'SBCS_Snapshot10', b1)
    assert _is_linked(a, 'SBCS_Snapshot10', b1)
    if hasattr(b1, 'SBCS_ControlProgram11'):
        assert _is_linked(b1, 'SBCS_ControlProgram11', a)
    _safe_set(a, 'SBCS_Snapshot10', b2)
    assert _is_linked(a, 'SBCS_Snapshot10', b2)
    if hasattr(b1, 'SBCS_ControlProgram11'):
        assert not _is_linked(b1, 'SBCS_ControlProgram11', a)
    if hasattr(b2, 'SBCS_ControlProgram11'):
        assert _is_linked(b2, 'SBCS_ControlProgram11', a)
    _safe_set(a, 'SBCS_Snapshot10', None)
    assert not _is_linked(a, 'SBCS_Snapshot10', b2)
    if hasattr(b2, 'SBCS_ControlProgram11'):
        assert not _is_linked(b2, 'SBCS_ControlProgram11', a)


def test_assoc_PumpControlerPump28_link_reassign_clear():
    ...
    if hasattr(b1, 'SBCS_PumpControler29'):
        assert _is_linked(b1, 'SBCS_PumpControler29', a)
    _safe_set(a, 'SBCS_Pump', b2)
    assert _is_linked(a, 'SBCS_Pump', b2)
    if hasattr(b1, 'SBCS_PumpControler29'):
        assert not _is_linked(b1, 'SBCS_PumpControler29', a)
    if hasattr(b2, 'SBCS_PumpControler29'):
        assert _is_linked(b2, 'SBCS_PumpControler29', a)
    _safe_set(a, 'SBCS_Pump', None)
    assert not _is_linked(a, 'SBCS_Pump', b2)
    if hasattr(b2, 'SBCS_PumpControler29'):
        assert not _is_linked(b2, 'SBCS_PumpControler29', a)


def test_assoc_SBPost17_link_reassign_clear():
    ...
    if hasattr(b1, 'SBCS_SteamBoiler_OpenValve18'):
        assert _is_linked(b1, 'SBCS_SteamBoiler_OpenValve18', a)
    _safe_set(a, 'SBCS_SteamBoiler19', b2)
    assert _is_linked(a, 'SBCS_SteamBoiler19', b2)
    if hasattr(b1, 'SBCS_SteamBoiler_OpenValve18'):
        assert not _is_linked(b1, 'SBCS_SteamBoiler_OpenValve18', a)
    if hasattr(b2, 'SBCS_SteamBoiler_OpenValve18'):
        assert _is_linked(b2, 'SBCS_SteamBoiler_OpenValve18', a)
    _safe_set(a, 'SBCS_SteamBoiler19', None)
    assert not _is_linked(a, 'SBCS_SteamBoiler19', b2)
    if hasattr(b2, 'SBCS_SteamBoiler_OpenValve18'):
        assert not _is_linked(b2, 'SBCS_SteamBoiler_OpenValve18', a)


def test_assoc_SBPre15_link_reassign_clear():
    a = SBCS_SteamBoiler(valveOpen="sample_text")
    b1 = SBCS_SteamBoiler_OpenValve()
    b2 = SBCS_SteamBoiler_OpenValve()
...
def test_assoc_SnapshotControlProgram25_link_reassign_clear():
    a = SBCS_Snapshot()
    b1 = SBCS_ControlProgram(mode="sample_text", wlmdFailure=True)
    b2 = SBCS_ControlProgram(mode="sample_text_2", wlmdFailure=False)
    _safe_set(a, 'SBCS_Snapshot27', b1)
    assert _is_linked(a, 'SBCS_Snapshot27', b1)
    if hasattr(b1, 'SBCS_ControlProgram26'):
        assert _is_linked(b1, 'SBCS_ControlProgram26', a)
    _safe_set(a, 'SBCS_Snapshot27', b2)
    assert _is_linked(a, 'SBCS_Snapshot27', b2)
    if hasattr(b1, 'SBCS_ControlProgram26'):
        assert not _is_linked(b1, 'SBCS_ControlProgram26', a)
    if hasattr(b2, 'SBCS_ControlProgram26'):
        assert _is_linked(b2, 'SBCS_ControlProgram26', a)
    _safe_set(a, 'SBCS_Snapshot27', None)
    assert not _is_linked(a, 'SBCS_Snapshot27', b2)
    if hasattr(b2, 'SBCS_ControlProgram26'):
        assert not _is_linked(b2, 'SBCS_ControlProgram26', a)


def test_assoc_SteamBoilerControlProgram7_link_reassign_clear():
    a = SBCS_SteamBoiler(valveOpen="sample_text")
    b1 = SBCS_ControlProgram(mode="sample_text", wlmdFailure=True)
    b2 = SBCS_ControlProgram(mode="sample_text_2", wlmdFailure=False)
    _safe_set(a, 'SBCS_SteamBoiler', b1)
    assert _is_linked(a, 'SBCS_SteamBoiler', b1)
    if hasattr(b1, 'SBCS_ControlProgram8'):
        assert _is_linked(b1, 'SBCS_ControlProgram8', a)
    _safe_set(a, 'SBCS_SteamBoiler', b2)
    assert _is_linked(a, 'SBCS_SteamBoiler', b2)
    if hasattr(b1, 'SBCS_ControlProgram8'):
        assert not _is_linked(b1, 'SBCS_ControlProgram8', a)
    if hasattr(b2, 'SBCS_ControlProgram8'):
        assert _is_linked(b2, 'SBCS_ControlProgram8', a)
    _safe_set(a, 'SBCS_SteamBoiler', None)
    assert not _is_linked(a, 'SBCS_SteamBoiler', b2)
    if hasattr(b2, 'SBCS_ControlProgram8'):
        assert not _is_linked(b2, 'SBCS_ControlProgram8', a)


def test_assoc_wlmdPost5_link_reassign_clear():
