...
def test_expediente_is_not_abstract():
    assert not inspect.isabstract(Expediente)


def test_expediente_constructor_exists():
    assert callable(Expediente.__init__)


def test_expediente_constructor_args():
    sig = inspect.signature(Expediente.__init__)
    params = list(sig.parameters.keys())
    assert "ownerName" in params, "Missing parameter 'ownerName'"
    assert "balance" in params, "Missing parameter 'balance'"

def test_expediente_has_ownerName():
    assert hasattr(Expediente, "ownerName")
    descriptor = None
    for klass in Expediente.__mro__:
        if "ownerName" in klass.__dict__:
            descriptor = klass.__dict__["ownerName"]
            break
    assert isinstance(descriptor, property)

def test_expediente_has_balance():
    assert hasattr(Expediente, "balance")
    descriptor = None
    for klass in Expediente.__mro__:
...
@given(instance=Expediente_strategy)
def test_expediente_ownerName_setter(instance):
...
@given(instance=Expediente_strategy)
def test_expediente_balance_setter(instance):
