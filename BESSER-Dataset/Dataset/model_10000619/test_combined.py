...
def test_hyp_expediente_is_not_abstract():
    assert not inspect.isabstract(Expediente)


def test_hyp_expediente_constructor_exists():
    assert callable(Expediente.__init__)


def test_hyp_expediente_constructor_args():
    sig = inspect.signature(Expediente.__init__)
    params = list(sig.parameters.keys())
...
@given(instance=Expediente_strategy)
def test_hyp_expediente_ownerName_setter(instance):
...
@given(instance=Expediente_strategy)
def test_hyp_expediente_balance_setter(instance):
...
def test_Expediente_balance_value_roundtrip():
    instance = Expediente(balance=3.14, ownerName="sample_text")
    assert instance.balance == 3.14
    instance.balance = 9.99
    assert instance.balance == 9.99


def test_Expediente_ownerName_value_roundtrip():
    instance = Expediente(balance=3.14, ownerName="sample_text")
...
Expediente_strategy = st.builds(Expediente, balance=st.floats(allow_nan=False, allow_infinity=False), ownerName=safe_text)
@given(instance=Expediente_strategy)
@settings(max_examples=25)
def test_Expediente_instantiation(instance):
    assert isinstance(instance, Expediente)
