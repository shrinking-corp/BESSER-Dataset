...
def test_hyp_changetype_exists():
    # Check that the Enumeration exists
    assert ChangeType is not None

def test_hyp_changetype_has_all_literals():
    # Collect the names of literals in this Enumeration
...
def test_hyp_arduino_waitfor_mode_setter(instance):
...
def test_arduino_WaitFor_mode_value_roundtrip():
    instance = arduino_WaitFor(mode="sample_text")
...
def test_arduino_WaitFor_isa_Utilities():
    instance = arduino_WaitFor(mode="sample_text")
    assert isinstance(instance, Utilities)
...
def test_assoc_pin34_link_reassign_clear():
    a = arduino_WaitFor(mode="sample_text")
    b1 = arduino_Pin(level=7)
    b2 = arduino_Pin(level=13)
    _safe_set(a, 'arduino_WaitFor', b1)
    assert _is_linked(a, 'arduino_WaitFor', b1)
    if hasattr(b1, 'arduino_Pin'):
        assert _is_linked(b1, 'arduino_Pin', a)
    _safe_set(a, 'arduino_WaitFor', b2)
    assert _is_linked(a, 'arduino_WaitFor', b2)
    if hasattr(b1, 'arduino_Pin'):
        assert not _is_linked(b1, 'arduino_Pin', a)
    if hasattr(b2, 'arduino_Pin'):
        assert _is_linked(b2, 'arduino_Pin', a)
    _safe_set(a, 'arduino_WaitFor', None)
    assert not _is_linked(a, 'arduino_WaitFor', b2)
    if hasattr(b2, 'arduino_Pin'):
        assert not _is_linked(b2, 'arduino_Pin', a)
...
def test_assoc_variable42_link_reassign_clear():
...
arduino_Pin_strategy = st.builds(arduino_Pin, level=st.integers())
@given(instance=arduino_Pin_strategy)
@settings(max_examples=25)
...
arduino_WaitFor_strategy = st.builds(arduino_WaitFor, mode=safe_text)
@given(instance=arduino_WaitFor_strategy)
@settings(max_examples=25)
