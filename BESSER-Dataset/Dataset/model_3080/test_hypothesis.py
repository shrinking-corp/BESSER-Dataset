...
def test_caltrop_realm_is_not_abstract():
    assert not inspect.isabstract(caltrop_Realm)


def test_caltrop_realm_constructor_exists():
    assert callable(caltrop_Realm.__init__)


def test_caltrop_realm_constructor_args():
    sig = inspect.signature(caltrop_Realm.__init__)
    params = list(sig.parameters.keys())
    assert "key" in params, "Missing parameter 'key'"

def test_caltrop_realm_has_key():
    assert hasattr(caltrop_Realm, "key")
    descriptor = None
    for klass in caltrop_Realm.__mro__:
...
def test_caltrop_statevariable_has_binding():
    assert hasattr(caltrop_StateVariable, "binding")
    descriptor = None
...
caltrop_Realm_strategy = st.builds(
    caltrop_Realm,
    key=
        safe_text
)
...
@given(instance=caltrop_Realm_strategy)
def test_caltrop_realm_key_setter(instance):
...
@given(instance=caltrop_StateVariable_strategy)
def test_caltrop_statevariable_binding_setter(instance):
