...
def test_assoc_BankAccount_ClassA_link_reassign_clear():
    a = ClassA(packageAttribute="sample_text", privateAttribute=7, protectedAttribute="sample_text", publicAttribute=3.14)
    b1 = BankAccount(balance=3.14, ownerName="sample_text")
    b2 = BankAccount(balance=9.99, ownerName="sample_text_2")
    _safe_set(a, 'bankAccount7', b1)
    assert _is_linked(a, 'bankAccount7', b1)
    if hasattr(b1, 'classA6'):
        assert _is_linked(b1, 'classA6', a)
    _safe_set(a, 'bankAccount7', b2)
    assert _is_linked(a, 'bankAccount7', b2)
    if hasattr(b1, 'classA6'):
        assert not _is_linked(b1, 'classA6', a)
    if hasattr(b2, 'classA6'):
        assert _is_linked(b2, 'classA6', a)
    _safe_set(a, 'bankAccount7', None)
    assert not _is_linked(a, 'bankAccount7', b2)
    if hasattr(b2, 'classA6'):
        assert not _is_linked(b2, 'classA6', a)
