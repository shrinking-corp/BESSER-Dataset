...
def test_ale_mult_has_op():
    assert hasattr(ale_Mult, "op")
    descriptor = None
    for klass in ale_Mult.__mro__:
        if "op" in klass.__dict__:
            descriptor = klass.__dict__["op"]
            break
    assert isinstance(descriptor, property)
...
def test_ale_add_has_op():
    assert hasattr(ale_Add, "op")
    descriptor = None
    for klass in ale_Add.__mro__:
        if "op" in klass.__dict__:
            descriptor = klass.__dict__["op"]
            break
    assert isinstance(descriptor, property)
...
@given(instance=ale_Mult_strategy)
def test_ale_mult_op_setter(instance):
    original = instance.op
    instance.op = original
    assert instance.op == original
...
@given(instance=ale_Add_strategy)
def test_ale_add_op_setter(instance):
    original = instance.op
    instance.op = original
    assert instance.op == original
