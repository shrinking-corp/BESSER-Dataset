...
def test_assoc_billing_loan_link_reassign_clear():
    ...
    _safe_set(a, 'billing_loan_115', b1)
    assert _is_linked(a, 'billing_loan_115', b1)
    if hasattr(b1, 'billing_loan_014'):
        assert _is_linked(b1, 'billing_loan_014', a)
    _safe_set(a, 'billing_loan_115', b2)
    assert _is_linked(a, 'billing_loan_115', b2)
    if hasattr(b1, 'billing_loan_014'):
        assert not _is_linked(b1, 'billing_loan_014', a)
    if hasattr(b2, 'billing_loan_014'):
        assert _is_linked(b2, 'billing_loan_014', a)
    _safe_set(a, 'billing_loan_115', None)
    assert not _is_linked(a, 'billing_loan_115', b2)
    if hasattr(b2, 'billing_loan_014'):
        assert not _is_linked(b2, 'billing_loan_014', a)
...
def test_assoc_department_staff_link_reassign_clear():
    ...
    _safe_set(a, 'department_staff_11', {b1})
    assert _is_linked(a, 'department_staff_11', b1)
    if hasattr(b1, 'department_staff_00'):
        assert _is_linked(b1, 'department_staff_00', a)
    _safe_set(a, 'department_staff_11', {b2})
    assert _is_linked(a, 'department_staff_11', b2)
    if hasattr(b1, 'department_staff_00'):
        assert not _is_linked(b1, 'department_staff_00', a)
    if hasattr(b2, 'department_staff_00'):
        assert _is_linked(b2, 'department_staff_00', a)
    _safe_set(a, 'department_staff_11', set())
    assert not _is_linked(a, 'department_staff_11', b2)
    if hasattr(b2, 'department_staff_00'):
        assert not _is_linked(b2, 'department_staff_00', a)
...
def test_assoc_doctor_test_link_reassign_clear():
    ...
    _safe_set(a, 'doctor_test_13', b1)
    assert _is_linked(a, 'doctor_test_13', b1)
    if hasattr(b1, 'doctor_test_02'):
        assert _is_linked(b1, 'doctor_test_02', a)
    _safe_set(a, 'doctor_test_13', b2)
    assert _is_linked(a, 'doctor_test_13', b2)
    if hasattr(b1, 'doctor_test_02'):
        assert not _is_linked(b1, 'doctor_test_02', a)
    if hasattr(b2, 'doctor_test_02'):
        assert _is_linked(b2, 'doctor_test_02', a)
    _safe_set(a, 'doctor_test_13', None)
    assert not _is_linked(a, 'doctor_test_13', b2)
    if hasattr(b2, 'doctor_test_02'):
        assert not _is_linked(b2, 'doctor_test_02', a)
...
def test_assoc_patient_billing_link_reassign_clear():
    ...
    b1 = Bill(amount="sample_text", bill_no="sample_text", patient_name="sample_text")
    b2 = Bill(amount="sample_text_2", bill_no="sample_text_2", patient_name="sample_text_2")
    _safe_set(a, 'patient_billing_08', b1)
    assert _is_linked(a, 'patient_billing_08', b1)
    if hasattr(b1, 'patient_billing_19'):
        assert _is_linked(b1, 'patient_billing_19', a)
    _safe_set(a, 'patient_billing_08', b2)
    assert _is_linked(a, 'patient_billing_08', b2)
    if hasattr(b1, 'patient_billing_19'):
        assert not _is_linked(b1, 'patient_billing_19', a)
    if hasattr(b2, 'patient_billing_19'):
        assert _is_linked(b2, 'patient_billing_19', a)
    _safe_set(a, 'patient_billing_08', None)
    assert not _is_linked(a, 'patient_billing_08', b2)
    if hasattr(b2, 'patient_billing_19'):
        assert not _is_linked(b2, 'patient_billing_19', a)
...
def test_assoc_patient_doctor_link_reassign_clear():
    ...
    b1 = doctor(dept="sample_text", did="sample_text", name="sample_text", phone_no="sample_text", specilization="sample_text")
    b2 = doctor(dept="sample_text_2", did="sample_text_2", name="sample_text_2", phone_no="sample_text_2", specilization="sample_text_2")
    _safe_set(a, 'patient_doctor_06', b1)
    assert _is_linked(a, 'patient_doctor_06', b1)
    if hasattr(b1, 'patient_doctor_17'):
        assert _is_linked(b1, 'patient_doctor_17', a)
    _safe_set(a, 'patient_doctor_06', b2)
    assert _is_linked(a, 'patient_doctor_06', b2)
    if hasattr(b1, 'patient_doctor_17'):
        assert not _is_linked(b1, 'patient_doctor_17', a)
    if hasattr(b2, 'patient_doctor_17'):
        assert _is_linked(b2, 'patient_doctor_17', a)
    _safe_set(a, 'patient_doctor_06', None)
    assert not _is_linked(a, 'patient_doctor_06', b2)
    if hasattr(b2, 'patient_doctor_17'):
        assert not _is_linked(b2, 'patient_doctor_17', a)
...
def test_assoc_patient_room_link_reassign_clear():
    ...
    _safe_set(a, 'patient_room_15', b1)
    assert _is_linked(a, 'patient_room_15', b1)
    if hasattr(b1, 'patient_room_04'):
        assert _is_linked(b1, 'patient_room_04', a)
    _safe_set(a, 'patient_room_15', b2)
    assert _is_linked(a, 'patient_room_15', b2)
    if hasattr(b1, 'patient_room_04'):
        assert not _is_linked(b1, 'patient_room_04', a)
    if hasattr(b2, 'patient_room_04'):
        assert _is_linked(b2, 'patient_room_04', a)
    _safe_set(a, 'patient_room_15', None)
    assert not _is_linked(a, 'patient_room_15', b2)
    if hasattr(b2, 'patient_room_04'):
        assert not _is_linked(b2, 'patient_room_04', a)
...
def test_assoc_receptionist_billing_link_reassign_clear():
    ...
    b1 = Bill(amount="sample_text", bill_no="sample_text", patient_name="sample_text")
    b2 = Bill(amount="sample_text_2", bill_no="sample_text_2", patient_name="sample_text_2")
    _safe_set(a, 'receptionist_billing_012', {b1})
    assert _is_linked(a, 'receptionist_billing_012', b1)
    if hasattr(b1, 'receptionist_billing_113'):
        assert _is_linked(b1, 'receptionist_billing_113', a)
    _safe_set(a, 'receptionist_billing_012', {b2})
    assert _is_linked(a, 'receptionist_billing_012', b2)
    if hasattr(b1, 'receptionist_billing_113'):
        assert not _is_linked(b1, 'receptionist_billing_113', a)
    if hasattr(b2, 'receptionist_billing_113'):
        assert _is_linked(b2, 'receptionist_billing_113', a)
    _safe_set(a, 'receptionist_billing_012', set())
    assert not _is_linked(a, 'receptionist_billing_012', b2)
    if hasattr(b2, 'receptionist_billing_113'):
        assert not _is_linked(b2, 'receptionist_billing_113', a)
