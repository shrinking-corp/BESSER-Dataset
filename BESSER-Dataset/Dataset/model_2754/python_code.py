...
class PK461726_B461726:
    ...
    def B461726(self, value):
        ...
        if old_value is not None:
            if hasattr(old_value, "as_"):
                opp_val = getattr(old_value, "as_", None)
                if isinstance(opp_val, set):
                    opp_val.discard(self)
                
        # Add self to new opposite end
        if value is not None:
            if hasattr(value, "as_"):
                opp_val = getattr(value, "as_", None)
                if opp_val is None:
                    setattr(value, "as_", set([self]))
...
class PK461726_A461726:
    ...
    def __init__(self, name: str, as_: set["PK461726_B461726"] = None, A461726: "PK461726_B461726" = None):
        self.name = name
        self.as_ = as_ if as_ is not None else set()
    ...
    def as_(self, value):
        # Bidirectional consistency
        old_value = getattr(self, f"_PK461726_A461726__as_", None)
        self.__as_ = value if value is not None else set()
