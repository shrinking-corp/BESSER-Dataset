...
def test_hyp_employee_emailaddress_name_setter(instance):
...
@given(instance=employee_EmailAddress_strategy)
def test_hyp_employee_emailaddress_id_setter(instance):
...
@given(instance=employee_Address_strategy)
def test_hyp_employee_address_id_setter(instance):
    original = instance.id
    instance.id = original
    assert instance.id == original
...
@given(instance=employee_EmploymentPeriod_strategy)
def test_hyp_employee_employmentperiod_id_setter(instance):
    original = instance.id
    instance.id = original
    assert instance.id == original
...
@given(instance=employee_Employee_strategy)
def test_hyp_employee_employee_version_setter(instance):
...
def test_employee_Address_city_value_roundtrip():
    instance = employee_Address(city="sample_text", country="sample_text", id=7, postalCode="sample_text", province="sample_text", street="sample_text")
...
def test_employee_EmailAddress_address_value_roundtrip():
    instance = employee_EmailAddress(address="sample_text", id=7, name="sample_text")
...
def test_employee_Employee_firstName_value_roundtrip():
    instance = employee_Employee(firstName="sample_text", gender="sample_text", lastName="sample_text", responsibilities="sample_text", salary=3.14, version="sample_text")
...
def test_employee_Employee_version_value_roundtrip():
    instance = employee_Employee(firstName="sample_text", gender="sample_text", lastName="sample_text", responsibilities="sample_text", salary=3.14, version="sample_text")
...
def test_employee_EmploymentPeriod_endDate_value_roundtrip():
    instance = employee_EmploymentPeriod(endDate=date(2024, 1, 1), id=7, startDate=date(2024, 1, 1))
...
def test_assoc_address4_link_reassign_clear():
    a = employee_Employee(firstName="sample_text", gender="sample_text", lastName="sample_text", responsibilities="sample_text", salary=3.14, version="sample_text")
    b1 = employee_Address(city="sample_text", country="sample_text", id=7, postalCode="sample_text", province="sample_text", street="sample_text")
    b2 = employee_Address(city="sample_text_2", country="sample_text_2", id=13, postalCode="sample_text_2", province="sample_text_2", street="sample_text_2")
    ...
    _safe_set(a, 'employee_Employee5', None)
    assert not _is_linked(a, 'employee_Employee5', b2)
    if hasattr(b2, 'employee_Address'):
        assert not _is_linked(b2, 'employee_Address', a)


def test_assoc_degrees15_link_reassign_clear():
    a = employee_Employee(firstName="sample_text", gender="sample_text", lastName="sample_text", responsibilities="sample_text", salary=3.14, version="sample_text")
    b1 = employee_Degree(name="sample_text")
    b2 = employee_Degree(name="sample_text_2")
    _safe_set(a, 'employee_Employee16', {b1})
    assert _is_linked(a, 'employee_Employee16', b1)
    if hasattr(b1, 'employee_Degree'):
        assert _is_linked(b1, 'employee_Degree', a)
    _safe_set(a, 'employee_Employee16', {b2})
    assert _is_linked(a, 'employee_Employee16', b2)
    if hasattr(b1, 'employee_Degree'):
        assert not _is_linked(b1, 'employee_Degree', a)
    if hasattr(b2, 'employee_Degree'):
        assert _is_linked(b2, 'employee_Degree', a)
    _safe_set(a, 'employee_Employee16', set())
    assert not _is_linked(a, 'employee_Employee16', b2)
    if hasattr(b2, 'employee_Degree'):
        assert not _is_linked(b2, 'employee_Degree', a)


def test_assoc_emailAddresses20_link_reassign_clear():
    a = employee_Employee(firstName="sample_text", gender="sample_text", lastName="sample_text", responsibilities="sample_text", salary=3.14, version="sample_text")
    b1 = employee_EmailAddress(address="sample_text", id=7, name="sample_text")
    b2 = employee_EmailAddress(address="sample_text_2", id=13, name="sample_text_2")
    _safe_set(a, 'employee_Employee21', {b1})
    assert _is_linked(a, 'employee_Employee21', b1)
    if hasattr(b1, 'employee_EmailAddress'):
        assert _is_linked(b1, 'employee_EmailAddress', a)
    _safe_set(a, 'employee_Employee21', {b2})
    assert _is_linked(a, 'employee_Employee21', b2)
    if hasattr(b1, 'employee_EmailAddress'):
    ...
    _safe_set(a, 'employee_Employee21', set())
    assert not _is_linked(a, 'employee_Employee21', b2)
    if hasattr(b2, 'employee_EmailAddress'):
        assert not _is_linked(b2, 'employee_EmailAddress', a)


def test_assoc_jobTitle6_link_reassign_clear():
    a = employee_JobTitle(title="sample_text")
    b1 = employee_Employee(firstName="sample_text", gender="sample_text", lastName="sample_text", responsibilities="sample_text", salary=3.14, version="sample_text")
    b2 = employee_Employee(firstName="sample_text_2", gender="sample_text_2", lastName="sample_text_2", responsibilities="sample_text_2", salary=9.99, version="sample_text_2")
...
def test_assoc_managedEmployees12_link_reassign_clear():
    a = employee_Employee(firstName="sample_text", gender="sample_text", lastName="sample_text", responsibilities="sample_text", salary=3.14, version="sample_text")
    b1 = employee_Employee(firstName="sample_text", gender="sample_text", lastName="sample_text", responsibilities="sample_text", salary=3.14, version="sample_text")
    b2 = employee_Employee(firstName="sample_text_2", gender="sample_text_2", lastName="sample_text_2", responsibilities="sample_text_2", salary=9.99, version="sample_text_2")
    _safe_set(a, 'Employee13', b1)
    assert _is_linked(a, 'Employee13', b1)
    if hasattr(b1, 'manager'):
        assert _is_linked(b1, 'manager', a)
    _safe_set(a, 'Employee13', b2)
    assert _is_linked(a, 'Employee13', b2)
    if hasattr(b1, 'manager'):
        assert not _is_linked(b1, 'manager', a)
    if hasattr(b2, 'manager'):
        assert _is_linked(b2, 'manager', a)
    _safe_set(a, 'Employee13', None)
    assert not _is_linked(a, 'Employee13', b2)
    if hasattr(b2, 'manager'):
        assert not _is_linked(b2, 'manager', a)


def test_assoc_manager9_link_reassign_clear():
    a = employee_Employee(firstName="sample_text", gender="sample_text", lastName="sample_text", responsibilities="sample_text", salary=3.14, version="sample_text")
    b1 = employee_Employee(firstName="sample_text", gender="sample_text", lastName="sample_text", responsibilities="sample_text", salary=3.14, version="sample_text")
    b2 = employee_Employee(firstName="sample_text_2", gender="sample_text_2", lastName="sample_text_2", responsibilities="sample_text_2", salary=9.99, version="sample_text_2")
...
def test_assoc_period2_link_reassign_clear():
    a = employee_EmploymentPeriod(endDate=date(2024, 1, 1), id=7, startDate=date(2024, 1, 1))
    b1 = employee_Employee(firstName="sample_text", gender="sample_text", lastName="sample_text", responsibilities="sample_text", salary=3.14, version="sample_text")
    b2 = employee_Employee(firstName="sample_text_2", gender="sample_text_2", lastName="sample_text_2", responsibilities="sample_text_2", salary=9.99, version="sample_text_2")
    _safe_set(a, 'employee_EmploymentPeriod', b1)
    assert _is_linked(a, 'employee_EmploymentPeriod', b1)
    if hasattr(b1, 'employee_Employee3'):
        assert _is_linked(b1, 'employee_Employee3', a)
    _safe_set(a, 'employee_EmploymentPeriod', b2)
    assert _is_linked(a, 'employee_EmploymentPeriod', b2)
    if hasattr(b1, 'employee_Employee3'):
        assert not _is_linked(b1, 'employee_Employee3', a)
    if hasattr(b2, 'employee_Employee3'):
        assert _is_linked(b2, 'employee_Employee3', a)
    _safe_set(a, 'employee_EmploymentPeriod', None)
    assert not _is_linked(a, 'employee_EmploymentPeriod', b2)
    if hasattr(b2, 'employee_Employee3'):
        assert not _is_linked(b2, 'employee_Employee3', a)


def test_assoc_phoneNumbers14_link_reassign_clear():
    a = employee_PhoneNumber(areaCode="sample_text", number="sample_text", type="sample_text")
    b1 = employee_Employee(firstName="sample_text", gender="sample_text", lastName="sample_text", responsibilities="sample_text", salary=3.14, version="sample_text")
    b2 = employee_Employee(firstName="sample_text_2", gender="sample_text_2", lastName="sample_text_2", responsibilities="sample_text_2", salary=9.99, version="sample_text_2")
...
def test_assoc_projects17_link_reassign_clear():
    a = employee_Project(description="sample_text", name="sample_text")
    b1 = employee_Employee(firstName="sample_text", gender="sample_text", lastName="sample_text", responsibilities="sample_text", salary=3.14, version="sample_text")
    b2 = employee_Employee(firstName="sample_text_2", gender="sample_text_2", lastName="sample_text_2", responsibilities="sample_text_2", salary=9.99, version="sample_text_2")
    _safe_set(a, 'employee_Project19', b1)
    assert _is_linked(a, 'employee_Project19', b1)
    if hasattr(b1, 'employee_Employee18'):
        assert _is_linked(b1, 'employee_Employee18', a)
    _safe_set(a, 'employee_Project19', b2)
    assert _is_linked(a, 'employee_Project19', b2)
    if hasattr(b1, 'employee_Employee18'):
        assert not _is_linked(b1, 'employee_Employee18', a)
    if hasattr(b2, 'employee_Employee18'):
        assert _is_linked(b2, 'employee_Employee18', a)
    _safe_set(a, 'employee_Project19', None)
    assert not _is_linked(a, 'employee_Project19', b2)
    if hasattr(b2, 'employee_Employee18'):
        assert not _is_linked(b2, 'employee_Employee18', a)


def test_assoc_teamLeader0_link_reassign_clear():
    a = employee_Project(description="sample_text", name="sample_text")
    b1 = employee_Employee(firstName="sample_text", gender="sample_text", lastName="sample_text", responsibilities="sample_text", salary=3.14, version="sample_text")
    b2 = employee_Employee(firstName="sample_text_2", gender="sample_text_2", lastName="sample_text_2", responsibilities="sample_text_2", salary=9.99, version="sample_text_2")
...
employee_Address_strategy = st.builds(employee_Address, city=safe_text, country=safe_text, id=st.integers(), postalCode=safe_text, province=safe_text, street=safe_text)
@given(instance=employee_Address_strategy)
@settings(max_examples=25)
...
employee_EmailAddress_strategy = st.builds(employee_EmailAddress, address=safe_text, id=st.integers(), name=safe_text)
@given(instance=employee_EmailAddress_strategy)
@settings(max_examples=25)
...
employee_Employee_strategy = st.builds(employee_Employee, firstName=safe_text, gender=safe_text, lastName=safe_text, responsibilities=safe_text, salary=st.floats(allow_nan=False, allow_infinity=False), version=safe_text)
@given(instance=employee_Employee_strategy)
@settings(max_examples=25)
...
employee_EmploymentPeriod_strategy = st.builds(employee_EmploymentPeriod, endDate=st.dates(), id=st.integers(), startDate=st.dates())
@given(instance=employee_EmploymentPeriod_strategy)
@settings(max_examples=25)
