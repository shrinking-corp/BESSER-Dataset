...
def test_assoc_A_B22_link_reassign_clear():
    ...
    _safe_set(a, 'a13', b1)
    assert _is_linked(a, 'a13', b1)
    if hasattr(b1, 'b12'):
        assert _is_linked(b1, 'b12', a)
    _safe_set(a, 'a13', b2)
    assert _is_linked(a, 'a13', b2)
    if hasattr(b1, 'b12'):
        assert not _is_linked(b1, 'b12', a)
    if hasattr(b2, 'b12'):
        assert _is_linked(b2, 'b12', a)
    _safe_set(a, 'a13', None)
    assert not _is_linked(a, 'a13', b2)
    if hasattr(b2, 'b12'):
        assert not _is_linked(b2, 'b12', a)
...
def test_assoc_B_C22_link_reassign_clear():
    ...
    b2 = B2(attB=13)
    _safe_set(a, 'b15', b1)
    assert _is_linked(a, 'b15', b1)
    if hasattr(b1, 'c14'):
        assert _is_linked(b1, 'c14', a)
    _safe_set(a, 'b15', b2)
    assert _is_linked(a, 'b15', b2)
    if hasattr(b1, 'c14'):
        assert not _is_linked(b1, 'c14', a)
    if hasattr(b2, 'c14'):
        assert _is_linked(b2, 'c14', a)
    _safe_set(a, 'b15', None)
    assert not _is_linked(a, 'b15', b2)
    if hasattr(b2, 'c14'):
