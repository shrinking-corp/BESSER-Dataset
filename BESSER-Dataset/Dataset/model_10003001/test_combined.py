...
@given(instance=Utilisateur2_strategy)
def test_hyp_utilisateur2_nbAvis_setter(instance):
...
def test_Utilisateur2_adresse_value_roundtrip():
    instance = Utilisateur2(adresse="sample_text", age=7, nbAvis=7, nom="sample_text", photoDeProfil="sample_text")
    assert instance.adresse == "sample_text"
    instance.adresse = "sample_text_2"
    assert instance.adresse == "sample_text_2"
...
def test_Utilisateur2_nbAvis_value_roundtrip():
    instance = Utilisateur2(adresse="sample_text", age=7, nbAvis=7, nom="sample_text", photoDeProfil="sample_text")
    assert instance.nbAvis == 7
    instance.nbAvis = 13
    assert instance.nbAvis == 13
...
def test_assoc_Utilisateur_Avis_link_reassign_clear():
    a = Utilisateur2(adresse="sample_text", age=7, nbAvis=7, nom="sample_text", photoDeProfil="sample_text")
    b1 = Avis2(description="sample_text", note=7)
    b2 = Avis2(description="sample_text_2", note=13)
    _safe_set(a, 'avis12', {b1})
    assert _is_linked(a, 'avis12', b1)
    if hasattr(b1, 'utilisateur13'):
        assert _is_linked(b1, 'utilisateur13', a)
    _safe_set(a, 'avis12', {b2})
    assert _is_linked(a, 'avis12', b2)
    if hasattr(b1, 'utilisateur13'):
        assert not _is_linked(b1, 'utilisateur13', a)
    if hasattr(b2, 'utilisateur13'):
        assert _is_linked(b2, 'utilisateur13', a)
    _safe_set(a, 'avis12', set())
    assert not _is_linked(a, 'avis12', b2)
    if hasattr(b2, 'utilisateur13'):
        assert not _is_linked(b2, 'utilisateur13', a)
...
Utilisateur2_strategy = st.builds(Utilisateur2, adresse=safe_text, age=st.integers(), nbAvis=st.integers(), nom=safe_text, photoDeProfil=safe_text)
@given(instance=Utilisateur2_strategy)
@settings(max_examples=25)
