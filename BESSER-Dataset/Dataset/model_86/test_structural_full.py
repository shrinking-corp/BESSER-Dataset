...
def test_assoc_annotations272_link_reassign_clear():
    a = java_SingleVariableDeclaration(varargs=True)
    b1 = java_Annotation()
    b2 = java_Annotation()
    _safe_set(a, 'java_SingleVariableDeclaration273', {b1})
    assert _is_linked(a, 'java_SingleVariableDeclaration273', b1)
    if hasattr(b1, 'java_Annotation274'):
        assert _is_linked(b1, 'java_Annotation274', a)
    _safe_set(a, 'java_SingleVariableDeclaration273', {b2})
    assert _is_linked(a, 'java_SingleVariableDeclaration273', b2)
    if hasattr(b1, 'java_Annotation274'):
        assert not _is_linked(b1, 'java_Annotation274', a)
    if hasattr(b2, 'java_Annotation274'):
        assert _is_linked(b2, 'java_Annotation274', a)
    _safe_set(a, 'java_SingleVariableDeclaration273', set())
    assert not _is_linked(a, 'java_SingleVariableDeclaration273', b2)
    if hasattr(b2, 'java_Annotation274'):
        assert not _is_linked(b2, 'java_Annotation274', a)


def test_assoc_annotations336_link_reassign_clear():
    ...
    if hasattr(b1, 'java_Annotation337'):
        assert _is_linked(b1, 'java_Annotation337', a)
    _safe_set(a, 'java_VariableDeclarationStatement', {b2})
    assert _is_linked(a, 'java_VariableDeclarationStatement', b2)
    if hasattr(b1, 'java_Annotation337'):
        assert not _is_linked(b1, 'java_Annotation337', a)
    if hasattr(b2, 'java_Annotation337'):
        assert _is_linked(b2, 'java_Annotation337', a)
    _safe_set(a, 'java_VariableDeclarationStatement', set())
    assert not _is_linked(a, 'java_VariableDeclarationStatement', b2)
    if hasattr(b2, 'java_Annotation337'):
        assert not _is_linked(b2, 'java_Annotation337', a)


def test_assoc_archives221_link_reassign_clear():
    a = java_Model(name="sample_text")
    b1 = java_Archive(originalFilePath="sample_text")
    b2 = java_Archive(originalFilePath="sample_text_2")
    _safe_set(a, 'java_Model222', {b1})
    assert _is_linked(a, 'java_Model222', b1)
    if hasattr(b1, 'java_Archive223'):
        assert _is_linked(b1, 'java_Archive223', a)
    _safe_set(a, 'java_Model222', {b2})
    assert _is_linked(a, 'java_Model222', b2)
    if hasattr(b1, 'java_Archive223'):
        assert not _is_linked(b1, 'java_Archive223', a)
    if hasattr(b2, 'java_Archive223'):
        assert _is_linked(b2, 'java_Archive223', a)
    _safe_set(a, 'java_Model222', set())
    assert not _is_linked(a, 'java_Model222', b2)
    if hasattr(b2, 'java_Archive223'):
        assert not _is_linked(b2, 'java_Archive223', a)


def test_assoc_attributes188_link_reassign_clear():
    ...
    _safe_set(a, 'java_ManifestEntry189', {b1})
    assert _is_linked(a, 'java_ManifestEntry189', b1)
    if hasattr(b1, 'java_ManifestAttribute190'):
        assert _is_linked(b1, 'java_ManifestAttribute190', a)
    _safe_set(a, 'java_ManifestEntry189', {b2})
    assert _is_linked(a, 'java_ManifestEntry189', b2)
    if hasattr(b1, 'java_ManifestAttribute190'):
        assert not _is_linked(b1, 'java_ManifestAttribute190', a)
    if hasattr(b2, 'java_ManifestAttribute190'):
        assert _is_linked(b2, 'java_ManifestAttribute190', a)
    _safe_set(a, 'java_ManifestEntry189', set())
    assert not _is_linked(a, 'java_ManifestEntry189', b2)
    if hasattr(b2, 'java_ManifestAttribute190'):
        assert not _is_linked(b2, 'java_ManifestAttribute190', a)


def test_assoc_bodyDeclaration224_link_reassign_clear():
    ...
    if hasattr(b1, 'BodyDeclaration225'):
        assert _is_linked(b1, 'BodyDeclaration225', a)
    _safe_set(a, 'modifier', b2)
    assert _is_linked(a, 'modifier', b2)
    if hasattr(b1, 'BodyDeclaration225'):
        assert not _is_linked(b1, 'BodyDeclaration225', a)
    if hasattr(b2, 'BodyDeclaration225'):
        assert _is_linked(b2, 'BodyDeclaration225', a)
    _safe_set(a, 'modifier', None)
    assert not _is_linked(a, 'modifier', b2)
    if hasattr(b2, 'BodyDeclaration225'):
        assert not _is_linked(b2, 'BodyDeclaration225', a)
...
def test_assoc_bound338_link_reassign_clear():
    ...
    if hasattr(b1, 'java_TypeAccess339'):
        assert _is_linked(b1, 'java_TypeAccess339', a)
    _safe_set(a, 'java_WildCardType', b2)
    assert _is_linked(a, 'java_WildCardType', b2)
    if hasattr(b1, 'java_TypeAccess339'):
        assert not _is_linked(b1, 'java_TypeAccess339', a)
    if hasattr(b2, 'java_TypeAccess339'):
        assert _is_linked(b2, 'java_TypeAccess339', a)
    _safe_set(a, 'java_WildCardType', None)
    assert not _is_linked(a, 'java_WildCardType', b2)
    if hasattr(b2, 'java_TypeAccess339'):
        assert not _is_linked(b2, 'java_TypeAccess339', a)


def test_assoc_catchClause277_link_reassign_clear():
    a = java_SingleVariableDeclaration(varargs=True)
    b1 = java_CatchClause()
    b2 = java_CatchClause()
...
def test_assoc_commentList107_link_reassign_clear():
    ...
    if hasattr(b1, 'java_Comment109'):
        assert _is_linked(b1, 'java_Comment109', a)
    _safe_set(a, 'java_CompilationUnit108', {b2})
    assert _is_linked(a, 'java_CompilationUnit108', b2)
    if hasattr(b1, 'java_Comment109'):
        assert not _is_linked(b1, 'java_Comment109', a)
    if hasattr(b2, 'java_Comment109'):
        assert _is_linked(b2, 'java_Comment109', a)
    _safe_set(a, 'java_CompilationUnit108', set())
    assert not _is_linked(a, 'java_CompilationUnit108', b2)
    if hasattr(b2, 'java_Comment109'):
        assert not _is_linked(b2, 'java_Comment109', a)


def test_assoc_comments38_link_reassign_clear():
    a = java_Comment(content="sample_text", enclosedByParent=True, prefixOfParent=True)
    b1 = java_ASTNode()
    b2 = java_ASTNode()
    _safe_set(a, 'java_Comment39', b1)
    assert _is_linked(a, 'java_Comment39', b1)
    if hasattr(b1, 'java_ASTNode'):
        assert _is_linked(b1, 'java_ASTNode', a)
    _safe_set(a, 'java_Comment39', b2)
    assert _is_linked(a, 'java_Comment39', b2)
    if hasattr(b1, 'java_ASTNode'):
        assert not _is_linked(b1, 'java_ASTNode', a)
    if hasattr(b2, 'java_ASTNode'):
        assert _is_linked(b2, 'java_ASTNode', a)
    _safe_set(a, 'java_Comment39', None)
    assert not _is_linked(a, 'java_Comment39', b2)
    if hasattr(b2, 'java_ASTNode'):
...
def test_assoc_compilationUnits218_link_reassign_clear():
    a = java_Model(name="sample_text")
    b1 = java_CompilationUnit(originalFilePath="sample_text")
    b2 = java_CompilationUnit(originalFilePath="sample_text_2")
    _safe_set(a, 'java_Model219', {b1})
    assert _is_linked(a, 'java_Model219', b1)
    if hasattr(b1, 'java_CompilationUnit220'):
        assert _is_linked(b1, 'java_CompilationUnit220', a)
    _safe_set(a, 'java_Model219', {b2})
    assert _is_linked(a, 'java_Model219', b2)
    if hasattr(b1, 'java_CompilationUnit220'):
        assert not _is_linked(b1, 'java_CompilationUnit220', a)
    if hasattr(b2, 'java_CompilationUnit220'):
        assert _is_linked(b2, 'java_CompilationUnit220', a)
    _safe_set(a, 'java_Model219', set())
    assert not _is_linked(a, 'java_Model219', b2)
    if hasattr(b2, 'java_CompilationUnit220'):
        assert not _is_linked(b2, 'java_CompilationUnit220', a)
...
def test_assoc_elementType72_link_reassign_clear():
    ...
    if hasattr(b1, 'java_TypeAccess73'):
        assert _is_linked(b1, 'java_TypeAccess73', a)
    _safe_set(a, 'java_ArrayType', b2)
    assert _is_linked(a, 'java_ArrayType', b2)
    if hasattr(b1, 'java_TypeAccess73'):
        assert not _is_linked(b1, 'java_TypeAccess73', a)
    if hasattr(b2, 'java_TypeAccess73'):
        assert _is_linked(b2, 'java_TypeAccess73', a)
    _safe_set(a, 'java_ArrayType', None)
    assert not _is_linked(a, 'java_ArrayType', b2)
    if hasattr(b2, 'java_TypeAccess73'):
        assert not _is_linked(b2, 'java_TypeAccess73', a)


def test_assoc_enhancedForStatement278_link_reassign_clear():
    a = java_SingleVariableDeclaration(varargs=True)
...
def test_assoc_entryAttributes186_link_reassign_clear():
    a = java_ManifestEntry(name="sample_text")
    b1 = java_Manifest()
    b2 = java_Manifest()
    _safe_set(a, 'java_ManifestEntry', b1)
    assert _is_linked(a, 'java_ManifestEntry', b1)
    if hasattr(b1, 'java_Manifest187'):
        assert _is_linked(b1, 'java_Manifest187', a)
    _safe_set(a, 'java_ManifestEntry', b2)
    assert _is_linked(a, 'java_ManifestEntry', b2)
    if hasattr(b1, 'java_Manifest187'):
        assert not _is_linked(b1, 'java_Manifest187', a)
    if hasattr(b2, 'java_Manifest187'):
        assert _is_linked(b2, 'java_Manifest187', a)
    _safe_set(a, 'java_ManifestEntry', None)
    assert not _is_linked(a, 'java_ManifestEntry', b2)
    if hasattr(b2, 'java_Manifest187'):
        assert not _is_linked(b2, 'java_Manifest187', a)


def test_assoc_exception93_link_reassign_clear():
    a = java_SingleVariableDeclaration(varargs=True)
    b1 = java_CatchClause()
    b2 = java_CatchClause()
    _safe_set(a, 'SingleVariableDeclaration94', b1)
    assert _is_linked(a, 'SingleVariableDeclaration94', b1)
    if hasattr(b1, 'catchClause'):
        assert _is_linked(b1, 'catchClause', a)
    _safe_set(a, 'SingleVariableDeclaration94', b2)
    assert _is_linked(a, 'SingleVariableDeclaration94', b2)
    if hasattr(b1, 'catchClause'):
        assert not _is_linked(b1, 'catchClause', a)
    if hasattr(b2, 'catchClause'):
        assert _is_linked(b2, 'catchClause', a)
    _safe_set(a, 'SingleVariableDeclaration94', None)
    assert not _is_linked(a, 'SingleVariableDeclaration94', b2)
    if hasattr(b2, 'catchClause'):
        assert not _is_linked(b2, 'catchClause', a)
...
def test_assoc_extendedOperands168_link_reassign_clear():
    a = java_InfixExpression(operator="sample_text")
    b1 = java_Expression()
    b2 = java_Expression()
    _safe_set(a, 'java_InfixExpression169', {b1})
    assert _is_linked(a, 'java_InfixExpression169', b1)
    if hasattr(b1, 'java_Expression170'):
        assert _is_linked(b1, 'java_Expression170', a)
    _safe_set(a, 'java_InfixExpression169', {b2})
    assert _is_linked(a, 'java_InfixExpression169', b2)
    if hasattr(b1, 'java_Expression170'):
        assert not _is_linked(b1, 'java_Expression170', a)
    if hasattr(b2, 'java_Expression170'):
        assert _is_linked(b2, 'java_Expression170', a)
    _safe_set(a, 'java_InfixExpression169', set())
    assert not _is_linked(a, 'java_InfixExpression169', b2)
    if hasattr(b2, 'java_Expression170'):
        assert not _is_linked(b2, 'java_Expression170', a)


def test_assoc_fragments295_link_reassign_clear():
    a = java_TagElement(tagName="sample_text")
    b1 = java_ASTNode()
    b2 = java_ASTNode()
    _safe_set(a, 'java_TagElement296', {b1})
    assert _is_linked(a, 'java_TagElement296', b1)
    if hasattr(b1, 'java_ASTNode297'):
        assert _is_linked(b1, 'java_ASTNode297', a)
    _safe_set(a, 'java_TagElement296', {b2})
    assert _is_linked(a, 'java_TagElement296', b2)
    if hasattr(b1, 'java_ASTNode297'):
        assert not _is_linked(b1, 'java_ASTNode297', a)
    if hasattr(b2, 'java_ASTNode297'):
        assert _is_linked(b2, 'java_ASTNode297', a)
    _safe_set(a, 'java_TagElement296', set())
    assert not _is_linked(a, 'java_TagElement296', b2)
    if hasattr(b2, 'java_ASTNode297'):
        assert not _is_linked(b2, 'java_ASTNode297', a)


def test_assoc_importedElement162_link_reassign_clear():
...
def test_assoc_imports110_link_reassign_clear():
    ...
    if hasattr(b1, 'java_CompilationUnit111'):
        assert _is_linked(b1, 'java_CompilationUnit111', a)
    _safe_set(a, 'java_ImportDeclaration', b2)
    assert _is_linked(a, 'java_ImportDeclaration', b2)
    if hasattr(b1, 'java_CompilationUnit111'):
        assert not _is_linked(b1, 'java_CompilationUnit111', a)
    if hasattr(b2, 'java_CompilationUnit111'):
        assert _is_linked(b2, 'java_CompilationUnit111', a)
    _safe_set(a, 'java_ImportDeclaration', None)
    assert not _is_linked(a, 'java_ImportDeclaration', b2)
    if hasattr(b2, 'java_CompilationUnit111'):
        assert not _is_linked(b2, 'java_CompilationUnit111', a)


def test_assoc_initializer326_link_reassign_clear():
    ...
    if hasattr(b1, 'java_Expression327'):
        assert _is_linked(b1, 'java_Expression327', a)
    _safe_set(a, 'java_VariableDeclaration', b2)
    assert _is_linked(a, 'java_VariableDeclaration', b2)
    if hasattr(b1, 'java_Expression327'):
        assert not _is_linked(b1, 'java_Expression327', a)
    if hasattr(b2, 'java_Expression327'):
        assert _is_linked(b2, 'java_Expression327', a)
    _safe_set(a, 'java_VariableDeclaration', None)
    assert not _is_linked(a, 'java_VariableDeclaration', b2)
    if hasattr(b2, 'java_Expression327'):
        assert not _is_linked(b2, 'java_Expression327', a)


def test_assoc_leftHandSide74_link_reassign_clear():
...
def test_assoc_leftOperand165_link_reassign_clear():
    ...
    b1 = java_Expression()
    b2 = java_Expression()
    _safe_set(a, 'java_InfixExpression166', b1)
    assert _is_linked(a, 'java_InfixExpression166', b1)
    if hasattr(b1, 'java_Expression167'):
        assert _is_linked(b1, 'java_Expression167', a)
    _safe_set(a, 'java_InfixExpression166', b2)
    assert _is_linked(a, 'java_InfixExpression166', b2)
    if hasattr(b1, 'java_Expression167'):
        assert not _is_linked(b1, 'java_Expression167', a)
    if hasattr(b2, 'java_Expression167'):
        assert _is_linked(b2, 'java_Expression167', a)
    _safe_set(a, 'java_InfixExpression166', None)
    assert not _is_linked(a, 'java_InfixExpression166', b2)
    if hasattr(b2, 'java_Expression167'):
        assert not _is_linked(b2, 'java_Expression167', a)


def test_assoc_mainAttributes184_link_reassign_clear():
    ...
    if hasattr(b1, 'java_Manifest185'):
        assert _is_linked(b1, 'java_Manifest185', a)
    _safe_set(a, 'java_ManifestAttribute', b2)
    assert _is_linked(a, 'java_ManifestAttribute', b2)
    if hasattr(b1, 'java_Manifest185'):
        assert not _is_linked(b1, 'java_Manifest185', a)
    if hasattr(b2, 'java_Manifest185'):
        assert _is_linked(b2, 'java_Manifest185', a)
    _safe_set(a, 'java_ManifestAttribute', None)
    assert not _is_linked(a, 'java_ManifestAttribute', b2)
    if hasattr(b2, 'java_Manifest185'):
        assert not _is_linked(b2, 'java_Manifest185', a)


def test_assoc_manifest32_link_reassign_clear():
    a = java_Archive(originalFilePath="sample_text")
    b1 = java_Manifest()
    b2 = java_Manifest()
    _safe_set(a, 'java_Archive', b1)
    assert _is_linked(a, 'java_Archive', b1)
    if hasattr(b1, 'java_Manifest'):
        assert _is_linked(b1, 'java_Manifest', a)
    _safe_set(a, 'java_Archive', b2)
    assert _is_linked(a, 'java_Archive', b2)
    if hasattr(b1, 'java_Manifest'):
        assert not _is_linked(b1, 'java_Manifest', a)
    if hasattr(b2, 'java_Manifest'):
        assert _is_linked(b2, 'java_Manifest', a)
    _safe_set(a, 'java_Archive', None)
    assert not _is_linked(a, 'java_Archive', b2)
    if hasattr(b2, 'java_Manifest'):
        assert not _is_linked(b2, 'java_Manifest', a)


def test_assoc_member191_link_reassign_clear():
    a = java_NamedElement(name="sample_text", proxy=True)
    b1 = java_MemberRef()
    b2 = java_MemberRef()
...
def test_assoc_methodDeclaration275_link_reassign_clear():
    a = java_SingleVariableDeclaration(varargs=True)
    ...
    if hasattr(b1, 'AbstractMethodDeclaration276'):
        assert _is_linked(b1, 'AbstractMethodDeclaration276', a)
    _safe_set(a, 'parameters', b2)
    assert _is_linked(a, 'parameters', b2)
    if hasattr(b1, 'AbstractMethodDeclaration276'):
        assert not _is_linked(b1, 'AbstractMethodDeclaration276', a)
    if hasattr(b2, 'AbstractMethodDeclaration276'):
        assert _is_linked(b2, 'AbstractMethodDeclaration276', a)
    _safe_set(a, 'parameters', None)
    assert not _is_linked(a, 'parameters', b2)
    if hasattr(b2, 'AbstractMethodDeclaration276'):
        assert not _is_linked(b2, 'AbstractMethodDeclaration276', a)


def test_assoc_model236_link_reassign_clear():
    a = java_Model(name="sample_text")
    b1 = java_Package()
    b2 = java_Package()
    _safe_set(a, 'Model', b1)
    assert _is_linked(a, 'Model', b1)
    if hasattr(b1, 'ownedElements237'):
        assert _is_linked(b1, 'ownedElements237', a)
    _safe_set(a, 'Model', b2)
    assert _is_linked(a, 'Model', b2)
    if hasattr(b1, 'ownedElements237'):
        assert not _is_linked(b1, 'ownedElements237', a)
    if hasattr(b2, 'ownedElements237'):
        assert _is_linked(b2, 'ownedElements237', a)
    _safe_set(a, 'Model', None)
    assert not _is_linked(a, 'Model', b2)
    if hasattr(b2, 'ownedElements237'):
        assert not _is_linked(b2, 'ownedElements237', a)


def test_assoc_modifier268_link_reassign_clear():
    a = java_SingleVariableDeclaration(varargs=True)
    ...
    if hasattr(b1, 'Modifier269'):
        assert _is_linked(b1, 'Modifier269', a)
    _safe_set(a, 'singleVariableDeclaration', b2)
    assert _is_linked(a, 'singleVariableDeclaration', b2)
    if hasattr(b1, 'Modifier269'):
        assert not _is_linked(b1, 'Modifier269', a)
    if hasattr(b2, 'Modifier269'):
        assert _is_linked(b2, 'Modifier269', a)
    _safe_set(a, 'singleVariableDeclaration', None)
    assert not _is_linked(a, 'singleVariableDeclaration', b2)
    if hasattr(b2, 'Modifier269'):
        assert not _is_linked(b2, 'Modifier269', a)


def test_assoc_modifier329_link_reassign_clear():
    a = java_Modifier(inheritance="sample_text", native=True, static=True, strictfp=True, synchronized=True, transient=True, visibility="sample_text", volatile=True)
    b1 = java_VariableDeclarationExpression()
    b2 = java_VariableDeclarationExpression()
    _safe_set(a, 'Modifier330', b1)
    assert _is_linked(a, 'Modifier330', b1)
    if hasattr(b1, 'variableDeclarationExpression'):
        assert _is_linked(b1, 'variableDeclarationExpression', a)
    _safe_set(a, 'Modifier330', b2)
    assert _is_linked(a, 'Modifier330', b2)
    if hasattr(b1, 'variableDeclarationExpression'):
    ...
    _safe_set(a, 'Modifier330', None)
    assert not _is_linked(a, 'Modifier330', b2)
    if hasattr(b2, 'variableDeclarationExpression'):
        assert not _is_linked(b2, 'variableDeclarationExpression', a)


def test_assoc_modifier334_link_reassign_clear():
    a = java_VariableDeclarationStatement(extraArrayDimensions=7)
    b1 = java_Modifier(inheritance="sample_text", native=True, static=True, strictfp=True, synchronized=True, transient=True, visibility="sample_text", volatile=True)
    b2 = java_Modifier(inheritance="sample_text_2", native=False, static=False, strictfp=False, synchronized=False, transient=False, visibility="sample_text_2", volatile=False)
    _safe_set(a, 'variableDeclarationStatement', b1)
    assert _is_linked(a, 'variableDeclarationStatement', b1)
    if hasattr(b1, 'Modifier335'):
        assert _is_linked(b1, 'Modifier335', a)
    _safe_set(a, 'variableDeclarationStatement', b2)
    assert _is_linked(a, 'variableDeclarationStatement', b2)
    if hasattr(b1, 'Modifier335'):
        assert not _is_linked(b1, 'Modifier335', a)
    if hasattr(b2, 'Modifier335'):
        assert _is_linked(b2, 'Modifier335', a)
    _safe_set(a, 'variableDeclarationStatement', None)
    assert not _is_linked(a, 'variableDeclarationStatement', b2)
    if hasattr(b2, 'Modifier335'):
        assert not _is_linked(b2, 'Modifier335', a)


def test_assoc_modifier84_link_reassign_clear():
    a = java_Modifier(inheritance="sample_text", native=True, static=True, strictfp=True, synchronized=True, transient=True, visibility="sample_text", volatile=True)
    b1 = java_BodyDeclaration()
    b2 = java_BodyDeclaration()
    _safe_set(a, 'Modifier', b1)
...
def test_assoc_operand258_link_reassign_clear():
    ...
    if hasattr(b1, 'java_Expression259'):
        assert _is_linked(b1, 'java_Expression259', a)
    _safe_set(a, 'java_PostfixExpression', b2)
    assert _is_linked(a, 'java_PostfixExpression', b2)
    if hasattr(b1, 'java_Expression259'):
        assert not _is_linked(b1, 'java_Expression259', a)
    if hasattr(b2, 'java_Expression259'):
        assert _is_linked(b2, 'java_Expression259', a)
    _safe_set(a, 'java_PostfixExpression', None)
    assert not _is_linked(a, 'java_PostfixExpression', b2)
    if hasattr(b2, 'java_Expression259'):
        assert not _is_linked(b2, 'java_Expression259', a)


def test_assoc_operand260_link_reassign_clear():
    ...
    if hasattr(b1, 'java_Expression261'):
        assert _is_linked(b1, 'java_Expression261', a)
    _safe_set(a, 'java_PrefixExpression', b2)
    assert _is_linked(a, 'java_PrefixExpression', b2)
    if hasattr(b1, 'java_Expression261'):
        assert not _is_linked(b1, 'java_Expression261', a)
    if hasattr(b2, 'java_Expression261'):
        assert _is_linked(b2, 'java_Expression261', a)
    _safe_set(a, 'java_PrefixExpression', None)
    assert not _is_linked(a, 'java_PrefixExpression', b2)
    if hasattr(b2, 'java_Expression261'):
        assert not _is_linked(b2, 'java_Expression261', a)


def test_assoc_originalCompilationUnit40_link_reassign_clear():
    a = java_CompilationUnit(originalFilePath="sample_text")
    b1 = java_ASTNode()
    b2 = java_ASTNode()
    _safe_set(a, 'java_CompilationUnit', b1)
    assert _is_linked(a, 'java_CompilationUnit', b1)
    if hasattr(b1, 'java_ASTNode41'):
        assert _is_linked(b1, 'java_ASTNode41', a)
    _safe_set(a, 'java_CompilationUnit', b2)
    assert _is_linked(a, 'java_CompilationUnit', b2)
    if hasattr(b1, 'java_ASTNode41'):
        assert not _is_linked(b1, 'java_ASTNode41', a)
    if hasattr(b2, 'java_ASTNode41'):
        assert _is_linked(b2, 'java_ASTNode41', a)
    _safe_set(a, 'java_CompilationUnit', None)
    assert not _is_linked(a, 'java_CompilationUnit', b2)
    if hasattr(b2, 'java_ASTNode41'):
        assert not _is_linked(b2, 'java_ASTNode41', a)
...
def test_assoc_ownedElements213_link_reassign_clear():
    ...
    _safe_set(a, 'model', {b1})
    assert _is_linked(a, 'model', b1)
    if hasattr(b1, 'Package214'):
        assert _is_linked(b1, 'Package214', a)
    _safe_set(a, 'model', {b2})
    assert _is_linked(a, 'model', b2)
    if hasattr(b1, 'Package214'):
        assert not _is_linked(b1, 'Package214', a)
    if hasattr(b2, 'Package214'):
        assert _is_linked(b2, 'Package214', a)
    _safe_set(a, 'model', set())
    assert not _is_linked(a, 'model', b2)
    if hasattr(b2, 'Package214'):
        assert not _is_linked(b2, 'Package214', a)
...
def test_assoc_package112_link_reassign_clear():
    ...
    b1 = java_Package()
    b2 = java_Package()
    _safe_set(a, 'java_CompilationUnit113', b1)
    assert _is_linked(a, 'java_CompilationUnit113', b1)
    if hasattr(b1, 'java_Package'):
        assert _is_linked(b1, 'java_Package', a)
    _safe_set(a, 'java_CompilationUnit113', b2)
    assert _is_linked(a, 'java_CompilationUnit113', b2)
    if hasattr(b1, 'java_Package'):
        assert not _is_linked(b1, 'java_Package', a)
    if hasattr(b2, 'java_Package'):
        assert _is_linked(b2, 'java_Package', a)
    _safe_set(a, 'java_CompilationUnit113', None)
    assert not _is_linked(a, 'java_CompilationUnit113', b2)
    if hasattr(b2, 'java_Package'):
        assert not _is_linked(b2, 'java_Package', a)


def test_assoc_parameter129_link_reassign_clear():
    a = java_SingleVariableDeclaration(varargs=True)
    b1 = java_EnhancedForStatement()
    b2 = java_EnhancedForStatement()
    _safe_set(a, 'SingleVariableDeclaration130', b1)
    assert _is_linked(a, 'SingleVariableDeclaration130', b1)
    if hasattr(b1, 'enhancedForStatement'):
        assert _is_linked(b1, 'enhancedForStatement', a)
    _safe_set(a, 'SingleVariableDeclaration130', b2)
    assert _is_linked(a, 'SingleVariableDeclaration130', b2)
    if hasattr(b1, 'enhancedForStatement'):
        assert not _is_linked(b1, 'enhancedForStatement', a)
    if hasattr(b2, 'enhancedForStatement'):
        assert _is_linked(b2, 'enhancedForStatement', a)
    _safe_set(a, 'SingleVariableDeclaration130', None)
    assert not _is_linked(a, 'SingleVariableDeclaration130', b2)
    if hasattr(b2, 'enhancedForStatement'):
        assert not _is_linked(b2, 'enhancedForStatement', a)
...
def test_assoc_parameters208_link_reassign_clear():
    ...
    if hasattr(b1, 'java_MethodRef209'):
        assert _is_linked(b1, 'java_MethodRef209', a)
    _safe_set(a, 'java_MethodRefParameter', b2)
    assert _is_linked(a, 'java_MethodRefParameter', b2)
    if hasattr(b1, 'java_MethodRef209'):
        assert not _is_linked(b1, 'java_MethodRef209', a)
    if hasattr(b2, 'java_MethodRef209'):
        assert _is_linked(b2, 'java_MethodRef209', a)
    _safe_set(a, 'java_MethodRefParameter', None)
    assert not _is_linked(a, 'java_MethodRefParameter', b2)
    if hasattr(b2, 'java_MethodRef209'):
        assert not _is_linked(b2, 'java_MethodRef209', a)


def test_assoc_redefinedMethodDeclaration198_link_reassign_clear():
...
def test_assoc_redefinitions200_link_reassign_clear():
    a = java_MethodDeclaration(extraArrayDimensions=7)
    b1 = java_MethodDeclaration(extraArrayDimensions=7)
    b2 = java_MethodDeclaration(extraArrayDimensions=13)
    _safe_set(a, 'MethodDeclaration201', b1)
    assert _is_linked(a, 'MethodDeclaration201', b1)
    if hasattr(b1, 'redefinedMethodDeclaration'):
        assert _is_linked(b1, 'redefinedMethodDeclaration', a)
    _safe_set(a, 'MethodDeclaration201', b2)
    assert _is_linked(a, 'MethodDeclaration201', b2)
    if hasattr(b1, 'redefinedMethodDeclaration'):
    ...
    _safe_set(a, 'MethodDeclaration201', None)
    assert not _is_linked(a, 'MethodDeclaration201', b2)
    if hasattr(b2, 'redefinedMethodDeclaration'):
        assert not _is_linked(b2, 'redefinedMethodDeclaration', a)


def test_assoc_returnType195_link_reassign_clear():
    a = java_MethodDeclaration(extraArrayDimensions=7)
    b1 = java_TypeAccess()
    b2 = java_TypeAccess()
    _safe_set(a, 'java_MethodDeclaration', b1)
    assert _is_linked(a, 'java_MethodDeclaration', b1)
    if hasattr(b1, 'java_TypeAccess196'):
        assert _is_linked(b1, 'java_TypeAccess196', a)
    _safe_set(a, 'java_MethodDeclaration', b2)
    assert _is_linked(a, 'java_MethodDeclaration', b2)
    if hasattr(b1, 'java_TypeAccess196'):
        assert not _is_linked(b1, 'java_TypeAccess196', a)
    if hasattr(b2, 'java_TypeAccess196'):
        assert _is_linked(b2, 'java_TypeAccess196', a)
    _safe_set(a, 'java_MethodDeclaration', None)
    assert not _is_linked(a, 'java_MethodDeclaration', b2)
    if hasattr(b2, 'java_TypeAccess196'):
        assert not _is_linked(b2, 'java_TypeAccess196', a)


def test_assoc_rightHandSide76_link_reassign_clear():
    a = java_Assignment(operator="sample_text")
    b1 = java_Expression()
    b2 = java_Expression()
    _safe_set(a, 'java_Assignment77', b1)
    assert _is_linked(a, 'java_Assignment77', b1)
    if hasattr(b1, 'java_Expression78'):
        assert _is_linked(b1, 'java_Expression78', a)
    _safe_set(a, 'java_Assignment77', b2)
    assert _is_linked(a, 'java_Assignment77', b2)
    if hasattr(b1, 'java_Expression78'):
        assert not _is_linked(b1, 'java_Expression78', a)
    if hasattr(b2, 'java_Expression78'):
        assert _is_linked(b2, 'java_Expression78', a)
    _safe_set(a, 'java_Assignment77', None)
    assert not _is_linked(a, 'java_Assignment77', b2)
    if hasattr(b2, 'java_Expression78'):
        assert not _is_linked(b2, 'java_Expression78', a)


def test_assoc_rightOperand163_link_reassign_clear():
    a = java_InfixExpression(operator="sample_text")
    b1 = java_Expression()
    b2 = java_Expression()
    _safe_set(a, 'java_InfixExpression', b1)
    assert _is_linked(a, 'java_InfixExpression', b1)
    if hasattr(b1, 'java_Expression164'):
        assert _is_linked(b1, 'java_Expression164', a)
    _safe_set(a, 'java_InfixExpression', b2)
    assert _is_linked(a, 'java_InfixExpression', b2)
    if hasattr(b1, 'java_Expression164'):
        assert not _is_linked(b1, 'java_Expression164', a)
    if hasattr(b2, 'java_Expression164'):
        assert _is_linked(b2, 'java_Expression164', a)
    _safe_set(a, 'java_InfixExpression', None)
    assert not _is_linked(a, 'java_InfixExpression', b2)
    if hasattr(b2, 'java_Expression164'):
        assert not _is_linked(b2, 'java_Expression164', a)


def test_assoc_singleVariableDeclaration226_link_reassign_clear():
    a = java_SingleVariableDeclaration(varargs=True)
    b1 = java_Modifier(inheritance="sample_text", native=True, static=True, strictfp=True, synchronized=True, transient=True, visibility="sample_text", volatile=True)
    b2 = java_Modifier(inheritance="sample_text_2", native=False, static=False, strictfp=False, synchronized=False, transient=False, visibility="sample_text_2", volatile=False)
    _safe_set(a, 'SingleVariableDeclaration228', b1)
    assert _is_linked(a, 'SingleVariableDeclaration228', b1)
    if hasattr(b1, 'modifier227'):
        assert _is_linked(b1, 'modifier227', a)
    _safe_set(a, 'SingleVariableDeclaration228', b2)
    assert _is_linked(a, 'SingleVariableDeclaration228', b2)
    if hasattr(b1, 'modifier227'):
        assert not _is_linked(b1, 'modifier227', a)
    if hasattr(b2, 'modifier227'):
        assert _is_linked(b2, 'modifier227', a)
    _safe_set(a, 'SingleVariableDeclaration228', None)
    assert not _is_linked(a, 'SingleVariableDeclaration228', b2)
    if hasattr(b2, 'modifier227'):
        assert not _is_linked(b2, 'modifier227', a)


def test_assoc_tags178_link_reassign_clear():
    a = java_TagElement(tagName="sample_text")
    b1 = java_Javadoc()
    b2 = java_Javadoc()
...
def test_assoc_type210_link_reassign_clear():
    ...
    b1 = java_TypeAccess()
    b2 = java_TypeAccess()
    _safe_set(a, 'java_MethodRefParameter211', b1)
    assert _is_linked(a, 'java_MethodRefParameter211', b1)
    if hasattr(b1, 'java_TypeAccess212'):
        assert _is_linked(b1, 'java_TypeAccess212', a)
    _safe_set(a, 'java_MethodRefParameter211', b2)
    assert _is_linked(a, 'java_MethodRefParameter211', b2)
    if hasattr(b1, 'java_TypeAccess212'):
        assert not _is_linked(b1, 'java_TypeAccess212', a)
    if hasattr(b2, 'java_TypeAccess212'):
        assert _is_linked(b2, 'java_TypeAccess212', a)
    _safe_set(a, 'java_MethodRefParameter211', None)
    assert not _is_linked(a, 'java_MethodRefParameter211', b2)
    if hasattr(b2, 'java_TypeAccess212'):
        assert not _is_linked(b2, 'java_TypeAccess212', a)
...
def test_assoc_type270_link_reassign_clear():
    ...
    if hasattr(b1, 'java_TypeAccess271'):
        assert _is_linked(b1, 'java_TypeAccess271', a)
    _safe_set(a, 'java_SingleVariableDeclaration', b2)
    assert _is_linked(a, 'java_SingleVariableDeclaration', b2)
    if hasattr(b1, 'java_TypeAccess271'):
        assert not _is_linked(b1, 'java_TypeAccess271', a)
    if hasattr(b2, 'java_TypeAccess271'):
        assert _is_linked(b2, 'java_TypeAccess271', a)
    _safe_set(a, 'java_SingleVariableDeclaration', None)
    assert not _is_linked(a, 'java_SingleVariableDeclaration', b2)
    if hasattr(b2, 'java_TypeAccess271'):
        assert not _is_linked(b2, 'java_TypeAccess271', a)


def test_assoc_types114_link_reassign_clear():
    a = java_CompilationUnit(originalFilePath="sample_text")
    b1 = java_AbstractTypeDeclaration()
    b2 = java_AbstractTypeDeclaration()
    _safe_set(a, 'java_CompilationUnit115', {b1})
    assert _is_linked(a, 'java_CompilationUnit115', b1)
    if hasattr(b1, 'java_AbstractTypeDeclaration116'):
        assert _is_linked(b1, 'java_AbstractTypeDeclaration116', a)
    _safe_set(a, 'java_CompilationUnit115', {b2})
    assert _is_linked(a, 'java_CompilationUnit115', b2)
    if hasattr(b1, 'java_AbstractTypeDeclaration116'):
        assert not _is_linked(b1, 'java_AbstractTypeDeclaration116', a)
    if hasattr(b2, 'java_AbstractTypeDeclaration116'):
        assert _is_linked(b2, 'java_AbstractTypeDeclaration116', a)
    _safe_set(a, 'java_CompilationUnit115', set())
    assert not _is_linked(a, 'java_CompilationUnit115', b2)
    if hasattr(b2, 'java_AbstractTypeDeclaration116'):
        assert not _is_linked(b2, 'java_AbstractTypeDeclaration116', a)


def test_assoc_unresolvedItems216_link_reassign_clear():
    a = java_Model(name="sample_text")
    b1 = java_UnresolvedItem()
    b2 = java_UnresolvedItem()
    _safe_set(a, 'java_Model217', {b1})
    assert _is_linked(a, 'java_Model217', b1)
    if hasattr(b1, 'java_UnresolvedItem'):
        assert _is_linked(b1, 'java_UnresolvedItem', a)
    _safe_set(a, 'java_Model217', {b2})
    assert _is_linked(a, 'java_Model217', b2)
    if hasattr(b1, 'java_UnresolvedItem'):
    ...
    _safe_set(a, 'java_Model217', set())
    assert not _is_linked(a, 'java_Model217', b2)
    if hasattr(b2, 'java_UnresolvedItem'):
        assert not _is_linked(b2, 'java_UnresolvedItem', a)


def test_assoc_usageInVariableAccess328_link_reassign_clear():
    a = java_VariableDeclaration(extraArrayDimensions=7)
...
def test_assoc_usagesInImports233_link_reassign_clear():
    a = java_NamedElement(name="sample_text", proxy=True)
    b1 = java_ImportDeclaration(static=True)
    b2 = java_ImportDeclaration(static=False)
...
def test_assoc_variable264_link_reassign_clear():
    a = java_VariableDeclaration(extraArrayDimensions=7)
    b1 = java_SingleVariableAccess()
    b2 = java_SingleVariableAccess()
...
def test_assoc_variableDeclarationExpression231_link_reassign_clear():
    a = java_Modifier(inheritance="sample_text", native=True, static=True, strictfp=True, synchronized=True, transient=True, visibility="sample_text", volatile=True)
    b1 = java_VariableDeclarationExpression()
    b2 = java_VariableDeclarationExpression()
    _safe_set(a, 'modifier232', b1)
    assert _is_linked(a, 'modifier232', b1)
    if hasattr(b1, 'VariableDeclarationExpression'):
        assert _is_linked(b1, 'VariableDeclarationExpression', a)
    _safe_set(a, 'modifier232', b2)
    assert _is_linked(a, 'modifier232', b2)
    if hasattr(b1, 'VariableDeclarationExpression'):
        assert not _is_linked(b1, 'VariableDeclarationExpression', a)
    if hasattr(b2, 'VariableDeclarationExpression'):
        assert _is_linked(b2, 'VariableDeclarationExpression', a)
    _safe_set(a, 'modifier232', None)
    assert not _is_linked(a, 'modifier232', b2)
    if hasattr(b2, 'VariableDeclarationExpression'):
        assert not _is_linked(b2, 'VariableDeclarationExpression', a)


def test_assoc_variableDeclarationStatement229_link_reassign_clear():
    a = java_VariableDeclarationStatement(extraArrayDimensions=7)
    b1 = java_Modifier(inheritance="sample_text", native=True, static=True, strictfp=True, synchronized=True, transient=True, visibility="sample_text", volatile=True)
    b2 = java_Modifier(inheritance="sample_text_2", native=False, static=False, strictfp=False, synchronized=False, transient=False, visibility="sample_text_2", volatile=False)
    _safe_set(a, 'VariableDeclarationStatement', b1)
    assert _is_linked(a, 'VariableDeclarationStatement', b1)
    if hasattr(b1, 'modifier230'):
        assert _is_linked(b1, 'modifier230', a)
    _safe_set(a, 'VariableDeclarationStatement', b2)
    assert _is_linked(a, 'VariableDeclarationStatement', b2)
    if hasattr(b1, 'modifier230'):
        assert not _is_linked(b1, 'modifier230', a)
    if hasattr(b2, 'modifier230'):
        assert _is_linked(b2, 'modifier230', a)
    _safe_set(a, 'VariableDeclarationStatement', None)
    assert not _is_linked(a, 'VariableDeclarationStatement', b2)
    if hasattr(b2, 'modifier230'):
        assert not _is_linked(b2, 'modifier230', a)
