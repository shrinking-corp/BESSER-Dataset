...
def test_hyp_simplerdbms_primary_key_is_not_abstract():
    assert not inspect.isabstract(simplerdbms_Primary_Key)


def test_hyp_simplerdbms_primary_key_constructor_exists():
    assert callable(simplerdbms_Primary_Key.__init__)


def test_hyp_simplerdbms_primary_key_constructor_args():
    sig = inspect.signature(simplerdbms_Primary_Key.__init__)
    params = list(sig.parameters.keys())
...
simplerdbms_Primary_Key_strategy = st.builds(
    simplerdbms_Primary_Key,
)
...
def test_simplerdbms_Primary_Key_isa_Key():
    instance = simplerdbms_Primary_Key()
...
def test_assoc_columns19_link_reassign_clear():
    ...
    _safe_set(a, 'Column20', b1)
    assert _is_linked(a, 'Column20', b1)
    if hasattr(b1, 'owner'):
        assert _is_linked(b1, 'owner', a)
    _safe_set(a, 'Column20', b2)
    assert _is_linked(a, 'Column20', b2)
    if hasattr(b1, 'owner'):
        assert not _is_linked(b1, 'owner', a)
    if hasattr(b2, 'owner'):
        assert _is_linked(b2, 'owner', a)
    _safe_set(a, 'Column20', None)
    assert not _is_linked(a, 'Column20', b2)
    if hasattr(b2, 'owner'):
...
simplerdbms_Primary_Key_strategy = st.builds(simplerdbms_Primary_Key)
@given(instance=simplerdbms_Primary_Key_strategy)
@settings(max_examples=25)
def test_simplerdbms_Primary_Key_instantiation(instance):
    assert isinstance(instance, simplerdbms_Primary_Key)
