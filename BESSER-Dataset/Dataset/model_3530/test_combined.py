...
def test_hyp_a_a_m_setter(instance):
    original = instance.m
    instance.m = original
...
def test_a_A_m_value_roundtrip():
    instance = a_A(m=7)
    assert instance.m == 7
    instance.m = 13
    assert instance.m == 13


def test_assoc_ms0_link_reassign_clear():
    a = a_A(m=7)
    b1 = a_B()
    b2 = a_B()
    _safe_set(a, 'a_A', {b1})
...
a_A_strategy = st.builds(a_A, m=st.integers())
@given(instance=a_A_strategy)
