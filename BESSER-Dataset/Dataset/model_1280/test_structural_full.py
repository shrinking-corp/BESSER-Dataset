...
def test_ocl_dm_EAssociationEnd_mult_value_roundtrip():
    instance = ocl_dm_EAssociationEnd(mult="sample_text", name="sample_text")
...
def test_assoc_argument32_link_reassign_clear():
    a = ocl_exp_EOperationCallExp(referredOperation="sample_text")
    b1 = EOclExpression()
    b2 = EOclExpression()
    _safe_set(a, 'parentCall', {b1})
    assert _is_linked(a, 'parentCall', b1)
    if hasattr(b1, 'EOclExpression33'):
        assert _is_linked(b1, 'EOclExpression33', a)
    _safe_set(a, 'parentCall', {b2})
    assert _is_linked(a, 'parentCall', b2)
    if hasattr(b1, 'EOclExpression33'):
        assert not _is_linked(b1, 'EOclExpression33', a)
    if hasattr(b2, 'EOclExpression33'):
        assert _is_linked(b2, 'EOclExpression33', a)
    _safe_set(a, 'parentCall', set())
    assert not _is_linked(a, 'parentCall', b2)
    if hasattr(b2, 'EOclExpression33'):
        assert not _is_linked(b2, 'EOclExpression33', a)
...
def test_assoc_initExpression17_link_reassign_clear():
    a = ocl_exp_EVariable(name="sample_text")
    b1 = EOclExpression()
    b2 = EOclExpression()
    _safe_set(a, 'initializedElement', b1)
    assert _is_linked(a, 'initializedElement', b1)
    if hasattr(b1, 'EOclExpression18'):
        assert _is_linked(b1, 'EOclExpression18', a)
    _safe_set(a, 'initializedElement', b2)
    assert _is_linked(a, 'initializedElement', b2)
    if hasattr(b1, 'EOclExpression18'):
        assert not _is_linked(b1, 'EOclExpression18', a)
    if hasattr(b2, 'EOclExpression18'):
        assert _is_linked(b2, 'EOclExpression18', a)
    _safe_set(a, 'initializedElement', None)
    assert not _is_linked(a, 'initializedElement', b2)
    if hasattr(b2, 'EOclExpression18'):
        assert not _is_linked(b2, 'EOclExpression18', a)


def test_assoc_loopExp16_link_reassign_clear():
    a = ocl_exp_EVariable(name="sample_text")
    b1 = ELoopExp()
    b2 = ELoopExp()
...
def test_assoc_opp4_link_reassign_clear():
    a = ocl_dm_EAssociationEnd(mult="sample_text", name="sample_text")
    b1 = EAssociationEnd()
    b2 = EAssociationEnd()
    _safe_set(a, 'ocl_dm_EAssociationEnd5', b1)
    assert _is_linked(a, 'ocl_dm_EAssociationEnd5', b1)
    if hasattr(b1, 'EAssociationEnd6'):
        assert _is_linked(b1, 'EAssociationEnd6', a)
    _safe_set(a, 'ocl_dm_EAssociationEnd5', b2)
    assert _is_linked(a, 'ocl_dm_EAssociationEnd5', b2)
    if hasattr(b1, 'EAssociationEnd6'):
        assert not _is_linked(b1, 'EAssociationEnd6', a)
    if hasattr(b2, 'EAssociationEnd6'):
        assert _is_linked(b2, 'EAssociationEnd6', a)
    _safe_set(a, 'ocl_dm_EAssociationEnd5', None)
    assert not _is_linked(a, 'ocl_dm_EAssociationEnd5', b2)
    if hasattr(b2, 'EAssociationEnd6'):
        assert not _is_linked(b2, 'EAssociationEnd6', a)


def test_assoc_referredSignal34_link_reassign_clear():
...
ocl_dm_EAssociationEnd_strategy = st.builds(ocl_dm_EAssociationEnd, mult=safe_text, name=safe_text)
