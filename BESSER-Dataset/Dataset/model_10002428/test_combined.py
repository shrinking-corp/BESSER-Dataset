...
def test_hyp_______is_not_abstract():
    assert not inspect.isabstract(_____)


def test_hyp_______constructor_exists():
    assert callable(_____.__init__)


def test_hyp_______constructor_args():
    sig = inspect.signature(_____.__init__)
    params = list(sig.parameters.keys())
...
@given(instance=______strategy)
def test_hyp_______ownerName_setter(instance):
...
@given(instance=______strategy)
def test_hyp_______balance_setter(instance):
...
def test_______balance_value_roundtrip():
    instance = _____(balance=3.14, ownerName="sample_text")
    assert instance.balance == 3.14
    instance.balance = 9.99
    assert instance.balance == 9.99


def test_______ownerName_value_roundtrip():
    instance = _____(balance=3.14, ownerName="sample_text")
...
______strategy = st.builds(_____, balance=st.floats(allow_nan=False, allow_infinity=False), ownerName=safe_text)
@given(instance=______strategy)
@settings(max_examples=25)
def test_______instantiation(instance):
    assert isinstance(instance, _____)
