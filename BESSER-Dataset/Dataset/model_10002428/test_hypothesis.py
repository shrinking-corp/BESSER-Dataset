...
def test_______is_not_abstract():
    assert not inspect.isabstract(_____)


def test_______constructor_exists():
    assert callable(_____.__init__)


def test_______constructor_args():
    sig = inspect.signature(_____.__init__)
    params = list(sig.parameters.keys())
    assert "ownerName" in params, "Missing parameter 'ownerName'"
    assert "balance" in params, "Missing parameter 'balance'"

def test_______has_ownerName():
    assert hasattr(_____, "ownerName")
    descriptor = None
    for klass in _____.__mro__:
        if "ownerName" in klass.__dict__:
            descriptor = klass.__dict__["ownerName"]
            break
    assert isinstance(descriptor, property)

def test_______has_balance():
    assert hasattr(_____, "balance")
    descriptor = None
    for klass in _____.__mro__:
...
@given(instance=______strategy)
def test_______ownerName_setter(instance):
...
@given(instance=______strategy)
def test_______balance_setter(instance):
