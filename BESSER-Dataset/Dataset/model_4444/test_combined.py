...
def test_hyp_iot_fetchdatacondition_condition_setter(instance):
...
def test_ioT_FetchDataCondition_condition_value_roundtrip():
    instance = ioT_FetchDataCondition(condition="sample_text")
...
def test_assoc_duration34_link_reassign_clear():
    a = ioT_Time(time=7)
    ...
    if hasattr(b1, 'ioT_FetchDataExpression'):
        assert _is_linked(b1, 'ioT_FetchDataExpression', a)
    _safe_set(a, 'ioT_Time', b2)
    assert _is_linked(a, 'ioT_Time', b2)
    if hasattr(b1, 'ioT_FetchDataExpression'):
        assert not _is_linked(b1, 'ioT_FetchDataExpression', a)
    if hasattr(b2, 'ioT_FetchDataExpression'):
        assert _is_linked(b2, 'ioT_FetchDataExpression', a)
    _safe_set(a, 'ioT_Time', None)
    assert not _is_linked(a, 'ioT_Time', b2)
    if hasattr(b2, 'ioT_FetchDataExpression'):
        assert not _is_linked(b2, 'ioT_FetchDataExpression', a)
...
ioT_FetchDataCondition_strategy = st.builds(ioT_FetchDataCondition, condition=safe_text)
@given(instance=ioT_FetchDataCondition_strategy)
@settings(max_examples=25)
