...
def test_domainmodel_parameter_has_name():
    assert hasattr(domainmodel_Parameter, "name")
    descriptor = None
    for klass in domainmodel_Parameter.__mro__:
        if "name" in klass.__dict__:
            descriptor = klass.__dict__["name"]
            break
    assert isinstance(descriptor, property)
...
@given(instance=domainmodel_Parameter_strategy)
def test_domainmodel_parameter_name_setter(instance):
    original = instance.name
    instance.name = original
    assert instance.name == original
