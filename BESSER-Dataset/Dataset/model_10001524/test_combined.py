...
def test_assoc_Class2_Class3_link_reassign_clear():
...
def test_assoc_Class4_Class6_link_reassign_clear():
    a = C(attC1=7, attC2=True)
    b1 = B(attB=7)
    b2 = B(attB=13)
    _safe_set(a, 'class41', b1)
    assert _is_linked(a, 'class41', b1)
    if hasattr(b1, 'c0'):
        assert _is_linked(b1, 'c0', a)
    _safe_set(a, 'class41', b2)
    assert _is_linked(a, 'class41', b2)
    if hasattr(b1, 'c0'):
        assert not _is_linked(b1, 'c0', a)
    if hasattr(b2, 'c0'):
        assert _is_linked(b2, 'c0', a)
    _safe_set(a, 'class41', None)
    assert not _is_linked(a, 'class41', b2)
    if hasattr(b2, 'c0'):
