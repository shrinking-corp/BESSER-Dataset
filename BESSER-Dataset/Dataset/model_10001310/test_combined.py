...
def test_assoc_A_R_link_reassign_clear():
    ...
    b2 = R()
    _safe_set(a, 'r4', b1)
    assert _is_linked(a, 'r4', b1)
    if hasattr(b1, 'aR5'):
        assert _is_linked(b1, 'aR5', a)
    _safe_set(a, 'r4', b2)
    assert _is_linked(a, 'r4', b2)
    if hasattr(b1, 'aR5'):
        assert not _is_linked(b1, 'aR5', a)
    if hasattr(b2, 'aR5'):
        assert _is_linked(b2, 'aR5', a)
    _safe_set(a, 'r4', None)
    assert not _is_linked(a, 'r4', b2)
    if hasattr(b2, 'aR5'):
        assert not _is_linked(b2, 'aR5', a)
...
def test_assoc_B_C_link_reassign_clear():
    ...
    b1 = B(attB=7)
    b2 = B(attB=13)
    _safe_set(a, 'genou3', b1)
    assert _is_linked(a, 'genou3', b1)
    if hasattr(b1, 'c2'):
        assert _is_linked(b1, 'c2', a)
    _safe_set(a, 'genou3', b2)
    assert _is_linked(a, 'genou3', b2)
    if hasattr(b1, 'c2'):
        assert not _is_linked(b1, 'c2', a)
    if hasattr(b2, 'c2'):
        assert _is_linked(b2, 'c2', a)
    _safe_set(a, 'genou3', None)
    assert not _is_linked(a, 'genou3', b2)
    if hasattr(b2, 'c2'):
