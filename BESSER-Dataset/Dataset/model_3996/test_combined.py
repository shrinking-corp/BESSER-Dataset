...
def test_hyp_class_package_is_not_abstract():
    assert not inspect.isabstract(Class_Package)


def test_hyp_class_package_constructor_exists():
    assert callable(Class_Package.__init__)


def test_hyp_class_package_constructor_args():
...
def test_assoc_att4_link_reassign_clear():
...
def test_assoc_supers3_link_reassign_clear():
    a = Class_Class(isAbstract=True)
    ...
    if hasattr(b1, 'Class_Class2'):
        assert _is_linked(b1, 'Class_Class2', a)
    _safe_set(a, 'Class_Class', b2)
    assert _is_linked(a, 'Class_Class', b2)
    if hasattr(b1, 'Class_Class2'):
        assert not _is_linked(b1, 'Class_Class2', a)
    if hasattr(b2, 'Class_Class2'):
        assert _is_linked(b2, 'Class_Class2', a)
    _safe_set(a, 'Class_Class', None)
    assert not _is_linked(a, 'Class_Class', b2)
    if hasattr(b2, 'Class_Class2'):
        assert not _is_linked(b2, 'Class_Class2', a)
