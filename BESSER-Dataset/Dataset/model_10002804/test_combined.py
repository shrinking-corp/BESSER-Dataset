...
def test_hyp_teacher_interface_is_not_abstract():
    assert not inspect.isabstract(Teacher_Interface)


def test_hyp_teacher_interface_constructor_exists():
    assert callable(Teacher_Interface.__init__)


def test_hyp_teacher_interface_constructor_args():
    sig = inspect.signature(Teacher_Interface.__init__)
    params = list(sig.parameters.keys())
...
Teacher_Interface_strategy = st.builds(
    Teacher_Interface,
)
