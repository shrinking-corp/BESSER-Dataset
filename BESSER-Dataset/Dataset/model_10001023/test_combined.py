...
def test_assoc_FACULTY_ADMIN_link_reassign_clear():
    ...
    _safe_set(a, 'aDMIN24', b1)
    assert _is_linked(a, 'aDMIN24', b1)
    if hasattr(b1, 'fACULTY25'):
        assert _is_linked(b1, 'fACULTY25', a)
    _safe_set(a, 'aDMIN24', b2)
    assert _is_linked(a, 'aDMIN24', b2)
    if hasattr(b1, 'fACULTY25'):
        assert not _is_linked(b1, 'fACULTY25', a)
    if hasattr(b2, 'fACULTY25'):
        assert _is_linked(b2, 'fACULTY25', a)
    _safe_set(a, 'aDMIN24', None)
    assert not _is_linked(a, 'aDMIN24', b2)
    if hasattr(b2, 'fACULTY25'):
        assert not _is_linked(b2, 'fACULTY25', a)
...
def test_assoc_FACULTY_STUDENT_link_reassign_clear():
    ...
    _safe_set(a, 'fACULTY23', {b1})
    assert _is_linked(a, 'fACULTY23', b1)
    if hasattr(b1, 'sTUDENT22'):
        assert _is_linked(b1, 'sTUDENT22', a)
    _safe_set(a, 'fACULTY23', {b2})
    assert _is_linked(a, 'fACULTY23', b2)
    if hasattr(b1, 'sTUDENT22'):
        assert not _is_linked(b1, 'sTUDENT22', a)
    if hasattr(b2, 'sTUDENT22'):
        assert _is_linked(b2, 'sTUDENT22', a)
    _safe_set(a, 'fACULTY23', set())
    assert not _is_linked(a, 'fACULTY23', b2)
    if hasattr(b2, 'sTUDENT22'):
...
def test_assoc_PARENT_ADMIN_link_reassign_clear():
    ...
    b1 = ADMIN(id="sample_text", password="sample_text")
    b2 = ADMIN(id="sample_text_2", password="sample_text_2")
    _safe_set(a, 'aDMIN28', b1)
    assert _is_linked(a, 'aDMIN28', b1)
    if hasattr(b1, 'pARENT29'):
        assert _is_linked(b1, 'pARENT29', a)
    _safe_set(a, 'aDMIN28', b2)
    assert _is_linked(a, 'aDMIN28', b2)
    if hasattr(b1, 'pARENT29'):
        assert not _is_linked(b1, 'pARENT29', a)
    if hasattr(b2, 'pARENT29'):
        assert _is_linked(b2, 'pARENT29', a)
    _safe_set(a, 'aDMIN28', None)
    assert not _is_linked(a, 'aDMIN28', b2)
    if hasattr(b2, 'pARENT29'):
        assert not _is_linked(b2, 'pARENT29', a)
...
def test_assoc_STUDENT_ADMIN_link_reassign_clear():
    ...
    if hasattr(b1, 'sTUDENT27'):
        assert _is_linked(b1, 'sTUDENT27', a)
    _safe_set(a, 'aDMIN26', b2)
    assert _is_linked(a, 'aDMIN26', b2)
    if hasattr(b1, 'sTUDENT27'):
        assert not _is_linked(b1, 'sTUDENT27', a)
    if hasattr(b2, 'sTUDENT27'):
        assert _is_linked(b2, 'sTUDENT27', a)
    _safe_set(a, 'aDMIN26', None)
    assert not _is_linked(a, 'aDMIN26', b2)
    if hasattr(b2, 'sTUDENT27'):
        assert not _is_linked(b2, 'sTUDENT27', a)
