...
def test_assoc_articles3_link_reassign_clear():
    ...
    if hasattr(b1, 'DBLP.ecoreArticle'):
        assert _is_linked(b1, 'DBLP.ecoreArticle', a)
    _safe_set(a, 'journal', {b2})
    assert _is_linked(a, 'journal', b2)
    if hasattr(b1, 'DBLP.ecoreArticle'):
        assert not _is_linked(b1, 'DBLP.ecoreArticle', a)
    if hasattr(b2, 'DBLP.ecoreArticle'):
        assert _is_linked(b2, 'DBLP.ecoreArticle', a)
    _safe_set(a, 'journal', set())
    assert not _is_linked(a, 'journal', b2)
    if hasattr(b2, 'DBLP.ecoreArticle'):
        assert not _is_linked(b2, 'DBLP.ecoreArticle', a)
...
def test_assoc_authors0_link_reassign_clear():
    ...
    if hasattr(b1, 'DBLP.ecoreAuthor'):
        assert _is_linked(b1, 'DBLP.ecoreAuthor', a)
    _safe_set(a, 'records', {b2})
    assert _is_linked(a, 'records', b2)
    if hasattr(b1, 'DBLP.ecoreAuthor'):
        assert not _is_linked(b1, 'DBLP.ecoreAuthor', a)
    if hasattr(b2, 'DBLP.ecoreAuthor'):
        assert _is_linked(b2, 'DBLP.ecoreAuthor', a)
    _safe_set(a, 'records', set())
    assert not _is_linked(a, 'records', b2)
    if hasattr(b2, 'DBLP.ecoreAuthor'):
        assert not _is_linked(b2, 'DBLP.ecoreAuthor', a)
...
def test_assoc_journal1_link_reassign_clear():
    a = DBLP_Article(fromPage=7, month="sample_text", number=7, title="sample_text", toPage=7, volume="sample_text", year=7)
    b1 = DBLP_Journal(name="sample_text")
    b2 = DBLP_Journal(name="sample_text_2")
    _safe_set(a, 'articles', b1)
    assert _is_linked(a, 'articles', b1)
    if hasattr(b1, 'DBLP.ecoreJournal'):
        assert _is_linked(b1, 'DBLP.ecoreJournal', a)
    _safe_set(a, 'articles', b2)
    assert _is_linked(a, 'articles', b2)
    if hasattr(b1, 'DBLP.ecoreJournal'):
        assert not _is_linked(b1, 'DBLP.ecoreJournal', a)
    if hasattr(b2, 'DBLP.ecoreJournal'):
        assert _is_linked(b2, 'DBLP.ecoreJournal', a)
    _safe_set(a, 'articles', None)
    assert not _is_linked(a, 'articles', b2)
    if hasattr(b2, 'DBLP.ecoreJournal'):
        assert not _is_linked(b2, 'DBLP.ecoreJournal', a)
...
def test_assoc_records2_link_reassign_clear():
    a = DBLP_Author(name="sample_text")
    b1 = DBLP_Record(ee="sample_text", key="sample_text", mdate="sample_text", url="sample_text")
    b2 = DBLP_Record(ee="sample_text_2", key="sample_text_2", mdate="sample_text_2", url="sample_text_2")
    _safe_set(a, 'authors', {b1})
    assert _is_linked(a, 'authors', b1)
    if hasattr(b1, 'DBLP.ecoreRecord'):
        assert _is_linked(b1, 'DBLP.ecoreRecord', a)
    _safe_set(a, 'authors', {b2})
    assert _is_linked(a, 'authors', b2)
    if hasattr(b1, 'DBLP.ecoreRecord'):
        assert not _is_linked(b1, 'DBLP.ecoreRecord', a)
    if hasattr(b2, 'DBLP.ecoreRecord'):
        assert _is_linked(b2, 'DBLP.ecoreRecord', a)
    _safe_set(a, 'authors', set())
    assert not _is_linked(a, 'authors', b2)
    if hasattr(b2, 'DBLP.ecoreRecord'):
        assert not _is_linked(b2, 'DBLP.ecoreRecord', a)
