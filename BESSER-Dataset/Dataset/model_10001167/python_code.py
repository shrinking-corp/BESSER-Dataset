...
class Salary:
    ...
    def __init__(self, Emp_Id: int, Sly_Basic: float, Sly_Increment: float, Sly_Decrement: float, Sly_Netgross: float, OverTime: str):
        self.Emp_Id = Emp_Id
...
class Employee2:
    ...
    def __init__(self, Emp_Address: str, Emp_DOB: date, Emp_Department: str, Emp_Date_Of_Joint: date, Emp_Position: str, Emp_Salary: float, Emp_Id: int, Emp_Name: str, Emp_ContactNo: str, Emp_Email: str, Emp_NIC: str, leave4: "Leave" = None):
    ...
    @property
    def leave4(self):
        return self.__leave4
    @leave4.setter
    def leave4(self, value):
        # Bidirectional consistency
        old_value = getattr(self, f"_Employee2__leave4", None)
        self.__leave4 = value
...
class Leave:
    ...
    def __init__(self, leave_id: int, Emp_Id: int, Leave_Title: str, Leave_detail: str, Leave_ApplyDate: date, Leave_StartDate: date, Leave_EndDate: date, Leave_NoOfDays: int, Leave_Status: str, employee5: "Employee2" = None):
    ...
    def employee5(self, value):
        # Bidirectional consistency
        old_value = getattr(self, f"_Leave__employee5", None)
        self.__employee5 = value
        
        # Remove self from old opposite end
        if old_value is not None:
            if hasattr(old_value, "leave4"):
                opp_val = getattr(old_value, "leave4", None)
                if opp_val == self:
                    setattr(old_value, "leave4", None)
                
        # Add self to new opposite end
        if value is not None:
            if hasattr(value, "leave4"):
                opp_val = getattr(value, "leave4", None)
                setattr(value, "leave4", self)
...
class Attendance:
    ...
    def __init__(self, Attend_date: date, Emp_id: str, AttendTime: str, Leaving_Time: str):
