...
def test_profile_Resource_name_value_roundtrip():
    instance = profile_Resource(name="sample_text", type="sample_text")
    assert instance.name == "sample_text"
    instance.name = "sample_text_2"
    assert instance.name == "sample_text_2"
...
def test_assoc_constraints1_link_reassign_clear():
    ...
    _safe_set(a, 'platform2', {b1})
    assert _is_linked(a, 'platform2', b1)
    if hasattr(b1, 'Constraint'):
        assert _is_linked(b1, 'Constraint', a)
    _safe_set(a, 'platform2', {b2})
    assert _is_linked(a, 'platform2', b2)
    if hasattr(b1, 'Constraint'):
        assert not _is_linked(b1, 'Constraint', a)
    if hasattr(b2, 'Constraint'):
        assert _is_linked(b2, 'Constraint', a)
    _safe_set(a, 'platform2', set())
    assert not _is_linked(a, 'platform2', b2)
    if hasattr(b2, 'Constraint'):
        assert not _is_linked(b2, 'Constraint', a)


def test_assoc_platform3_link_reassign_clear():
    a = profile_Resource(name="sample_text", type="sample_text")
    ...
    if hasattr(b1, 'PlatformProfile'):
        assert _is_linked(b1, 'PlatformProfile', a)
    _safe_set(a, 'resources', b2)
    assert _is_linked(a, 'resources', b2)
    if hasattr(b1, 'PlatformProfile'):
        assert not _is_linked(b1, 'PlatformProfile', a)
    if hasattr(b2, 'PlatformProfile'):
        assert _is_linked(b2, 'PlatformProfile', a)
    _safe_set(a, 'resources', None)
    assert not _is_linked(a, 'resources', b2)
    if hasattr(b2, 'PlatformProfile'):
        assert not _is_linked(b2, 'PlatformProfile', a)


def test_assoc_platform4_link_reassign_clear():
    a = profile_PlatformProfile(name="sample_text")
    b1 = profile_Constraint(bound=7, isDerivation=True, operation="sample_text", type="sample_text")
    b2 = profile_Constraint(bound=13, isDerivation=False, operation="sample_text_2", type="sample_text_2")
    _safe_set(a, 'PlatformProfile5', b1)
    assert _is_linked(a, 'PlatformProfile5', b1)
    if hasattr(b1, 'constraints'):
        assert _is_linked(b1, 'constraints', a)
    _safe_set(a, 'PlatformProfile5', b2)
    assert _is_linked(a, 'PlatformProfile5', b2)
    if hasattr(b1, 'constraints'):
        assert not _is_linked(b1, 'constraints', a)
    if hasattr(b2, 'constraints'):
        assert _is_linked(b2, 'constraints', a)
    _safe_set(a, 'PlatformProfile5', None)
    assert not _is_linked(a, 'PlatformProfile5', b2)
    if hasattr(b2, 'constraints'):
...
def test_assoc_resources0_link_reassign_clear():
    ...
    if hasattr(b1, 'platform'):
        assert _is_linked(b1, 'platform', a)
    _safe_set(a, 'Resource', b2)
    assert _is_linked(a, 'Resource', b2)
    if hasattr(b1, 'platform'):
        assert not _is_linked(b1, 'platform', a)
    if hasattr(b2, 'platform'):
        assert _is_linked(b2, 'platform', a)
    _safe_set(a, 'Resource', None)
    assert not _is_linked(a, 'Resource', b2)
    if hasattr(b2, 'platform'):
        assert not _is_linked(b2, 'platform', a)
...
profile_Resource_strategy = st.builds(profile_Resource, name=safe_text, type=safe_text)
