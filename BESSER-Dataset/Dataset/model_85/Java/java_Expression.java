





import java.util.List;
import java.util.ArrayList;

public class java_Expression extends ASTNode {






    private java_SwitchCase java_switchcase;




    private java_SingleVariableAccess java_singlevariableaccess;




    private java_ParenthesizedExpression java_parenthesizedexpression;




    private java_InstanceofExpression java_instanceofexpression;




    private java_ExpressionStatement java_expressionstatement;




    private java_SynchronizedStatement java_synchronizedstatement;




    private java_VariableDeclaration java_variabledeclaration;




    private java_AbstractMethodInvocation java_abstractmethodinvocation;




    private java_IfStatement java_ifstatement;




    private java_ForStatement java_forstatement;




    private java_InfixExpression java_infixexpression;




    private java_ForStatement java_forstatement;




    private java_EnhancedForStatement java_enhancedforstatement;




    private java_ThrowStatement java_throwstatement;




    private java_InfixExpression java_infixexpression;




    private java_MethodInvocation java_methodinvocation;




    private java_PostfixExpression java_postfixexpression;




    private java_ReturnStatement java_returnstatement;




    private java_ForStatement java_forstatement;




    private java_PrefixExpression java_prefixexpression;




    private java_SuperConstructorInvocation java_superconstructorinvocation;




    private java_FieldAccess java_fieldaccess;




    private java_SwitchStatement java_switchstatement;




    private java_WhileStatement java_whilestatement;




    private java_InfixExpression java_infixexpression;




    private java_EnumConstantDeclaration java_enumconstantdeclaration;


    public java_Expression(
    ) {
        super(
        );
    }



    public java_SwitchCase getJava_switchcase() {
        return java_switchcase;
    }

    public void setJava_switchcase(java_SwitchCase java_switchcase) {
        this.java_switchcase = java_switchcase;
    }
    public java_SingleVariableAccess getJava_singlevariableaccess() {
        return java_singlevariableaccess;
    }

    public void setJava_singlevariableaccess(java_SingleVariableAccess java_singlevariableaccess) {
        this.java_singlevariableaccess = java_singlevariableaccess;
    }
    public java_ParenthesizedExpression getJava_parenthesizedexpression() {
        return java_parenthesizedexpression;
    }

    public void setJava_parenthesizedexpression(java_ParenthesizedExpression java_parenthesizedexpression) {
        this.java_parenthesizedexpression = java_parenthesizedexpression;
    }
    public java_InstanceofExpression getJava_instanceofexpression() {
        return java_instanceofexpression;
    }

    public void setJava_instanceofexpression(java_InstanceofExpression java_instanceofexpression) {
        this.java_instanceofexpression = java_instanceofexpression;
    }
    public java_ExpressionStatement getJava_expressionstatement() {
        return java_expressionstatement;
    }

    public void setJava_expressionstatement(java_ExpressionStatement java_expressionstatement) {
        this.java_expressionstatement = java_expressionstatement;
    }
    public java_SynchronizedStatement getJava_synchronizedstatement() {
        return java_synchronizedstatement;
    }

    public void setJava_synchronizedstatement(java_SynchronizedStatement java_synchronizedstatement) {
        this.java_synchronizedstatement = java_synchronizedstatement;
    }
    public java_VariableDeclaration getJava_variabledeclaration() {
        return java_variabledeclaration;
    }

    public void setJava_variabledeclaration(java_VariableDeclaration java_variabledeclaration) {
        this.java_variabledeclaration = java_variabledeclaration;
    }
    public java_AbstractMethodInvocation getJava_abstractmethodinvocation() {
        return java_abstractmethodinvocation;
    }

    public void setJava_abstractmethodinvocation(java_AbstractMethodInvocation java_abstractmethodinvocation) {
        this.java_abstractmethodinvocation = java_abstractmethodinvocation;
    }
    public java_IfStatement getJava_ifstatement() {
        return java_ifstatement;
    }

    public void setJava_ifstatement(java_IfStatement java_ifstatement) {
        this.java_ifstatement = java_ifstatement;
    }
    public java_ForStatement getJava_forstatement() {
        return java_forstatement;
    }

    public void setJava_forstatement(java_ForStatement java_forstatement) {
        this.java_forstatement = java_forstatement;
    }
    public java_InfixExpression getJava_infixexpression() {
        return java_infixexpression;
    }

    public void setJava_infixexpression(java_InfixExpression java_infixexpression) {
        this.java_infixexpression = java_infixexpression;
    }
    public java_ForStatement getJava_forstatement() {
        return java_forstatement;
    }

    public void setJava_forstatement(java_ForStatement java_forstatement) {
        this.java_forstatement = java_forstatement;
    }
    public java_EnhancedForStatement getJava_enhancedforstatement() {
        return java_enhancedforstatement;
    }

    public void setJava_enhancedforstatement(java_EnhancedForStatement java_enhancedforstatement) {
        this.java_enhancedforstatement = java_enhancedforstatement;
    }
    public java_ThrowStatement getJava_throwstatement() {
        return java_throwstatement;
    }

    public void setJava_throwstatement(java_ThrowStatement java_throwstatement) {
        this.java_throwstatement = java_throwstatement;
    }
    public java_InfixExpression getJava_infixexpression() {
        return java_infixexpression;
    }

    public void setJava_infixexpression(java_InfixExpression java_infixexpression) {
        this.java_infixexpression = java_infixexpression;
    }
    public java_MethodInvocation getJava_methodinvocation() {
        return java_methodinvocation;
    }

    public void setJava_methodinvocation(java_MethodInvocation java_methodinvocation) {
        this.java_methodinvocation = java_methodinvocation;
    }
    public java_PostfixExpression getJava_postfixexpression() {
        return java_postfixexpression;
    }

    public void setJava_postfixexpression(java_PostfixExpression java_postfixexpression) {
        this.java_postfixexpression = java_postfixexpression;
    }
    public java_ReturnStatement getJava_returnstatement() {
        return java_returnstatement;
    }

    public void setJava_returnstatement(java_ReturnStatement java_returnstatement) {
        this.java_returnstatement = java_returnstatement;
    }
    public java_ForStatement getJava_forstatement() {
        return java_forstatement;
    }

    public void setJava_forstatement(java_ForStatement java_forstatement) {
        this.java_forstatement = java_forstatement;
    }
    public java_PrefixExpression getJava_prefixexpression() {
        return java_prefixexpression;
    }

    public void setJava_prefixexpression(java_PrefixExpression java_prefixexpression) {
        this.java_prefixexpression = java_prefixexpression;
    }
    public java_SuperConstructorInvocation getJava_superconstructorinvocation() {
        return java_superconstructorinvocation;
    }

    public void setJava_superconstructorinvocation(java_SuperConstructorInvocation java_superconstructorinvocation) {
        this.java_superconstructorinvocation = java_superconstructorinvocation;
    }
    public java_FieldAccess getJava_fieldaccess() {
        return java_fieldaccess;
    }

    public void setJava_fieldaccess(java_FieldAccess java_fieldaccess) {
        this.java_fieldaccess = java_fieldaccess;
    }
    public java_SwitchStatement getJava_switchstatement() {
        return java_switchstatement;
    }

    public void setJava_switchstatement(java_SwitchStatement java_switchstatement) {
        this.java_switchstatement = java_switchstatement;
    }
    public java_WhileStatement getJava_whilestatement() {
        return java_whilestatement;
    }

    public void setJava_whilestatement(java_WhileStatement java_whilestatement) {
        this.java_whilestatement = java_whilestatement;
    }
    public java_InfixExpression getJava_infixexpression() {
        return java_infixexpression;
    }

    public void setJava_infixexpression(java_InfixExpression java_infixexpression) {
        this.java_infixexpression = java_infixexpression;
    }
    public java_EnumConstantDeclaration getJava_enumconstantdeclaration() {
        return java_enumconstantdeclaration;
    }

    public void setJava_enumconstantdeclaration(java_EnumConstantDeclaration java_enumconstantdeclaration) {
        this.java_enumconstantdeclaration = java_enumconstantdeclaration;
    }

}