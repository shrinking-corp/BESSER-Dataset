...
def test_myclass20_is_not_abstract():
    assert not inspect.isabstract(MyClass20)


def test_myclass20_constructor_exists():
    assert callable(MyClass20.__init__)


def test_myclass20_constructor_args():
    sig = inspect.signature(MyClass20.__init__)
...
@given(instance=MyClass20_strategy)
@settings(max_examples=50)
def test_myclass20_instantiation(instance):
    assert isinstance(instance, MyClass20)
