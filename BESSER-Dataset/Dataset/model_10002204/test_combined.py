...
def test_hyp_marriage_is_not_abstract():
    assert not inspect.isabstract(Marriage)


def test_hyp_marriage_constructor_exists():
    assert callable(Marriage.__init__)


def test_hyp_marriage_constructor_args():
    sig = inspect.signature(Marriage.__init__)
    params = list(sig.parameters.keys())
...
def test_assoc_B_A_link_reassign_clear():
    ...
    b2 = B1()
    _safe_set(a, 'c13', b1)
    assert _is_linked(a, 'c13', b1)
    if hasattr(b1, 'B_A_012'):
        assert _is_linked(b1, 'B_A_012', a)
    _safe_set(a, 'c13', b2)
    assert _is_linked(a, 'c13', b2)
    if hasattr(b1, 'B_A_012'):
        assert not _is_linked(b1, 'B_A_012', a)
    if hasattr(b2, 'B_A_012'):
        assert _is_linked(b2, 'B_A_012', a)
    _safe_set(a, 'c13', None)
    assert not _is_linked(a, 'c13', b2)
    if hasattr(b2, 'B_A_012'):
        assert not _is_linked(b2, 'B_A_012', a)
...
def test_assoc_Personne_Union_link_reassign_clear():
    ...
    if hasattr(b1, 'unionActuelle8'):
        assert _is_linked(b1, 'unionActuelle8', a)
    _safe_set(a, 'personnes9', {b2})
    assert _is_linked(a, 'personnes9', b2)
    if hasattr(b1, 'unionActuelle8'):
        assert not _is_linked(b1, 'unionActuelle8', a)
    if hasattr(b2, 'unionActuelle8'):
        assert _is_linked(b2, 'unionActuelle8', a)
    _safe_set(a, 'personnes9', set())
    assert not _is_linked(a, 'personnes9', b2)
    if hasattr(b2, 'unionActuelle8'):
        assert not _is_linked(b2, 'unionActuelle8', a)
...
Marriage_strategy = st.builds(Marriage)
@given(instance=Marriage_strategy)
@settings(max_examples=25)
def test_Marriage_instantiation(instance):
    assert isinstance(instance, Marriage)
