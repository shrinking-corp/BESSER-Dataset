...
def test_hyp_a_constructor_args():
    ...
    params = list(sig.parameters.keys())
    assert "attaA" in params, "Missing parameter 'attaA'"
...
def test_hyp_a_attaA_setter(instance):
    original = instance.attaA
    instance.attaA = original
    assert instance.attaA == original
...
def test_A_attaA_value_roundtrip():
    instance = A(attaA="sample_text")
    assert instance.attaA == "sample_text"
    instance.attaA = "sample_text_2"
    assert instance.attaA == "sample_text_2"
...
def test_assoc_B_A_link_reassign_clear():
    a = B(attB=7)
    b1 = A(attaA="sample_text")
    b2 = A(attaA="sample_text_2")
    _safe_set(a, 'a0', b1)
...
A_strategy = st.builds(A, attaA=safe_text)
@given(instance=A_strategy)
