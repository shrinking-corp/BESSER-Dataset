...
def test_assoc_Bill_Discount_link_reassign_clear():
    ...
    b1 = Bill()
    b2 = Bill()
    _safe_set(a, 'bill9', b1)
    assert _is_linked(a, 'bill9', b1)
    if hasattr(b1, 'discount8'):
        assert _is_linked(b1, 'discount8', a)
    _safe_set(a, 'bill9', b2)
    assert _is_linked(a, 'bill9', b2)
    if hasattr(b1, 'discount8'):
        assert not _is_linked(b1, 'discount8', a)
    if hasattr(b2, 'discount8'):
        assert _is_linked(b2, 'discount8', a)
    _safe_set(a, 'bill9', None)
    assert not _is_linked(a, 'bill9', b2)
    if hasattr(b2, 'discount8'):
        assert not _is_linked(b2, 'discount8', a)
...
def test_assoc_Order_Drinks_link_reassign_clear():
    ...
    b1 = Order()
    b2 = Order()
    _safe_set(a, 'Order_Drinks_17', b1)
    assert _is_linked(a, 'Order_Drinks_17', b1)
    if hasattr(b1, 'Order_Drinks_06'):
        assert _is_linked(b1, 'Order_Drinks_06', a)
    _safe_set(a, 'Order_Drinks_17', b2)
    assert _is_linked(a, 'Order_Drinks_17', b2)
    if hasattr(b1, 'Order_Drinks_06'):
        assert not _is_linked(b1, 'Order_Drinks_06', a)
    if hasattr(b2, 'Order_Drinks_06'):
        assert _is_linked(b2, 'Order_Drinks_06', a)
    _safe_set(a, 'Order_Drinks_17', None)
    assert not _is_linked(a, 'Order_Drinks_17', b2)
    if hasattr(b2, 'Order_Drinks_06'):
        assert not _is_linked(b2, 'Order_Drinks_06', a)
...
def test_assoc_Order_Menu_link_reassign_clear():
    ...
    b1 = Order()
    b2 = Order()
    _safe_set(a, 'Order_Menu_15', b1)
    assert _is_linked(a, 'Order_Menu_15', b1)
    if hasattr(b1, 'Order_Menu_04'):
        assert _is_linked(b1, 'Order_Menu_04', a)
    _safe_set(a, 'Order_Menu_15', b2)
    assert _is_linked(a, 'Order_Menu_15', b2)
    if hasattr(b1, 'Order_Menu_04'):
        assert not _is_linked(b1, 'Order_Menu_04', a)
    if hasattr(b2, 'Order_Menu_04'):
        assert _is_linked(b2, 'Order_Menu_04', a)
    _safe_set(a, 'Order_Menu_15', None)
    assert not _is_linked(a, 'Order_Menu_15', b2)
    if hasattr(b2, 'Order_Menu_04'):
        assert not _is_linked(b2, 'Order_Menu_04', a)
