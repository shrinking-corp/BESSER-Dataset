...
def test_assoc_package29_link_reassign_clear():
    ...
    if hasattr(b1, 'Package30'):
        assert _is_linked(b1, 'Package30', a)
    _safe_set(a, 'EMOF_Type', b2)
    assert _is_linked(a, 'EMOF_Type', b2)
    if hasattr(b1, 'Package30'):
        assert not _is_linked(b1, 'Package30', a)
    if hasattr(b2, 'Package30'):
        assert _is_linked(b2, 'Package30', a)
    _safe_set(a, 'EMOF_Type', None)
    assert not _is_linked(a, 'EMOF_Type', b2)
    if hasattr(b2, 'Package30'):
        assert not _is_linked(b2, 'Package30', a)
