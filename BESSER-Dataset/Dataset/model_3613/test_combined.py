...
def test_pp_Definition_isa_Expression():
    instance = pp_Definition(className="sample_text")
    assert isinstance(instance, Expression)
...
def test_pp_IfExpression_isa_Expression():
...
def test_pp_NodeDefinition_isa_Expression():
...
def test_pp_UnlessExpression_isa_Expression():
...
def test_assoc_arguments13_link_reassign_clear():
    ...
    if hasattr(b1, 'pp_DefinitionArgumentList14'):
        assert _is_linked(b1, 'pp_DefinitionArgumentList14', a)
    _safe_set(a, 'pp_DefinitionArgument', b2)
    assert _is_linked(a, 'pp_DefinitionArgument', b2)
    if hasattr(b1, 'pp_DefinitionArgumentList14'):
        assert not _is_linked(b1, 'pp_DefinitionArgumentList14', a)
    if hasattr(b2, 'pp_DefinitionArgumentList14'):
        assert _is_linked(b2, 'pp_DefinitionArgumentList14', a)
    _safe_set(a, 'pp_DefinitionArgument', None)
    assert not _is_linked(a, 'pp_DefinitionArgument', b2)
    if hasattr(b2, 'pp_DefinitionArgumentList14'):
        assert not _is_linked(b2, 'pp_DefinitionArgumentList14', a)
...
def test_assoc_methodExpr99_link_reassign_clear():
    ...
    if hasattr(b1, 'pp_Expression100'):
        assert _is_linked(b1, 'pp_Expression100', a)
    _safe_set(a, 'pp_MethodCall', b2)
    assert _is_linked(a, 'pp_MethodCall', b2)
    if hasattr(b1, 'pp_Expression100'):
        assert not _is_linked(b1, 'pp_Expression100', a)
    if hasattr(b2, 'pp_Expression100'):
        assert _is_linked(b2, 'pp_Expression100', a)
    _safe_set(a, 'pp_MethodCall', None)
    assert not _is_linked(a, 'pp_MethodCall', b2)
    if hasattr(b2, 'pp_Expression100'):
        assert not _is_linked(b2, 'pp_Expression100', a)
...
def test_assoc_statements10_link_reassign_clear():
    ...
    b1 = pp_Expression()
    b2 = pp_Expression()
    _safe_set(a, 'pp_Definition11', {b1})
    assert _is_linked(a, 'pp_Definition11', b1)
    if hasattr(b1, 'pp_Expression12'):
        assert _is_linked(b1, 'pp_Expression12', a)
    _safe_set(a, 'pp_Definition11', {b2})
    assert _is_linked(a, 'pp_Definition11', b2)
    if hasattr(b1, 'pp_Expression12'):
        assert not _is_linked(b1, 'pp_Expression12', a)
    if hasattr(b2, 'pp_Expression12'):
        assert _is_linked(b2, 'pp_Expression12', a)
    _safe_set(a, 'pp_Definition11', set())
    assert not _is_linked(a, 'pp_Definition11', b2)
    if hasattr(b2, 'pp_Expression12'):
        assert not _is_linked(b2, 'pp_Expression12', a)
