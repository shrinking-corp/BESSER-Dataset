...
class Buyer:
    ...
    def __init__(self, name: str, id: int, mailid: str, address: str, phoneno: int, T: str, customercare26: set["Customercare"] = None):
        self.name = name
        self.id = id
        self.mailid = mailid
    ...
    def customercare26(self, value):
        # Bidirectional consistency
        old_value = getattr(self, f"_Buyer__customercare26", None)
...
class Seller:
    ...
    def __init__(self, name: str, id: int, mailid: str, address: str, phoneno: int, T: str, customercare33: "Customercare" = None):
        self.name = name
        self.id = id
        self.mailid = mailid
        self.address = address
    ...
    def customercare33(self, value):
        # Bidirectional consistency
        old_value = getattr(self, f"_Seller__customercare33", None)
        self.__customercare33 = value
        
        # Remove self from old opposite end
        if old_value is not None:
            if hasattr(old_value, "seller32"):
                opp_val = getattr(old_value, "seller32", None)
                if isinstance(opp_val, set):
                    opp_val.discard(self)
                
        # Add self to new opposite end
        if value is not None:
            if hasattr(value, "seller32"):
                opp_val = getattr(value, "seller32", None)
                if opp_val is None:
                    setattr(value, "seller32", set([self]))
                elif isinstance(opp_val, set):
                    opp_val.add(self)
...
class Customercare:
    ...
    def __init__(self, no: int, address: str, customer27: set["Buyer"] = None, seller32: set["Seller"] = None):
        self.no = no
        self.address = address
        self.customer27 = customer27 if customer27 is not None else set()
        self.seller32 = seller32 if seller32 is not None else set()
    ...
    @property
    def seller32(self):
        return self.__seller32
    @seller32.setter
    def seller32(self, value):
        # Bidirectional consistency
        old_value = getattr(self, f"_Customercare__seller32", None)
        self.__seller32 = value if value is not None else set()
        
        # Remove self from old opposite end
        if old_value is not None:
            for item in old_value:
                if hasattr(item, "customercare33"):
                    opp_val = getattr(item, "customercare33", None)
                    
                    if opp_val == self:
                        setattr(item, "customercare33", None)
        ...
        if value is not None:
            for item in value:
                if hasattr(item, "customercare33"):
                    ...
                    setattr(item, "customercare33", self)
...
class Shipment:
    ...
    def __init__(self, packing: str, warehouse29: set["sellers_warehouse"] = None):
...
class Transaction:
    ...
    def __init__(self, cashondelivery: int):
...
class sellers_warehouse:
    ...
    def shipment28(self, value):
        # Bidirectional consistency
        old_value = getattr(self, f"_sellers_warehouse__shipment28", None)
    ...
    def product31(self, value):
        # Bidirectional consistency
        old_value = getattr(self, f"_sellers_warehouse__product31", None)
...
class Product:
    ...
    def __init__(self, name: str, id: int, type: str, seller: str, oRDER25: set["Order"] = None, warehouse30: set["sellers_warehouse"] = None):
        self.name = name
        self.id = id
        self.type = type
        self.seller = seller
