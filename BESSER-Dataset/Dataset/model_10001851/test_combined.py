...
def test_C_attC_value_roundtrip():
    instance = C(attC=7, attC2=True)
    assert instance.attC == 7
    instance.attC = 13
    assert instance.attC == 13
...
def test_Y_attY_value_roundtrip():
    instance = Y(attY=7)
    assert instance.attY == 7
    instance.attY = 13
    assert instance.attY == 13
...
C_strategy = st.builds(C, attC=st.integers(), attC2=st.booleans())
@given(instance=C_strategy)
@settings(max_examples=25)
...
Y_strategy = st.builds(Y, attY=st.integers())
@given(instance=Y_strategy)
@settings(max_examples=25)
