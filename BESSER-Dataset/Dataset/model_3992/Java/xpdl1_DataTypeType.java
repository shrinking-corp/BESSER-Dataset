





import java.util.List;
import java.util.ArrayList;

public class xpdl1_DataTypeType  {






    private xpdl1_ListTypeType xpdl1_listtypetype;




    private xpdl1_DeclaredTypeType xpdl1_declaredtypetype;




    private xpdl1_BasicTypeType xpdl1_basictypetype;




    private xpdl1_EnumerationTypeType xpdl1_enumerationtypetype;




    private xpdl1_SchemaTypeType xpdl1_schematypetype;




    private xpdl1_RecordTypeType xpdl1_recordtypetype;




    private xpdl1_ArrayTypeType xpdl1_arraytypetype;




    private xpdl1_UnionTypeType xpdl1_uniontypetype;




    private xpdl1_DataFieldType xpdl1_datafieldtype;




    private xpdl1_ExternalReferenceType xpdl1_externalreferencetype;


    public xpdl1_DataTypeType(
    ) {
    }



    public xpdl1_ListTypeType getXpdl1_listtypetype() {
        return xpdl1_listtypetype;
    }

    public void setXpdl1_listtypetype(xpdl1_ListTypeType xpdl1_listtypetype) {
        this.xpdl1_listtypetype = xpdl1_listtypetype;
    }
    public xpdl1_DeclaredTypeType getXpdl1_declaredtypetype() {
        return xpdl1_declaredtypetype;
    }

    public void setXpdl1_declaredtypetype(xpdl1_DeclaredTypeType xpdl1_declaredtypetype) {
        this.xpdl1_declaredtypetype = xpdl1_declaredtypetype;
    }
    public xpdl1_BasicTypeType getXpdl1_basictypetype() {
        return xpdl1_basictypetype;
    }

    public void setXpdl1_basictypetype(xpdl1_BasicTypeType xpdl1_basictypetype) {
        this.xpdl1_basictypetype = xpdl1_basictypetype;
    }
    public xpdl1_EnumerationTypeType getXpdl1_enumerationtypetype() {
        return xpdl1_enumerationtypetype;
    }

    public void setXpdl1_enumerationtypetype(xpdl1_EnumerationTypeType xpdl1_enumerationtypetype) {
        this.xpdl1_enumerationtypetype = xpdl1_enumerationtypetype;
    }
    public xpdl1_SchemaTypeType getXpdl1_schematypetype() {
        return xpdl1_schematypetype;
    }

    public void setXpdl1_schematypetype(xpdl1_SchemaTypeType xpdl1_schematypetype) {
        this.xpdl1_schematypetype = xpdl1_schematypetype;
    }
    public xpdl1_RecordTypeType getXpdl1_recordtypetype() {
        return xpdl1_recordtypetype;
    }

    public void setXpdl1_recordtypetype(xpdl1_RecordTypeType xpdl1_recordtypetype) {
        this.xpdl1_recordtypetype = xpdl1_recordtypetype;
    }
    public xpdl1_ArrayTypeType getXpdl1_arraytypetype() {
        return xpdl1_arraytypetype;
    }

    public void setXpdl1_arraytypetype(xpdl1_ArrayTypeType xpdl1_arraytypetype) {
        this.xpdl1_arraytypetype = xpdl1_arraytypetype;
    }
    public xpdl1_UnionTypeType getXpdl1_uniontypetype() {
        return xpdl1_uniontypetype;
    }

    public void setXpdl1_uniontypetype(xpdl1_UnionTypeType xpdl1_uniontypetype) {
        this.xpdl1_uniontypetype = xpdl1_uniontypetype;
    }
    public xpdl1_DataFieldType getXpdl1_datafieldtype() {
        return xpdl1_datafieldtype;
    }

    public void setXpdl1_datafieldtype(xpdl1_DataFieldType xpdl1_datafieldtype) {
        this.xpdl1_datafieldtype = xpdl1_datafieldtype;
    }
    public xpdl1_ExternalReferenceType getXpdl1_externalreferencetype() {
        return xpdl1_externalreferencetype;
    }

    public void setXpdl1_externalreferencetype(xpdl1_ExternalReferenceType xpdl1_externalreferencetype) {
        this.xpdl1_externalreferencetype = xpdl1_externalreferencetype;
    }

}