...
def test_admin_has_aId():
    assert hasattr(Admin, "aId")
    descriptor = None
...
def test_admin_has_aName():
    assert hasattr(Admin, "aName")
    descriptor = None
    for klass in Admin.__mro__:
...
@given(instance=Admin_strategy)
def test_admin_aId_setter(instance):
...
@given(instance=Admin_strategy)
def test_admin_aName_setter(instance):
