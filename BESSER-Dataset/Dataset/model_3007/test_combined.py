...
def test_hyp_minijava_nativeexpression_is_not_abstract():
    assert not inspect.isabstract(miniJava_NativeExpression)


def test_hyp_minijava_nativeexpression_constructor_exists():
    assert callable(miniJava_NativeExpression.__init__)


def test_hyp_minijava_nativeexpression_constructor_args():
...
def test_hyp_minijava_nativeexpression_value_setter(instance):
    original = instance.value
    instance.value = original
    assert instance.value == original
...
def test_miniJava_NativeExpression_value_value_roundtrip():
    instance = miniJava_NativeExpression(value="sample_text")
    assert instance.value == "sample_text"
    instance.value = "sample_text_2"
    assert instance.value == "sample_text_2"
...
def test_miniJava_NativeExpression_isa_Expression():
    instance = miniJava_NativeExpression(value="sample_text")
    assert isinstance(instance, Expression)
...
def test_assoc_cache12_link_reassign_clear():
    a = miniJava_Method(isabstract=True, isstatic=True)
    ...
    _safe_set(a, 'miniJava_Method13', set())
...
def test_assoc_implementz3_link_reassign_clear():
    a = miniJava_TypeDeclaration(accessLevel="sample_text")
...
def test_assoc_instance210_link_reassign_clear():
    a = miniJava_ArrayInstance(size=7)
    b1 = miniJava_ArrayRefValue()
    b2 = miniJava_ArrayRefValue()
    _safe_set(a, 'miniJava_ArrayInstance211', b1)
    assert _is_linked(a, 'miniJava_ArrayInstance211', b1)
    if hasattr(b1, 'miniJava_ArrayRefValue'):
        assert _is_linked(b1, 'miniJava_ArrayRefValue', a)
    _safe_set(a, 'miniJava_ArrayInstance211', b2)
    assert _is_linked(a, 'miniJava_ArrayInstance211', b2)
    if hasattr(b1, 'miniJava_ArrayRefValue'):
    ...
    _safe_set(a, 'miniJava_ArrayInstance211', None)
    assert not _is_linked(a, 'miniJava_ArrayInstance211', b2)
    if hasattr(b2, 'miniJava_ArrayRefValue'):
        assert not _is_linked(b2, 'miniJava_ArrayRefValue', a)


def test_assoc_key218_link_reassign_clear():
    a = miniJava_Clazz(isabstract=True)
    b1 = miniJava_ClazzToMethodMap()
    b2 = miniJava_ClazzToMethodMap()
    _safe_set(a, 'miniJava_Clazz220', b1)
    assert _is_linked(a, 'miniJava_Clazz220', b1)
    if hasattr(b1, 'miniJava_ClazzToMethodMap219'):
        assert _is_linked(b1, 'miniJava_ClazzToMethodMap219', a)
    _safe_set(a, 'miniJava_Clazz220', b2)
    assert _is_linked(a, 'miniJava_Clazz220', b2)
    if hasattr(b1, 'miniJava_ClazzToMethodMap219'):
        assert not _is_linked(b1, 'miniJava_ClazzToMethodMap219', a)
    if hasattr(b2, 'miniJava_ClazzToMethodMap219'):
        assert _is_linked(b2, 'miniJava_ClazzToMethodMap219', a)
    _safe_set(a, 'miniJava_Clazz220', None)
    assert not _is_linked(a, 'miniJava_Clazz220', b2)
    if hasattr(b2, 'miniJava_ClazzToMethodMap219'):
        assert not _is_linked(b2, 'miniJava_ClazzToMethodMap219', a)
...
def test_assoc_method131_link_reassign_clear():
    ...
    _safe_set(a, 'miniJava_Method133', b1)
    assert _is_linked(a, 'miniJava_Method133', b1)
    if hasattr(b1, 'miniJava_MethodCall132'):
        assert _is_linked(b1, 'miniJava_MethodCall132', a)
    _safe_set(a, 'miniJava_Method133', b2)
    assert _is_linked(a, 'miniJava_Method133', b2)
    if hasattr(b1, 'miniJava_MethodCall132'):
        assert not _is_linked(b1, 'miniJava_MethodCall132', a)
    if hasattr(b2, 'miniJava_MethodCall132'):
        assert _is_linked(b2, 'miniJava_MethodCall132', a)
    _safe_set(a, 'miniJava_Method133', None)
    assert not _is_linked(a, 'miniJava_Method133', b2)
    if hasattr(b2, 'miniJava_MethodCall132'):
        assert not _is_linked(b2, 'miniJava_MethodCall132', a)


def test_assoc_outputStream169_link_reassign_clear():
    ...
    if hasattr(b1, 'miniJava_State170'):
        assert _is_linked(b1, 'miniJava_State170', a)
    _safe_set(a, 'miniJava_OutputStream', b2)
    assert _is_linked(a, 'miniJava_OutputStream', b2)
    if hasattr(b1, 'miniJava_State170'):
        assert not _is_linked(b1, 'miniJava_State170', a)
    if hasattr(b2, 'miniJava_State170'):
        assert _is_linked(b2, 'miniJava_State170', a)
    _safe_set(a, 'miniJava_OutputStream', None)
    assert not _is_linked(a, 'miniJava_OutputStream', b2)
    if hasattr(b2, 'miniJava_State170'):
        assert not _is_linked(b2, 'miniJava_State170', a)
...
def test_assoc_superClass8_link_reassign_clear():
    ...
    if hasattr(b1, 'miniJava_Clazz7'):
        assert _is_linked(b1, 'miniJava_Clazz7', a)
    _safe_set(a, 'miniJava_Clazz', b2)
    assert _is_linked(a, 'miniJava_Clazz', b2)
    if hasattr(b1, 'miniJava_Clazz7'):
        assert not _is_linked(b1, 'miniJava_Clazz7', a)
    if hasattr(b2, 'miniJava_Clazz7'):
        assert _is_linked(b2, 'miniJava_Clazz7', a)
    _safe_set(a, 'miniJava_Clazz', None)
    assert not _is_linked(a, 'miniJava_Clazz', b2)
    if hasattr(b2, 'miniJava_Clazz7'):
        assert not _is_linked(b2, 'miniJava_Clazz7', a)
...
def test_assoc_type137_link_reassign_clear():
    ...
    _safe_set(a, 'miniJava_Clazz138', b1)
    assert _is_linked(a, 'miniJava_Clazz138', b1)
    if hasattr(b1, 'miniJava_NewObject'):
        assert _is_linked(b1, 'miniJava_NewObject', a)
    _safe_set(a, 'miniJava_Clazz138', b2)
    assert _is_linked(a, 'miniJava_Clazz138', b2)
    if hasattr(b1, 'miniJava_NewObject'):
        assert not _is_linked(b1, 'miniJava_NewObject', a)
    if hasattr(b2, 'miniJava_NewObject'):
        assert _is_linked(b2, 'miniJava_NewObject', a)
    _safe_set(a, 'miniJava_Clazz138', None)
    assert not _is_linked(a, 'miniJava_Clazz138', b2)
    if hasattr(b2, 'miniJava_NewObject'):
        assert not _is_linked(b2, 'miniJava_NewObject', a)


def test_assoc_type202_link_reassign_clear():
    a = miniJava_Clazz(isabstract=True)
    b1 = miniJava_ObjectInstance()
    b2 = miniJava_ObjectInstance()
    _safe_set(a, 'miniJava_Clazz204', b1)
    assert _is_linked(a, 'miniJava_Clazz204', b1)
    if hasattr(b1, 'miniJava_ObjectInstance203'):
        assert _is_linked(b1, 'miniJava_ObjectInstance203', a)
    _safe_set(a, 'miniJava_Clazz204', b2)
    assert _is_linked(a, 'miniJava_Clazz204', b2)
    if hasattr(b1, 'miniJava_ObjectInstance203'):
        assert not _is_linked(b1, 'miniJava_ObjectInstance203', a)
    if hasattr(b2, 'miniJava_ObjectInstance203'):
        assert _is_linked(b2, 'miniJava_ObjectInstance203', a)
    _safe_set(a, 'miniJava_Clazz204', None)
    assert not _is_linked(a, 'miniJava_Clazz204', b2)
    if hasattr(b2, 'miniJava_ObjectInstance203'):
        assert not _is_linked(b2, 'miniJava_ObjectInstance203', a)


def test_assoc_value205_link_reassign_clear():
    a = miniJava_ArrayInstance(size=7)
    b1 = miniJava_Value()
    b2 = miniJava_Value()
    _safe_set(a, 'miniJava_ArrayInstance206', {b1})
    assert _is_linked(a, 'miniJava_ArrayInstance206', b1)
    if hasattr(b1, 'miniJava_Value207'):
        assert _is_linked(b1, 'miniJava_Value207', a)
    _safe_set(a, 'miniJava_ArrayInstance206', {b2})
    assert _is_linked(a, 'miniJava_ArrayInstance206', b2)
    if hasattr(b1, 'miniJava_Value207'):
        assert not _is_linked(b1, 'miniJava_Value207', a)
    if hasattr(b2, 'miniJava_Value207'):
        assert _is_linked(b2, 'miniJava_Value207', a)
    _safe_set(a, 'miniJava_ArrayInstance206', set())
    assert not _is_linked(a, 'miniJava_ArrayInstance206', b2)
    if hasattr(b2, 'miniJava_Value207'):
        assert not _is_linked(b2, 'miniJava_Value207', a)


def test_assoc_value221_link_reassign_clear():
    a = miniJava_Method(isabstract=True, isstatic=True)
    b1 = miniJava_ClazzToMethodMap()
    b2 = miniJava_ClazzToMethodMap()
    _safe_set(a, 'miniJava_Method223', b1)
    assert _is_linked(a, 'miniJava_Method223', b1)
    if hasattr(b1, 'miniJava_ClazzToMethodMap222'):
        assert _is_linked(b1, 'miniJava_ClazzToMethodMap222', a)
    _safe_set(a, 'miniJava_Method223', b2)
    assert _is_linked(a, 'miniJava_Method223', b2)
    if hasattr(b1, 'miniJava_ClazzToMethodMap222'):
    ...
    _safe_set(a, 'miniJava_Method223', None)
    assert not _is_linked(a, 'miniJava_Method223', b2)
    if hasattr(b2, 'miniJava_ClazzToMethodMap222'):
...
miniJava_NativeExpression_strategy = st.builds(miniJava_NativeExpression, value=safe_text)
@given(instance=miniJava_NativeExpression_strategy)
@settings(max_examples=25)
