...
from python_code import (
    ...
    generate_attendance_report_external,
...
def test_hyp_generate_attendance_report_external_is_not_abstract():
    assert not inspect.isabstract(generate_attendance_report_external)


def test_hyp_generate_attendance_report_external_constructor_exists():
    assert callable(generate_attendance_report_external.__init__)


def test_hyp_generate_attendance_report_external_constructor_args():
    sig = inspect.signature(generate_attendance_report_external.__init__)
    params = list(sig.parameters.keys())
...
generate_attendance_report_external_strategy = st.builds(
    generate_attendance_report_external,
)
...
def test_assoc_FACULTY_ADMIN_link_reassign_clear():
    ...
    if hasattr(b1, 'fACULTY31'):
        assert _is_linked(b1, 'fACULTY31', a)
    _safe_set(a, 'aDMIN30', b2)
    assert _is_linked(a, 'aDMIN30', b2)
    if hasattr(b1, 'fACULTY31'):
        assert not _is_linked(b1, 'fACULTY31', a)
    if hasattr(b2, 'fACULTY31'):
        assert _is_linked(b2, 'fACULTY31', a)
    _safe_set(a, 'aDMIN30', None)
    assert not _is_linked(a, 'aDMIN30', b2)
    if hasattr(b2, 'fACULTY31'):
        assert not _is_linked(b2, 'fACULTY31', a)
...
def test_assoc_FACULTY_STUDENT_link_reassign_clear():
    ...
    _safe_set(a, 'fACULTY29', {b1})
    assert _is_linked(a, 'fACULTY29', b1)
    if hasattr(b1, 'sTUDENT28'):
        assert _is_linked(b1, 'sTUDENT28', a)
    _safe_set(a, 'fACULTY29', {b2})
    assert _is_linked(a, 'fACULTY29', b2)
    if hasattr(b1, 'sTUDENT28'):
        assert not _is_linked(b1, 'sTUDENT28', a)
    if hasattr(b2, 'sTUDENT28'):
        assert _is_linked(b2, 'sTUDENT28', a)
    _safe_set(a, 'fACULTY29', set())
    assert not _is_linked(a, 'fACULTY29', b2)
    if hasattr(b2, 'sTUDENT28'):
        assert not _is_linked(b2, 'sTUDENT28', a)
...
def test_assoc_PARENT_ADMIN_link_reassign_clear():
    ...
    b1 = ADMIN(id="sample_text", password="sample_text")
    b2 = ADMIN(id="sample_text_2", password="sample_text_2")
    _safe_set(a, 'aDMIN34', b1)
    assert _is_linked(a, 'aDMIN34', b1)
    if hasattr(b1, 'pARENT35'):
        assert _is_linked(b1, 'pARENT35', a)
    _safe_set(a, 'aDMIN34', b2)
    assert _is_linked(a, 'aDMIN34', b2)
    if hasattr(b1, 'pARENT35'):
        assert not _is_linked(b1, 'pARENT35', a)
    if hasattr(b2, 'pARENT35'):
        assert _is_linked(b2, 'pARENT35', a)
    _safe_set(a, 'aDMIN34', None)
    assert not _is_linked(a, 'aDMIN34', b2)
    if hasattr(b2, 'pARENT35'):
        assert not _is_linked(b2, 'pARENT35', a)
...
def test_assoc_STUDENT_ADMIN_link_reassign_clear():
    ...
    if hasattr(b1, 'sTUDENT33'):
        assert _is_linked(b1, 'sTUDENT33', a)
    _safe_set(a, 'aDMIN32', b2)
    assert _is_linked(a, 'aDMIN32', b2)
    if hasattr(b1, 'sTUDENT33'):
        assert not _is_linked(b1, 'sTUDENT33', a)
    if hasattr(b2, 'sTUDENT33'):
        assert _is_linked(b2, 'sTUDENT33', a)
    _safe_set(a, 'aDMIN32', None)
    assert not _is_linked(a, 'aDMIN32', b2)
    if hasattr(b2, 'sTUDENT33'):
        assert not _is_linked(b2, 'sTUDENT33', a)
...
generate_attendance_report_external_strategy = st.builds(generate_attendance_report_external)
@given(instance=generate_attendance_report_external_strategy)
@settings(max_examples=25)
def test_generate_attendance_report_external_instantiation(instance):
    assert isinstance(instance, generate_attendance_report_external)
