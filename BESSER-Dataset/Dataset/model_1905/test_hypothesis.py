...
def test_unifiedmetamodel__layersegment_has_name():
    assert hasattr(UnifiedMetamodel__LayerSegment, "name")
    descriptor = None
    for klass in UnifiedMetamodel__LayerSegment.__mro__:
        if "name" in klass.__dict__:
            descriptor = klass.__dict__["name"]
            break
    assert isinstance(descriptor, property)
...
@given(instance=UnifiedMetamodel__LayerSegment_strategy)
def test_unifiedmetamodel__layersegment_name_setter(instance):
    original = instance.name
    instance.name = original
    assert instance.name == original
