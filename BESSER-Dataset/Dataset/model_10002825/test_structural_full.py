...
def test_assoc_Guest_Booking_link_reassign_clear():
    a = Booking(_numberOfNights=7, bookingDate="sample_text", checkInDate="sample_text", checkOutDate="sample_text")
    b1 = Guest()
    b2 = Guest()
    _safe_set(a, 'guest11', {b1})
    assert _is_linked(a, 'guest11', b1)
    if hasattr(b1, 'booking10'):
        assert _is_linked(b1, 'booking10', a)
    _safe_set(a, 'guest11', {b2})
    assert _is_linked(a, 'guest11', b2)
    if hasattr(b1, 'booking10'):
        assert not _is_linked(b1, 'booking10', a)
    if hasattr(b2, 'booking10'):
        assert _is_linked(b2, 'booking10', a)
    _safe_set(a, 'guest11', set())
    assert not _is_linked(a, 'guest11', b2)
    if hasattr(b2, 'booking10'):
