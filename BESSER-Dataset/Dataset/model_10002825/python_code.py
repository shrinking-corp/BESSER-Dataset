...
class Booking:
    ...
    def __init__(self, bookingDate: str, checkInDate: str, checkOutDate: str, _numberOfNights: int, hotel8: "Hotel" = None, guest11: set["Guest"] = None):
        self.bookingDate = bookingDate
        self.checkInDate = checkInDate
        self.checkOutDate = checkOutDate
        self._numberOfNights = _numberOfNights
        self.hotel8 = hotel8
        self.guest11 = guest11 if guest11 is not None else set()
    ...
    def guest11(self, value):
        # Bidirectional consistency
        old_value = getattr(self, f"_Booking__guest11", None)
        self.__guest11 = value if value is not None else set()
        
        # Remove self from old opposite end
        if old_value is not None:
            for item in old_value:
                if hasattr(item, "booking10"):
                    opp_val = getattr(item, "booking10", None)
        ...
        if value is not None:
            for item in value:
                if hasattr(item, "booking10"):
                    ...
                    if opp_val is None:
                        setattr(item, "booking10", set([self]))
                    elif isinstance(opp_val, set):
                        opp_val.add(self)
...
class Room:
    ...
    def __init__(self, name: str, roomType5: "RoomType" = None, occupant7: set["Guest"] = None):
        self.name = name
        self.roomType5 = roomType5
        self.occupant7 = occupant7 if occupant7 is not None else set()
    ...
    def occupant7(self, value):
        ...
        self.__occupant7 = value if value is not None else set()
        
        # Remove self from old opposite end
        if old_value is not None:
            for item in old_value:
                if hasattr(item, "occupied6"):
                    opp_val = getattr(item, "occupied6", None)
                    
                    if opp_val == self:
                        setattr(item, "occupied6", None)
