...
@given(instance=stext_VariableDefinition_strategy)
def test_hyp_stext_variabledefinition_const_setter(instance):
...
def test_stext_VariableDefinition_const_value_roundtrip():
    instance = stext_VariableDefinition(const=True, external=True, readonly=True)
...
def test_assoc_initialValue9_link_reassign_clear():
    a = stext_VariableDefinition(const=True, external=True, readonly=True)
    b1 = stext_Expression()
    b2 = stext_Expression()
...
stext_VariableDefinition_strategy = st.builds(stext_VariableDefinition, const=st.booleans(), external=st.booleans(), readonly=st.booleans())
@given(instance=stext_VariableDefinition_strategy)
@settings(max_examples=25)
