...
def test_v2_is_not_abstract():
    assert not inspect.isabstract(V2)


def test_v2_constructor_exists():
    assert callable(V2.__init__)


def test_v2_constructor_args():
    sig = inspect.signature(V2.__init__)
    params = list(sig.parameters.keys())



def test_v1_is_not_abstract():
    assert not inspect.isabstract(V1)


def test_v1_constructor_exists():
    assert callable(V1.__init__)


def test_v1_constructor_args():
    sig = inspect.signature(V1.__init__)
    params = list(sig.parameters.keys())



def test_v_is_not_abstract():
    assert not inspect.isabstract(V)


def test_v_constructor_exists():
    assert callable(V.__init__)


def test_v_constructor_args():
...
def test_v_has_attV1():
    assert hasattr(V, "attV1")
    descriptor = None
    for klass in V.__mro__:
        if "attV1" in klass.__dict__:
            descriptor = klass.__dict__["attV1"]
            break
    assert isinstance(descriptor, property)

def test_v_has_attV2():
    assert hasattr(V, "attV2")
    descriptor = None
    for klass in V.__mro__:
        if "attV2" in klass.__dict__:
            descriptor = klass.__dict__["attV2"]
            break
    assert isinstance(descriptor, property)
...
def test_w_has_attW():
    assert hasattr(W, "attW")
    descriptor = None
    for klass in W.__mro__:
        if "attW" in klass.__dict__:
            descriptor = klass.__dict__["attW"]
            break
    assert isinstance(descriptor, property)
...
def test_x_has_attX():
    assert hasattr(X, "attX")
    descriptor = None
    for klass in X.__mro__:
        if "attX" in klass.__dict__:
            descriptor = klass.__dict__["attX"]
            break
    assert isinstance(descriptor, property)
...
@given(instance=V_strategy)
def test_v_attV1_setter(instance):
...
@given(instance=V_strategy)
def test_v_attV2_setter(instance):
...
@given(instance=W_strategy)
def test_w_attW_setter(instance):
...
@given(instance=X_strategy)
def test_x_attX_setter(instance):
