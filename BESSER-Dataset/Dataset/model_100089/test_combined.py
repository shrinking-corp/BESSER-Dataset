...
def test_hyp_mavenmaven_jellyforeach_is_not_abstract():
    assert not inspect.isabstract(MavenMaven_JellyForEach)


def test_hyp_mavenmaven_jellyforeach_constructor_exists():
    assert callable(MavenMaven_JellyForEach.__init__)


def test_hyp_mavenmaven_jellyforeach_constructor_args():
...
def test_hyp_mavenmaven_jellyforeach_indexVar_setter(instance):
...
@given(instance=MavenMaven_JellyForEach_strategy)
def test_hyp_mavenmaven_jellyforeach_items_setter(instance):
...
@given(instance=MavenMaven_JellyForEach_strategy)
def test_hyp_mavenmaven_jellyforeach_var_setter(instance):
...
def test_MavenMaven_JellyForEach_indexVar_value_roundtrip():
    instance = MavenMaven_JellyForEach(indexVar="sample_text", items="sample_text", var="sample_text")
    assert instance.indexVar == "sample_text"
    instance.indexVar = "sample_text_2"
    assert instance.indexVar == "sample_text_2"


def test_MavenMaven_JellyForEach_items_value_roundtrip():
    instance = MavenMaven_JellyForEach(indexVar="sample_text", items="sample_text", var="sample_text")
...
def test_MavenMaven_JellyForEach_var_value_roundtrip():
    instance = MavenMaven_JellyForEach(indexVar="sample_text", items="sample_text", var="sample_text")
...
def test_MavenMaven_JellyForEach_isa_JellyCommand():
    instance = MavenMaven_JellyForEach(indexVar="sample_text", items="sample_text", var="sample_text")
...
def test_assoc_classPath54_link_reassign_clear():
    ...
    b1 = ClassPath()
    b2 = ClassPath()
    _safe_set(a, 'MavenMaven_Javac55', b1)
    assert _is_linked(a, 'MavenMaven_Javac55', b1)
    if hasattr(b1, 'ClassPath56'):
        assert _is_linked(b1, 'ClassPath56', a)
    _safe_set(a, 'MavenMaven_Javac55', b2)
    assert _is_linked(a, 'MavenMaven_Javac55', b2)
    if hasattr(b1, 'ClassPath56'):
        assert not _is_linked(b1, 'ClassPath56', a)
    if hasattr(b2, 'ClassPath56'):
        assert _is_linked(b2, 'ClassPath56', a)
    _safe_set(a, 'MavenMaven_Javac55', None)
    assert not _is_linked(a, 'MavenMaven_Javac55', b2)
    if hasattr(b2, 'ClassPath56'):
        assert not _is_linked(b2, 'ClassPath56', a)


def test_assoc_contents14_link_reassign_clear():
    a = MavenMaven_JellyForEach(indexVar="sample_text", items="sample_text", var="sample_text")
    b1 = ContentsGoal()
    b2 = ContentsGoal()
    _safe_set(a, 'MavenMaven_JellyForEach', b1)
    assert _is_linked(a, 'MavenMaven_JellyForEach', b1)
    if hasattr(b1, 'ContentsGoal'):
        assert _is_linked(b1, 'ContentsGoal', a)
    _safe_set(a, 'MavenMaven_JellyForEach', b2)
    assert _is_linked(a, 'MavenMaven_JellyForEach', b2)
    if hasattr(b1, 'ContentsGoal'):
        assert not _is_linked(b1, 'ContentsGoal', a)
    if hasattr(b2, 'ContentsGoal'):
        assert _is_linked(b2, 'ContentsGoal', a)
    _safe_set(a, 'MavenMaven_JellyForEach', None)
    assert not _is_linked(a, 'MavenMaven_JellyForEach', b2)
    if hasattr(b2, 'ContentsGoal'):
        assert not _is_linked(b2, 'ContentsGoal', a)


def test_assoc_exclude30_link_reassign_clear():
    a = MavenMaven_FileSet(dir="sample_text")
    b1 = Excludes()
    b2 = Excludes()
    _safe_set(a, 'MavenMaven_FileSet31', {b1})
    assert _is_linked(a, 'MavenMaven_FileSet31', b1)
    if hasattr(b1, 'Excludes'):
        assert _is_linked(b1, 'Excludes', a)
    _safe_set(a, 'MavenMaven_FileSet31', {b2})
    assert _is_linked(a, 'MavenMaven_FileSet31', b2)
    if hasattr(b1, 'Excludes'):
        assert not _is_linked(b1, 'Excludes', a)
    if hasattr(b2, 'Excludes'):
        assert _is_linked(b2, 'Excludes', a)
    _safe_set(a, 'MavenMaven_FileSet31', set())
    assert not _is_linked(a, 'MavenMaven_FileSet31', b2)
    if hasattr(b2, 'Excludes'):
        assert not _is_linked(b2, 'Excludes', a)


def test_assoc_fileset39_link_reassign_clear():
    a = MavenMaven_Path(id="sample_text", refid="sample_text")
    b1 = FileSet()
    b2 = FileSet()
    _safe_set(a, 'MavenMaven_Path40', {b1})
    assert _is_linked(a, 'MavenMaven_Path40', b1)
    if hasattr(b1, 'FileSet'):
        assert _is_linked(b1, 'FileSet', a)
    _safe_set(a, 'MavenMaven_Path40', {b2})
    assert _is_linked(a, 'MavenMaven_Path40', b2)
    if hasattr(b1, 'FileSet'):
        assert not _is_linked(b1, 'FileSet', a)
    if hasattr(b2, 'FileSet'):
        assert _is_linked(b2, 'FileSet', a)
    _safe_set(a, 'MavenMaven_Path40', set())
    assert not _is_linked(a, 'MavenMaven_Path40', b2)
    if hasattr(b2, 'FileSet'):
...
def test_assoc_fileset43_link_reassign_clear():
    ...
    b1 = FileSet()
    b2 = FileSet()
    _safe_set(a, 'MavenMaven_ClassPath44', {b1})
    assert _is_linked(a, 'MavenMaven_ClassPath44', b1)
    if hasattr(b1, 'FileSet45'):
        assert _is_linked(b1, 'FileSet45', a)
    _safe_set(a, 'MavenMaven_ClassPath44', {b2})
    assert _is_linked(a, 'MavenMaven_ClassPath44', b2)
    if hasattr(b1, 'FileSet45'):
        assert not _is_linked(b1, 'FileSet45', a)
    if hasattr(b2, 'FileSet45'):
        assert _is_linked(b2, 'FileSet45', a)
    _safe_set(a, 'MavenMaven_ClassPath44', set())
    assert not _is_linked(a, 'MavenMaven_ClassPath44', b2)
    if hasattr(b2, 'FileSet45'):
        assert not _is_linked(b2, 'FileSet45', a)


def test_assoc_fileset57_link_reassign_clear():
    ...
    if hasattr(b1, 'FileSet58'):
        assert _is_linked(b1, 'FileSet58', a)
    _safe_set(a, 'MavenMaven_Copy', b2)
    assert _is_linked(a, 'MavenMaven_Copy', b2)
    if hasattr(b1, 'FileSet58'):
        assert not _is_linked(b1, 'FileSet58', a)
    if hasattr(b2, 'FileSet58'):
        assert _is_linked(b2, 'FileSet58', a)
    _safe_set(a, 'MavenMaven_Copy', None)
    assert not _is_linked(a, 'MavenMaven_Copy', b2)
    if hasattr(b2, 'FileSet58'):
        assert not _is_linked(b2, 'FileSet58', a)


def test_assoc_filter32_link_reassign_clear():
...
def test_assoc_filterset59_link_reassign_clear():
    a = MavenMaven_Copy(file="sample_text", filtering="sample_text", flatten="sample_text", includeEmptyDirs="sample_text", overwrite="sample_text", presservelastmodified="sample_text", todir="sample_text", tofile="sample_text")
    b1 = FilterSet()
    b2 = FilterSet()
...
def test_assoc_filtersfile33_link_reassign_clear():
    a = MavenMaven_FilterSet(endtoken="sample_text", starttoken="sample_text")
    b1 = FiltersFile()
    b2 = FiltersFile()
    _safe_set(a, 'MavenMaven_FilterSet34', {b1})
    assert _is_linked(a, 'MavenMaven_FilterSet34', b1)
    if hasattr(b1, 'FiltersFile'):
        assert _is_linked(b1, 'FiltersFile', a)
    _safe_set(a, 'MavenMaven_FilterSet34', {b2})
    assert _is_linked(a, 'MavenMaven_FilterSet34', b2)
    if hasattr(b1, 'FiltersFile'):
        assert not _is_linked(b1, 'FiltersFile', a)
    if hasattr(b2, 'FiltersFile'):
        assert _is_linked(b2, 'FiltersFile', a)
    _safe_set(a, 'MavenMaven_FilterSet34', set())
    assert not _is_linked(a, 'MavenMaven_FilterSet34', b2)
    if hasattr(b2, 'FiltersFile'):
        assert not _is_linked(b2, 'FiltersFile', a)


def test_assoc_inExcludes52_link_reassign_clear():
    a = MavenMaven_Javac(debug="sample_text", deprecation="sample_text", destdir="sample_text", fork="sample_text", optimize="sample_text", srcdir="sample_text")
    b1 = InExcludes()
    b2 = InExcludes()
    _safe_set(a, 'MavenMaven_Javac', {b1})
    assert _is_linked(a, 'MavenMaven_Javac', b1)
    if hasattr(b1, 'InExcludes53'):
        assert _is_linked(b1, 'InExcludes53', a)
    _safe_set(a, 'MavenMaven_Javac', {b2})
    assert _is_linked(a, 'MavenMaven_Javac', b2)
    if hasattr(b1, 'InExcludes53'):
        assert not _is_linked(b1, 'InExcludes53', a)
    if hasattr(b2, 'InExcludes53'):
        assert _is_linked(b2, 'InExcludes53', a)
    _safe_set(a, 'MavenMaven_Javac', set())
    assert not _is_linked(a, 'MavenMaven_Javac', b2)
    if hasattr(b2, 'InExcludes53'):
        assert not _is_linked(b2, 'InExcludes53', a)


def test_assoc_include28_link_reassign_clear():
    a = MavenMaven_FileSet(dir="sample_text")
    b1 = Includes()
    b2 = Includes()
...
def test_assoc_mapper61_link_reassign_clear():
    a = MavenMaven_Copy(file="sample_text", filtering="sample_text", flatten="sample_text", includeEmptyDirs="sample_text", overwrite="sample_text", presservelastmodified="sample_text", todir="sample_text", tofile="sample_text")
    b1 = Mapper()
    b2 = Mapper()
    _safe_set(a, 'MavenMaven_Copy62', b1)
    assert _is_linked(a, 'MavenMaven_Copy62', b1)
    if hasattr(b1, 'Mapper'):
        assert _is_linked(b1, 'Mapper', a)
    _safe_set(a, 'MavenMaven_Copy62', b2)
    assert _is_linked(a, 'MavenMaven_Copy62', b2)
    if hasattr(b1, 'Mapper'):
        assert not _is_linked(b1, 'Mapper', a)
    if hasattr(b2, 'Mapper'):
        assert _is_linked(b2, 'Mapper', a)
    _safe_set(a, 'MavenMaven_Copy62', None)
    assert not _is_linked(a, 'MavenMaven_Copy62', b2)
    if hasattr(b2, 'Mapper'):
        assert not _is_linked(b2, 'Mapper', a)


def test_assoc_path35_link_reassign_clear():
    a = MavenMaven_Path(id="sample_text", refid="sample_text")
    b1 = Path()
    b2 = Path()
    _safe_set(a, 'MavenMaven_Path', b1)
    assert _is_linked(a, 'MavenMaven_Path', b1)
    if hasattr(b1, 'Path36'):
        assert _is_linked(b1, 'Path36', a)
    _safe_set(a, 'MavenMaven_Path', b2)
    assert _is_linked(a, 'MavenMaven_Path', b2)
    if hasattr(b1, 'Path36'):
        assert not _is_linked(b1, 'Path36', a)
    if hasattr(b2, 'Path36'):
        assert _is_linked(b2, 'Path36', a)
    _safe_set(a, 'MavenMaven_Path', None)
    assert not _is_linked(a, 'MavenMaven_Path', b2)
    if hasattr(b2, 'Path36'):
        assert not _is_linked(b2, 'Path36', a)


def test_assoc_pathElement37_link_reassign_clear():
    a = MavenMaven_Path(id="sample_text", refid="sample_text")
    b1 = PathElement()
    b2 = PathElement()
...
def test_assoc_pathElement41_link_reassign_clear():
    ...
    if hasattr(b1, 'PathElement42'):
        assert _is_linked(b1, 'PathElement42', a)
    _safe_set(a, 'MavenMaven_ClassPath', {b2})
    assert _is_linked(a, 'MavenMaven_ClassPath', b2)
    if hasattr(b1, 'PathElement42'):
        assert not _is_linked(b1, 'PathElement42', a)
    if hasattr(b2, 'PathElement42'):
        assert _is_linked(b2, 'PathElement42', a)
    _safe_set(a, 'MavenMaven_ClassPath', set())
    assert not _is_linked(a, 'MavenMaven_ClassPath', b2)
    if hasattr(b2, 'PathElement42'):
        assert not _is_linked(b2, 'PathElement42', a)


def test_assoc_patternset27_link_reassign_clear():
    a = MavenMaven_FileSet(dir="sample_text")
    b1 = PatternSet()
    b2 = PatternSet()
...
def test_assoc_postGoal24_link_reassign_clear():
    a = MavenMaven_Goal(name="sample_text")
    b1 = PostGoal()
    b2 = PostGoal()
    _safe_set(a, 'centralGoal25', b1)
    assert _is_linked(a, 'centralGoal25', b1)
    if hasattr(b1, 'PostGoal'):
        assert _is_linked(b1, 'PostGoal', a)
    _safe_set(a, 'centralGoal25', b2)
    assert _is_linked(a, 'centralGoal25', b2)
    if hasattr(b1, 'PostGoal'):
        assert not _is_linked(b1, 'PostGoal', a)
    if hasattr(b2, 'PostGoal'):
        assert _is_linked(b2, 'PostGoal', a)
    _safe_set(a, 'centralGoal25', None)
    assert not _is_linked(a, 'centralGoal25', b2)
    if hasattr(b2, 'PostGoal'):
        assert not _is_linked(b2, 'PostGoal', a)


def test_assoc_preGoal23_link_reassign_clear():
    a = MavenMaven_Goal(name="sample_text")
    b1 = PreGoal()
    b2 = PreGoal()
...
MavenMaven_JellyForEach_strategy = st.builds(MavenMaven_JellyForEach, indexVar=safe_text, items=safe_text, var=safe_text)
@given(instance=MavenMaven_JellyForEach_strategy)
@settings(max_examples=25)
