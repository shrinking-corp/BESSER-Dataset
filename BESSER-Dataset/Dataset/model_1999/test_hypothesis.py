...
def test_trace_traceproperty_is_not_abstract():
    assert not inspect.isabstract(trace_TraceProperty)


def test_trace_traceproperty_constructor_exists():
    assert callable(trace_TraceProperty.__init__)


def test_trace_traceproperty_constructor_args():
...
def test_trace_traceproperty_has_propertyName():
    assert hasattr(trace_TraceProperty, "propertyName")
    descriptor = None
    for klass in trace_TraceProperty.__mro__:
        if "propertyName" in klass.__dict__:
            descriptor = klass.__dict__["propertyName"]
            break
    assert isinstance(descriptor, property)

def test_trace_traceproperty_has_resolved():
    assert hasattr(trace_TraceProperty, "resolved")
    descriptor = None
    for klass in trace_TraceProperty.__mro__:
...
@given(instance=trace_TraceProperty_strategy)
def test_trace_traceproperty_propertyName_setter(instance):
...
@given(instance=trace_TraceProperty_strategy)
def test_trace_traceproperty_resolved_setter(instance):
...
def test_trace_traceproperty_resolvebinding_changes_state(instance):
