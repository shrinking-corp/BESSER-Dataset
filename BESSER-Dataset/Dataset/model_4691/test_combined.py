...
def test_assoc_father6_link_reassign_clear():
    ...
    b1 = basicfamily_Man()
    b2 = basicfamily_Man()
    _safe_set(a, 'basicfamily_Person7', b1)
    assert _is_linked(a, 'basicfamily_Person7', b1)
    if hasattr(b1, 'basicfamily_Man'):
        assert _is_linked(b1, 'basicfamily_Man', a)
    _safe_set(a, 'basicfamily_Person7', b2)
    assert _is_linked(a, 'basicfamily_Person7', b2)
    if hasattr(b1, 'basicfamily_Man'):
        assert not _is_linked(b1, 'basicfamily_Man', a)
    if hasattr(b2, 'basicfamily_Man'):
        assert _is_linked(b2, 'basicfamily_Man', a)
    _safe_set(a, 'basicfamily_Person7', None)
    assert not _is_linked(a, 'basicfamily_Person7', b2)
    if hasattr(b2, 'basicfamily_Man'):
        assert not _is_linked(b2, 'basicfamily_Man', a)


def test_assoc_mermbers0_link_reassign_clear():
    a = basicfamily_Person(name="sample_text")
