...
def test_hyp_miniocl_pathelementcs_pathName_setter(instance):
...
def test_miniOCL_PathElementCS_pathName_value_roundtrip():
    instance = miniOCL_PathElementCS(pathName="sample_text")
...
def test_miniOCL_NameExpCS_isa_PrimaryExpCS():
...
def test_assoc_path55_link_reassign_clear():
    a = miniOCL_PathElementCS(pathName="sample_text")
    b1 = miniOCL_PathNameCS()
    b2 = miniOCL_PathNameCS()
    _safe_set(a, 'miniOCL_PathElementCS', b1)
    assert _is_linked(a, 'miniOCL_PathElementCS', b1)
    if hasattr(b1, 'miniOCL_PathNameCS56'):
        assert _is_linked(b1, 'miniOCL_PathNameCS56', a)
    _safe_set(a, 'miniOCL_PathElementCS', b2)
    assert _is_linked(a, 'miniOCL_PathElementCS', b2)
    if hasattr(b1, 'miniOCL_PathNameCS56'):
        assert not _is_linked(b1, 'miniOCL_PathNameCS56', a)
    if hasattr(b2, 'miniOCL_PathNameCS56'):
        assert _is_linked(b2, 'miniOCL_PathNameCS56', a)
    _safe_set(a, 'miniOCL_PathElementCS', None)
    assert not _is_linked(a, 'miniOCL_PathElementCS', b2)
    if hasattr(b2, 'miniOCL_PathNameCS56'):
        assert not _is_linked(b2, 'miniOCL_PathNameCS56', a)
...
miniOCL_PathElementCS_strategy = st.builds(miniOCL_PathElementCS, pathName=safe_text)
@given(instance=miniOCL_PathElementCS_strategy)
@settings(max_examples=25)
