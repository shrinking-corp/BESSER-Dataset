...
def test_assoc_contents0_link_reassign_clear():
    a = DB_Database(name="sample_text")
    b1 = DB_DatabaseElement()
    b2 = DB_DatabaseElement()
    _safe_set(a, 'database', {b1})
    assert _is_linked(a, 'database', b1)
    if hasattr(b1, 'DatabaseElement'):
        assert _is_linked(b1, 'DatabaseElement', a)
    _safe_set(a, 'database', {b2})
    assert _is_linked(a, 'database', b2)
    if hasattr(b1, 'DatabaseElement'):
        assert not _is_linked(b1, 'DatabaseElement', a)
    if hasattr(b2, 'DatabaseElement'):
        assert _is_linked(b2, 'DatabaseElement', a)
    _safe_set(a, 'database', set())
    assert not _is_linked(a, 'database', b2)
    if hasattr(b2, 'DatabaseElement'):
        assert not _is_linked(b2, 'DatabaseElement', a)
