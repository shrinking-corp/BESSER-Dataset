...
def test_assoc_bodyElements10_link_reassign_clear():
    a = HTML_BODY(alink="sample_text", background="sample_text", bgcolor="sample_text", link="sample_text", text="sample_text", vlink="sample_text")
    b1 = HTML_BODYElement()
    b2 = HTML_BODYElement()
    _safe_set(a, 'HTML_BODY', {b1})
    assert _is_linked(a, 'HTML_BODY', b1)
    if hasattr(b1, 'HTML_BODYElement'):
        assert _is_linked(b1, 'HTML_BODYElement', a)
    _safe_set(a, 'HTML_BODY', {b2})
    assert _is_linked(a, 'HTML_BODY', b2)
    if hasattr(b1, 'HTML_BODYElement'):
        assert not _is_linked(b1, 'HTML_BODYElement', a)
    if hasattr(b2, 'HTML_BODYElement'):
        assert _is_linked(b2, 'HTML_BODYElement', a)
    _safe_set(a, 'HTML_BODY', set())
    assert not _is_linked(a, 'HTML_BODY', b2)
    if hasattr(b2, 'HTML_BODYElement'):
        assert not _is_linked(b2, 'HTML_BODYElement', a)
...
def test_assoc_children4_link_reassign_clear():
    ...
    _safe_set(a, 'HTML_HTMLElement', b1)
    assert _is_linked(a, 'HTML_HTMLElement', b1)
    if hasattr(b1, 'HTML_HTMLElement3'):
        assert _is_linked(b1, 'HTML_HTMLElement3', a)
    _safe_set(a, 'HTML_HTMLElement', b2)
    assert _is_linked(a, 'HTML_HTMLElement', b2)
    if hasattr(b1, 'HTML_HTMLElement3'):
        assert not _is_linked(b1, 'HTML_HTMLElement3', a)
    if hasattr(b2, 'HTML_HTMLElement3'):
        assert _is_linked(b2, 'HTML_HTMLElement3', a)
    _safe_set(a, 'HTML_HTMLElement', None)
    assert not _is_linked(a, 'HTML_HTMLElement', b2)
    if hasattr(b2, 'HTML_HTMLElement3'):
        assert not _is_linked(b2, 'HTML_HTMLElement3', a)


def test_assoc_html11_link_reassign_clear():
    a = HTML_BODY(alink="sample_text", background="sample_text", bgcolor="sample_text", link="sample_text", text="sample_text", vlink="sample_text")
    b1 = HTML_HTML()
    b2 = HTML_HTML()
    _safe_set(a, 'body', b1)
    assert _is_linked(a, 'body', b1)
    if hasattr(b1, 'HTML12'):
        assert _is_linked(b1, 'HTML12', a)
    _safe_set(a, 'body', b2)
    assert _is_linked(a, 'body', b2)
    if hasattr(b1, 'HTML12'):
        assert not _is_linked(b1, 'HTML12', a)
    if hasattr(b2, 'HTML12'):
        assert _is_linked(b2, 'HTML12', a)
    _safe_set(a, 'body', None)
    assert not _is_linked(a, 'body', b2)
    if hasattr(b2, 'HTML12'):
        assert not _is_linked(b2, 'HTML12', a)
...
def test_assoc_tds15_link_reassign_clear():
    a = HTML_TR(align="sample_text", valign="sample_text")
...
def test_assoc_tr16_link_reassign_clear():
    a = HTML_TR(align="sample_text", valign="sample_text")
    b1 = HTML_TD(align="sample_text", colspan="sample_text", rowspan="sample_text", valign="sample_text", width="sample_text")
    b2 = HTML_TD(align="sample_text_2", colspan="sample_text_2", rowspan="sample_text_2", valign="sample_text_2", width="sample_text_2")
    _safe_set(a, 'TR17', b1)
    assert _is_linked(a, 'TR17', b1)
    if hasattr(b1, 'tds'):
        assert _is_linked(b1, 'tds', a)
    _safe_set(a, 'TR17', b2)
    assert _is_linked(a, 'TR17', b2)
    if hasattr(b1, 'tds'):
        assert not _is_linked(b1, 'tds', a)
    if hasattr(b2, 'tds'):
        assert _is_linked(b2, 'tds', a)
    _safe_set(a, 'TR17', None)
    assert not _is_linked(a, 'TR17', b2)
    if hasattr(b2, 'tds'):
        assert not _is_linked(b2, 'tds', a)


def test_assoc_trs13_link_reassign_clear():
    a = HTML_TR(align="sample_text", valign="sample_text")
    b1 = HTML_TABLE(border="sample_text", cellpadding="sample_text", cellspacing="sample_text", width="sample_text")
    b2 = HTML_TABLE(border="sample_text_2", cellpadding="sample_text_2", cellspacing="sample_text_2", width="sample_text_2")
    _safe_set(a, 'TR', b1)
