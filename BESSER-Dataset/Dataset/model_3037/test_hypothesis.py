...
def test_basicfsm_vardecl_is_not_abstract():
    assert not inspect.isabstract(basicfsm_VarDecl)


def test_basicfsm_vardecl_constructor_exists():
    assert callable(basicfsm_VarDecl.__init__)


def test_basicfsm_vardecl_constructor_args():
    sig = inspect.signature(basicfsm_VarDecl.__init__)
    params = list(sig.parameters.keys())
    assert "name" in params, "Missing parameter 'name'"
    assert "value" in params, "Missing parameter 'value'"

def test_basicfsm_vardecl_has_name():
    assert hasattr(basicfsm_VarDecl, "name")
    descriptor = None
    for klass in basicfsm_VarDecl.__mro__:
        if "name" in klass.__dict__:
            descriptor = klass.__dict__["name"]
            break
    assert isinstance(descriptor, property)

def test_basicfsm_vardecl_has_value():
    assert hasattr(basicfsm_VarDecl, "value")
    descriptor = None
    for klass in basicfsm_VarDecl.__mro__:
...
@given(instance=basicfsm_VarDecl_strategy)
def test_basicfsm_vardecl_name_setter(instance):
    original = instance.name
    instance.name = original
    assert instance.name == original
...
@given(instance=basicfsm_VarDecl_strategy)
def test_basicfsm_vardecl_value_setter(instance):
