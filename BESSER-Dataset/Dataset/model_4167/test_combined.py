...
def test_assoc_greetings0_link_reassign_clear():
    ...
    if hasattr(b1, 'greetings_Model'):
        assert _is_linked(b1, 'greetings_Model', a)
    _safe_set(a, 'greetings_Greeting', b2)
    assert _is_linked(a, 'greetings_Greeting', b2)
    if hasattr(b1, 'greetings_Model'):
        assert not _is_linked(b1, 'greetings_Model', a)
    if hasattr(b2, 'greetings_Model'):
        assert _is_linked(b2, 'greetings_Model', a)
    _safe_set(a, 'greetings_Greeting', None)
    assert not _is_linked(a, 'greetings_Greeting', b2)
    if hasattr(b2, 'greetings_Model'):
        assert not _is_linked(b2, 'greetings_Model', a)
