...
def test_assoc_form8_link_reassign_clear():
...
def test_assoc_items6_link_reassign_clear():
    ...
    _safe_set(a, 'forms_Item7', b1)
    assert _is_linked(a, 'forms_Item7', b1)
    if hasattr(b1, 'forms_Group'):
        assert _is_linked(b1, 'forms_Group', a)
    _safe_set(a, 'forms_Item7', b2)
    assert _is_linked(a, 'forms_Item7', b2)
    if hasattr(b1, 'forms_Group'):
        assert not _is_linked(b1, 'forms_Group', a)
    if hasattr(b2, 'forms_Group'):
        assert _is_linked(b2, 'forms_Group', a)
    _safe_set(a, 'forms_Item7', None)
    assert not _is_linked(a, 'forms_Item7', b2)
    if hasattr(b2, 'forms_Group'):
        assert not _is_linked(b2, 'forms_Group', a)
