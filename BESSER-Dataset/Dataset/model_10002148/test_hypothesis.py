...
def test_class_has_asdasd():
    assert hasattr(Class, "asdasd")
    descriptor = None
    for klass in Class.__mro__:
        if "asdasd" in klass.__dict__:
            descriptor = klass.__dict__["asdasd"]
            break
    assert isinstance(descriptor, property)

def test_class_has_qweqwe():
    assert hasattr(Class, "qweqwe")
    descriptor = None
    for klass in Class.__mro__:
        if "qweqwe" in klass.__dict__:
            descriptor = klass.__dict__["qweqwe"]
            break
    assert isinstance(descriptor, property)
...
@given(instance=Class_strategy)
def test_class_asdasd_setter(instance):
...
@given(instance=Class_strategy)
def test_class_qweqwe_setter(instance):
