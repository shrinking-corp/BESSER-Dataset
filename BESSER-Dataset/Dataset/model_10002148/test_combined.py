...
def test_hyp_class_constructor_args():
    ...
    params = list(sig.parameters.keys())
    assert "asdasd" in params, "Missing parameter 'asdasd'"
    assert "qweqwe" in params, "Missing parameter 'qweqwe'"
...
def test_hyp_class_asdasd_setter(instance):
    original = instance.asdasd
    instance.asdasd = original
    assert instance.asdasd == original



@given(instance=Class_strategy)
def test_hyp_class_qweqwe_setter(instance):
    original = instance.qweqwe
    instance.qweqwe = original
    assert instance.qweqwe == original
...
def test_Class_asdasd_value_roundtrip():
    instance = Class(asdasd="sample_text", qweqwe="sample_text")
    assert instance.asdasd == "sample_text"
    instance.asdasd = "sample_text_2"
    assert instance.asdasd == "sample_text_2"


def test_Class_qweqwe_value_roundtrip():
    instance = Class(asdasd="sample_text", qweqwe="sample_text")
    assert instance.qweqwe == "sample_text"
    instance.qweqwe = "sample_text_2"
    assert instance.qweqwe == "sample_text_2"
...
def test_assoc_ClassC_ClassB_link_reassign_clear():
    a = ClassC(packageAttribute="sample_text", privateAttribute=7, protectedAttribute="sample_text", publicAttribute=3.14)
    b1 = ClassB()
    b2 = ClassB()
    _safe_set(a, 'classB6', b1)
    assert _is_linked(a, 'classB6', b1)
    if hasattr(b1, 'classC7'):
        assert _is_linked(b1, 'classC7', a)
    _safe_set(a, 'classB6', b2)
    assert _is_linked(a, 'classB6', b2)
    if hasattr(b1, 'classC7'):
        assert not _is_linked(b1, 'classC7', a)
    if hasattr(b2, 'classC7'):
        assert _is_linked(b2, 'classC7', a)
    _safe_set(a, 'classB6', None)
    assert not _is_linked(a, 'classB6', b2)
    if hasattr(b2, 'classC7'):
        assert not _is_linked(b2, 'classC7', a)
...
Class_strategy = st.builds(Class, asdasd=safe_text, qweqwe=safe_text)
@given(instance=Class_strategy)
@settings(max_examples=25)
