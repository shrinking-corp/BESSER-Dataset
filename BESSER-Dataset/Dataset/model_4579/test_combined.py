...
def test_hyp_dsl_metadata_is_not_abstract():
    assert not inspect.isabstract(dsl_Metadata)


def test_hyp_dsl_metadata_constructor_exists():
    assert callable(dsl_Metadata.__init__)


def test_hyp_dsl_metadata_constructor_args():
    sig = inspect.signature(dsl_Metadata.__init__)
    params = list(sig.parameters.keys())



def test_hyp_dsl_servicemetadata_is_not_abstract():
    assert not inspect.isabstract(dsl_ServiceMetaData)


def test_hyp_dsl_servicemetadata_constructor_exists():
    assert callable(dsl_ServiceMetaData.__init__)


def test_hyp_dsl_servicemetadata_constructor_args():
...
def test_hyp_dsl_servicemetadata_serviceID_setter(instance):
