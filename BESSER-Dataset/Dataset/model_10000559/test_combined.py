...
def test_assoc_downwardBound_Floor_Passenger_4_link_reassign_clear():
    ...
    if hasattr(b1, 'downwardBound11'):
        assert _is_linked(b1, 'downwardBound11', a)
    _safe_set(a, 'floor10', b2)
    assert _is_linked(a, 'floor10', b2)
    if hasattr(b1, 'downwardBound11'):
        assert not _is_linked(b1, 'downwardBound11', a)
    if hasattr(b2, 'downwardBound11'):
        assert _is_linked(b2, 'downwardBound11', a)
    _safe_set(a, 'floor10', None)
    assert not _is_linked(a, 'floor10', b2)
    if hasattr(b2, 'downwardBound11'):
        assert not _is_linked(b2, 'downwardBound11', a)
