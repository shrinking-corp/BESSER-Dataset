...
def test_dbrouting_documentroot_is_not_abstract():
    assert not inspect.isabstract(dbrouting_DocumentRoot)


def test_dbrouting_documentroot_constructor_exists():
    assert callable(dbrouting_DocumentRoot.__init__)


def test_dbrouting_documentroot_constructor_args():
    sig = inspect.signature(dbrouting_DocumentRoot.__init__)
    params = list(sig.parameters.keys())
    assert "mixed" in params, "Missing parameter 'mixed'"

def test_dbrouting_documentroot_has_mixed():
    assert hasattr(dbrouting_DocumentRoot, "mixed")
    descriptor = None
    for klass in dbrouting_DocumentRoot.__mro__:
...
@given(instance=dbrouting_DocumentRoot_strategy)
def test_dbrouting_documentroot_mixed_setter(instance):
