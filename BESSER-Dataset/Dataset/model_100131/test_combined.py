...
def test_hyp_dbrouting_documentroot_is_not_abstract():
    assert not inspect.isabstract(dbrouting_DocumentRoot)


def test_hyp_dbrouting_documentroot_constructor_exists():
    assert callable(dbrouting_DocumentRoot.__init__)


def test_hyp_dbrouting_documentroot_constructor_args():
    sig = inspect.signature(dbrouting_DocumentRoot.__init__)
    params = list(sig.parameters.keys())
...
dbrouting_DocumentRoot_strategy = st.builds(
    dbrouting_DocumentRoot,
    mixed=
        safe_text
)
...
@given(instance=dbrouting_DocumentRoot_strategy)
def test_hyp_dbrouting_documentroot_mixed_setter(instance):
...
def test_dbrouting_DocumentRoot_mixed_value_roundtrip():
    instance = dbrouting_DocumentRoot(mixed="sample_text")
...
def test_assoc_executor4_link_reassign_clear():
    ...
    b1 = dbrouting_DocumentRoot(mixed="sample_text")
    b2 = dbrouting_DocumentRoot(mixed="sample_text_2")
    _safe_set(a, 'dbrouting_Executor', b1)
    assert _is_linked(a, 'dbrouting_Executor', b1)
    if hasattr(b1, 'dbrouting_DocumentRoot5'):
        assert _is_linked(b1, 'dbrouting_DocumentRoot5', a)
    _safe_set(a, 'dbrouting_Executor', b2)
    assert _is_linked(a, 'dbrouting_Executor', b2)
    if hasattr(b1, 'dbrouting_DocumentRoot5'):
        assert not _is_linked(b1, 'dbrouting_DocumentRoot5', a)
    if hasattr(b2, 'dbrouting_DocumentRoot5'):
        assert _is_linked(b2, 'dbrouting_DocumentRoot5', a)
    _safe_set(a, 'dbrouting_Executor', None)
    assert not _is_linked(a, 'dbrouting_Executor', b2)
    if hasattr(b2, 'dbrouting_DocumentRoot5'):
        assert not _is_linked(b2, 'dbrouting_DocumentRoot5', a)
...
def test_assoc_resultSetRowSelector6_link_reassign_clear():
    ...
    if hasattr(b1, 'dbrouting_DocumentRoot7'):
        assert _is_linked(b1, 'dbrouting_DocumentRoot7', a)
    _safe_set(a, 'dbrouting_ResultSetRowSelector', b2)
    assert _is_linked(a, 'dbrouting_ResultSetRowSelector', b2)
    if hasattr(b1, 'dbrouting_DocumentRoot7'):
        assert not _is_linked(b1, 'dbrouting_DocumentRoot7', a)
    if hasattr(b2, 'dbrouting_DocumentRoot7'):
        assert _is_linked(b2, 'dbrouting_DocumentRoot7', a)
    _safe_set(a, 'dbrouting_ResultSetRowSelector', None)
    assert not _is_linked(a, 'dbrouting_ResultSetRowSelector', b2)
    if hasattr(b2, 'dbrouting_DocumentRoot7'):
        assert not _is_linked(b2, 'dbrouting_DocumentRoot7', a)


def test_assoc_xMLNSPrefixMap0_link_reassign_clear():
    a = dbrouting_DocumentRoot(mixed="sample_text")
    b1 = dbrouting_EStringToStringMapEntry()
    b2 = dbrouting_EStringToStringMapEntry()
    _safe_set(a, 'dbrouting_DocumentRoot', {b1})
    assert _is_linked(a, 'dbrouting_DocumentRoot', b1)
    if hasattr(b1, 'dbrouting_EStringToStringMapEntry'):
        assert _is_linked(b1, 'dbrouting_EStringToStringMapEntry', a)
    _safe_set(a, 'dbrouting_DocumentRoot', {b2})
    assert _is_linked(a, 'dbrouting_DocumentRoot', b2)
    if hasattr(b1, 'dbrouting_EStringToStringMapEntry'):
    ...
    _safe_set(a, 'dbrouting_DocumentRoot', set())
    assert not _is_linked(a, 'dbrouting_DocumentRoot', b2)
    if hasattr(b2, 'dbrouting_EStringToStringMapEntry'):
...
def test_assoc_xSISchemaLocation1_link_reassign_clear():
    ...
    b1 = dbrouting_EStringToStringMapEntry()
    b2 = dbrouting_EStringToStringMapEntry()
    _safe_set(a, 'dbrouting_DocumentRoot2', {b1})
    assert _is_linked(a, 'dbrouting_DocumentRoot2', b1)
    if hasattr(b1, 'dbrouting_EStringToStringMapEntry3'):
        assert _is_linked(b1, 'dbrouting_EStringToStringMapEntry3', a)
    _safe_set(a, 'dbrouting_DocumentRoot2', {b2})
    assert _is_linked(a, 'dbrouting_DocumentRoot2', b2)
    if hasattr(b1, 'dbrouting_EStringToStringMapEntry3'):
    ...
    _safe_set(a, 'dbrouting_DocumentRoot2', set())
    assert not _is_linked(a, 'dbrouting_DocumentRoot2', b2)
    if hasattr(b2, 'dbrouting_EStringToStringMapEntry3'):
...
dbrouting_DocumentRoot_strategy = st.builds(dbrouting_DocumentRoot, mixed=safe_text)
@given(instance=dbrouting_DocumentRoot_strategy)
@settings(max_examples=25)
def test_dbrouting_DocumentRoot_instantiation(instance):
    assert isinstance(instance, dbrouting_DocumentRoot)
