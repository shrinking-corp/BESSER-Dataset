...
def test_powered_vechile_is_not_abstract():
    assert not inspect.isabstract(powered_vechile)


def test_powered_vechile_constructor_exists():
    assert callable(powered_vechile.__init__)


def test_powered_vechile_constructor_args():
    sig = inspect.signature(powered_vechile.__init__)
    params = list(sig.parameters.keys())



def test_engine_interface_is_not_abstract():
    assert not inspect.isabstract(Engine_Interface)


def test_engine_interface_constructor_exists():
    assert callable(Engine_Interface.__init__)


def test_engine_interface_constructor_args():
    sig = inspect.signature(Engine_Interface.__init__)
    params = list(sig.parameters.keys())
...
powered_vechile_strategy = st.builds(
    powered_vechile,
)
Engine_Interface_strategy = st.builds(
    Engine_Interface,
)
...
@given(instance=powered_vechile_strategy)
@settings(max_examples=50)
def test_powered_vechile_instantiation(instance):
    assert isinstance(instance, powered_vechile)

@given(instance=Engine_Interface_strategy)
@settings(max_examples=50)
def test_engine_interface_instantiation(instance):
    assert isinstance(instance, Engine_Interface)
