...
def test_assoc_courses22_link_reassign_clear():
    ...
    if hasattr(b1, 'Course23'):
        assert _is_linked(b1, 'Course23', a)
    _safe_set(a, 'teacher', {b2})
    assert _is_linked(a, 'teacher', b2)
    if hasattr(b1, 'Course23'):
        assert not _is_linked(b1, 'Course23', a)
    if hasattr(b2, 'Course23'):
        assert _is_linked(b2, 'Course23', a)
    _safe_set(a, 'teacher', set())
    assert not _is_linked(a, 'teacher', b2)
    if hasattr(b2, 'Course23'):
        assert not _is_linked(b2, 'Course23', a)
...
def test_assoc_homeroomTeacher17_link_reassign_clear():
    ...
    if hasattr(b1, 'school_SchoolClass'):
        assert _is_linked(b1, 'school_SchoolClass', a)
    _safe_set(a, 'school_Teacher', b2)
    assert _is_linked(a, 'school_Teacher', b2)
    if hasattr(b1, 'school_SchoolClass'):
        assert not _is_linked(b1, 'school_SchoolClass', a)
    if hasattr(b2, 'school_SchoolClass'):
        assert _is_linked(b2, 'school_SchoolClass', a)
    _safe_set(a, 'school_Teacher', None)
    assert not _is_linked(a, 'school_Teacher', b2)
    if hasattr(b2, 'school_SchoolClass'):
        assert not _is_linked(b2, 'school_SchoolClass', a)


def test_assoc_homeroomedClass24_link_reassign_clear():
    a = school_Teacher(name="sample_text")
    b1 = school_SchoolClass(code="sample_text")
    b2 = school_SchoolClass(code="sample_text_2")
    _safe_set(a, 'school_Teacher25', b1)
    assert _is_linked(a, 'school_Teacher25', b1)
    if hasattr(b1, 'school_SchoolClass26'):
        assert _is_linked(b1, 'school_SchoolClass26', a)
    _safe_set(a, 'school_Teacher25', b2)
    assert _is_linked(a, 'school_Teacher25', b2)
    if hasattr(b1, 'school_SchoolClass26'):
        assert not _is_linked(b1, 'school_SchoolClass26', a)
    if hasattr(b2, 'school_SchoolClass26'):
        assert _is_linked(b2, 'school_SchoolClass26', a)
    _safe_set(a, 'school_Teacher25', None)
    assert not _is_linked(a, 'school_Teacher25', b2)
    if hasattr(b2, 'school_SchoolClass26'):
        assert not _is_linked(b2, 'school_SchoolClass26', a)
...
def test_assoc_school20_link_reassign_clear():
    a = school_Teacher(name="sample_text")
    ...
    if hasattr(b1, 'School21'):
        assert _is_linked(b1, 'School21', a)
    _safe_set(a, 'teachers', b2)
    assert _is_linked(a, 'teachers', b2)
    if hasattr(b1, 'School21'):
        assert not _is_linked(b1, 'School21', a)
    if hasattr(b2, 'School21'):
        assert _is_linked(b2, 'School21', a)
    _safe_set(a, 'teachers', None)
    assert not _is_linked(a, 'teachers', b2)
    if hasattr(b2, 'School21'):
        assert not _is_linked(b2, 'School21', a)


def test_assoc_school27_link_reassign_clear():
    a = school_Year(startingDate=7)
    b1 = school_School(address="sample_text", name="sample_text")
    b2 = school_School(address="sample_text_2", name="sample_text_2")
    _safe_set(a, 'years', b1)
    assert _is_linked(a, 'years', b1)
    if hasattr(b1, 'School28'):
        assert _is_linked(b1, 'School28', a)
    _safe_set(a, 'years', b2)
    assert _is_linked(a, 'years', b2)
    if hasattr(b1, 'School28'):
        assert not _is_linked(b1, 'School28', a)
    if hasattr(b2, 'School28'):
        assert _is_linked(b2, 'School28', a)
    _safe_set(a, 'years', None)
    assert not _is_linked(a, 'years', b2)
    if hasattr(b2, 'School28'):
        assert not _is_linked(b2, 'School28', a)


def test_assoc_schoolClass18_link_reassign_clear():
    a = school_Student(name="sample_text")
    b1 = school_SchoolClass(code="sample_text")
    b2 = school_SchoolClass(code="sample_text_2")
    _safe_set(a, 'students', b1)
    assert _is_linked(a, 'students', b1)
    if hasattr(b1, 'SchoolClass19'):
        assert _is_linked(b1, 'SchoolClass19', a)
    _safe_set(a, 'students', b2)
    assert _is_linked(a, 'students', b2)
    if hasattr(b1, 'SchoolClass19'):
        assert not _is_linked(b1, 'SchoolClass19', a)
    if hasattr(b2, 'SchoolClass19'):
        assert _is_linked(b2, 'SchoolClass19', a)
    _safe_set(a, 'students', None)
    assert not _is_linked(a, 'students', b2)
    if hasattr(b2, 'SchoolClass19'):
        assert not _is_linked(b2, 'SchoolClass19', a)
...
def test_assoc_schoolClasses29_link_reassign_clear():
    a = school_Year(startingDate=7)
    b1 = school_SchoolClass(code="sample_text")
    b2 = school_SchoolClass(code="sample_text_2")
    _safe_set(a, 'year', {b1})
