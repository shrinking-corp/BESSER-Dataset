...
def test_hyp_gmfgraph_figurehandle_is_not_abstract():
    assert not inspect.isabstract(gmfgraph_FigureHandle)


def test_hyp_gmfgraph_figurehandle_constructor_exists():
    assert callable(gmfgraph_FigureHandle.__init__)


def test_hyp_gmfgraph_figurehandle_constructor_args():
...
def test_hyp_figurehandle_is_not_abstract():
    assert not inspect.isabstract(FigureHandle)


def test_hyp_figurehandle_constructor_exists():
    assert callable(FigureHandle.__init__)


def test_hyp_figurehandle_constructor_args():
...
def test_hyp_figuremarker_is_not_abstract():
    assert not inspect.isabstract(FigureMarker)


def test_hyp_figuremarker_constructor_exists():
    assert callable(FigureMarker.__init__)


def test_hyp_figuremarker_constructor_args():
...
def test_hyp_gmfgraph_figuremarker_is_not_abstract():
    assert not inspect.isabstract(gmfgraph_FigureMarker)


def test_hyp_gmfgraph_figuremarker_constructor_exists():
    assert callable(gmfgraph_FigureMarker.__init__)


def test_hyp_gmfgraph_figuremarker_constructor_args():
...
def test_hyp_gmfgraph_diagramelement_find_changes_state(instance):
...
def test_hyp_gmfgraph_customclass_bundleName_setter(instance):
...
def test_gmfgraph_CustomClass_bundleName_value_roundtrip():
    instance = gmfgraph_CustomClass(bundleName="sample_text", qualifiedClassName="sample_text")
...
def test_gmfgraph_DiagramLabel_elementIcon_value_roundtrip():
    instance = gmfgraph_DiagramLabel(elementIcon=True)
...
def test_gmfgraph_Node_isa_DiagramElement():
    instance = gmfgraph_Node(affixedParentSide="sample_text", resizeConstraint="sample_text")
    assert isinstance(instance, DiagramElement)


def test_gmfgraph_ConnectionFigure_isa_Figure():
    instance = gmfgraph_ConnectionFigure()
    assert isinstance(instance, Figure)


def test_gmfgraph_CustomFigure_isa_Figure():
    instance = gmfgraph_CustomFigure()
    assert isinstance(instance, Figure)


def test_gmfgraph_DecorationFigure_isa_Figure():
    instance = gmfgraph_DecorationFigure()
    assert isinstance(instance, Figure)


def test_gmfgraph_Label_isa_Figure():
    instance = gmfgraph_Label(text="sample_text")
    assert isinstance(instance, Figure)


def test_gmfgraph_LabeledContainer_isa_Figure():
    instance = gmfgraph_LabeledContainer()
    assert isinstance(instance, Figure)


def test_gmfgraph_Shape_isa_Figure():
    instance = gmfgraph_Shape(fill=True, lineKind="sample_text", lineWidth=7, outline=True, xorFill=True, xorOutline=True)
    assert isinstance(instance, Figure)


def test_gmfgraph_Figure_isa_FigureHandle():
    instance = gmfgraph_Figure()
    assert isinstance(instance, FigureHandle)


def test_gmfgraph_FigureAccessor_isa_FigureHandle():
    instance = gmfgraph_FigureAccessor(accessor="sample_text")
    assert isinstance(instance, FigureHandle)


def test_gmfgraph_Figure_isa_FigureMarker():
    instance = gmfgraph_Figure()
    assert isinstance(instance, FigureMarker)


def test_gmfgraph_FigureRef_isa_FigureMarker():
...
def test_gmfgraph_Figure_isa_Identity():
    instance = gmfgraph_Figure()
    assert isinstance(instance, Identity)
...
def test_gmfgraph_FigureMarker_isa_Layoutable():
    instance = gmfgraph_FigureMarker()
...
def test_assoc_attributes58_link_reassign_clear():
    a = gmfgraph_CustomClass(bundleName="sample_text", qualifiedClassName="sample_text")
...
def test_assoc_color63_link_reassign_clear():
    a = gmfgraph_LineBorder(width=7)
    b1 = gmfgraph_Color()
    b2 = gmfgraph_Color()
    _safe_set(a, 'gmfgraph_LineBorder', b1)
    assert _is_linked(a, 'gmfgraph_LineBorder', b1)
    if hasattr(b1, 'gmfgraph_Color64'):
        assert _is_linked(b1, 'gmfgraph_Color64', a)
    _safe_set(a, 'gmfgraph_LineBorder', b2)
    assert _is_linked(a, 'gmfgraph_LineBorder', b2)
    if hasattr(b1, 'gmfgraph_Color64'):
        assert not _is_linked(b1, 'gmfgraph_Color64', a)
    if hasattr(b2, 'gmfgraph_Color64'):
        assert _is_linked(b2, 'gmfgraph_Color64', a)
    _safe_set(a, 'gmfgraph_LineBorder', None)
    assert not _is_linked(a, 'gmfgraph_LineBorder', b2)
    if hasattr(b2, 'gmfgraph_Color64'):
        assert not _is_linked(b2, 'gmfgraph_Color64', a)
...
def test_assoc_customChildren60_link_reassign_clear():
    a = gmfgraph_FigureAccessor(accessor="sample_text")
    b1 = gmfgraph_CustomFigure()
    b2 = gmfgraph_CustomFigure()
    _safe_set(a, 'gmfgraph_FigureAccessor62', b1)
    assert _is_linked(a, 'gmfgraph_FigureAccessor62', b1)
    if hasattr(b1, 'gmfgraph_CustomFigure61'):
        assert _is_linked(b1, 'gmfgraph_CustomFigure61', a)
    _safe_set(a, 'gmfgraph_FigureAccessor62', b2)
    assert _is_linked(a, 'gmfgraph_FigureAccessor62', b2)
    if hasattr(b1, 'gmfgraph_CustomFigure61'):
        assert not _is_linked(b1, 'gmfgraph_CustomFigure61', a)
    if hasattr(b2, 'gmfgraph_CustomFigure61'):
        assert _is_linked(b2, 'gmfgraph_CustomFigure61', a)
    _safe_set(a, 'gmfgraph_FigureAccessor62', None)
    assert not _is_linked(a, 'gmfgraph_FigureAccessor62', b2)
    if hasattr(b2, 'gmfgraph_CustomFigure61'):
        assert not _is_linked(b2, 'gmfgraph_CustomFigure61', a)


def test_assoc_defaultSize19_link_reassign_clear():
...
def test_assoc_facets12_link_reassign_clear():
    a = gmfgraph_DiagramElement()
    b1 = gmfgraph_VisualFacet()
    b2 = gmfgraph_VisualFacet()
    _safe_set(a, 'gmfgraph_DiagramElement', {b1})
    assert _is_linked(a, 'gmfgraph_DiagramElement', b1)
    if hasattr(b1, 'gmfgraph_VisualFacet'):
        assert _is_linked(b1, 'gmfgraph_VisualFacet', a)
    _safe_set(a, 'gmfgraph_DiagramElement', {b2})
    assert _is_linked(a, 'gmfgraph_DiagramElement', b2)
    if hasattr(b1, 'gmfgraph_VisualFacet'):
        assert not _is_linked(b1, 'gmfgraph_VisualFacet', a)
    if hasattr(b2, 'gmfgraph_VisualFacet'):
        assert _is_linked(b2, 'gmfgraph_VisualFacet', a)
    _safe_set(a, 'gmfgraph_DiagramElement', set())
    assert not _is_linked(a, 'gmfgraph_DiagramElement', b2)
    if hasattr(b2, 'gmfgraph_VisualFacet'):
        assert not _is_linked(b2, 'gmfgraph_VisualFacet', a)


def test_assoc_figure11_link_reassign_clear():
    a = gmfgraph_DiagramElement()
    b1 = gmfgraph_FigureHandle()
    b2 = gmfgraph_FigureHandle()
    _safe_set(a, 'referencingElements', b1)
    assert _is_linked(a, 'referencingElements', b1)
    if hasattr(b1, 'FigureHandle'):
        assert _is_linked(b1, 'FigureHandle', a)
    _safe_set(a, 'referencingElements', b2)
    assert _is_linked(a, 'referencingElements', b2)
    if hasattr(b1, 'FigureHandle'):
        assert not _is_linked(b1, 'FigureHandle', a)
    if hasattr(b2, 'FigureHandle'):
        assert _is_linked(b2, 'FigureHandle', a)
    _safe_set(a, 'referencingElements', None)
    assert not _is_linked(a, 'referencingElements', b2)
    if hasattr(b2, 'FigureHandle'):
        assert not _is_linked(b2, 'FigureHandle', a)
...
def test_assoc_figures9_link_reassign_clear():
    ...
    b1 = gmfgraph_Figure()
    b2 = gmfgraph_Figure()
    _safe_set(a, 'gmfgraph_FigureGallery10', {b1})
    assert _is_linked(a, 'gmfgraph_FigureGallery10', b1)
    if hasattr(b1, 'gmfgraph_Figure'):
        assert _is_linked(b1, 'gmfgraph_Figure', a)
    _safe_set(a, 'gmfgraph_FigureGallery10', {b2})
    assert _is_linked(a, 'gmfgraph_FigureGallery10', b2)
    if hasattr(b1, 'gmfgraph_Figure'):
        assert not _is_linked(b1, 'gmfgraph_Figure', a)
    if hasattr(b2, 'gmfgraph_Figure'):
        assert _is_linked(b2, 'gmfgraph_Figure', a)
    _safe_set(a, 'gmfgraph_FigureGallery10', set())
    assert not _is_linked(a, 'gmfgraph_FigureGallery10', b2)
    if hasattr(b2, 'gmfgraph_Figure'):
        assert not _is_linked(b2, 'gmfgraph_Figure', a)


def test_assoc_insets39_link_reassign_clear():
...
def test_assoc_insets65_link_reassign_clear():
    a = gmfgraph_Insets(bottom=7, left=7, right=7, top=7)
    b1 = gmfgraph_MarginBorder()
    b2 = gmfgraph_MarginBorder()
    _safe_set(a, 'gmfgraph_Insets66', b1)
    assert _is_linked(a, 'gmfgraph_Insets66', b1)
    if hasattr(b1, 'gmfgraph_MarginBorder'):
        assert _is_linked(b1, 'gmfgraph_MarginBorder', a)
    _safe_set(a, 'gmfgraph_Insets66', b2)
    assert _is_linked(a, 'gmfgraph_Insets66', b2)
    if hasattr(b1, 'gmfgraph_MarginBorder'):
    ...
    _safe_set(a, 'gmfgraph_Insets66', None)
    assert not _is_linked(a, 'gmfgraph_Insets66', b2)
    if hasattr(b2, 'gmfgraph_MarginBorder'):
        assert not _is_linked(b2, 'gmfgraph_MarginBorder', a)


def test_assoc_labels7_link_reassign_clear():
    a = gmfgraph_DiagramLabel(elementIcon=True)
    b1 = gmfgraph_Canvas()
    b2 = gmfgraph_Canvas()
...
def test_assoc_location43_link_reassign_clear():
...
def test_assoc_margins77_link_reassign_clear():
    ...
    if hasattr(b1, 'gmfgraph_Dimension78'):
        assert _is_linked(b1, 'gmfgraph_Dimension78', a)
    _safe_set(a, 'gmfgraph_GridLayout', b2)
    assert _is_linked(a, 'gmfgraph_GridLayout', b2)
    if hasattr(b1, 'gmfgraph_Dimension78'):
        assert not _is_linked(b1, 'gmfgraph_Dimension78', a)
    if hasattr(b2, 'gmfgraph_Dimension78'):
        assert _is_linked(b2, 'gmfgraph_Dimension78', a)
    _safe_set(a, 'gmfgraph_GridLayout', None)
    assert not _is_linked(a, 'gmfgraph_GridLayout', b2)
    if hasattr(b2, 'gmfgraph_Dimension78'):
        assert not _is_linked(b2, 'gmfgraph_Dimension78', a)


def test_assoc_maximumSize28_link_reassign_clear():
    a = gmfgraph_Dimension(dx=7, dy=7)
    b1 = gmfgraph_Figure()
    b2 = gmfgraph_Figure()
    _safe_set(a, 'gmfgraph_Dimension30', b1)
    assert _is_linked(a, 'gmfgraph_Dimension30', b1)
    if hasattr(b1, 'gmfgraph_Figure29'):
        assert _is_linked(b1, 'gmfgraph_Figure29', a)
    _safe_set(a, 'gmfgraph_Dimension30', b2)
    assert _is_linked(a, 'gmfgraph_Dimension30', b2)
    if hasattr(b1, 'gmfgraph_Figure29'):
        assert not _is_linked(b1, 'gmfgraph_Figure29', a)
    if hasattr(b2, 'gmfgraph_Figure29'):
        assert _is_linked(b2, 'gmfgraph_Figure29', a)
    _safe_set(a, 'gmfgraph_Dimension30', None)
    assert not _is_linked(a, 'gmfgraph_Dimension30', b2)
    if hasattr(b2, 'gmfgraph_Figure29'):
        assert not _is_linked(b2, 'gmfgraph_Figure29', a)


def test_assoc_minimumSize31_link_reassign_clear():
    a = gmfgraph_Dimension(dx=7, dy=7)
    b1 = gmfgraph_Figure()
    b2 = gmfgraph_Figure()
    _safe_set(a, 'gmfgraph_Dimension33', b1)
    assert _is_linked(a, 'gmfgraph_Dimension33', b1)
    if hasattr(b1, 'gmfgraph_Figure32'):
        assert _is_linked(b1, 'gmfgraph_Figure32', a)
    _safe_set(a, 'gmfgraph_Dimension33', b2)
    assert _is_linked(a, 'gmfgraph_Dimension33', b2)
    if hasattr(b1, 'gmfgraph_Figure32'):
        assert not _is_linked(b1, 'gmfgraph_Figure32', a)
    if hasattr(b2, 'gmfgraph_Figure32'):
        assert _is_linked(b2, 'gmfgraph_Figure32', a)
    _safe_set(a, 'gmfgraph_Dimension33', None)
    assert not _is_linked(a, 'gmfgraph_Dimension33', b2)
    if hasattr(b2, 'gmfgraph_Figure32'):
        assert not _is_linked(b2, 'gmfgraph_Figure32', a)


def test_assoc_nodeFigure13_link_reassign_clear():
    a = gmfgraph_Node(affixedParentSide="sample_text", resizeConstraint="sample_text")
    b1 = gmfgraph_Figure()
    b2 = gmfgraph_Figure()
    _safe_set(a, 'gmfgraph_Node14', b1)
    assert _is_linked(a, 'gmfgraph_Node14', b1)
    if hasattr(b1, 'gmfgraph_Figure15'):
        assert _is_linked(b1, 'gmfgraph_Figure15', a)
    _safe_set(a, 'gmfgraph_Node14', b2)
    assert _is_linked(a, 'gmfgraph_Node14', b2)
    if hasattr(b1, 'gmfgraph_Figure15'):
        assert not _is_linked(b1, 'gmfgraph_Figure15', a)
    if hasattr(b2, 'gmfgraph_Figure15'):
        assert _is_linked(b2, 'gmfgraph_Figure15', a)
    _safe_set(a, 'gmfgraph_Node14', None)
    assert not _is_linked(a, 'gmfgraph_Node14', b2)
    if hasattr(b2, 'gmfgraph_Figure15'):
        assert not _is_linked(b2, 'gmfgraph_Figure15', a)
...
def test_assoc_preferredSize34_link_reassign_clear():
    a = gmfgraph_Dimension(dx=7, dy=7)
    b1 = gmfgraph_Figure()
    b2 = gmfgraph_Figure()
    _safe_set(a, 'gmfgraph_Dimension36', b1)
    assert _is_linked(a, 'gmfgraph_Dimension36', b1)
    if hasattr(b1, 'gmfgraph_Figure35'):
        assert _is_linked(b1, 'gmfgraph_Figure35', a)
    _safe_set(a, 'gmfgraph_Dimension36', b2)
    assert _is_linked(a, 'gmfgraph_Dimension36', b2)
    if hasattr(b1, 'gmfgraph_Figure35'):
        assert not _is_linked(b1, 'gmfgraph_Figure35', a)
    if hasattr(b2, 'gmfgraph_Figure35'):
        assert _is_linked(b2, 'gmfgraph_Figure35', a)
    _safe_set(a, 'gmfgraph_Dimension36', None)
    assert not _is_linked(a, 'gmfgraph_Dimension36', b2)
    if hasattr(b2, 'gmfgraph_Figure35'):
        assert not _is_linked(b2, 'gmfgraph_Figure35', a)


def test_assoc_referencingElements21_link_reassign_clear():
    a = gmfgraph_DiagramElement()
    b1 = gmfgraph_FigureHandle()
    b2 = gmfgraph_FigureHandle()
    _safe_set(a, 'DiagramElement', b1)
    assert _is_linked(a, 'DiagramElement', b1)
    if hasattr(b1, 'figure'):
        assert _is_linked(b1, 'figure', a)
    _safe_set(a, 'DiagramElement', b2)
    assert _is_linked(a, 'DiagramElement', b2)
    if hasattr(b1, 'figure'):
        assert not _is_linked(b1, 'figure', a)
    if hasattr(b2, 'figure'):
        assert _is_linked(b2, 'figure', a)
    _safe_set(a, 'DiagramElement', None)
    assert not _is_linked(a, 'DiagramElement', b2)
    if hasattr(b2, 'figure'):
        assert not _is_linked(b2, 'figure', a)


def test_assoc_resolvedChildren50_link_reassign_clear():
    ...
    if hasattr(b1, 'gmfgraph_Figure51'):
        assert _is_linked(b1, 'gmfgraph_Figure51', a)
    _safe_set(a, 'gmfgraph_Shape', {b2})
    assert _is_linked(a, 'gmfgraph_Shape', b2)
    if hasattr(b1, 'gmfgraph_Figure51'):
        assert not _is_linked(b1, 'gmfgraph_Figure51', a)
    if hasattr(b2, 'gmfgraph_Figure51'):
        assert _is_linked(b2, 'gmfgraph_Figure51', a)
    _safe_set(a, 'gmfgraph_Shape', set())
    assert not _is_linked(a, 'gmfgraph_Shape', b2)
    if hasattr(b2, 'gmfgraph_Figure51'):
        assert not _is_linked(b2, 'gmfgraph_Figure51', a)


def test_assoc_size45_link_reassign_clear():
    a = gmfgraph_Point(x=7, y=7)
    b1 = gmfgraph_Figure()
    b2 = gmfgraph_Figure()
    _safe_set(a, 'gmfgraph_Point47', b1)
    assert _is_linked(a, 'gmfgraph_Point47', b1)
    if hasattr(b1, 'gmfgraph_Figure46'):
        assert _is_linked(b1, 'gmfgraph_Figure46', a)
    _safe_set(a, 'gmfgraph_Point47', b2)
    assert _is_linked(a, 'gmfgraph_Point47', b2)
    if hasattr(b1, 'gmfgraph_Figure46'):
        assert not _is_linked(b1, 'gmfgraph_Figure46', a)
    if hasattr(b2, 'gmfgraph_Figure46'):
        assert _is_linked(b2, 'gmfgraph_Figure46', a)
    _safe_set(a, 'gmfgraph_Point47', None)
    assert not _is_linked(a, 'gmfgraph_Point47', b2)
    if hasattr(b2, 'gmfgraph_Figure46'):
        assert not _is_linked(b2, 'gmfgraph_Figure46', a)


def test_assoc_size86_link_reassign_clear():
    a = gmfgraph_Dimension(dx=7, dy=7)
    b1 = gmfgraph_XYLayoutData()
    b2 = gmfgraph_XYLayoutData()
    _safe_set(a, 'gmfgraph_Dimension88', b1)
    assert _is_linked(a, 'gmfgraph_Dimension88', b1)
    if hasattr(b1, 'gmfgraph_XYLayoutData87'):
        assert _is_linked(b1, 'gmfgraph_XYLayoutData87', a)
    _safe_set(a, 'gmfgraph_Dimension88', b2)
    assert _is_linked(a, 'gmfgraph_Dimension88', b2)
    if hasattr(b1, 'gmfgraph_XYLayoutData87'):
        assert not _is_linked(b1, 'gmfgraph_XYLayoutData87', a)
    if hasattr(b2, 'gmfgraph_XYLayoutData87'):
        assert _is_linked(b2, 'gmfgraph_XYLayoutData87', a)
    _safe_set(a, 'gmfgraph_Dimension88', None)
    assert not _is_linked(a, 'gmfgraph_Dimension88', b2)
    if hasattr(b2, 'gmfgraph_XYLayoutData87'):
        assert not _is_linked(b2, 'gmfgraph_XYLayoutData87', a)


def test_assoc_sizeHint73_link_reassign_clear():
    ...
    if hasattr(b1, 'gmfgraph_Dimension74'):
        assert _is_linked(b1, 'gmfgraph_Dimension74', a)
    _safe_set(a, 'gmfgraph_GridLayoutData', b2)
    assert _is_linked(a, 'gmfgraph_GridLayoutData', b2)
    if hasattr(b1, 'gmfgraph_Dimension74'):
        assert not _is_linked(b1, 'gmfgraph_Dimension74', a)
    if hasattr(b2, 'gmfgraph_Dimension74'):
        assert _is_linked(b2, 'gmfgraph_Dimension74', a)
    _safe_set(a, 'gmfgraph_GridLayoutData', None)
    assert not _is_linked(a, 'gmfgraph_GridLayoutData', b2)
    if hasattr(b2, 'gmfgraph_Dimension74'):
        assert not _is_linked(b2, 'gmfgraph_Dimension74', a)


def test_assoc_spacing79_link_reassign_clear():
    a = gmfgraph_GridLayout(equalWidth=True, numColumns=7)
    b1 = gmfgraph_Dimension(dx=7, dy=7)
    b2 = gmfgraph_Dimension(dx=13, dy=13)
    _safe_set(a, 'gmfgraph_GridLayout80', b1)
    assert _is_linked(a, 'gmfgraph_GridLayout80', b1)
    if hasattr(b1, 'gmfgraph_Dimension81'):
        assert _is_linked(b1, 'gmfgraph_Dimension81', a)
    _safe_set(a, 'gmfgraph_GridLayout80', b2)
    assert _is_linked(a, 'gmfgraph_GridLayout80', b2)
    if hasattr(b1, 'gmfgraph_Dimension81'):
        assert not _is_linked(b1, 'gmfgraph_Dimension81', a)
    if hasattr(b2, 'gmfgraph_Dimension81'):
        assert _is_linked(b2, 'gmfgraph_Dimension81', a)
    _safe_set(a, 'gmfgraph_GridLayout80', None)
    assert not _is_linked(a, 'gmfgraph_GridLayout80', b2)
    if hasattr(b2, 'gmfgraph_Dimension81'):
        assert not _is_linked(b2, 'gmfgraph_Dimension81', a)


def test_assoc_spacing82_link_reassign_clear():
    a = gmfgraph_Dimension(dx=7, dy=7)
    b1 = gmfgraph_BorderLayout()
    b2 = gmfgraph_BorderLayout()
    _safe_set(a, 'gmfgraph_Dimension83', b1)
    assert _is_linked(a, 'gmfgraph_Dimension83', b1)
    if hasattr(b1, 'gmfgraph_BorderLayout'):
        assert _is_linked(b1, 'gmfgraph_BorderLayout', a)
    _safe_set(a, 'gmfgraph_Dimension83', b2)
    assert _is_linked(a, 'gmfgraph_Dimension83', b2)
    if hasattr(b1, 'gmfgraph_BorderLayout'):
    ...
    _safe_set(a, 'gmfgraph_Dimension83', None)
    assert not _is_linked(a, 'gmfgraph_Dimension83', b2)
    if hasattr(b2, 'gmfgraph_BorderLayout'):
        assert not _is_linked(b2, 'gmfgraph_BorderLayout', a)


def test_assoc_template52_link_reassign_clear():
    a = gmfgraph_Point(x=7, y=7)
    b1 = gmfgraph_Polyline()
    b2 = gmfgraph_Polyline()
    _safe_set(a, 'gmfgraph_Point53', b1)
    assert _is_linked(a, 'gmfgraph_Point53', b1)
    if hasattr(b1, 'gmfgraph_Polyline'):
        assert _is_linked(b1, 'gmfgraph_Polyline', a)
    _safe_set(a, 'gmfgraph_Point53', b2)
    assert _is_linked(a, 'gmfgraph_Point53', b2)
    if hasattr(b1, 'gmfgraph_Polyline'):
        assert not _is_linked(b1, 'gmfgraph_Polyline', a)
    if hasattr(b2, 'gmfgraph_Polyline'):
        assert _is_linked(b2, 'gmfgraph_Polyline', a)
    _safe_set(a, 'gmfgraph_Point53', None)
    assert not _is_linked(a, 'gmfgraph_Point53', b2)
    if hasattr(b2, 'gmfgraph_Polyline'):
        assert not _is_linked(b2, 'gmfgraph_Polyline', a)


def test_assoc_topLeft84_link_reassign_clear():
    a = gmfgraph_Point(x=7, y=7)
    b1 = gmfgraph_XYLayoutData()
    b2 = gmfgraph_XYLayoutData()
    _safe_set(a, 'gmfgraph_Point85', b1)
    assert _is_linked(a, 'gmfgraph_Point85', b1)
    if hasattr(b1, 'gmfgraph_XYLayoutData'):
        assert _is_linked(b1, 'gmfgraph_XYLayoutData', a)
    _safe_set(a, 'gmfgraph_Point85', b2)
    assert _is_linked(a, 'gmfgraph_Point85', b2)
    if hasattr(b1, 'gmfgraph_XYLayoutData'):
    ...
    _safe_set(a, 'gmfgraph_Point85', None)
    assert not _is_linked(a, 'gmfgraph_Point85', b2)
    if hasattr(b2, 'gmfgraph_XYLayoutData'):
        assert not _is_linked(b2, 'gmfgraph_XYLayoutData', a)


def test_assoc_typedFigure59_link_reassign_clear():
    a = gmfgraph_FigureAccessor(accessor="sample_text")
    b1 = gmfgraph_CustomFigure()
    b2 = gmfgraph_CustomFigure()
    _safe_set(a, 'gmfgraph_FigureAccessor', b1)
    assert _is_linked(a, 'gmfgraph_FigureAccessor', b1)
    if hasattr(b1, 'gmfgraph_CustomFigure'):
        assert _is_linked(b1, 'gmfgraph_CustomFigure', a)
    _safe_set(a, 'gmfgraph_FigureAccessor', b2)
    assert _is_linked(a, 'gmfgraph_FigureAccessor', b2)
    if hasattr(b1, 'gmfgraph_CustomFigure'):
    ...
    _safe_set(a, 'gmfgraph_FigureAccessor', None)
    assert not _is_linked(a, 'gmfgraph_FigureAccessor', b2)
    if hasattr(b2, 'gmfgraph_CustomFigure'):
...
FigureHandle_strategy = st.builds(FigureHandle)
...
FigureMarker_strategy = st.builds(FigureMarker)
...
gmfgraph_CustomClass_strategy = st.builds(gmfgraph_CustomClass, bundleName=safe_text, qualifiedClassName=safe_text)
@given(instance=gmfgraph_CustomClass_strategy)
@settings(max_examples=25)
...
gmfgraph_DiagramLabel_strategy = st.builds(gmfgraph_DiagramLabel, elementIcon=st.booleans())
@given(instance=gmfgraph_DiagramLabel_strategy)
@settings(max_examples=25)
...
gmfgraph_FigureHandle_strategy = st.builds(gmfgraph_FigureHandle)
...
gmfgraph_FigureMarker_strategy = st.builds(gmfgraph_FigureMarker)
