...
def test_hyp_adminpanel_is_not_abstract():
    assert not inspect.isabstract(AdminPanel)


def test_hyp_adminpanel_constructor_exists():
    assert callable(AdminPanel.__init__)


def test_hyp_adminpanel_constructor_args():
...
def test_hyp_adminpanel_UserLevel_setter(instance):
...
@given(instance=AdminPanel_strategy)
def test_hyp_adminpanel_UserName_setter(instance):
...
@given(instance=AdminPanel_strategy)
def test_hyp_adminpanel_UserID_setter(instance):
...
def test_AdminPanel_UserID_value_roundtrip():
    instance = AdminPanel(UserID=7, UserLevel=7, UserName="sample_text")
    assert instance.UserID == 7
    instance.UserID = 13
    assert instance.UserID == 13


def test_AdminPanel_UserLevel_value_roundtrip():
    instance = AdminPanel(UserID=7, UserLevel=7, UserName="sample_text")
...
def test_AdminPanel_UserName_value_roundtrip():
    instance = AdminPanel(UserID=7, UserLevel=7, UserName="sample_text")
...
def test_assoc_AdminPanel_Menu_link_reassign_clear():
    a = Menu(Availability=7, Category="sample_text", MenuItem="sample_text", Price=3.14)
    b1 = AdminPanel(UserID=7, UserLevel=7, UserName="sample_text")
    b2 = AdminPanel(UserID=13, UserLevel=13, UserName="sample_text_2")
    _safe_set(a, 'adminPanel9', b1)
    assert _is_linked(a, 'adminPanel9', b1)
    if hasattr(b1, 'menu8'):
        assert _is_linked(b1, 'menu8', a)
    _safe_set(a, 'adminPanel9', b2)
    assert _is_linked(a, 'adminPanel9', b2)
    if hasattr(b1, 'menu8'):
        assert not _is_linked(b1, 'menu8', a)
    if hasattr(b2, 'menu8'):
        assert _is_linked(b2, 'menu8', a)
    _safe_set(a, 'adminPanel9', None)
    assert not _is_linked(a, 'adminPanel9', b2)
    if hasattr(b2, 'menu8'):
        assert not _is_linked(b2, 'menu8', a)
...
def test_assoc_BookedTables_Table_link_reassign_clear():
    ...
    _safe_set(a, 'bookedTables17', {b1})
    assert _is_linked(a, 'bookedTables17', b1)
    if hasattr(b1, 'table16'):
        assert _is_linked(b1, 'table16', a)
    _safe_set(a, 'bookedTables17', {b2})
    assert _is_linked(a, 'bookedTables17', b2)
    if hasattr(b1, 'table16'):
        assert not _is_linked(b1, 'table16', a)
    if hasattr(b2, 'table16'):
        assert _is_linked(b2, 'table16', a)
    _safe_set(a, 'bookedTables17', set())
    assert not _is_linked(a, 'bookedTables17', b2)
    if hasattr(b2, 'table16'):
...
def test_assoc_OrderItem_Order_link_reassign_clear():
    ...
    if hasattr(b1, 'orderItem13'):
        assert _is_linked(b1, 'orderItem13', a)
    _safe_set(a, 'order12', b2)
    assert _is_linked(a, 'order12', b2)
    if hasattr(b1, 'orderItem13'):
        assert not _is_linked(b1, 'orderItem13', a)
    if hasattr(b2, 'orderItem13'):
        assert _is_linked(b2, 'orderItem13', a)
    _safe_set(a, 'order12', None)
    assert not _is_linked(a, 'order12', b2)
    if hasattr(b2, 'orderItem13'):
        assert not _is_linked(b2, 'orderItem13', a)
...
def test_assoc_Order_Users_link_reassign_clear():
    ...
    b1 = Order(Date=date(2024, 1, 1), DicountLvl=7, OrderID=7, Total=3.14, UserID=7)
    b2 = Order(Date=date(2025, 6, 15), DicountLvl=13, OrderID=13, Total=9.99, UserID=13)
    _safe_set(a, 'order15', {b1})
    assert _is_linked(a, 'order15', b1)
    if hasattr(b1, 'users14'):
        assert _is_linked(b1, 'users14', a)
    _safe_set(a, 'order15', {b2})
    assert _is_linked(a, 'order15', b2)
    if hasattr(b1, 'users14'):
        assert not _is_linked(b1, 'users14', a)
    if hasattr(b2, 'users14'):
        assert _is_linked(b2, 'users14', a)
    _safe_set(a, 'order15', set())
    assert not _is_linked(a, 'order15', b2)
    if hasattr(b2, 'users14'):
        assert not _is_linked(b2, 'users14', a)
...
def test_assoc_ViewOrder_OrderItem_link_reassign_clear():
    ...
    _safe_set(a, 'orderItem10', {b1})
    assert _is_linked(a, 'orderItem10', b1)
    if hasattr(b1, 'viewOrder11'):
        assert _is_linked(b1, 'viewOrder11', a)
    _safe_set(a, 'orderItem10', {b2})
    assert _is_linked(a, 'orderItem10', b2)
    if hasattr(b1, 'viewOrder11'):
        assert not _is_linked(b1, 'viewOrder11', a)
    if hasattr(b2, 'viewOrder11'):
        assert _is_linked(b2, 'viewOrder11', a)
    _safe_set(a, 'orderItem10', set())
    assert not _is_linked(a, 'orderItem10', b2)
    if hasattr(b2, 'viewOrder11'):
        assert not _is_linked(b2, 'viewOrder11', a)
...
AdminPanel_strategy = st.builds(AdminPanel, UserID=st.integers(), UserLevel=st.integers(), UserName=safe_text)
@given(instance=AdminPanel_strategy)
