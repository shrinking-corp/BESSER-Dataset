...
def test_artista_is_not_abstract():
    assert not inspect.isabstract(Artista)


def test_artista_constructor_exists():
    assert callable(Artista.__init__)


def test_artista_constructor_args():
    sig = inspect.signature(Artista.__init__)
...
def test_mecenas_is_not_abstract():
    assert not inspect.isabstract(Mecenas)


def test_mecenas_constructor_exists():
    assert callable(Mecenas.__init__)


def test_mecenas_constructor_args():
    sig = inspect.signature(Mecenas.__init__)
    params = list(sig.parameters.keys())
...
Mecenas_strategy = st.builds(
    Mecenas,
)

@given(instance=Artista_strategy)
@settings(max_examples=50)
def test_artista_instantiation(instance):
    assert isinstance(instance, Artista)
...
@given(instance=Mecenas_strategy)
@settings(max_examples=50)
def test_mecenas_instantiation(instance):
    assert isinstance(instance, Mecenas)
