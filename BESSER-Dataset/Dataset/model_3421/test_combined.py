...
def test_hyp_tracker_unappliedtags_is_not_abstract():
    assert not inspect.isabstract(tracker_UnAppliedTags)


def test_hyp_tracker_unappliedtags_constructor_exists():
    assert callable(tracker_UnAppliedTags.__init__)


def test_hyp_tracker_unappliedtags_constructor_args():
    sig = inspect.signature(tracker_UnAppliedTags.__init__)
    params = list(sig.parameters.keys())



def test_hyp_tracker_eventhistory_is_not_abstract():
    assert not inspect.isabstract(tracker_EventHistory)


def test_hyp_tracker_eventhistory_constructor_exists():
    assert callable(tracker_EventHistory.__init__)


def test_hyp_tracker_eventhistory_constructor_args():
    sig = inspect.signature(tracker_EventHistory.__init__)
    params = list(sig.parameters.keys())



def test_hyp_tracker_animals_is_not_abstract():
    assert not inspect.isabstract(tracker_Animals)


def test_hyp_tracker_animals_constructor_exists():
    assert callable(tracker_Animals.__init__)


def test_hyp_tracker_animals_constructor_args():
...
def test_hyp_tracker_animalid_is_not_abstract():
    assert not inspect.isabstract(tracker_AnimalId)


def test_hyp_tracker_animalid_constructor_exists():
    assert callable(tracker_AnimalId.__init__)


def test_hyp_tracker_animalid_constructor_args():
...
def test_hyp_tracker_animalid_usainNumberUsed_setter(instance):
...
@given(instance=tracker_AnimalId_strategy)
def test_hyp_tracker_animalid_idNumber_setter(instance):
...
def test_tracker_AnimalId_idNumber_value_roundtrip():
    instance = tracker_AnimalId(idNumber="sample_text", usainNumberUsed=True)
    assert instance.idNumber == "sample_text"
    instance.idNumber = "sample_text_2"
    assert instance.idNumber == "sample_text_2"


def test_tracker_AnimalId_usainNumberUsed_value_roundtrip():
    instance = tracker_AnimalId(idNumber="sample_text", usainNumberUsed=True)
...
def test_tracker_Event_comments_value_roundtrip():
    instance = tracker_Event(comments="sample_text", correction=True, dateTime="sample_text", electronicallyRead=True, eventCode=7, id="sample_text")
...
def test_assoc_ain0_link_reassign_clear():
    a = tracker_AnimalId(idNumber="sample_text", usainNumberUsed=True)
    ...
    _safe_set(a, 'tracker_AnimalId', b1)
    assert _is_linked(a, 'tracker_AnimalId', b1)
    if hasattr(b1, 'tracker_Animal'):
        assert _is_linked(b1, 'tracker_Animal', a)
    _safe_set(a, 'tracker_AnimalId', b2)
    assert _is_linked(a, 'tracker_AnimalId', b2)
    if hasattr(b1, 'tracker_Animal'):
        assert not _is_linked(b1, 'tracker_Animal', a)
    if hasattr(b2, 'tracker_Animal'):
        assert _is_linked(b2, 'tracker_Animal', a)
    _safe_set(a, 'tracker_AnimalId', None)
    assert not _is_linked(a, 'tracker_AnimalId', b2)
    if hasattr(b2, 'tracker_Animal'):
        assert not _is_linked(b2, 'tracker_Animal', a)


def test_assoc_ain1_link_reassign_clear():
    a = tracker_Event(comments="sample_text", correction=True, dateTime="sample_text", electronicallyRead=True, eventCode=7, id="sample_text")
    b1 = tracker_AnimalId(idNumber="sample_text", usainNumberUsed=True)
    b2 = tracker_AnimalId(idNumber="sample_text_2", usainNumberUsed=False)
    _safe_set(a, 'tracker_Event', b1)
    assert _is_linked(a, 'tracker_Event', b1)
    if hasattr(b1, 'tracker_AnimalId2'):
        assert _is_linked(b1, 'tracker_AnimalId2', a)
    _safe_set(a, 'tracker_Event', b2)
    assert _is_linked(a, 'tracker_Event', b2)
    if hasattr(b1, 'tracker_AnimalId2'):
        assert not _is_linked(b1, 'tracker_AnimalId2', a)
    if hasattr(b2, 'tracker_AnimalId2'):
        assert _is_linked(b2, 'tracker_AnimalId2', a)
    _safe_set(a, 'tracker_Event', None)
    assert not _is_linked(a, 'tracker_Event', b2)
    if hasattr(b2, 'tracker_AnimalId2'):
        assert not _is_linked(b2, 'tracker_AnimalId2', a)


def test_assoc_ain14_link_reassign_clear():
    a = tracker_AnimalId(idNumber="sample_text", usainNumberUsed=True)
    b1 = tracker_UnAppliedTags()
    b2 = tracker_UnAppliedTags()
    _safe_set(a, 'tracker_AnimalId16', b1)
    assert _is_linked(a, 'tracker_AnimalId16', b1)
    if hasattr(b1, 'tracker_UnAppliedTags15'):
        assert _is_linked(b1, 'tracker_UnAppliedTags15', a)
    _safe_set(a, 'tracker_AnimalId16', b2)
    assert _is_linked(a, 'tracker_AnimalId16', b2)
    if hasattr(b1, 'tracker_UnAppliedTags15'):
        assert not _is_linked(b1, 'tracker_UnAppliedTags15', a)
    if hasattr(b2, 'tracker_UnAppliedTags15'):
        assert _is_linked(b2, 'tracker_UnAppliedTags15', a)
    _safe_set(a, 'tracker_AnimalId16', None)
    assert not _is_linked(a, 'tracker_AnimalId16', b2)
    if hasattr(b2, 'tracker_UnAppliedTags15'):
        assert not _is_linked(b2, 'tracker_UnAppliedTags15', a)


def test_assoc_animal11_link_reassign_clear():
    a = tracker_Animal(age="sample_text", birthDate="sample_text", breed="sample_text", idNumber="sample_text", sex="sample_text", sexCode="sample_text", species="sample_text", speciesCode="sample_text")
    b1 = tracker_Animals()
    b2 = tracker_Animals()
    _safe_set(a, 'tracker_Animal13', b1)
    assert _is_linked(a, 'tracker_Animal13', b1)
    if hasattr(b1, 'tracker_Animals12'):
        assert _is_linked(b1, 'tracker_Animals12', a)
    _safe_set(a, 'tracker_Animal13', b2)
    assert _is_linked(a, 'tracker_Animal13', b2)
    if hasattr(b1, 'tracker_Animals12'):
        assert not _is_linked(b1, 'tracker_Animals12', a)
    if hasattr(b2, 'tracker_Animals12'):
        assert _is_linked(b2, 'tracker_Animals12', a)
    _safe_set(a, 'tracker_Animal13', None)
    assert not _is_linked(a, 'tracker_Animal13', b2)
    if hasattr(b2, 'tracker_Animals12'):
        assert not _is_linked(b2, 'tracker_Animals12', a)
...
def test_assoc_animals3_link_reassign_clear():
    ...
    if hasattr(b1, 'tracker_Animals'):
        assert _is_linked(b1, 'tracker_Animals', a)
    _safe_set(a, 'tracker_Premises', b2)
    assert _is_linked(a, 'tracker_Premises', b2)
    if hasattr(b1, 'tracker_Animals'):
        assert not _is_linked(b1, 'tracker_Animals', a)
    if hasattr(b2, 'tracker_Animals'):
        assert _is_linked(b2, 'tracker_Animals', a)
    _safe_set(a, 'tracker_Premises', None)
    assert not _is_linked(a, 'tracker_Premises', b2)
    if hasattr(b2, 'tracker_Animals'):
        assert not _is_linked(b2, 'tracker_Animals', a)


def test_assoc_eventHistory4_link_reassign_clear():
    a = tracker_Premises(emailContact="sample_text", premisesId="sample_text")
    b1 = tracker_EventHistory()
    b2 = tracker_EventHistory()
    _safe_set(a, 'tracker_Premises5', b1)
    assert _is_linked(a, 'tracker_Premises5', b1)
    if hasattr(b1, 'tracker_EventHistory'):
        assert _is_linked(b1, 'tracker_EventHistory', a)
    _safe_set(a, 'tracker_Premises5', b2)
    assert _is_linked(a, 'tracker_Premises5', b2)
    if hasattr(b1, 'tracker_EventHistory'):
        assert not _is_linked(b1, 'tracker_EventHistory', a)
    if hasattr(b2, 'tracker_EventHistory'):
        assert _is_linked(b2, 'tracker_EventHistory', a)
    _safe_set(a, 'tracker_Premises5', None)
    assert not _is_linked(a, 'tracker_Premises5', b2)
    if hasattr(b2, 'tracker_EventHistory'):
        assert not _is_linked(b2, 'tracker_EventHistory', a)
...
def test_assoc_events8_link_reassign_clear():
    ...
    b1 = tracker_EventHistory()
    b2 = tracker_EventHistory()
    _safe_set(a, 'tracker_Event10', b1)
    assert _is_linked(a, 'tracker_Event10', b1)
    if hasattr(b1, 'tracker_EventHistory9'):
        assert _is_linked(b1, 'tracker_EventHistory9', a)
    _safe_set(a, 'tracker_Event10', b2)
    assert _is_linked(a, 'tracker_Event10', b2)
    if hasattr(b1, 'tracker_EventHistory9'):
        assert not _is_linked(b1, 'tracker_EventHistory9', a)
    if hasattr(b2, 'tracker_EventHistory9'):
        assert _is_linked(b2, 'tracker_EventHistory9', a)
    _safe_set(a, 'tracker_Event10', None)
    assert not _is_linked(a, 'tracker_Event10', b2)
    if hasattr(b2, 'tracker_EventHistory9'):
        assert not _is_linked(b2, 'tracker_EventHistory9', a)


def test_assoc_unAppliedTags6_link_reassign_clear():
    a = tracker_Premises(emailContact="sample_text", premisesId="sample_text")
    b1 = tracker_UnAppliedTags()
    b2 = tracker_UnAppliedTags()
    _safe_set(a, 'tracker_Premises7', b1)
    assert _is_linked(a, 'tracker_Premises7', b1)
    if hasattr(b1, 'tracker_UnAppliedTags'):
        assert _is_linked(b1, 'tracker_UnAppliedTags', a)
    _safe_set(a, 'tracker_Premises7', b2)
    assert _is_linked(a, 'tracker_Premises7', b2)
    if hasattr(b1, 'tracker_UnAppliedTags'):
        assert not _is_linked(b1, 'tracker_UnAppliedTags', a)
    if hasattr(b2, 'tracker_UnAppliedTags'):
        assert _is_linked(b2, 'tracker_UnAppliedTags', a)
    _safe_set(a, 'tracker_Premises7', None)
    assert not _is_linked(a, 'tracker_Premises7', b2)
    if hasattr(b2, 'tracker_UnAppliedTags'):
        assert not _is_linked(b2, 'tracker_UnAppliedTags', a)
...
tracker_AnimalId_strategy = st.builds(tracker_AnimalId, idNumber=safe_text, usainNumberUsed=st.booleans())
@given(instance=tracker_AnimalId_strategy)
@settings(max_examples=25)
...
tracker_Animals_strategy = st.builds(tracker_Animals)
...
tracker_Event_strategy = st.builds(tracker_Event, comments=safe_text, correction=st.booleans(), dateTime=safe_text, electronicallyRead=st.booleans(), eventCode=st.integers(), id=safe_text)
@given(instance=tracker_Event_strategy)
@settings(max_examples=25)
...
tracker_EventHistory_strategy = st.builds(tracker_EventHistory)
...
tracker_UnAppliedTags_strategy = st.builds(tracker_UnAppliedTags)
