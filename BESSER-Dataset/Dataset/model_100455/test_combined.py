...
def test_assoc_args93_link_reassign_clear():
    ...
    _safe_set(a, 'stext_FeatureCall94', set())
    assert not _is_linked(a, 'stext_FeatureCall94', b2)
    if hasattr(b2, 'stext_Expression95'):
        assert not _is_linked(b2, 'stext_Expression95', a)


def test_assoc_args98_link_reassign_clear():
    a = stext_ElementReferenceExpression(operationCall=True)
    b1 = stext_Expression()
    b2 = stext_Expression()
    _safe_set(a, 'stext_ElementReferenceExpression99', {b1})
    assert _is_linked(a, 'stext_ElementReferenceExpression99', b1)
    if hasattr(b1, 'stext_Expression100'):
        assert _is_linked(b1, 'stext_Expression100', a)
    _safe_set(a, 'stext_ElementReferenceExpression99', {b2})
    assert _is_linked(a, 'stext_ElementReferenceExpression99', b2)
    if hasattr(b1, 'stext_Expression100'):
    ...
    _safe_set(a, 'stext_ElementReferenceExpression99', set())
    assert not _is_linked(a, 'stext_ElementReferenceExpression99', b2)
    if hasattr(b2, 'stext_Expression100'):
...
def test_assoc_expression28_link_reassign_clear():
    ...
    b1 = stext_Expression()
    b2 = stext_Expression()
    _safe_set(a, 'stext_AssignmentExpression29', b1)
    assert _is_linked(a, 'stext_AssignmentExpression29', b1)
    if hasattr(b1, 'stext_Expression30'):
        assert _is_linked(b1, 'stext_Expression30', a)
    _safe_set(a, 'stext_AssignmentExpression29', b2)
    assert _is_linked(a, 'stext_AssignmentExpression29', b2)
    if hasattr(b1, 'stext_Expression30'):
        assert not _is_linked(b1, 'stext_Expression30', a)
    if hasattr(b2, 'stext_Expression30'):
        assert _is_linked(b2, 'stext_Expression30', a)
    _safe_set(a, 'stext_AssignmentExpression29', None)
    assert not _is_linked(a, 'stext_AssignmentExpression29', b2)
    if hasattr(b2, 'stext_Expression30'):
        assert not _is_linked(b2, 'stext_Expression30', a)


def test_assoc_feature91_link_reassign_clear():
    a = stext_FeatureCall(operationCall=True)
    b1 = stext_EObject()
    b2 = stext_EObject()
    ...
    _safe_set(a, 'stext_FeatureCall92', None)
...
def test_assoc_leftOperand66_link_reassign_clear():
    ...
    if hasattr(b1, 'stext_Expression67'):
        assert _is_linked(b1, 'stext_Expression67', a)
    _safe_set(a, 'stext_LogicalRelationExpression', b2)
    assert _is_linked(a, 'stext_LogicalRelationExpression', b2)
    if hasattr(b1, 'stext_Expression67'):
        assert not _is_linked(b1, 'stext_Expression67', a)
    if hasattr(b2, 'stext_Expression67'):
        assert _is_linked(b2, 'stext_Expression67', a)
    _safe_set(a, 'stext_LogicalRelationExpression', None)
    assert not _is_linked(a, 'stext_LogicalRelationExpression', b2)
    if hasattr(b2, 'stext_Expression67'):
        assert not _is_linked(b2, 'stext_Expression67', a)


def test_assoc_leftOperand71_link_reassign_clear():
...
def test_assoc_leftOperand76_link_reassign_clear():
...
def test_assoc_leftOperand81_link_reassign_clear():
...
def test_assoc_operand86_link_reassign_clear():
...
def test_assoc_owner89_link_reassign_clear():
    a = stext_FeatureCall(operationCall=True)
    b1 = stext_Expression()
    b2 = stext_Expression()
    _safe_set(a, 'stext_FeatureCall', b1)
    assert _is_linked(a, 'stext_FeatureCall', b1)
    if hasattr(b1, 'stext_Expression90'):
        assert _is_linked(b1, 'stext_Expression90', a)
    _safe_set(a, 'stext_FeatureCall', b2)
    assert _is_linked(a, 'stext_FeatureCall', b2)
    if hasattr(b1, 'stext_Expression90'):
        assert not _is_linked(b1, 'stext_Expression90', a)
    if hasattr(b2, 'stext_Expression90'):
        assert _is_linked(b2, 'stext_Expression90', a)
    _safe_set(a, 'stext_FeatureCall', None)
    assert not _is_linked(a, 'stext_FeatureCall', b2)
    if hasattr(b2, 'stext_Expression90'):
        assert not _is_linked(b2, 'stext_Expression90', a)


def test_assoc_reference96_link_reassign_clear():
    a = stext_ElementReferenceExpression(operationCall=True)
    b1 = stext_EObject()
    b2 = stext_EObject()
    _safe_set(a, 'stext_ElementReferenceExpression', b1)
    assert _is_linked(a, 'stext_ElementReferenceExpression', b1)
    if hasattr(b1, 'stext_EObject97'):
        assert _is_linked(b1, 'stext_EObject97', a)
    _safe_set(a, 'stext_ElementReferenceExpression', b2)
    assert _is_linked(a, 'stext_ElementReferenceExpression', b2)
    if hasattr(b1, 'stext_EObject97'):
        assert not _is_linked(b1, 'stext_EObject97', a)
    if hasattr(b2, 'stext_EObject97'):
        assert _is_linked(b2, 'stext_EObject97', a)
    _safe_set(a, 'stext_ElementReferenceExpression', None)
    assert not _is_linked(a, 'stext_ElementReferenceExpression', b2)
    if hasattr(b2, 'stext_EObject97'):
        assert not _is_linked(b2, 'stext_EObject97', a)


def test_assoc_rightOperand68_link_reassign_clear():
    a = stext_LogicalRelationExpression(operator="sample_text")
    b1 = stext_Expression()
    b2 = stext_Expression()
    _safe_set(a, 'stext_LogicalRelationExpression69', b1)
    assert _is_linked(a, 'stext_LogicalRelationExpression69', b1)
    if hasattr(b1, 'stext_Expression70'):
        assert _is_linked(b1, 'stext_Expression70', a)
    _safe_set(a, 'stext_LogicalRelationExpression69', b2)
    assert _is_linked(a, 'stext_LogicalRelationExpression69', b2)
    if hasattr(b1, 'stext_Expression70'):
        assert not _is_linked(b1, 'stext_Expression70', a)
    if hasattr(b2, 'stext_Expression70'):
        assert _is_linked(b2, 'stext_Expression70', a)
    _safe_set(a, 'stext_LogicalRelationExpression69', None)
    assert not _is_linked(a, 'stext_LogicalRelationExpression69', b2)
    if hasattr(b2, 'stext_Expression70'):
        assert not _is_linked(b2, 'stext_Expression70', a)


def test_assoc_rightOperand73_link_reassign_clear():
    a = stext_ShiftExpression(operator="sample_text")
    b1 = stext_Expression()
    b2 = stext_Expression()
    _safe_set(a, 'stext_ShiftExpression74', b1)
    assert _is_linked(a, 'stext_ShiftExpression74', b1)
    if hasattr(b1, 'stext_Expression75'):
        assert _is_linked(b1, 'stext_Expression75', a)
    _safe_set(a, 'stext_ShiftExpression74', b2)
    assert _is_linked(a, 'stext_ShiftExpression74', b2)
    if hasattr(b1, 'stext_Expression75'):
        assert not _is_linked(b1, 'stext_Expression75', a)
    if hasattr(b2, 'stext_Expression75'):
        assert _is_linked(b2, 'stext_Expression75', a)
    _safe_set(a, 'stext_ShiftExpression74', None)
    assert not _is_linked(a, 'stext_ShiftExpression74', b2)
    if hasattr(b2, 'stext_Expression75'):
        assert not _is_linked(b2, 'stext_Expression75', a)


def test_assoc_rightOperand78_link_reassign_clear():
    a = stext_NumericalAddSubtractExpression(operator="sample_text")
    b1 = stext_Expression()
    b2 = stext_Expression()
    _safe_set(a, 'stext_NumericalAddSubtractExpression79', b1)
    assert _is_linked(a, 'stext_NumericalAddSubtractExpression79', b1)
    if hasattr(b1, 'stext_Expression80'):
        assert _is_linked(b1, 'stext_Expression80', a)
    _safe_set(a, 'stext_NumericalAddSubtractExpression79', b2)
    assert _is_linked(a, 'stext_NumericalAddSubtractExpression79', b2)
    if hasattr(b1, 'stext_Expression80'):
        assert not _is_linked(b1, 'stext_Expression80', a)
    if hasattr(b2, 'stext_Expression80'):
        assert _is_linked(b2, 'stext_Expression80', a)
    _safe_set(a, 'stext_NumericalAddSubtractExpression79', None)
    assert not _is_linked(a, 'stext_NumericalAddSubtractExpression79', b2)
    if hasattr(b2, 'stext_Expression80'):
        assert not _is_linked(b2, 'stext_Expression80', a)


def test_assoc_rightOperand83_link_reassign_clear():
    a = stext_NumericalMultiplyDivideExpression(operator="sample_text")
    b1 = stext_Expression()
    b2 = stext_Expression()
    _safe_set(a, 'stext_NumericalMultiplyDivideExpression84', b1)
    assert _is_linked(a, 'stext_NumericalMultiplyDivideExpression84', b1)
    if hasattr(b1, 'stext_Expression85'):
        assert _is_linked(b1, 'stext_Expression85', a)
    _safe_set(a, 'stext_NumericalMultiplyDivideExpression84', b2)
    assert _is_linked(a, 'stext_NumericalMultiplyDivideExpression84', b2)
    if hasattr(b1, 'stext_Expression85'):
        assert not _is_linked(b1, 'stext_Expression85', a)
    if hasattr(b2, 'stext_Expression85'):
        assert _is_linked(b2, 'stext_Expression85', a)
    _safe_set(a, 'stext_NumericalMultiplyDivideExpression84', None)
    assert not _is_linked(a, 'stext_NumericalMultiplyDivideExpression84', b2)
    if hasattr(b2, 'stext_Expression85'):
...
def test_assoc_value13_link_reassign_clear():
    ...
    if hasattr(b1, 'stext_Expression14'):
        assert _is_linked(b1, 'stext_Expression14', a)
    _safe_set(a, 'stext_TimeEventSpec', b2)
    assert _is_linked(a, 'stext_TimeEventSpec', b2)
    if hasattr(b1, 'stext_Expression14'):
        assert not _is_linked(b1, 'stext_Expression14', a)
    if hasattr(b2, 'stext_Expression14'):
        assert _is_linked(b2, 'stext_Expression14', a)
    _safe_set(a, 'stext_TimeEventSpec', None)
    assert not _is_linked(a, 'stext_TimeEventSpec', b2)
    if hasattr(b2, 'stext_Expression14'):
        assert not _is_linked(b2, 'stext_Expression14', a)


def test_assoc_varRef26_link_reassign_clear():
    a = stext_AssignmentExpression(operator="sample_text")
    b1 = stext_Expression()
    b2 = stext_Expression()
