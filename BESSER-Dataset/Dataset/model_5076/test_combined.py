...
from python_code import (
    ...
    CarRental2_TernaryRelationMaintenance,
    CarRental2_CarGroup,
    CarRental2_Car,
    CarRental2_Branch,
    CarRental2_Rental,
...
def test_hyp_carrental2_ternaryrelationmaintenance_is_not_abstract():
    assert not inspect.isabstract(CarRental2_TernaryRelationMaintenance)


def test_hyp_carrental2_ternaryrelationmaintenance_constructor_exists():
    assert callable(CarRental2_TernaryRelationMaintenance.__init__)


def test_hyp_carrental2_ternaryrelationmaintenance_constructor_args():
    sig = inspect.signature(CarRental2_TernaryRelationMaintenance.__init__)
    params = list(sig.parameters.keys())
...
CarRental2_TernaryRelationMaintenance_strategy = st.builds(
    CarRental2_TernaryRelationMaintenance,
)
...
def test_CarRental2_Branch_location_value_roundtrip():
    instance = CarRental2_Branch(location=7)
...
def test_CarRental2_CarGroup_kind_value_roundtrip():
    instance = CarRental2_CarGroup(kind=7)
...
def test_CarRental2_Check_description_value_roundtrip():
    instance = CarRental2_Check(description=7)
    assert instance.description == 7
    instance.description = 13
    assert instance.description == 13


def test_CarRental2_Customer_address_value_roundtrip():
    instance = CarRental2_Customer(address=7)
...
def test_CarRental2_Person_age_value_roundtrip():
    instance = CarRental2_Person(age=7, firstname=7, isMarried=True, lastname=7)
...
def test_CarRental2_Rental_fromDate_value_roundtrip():
    instance = CarRental2_Rental(fromDate=7, untilDate=7)
    assert instance.fromDate == 7
    instance.fromDate = 13
    assert instance.fromDate == 13


def test_CarRental2_Rental_untilDate_value_roundtrip():
    instance = CarRental2_Rental(fromDate=7, untilDate=7)
    assert instance.untilDate == 7
    instance.untilDate = 13
    assert instance.untilDate == 13


def test_CarRental2_ServiceDepot_location_value_roundtrip():
    instance = CarRental2_ServiceDepot(location=7)
    assert instance.location == 7
    instance.location = 13
    assert instance.location == 13
...
def test_assoc_Assignment_Car16_link_reassign_clear():
    a = CarRental2_Rental(fromDate=7, untilDate=7)
    b1 = CarRental2_Car(id=7)
    b2 = CarRental2_Car(id=13)
...
def test_assoc_Booking_Customer11_link_reassign_clear():
    a = CarRental2_Rental(fromDate=7, untilDate=7)
    b1 = CarRental2_Customer(address=7)
    b2 = CarRental2_Customer(address=13)
...
def test_assoc_Employment_Branch_role_employer2_link_reassign_clear():
    ...
    b1 = CarRental2_Branch(location=7)
    b2 = CarRental2_Branch(location=13)
...
def test_assoc_Quality_CarGroup_role_higher28_link_reassign_clear():
    a = CarRental2_CarGroup(kind=7)
    b1 = CarRental2_CarGroup(kind=7)
    b2 = CarRental2_CarGroup(kind=13)
...
def test_assoc_toCar44_link_reassign_clear():
    a = CarRental2_Car(id=7)
    b1 = CarRental2_TernaryRelationMaintenance()
    b2 = CarRental2_TernaryRelationMaintenance()
    _safe_set(a, 'Car46', b1)
    assert _is_linked(a, 'Car46', b1)
    if hasattr(b1, 'toMaintenance45'):
        assert _is_linked(b1, 'toMaintenance45', a)
    _safe_set(a, 'Car46', b2)
    assert _is_linked(a, 'Car46', b2)
    if hasattr(b1, 'toMaintenance45'):
        assert not _is_linked(b1, 'toMaintenance45', a)
    if hasattr(b2, 'toMaintenance45'):
        assert _is_linked(b2, 'toMaintenance45', a)
    _safe_set(a, 'Car46', None)
    assert not _is_linked(a, 'Car46', b2)
    if hasattr(b2, 'toMaintenance45'):
        assert not _is_linked(b2, 'toMaintenance45', a)


def test_assoc_toCheck42_link_reassign_clear():
    a = CarRental2_Check(description=7)
    b1 = CarRental2_TernaryRelationMaintenance()
    b2 = CarRental2_TernaryRelationMaintenance()
    _safe_set(a, 'Check', b1)
    assert _is_linked(a, 'Check', b1)
    if hasattr(b1, 'toMaintenance43'):
        assert _is_linked(b1, 'toMaintenance43', a)
    _safe_set(a, 'Check', b2)
    assert _is_linked(a, 'Check', b2)
    if hasattr(b1, 'toMaintenance43'):
        assert not _is_linked(b1, 'toMaintenance43', a)
    if hasattr(b2, 'toMaintenance43'):
        assert _is_linked(b2, 'toMaintenance43', a)
    _safe_set(a, 'Check', None)
    assert not _is_linked(a, 'Check', b2)
    if hasattr(b2, 'toMaintenance43'):
        assert not _is_linked(b2, 'toMaintenance43', a)


def test_assoc_toMaintenance36_link_reassign_clear():
    a = CarRental2_Car(id=7)
    b1 = CarRental2_TernaryRelationMaintenance()
    b2 = CarRental2_TernaryRelationMaintenance()
    _safe_set(a, 'toCar', {b1})
    assert _is_linked(a, 'toCar', b1)
    if hasattr(b1, 'TernaryRelationMaintenance'):
        assert _is_linked(b1, 'TernaryRelationMaintenance', a)
    _safe_set(a, 'toCar', {b2})
    assert _is_linked(a, 'toCar', b2)
    if hasattr(b1, 'TernaryRelationMaintenance'):
        assert not _is_linked(b1, 'TernaryRelationMaintenance', a)
    if hasattr(b2, 'TernaryRelationMaintenance'):
        assert _is_linked(b2, 'TernaryRelationMaintenance', a)
    _safe_set(a, 'toCar', set())
    assert not _is_linked(a, 'toCar', b2)
    if hasattr(b2, 'TernaryRelationMaintenance'):
        assert not _is_linked(b2, 'TernaryRelationMaintenance', a)


def test_assoc_toMaintenance37_link_reassign_clear():
    a = CarRental2_ServiceDepot(location=7)
    b1 = CarRental2_TernaryRelationMaintenance()
    b2 = CarRental2_TernaryRelationMaintenance()
    _safe_set(a, 'toServiceDepot', {b1})
    assert _is_linked(a, 'toServiceDepot', b1)
    if hasattr(b1, 'TernaryRelationMaintenance38'):
        assert _is_linked(b1, 'TernaryRelationMaintenance38', a)
    _safe_set(a, 'toServiceDepot', {b2})
    assert _is_linked(a, 'toServiceDepot', b2)
    if hasattr(b1, 'TernaryRelationMaintenance38'):
        assert not _is_linked(b1, 'TernaryRelationMaintenance38', a)
    if hasattr(b2, 'TernaryRelationMaintenance38'):
        assert _is_linked(b2, 'TernaryRelationMaintenance38', a)
    _safe_set(a, 'toServiceDepot', set())
    assert not _is_linked(a, 'toServiceDepot', b2)
    if hasattr(b2, 'TernaryRelationMaintenance38'):
        assert not _is_linked(b2, 'TernaryRelationMaintenance38', a)


def test_assoc_toMaintenance39_link_reassign_clear():
    a = CarRental2_Check(description=7)
    b1 = CarRental2_TernaryRelationMaintenance()
    b2 = CarRental2_TernaryRelationMaintenance()
    _safe_set(a, 'toCheck', {b1})
    assert _is_linked(a, 'toCheck', b1)
    if hasattr(b1, 'TernaryRelationMaintenance40'):
        assert _is_linked(b1, 'TernaryRelationMaintenance40', a)
    _safe_set(a, 'toCheck', {b2})
    assert _is_linked(a, 'toCheck', b2)
    if hasattr(b1, 'TernaryRelationMaintenance40'):
        assert not _is_linked(b1, 'TernaryRelationMaintenance40', a)
    if hasattr(b2, 'TernaryRelationMaintenance40'):
        assert _is_linked(b2, 'TernaryRelationMaintenance40', a)
    _safe_set(a, 'toCheck', set())
    assert not _is_linked(a, 'toCheck', b2)
    if hasattr(b2, 'TernaryRelationMaintenance40'):
        assert not _is_linked(b2, 'TernaryRelationMaintenance40', a)


def test_assoc_toServiceDepot41_link_reassign_clear():
    a = CarRental2_ServiceDepot(location=7)
    b1 = CarRental2_TernaryRelationMaintenance()
    b2 = CarRental2_TernaryRelationMaintenance()
    _safe_set(a, 'ServiceDepot', b1)
    assert _is_linked(a, 'ServiceDepot', b1)
    if hasattr(b1, 'toMaintenance'):
        assert _is_linked(b1, 'toMaintenance', a)
    _safe_set(a, 'ServiceDepot', b2)
    assert _is_linked(a, 'ServiceDepot', b2)
    if hasattr(b1, 'toMaintenance'):
        assert not _is_linked(b1, 'toMaintenance', a)
    if hasattr(b2, 'toMaintenance'):
        assert _is_linked(b2, 'toMaintenance', a)
    _safe_set(a, 'ServiceDepot', None)
    assert not _is_linked(a, 'ServiceDepot', b2)
    if hasattr(b2, 'toMaintenance'):
        assert not _is_linked(b2, 'toMaintenance', a)
...
CarRental2_Branch_strategy = st.builds(CarRental2_Branch, location=st.integers())
@given(instance=CarRental2_Branch_strategy)
...
CarRental2_Car_strategy = st.builds(CarRental2_Car, id=st.integers())
@given(instance=CarRental2_Car_strategy)
@settings(max_examples=25)
...
CarRental2_CarGroup_strategy = st.builds(CarRental2_CarGroup, kind=st.integers())
@given(instance=CarRental2_CarGroup_strategy)
@settings(max_examples=25)
...
CarRental2_Check_strategy = st.builds(CarRental2_Check, description=st.integers())
@given(instance=CarRental2_Check_strategy)
@settings(max_examples=25)
...
CarRental2_Customer_strategy = st.builds(CarRental2_Customer, address=st.integers())
@given(instance=CarRental2_Customer_strategy)
@settings(max_examples=25)
...
CarRental2_Person_strategy = st.builds(CarRental2_Person, age=st.integers(), firstname=st.integers(), isMarried=st.booleans(), lastname=st.integers())
@given(instance=CarRental2_Person_strategy)
@settings(max_examples=25)
...
CarRental2_Rental_strategy = st.builds(CarRental2_Rental, fromDate=st.integers(), untilDate=st.integers())
@given(instance=CarRental2_Rental_strategy)
@settings(max_examples=25)
...
CarRental2_ServiceDepot_strategy = st.builds(CarRental2_ServiceDepot, location=st.integers())
@given(instance=CarRental2_ServiceDepot_strategy)
@settings(max_examples=25)
...
CarRental2_TernaryRelationMaintenance_strategy = st.builds(CarRental2_TernaryRelationMaintenance)
@given(instance=CarRental2_TernaryRelationMaintenance_strategy)
@settings(max_examples=25)
def test_CarRental2_TernaryRelationMaintenance_instantiation(instance):
    assert isinstance(instance, CarRental2_TernaryRelationMaintenance)
