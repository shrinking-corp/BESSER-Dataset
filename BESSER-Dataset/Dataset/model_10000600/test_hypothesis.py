...
def test_course_adminstrator_is_not_abstract():
    assert not inspect.isabstract(Course_Adminstrator)


def test_course_adminstrator_constructor_exists():
    assert callable(Course_Adminstrator.__init__)


def test_course_adminstrator_constructor_args():
    sig = inspect.signature(Course_Adminstrator.__init__)
    params = list(sig.parameters.keys())
    assert "attribute" in params, "Missing parameter 'attribute'"

def test_course_adminstrator_has_attribute():
    assert hasattr(Course_Adminstrator, "attribute")
    descriptor = None
    for klass in Course_Adminstrator.__mro__:
...
@given(instance=Course_Adminstrator_strategy)
def test_course_adminstrator_attribute_setter(instance):
