...
def test_hyp_course_adminstrator_is_not_abstract():
    assert not inspect.isabstract(Course_Adminstrator)


def test_hyp_course_adminstrator_constructor_exists():
    assert callable(Course_Adminstrator.__init__)


def test_hyp_course_adminstrator_constructor_args():
    sig = inspect.signature(Course_Adminstrator.__init__)
    params = list(sig.parameters.keys())
...
Course_Adminstrator_strategy = st.builds(
    Course_Adminstrator,
    attribute=
        safe_text
)
...
@given(instance=Course_Adminstrator_strategy)
def test_hyp_course_adminstrator_attribute_setter(instance):
...
def test_Course_Adminstrator_attribute_value_roundtrip():
    instance = Course_Adminstrator(attribute="sample_text")
...
Course_Adminstrator_strategy = st.builds(Course_Adminstrator, attribute=safe_text)
@given(instance=Course_Adminstrator_strategy)
@settings(max_examples=25)
def test_Course_Adminstrator_instantiation(instance):
    assert isinstance(instance, Course_Adminstrator)
