...
def test_assoc__from109_link_reassign_clear():
    a = robochart_Variable(modifier="sample_text")
    b1 = robochart_Trigger(_type="sample_text")
    b2 = robochart_Trigger(_type="sample_text_2")
    _safe_set(a, 'robochart_Variable111', b1)
    assert _is_linked(a, 'robochart_Variable111', b1)
    if hasattr(b1, 'robochart_Trigger110'):
        assert _is_linked(b1, 'robochart_Trigger110', a)
    _safe_set(a, 'robochart_Variable111', b2)
    assert _is_linked(a, 'robochart_Variable111', b2)
    if hasattr(b1, 'robochart_Trigger110'):
        assert not _is_linked(b1, 'robochart_Trigger110', a)
    if hasattr(b2, 'robochart_Trigger110'):
        assert _is_linked(b2, 'robochart_Trigger110', a)
    _safe_set(a, 'robochart_Variable111', None)
    assert not _is_linked(a, 'robochart_Variable111', b2)
    if hasattr(b2, 'robochart_Trigger110'):
        assert not _is_linked(b2, 'robochart_Trigger110', a)


def test_assoc__predicate112_link_reassign_clear():
    a = robochart_Trigger(_type="sample_text")
    b1 = robochart_Expression()
    b2 = robochart_Expression()
    _safe_set(a, 'robochart_Trigger113', b1)
    assert _is_linked(a, 'robochart_Trigger113', b1)
    if hasattr(b1, 'robochart_Expression114'):
        assert _is_linked(b1, 'robochart_Expression114', a)
    _safe_set(a, 'robochart_Trigger113', b2)
    assert _is_linked(a, 'robochart_Trigger113', b2)
    if hasattr(b1, 'robochart_Expression114'):
        assert not _is_linked(b1, 'robochart_Expression114', a)
    if hasattr(b2, 'robochart_Expression114'):
        assert _is_linked(b2, 'robochart_Expression114', a)
    _safe_set(a, 'robochart_Trigger113', None)
    assert not _is_linked(a, 'robochart_Trigger113', b2)
    if hasattr(b2, 'robochart_Expression114'):
        assert not _is_linked(b2, 'robochart_Expression114', a)


def test_assoc_base318_link_reassign_clear():
    a = robochart_VectorType(size=7)
    b1 = robochart_Type()
    b2 = robochart_Type()
    _safe_set(a, 'robochart_VectorType', b1)
    assert _is_linked(a, 'robochart_VectorType', b1)
    if hasattr(b1, 'robochart_Type319'):
        assert _is_linked(b1, 'robochart_Type319', a)
    _safe_set(a, 'robochart_VectorType', b2)
    assert _is_linked(a, 'robochart_VectorType', b2)
    if hasattr(b1, 'robochart_Type319'):
        assert not _is_linked(b1, 'robochart_Type319', a)
    if hasattr(b2, 'robochart_Type319'):
        assert _is_linked(b2, 'robochart_Type319', a)
    _safe_set(a, 'robochart_VectorType', None)
    assert not _is_linked(a, 'robochart_VectorType', b2)
    if hasattr(b2, 'robochart_Type319'):
        assert not _is_linked(b2, 'robochart_Type319', a)


def test_assoc_base320_link_reassign_clear():
    a = robochart_MatrixType(columns=7, rows=7)
    b1 = robochart_Type()
    b2 = robochart_Type()
    _safe_set(a, 'robochart_MatrixType', b1)
    assert _is_linked(a, 'robochart_MatrixType', b1)
    if hasattr(b1, 'robochart_Type321'):
        assert _is_linked(b1, 'robochart_Type321', a)
    _safe_set(a, 'robochart_MatrixType', b2)
    assert _is_linked(a, 'robochart_MatrixType', b2)
    if hasattr(b1, 'robochart_Type321'):
        assert not _is_linked(b1, 'robochart_Type321', a)
    if hasattr(b2, 'robochart_Type321'):
        assert _is_linked(b2, 'robochart_Type321', a)
    _safe_set(a, 'robochart_MatrixType', None)
    assert not _is_linked(a, 'robochart_MatrixType', b2)
    if hasattr(b2, 'robochart_Type321'):
        assert not _is_linked(b2, 'robochart_Type321', a)


def test_assoc_connections133_link_reassign_clear():
    ...
    if hasattr(b1, 'robochart_ControllerDef134'):
        assert _is_linked(b1, 'robochart_ControllerDef134', a)
    _safe_set(a, 'robochart_Connection', b2)
    assert _is_linked(a, 'robochart_Connection', b2)
    if hasattr(b1, 'robochart_ControllerDef134'):
        assert not _is_linked(b1, 'robochart_ControllerDef134', a)
    if hasattr(b2, 'robochart_ControllerDef134'):
        assert _is_linked(b2, 'robochart_ControllerDef134', a)
    _safe_set(a, 'robochart_Connection', None)
    assert not _is_linked(a, 'robochart_Connection', b2)
    if hasattr(b2, 'robochart_ControllerDef134'):
        assert not _is_linked(b2, 'robochart_ControllerDef134', a)


def test_assoc_connections148_link_reassign_clear():
    a = robochart_Connection(async_=True, bidirec=True)
    b1 = robochart_RCModule()
    b2 = robochart_RCModule()
    _safe_set(a, 'robochart_Connection150', b1)
    assert _is_linked(a, 'robochart_Connection150', b1)
    if hasattr(b1, 'robochart_RCModule149'):
        assert _is_linked(b1, 'robochart_RCModule149', a)
    _safe_set(a, 'robochart_Connection150', b2)
    assert _is_linked(a, 'robochart_Connection150', b2)
    if hasattr(b1, 'robochart_RCModule149'):
        assert not _is_linked(b1, 'robochart_RCModule149', a)
    if hasattr(b2, 'robochart_RCModule149'):
        assert _is_linked(b2, 'robochart_RCModule149', a)
    _safe_set(a, 'robochart_Connection150', None)
    assert not _is_linked(a, 'robochart_Connection150', b2)
    if hasattr(b2, 'robochart_RCModule149'):
        assert not _is_linked(b2, 'robochart_RCModule149', a)


def test_assoc_efrom140_link_reassign_clear():
    ...
    _safe_set(a, 'robochart_Event142', b1)
    assert _is_linked(a, 'robochart_Event142', b1)
    if hasattr(b1, 'robochart_Connection141'):
        assert _is_linked(b1, 'robochart_Connection141', a)
    _safe_set(a, 'robochart_Event142', b2)
    assert _is_linked(a, 'robochart_Event142', b2)
    if hasattr(b1, 'robochart_Connection141'):
        assert not _is_linked(b1, 'robochart_Connection141', a)
    if hasattr(b2, 'robochart_Connection141'):
        assert _is_linked(b2, 'robochart_Connection141', a)
    _safe_set(a, 'robochart_Event142', None)
    assert not _is_linked(a, 'robochart_Event142', b2)
    if hasattr(b2, 'robochart_Connection141'):
        assert not _is_linked(b2, 'robochart_Connection141', a)


def test_assoc_eto143_link_reassign_clear():
    a = robochart_Event(broadcast=True)
    b1 = robochart_Connection(async_=True, bidirec=True)
    b2 = robochart_Connection(async_=False, bidirec=False)
    _safe_set(a, 'robochart_Event145', b1)
    assert _is_linked(a, 'robochart_Event145', b1)
    if hasattr(b1, 'robochart_Connection144'):
        assert _is_linked(b1, 'robochart_Connection144', a)
    _safe_set(a, 'robochart_Event145', b2)
    assert _is_linked(a, 'robochart_Event145', b2)
    if hasattr(b1, 'robochart_Connection144'):
        assert not _is_linked(b1, 'robochart_Connection144', a)
    if hasattr(b2, 'robochart_Connection144'):
        assert _is_linked(b2, 'robochart_Connection144', a)
    _safe_set(a, 'robochart_Event145', None)
    assert not _is_linked(a, 'robochart_Event145', b2)
    if hasattr(b2, 'robochart_Connection144'):
        assert not _is_linked(b2, 'robochart_Connection144', a)


def test_assoc_event106_link_reassign_clear():
    a = robochart_Trigger(_type="sample_text")
    b1 = robochart_Event(broadcast=True)
    b2 = robochart_Event(broadcast=False)
    _safe_set(a, 'robochart_Trigger107', b1)
    assert _is_linked(a, 'robochart_Trigger107', b1)
    if hasattr(b1, 'robochart_Event108'):
        assert _is_linked(b1, 'robochart_Event108', a)
    _safe_set(a, 'robochart_Trigger107', b2)
    assert _is_linked(a, 'robochart_Trigger107', b2)
    if hasattr(b1, 'robochart_Event108'):
        assert not _is_linked(b1, 'robochart_Event108', a)
    if hasattr(b2, 'robochart_Event108'):
        assert _is_linked(b2, 'robochart_Event108', a)
    _safe_set(a, 'robochart_Trigger107', None)
    assert not _is_linked(a, 'robochart_Trigger107', b2)
    if hasattr(b2, 'robochart_Event108'):
        assert not _is_linked(b2, 'robochart_Event108', a)
...
def test_assoc_from_135_link_reassign_clear():
    a = robochart_Connection(async_=True, bidirec=True)
    b1 = robochart_ConnectionNode()
    b2 = robochart_ConnectionNode()
    _safe_set(a, 'robochart_Connection136', b1)
    assert _is_linked(a, 'robochart_Connection136', b1)
    if hasattr(b1, 'robochart_ConnectionNode'):
        assert _is_linked(b1, 'robochart_ConnectionNode', a)
    _safe_set(a, 'robochart_Connection136', b2)
    assert _is_linked(a, 'robochart_Connection136', b2)
    if hasattr(b1, 'robochart_ConnectionNode'):
    ...
    _safe_set(a, 'robochart_Connection136', None)
    assert not _is_linked(a, 'robochart_Connection136', b2)
    if hasattr(b2, 'robochart_ConnectionNode'):
...
def test_assoc_lrange289_link_reassign_clear():
    ...
    if hasattr(b1, 'robochart_Expression290'):
        assert _is_linked(b1, 'robochart_Expression290', a)
    _safe_set(a, 'robochart_RangeExp', b2)
    assert _is_linked(a, 'robochart_RangeExp', b2)
    if hasattr(b1, 'robochart_Expression290'):
        assert not _is_linked(b1, 'robochart_Expression290', a)
    if hasattr(b2, 'robochart_Expression290'):
        assert _is_linked(b2, 'robochart_Expression290', a)
    _safe_set(a, 'robochart_RangeExp', None)
    assert not _is_linked(a, 'robochart_RangeExp', b2)
    if hasattr(b2, 'robochart_Expression290'):
        assert not _is_linked(b2, 'robochart_Expression290', a)


def test_assoc_name309_link_reassign_clear():
    a = robochart_Variable(modifier="sample_text")
    b1 = robochart_VarRef()
    b2 = robochart_VarRef()
    _safe_set(a, 'robochart_Variable310', b1)
    assert _is_linked(a, 'robochart_Variable310', b1)
    if hasattr(b1, 'robochart_VarRef'):
        assert _is_linked(b1, 'robochart_VarRef', a)
    _safe_set(a, 'robochart_Variable310', b2)
    assert _is_linked(a, 'robochart_Variable310', b2)
    if hasattr(b1, 'robochart_VarRef'):
        assert not _is_linked(b1, 'robochart_VarRef', a)
    if hasattr(b2, 'robochart_VarRef'):
        assert _is_linked(b2, 'robochart_VarRef', a)
    _safe_set(a, 'robochart_Variable310', None)
    assert not _is_linked(a, 'robochart_Variable310', b2)
    if hasattr(b2, 'robochart_VarRef'):
        assert not _is_linked(b2, 'robochart_VarRef', a)


def test_assoc_operation182_link_reassign_clear():
    a = robochart_OperationSig(terminates=True)
    b1 = robochart_Call()
    b2 = robochart_Call()
    _safe_set(a, 'robochart_OperationSig183', b1)
    assert _is_linked(a, 'robochart_OperationSig183', b1)
    if hasattr(b1, 'robochart_Call'):
        assert _is_linked(b1, 'robochart_Call', a)
    _safe_set(a, 'robochart_OperationSig183', b2)
    assert _is_linked(a, 'robochart_OperationSig183', b2)
    if hasattr(b1, 'robochart_Call'):
        assert not _is_linked(b1, 'robochart_Call', a)
    if hasattr(b2, 'robochart_Call'):
        assert _is_linked(b2, 'robochart_Call', a)
    _safe_set(a, 'robochart_OperationSig183', None)
    assert not _is_linked(a, 'robochart_OperationSig183', b2)
    if hasattr(b2, 'robochart_Call'):
...
def test_assoc_parameter115_link_reassign_clear():
    ...
    b1 = robochart_Trigger(_type="sample_text")
    b2 = robochart_Trigger(_type="sample_text_2")
    _safe_set(a, 'robochart_Variable117', b1)
    assert _is_linked(a, 'robochart_Variable117', b1)
    if hasattr(b1, 'robochart_Trigger116'):
        assert _is_linked(b1, 'robochart_Trigger116', a)
    _safe_set(a, 'robochart_Variable117', b2)
    assert _is_linked(a, 'robochart_Variable117', b2)
    if hasattr(b1, 'robochart_Trigger116'):
        assert not _is_linked(b1, 'robochart_Trigger116', a)
    if hasattr(b2, 'robochart_Trigger116'):
        assert _is_linked(b2, 'robochart_Trigger116', a)
    _safe_set(a, 'robochart_Variable117', None)
    assert not _is_linked(a, 'robochart_Variable117', b2)
    if hasattr(b2, 'robochart_Trigger116'):
        assert not _is_linked(b2, 'robochart_Trigger116', a)
...
def test_assoc_reset124_link_reassign_clear():
    a = robochart_Trigger(_type="sample_text")
    b1 = robochart_ClockReset()
    b2 = robochart_ClockReset()
    _safe_set(a, 'robochart_Trigger125', {b1})
    assert _is_linked(a, 'robochart_Trigger125', b1)
    if hasattr(b1, 'robochart_ClockReset'):
        assert _is_linked(b1, 'robochart_ClockReset', a)
    _safe_set(a, 'robochart_Trigger125', {b2})
    assert _is_linked(a, 'robochart_Trigger125', b2)
    if hasattr(b1, 'robochart_ClockReset'):
    ...
    _safe_set(a, 'robochart_Trigger125', set())
    assert not _is_linked(a, 'robochart_Trigger125', b2)
    if hasattr(b2, 'robochart_ClockReset'):
        assert not _is_linked(b2, 'robochart_ClockReset', a)


def test_assoc_rrange291_link_reassign_clear():
    a = robochart_RangeExp(linterval="sample_text", rinterval="sample_text")
    b1 = robochart_Expression()
    b2 = robochart_Expression()
    _safe_set(a, 'robochart_RangeExp292', b1)
    assert _is_linked(a, 'robochart_RangeExp292', b1)
    if hasattr(b1, 'robochart_Expression293'):
        assert _is_linked(b1, 'robochart_Expression293', a)
    _safe_set(a, 'robochart_RangeExp292', b2)
    assert _is_linked(a, 'robochart_RangeExp292', b2)
    if hasattr(b1, 'robochart_Expression293'):
        assert not _is_linked(b1, 'robochart_Expression293', a)
    if hasattr(b2, 'robochart_Expression293'):
        assert _is_linked(b2, 'robochart_Expression293', a)
    _safe_set(a, 'robochart_RangeExp292', None)
    assert not _is_linked(a, 'robochart_RangeExp292', b2)
    if hasattr(b2, 'robochart_Expression293'):
        assert not _is_linked(b2, 'robochart_Expression293', a)


def test_assoc_time121_link_reassign_clear():
    a = robochart_Variable(modifier="sample_text")
    b1 = robochart_Trigger(_type="sample_text")
    b2 = robochart_Trigger(_type="sample_text_2")
    _safe_set(a, 'robochart_Variable123', b1)
    assert _is_linked(a, 'robochart_Variable123', b1)
    if hasattr(b1, 'robochart_Trigger122'):
        assert _is_linked(b1, 'robochart_Trigger122', a)
    _safe_set(a, 'robochart_Variable123', b2)
    assert _is_linked(a, 'robochart_Variable123', b2)
    if hasattr(b1, 'robochart_Trigger122'):
        assert not _is_linked(b1, 'robochart_Trigger122', a)
    if hasattr(b2, 'robochart_Trigger122'):
        assert _is_linked(b2, 'robochart_Trigger122', a)
    _safe_set(a, 'robochart_Variable123', None)
    assert not _is_linked(a, 'robochart_Variable123', b2)
    if hasattr(b2, 'robochart_Trigger122'):
        assert not _is_linked(b2, 'robochart_Trigger122', a)


def test_assoc_to137_link_reassign_clear():
    a = robochart_Connection(async_=True, bidirec=True)
    b1 = robochart_ConnectionNode()
    b2 = robochart_ConnectionNode()
    _safe_set(a, 'robochart_Connection138', b1)
    assert _is_linked(a, 'robochart_Connection138', b1)
    if hasattr(b1, 'robochart_ConnectionNode139'):
        assert _is_linked(b1, 'robochart_ConnectionNode139', a)
    _safe_set(a, 'robochart_Connection138', b2)
    assert _is_linked(a, 'robochart_Connection138', b2)
    if hasattr(b1, 'robochart_ConnectionNode139'):
        assert not _is_linked(b1, 'robochart_ConnectionNode139', a)
    if hasattr(b2, 'robochart_ConnectionNode139'):
        assert _is_linked(b2, 'robochart_ConnectionNode139', a)
    _safe_set(a, 'robochart_Connection138', None)
    assert not _is_linked(a, 'robochart_Connection138', b2)
    if hasattr(b2, 'robochart_ConnectionNode139'):
        assert not _is_linked(b2, 'robochart_ConnectionNode139', a)


def test_assoc_trigger176_link_reassign_clear():
    a = robochart_Trigger(_type="sample_text")
    b1 = robochart_SendEvent()
    b2 = robochart_SendEvent()
    _safe_set(a, 'robochart_Trigger177', b1)
    assert _is_linked(a, 'robochart_Trigger177', b1)
    if hasattr(b1, 'robochart_SendEvent'):
        assert _is_linked(b1, 'robochart_SendEvent', a)
    _safe_set(a, 'robochart_Trigger177', b2)
    assert _is_linked(a, 'robochart_Trigger177', b2)
    if hasattr(b1, 'robochart_SendEvent'):
    ...
    _safe_set(a, 'robochart_Trigger177', None)
    assert not _is_linked(a, 'robochart_Trigger177', b2)
    if hasattr(b2, 'robochart_SendEvent'):
        assert not _is_linked(b2, 'robochart_SendEvent', a)


def test_assoc_trigger93_link_reassign_clear():
    a = robochart_Trigger(_type="sample_text")
    b1 = robochart_Transition()
    b2 = robochart_Transition()
    _safe_set(a, 'robochart_Trigger', b1)
    assert _is_linked(a, 'robochart_Trigger', b1)
    if hasattr(b1, 'robochart_Transition94'):
        assert _is_linked(b1, 'robochart_Transition94', a)
    _safe_set(a, 'robochart_Trigger', b2)
    assert _is_linked(a, 'robochart_Trigger', b2)
    if hasattr(b1, 'robochart_Transition94'):
        assert not _is_linked(b1, 'robochart_Transition94', a)
    if hasattr(b2, 'robochart_Transition94'):
        assert _is_linked(b2, 'robochart_Transition94', a)
    _safe_set(a, 'robochart_Trigger', None)
    assert not _is_linked(a, 'robochart_Trigger', b2)
    if hasattr(b2, 'robochart_Transition94'):
        assert not _is_linked(b2, 'robochart_Transition94', a)
...
def test_assoc_value118_link_reassign_clear():
    a = robochart_Trigger(_type="sample_text")
    b1 = robochart_Expression()
    b2 = robochart_Expression()
    _safe_set(a, 'robochart_Trigger119', b1)
    assert _is_linked(a, 'robochart_Trigger119', b1)
    if hasattr(b1, 'robochart_Expression120'):
        assert _is_linked(b1, 'robochart_Expression120', a)
    _safe_set(a, 'robochart_Trigger119', b2)
    assert _is_linked(a, 'robochart_Trigger119', b2)
    if hasattr(b1, 'robochart_Expression120'):
        assert not _is_linked(b1, 'robochart_Expression120', a)
    if hasattr(b2, 'robochart_Expression120'):
        assert _is_linked(b2, 'robochart_Expression120', a)
    _safe_set(a, 'robochart_Trigger119', None)
    assert not _is_linked(a, 'robochart_Trigger119', b2)
    if hasattr(b2, 'robochart_Expression120'):
        assert not _is_linked(b2, 'robochart_Expression120', a)
...
def test_assoc_value250_link_reassign_clear():
    ...
    b1 = robochart_VarExp()
    b2 = robochart_VarExp()
    _safe_set(a, 'robochart_Variable251', b1)
    assert _is_linked(a, 'robochart_Variable251', b1)
    if hasattr(b1, 'robochart_VarExp'):
        assert _is_linked(b1, 'robochart_VarExp', a)
    _safe_set(a, 'robochart_Variable251', b2)
    assert _is_linked(a, 'robochart_Variable251', b2)
    if hasattr(b1, 'robochart_VarExp'):
        assert not _is_linked(b1, 'robochart_VarExp', a)
    if hasattr(b2, 'robochart_VarExp'):
        assert _is_linked(b2, 'robochart_VarExp', a)
    _safe_set(a, 'robochart_Variable251', None)
    assert not _is_linked(a, 'robochart_Variable251', b2)
    if hasattr(b2, 'robochart_VarExp'):
...
def test_assoc_variables204_link_reassign_clear():
    ...
    _safe_set(a, 'robochart_Variable205', b1)
    assert _is_linked(a, 'robochart_Variable205', b1)
    if hasattr(b1, 'robochart_QuantifierExpression'):
        assert _is_linked(b1, 'robochart_QuantifierExpression', a)
    _safe_set(a, 'robochart_Variable205', b2)
    assert _is_linked(a, 'robochart_Variable205', b2)
    if hasattr(b1, 'robochart_QuantifierExpression'):
    ...
    _safe_set(a, 'robochart_Variable205', None)
    assert not _is_linked(a, 'robochart_Variable205', b2)
    if hasattr(b2, 'robochart_QuantifierExpression'):
        assert not _is_linked(b2, 'robochart_QuantifierExpression', a)


def test_assoc_variables212_link_reassign_clear():
    a = robochart_Variable(modifier="sample_text")
    b1 = robochart_LambdaExp()
    b2 = robochart_LambdaExp()
    _safe_set(a, 'robochart_Variable213', b1)
    assert _is_linked(a, 'robochart_Variable213', b1)
    if hasattr(b1, 'robochart_LambdaExp'):
        assert _is_linked(b1, 'robochart_LambdaExp', a)
    _safe_set(a, 'robochart_Variable213', b2)
    assert _is_linked(a, 'robochart_Variable213', b2)
    if hasattr(b1, 'robochart_LambdaExp'):
    ...
    _safe_set(a, 'robochart_Variable213', None)
    assert not _is_linked(a, 'robochart_Variable213', b2)
    if hasattr(b2, 'robochart_LambdaExp'):
        assert not _is_linked(b2, 'robochart_LambdaExp', a)


def test_assoc_variables274_link_reassign_clear():
    a = robochart_Variable(modifier="sample_text")
    b1 = robochart_SetComp()
    b2 = robochart_SetComp()
    _safe_set(a, 'robochart_Variable275', b1)
    assert _is_linked(a, 'robochart_Variable275', b1)
    if hasattr(b1, 'robochart_SetComp'):
        assert _is_linked(b1, 'robochart_SetComp', a)
    _safe_set(a, 'robochart_Variable275', b2)
    assert _is_linked(a, 'robochart_Variable275', b2)
    if hasattr(b1, 'robochart_SetComp'):
        assert not _is_linked(b1, 'robochart_SetComp', a)
    if hasattr(b2, 'robochart_SetComp'):
        assert _is_linked(b2, 'robochart_SetComp', a)
    _safe_set(a, 'robochart_Variable275', None)
    assert not _is_linked(a, 'robochart_Variable275', b2)
    if hasattr(b2, 'robochart_SetComp'):
