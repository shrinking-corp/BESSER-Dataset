...
def test_hyp_webuser_constructor_args():
    ...
    params = list(sig.parameters.keys())
    assert "__" in params, "Missing parameter '__'"
    assert "password" in params, "Missing parameter 'password'"
    assert "state" in params, "Missing parameter 'state'"

def test_hyp_webuser_has___():
    assert hasattr(WebUser, "__")
    descriptor = None
    for klass in WebUser.__mro__:
        if "__" in klass.__dict__:
            descriptor = klass.__dict__["__"]
            break
    assert isinstance(descriptor, property)
...
@given(instance=WebUser_strategy)
def test_hyp_webuser____setter(instance):
    original = instance.__
    instance.__ = original
    assert instance.__ == original
