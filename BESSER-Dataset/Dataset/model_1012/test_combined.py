...
def test_assoc_stateEnd5_link_reassign_clear():
    a = fsm_State(name="sample_text")
    b1 = fsm_Transition()
    b2 = fsm_Transition()
    _safe_set(a, 'fsm_State7', b1)
    assert _is_linked(a, 'fsm_State7', b1)
    if hasattr(b1, 'fsm_Transition6'):
        assert _is_linked(b1, 'fsm_Transition6', a)
    _safe_set(a, 'fsm_State7', b2)
    assert _is_linked(a, 'fsm_State7', b2)
    if hasattr(b1, 'fsm_Transition6'):
        assert not _is_linked(b1, 'fsm_Transition6', a)
    if hasattr(b2, 'fsm_Transition6'):
        assert _is_linked(b2, 'fsm_Transition6', a)
    _safe_set(a, 'fsm_State7', None)
    assert not _is_linked(a, 'fsm_State7', b2)
    if hasattr(b2, 'fsm_Transition6'):
        assert not _is_linked(b2, 'fsm_Transition6', a)


def test_assoc_stateStart8_link_reassign_clear():
    a = fsm_State(name="sample_text")
    b1 = fsm_Transition()
    b2 = fsm_Transition()
    _safe_set(a, 'fsm_State10', b1)
    assert _is_linked(a, 'fsm_State10', b1)
    if hasattr(b1, 'fsm_Transition9'):
        assert _is_linked(b1, 'fsm_Transition9', a)
    _safe_set(a, 'fsm_State10', b2)
    assert _is_linked(a, 'fsm_State10', b2)
    if hasattr(b1, 'fsm_Transition9'):
        assert not _is_linked(b1, 'fsm_Transition9', a)
    if hasattr(b2, 'fsm_Transition9'):
        assert _is_linked(b2, 'fsm_Transition9', a)
    _safe_set(a, 'fsm_State10', None)
    assert not _is_linked(a, 'fsm_State10', b2)
    if hasattr(b2, 'fsm_Transition9'):
        assert not _is_linked(b2, 'fsm_Transition9', a)
