...
def test_hyp_atlstatic_atl_lazymatchedrule_is_not_abstract():
    assert not inspect.isabstract(atlstatic_ATL_LazyMatchedRule)


def test_hyp_atlstatic_atl_lazymatchedrule_constructor_exists():
    assert callable(atlstatic_ATL_LazyMatchedRule.__init__)


def test_hyp_atlstatic_atl_lazymatchedrule_constructor_args():
...
def test_hyp_atlstatic_atl_lazymatchedrule_isUnique_setter(instance):
...
def test_hyp_atlstatic_atl_matchedrule_isRefining_setter(instance):
...
@given(instance=atlstatic_ATL_MatchedRule_strategy)
def test_hyp_atlstatic_atl_matchedrule_isNoDefault_setter(instance):
...
@given(instance=atlstatic_ATL_MatchedRule_strategy)
def test_hyp_atlstatic_atl_matchedrule_isAbstract_setter(instance):
...
def test_atlstatic_ATL_LazyMatchedRule_isUnique_value_roundtrip():
    instance = atlstatic_ATL_LazyMatchedRule(isUnique="sample_text")
...
def test_atlstatic_ATL_MatchedRule_isAbstract_value_roundtrip():
    instance = atlstatic_ATL_MatchedRule(isAbstract="sample_text", isNoDefault="sample_text", isRefining="sample_text")
    assert instance.isAbstract == "sample_text"
    instance.isAbstract = "sample_text_2"
    assert instance.isAbstract == "sample_text_2"


def test_atlstatic_ATL_MatchedRule_isNoDefault_value_roundtrip():
    instance = atlstatic_ATL_MatchedRule(isAbstract="sample_text", isNoDefault="sample_text", isRefining="sample_text")
...
def test_atlstatic_ATL_MatchedRule_isRefining_value_roundtrip():
    instance = atlstatic_ATL_MatchedRule(isAbstract="sample_text", isNoDefault="sample_text", isRefining="sample_text")
...
def test_atlstatic_ATL_LazyMatchedRule_isa_MatchedRule():
    instance = atlstatic_ATL_LazyMatchedRule(isUnique="sample_text")
    assert isinstance(instance, MatchedRule)


def test_atlstatic_ATL_Helper_isa_ModuleElement():
...
def test_atlstatic_ATL_CalledRule_isa_Rule():
    instance = atlstatic_ATL_CalledRule(isEndpoint="sample_text", isEntrypoint="sample_text")
    assert isinstance(instance, Rule)


def test_atlstatic_ATL_MatchedRule_isa_Rule():
    instance = atlstatic_ATL_MatchedRule(isAbstract="sample_text", isNoDefault="sample_text", isRefining="sample_text")
    assert isinstance(instance, Rule)
...
def test_assoc_arguments125_link_reassign_clear():
...
def test_assoc_attribute162_link_reassign_clear():
    a = atlstatic_OCL_OclType(name="sample_text")
    b1 = Attribute()
    b2 = Attribute()
    _safe_set(a, 'type163', b1)
    assert _is_linked(a, 'type163', b1)
    if hasattr(b1, 'Attribute164'):
        assert _is_linked(b1, 'Attribute164', a)
    _safe_set(a, 'type163', b2)
    assert _is_linked(a, 'type163', b2)
    if hasattr(b1, 'Attribute164'):
        assert not _is_linked(b1, 'Attribute164', a)
    if hasattr(b2, 'Attribute164'):
        assert _is_linked(b2, 'Attribute164', a)
    _safe_set(a, 'type163', None)
    assert not _is_linked(a, 'type163', b2)
    if hasattr(b2, 'Attribute164'):
        assert not _is_linked(b2, 'Attribute164', a)


def test_assoc_baseExp150_link_reassign_clear():
...
def test_assoc_children22_link_reassign_clear():
    a = atlstatic_ATL_MatchedRule(isAbstract="sample_text", isNoDefault="sample_text", isRefining="sample_text")
    b1 = MatchedRule()
    b2 = MatchedRule()
...
def test_assoc_collectionTypes167_link_reassign_clear():
    a = atlstatic_OCL_OclType(name="sample_text")
    b1 = CollectionType()
    b2 = CollectionType()
...
def test_assoc_definitions156_link_reassign_clear():
    a = atlstatic_OCL_OclType(name="sample_text")
...
def test_assoc_initExpression146_link_reassign_clear():
    a = atlstatic_OCL_VariableDeclaration(id="sample_text", varName="sample_text")
    b1 = OclExpression()
    b2 = OclExpression()
...
def test_assoc_initExpression196_link_reassign_clear():
    a = atlstatic_OCL_Attribute(name="sample_text")
    b1 = OclExpression()
    b2 = OclExpression()
    _safe_set(a, 'owningAttribute', b1)
    assert _is_linked(a, 'owningAttribute', b1)
    if hasattr(b1, 'OclExpression197'):
        assert _is_linked(b1, 'OclExpression197', a)
    _safe_set(a, 'owningAttribute', b2)
    assert _is_linked(a, 'owningAttribute', b2)
    if hasattr(b1, 'OclExpression197'):
        assert not _is_linked(b1, 'OclExpression197', a)
    if hasattr(b2, 'OclExpression197'):
        assert _is_linked(b2, 'OclExpression197', a)
    _safe_set(a, 'owningAttribute', None)
    assert not _is_linked(a, 'owningAttribute', b2)
    if hasattr(b2, 'OclExpression197'):
        assert not _is_linked(b2, 'OclExpression197', a)


def test_assoc_letExp148_link_reassign_clear():
    a = atlstatic_OCL_VariableDeclaration(id="sample_text", varName="sample_text")
    b1 = LetExp()
    b2 = LetExp()
    _safe_set(a, 'variable', b1)
    assert _is_linked(a, 'variable', b1)
    if hasattr(b1, 'LetExp149'):
        assert _is_linked(b1, 'LetExp149', a)
    _safe_set(a, 'variable', b2)
    assert _is_linked(a, 'variable', b2)
    if hasattr(b1, 'LetExp149'):
        assert not _is_linked(b1, 'LetExp149', a)
    if hasattr(b2, 'LetExp149'):
        assert _is_linked(b2, 'LetExp149', a)
    _safe_set(a, 'variable', None)
    assert not _is_linked(a, 'variable', b2)
    if hasattr(b2, 'LetExp149'):
        assert not _is_linked(b2, 'LetExp149', a)
...
def test_assoc_mapType165_link_reassign_clear():
    a = atlstatic_OCL_OclType(name="sample_text")
    b1 = MapType()
    b2 = MapType()
    _safe_set(a, 'keyType', b1)
    assert _is_linked(a, 'keyType', b1)
    if hasattr(b1, 'MapType166'):
        assert _is_linked(b1, 'MapType166', a)
    _safe_set(a, 'keyType', b2)
    assert _is_linked(a, 'keyType', b2)
    if hasattr(b1, 'MapType166'):
        assert not _is_linked(b1, 'MapType166', a)
    if hasattr(b2, 'MapType166'):
        assert _is_linked(b2, 'MapType166', a)
    _safe_set(a, 'keyType', None)
    assert not _is_linked(a, 'keyType', b2)
    if hasattr(b2, 'MapType166'):
        assert not _is_linked(b2, 'MapType166', a)


def test_assoc_mapType2161_link_reassign_clear():
    a = atlstatic_OCL_OclType(name="sample_text")
    b1 = MapType()
    b2 = MapType()
...
def test_assoc_oclExpression157_link_reassign_clear():
    a = atlstatic_OCL_OclType(name="sample_text")
    b1 = OclExpression()
    b2 = OclExpression()
    _safe_set(a, 'type', b1)
...
def test_assoc_operation159_link_reassign_clear():
    a = atlstatic_OCL_OclType(name="sample_text")
    b1 = Operation()
    b2 = Operation()
    _safe_set(a, 'returnType', b1)
    assert _is_linked(a, 'returnType', b1)
    if hasattr(b1, 'Operation160'):
        assert _is_linked(b1, 'Operation160', a)
    _safe_set(a, 'returnType', b2)
    assert _is_linked(a, 'returnType', b2)
    if hasattr(b1, 'Operation160'):
        assert not _is_linked(b1, 'Operation160', a)
    if hasattr(b2, 'Operation160'):
        assert _is_linked(b2, 'Operation160', a)
    _safe_set(a, 'returnType', None)
    assert not _is_linked(a, 'returnType', b2)
    if hasattr(b2, 'Operation160'):
        assert not _is_linked(b2, 'Operation160', a)
...
def test_assoc_outPatternElement61_link_reassign_clear():
    ...
    if hasattr(b1, 'OutPatternElement62'):
        assert _is_linked(b1, 'OutPatternElement62', a)
    _safe_set(a, 'bindings', b2)
    assert _is_linked(a, 'bindings', b2)
    if hasattr(b1, 'OutPatternElement62'):
        assert not _is_linked(b1, 'OutPatternElement62', a)
    if hasattr(b2, 'OutPatternElement62'):
        assert _is_linked(b2, 'OutPatternElement62', a)
    _safe_set(a, 'bindings', None)
    assert not _is_linked(a, 'bindings', b2)
    if hasattr(b2, 'OutPatternElement62'):
        assert not _is_linked(b2, 'OutPatternElement62', a)


def test_assoc_parameters200_link_reassign_clear():
    a = atlstatic_OCL_Operation(name="sample_text")
    b1 = Parameter()
    b2 = Parameter()
    _safe_set(a, 'atlstatic_OCL_Operation', {b1})
    assert _is_linked(a, 'atlstatic_OCL_Operation', b1)
    if hasattr(b1, 'Parameter201'):
        assert _is_linked(b1, 'Parameter201', a)
    _safe_set(a, 'atlstatic_OCL_Operation', {b2})
    assert _is_linked(a, 'atlstatic_OCL_Operation', b2)
    if hasattr(b1, 'Parameter201'):
        assert not _is_linked(b1, 'Parameter201', a)
    if hasattr(b2, 'Parameter201'):
        assert _is_linked(b2, 'Parameter201', a)
    _safe_set(a, 'atlstatic_OCL_Operation', set())
    assert not _is_linked(a, 'atlstatic_OCL_Operation', b2)
    if hasattr(b2, 'Parameter201'):
        assert not _is_linked(b2, 'Parameter201', a)
...
def test_assoc_returnType202_link_reassign_clear():
    a = atlstatic_OCL_Operation(name="sample_text")
    b1 = OclType()
    b2 = OclType()
...
def test_assoc_tupleType177_link_reassign_clear():
...
def test_assoc_type144_link_reassign_clear():
    ...
    if hasattr(b1, 'OclType145'):
        assert _is_linked(b1, 'OclType145', a)
    _safe_set(a, 'variableDeclaration', b2)
    assert _is_linked(a, 'variableDeclaration', b2)
    if hasattr(b1, 'OclType145'):
        assert not _is_linked(b1, 'OclType145', a)
    if hasattr(b2, 'OclType145'):
        assert _is_linked(b2, 'OclType145', a)
    _safe_set(a, 'variableDeclaration', None)
    assert not _is_linked(a, 'variableDeclaration', b2)
    if hasattr(b2, 'OclType145'):
        assert not _is_linked(b2, 'OclType145', a)
...
def test_assoc_type175_link_reassign_clear():
    ...
    if hasattr(b1, 'OclType176'):
        assert _is_linked(b1, 'OclType176', a)
    _safe_set(a, 'tupleTypeAttribute', b2)
    assert _is_linked(a, 'tupleTypeAttribute', b2)
    if hasattr(b1, 'OclType176'):
        assert not _is_linked(b1, 'OclType176', a)
    if hasattr(b2, 'OclType176'):
        assert _is_linked(b2, 'OclType176', a)
    _safe_set(a, 'tupleTypeAttribute', None)
    assert not _is_linked(a, 'tupleTypeAttribute', b2)
    if hasattr(b2, 'OclType176'):
        assert not _is_linked(b2, 'OclType176', a)


def test_assoc_type198_link_reassign_clear():
    a = atlstatic_OCL_Attribute(name="sample_text")
    b1 = OclType()
    b2 = OclType()
...
def test_assoc_unit65_link_reassign_clear():
...
def test_assoc_value59_link_reassign_clear():
    ...
    if hasattr(b1, 'OclExpression60'):
        assert _is_linked(b1, 'OclExpression60', a)
    _safe_set(a, 'atlstatic_ATL_Binding', b2)
    assert _is_linked(a, 'atlstatic_ATL_Binding', b2)
    if hasattr(b1, 'OclExpression60'):
        assert not _is_linked(b1, 'OclExpression60', a)
    if hasattr(b2, 'OclExpression60'):
        assert _is_linked(b2, 'OclExpression60', a)
    _safe_set(a, 'atlstatic_ATL_Binding', None)
    assert not _is_linked(a, 'atlstatic_ATL_Binding', b2)
    if hasattr(b2, 'OclExpression60'):
        assert not _is_linked(b2, 'OclExpression60', a)
...
def test_assoc_value73_link_reassign_clear():
    ...
    b1 = OclExpression()
    b2 = OclExpression()
    _safe_set(a, 'atlstatic_ATL_BindingStat74', b1)
    assert _is_linked(a, 'atlstatic_ATL_BindingStat74', b1)
    if hasattr(b1, 'OclExpression75'):
        assert _is_linked(b1, 'OclExpression75', a)
    _safe_set(a, 'atlstatic_ATL_BindingStat74', b2)
    assert _is_linked(a, 'atlstatic_ATL_BindingStat74', b2)
    if hasattr(b1, 'OclExpression75'):
        assert not _is_linked(b1, 'OclExpression75', a)
    if hasattr(b2, 'OclExpression75'):
        assert _is_linked(b2, 'OclExpression75', a)
    _safe_set(a, 'atlstatic_ATL_BindingStat74', None)
    assert not _is_linked(a, 'atlstatic_ATL_BindingStat74', b2)
    if hasattr(b2, 'OclExpression75'):
        assert not _is_linked(b2, 'OclExpression75', a)


def test_assoc_variableDeclaration170_link_reassign_clear():
    a = atlstatic_OCL_OclType(name="sample_text")
    b1 = VariableDeclaration()
    b2 = VariableDeclaration()
    _safe_set(a, 'type171', b1)
    assert _is_linked(a, 'type171', b1)
    if hasattr(b1, 'VariableDeclaration172'):
        assert _is_linked(b1, 'VariableDeclaration172', a)
    _safe_set(a, 'type171', b2)
    assert _is_linked(a, 'type171', b2)
    if hasattr(b1, 'VariableDeclaration172'):
        assert not _is_linked(b1, 'VariableDeclaration172', a)
    if hasattr(b2, 'VariableDeclaration172'):
        assert _is_linked(b2, 'VariableDeclaration172', a)
    _safe_set(a, 'type171', None)
    assert not _is_linked(a, 'type171', b2)
    if hasattr(b2, 'VariableDeclaration172'):
        assert not _is_linked(b2, 'VariableDeclaration172', a)


def test_assoc_variableExp151_link_reassign_clear():
    a = atlstatic_OCL_VariableDeclaration(id="sample_text", varName="sample_text")
    b1 = VariableExp()
    b2 = VariableExp()
...
atlstatic_ATL_LazyMatchedRule_strategy = st.builds(atlstatic_ATL_LazyMatchedRule, isUnique=safe_text)
@given(instance=atlstatic_ATL_LazyMatchedRule_strategy)
@settings(max_examples=25)
...
atlstatic_ATL_MatchedRule_strategy = st.builds(atlstatic_ATL_MatchedRule, isAbstract=safe_text, isNoDefault=safe_text, isRefining=safe_text)
@given(instance=atlstatic_ATL_MatchedRule_strategy)
@settings(max_examples=25)
