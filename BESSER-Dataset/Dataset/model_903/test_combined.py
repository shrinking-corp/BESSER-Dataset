...
def test_hyp_spem_activity_endnode_is_not_abstract():
    assert not inspect.isabstract(spem_activity_EndNode)


def test_hyp_spem_activity_endnode_constructor_exists():
    assert callable(spem_activity_EndNode.__init__)


def test_hyp_spem_activity_endnode_constructor_args():
    sig = inspect.signature(spem_activity_EndNode.__init__)
    params = list(sig.parameters.keys())



def test_hyp_spem_activity_joinnode_is_not_abstract():
    assert not inspect.isabstract(spem_activity_JoinNode)


def test_hyp_spem_activity_joinnode_constructor_exists():
    assert callable(spem_activity_JoinNode.__init__)


def test_hyp_spem_activity_joinnode_constructor_args():
    sig = inspect.signature(spem_activity_JoinNode.__init__)
    params = list(sig.parameters.keys())



def test_hyp_spem_activity_forknode_is_not_abstract():
    assert not inspect.isabstract(spem_activity_ForkNode)


def test_hyp_spem_activity_forknode_constructor_exists():
    assert callable(spem_activity_ForkNode.__init__)


def test_hyp_spem_activity_forknode_constructor_args():
    sig = inspect.signature(spem_activity_ForkNode.__init__)
    params = list(sig.parameters.keys())



def test_hyp_spem_activity_decisionnode_is_not_abstract():
    assert not inspect.isabstract(spem_activity_DecisionNode)


def test_hyp_spem_activity_decisionnode_constructor_exists():
    assert callable(spem_activity_DecisionNode.__init__)


def test_hyp_spem_activity_decisionnode_constructor_args():
    sig = inspect.signature(spem_activity_DecisionNode.__init__)
    params = list(sig.parameters.keys())



def test_hyp_spem_activity_startnode_is_not_abstract():
    assert not inspect.isabstract(spem_activity_StartNode)


def test_hyp_spem_activity_startnode_constructor_exists():
    assert callable(spem_activity_StartNode.__init__)


def test_hyp_spem_activity_startnode_constructor_args():
    sig = inspect.signature(spem_activity_StartNode.__init__)
    params = list(sig.parameters.keys())



def test_hyp_activity_spem_breakdownelement_is_not_abstract():
    assert not inspect.isabstract(activity_spem_BreakdownElement)


def test_hyp_activity_spem_breakdownelement_constructor_exists():
    assert callable(activity_spem_BreakdownElement.__init__)


def test_hyp_activity_spem_breakdownelement_constructor_args():
...
def test_hyp_spem_activity_edge_is_not_abstract():
    assert not inspect.isabstract(spem_activity_Edge)


def test_hyp_spem_activity_edge_constructor_exists():
    assert callable(spem_activity_Edge.__init__)


def test_hyp_spem_activity_edge_constructor_args():
...
def test_hyp_spem_activity_node_is_not_abstract():
    assert not inspect.isabstract(spem_activity_Node)


def test_hyp_spem_activity_node_constructor_exists():
    assert callable(spem_activity_Node.__init__)


def test_hyp_spem_activity_node_constructor_args():
...
def test_hyp_spem_activity_edge_guard_setter(instance):
...
def test_spem_activity_Edge_guard_value_roundtrip():
    instance = spem_activity_Edge(guard="sample_text")
...
def test_spem_activity_Edge_isa_BreakdownElement():
    instance = spem_activity_Edge(guard="sample_text")
    assert isinstance(instance, BreakdownElement)


def test_spem_activity_Node_isa_BreakdownElement():
    instance = spem_activity_Node()
    assert isinstance(instance, BreakdownElement)
...
def test_spem_activity_DecisionNode_isa_Node():
    instance = spem_activity_DecisionNode()
    assert isinstance(instance, Node)


def test_spem_activity_EndNode_isa_Node():
    instance = spem_activity_EndNode()
    assert isinstance(instance, Node)


def test_spem_activity_ForkNode_isa_Node():
    instance = spem_activity_ForkNode()
    assert isinstance(instance, Node)


def test_spem_activity_JoinNode_isa_Node():
    instance = spem_activity_JoinNode()
    assert isinstance(instance, Node)


def test_spem_activity_StartNode_isa_Node():
    instance = spem_activity_StartNode()
    assert isinstance(instance, Node)
...
def test_assoc_source211_link_reassign_clear():
    a = spem_activity_Edge(guard="sample_text")
    b1 = activity_spem_BreakdownElement()
    b2 = activity_spem_BreakdownElement()
    _safe_set(a, 'spem_activity_Edge', b1)
    assert _is_linked(a, 'spem_activity_Edge', b1)
    if hasattr(b1, 'activity_spem_BreakdownElement'):
        assert _is_linked(b1, 'activity_spem_BreakdownElement', a)
    _safe_set(a, 'spem_activity_Edge', b2)
    assert _is_linked(a, 'spem_activity_Edge', b2)
    if hasattr(b1, 'activity_spem_BreakdownElement'):
        assert not _is_linked(b1, 'activity_spem_BreakdownElement', a)
    if hasattr(b2, 'activity_spem_BreakdownElement'):
        assert _is_linked(b2, 'activity_spem_BreakdownElement', a)
    _safe_set(a, 'spem_activity_Edge', None)
    assert not _is_linked(a, 'spem_activity_Edge', b2)
    if hasattr(b2, 'activity_spem_BreakdownElement'):
        assert not _is_linked(b2, 'activity_spem_BreakdownElement', a)
...
def test_assoc_step64_link_reassign_clear():
    a = spem_Step(name="sample_text")
    b1 = spem_TaskDefinition()
    b2 = spem_TaskDefinition()
    _safe_set(a, 'spem_Step', b1)
    assert _is_linked(a, 'spem_Step', b1)
    if hasattr(b1, 'spem_TaskDefinition65'):
        assert _is_linked(b1, 'spem_TaskDefinition65', a)
    _safe_set(a, 'spem_Step', b2)
    assert _is_linked(a, 'spem_Step', b2)
    if hasattr(b1, 'spem_TaskDefinition65'):
        assert not _is_linked(b1, 'spem_TaskDefinition65', a)
    if hasattr(b2, 'spem_TaskDefinition65'):
        assert _is_linked(b2, 'spem_TaskDefinition65', a)
    _safe_set(a, 'spem_Step', None)
    assert not _is_linked(a, 'spem_Step', b2)
    if hasattr(b2, 'spem_TaskDefinition65'):
        assert not _is_linked(b2, 'spem_TaskDefinition65', a)
...
def test_assoc_target212_link_reassign_clear():
    a = spem_activity_Edge(guard="sample_text")
    b1 = activity_spem_BreakdownElement()
    b2 = activity_spem_BreakdownElement()
    _safe_set(a, 'spem_activity_Edge213', b1)
    assert _is_linked(a, 'spem_activity_Edge213', b1)
    if hasattr(b1, 'activity_spem_BreakdownElement214'):
        assert _is_linked(b1, 'activity_spem_BreakdownElement214', a)
    _safe_set(a, 'spem_activity_Edge213', b2)
    assert _is_linked(a, 'spem_activity_Edge213', b2)
    if hasattr(b1, 'activity_spem_BreakdownElement214'):
        assert not _is_linked(b1, 'activity_spem_BreakdownElement214', a)
    if hasattr(b2, 'activity_spem_BreakdownElement214'):
        assert _is_linked(b2, 'activity_spem_BreakdownElement214', a)
    _safe_set(a, 'spem_activity_Edge213', None)
    assert not _is_linked(a, 'spem_activity_Edge213', b2)
    if hasattr(b2, 'activity_spem_BreakdownElement214'):
        assert not _is_linked(b2, 'activity_spem_BreakdownElement214', a)
...
activity_spem_BreakdownElement_strategy = st.builds(activity_spem_BreakdownElement)
...
spem_activity_DecisionNode_strategy = st.builds(spem_activity_DecisionNode)
...
spem_activity_Edge_strategy = st.builds(spem_activity_Edge, guard=safe_text)
@given(instance=spem_activity_Edge_strategy)
@settings(max_examples=25)
...
spem_activity_EndNode_strategy = st.builds(spem_activity_EndNode)
...
spem_activity_ForkNode_strategy = st.builds(spem_activity_ForkNode)
...
spem_activity_JoinNode_strategy = st.builds(spem_activity_JoinNode)
...
spem_activity_Node_strategy = st.builds(spem_activity_Node)
...
spem_activity_StartNode_strategy = st.builds(spem_activity_StartNode)
