...
def test_hyp_b_program_is_not_abstract():
    assert not inspect.isabstract(b_Program)


def test_hyp_b_program_constructor_exists():
    assert callable(b_Program.__init__)


def test_hyp_b_program_constructor_args():
...
def test_assoc_left181_link_reassign_clear():
    ...
    if hasattr(b1, 'b_LogicalExpr182'):
        assert _is_linked(b1, 'b_LogicalExpr182', a)
    _safe_set(a, 'b_InequalityExpr', b2)
    assert _is_linked(a, 'b_InequalityExpr', b2)
    if hasattr(b1, 'b_LogicalExpr182'):
        assert not _is_linked(b1, 'b_LogicalExpr182', a)
    if hasattr(b2, 'b_LogicalExpr182'):
        assert _is_linked(b2, 'b_LogicalExpr182', a)
    _safe_set(a, 'b_InequalityExpr', None)
    assert not _is_linked(a, 'b_InequalityExpr', b2)
    if hasattr(b2, 'b_LogicalExpr182'):
        assert not _is_linked(b2, 'b_LogicalExpr182', a)
...
def test_assoc_op151_link_reassign_clear():
    a = b_Operation(name="sample_text")
    b1 = b_Call()
    b2 = b_Call()
    _safe_set(a, 'b_Operation153', b1)
    assert _is_linked(a, 'b_Operation153', b1)
    if hasattr(b1, 'b_Call152'):
        assert _is_linked(b1, 'b_Call152', a)
    _safe_set(a, 'b_Operation153', b2)
    assert _is_linked(a, 'b_Operation153', b2)
    if hasattr(b1, 'b_Call152'):
        assert not _is_linked(b1, 'b_Call152', a)
    if hasattr(b2, 'b_Call152'):
        assert _is_linked(b2, 'b_Call152', a)
    _safe_set(a, 'b_Operation153', None)
    assert not _is_linked(a, 'b_Operation153', b2)
    if hasattr(b2, 'b_Call152'):
        assert not _is_linked(b2, 'b_Call152', a)


def test_assoc_op157_link_reassign_clear():
    a = b_Operation(name="sample_text")
    b1 = b_SimpleCall()
    b2 = b_SimpleCall()
    _safe_set(a, 'b_Operation158', b1)
    assert _is_linked(a, 'b_Operation158', b1)
    if hasattr(b1, 'b_SimpleCall'):
        assert _is_linked(b1, 'b_SimpleCall', a)
    _safe_set(a, 'b_Operation158', b2)
    assert _is_linked(a, 'b_Operation158', b2)
    if hasattr(b1, 'b_SimpleCall'):
        assert not _is_linked(b1, 'b_SimpleCall', a)
    if hasattr(b2, 'b_SimpleCall'):
        assert _is_linked(b2, 'b_SimpleCall', a)
    _safe_set(a, 'b_Operation158', None)
    assert not _is_linked(a, 'b_Operation158', b2)
    if hasattr(b2, 'b_SimpleCall'):
        assert not _is_linked(b2, 'b_SimpleCall', a)


def test_assoc_operations162_link_reassign_clear():
    a = b_Operation(name="sample_text")
    b1 = b_LocalOperations()
    b2 = b_LocalOperations()
    _safe_set(a, 'b_Operation164', b1)
    assert _is_linked(a, 'b_Operation164', b1)
    if hasattr(b1, 'b_LocalOperations163'):
        assert _is_linked(b1, 'b_LocalOperations163', a)
    _safe_set(a, 'b_Operation164', b2)
    assert _is_linked(a, 'b_Operation164', b2)
    if hasattr(b1, 'b_LocalOperations163'):
        assert not _is_linked(b1, 'b_LocalOperations163', a)
    if hasattr(b2, 'b_LocalOperations163'):
        assert _is_linked(b2, 'b_LocalOperations163', a)
    _safe_set(a, 'b_Operation164', None)
    assert not _is_linked(a, 'b_Operation164', b2)
    if hasattr(b2, 'b_LocalOperations163'):
        assert not _is_linked(b2, 'b_LocalOperations163', a)
...
def test_assoc_range172_link_reassign_clear():
    a = b_Range(lowerBound=7)
    b1 = b_PropertyRange()
    b2 = b_PropertyRange()
    _safe_set(a, 'b_Range173', b1)
    assert _is_linked(a, 'b_Range173', b1)
    if hasattr(b1, 'b_PropertyRange'):
        assert _is_linked(b1, 'b_PropertyRange', a)
    _safe_set(a, 'b_Range173', b2)
    assert _is_linked(a, 'b_Range173', b2)
    if hasattr(b1, 'b_PropertyRange'):
        assert not _is_linked(b1, 'b_PropertyRange', a)
    if hasattr(b2, 'b_PropertyRange'):
        assert _is_linked(b2, 'b_PropertyRange', a)
    _safe_set(a, 'b_Range173', None)
    assert not _is_linked(a, 'b_Range173', b2)
    if hasattr(b2, 'b_PropertyRange'):
        assert not _is_linked(b2, 'b_PropertyRange', a)


def test_assoc_rets149_link_reassign_clear():
    a = b_Variable(name="sample_text")
    b1 = b_Call()
    b2 = b_Call()
    _safe_set(a, 'b_Variable150', b1)
    assert _is_linked(a, 'b_Variable150', b1)
    if hasattr(b1, 'b_Call'):
        assert _is_linked(b1, 'b_Call', a)
    _safe_set(a, 'b_Variable150', b2)
    assert _is_linked(a, 'b_Variable150', b2)
    if hasattr(b1, 'b_Call'):
        assert not _is_linked(b1, 'b_Call', a)
    if hasattr(b2, 'b_Call'):
        assert _is_linked(b2, 'b_Call', a)
    _safe_set(a, 'b_Variable150', None)
    assert not _is_linked(a, 'b_Variable150', b2)
    if hasattr(b2, 'b_Call'):
...
def test_assoc_right183_link_reassign_clear():
    ...
    b1 = b_LogicalExpr()
    b2 = b_LogicalExpr()
    _safe_set(a, 'b_InequalityExpr184', b1)
    assert _is_linked(a, 'b_InequalityExpr184', b1)
    if hasattr(b1, 'b_LogicalExpr185'):
        assert _is_linked(b1, 'b_LogicalExpr185', a)
    _safe_set(a, 'b_InequalityExpr184', b2)
    assert _is_linked(a, 'b_InequalityExpr184', b2)
    if hasattr(b1, 'b_LogicalExpr185'):
        assert not _is_linked(b1, 'b_LogicalExpr185', a)
    if hasattr(b2, 'b_LogicalExpr185'):
        assert _is_linked(b2, 'b_LogicalExpr185', a)
    _safe_set(a, 'b_InequalityExpr184', None)
    assert not _is_linked(a, 'b_InequalityExpr184', b2)
    if hasattr(b2, 'b_LogicalExpr185'):
        assert not _is_linked(b2, 'b_LogicalExpr185', a)
...
def test_assoc_type165_link_reassign_clear():
    ...
    b1 = b_Ref()
    b2 = b_Ref()
    _safe_set(a, 'b_Variable166', b1)
    assert _is_linked(a, 'b_Variable166', b1)
    if hasattr(b1, 'b_Ref'):
        assert _is_linked(b1, 'b_Ref', a)
    _safe_set(a, 'b_Variable166', b2)
    assert _is_linked(a, 'b_Variable166', b2)
    if hasattr(b1, 'b_Ref'):
        assert not _is_linked(b1, 'b_Ref', a)
    if hasattr(b2, 'b_Ref'):
        assert _is_linked(b2, 'b_Ref', a)
    _safe_set(a, 'b_Variable166', None)
    assert not _is_linked(a, 'b_Variable166', b2)
    if hasattr(b2, 'b_Ref'):
...
def test_assoc_var167_link_reassign_clear():
    a = b_Variable(name="sample_text")
    b1 = b_Ref()
    b2 = b_Ref()
    _safe_set(a, 'b_Variable169', b1)
    assert _is_linked(a, 'b_Variable169', b1)
    if hasattr(b1, 'b_Ref168'):
        assert _is_linked(b1, 'b_Ref168', a)
    _safe_set(a, 'b_Variable169', b2)
    assert _is_linked(a, 'b_Variable169', b2)
    if hasattr(b1, 'b_Ref168'):
        assert not _is_linked(b1, 'b_Ref168', a)
    if hasattr(b2, 'b_Ref168'):
        assert _is_linked(b2, 'b_Ref168', a)
    _safe_set(a, 'b_Variable169', None)
    assert not _is_linked(a, 'b_Variable169', b2)
    if hasattr(b2, 'b_Ref168'):
        assert not _is_linked(b2, 'b_Ref168', a)


def test_assoc_var195_link_reassign_clear():
    a = b_Variable(name="sample_text")
    b1 = b_TypeConstraint()
    b2 = b_TypeConstraint()
    _safe_set(a, 'b_Variable196', b1)
    assert _is_linked(a, 'b_Variable196', b1)
    if hasattr(b1, 'b_TypeConstraint'):
        assert _is_linked(b1, 'b_TypeConstraint', a)
    _safe_set(a, 'b_Variable196', b2)
    assert _is_linked(a, 'b_Variable196', b2)
    if hasattr(b1, 'b_TypeConstraint'):
        assert not _is_linked(b1, 'b_TypeConstraint', a)
    if hasattr(b2, 'b_TypeConstraint'):
        assert _is_linked(b2, 'b_TypeConstraint', a)
    _safe_set(a, 'b_Variable196', None)
    assert not _is_linked(a, 'b_Variable196', b2)
    if hasattr(b2, 'b_TypeConstraint'):
...
def test_assoc_vars219_link_reassign_clear():
    a = b_Variable(name="sample_text")
    b1 = b_ReturnTuple()
    b2 = b_ReturnTuple()
    _safe_set(a, 'b_Variable220', b1)
    assert _is_linked(a, 'b_Variable220', b1)
    if hasattr(b1, 'b_ReturnTuple'):
        assert _is_linked(b1, 'b_ReturnTuple', a)
    _safe_set(a, 'b_Variable220', b2)
    assert _is_linked(a, 'b_Variable220', b2)
    if hasattr(b1, 'b_ReturnTuple'):
        assert not _is_linked(b1, 'b_ReturnTuple', a)
    if hasattr(b2, 'b_ReturnTuple'):
        assert _is_linked(b2, 'b_ReturnTuple', a)
    _safe_set(a, 'b_Variable220', None)
    assert not _is_linked(a, 'b_Variable220', b2)
    if hasattr(b2, 'b_ReturnTuple'):
...
b_Program_strategy = st.builds(b_Program)
