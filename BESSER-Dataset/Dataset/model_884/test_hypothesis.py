...
def test_simplepdl_need_is_not_abstract():
    assert not inspect.isabstract(simplepdl_Need)


def test_simplepdl_need_constructor_exists():
    assert callable(simplepdl_Need.__init__)


def test_simplepdl_need_constructor_args():
    sig = inspect.signature(simplepdl_Need.__init__)
    params = list(sig.parameters.keys())
    assert "quantity" in params, "Missing parameter 'quantity'"

def test_simplepdl_need_has_quantity():
    assert hasattr(simplepdl_Need, "quantity")
    descriptor = None
    for klass in simplepdl_Need.__mro__:
...
def test_simplepdl_needset_is_not_abstract():
    assert not inspect.isabstract(simplepdl_NeedSet)


def test_simplepdl_needset_constructor_exists():
    assert callable(simplepdl_NeedSet.__init__)


def test_simplepdl_needset_constructor_args():
    sig = inspect.signature(simplepdl_NeedSet.__init__)
    params = list(sig.parameters.keys())
    assert "name" in params, "Missing parameter 'name'"

def test_simplepdl_needset_has_name():
    assert hasattr(simplepdl_NeedSet, "name")
    descriptor = None
    for klass in simplepdl_NeedSet.__mro__:
        if "name" in klass.__dict__:
            descriptor = klass.__dict__["name"]
            break
    assert isinstance(descriptor, property)
...
@given(instance=simplepdl_Need_strategy)
def test_simplepdl_need_quantity_setter(instance):
...
@given(instance=simplepdl_NeedSet_strategy)
def test_simplepdl_needset_name_setter(instance):
    original = instance.name
    instance.name = original
    assert instance.name == original
