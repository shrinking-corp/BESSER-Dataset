...
def test_hyp_simplepdl_need_is_not_abstract():
    assert not inspect.isabstract(simplepdl_Need)


def test_hyp_simplepdl_need_constructor_exists():
    assert callable(simplepdl_Need.__init__)


def test_hyp_simplepdl_need_constructor_args():
    sig = inspect.signature(simplepdl_Need.__init__)
    params = list(sig.parameters.keys())
...
def test_hyp_simplepdl_needset_is_not_abstract():
    assert not inspect.isabstract(simplepdl_NeedSet)


def test_hyp_simplepdl_needset_constructor_exists():
    assert callable(simplepdl_NeedSet.__init__)


def test_hyp_simplepdl_needset_constructor_args():
    sig = inspect.signature(simplepdl_NeedSet.__init__)
    params = list(sig.parameters.keys())
    assert "name" in params, "Missing parameter 'name'"
...
simplepdl_Need_strategy = st.builds(
    simplepdl_Need,
    quantity=
        st.integers()
)
simplepdl_NeedSet_strategy = st.builds(
    simplepdl_NeedSet,
    name=
        safe_text
)
...
@given(instance=simplepdl_Need_strategy)
def test_hyp_simplepdl_need_quantity_setter(instance):
...
@given(instance=simplepdl_NeedSet_strategy)
def test_hyp_simplepdl_needset_name_setter(instance):
    original = instance.name
    instance.name = original
    assert instance.name == original
...
def test_simplepdl_Need_quantity_value_roundtrip():
    instance = simplepdl_Need(quantity=7)
    assert instance.quantity == 7
    instance.quantity = 13
    assert instance.quantity == 13


def test_simplepdl_NeedSet_name_value_roundtrip():
    instance = simplepdl_NeedSet(name="sample_text")
...
def test_simplepdl_Need_isa_ProcessElement():
    instance = simplepdl_Need(quantity=7)
    assert isinstance(instance, ProcessElement)


def test_simplepdl_NeedSet_isa_ProcessElement():
    instance = simplepdl_NeedSet(name="sample_text")
    assert isinstance(instance, ProcessElement)
...
def test_assoc_needSet11_link_reassign_clear():
    a = simplepdl_NeedSet(name="sample_text")
    b1 = simplepdl_Need(quantity=7)
    b2 = simplepdl_Need(quantity=13)
    _safe_set(a, 'NeedSet12', b1)
    assert _is_linked(a, 'NeedSet12', b1)
    if hasattr(b1, 'needs'):
        assert _is_linked(b1, 'needs', a)
    _safe_set(a, 'NeedSet12', b2)
    assert _is_linked(a, 'NeedSet12', b2)
    if hasattr(b1, 'needs'):
        assert not _is_linked(b1, 'needs', a)
    if hasattr(b2, 'needs'):
        assert _is_linked(b2, 'needs', a)
    _safe_set(a, 'NeedSet12', None)
    assert not _is_linked(a, 'NeedSet12', b2)
    if hasattr(b2, 'needs'):
        assert not _is_linked(b2, 'needs', a)


def test_assoc_needSets5_link_reassign_clear():
    a = simplepdl_WorkDefinition(name="sample_text")
    b1 = simplepdl_NeedSet(name="sample_text")
    b2 = simplepdl_NeedSet(name="sample_text_2")
    _safe_set(a, 'wd', {b1})
    assert _is_linked(a, 'wd', b1)
    if hasattr(b1, 'NeedSet'):
        assert _is_linked(b1, 'NeedSet', a)
    _safe_set(a, 'wd', {b2})
    assert _is_linked(a, 'wd', b2)
    if hasattr(b1, 'NeedSet'):
        assert not _is_linked(b1, 'NeedSet', a)
    if hasattr(b2, 'NeedSet'):
        assert _is_linked(b2, 'NeedSet', a)
    _safe_set(a, 'wd', set())
    assert not _is_linked(a, 'wd', b2)
    if hasattr(b2, 'NeedSet'):
        assert not _is_linked(b2, 'NeedSet', a)


def test_assoc_needs13_link_reassign_clear():
    a = simplepdl_NeedSet(name="sample_text")
    b1 = simplepdl_Need(quantity=7)
    b2 = simplepdl_Need(quantity=13)
    _safe_set(a, 'needSet', {b1})
    assert _is_linked(a, 'needSet', b1)
    if hasattr(b1, 'Need'):
        assert _is_linked(b1, 'Need', a)
    _safe_set(a, 'needSet', {b2})
    assert _is_linked(a, 'needSet', b2)
    if hasattr(b1, 'Need'):
        assert not _is_linked(b1, 'Need', a)
    if hasattr(b2, 'Need'):
        assert _is_linked(b2, 'Need', a)
    _safe_set(a, 'needSet', set())
    assert not _is_linked(a, 'needSet', b2)
    if hasattr(b2, 'Need'):
        assert not _is_linked(b2, 'Need', a)
...
def test_assoc_ressource10_link_reassign_clear():
    a = simplepdl_Ressource(name="sample_text", quantity=7)
    b1 = simplepdl_Need(quantity=7)
    b2 = simplepdl_Need(quantity=13)
    _safe_set(a, 'simplepdl_Ressource', b1)
    assert _is_linked(a, 'simplepdl_Ressource', b1)
    if hasattr(b1, 'simplepdl_Need'):
        assert _is_linked(b1, 'simplepdl_Need', a)
    _safe_set(a, 'simplepdl_Ressource', b2)
    assert _is_linked(a, 'simplepdl_Ressource', b2)
    if hasattr(b1, 'simplepdl_Need'):
        assert not _is_linked(b1, 'simplepdl_Need', a)
    if hasattr(b2, 'simplepdl_Need'):
        assert _is_linked(b2, 'simplepdl_Need', a)
    _safe_set(a, 'simplepdl_Ressource', None)
    assert not _is_linked(a, 'simplepdl_Ressource', b2)
    if hasattr(b2, 'simplepdl_Need'):
        assert not _is_linked(b2, 'simplepdl_Need', a)
...
simplepdl_Need_strategy = st.builds(simplepdl_Need, quantity=st.integers())
@given(instance=simplepdl_Need_strategy)
@settings(max_examples=25)
def test_simplepdl_Need_instantiation(instance):
    assert isinstance(instance, simplepdl_Need)


simplepdl_NeedSet_strategy = st.builds(simplepdl_NeedSet, name=safe_text)
@given(instance=simplepdl_NeedSet_strategy)
@settings(max_examples=25)
def test_simplepdl_NeedSet_instantiation(instance):
    assert isinstance(instance, simplepdl_NeedSet)
