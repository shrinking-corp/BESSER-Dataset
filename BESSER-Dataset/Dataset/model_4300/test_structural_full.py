...
def test_assoc_expressionRef14_link_reassign_clear():
    ...
    _safe_set(a, 'metrics_Metric15', b1)
    assert _is_linked(a, 'metrics_Metric15', b1)
    if hasattr(b1, 'metrics_Expression'):
        assert _is_linked(b1, 'metrics_Expression', a)
    _safe_set(a, 'metrics_Metric15', b2)
    assert _is_linked(a, 'metrics_Metric15', b2)
    if hasattr(b1, 'metrics_Expression'):
        assert not _is_linked(b1, 'metrics_Expression', a)
    if hasattr(b2, 'metrics_Expression'):
        assert _is_linked(b2, 'metrics_Expression', a)
    _safe_set(a, 'metrics_Metric15', None)
    assert not _is_linked(a, 'metrics_Metric15', b2)
    if hasattr(b2, 'metrics_Expression'):
...
def test_assoc_metricMapping24_link_reassign_clear():
    a = metrics_MetricSource(filterPattern="sample_text", metricLocation="sample_text", name="sample_text")
    b1 = metrics_Mapping(firstDataRow="sample_text", headerRow="sample_text", intervalHint="sample_text")
    b2 = metrics_Mapping(firstDataRow="sample_text_2", headerRow="sample_text_2", intervalHint="sample_text_2")
    _safe_set(a, 'metrics_MetricSource25', b1)
    assert _is_linked(a, 'metrics_MetricSource25', b1)
    if hasattr(b1, 'metrics_Mapping26'):
        assert _is_linked(b1, 'metrics_Mapping26', a)
    _safe_set(a, 'metrics_MetricSource25', b2)
    assert _is_linked(a, 'metrics_MetricSource25', b2)
    if hasattr(b1, 'metrics_Mapping26'):
        assert not _is_linked(b1, 'metrics_Mapping26', a)
    if hasattr(b2, 'metrics_Mapping26'):
        assert _is_linked(b2, 'metrics_Mapping26', a)
    _safe_set(a, 'metrics_MetricSource25', None)
    assert not _is_linked(a, 'metrics_MetricSource25', b2)
    if hasattr(b2, 'metrics_Mapping26'):
        assert not _is_linked(b2, 'metrics_Mapping26', a)


def test_assoc_metricRef31_link_reassign_clear():
    ...
    if hasattr(b1, 'metrics_Metric32'):
        assert _is_linked(b1, 'metrics_Metric32', a)
    _safe_set(a, 'metrics_ValueDataKind', b2)
    assert _is_linked(a, 'metrics_ValueDataKind', b2)
    if hasattr(b1, 'metrics_Metric32'):
        assert not _is_linked(b1, 'metrics_Metric32', a)
    if hasattr(b2, 'metrics_Metric32'):
        assert _is_linked(b2, 'metrics_Metric32', a)
    _safe_set(a, 'metrics_ValueDataKind', None)
    assert not _is_linked(a, 'metrics_ValueDataKind', b2)
    if hasattr(b2, 'metrics_Metric32'):
        assert not _is_linked(b2, 'metrics_Metric32', a)


def test_assoc_metricRetentionRules22_link_reassign_clear():
    ...
    _safe_set(a, 'metrics_MetricRetentionRule23', b1)
    assert _is_linked(a, 'metrics_MetricRetentionRule23', b1)
    if hasattr(b1, 'metrics_MetricRetentionRules'):
        assert _is_linked(b1, 'metrics_MetricRetentionRules', a)
    _safe_set(a, 'metrics_MetricRetentionRule23', b2)
    assert _is_linked(a, 'metrics_MetricRetentionRule23', b2)
    if hasattr(b1, 'metrics_MetricRetentionRules'):
    ...
    _safe_set(a, 'metrics_MetricRetentionRule23', None)
    assert not _is_linked(a, 'metrics_MetricRetentionRule23', b2)
    if hasattr(b2, 'metrics_MetricRetentionRules'):
        assert not _is_linked(b2, 'metrics_MetricRetentionRules', a)


def test_assoc_metricSourceRef16_link_reassign_clear():
    a = metrics_MetricSource(filterPattern="sample_text", metricLocation="sample_text", name="sample_text")
    b1 = metrics_Metric(description="sample_text", measurementKind="sample_text", measurementPoint="sample_text", name="sample_text")
    b2 = metrics_Metric(description="sample_text_2", measurementKind="sample_text_2", measurementPoint="sample_text_2", name="sample_text_2")
    _safe_set(a, 'metrics_MetricSource', b1)
    assert _is_linked(a, 'metrics_MetricSource', b1)
    if hasattr(b1, 'metrics_Metric17'):
        assert _is_linked(b1, 'metrics_Metric17', a)
    _safe_set(a, 'metrics_MetricSource', b2)
    assert _is_linked(a, 'metrics_MetricSource', b2)
    if hasattr(b1, 'metrics_Metric17'):
        assert not _is_linked(b1, 'metrics_Metric17', a)
    if hasattr(b2, 'metrics_Metric17'):
        assert _is_linked(b2, 'metrics_Metric17', a)
    _safe_set(a, 'metrics_MetricSource', None)
    assert not _is_linked(a, 'metrics_MetricSource', b2)
    if hasattr(b2, 'metrics_Metric17'):
        assert not _is_linked(b2, 'metrics_Metric17', a)


def test_assoc_metricValues30_link_reassign_clear():
...
def test_assoc_metrics13_link_reassign_clear():
    a = metrics_Metric(description="sample_text", measurementKind="sample_text", measurementPoint="sample_text", name="sample_text")
    b1 = metrics_Metric(description="sample_text", measurementKind="sample_text", measurementPoint="sample_text", name="sample_text")
    b2 = metrics_Metric(description="sample_text_2", measurementKind="sample_text_2", measurementPoint="sample_text_2", name="sample_text_2")
    _safe_set(a, 'metrics_Metric', b1)
    assert _is_linked(a, 'metrics_Metric', b1)
    if hasattr(b1, 'metrics_Metric12'):
        assert _is_linked(b1, 'metrics_Metric12', a)
    _safe_set(a, 'metrics_Metric', b2)
    assert _is_linked(a, 'metrics_Metric', b2)
    if hasattr(b1, 'metrics_Metric12'):
        assert not _is_linked(b1, 'metrics_Metric12', a)
    if hasattr(b2, 'metrics_Metric12'):
        assert _is_linked(b2, 'metrics_Metric12', a)
    _safe_set(a, 'metrics_Metric', None)
    assert not _is_linked(a, 'metrics_Metric', b2)
    if hasattr(b2, 'metrics_Metric12'):
        assert not _is_linked(b2, 'metrics_Metric12', a)
...
def test_assoc_retentionExpression20_link_reassign_clear():
    a = metrics_MetricRetentionRule(intervalHint="sample_text", name="sample_text", period="sample_text")
    b1 = metrics_Expression()
    b2 = metrics_Expression()
    _safe_set(a, 'metrics_MetricRetentionRule', b1)
    assert _is_linked(a, 'metrics_MetricRetentionRule', b1)
    if hasattr(b1, 'metrics_Expression21'):
        assert _is_linked(b1, 'metrics_Expression21', a)
    _safe_set(a, 'metrics_MetricRetentionRule', b2)
    assert _is_linked(a, 'metrics_MetricRetentionRule', b2)
    if hasattr(b1, 'metrics_Expression21'):
        assert not _is_linked(b1, 'metrics_Expression21', a)
    if hasattr(b2, 'metrics_Expression21'):
        assert _is_linked(b2, 'metrics_Expression21', a)
    _safe_set(a, 'metrics_MetricRetentionRule', None)
    assert not _is_linked(a, 'metrics_MetricRetentionRule', b2)
    if hasattr(b2, 'metrics_Expression21'):
        assert not _is_linked(b2, 'metrics_Expression21', a)


def test_assoc_statistics27_link_reassign_clear():
    a = metrics_MetricSource(filterPattern="sample_text", metricLocation="sample_text", name="sample_text")
    ...
    if hasattr(b1, 'metrics_MappingStatistic29'):
        assert _is_linked(b1, 'metrics_MappingStatistic29', a)
    _safe_set(a, 'metrics_MetricSource28', {b2})
    assert _is_linked(a, 'metrics_MetricSource28', b2)
    if hasattr(b1, 'metrics_MappingStatistic29'):
        assert not _is_linked(b1, 'metrics_MappingStatistic29', a)
    if hasattr(b2, 'metrics_MappingStatistic29'):
        assert _is_linked(b2, 'metrics_MappingStatistic29', a)
    _safe_set(a, 'metrics_MetricSource28', set())
    assert not _is_linked(a, 'metrics_MetricSource28', b2)
    if hasattr(b2, 'metrics_MappingStatistic29'):
        assert not _is_linked(b2, 'metrics_MappingStatistic29', a)


def test_assoc_unitRef18_link_reassign_clear():
    a = metrics_Metric(description="sample_text", measurementKind="sample_text", measurementPoint="sample_text", name="sample_text")
    b1 = metrics_Unit()
    b2 = metrics_Unit()
    _safe_set(a, 'metrics_Metric19', b1)
    assert _is_linked(a, 'metrics_Metric19', b1)
    if hasattr(b1, 'metrics_Unit'):
        assert _is_linked(b1, 'metrics_Unit', a)
    _safe_set(a, 'metrics_Metric19', b2)
    assert _is_linked(a, 'metrics_Metric19', b2)
    if hasattr(b1, 'metrics_Unit'):
        assert not _is_linked(b1, 'metrics_Unit', a)
    if hasattr(b2, 'metrics_Unit'):
        assert _is_linked(b2, 'metrics_Unit', a)
    _safe_set(a, 'metrics_Metric19', None)
    assert not _is_linked(a, 'metrics_Metric19', b2)
    if hasattr(b2, 'metrics_Unit'):
