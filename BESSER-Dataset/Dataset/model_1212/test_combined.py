...
def test_hyp_collectionoperationcallexp_is_not_abstract():
    assert not inspect.isabstract(CollectionOperationCallExp)


def test_hyp_collectionoperationcallexp_constructor_exists():
    assert callable(CollectionOperationCallExp.__init__)


def test_hyp_collectionoperationcallexp_constructor_args():
    sig = inspect.signature(CollectionOperationCallExp.__init__)
    params = list(sig.parameters.keys())



def test_hyp_atlext_ocl2_selectbykind_is_not_abstract():
    assert not inspect.isabstract(atlext_OCL2_SelectByKind)


def test_hyp_atlext_ocl2_selectbykind_constructor_exists():
    assert callable(atlext_OCL2_SelectByKind.__init__)


def test_hyp_atlext_ocl2_selectbykind_constructor_args():
...
def test_hyp_ruleresolutionstatus_exists():
    # Check that the Enumeration exists
    assert RuleResolutionStatus is not None

def test_hyp_ruleresolutionstatus_has_all_literals():
...
def test_hyp_atlext_atl_ruleresolutioninfo_status_setter(instance):
...
def test_hyp_atlext_ocl2_selectbykind_isExact_setter(instance):
...
def test_assoc_allInvolvedRules26_link_reassign_clear():
    a = atlext_ATL_RuleResolutionInfo(status="sample_text")
    b1 = MatchedRule()
    b2 = MatchedRule()
    _safe_set(a, 'atlext_ATL_RuleResolutionInfo27', {b1})
    assert _is_linked(a, 'atlext_ATL_RuleResolutionInfo27', b1)
    if hasattr(b1, 'MatchedRule28'):
        assert _is_linked(b1, 'MatchedRule28', a)
    _safe_set(a, 'atlext_ATL_RuleResolutionInfo27', {b2})
    assert _is_linked(a, 'atlext_ATL_RuleResolutionInfo27', b2)
    if hasattr(b1, 'MatchedRule28'):
        assert not _is_linked(b1, 'MatchedRule28', a)
    if hasattr(b2, 'MatchedRule28'):
        assert _is_linked(b2, 'MatchedRule28', a)
    _safe_set(a, 'atlext_ATL_RuleResolutionInfo27', set())
    assert not _is_linked(a, 'atlext_ATL_RuleResolutionInfo27', b2)
    if hasattr(b2, 'MatchedRule28'):
        assert not _is_linked(b2, 'MatchedRule28', a)
...
def test_assoc_rule25_link_reassign_clear():
    a = atlext_ATL_RuleResolutionInfo(status="sample_text")
    b1 = MatchedRule()
    b2 = MatchedRule()
