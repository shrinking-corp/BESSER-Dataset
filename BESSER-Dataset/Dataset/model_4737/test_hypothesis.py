...
def test_swml_contentlayer_has_name():
    assert hasattr(swml_ContentLayer, "name")
    descriptor = None
    for klass in swml_ContentLayer.__mro__:
        if "name" in klass.__dict__:
            descriptor = klass.__dict__["name"]
            break
    assert isinstance(descriptor, property)
...
def test_swml_hypertextlayer_has_name():
    assert hasattr(swml_HypertextLayer, "name")
    descriptor = None
    for klass in swml_HypertextLayer.__mro__:
        if "name" in klass.__dict__:
            descriptor = klass.__dict__["name"]
            break
    assert isinstance(descriptor, property)
...
@given(instance=swml_ContentLayer_strategy)
def test_swml_contentlayer_name_setter(instance):
    original = instance.name
    instance.name = original
    assert instance.name == original
...
@given(instance=swml_HypertextLayer_strategy)
def test_swml_hypertextlayer_name_setter(instance):
    original = instance.name
    instance.name = original
    assert instance.name == original
