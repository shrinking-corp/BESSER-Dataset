...
def test_hyp_m_attA_setter(instance):
...
def test_hyp_w_attC1_setter(instance):
...
@given(instance=W_strategy)
def test_hyp_w_attC2_setter(instance):
...
def test_hyp_n_attB_setter(instance):
...
def test_hyp_t_attY_setter(instance):
...
def test_M_attA_value_roundtrip():
    instance = M(attA="sample_text")
    assert instance.attA == "sample_text"
    instance.attA = "sample_text_2"
    assert instance.attA == "sample_text_2"


def test_N_attB_value_roundtrip():
    instance = N(attB=7)
    assert instance.attB == 7
    instance.attB = 13
    assert instance.attB == 13
...
def test_T_attY_value_roundtrip():
    instance = T(attY="sample_text")
    assert instance.attY == "sample_text"
    instance.attY = "sample_text_2"
    assert instance.attY == "sample_text_2"


def test_W_attC1_value_roundtrip():
    instance = W(attC1=7, attC2=True)
    assert instance.attC1 == 7
    instance.attC1 = 13
    assert instance.attC1 == 13


def test_W_attC2_value_roundtrip():
    instance = W(attC1=7, attC2=True)
    assert instance.attC2 == True
    instance.attC2 = False
    assert instance.attC2 == False
...
def test_assoc_Class3_Class4_link_reassign_clear():
    a = W(attC1=7, attC2=True)
    b1 = N(attB=7)
    b2 = N(attB=13)
    _safe_set(a, 'class37', b1)
    assert _is_linked(a, 'class37', b1)
    if hasattr(b1, 'class46'):
        assert _is_linked(b1, 'class46', a)
    _safe_set(a, 'class37', b2)
    assert _is_linked(a, 'class37', b2)
    if hasattr(b1, 'class46'):
        assert not _is_linked(b1, 'class46', a)
    if hasattr(b2, 'class46'):
        assert _is_linked(b2, 'class46', a)
    _safe_set(a, 'class37', None)
    assert not _is_linked(a, 'class37', b2)
    if hasattr(b2, 'class46'):
        assert not _is_linked(b2, 'class46', a)


def test_assoc_Class7_Class3_link_reassign_clear():
    a = N(attB=7)
    b1 = M(attA="sample_text")
    b2 = M(attA="sample_text_2")
    _safe_set(a, 'a5', b1)
...
def test_assoc_c_Class7_link_reassign_clear():
    a = M(attA="sample_text")
    b1 = R()
    b2 = R()
    _safe_set(a, 'c9', b1)
    assert _is_linked(a, 'c9', b1)
    if hasattr(b1, 'aR8'):
        assert _is_linked(b1, 'aR8', a)
    _safe_set(a, 'c9', b2)
    assert _is_linked(a, 'c9', b2)
    if hasattr(b1, 'aR8'):
        assert not _is_linked(b1, 'aR8', a)
    if hasattr(b2, 'aR8'):
        assert _is_linked(b2, 'aR8', a)
    _safe_set(a, 'c9', None)
    assert not _is_linked(a, 'c9', b2)
    if hasattr(b2, 'aR8'):
...
M_strategy = st.builds(M, attA=safe_text)
@given(instance=M_strategy)
@settings(max_examples=25)
...
N_strategy = st.builds(N, attB=st.integers())
@given(instance=N_strategy)
@settings(max_examples=25)
...
T_strategy = st.builds(T, attY=safe_text)
@given(instance=T_strategy)
@settings(max_examples=25)
...
W_strategy = st.builds(W, attC1=st.integers(), attC2=st.booleans())
@given(instance=W_strategy)
@settings(max_examples=25)
