...
class A(ABC):
    ...
    def __init__(self, attA: str, b0: set["B"] = None, r5: set["R"] = None):
        self.attA = attA
        self.b0 = b0 if b0 is not None else set()
        self.r5 = r5 if r5 is not None else set()
    ...
    def r5(self, value):
        ...
        self.__r5 = value if value is not None else set()
        
        # Remove self from old opposite end
        if old_value is not None:
            for item in old_value:
                if hasattr(item, "aR4"):
                    opp_val = getattr(item, "aR4", None)
