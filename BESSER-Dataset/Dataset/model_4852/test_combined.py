...
def test_assoc_baseUnit6_link_reassign_clear():
    ...
    _safe_set(a, 'units_BaseUnit7', b1)
    assert _is_linked(a, 'units_BaseUnit7', b1)
    if hasattr(b1, 'units_UnitLiteral'):
        assert _is_linked(b1, 'units_UnitLiteral', a)
    _safe_set(a, 'units_BaseUnit7', b2)
    assert _is_linked(a, 'units_BaseUnit7', b2)
    if hasattr(b1, 'units_UnitLiteral'):
        assert not _is_linked(b1, 'units_UnitLiteral', a)
    if hasattr(b2, 'units_UnitLiteral'):
        assert _is_linked(b2, 'units_UnitLiteral', a)
    _safe_set(a, 'units_BaseUnit7', None)
    assert not _is_linked(a, 'units_BaseUnit7', b2)
    if hasattr(b2, 'units_UnitLiteral'):
