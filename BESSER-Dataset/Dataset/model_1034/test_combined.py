...
def test_hyp_fsm_constraint_eval_changes_state(instance):
...
def test_assoc_guard15_link_reassign_clear():
    ...
    b1 = fsm_Transition()
    b2 = fsm_Transition()
    _safe_set(a, 'fsm_Constraint', b1)
    assert _is_linked(a, 'fsm_Constraint', b1)
    if hasattr(b1, 'fsm_Transition16'):
        assert _is_linked(b1, 'fsm_Transition16', a)
    _safe_set(a, 'fsm_Constraint', b2)
    assert _is_linked(a, 'fsm_Constraint', b2)
    if hasattr(b1, 'fsm_Transition16'):
        assert not _is_linked(b1, 'fsm_Transition16', a)
    if hasattr(b2, 'fsm_Transition16'):
        assert _is_linked(b2, 'fsm_Transition16', a)
    _safe_set(a, 'fsm_Constraint', None)
    assert not _is_linked(a, 'fsm_Constraint', b2)
    if hasattr(b2, 'fsm_Transition16'):
        assert not _is_linked(b2, 'fsm_Transition16', a)


def test_assoc_trigger9_link_reassign_clear():
    a = fsm_Trigger(expression="sample_text")
    b1 = fsm_Transition()
    b2 = fsm_Transition()
    _safe_set(a, 'fsm_Trigger', b1)
    assert _is_linked(a, 'fsm_Trigger', b1)
    if hasattr(b1, 'fsm_Transition10'):
        assert _is_linked(b1, 'fsm_Transition10', a)
    _safe_set(a, 'fsm_Trigger', b2)
    assert _is_linked(a, 'fsm_Trigger', b2)
    if hasattr(b1, 'fsm_Transition10'):
        assert not _is_linked(b1, 'fsm_Transition10', a)
    if hasattr(b2, 'fsm_Transition10'):
        assert _is_linked(b2, 'fsm_Transition10', a)
    _safe_set(a, 'fsm_Trigger', None)
    assert not _is_linked(a, 'fsm_Trigger', b2)
    if hasattr(b2, 'fsm_Transition10'):
        assert not _is_linked(b2, 'fsm_Transition10', a)
