...
def test_hyp_output_b_is_not_abstract():
    assert not inspect.isabstract(output_B)


def test_hyp_output_b_constructor_exists():
    assert callable(output_B.__init__)


def test_hyp_output_b_constructor_args():
...
def test_hyp_output_b_b_setter(instance):
    original = instance.b
    instance.b = original
...
def test_output_B_b_value_roundtrip():
    instance = output_B(b="sample_text")
...
output_B_strategy = st.builds(output_B, b=safe_text)
@given(instance=output_B_strategy)
