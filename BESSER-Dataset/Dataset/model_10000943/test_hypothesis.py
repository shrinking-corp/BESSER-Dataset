...
def test_accountancy_actor_is_not_abstract():
    assert not inspect.isabstract(Accountancy_Actor)


def test_accountancy_actor_constructor_exists():
    assert callable(Accountancy_Actor.__init__)


def test_accountancy_actor_constructor_args():
    sig = inspect.signature(Accountancy_Actor.__init__)
    params = list(sig.parameters.keys())



def test_personnel_info_actor_is_not_abstract():
    assert not inspect.isabstract(personnel_info_Actor)


def test_personnel_info_actor_constructor_exists():
    assert callable(personnel_info_Actor.__init__)


def test_personnel_info_actor_constructor_args():
    sig = inspect.signature(personnel_info_Actor.__init__)
    params = list(sig.parameters.keys())



def test_muhasebe_actor_is_not_abstract():
    assert not inspect.isabstract(muhasebe_Actor)


def test_muhasebe_actor_constructor_exists():
    assert callable(muhasebe_Actor.__init__)


def test_muhasebe_actor_constructor_args():
    sig = inspect.signature(muhasebe_Actor.__init__)
    params = list(sig.parameters.keys())



def test_muhasebe2_actor_is_not_abstract():
    assert not inspect.isabstract(Muhasebe2_Actor)


def test_muhasebe2_actor_constructor_exists():
    assert callable(Muhasebe2_Actor.__init__)


def test_muhasebe2_actor_constructor_args():
