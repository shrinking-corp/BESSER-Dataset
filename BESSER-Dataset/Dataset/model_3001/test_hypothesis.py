...
def test_typeb_outportb_has_name():
    assert hasattr(typeB_OutPortB, "name")
    descriptor = None
    for klass in typeB_OutPortB.__mro__:
...
def test_typeb_inportb_has_name():
    assert hasattr(typeB_InPortB, "name")
    descriptor = None
    for klass in typeB_InPortB.__mro__:
        if "name" in klass.__dict__:
            descriptor = klass.__dict__["name"]
            break
    assert isinstance(descriptor, property)
...
def test_typeb_nonreferencedclass2_is_not_abstract():
    assert not inspect.isabstract(typeB_NonReferencedClass2)


def test_typeb_nonreferencedclass2_constructor_exists():
    assert callable(typeB_NonReferencedClass2.__init__)


def test_typeb_nonreferencedclass2_constructor_args():
    sig = inspect.signature(typeB_NonReferencedClass2.__init__)
    params = list(sig.parameters.keys())



def test_typeb_nonreferencedclass_is_not_abstract():
    assert not inspect.isabstract(typeB_NonReferencedClass)


def test_typeb_nonreferencedclass_constructor_exists():
    assert callable(typeB_NonReferencedClass.__init__)


def test_typeb_nonreferencedclass_constructor_args():
    sig = inspect.signature(typeB_NonReferencedClass.__init__)
    params = list(sig.parameters.keys())
...
typeB_NonReferencedClass2_strategy = st.builds(
    typeB_NonReferencedClass2,
)
typeB_NonReferencedClass_strategy = st.builds(
    typeB_NonReferencedClass,
)
...
@given(instance=typeB_OutPortB_strategy)
def test_typeb_outportb_name_setter(instance):
...
@given(instance=typeB_InPortB_strategy)
def test_typeb_inportb_name_setter(instance):
    original = instance.name
    instance.name = original
    assert instance.name == original
