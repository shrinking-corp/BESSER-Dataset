...
def test_assoc_contents100_link_reassign_clear():
    ...
    if hasattr(b1, 'ecore_EAnnotation101'):
        assert _is_linked(b1, 'ecore_EAnnotation101', a)
    _safe_set(a, 'ecore_EObject', b2)
    assert _is_linked(a, 'ecore_EObject', b2)
    if hasattr(b1, 'ecore_EAnnotation101'):
        assert not _is_linked(b1, 'ecore_EAnnotation101', a)
    if hasattr(b2, 'ecore_EAnnotation101'):
        assert _is_linked(b2, 'ecore_EAnnotation101', a)
    _safe_set(a, 'ecore_EObject', None)
    assert not _is_linked(a, 'ecore_EObject', b2)
    if hasattr(b2, 'ecore_EAnnotation101'):
        assert not _is_linked(b2, 'ecore_EAnnotation101', a)


def test_assoc_details99_link_reassign_clear():
...
def test_assoc_eAnnotations5_link_reassign_clear():
...
def test_assoc_eClassifier73_link_reassign_clear():
    ...
    b1 = ecore_EGenericType()
    b2 = ecore_EGenericType()
    _safe_set(a, 'ecore_EClassifier75', b1)
    assert _is_linked(a, 'ecore_EClassifier75', b1)
    if hasattr(b1, 'ecore_EGenericType74'):
        assert _is_linked(b1, 'ecore_EGenericType74', a)
    _safe_set(a, 'ecore_EClassifier75', b2)
    assert _is_linked(a, 'ecore_EClassifier75', b2)
    if hasattr(b1, 'ecore_EGenericType74'):
        assert not _is_linked(b1, 'ecore_EGenericType74', a)
    if hasattr(b2, 'ecore_EGenericType74'):
        assert _is_linked(b2, 'ecore_EGenericType74', a)
    _safe_set(a, 'ecore_EClassifier75', None)
    assert not _is_linked(a, 'ecore_EClassifier75', b2)
    if hasattr(b2, 'ecore_EGenericType74'):
        assert not _is_linked(b2, 'ecore_EGenericType74', a)


def test_assoc_eClassifiers51_link_reassign_clear():
    ...
    _safe_set(a, 'ePackage52', set())
    assert not _is_linked(a, 'ePackage52', b2)
    if hasattr(b2, 'EClassifier'):
        assert not _is_linked(b2, 'EClassifier', a)


def test_assoc_eContainingClass1_link_reassign_clear():
    a = ecore_EStructuralFeature(changeable=True, defaultValue="sample_text", defaultValueLiteral="sample_text", derived=True, transient=True, unsettable=True, volatile=True)
    b1 = ecore_EClass(abstract=True, interface=True)
    b2 = ecore_EClass(abstract=False, interface=False)
...
def test_assoc_eContainingClass86_link_reassign_clear():
    a = ecore_EClass(abstract=True, interface=True)
    b1 = ecore_EOperation()
    b2 = ecore_EOperation()
    _safe_set(a, 'EClass87', b1)
    assert _is_linked(a, 'EClass87', b1)
    if hasattr(b1, 'eOperations'):
        assert _is_linked(b1, 'eOperations', a)
    _safe_set(a, 'EClass87', b2)
    assert _is_linked(a, 'EClass87', b2)
    if hasattr(b1, 'eOperations'):
        assert not _is_linked(b1, 'eOperations', a)
    if hasattr(b2, 'eOperations'):
        assert _is_linked(b2, 'eOperations', a)
    _safe_set(a, 'EClass87', None)
    assert not _is_linked(a, 'EClass87', b2)
    if hasattr(b2, 'eOperations'):
...
def test_assoc_eExceptions80_link_reassign_clear():
    a = ecore_EClassifier(defaultValue="sample_text", instanceClass="sample_text", instanceClassName="sample_text", instanceTypeName="sample_text")
    b1 = ecore_EOperation()
    b2 = ecore_EOperation()
    _safe_set(a, 'ecore_EClassifier82', b1)
    assert _is_linked(a, 'ecore_EClassifier82', b1)
    if hasattr(b1, 'ecore_EOperation81'):
        assert _is_linked(b1, 'ecore_EOperation81', a)
    _safe_set(a, 'ecore_EClassifier82', b2)
    assert _is_linked(a, 'ecore_EClassifier82', b2)
    if hasattr(b1, 'ecore_EOperation81'):
        assert not _is_linked(b1, 'ecore_EOperation81', a)
    if hasattr(b2, 'ecore_EOperation81'):
        assert _is_linked(b2, 'ecore_EOperation81', a)
    _safe_set(a, 'ecore_EClassifier82', None)
    assert not _is_linked(a, 'ecore_EClassifier82', b2)
    if hasattr(b2, 'ecore_EOperation81'):
        assert not _is_linked(b2, 'ecore_EOperation81', a)


def test_assoc_eFactoryInstance44_link_reassign_clear():
    a = ecore_EPackage(nsPrefix="sample_text", nsUri="sample_text")
    b1 = ecore_EFactory()
    b2 = ecore_EFactory()
...
def test_assoc_eGenericSuperTypes35_link_reassign_clear():
    ...
    if hasattr(b1, 'ecore_EGenericType37'):
        assert _is_linked(b1, 'ecore_EGenericType37', a)
    _safe_set(a, 'ecore_EClass36', {b2})
    assert _is_linked(a, 'ecore_EClass36', b2)
    if hasattr(b1, 'ecore_EGenericType37'):
        assert not _is_linked(b1, 'ecore_EGenericType37', a)
    if hasattr(b2, 'ecore_EGenericType37'):
        assert _is_linked(b2, 'ecore_EGenericType37', a)
    _safe_set(a, 'ecore_EClass36', set())
    assert not _is_linked(a, 'ecore_EClass36', b2)
    if hasattr(b2, 'ecore_EGenericType37'):
        assert not _is_linked(b2, 'ecore_EGenericType37', a)


def test_assoc_eGenericType3_link_reassign_clear():
    a = ecore_ETypedElement(lowerBound=7, many=True, ordered=True, required=True, unique=True, upperBound=7)
    b1 = ecore_EGenericType()
    b2 = ecore_EGenericType()
    _safe_set(a, 'ecore_ETypedElement4', b1)
    assert _is_linked(a, 'ecore_ETypedElement4', b1)
    if hasattr(b1, 'ecore_EGenericType'):
        assert _is_linked(b1, 'ecore_EGenericType', a)
    _safe_set(a, 'ecore_ETypedElement4', b2)
    assert _is_linked(a, 'ecore_ETypedElement4', b2)
    if hasattr(b1, 'ecore_EGenericType'):
        assert not _is_linked(b1, 'ecore_EGenericType', a)
    if hasattr(b2, 'ecore_EGenericType'):
        assert _is_linked(b2, 'ecore_EGenericType', a)
    _safe_set(a, 'ecore_ETypedElement4', None)
    assert not _is_linked(a, 'ecore_ETypedElement4', b2)
    if hasattr(b2, 'ecore_EGenericType'):
        assert not _is_linked(b2, 'ecore_EGenericType', a)


def test_assoc_eKeys96_link_reassign_clear():
    a = ecore_EReference(container=True, containment=True, resolveProxies=True)
    b1 = ecore_EAttribute(id=True)
    b2 = ecore_EAttribute(id=False)
    _safe_set(a, 'ecore_EReference97', {b1})
    assert _is_linked(a, 'ecore_EReference97', b1)
    if hasattr(b1, 'ecore_EAttribute98'):
        assert _is_linked(b1, 'ecore_EAttribute98', a)
    _safe_set(a, 'ecore_EReference97', {b2})
    assert _is_linked(a, 'ecore_EReference97', b2)
    if hasattr(b1, 'ecore_EAttribute98'):
        assert not _is_linked(b1, 'ecore_EAttribute98', a)
    if hasattr(b2, 'ecore_EAttribute98'):
        assert _is_linked(b2, 'ecore_EAttribute98', a)
    _safe_set(a, 'ecore_EReference97', set())
    assert not _is_linked(a, 'ecore_EReference97', b2)
    if hasattr(b2, 'ecore_EAttribute98'):
        assert not _is_linked(b2, 'ecore_EAttribute98', a)
...
def test_assoc_eModelElement105_link_reassign_clear():
    a = ecore_EModelElement()
    b1 = ecore_EAnnotation(source="sample_text")
    b2 = ecore_EAnnotation(source="sample_text_2")
    _safe_set(a, 'EModelElement', b1)
...
def test_assoc_eOpposite91_link_reassign_clear():
    a = ecore_EReference(container=True, containment=True, resolveProxies=True)
    ...
    if hasattr(b1, 'ecore_EReference92'):
        assert _is_linked(b1, 'ecore_EReference92', a)
    _safe_set(a, 'ecore_EReference90', b2)
    assert _is_linked(a, 'ecore_EReference90', b2)
    if hasattr(b1, 'ecore_EReference92'):
        assert not _is_linked(b1, 'ecore_EReference92', a)
    if hasattr(b2, 'ecore_EReference92'):
        assert _is_linked(b2, 'ecore_EReference92', a)
    _safe_set(a, 'ecore_EReference90', None)
    assert not _is_linked(a, 'ecore_EReference90', b2)
    if hasattr(b2, 'ecore_EReference92'):
        assert not _is_linked(b2, 'ecore_EReference92', a)
...
def test_assoc_ePackage53_link_reassign_clear():
    a = ecore_EPackage(nsPrefix="sample_text", nsUri="sample_text")
    b1 = ecore_EFactory()
    b2 = ecore_EFactory()
    _safe_set(a, 'EPackage54', b1)
    assert _is_linked(a, 'EPackage54', b1)
    if hasattr(b1, 'eFactoryInstance'):
        assert _is_linked(b1, 'eFactoryInstance', a)
    _safe_set(a, 'EPackage54', b2)
    assert _is_linked(a, 'EPackage54', b2)
    if hasattr(b1, 'eFactoryInstance'):
        assert not _is_linked(b1, 'eFactoryInstance', a)
    if hasattr(b2, 'eFactoryInstance'):
        assert _is_linked(b2, 'eFactoryInstance', a)
    _safe_set(a, 'EPackage54', None)
    assert not _is_linked(a, 'EPackage54', b2)
    if hasattr(b2, 'eFactoryInstance'):
        assert not _is_linked(b2, 'eFactoryInstance', a)


def test_assoc_eRawType64_link_reassign_clear():
    a = ecore_EClassifier(defaultValue="sample_text", instanceClass="sample_text", instanceClassName="sample_text", instanceTypeName="sample_text")
    b1 = ecore_EGenericType()
    b2 = ecore_EGenericType()
    _safe_set(a, 'ecore_EClassifier66', b1)
    assert _is_linked(a, 'ecore_EClassifier66', b1)
    if hasattr(b1, 'ecore_EGenericType65'):
        assert _is_linked(b1, 'ecore_EGenericType65', a)
    _safe_set(a, 'ecore_EClassifier66', b2)
    assert _is_linked(a, 'ecore_EClassifier66', b2)
    if hasattr(b1, 'ecore_EGenericType65'):
        assert not _is_linked(b1, 'ecore_EGenericType65', a)
    if hasattr(b2, 'ecore_EGenericType65'):
        assert _is_linked(b2, 'ecore_EGenericType65', a)
    _safe_set(a, 'ecore_EClassifier66', None)
    assert not _is_linked(a, 'ecore_EClassifier66', b2)
    if hasattr(b2, 'ecore_EGenericType65'):
        assert not _is_linked(b2, 'ecore_EGenericType65', a)


def test_assoc_eReferenceType93_link_reassign_clear():
    a = ecore_EReference(container=True, containment=True, resolveProxies=True)
    b1 = ecore_EClass(abstract=True, interface=True)
    b2 = ecore_EClass(abstract=False, interface=False)
    _safe_set(a, 'ecore_EReference94', b1)
    assert _is_linked(a, 'ecore_EReference94', b1)
    if hasattr(b1, 'ecore_EClass95'):
        assert _is_linked(b1, 'ecore_EClass95', a)
    _safe_set(a, 'ecore_EReference94', b2)
    assert _is_linked(a, 'ecore_EReference94', b2)
    if hasattr(b1, 'ecore_EClass95'):
        assert not _is_linked(b1, 'ecore_EClass95', a)
    if hasattr(b2, 'ecore_EClass95'):
        assert _is_linked(b2, 'ecore_EClass95', a)
    _safe_set(a, 'ecore_EReference94', None)
    assert not _is_linked(a, 'ecore_EReference94', b2)
    if hasattr(b2, 'ecore_EClass95'):
        assert not _is_linked(b2, 'ecore_EClass95', a)
...
def test_assoc_eSubpackages46_link_reassign_clear():
    a = ecore_EPackage(nsPrefix="sample_text", nsUri="sample_text")
...
def test_assoc_eSuperPackage49_link_reassign_clear():
    a = ecore_EPackage(nsPrefix="sample_text", nsUri="sample_text")
    b1 = ecore_EPackage(nsPrefix="sample_text", nsUri="sample_text")
    b2 = ecore_EPackage(nsPrefix="sample_text_2", nsUri="sample_text_2")
...
def test_assoc_eType2_link_reassign_clear():
    a = ecore_ETypedElement(lowerBound=7, many=True, ordered=True, required=True, unique=True, upperBound=7)
    b1 = ecore_EClassifier(defaultValue="sample_text", instanceClass="sample_text", instanceClassName="sample_text", instanceTypeName="sample_text")
    b2 = ecore_EClassifier(defaultValue="sample_text_2", instanceClass="sample_text_2", instanceClassName="sample_text_2", instanceTypeName="sample_text_2")
...
def test_assoc_eTypeParameters42_link_reassign_clear():
    ...
    _safe_set(a, 'ecore_EClassifier43', {b1})
    assert _is_linked(a, 'ecore_EClassifier43', b1)
    if hasattr(b1, 'ecore_ETypeParameter'):
        assert _is_linked(b1, 'ecore_ETypeParameter', a)
    _safe_set(a, 'ecore_EClassifier43', {b2})
    assert _is_linked(a, 'ecore_EClassifier43', b2)
    if hasattr(b1, 'ecore_ETypeParameter'):
    ...
    _safe_set(a, 'ecore_EClassifier43', set())
    assert not _is_linked(a, 'ecore_EClassifier43', b2)
    if hasattr(b2, 'ecore_ETypeParameter'):
...
def test_assoc_references102_link_reassign_clear():
    a = ecore_EObject()
    b1 = ecore_EAnnotation(source="sample_text")
    b2 = ecore_EAnnotation(source="sample_text_2")
    _safe_set(a, 'ecore_EObject104', b1)
    assert _is_linked(a, 'ecore_EObject104', b1)
    if hasattr(b1, 'ecore_EAnnotation103'):
        assert _is_linked(b1, 'ecore_EAnnotation103', a)
    _safe_set(a, 'ecore_EObject104', b2)
    assert _is_linked(a, 'ecore_EObject104', b2)
    if hasattr(b1, 'ecore_EAnnotation103'):
        assert not _is_linked(b1, 'ecore_EAnnotation103', a)
    if hasattr(b2, 'ecore_EAnnotation103'):
        assert _is_linked(b2, 'ecore_EAnnotation103', a)
    _safe_set(a, 'ecore_EObject104', None)
    assert not _is_linked(a, 'ecore_EObject104', b2)
    if hasattr(b2, 'ecore_EAnnotation103'):
        assert not _is_linked(b2, 'ecore_EAnnotation103', a)
