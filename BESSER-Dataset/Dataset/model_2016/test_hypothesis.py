...
def test_trace_debugtraceregion_has_useForDebugging():
    assert hasattr(trace_DebugTraceRegion, "useForDebugging")
    descriptor = None
    for klass in trace_DebugTraceRegion.__mro__:
        if "useForDebugging" in klass.__dict__:
            descriptor = klass.__dict__["useForDebugging"]
            break
    assert isinstance(descriptor, property)
...
@given(instance=trace_DebugTraceRegion_strategy)
def test_trace_debugtraceregion_useForDebugging_setter(instance):
