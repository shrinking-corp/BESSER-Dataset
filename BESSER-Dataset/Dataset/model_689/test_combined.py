...
def test_hyp_janus_essentialocl_variable_varType_setter(instance):
...
def test_Janus_essentialocl_Variable_varType_value_roundtrip():
    instance = Janus_essentialocl_Variable(varType="sample_text")
...
def test_assoc_LetExp86_link_reassign_clear():
    a = Janus_essentialocl_Variable(varType="sample_text")
    b1 = LetExp()
    b2 = LetExp()
    _safe_set(a, 'variable', b1)
    assert _is_linked(a, 'variable', b1)
    if hasattr(b1, 'LetExp87'):
        assert _is_linked(b1, 'LetExp87', a)
    _safe_set(a, 'variable', b2)
    assert _is_linked(a, 'variable', b2)
    if hasattr(b1, 'LetExp87'):
        assert not _is_linked(b1, 'LetExp87', a)
    if hasattr(b2, 'LetExp87'):
        assert _is_linked(b2, 'LetExp87', a)
    _safe_set(a, 'variable', None)
    assert not _is_linked(a, 'variable', b2)
    if hasattr(b2, 'LetExp87'):
        assert not _is_linked(b2, 'LetExp87', a)
...
def test_assoc_bindParameter88_link_reassign_clear():
    a = Janus_essentialocl_Variable(varType="sample_text")
    b1 = Parameter()
    b2 = Parameter()
    _safe_set(a, 'Janus_essentialocl_Variable89', b1)
    assert _is_linked(a, 'Janus_essentialocl_Variable89', b1)
    if hasattr(b1, 'Parameter90'):
        assert _is_linked(b1, 'Parameter90', a)
    _safe_set(a, 'Janus_essentialocl_Variable89', b2)
    assert _is_linked(a, 'Janus_essentialocl_Variable89', b2)
    if hasattr(b1, 'Parameter90'):
        assert not _is_linked(b1, 'Parameter90', a)
    if hasattr(b2, 'Parameter90'):
        assert _is_linked(b2, 'Parameter90', a)
    _safe_set(a, 'Janus_essentialocl_Variable89', None)
    assert not _is_linked(a, 'Janus_essentialocl_Variable89', b2)
    if hasattr(b2, 'Parameter90'):
        assert not _is_linked(b2, 'Parameter90', a)


def test_assoc_computeOwner91_link_reassign_clear():
    a = Janus_essentialocl_Variable(varType="sample_text")
    b1 = ComputeExp()
    b2 = ComputeExp()
    _safe_set(a, 'returnedElement', b1)
    assert _is_linked(a, 'returnedElement', b1)
    if hasattr(b1, 'ComputeExp'):
        assert _is_linked(b1, 'ComputeExp', a)
    _safe_set(a, 'returnedElement', b2)
    assert _is_linked(a, 'returnedElement', b2)
    if hasattr(b1, 'ComputeExp'):
        assert not _is_linked(b1, 'ComputeExp', a)
    if hasattr(b2, 'ComputeExp'):
        assert _is_linked(b2, 'ComputeExp', a)
    _safe_set(a, 'returnedElement', None)
    assert not _is_linked(a, 'returnedElement', b2)
    if hasattr(b2, 'ComputeExp'):
        assert not _is_linked(b2, 'ComputeExp', a)
...
def test_assoc_initExpression84_link_reassign_clear():
    ...
    b1 = OclExpression()
    b2 = OclExpression()
    _safe_set(a, 'Janus_essentialocl_Variable', b1)
    assert _is_linked(a, 'Janus_essentialocl_Variable', b1)
    if hasattr(b1, 'OclExpression85'):
        assert _is_linked(b1, 'OclExpression85', a)
    _safe_set(a, 'Janus_essentialocl_Variable', b2)
    assert _is_linked(a, 'Janus_essentialocl_Variable', b2)
    if hasattr(b1, 'OclExpression85'):
        assert not _is_linked(b1, 'OclExpression85', a)
    if hasattr(b2, 'OclExpression85'):
        assert _is_linked(b2, 'OclExpression85', a)
    _safe_set(a, 'Janus_essentialocl_Variable', None)
    assert not _is_linked(a, 'Janus_essentialocl_Variable', b2)
    if hasattr(b2, 'OclExpression85'):
...
Janus_essentialocl_Variable_strategy = st.builds(Janus_essentialocl_Variable, varType=safe_text)
@given(instance=Janus_essentialocl_Variable_strategy)
@settings(max_examples=25)
