...
def test_hyp_bowling_specialplayer_is_not_abstract():
    assert not inspect.isabstract(bowling_SpecialPlayer)


def test_hyp_bowling_specialplayer_constructor_exists():
    assert callable(bowling_SpecialPlayer.__init__)


def test_hyp_bowling_specialplayer_constructor_args():
    sig = inspect.signature(bowling_SpecialPlayer.__init__)
    params = list(sig.parameters.keys())
    assert "medals" in params, "Missing parameter 'medals'"
...
bowling_SpecialPlayer_strategy = st.builds(
    bowling_SpecialPlayer,
    medals=
        st.integers()
)
...
@given(instance=bowling_SpecialPlayer_strategy)
def test_hyp_bowling_specialplayer_medals_setter(instance):
    original = instance.medals
    instance.medals = original
    assert instance.medals == original
...
def test_bowling_SpecialPlayer_medals_value_roundtrip():
    instance = bowling_SpecialPlayer(medals=7)
    assert instance.medals == 7
    instance.medals = 13
    assert instance.medals == 13
...
def test_bowling_SpecialPlayer_isa_Player():
    instance = bowling_SpecialPlayer(medals=7)
...
bowling_SpecialPlayer_strategy = st.builds(bowling_SpecialPlayer, medals=st.integers())
@given(instance=bowling_SpecialPlayer_strategy)
@settings(max_examples=25)
def test_bowling_SpecialPlayer_instantiation(instance):
    assert isinstance(instance, bowling_SpecialPlayer)
