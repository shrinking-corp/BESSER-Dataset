...
def test_bowling_specialplayer_is_not_abstract():
    assert not inspect.isabstract(bowling_SpecialPlayer)


def test_bowling_specialplayer_constructor_exists():
    assert callable(bowling_SpecialPlayer.__init__)


def test_bowling_specialplayer_constructor_args():
    sig = inspect.signature(bowling_SpecialPlayer.__init__)
    params = list(sig.parameters.keys())
    assert "medals" in params, "Missing parameter 'medals'"

def test_bowling_specialplayer_has_medals():
    assert hasattr(bowling_SpecialPlayer, "medals")
    descriptor = None
    for klass in bowling_SpecialPlayer.__mro__:
        if "medals" in klass.__dict__:
            descriptor = klass.__dict__["medals"]
            break
    assert isinstance(descriptor, property)
...
@given(instance=bowling_SpecialPlayer_strategy)
def test_bowling_specialplayer_medals_setter(instance):
