...
def test_assoc_component40_link_reassign_clear():
    a = cloudml_core_Port(isLocal=True, portNumber=7)
    b1 = Component()
    b2 = Component()
    _safe_set(a, 'cloudml_core_Port', b1)
    assert _is_linked(a, 'cloudml_core_Port', b1)
    if hasattr(b1, 'Component41'):
        assert _is_linked(b1, 'Component41', a)
    _safe_set(a, 'cloudml_core_Port', b2)
    assert _is_linked(a, 'cloudml_core_Port', b2)
    if hasattr(b1, 'Component41'):
        assert not _is_linked(b1, 'Component41', a)
    if hasattr(b2, 'Component41'):
        assert _is_linked(b2, 'Component41', a)
    _safe_set(a, 'cloudml_core_Port', None)
    assert not _is_linked(a, 'cloudml_core_Port', b2)
    if hasattr(b2, 'Component41'):
        assert not _is_linked(b2, 'Component41', a)


def test_assoc_provide81_link_reassign_clear():
    ...
    b1 = VMPort()
    b2 = VMPort()
    _safe_set(a, 'cloudml_core_ExternalComponent82', {b1})
    assert _is_linked(a, 'cloudml_core_ExternalComponent82', b1)
    if hasattr(b1, 'VMPort83'):
        assert _is_linked(b1, 'VMPort83', a)
    _safe_set(a, 'cloudml_core_ExternalComponent82', {b2})
    assert _is_linked(a, 'cloudml_core_ExternalComponent82', b2)
    if hasattr(b1, 'VMPort83'):
        assert not _is_linked(b1, 'VMPort83', a)
    if hasattr(b2, 'VMPort83'):
        assert _is_linked(b2, 'VMPort83', a)
    _safe_set(a, 'cloudml_core_ExternalComponent82', set())
    assert not _is_linked(a, 'cloudml_core_ExternalComponent82', b2)
    if hasattr(b2, 'VMPort83'):
        assert not _is_linked(b2, 'VMPort83', a)


def test_assoc_provide84_link_reassign_clear():
...
def test_assoc_provided55_link_reassign_clear():
