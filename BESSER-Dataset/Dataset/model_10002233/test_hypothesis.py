...
def test_c1_has_att2():
    assert hasattr(C1, "att2")
    descriptor = None
    for klass in C1.__mro__:
...
def test_a1_has_allA():
    assert hasattr(A1, "allA")
    descriptor = None
    for klass in A1.__mro__:
        if "allA" in klass.__dict__:
            descriptor = klass.__dict__["allA"]
            break
    assert isinstance(descriptor, property)
...
@given(instance=C1_strategy)
def test_c1_att2_setter(instance):
...
@given(instance=A1_strategy)
def test_a1_allA_setter(instance):
