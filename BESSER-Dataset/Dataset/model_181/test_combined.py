...
def test_assoc_in_5_link_reassign_clear():
    a = lit_petriNets_Place(name="sample_text")
    b1 = lit_petriNets_TPArc()
    b2 = lit_petriNets_TPArc()
    _safe_set(a, 'dst', {b1})
...
def test_assoc_in_9_link_reassign_clear():
    ...
    b1 = lit_petriNets_PTArc()
    b2 = lit_petriNets_PTArc()
    _safe_set(a, 'dst10', {b1})
    assert _is_linked(a, 'dst10', b1)
    if hasattr(b1, 'PTArc11'):
        assert _is_linked(b1, 'PTArc11', a)
    _safe_set(a, 'dst10', {b2})
    assert _is_linked(a, 'dst10', b2)
    if hasattr(b1, 'PTArc11'):
        assert not _is_linked(b1, 'PTArc11', a)
    if hasattr(b2, 'PTArc11'):
        assert _is_linked(b2, 'PTArc11', a)
    _safe_set(a, 'dst10', set())
    assert not _is_linked(a, 'dst10', b2)
    if hasattr(b2, 'PTArc11'):
