...
@given(instance=bowling_Player_strategy)
def test_hyp_bowling_player_heigth_setter(instance):
...
def test_bowling_Player_dateOfBirth_value_roundtrip():
    instance = bowling_Player(dateOfBirth=date(2024, 1, 1), heigth=3.14, isProfessional=True, name="sample_text")
    assert instance.dateOfBirth == date(2024, 1, 1)
    instance.dateOfBirth = date(2025, 6, 15)
    assert instance.dateOfBirth == date(2025, 6, 15)


def test_bowling_Player_heigth_value_roundtrip():
    instance = bowling_Player(dateOfBirth=date(2024, 1, 1), heigth=3.14, isProfessional=True, name="sample_text")
    assert instance.heigth == 3.14
    instance.heigth = 9.99
    assert instance.heigth == 9.99
...
def test_assoc_Players0_link_reassign_clear():
    ...
    b1 = bowling_Player(dateOfBirth=date(2024, 1, 1), heigth=3.14, isProfessional=True, name="sample_text")
    b2 = bowling_Player(dateOfBirth=date(2025, 6, 15), heigth=9.99, isProfessional=False, name="sample_text_2")
    _safe_set(a, 'bowling_league', {b1})
    assert _is_linked(a, 'bowling_league', b1)
    if hasattr(b1, 'bowling_Player'):
        assert _is_linked(b1, 'bowling_Player', a)
    _safe_set(a, 'bowling_league', {b2})
    assert _is_linked(a, 'bowling_league', b2)
    if hasattr(b1, 'bowling_Player'):
        assert not _is_linked(b1, 'bowling_Player', a)
    if hasattr(b2, 'bowling_Player'):
        assert _is_linked(b2, 'bowling_Player', a)
    _safe_set(a, 'bowling_league', set())
    assert not _is_linked(a, 'bowling_league', b2)
    if hasattr(b2, 'bowling_Player'):
        assert not _is_linked(b2, 'bowling_Player', a)
...
def test_assoc_player4_link_reassign_clear():
    a = bowling_Player(dateOfBirth=date(2024, 1, 1), heigth=3.14, isProfessional=True, name="sample_text")
...
bowling_Player_strategy = st.builds(bowling_Player, dateOfBirth=st.dates(), heigth=st.floats(allow_nan=False, allow_infinity=False), isProfessional=st.booleans(), name=safe_text)
@given(instance=bowling_Player_strategy)
@settings(max_examples=25)
