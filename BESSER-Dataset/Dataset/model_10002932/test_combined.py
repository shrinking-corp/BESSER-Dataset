...
def test_assoc_association2_link_reassign_clear():
    ...
    if hasattr(b1, 'Shopping_cart1'):
        assert _is_linked(b1, 'Shopping_cart1', a)
    _safe_set(a, 'c0', b2)
    assert _is_linked(a, 'c0', b2)
    if hasattr(b1, 'Shopping_cart1'):
        assert not _is_linked(b1, 'Shopping_cart1', a)
    if hasattr(b2, 'Shopping_cart1'):
        assert _is_linked(b2, 'Shopping_cart1', a)
    _safe_set(a, 'c0', None)
    assert not _is_linked(a, 'c0', b2)
    if hasattr(b2, 'Shopping_cart1'):
        assert not _is_linked(b2, 'Shopping_cart1', a)
