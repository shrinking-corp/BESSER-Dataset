...
def test_hyp_petrinet_networksystem_is_not_abstract():
    assert not inspect.isabstract(petrinet_NetworkSystem)


def test_hyp_petrinet_networksystem_constructor_exists():
    assert callable(petrinet_NetworkSystem.__init__)


def test_hyp_petrinet_networksystem_constructor_args():
    sig = inspect.signature(petrinet_NetworkSystem.__init__)
    params = list(sig.parameters.keys())
...
petrinet_NetworkSystem_strategy = st.builds(
    petrinet_NetworkSystem,
)
...
def test_assoc_elements0_link_reassign_clear():
    ...
    b1 = petrinet_Element(name="sample_text")
    b2 = petrinet_Element(name="sample_text_2")
...
def test_assoc_fromNet1_link_reassign_clear():
    a = petrinet_PetriNet(name="sample_text")
    b1 = petrinet_Arc()
    b2 = petrinet_Arc()
    ...
    _safe_set(a, 'petrinet_PetriNet2', None)
...
def test_assoc_petrinets14_link_reassign_clear():
    a = petrinet_PetriNet(name="sample_text")
    b1 = petrinet_NetworkSystem()
    b2 = petrinet_NetworkSystem()
    _safe_set(a, 'petrinet_PetriNet15', b1)
    assert _is_linked(a, 'petrinet_PetriNet15', b1)
    if hasattr(b1, 'petrinet_NetworkSystem'):
        assert _is_linked(b1, 'petrinet_NetworkSystem', a)
    _safe_set(a, 'petrinet_PetriNet15', b2)
    assert _is_linked(a, 'petrinet_PetriNet15', b2)
    if hasattr(b1, 'petrinet_NetworkSystem'):
        assert not _is_linked(b1, 'petrinet_NetworkSystem', a)
    if hasattr(b2, 'petrinet_NetworkSystem'):
        assert _is_linked(b2, 'petrinet_NetworkSystem', a)
    _safe_set(a, 'petrinet_PetriNet15', None)
    assert not _is_linked(a, 'petrinet_PetriNet15', b2)
    if hasattr(b2, 'petrinet_NetworkSystem'):
        assert not _is_linked(b2, 'petrinet_NetworkSystem', a)
...
def test_assoc_toNet3_link_reassign_clear():
    a = petrinet_PetriNet(name="sample_text")
    b1 = petrinet_Arc()
    b2 = petrinet_Arc()
...
petrinet_NetworkSystem_strategy = st.builds(petrinet_NetworkSystem)
@given(instance=petrinet_NetworkSystem_strategy)
@settings(max_examples=25)
def test_petrinet_NetworkSystem_instantiation(instance):
    assert isinstance(instance, petrinet_NetworkSystem)
