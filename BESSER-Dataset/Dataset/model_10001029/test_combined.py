...
def test_assoc_Advertiser_Advertiesment_link_reassign_clear():
    ...
    _safe_set(a, 'advertiesment2', {b1})
    assert _is_linked(a, 'advertiesment2', b1)
    if hasattr(b1, 'advertiser3'):
        assert _is_linked(b1, 'advertiser3', a)
    _safe_set(a, 'advertiesment2', {b2})
    assert _is_linked(a, 'advertiesment2', b2)
    if hasattr(b1, 'advertiser3'):
        assert not _is_linked(b1, 'advertiser3', a)
    if hasattr(b2, 'advertiser3'):
        assert _is_linked(b2, 'advertiser3', a)
    _safe_set(a, 'advertiesment2', set())
    assert not _is_linked(a, 'advertiesment2', b2)
    if hasattr(b2, 'advertiser3'):
        assert not _is_linked(b2, 'advertiser3', a)
...
def test_assoc_Property_Buyer_link_reassign_clear():
    ...
    _safe_set(a, 'buyer8', b1)
    assert _is_linked(a, 'buyer8', b1)
    if hasattr(b1, 'property9'):
        assert _is_linked(b1, 'property9', a)
    _safe_set(a, 'buyer8', b2)
    assert _is_linked(a, 'buyer8', b2)
    if hasattr(b1, 'property9'):
        assert not _is_linked(b1, 'property9', a)
    if hasattr(b2, 'property9'):
        assert _is_linked(b2, 'property9', a)
    _safe_set(a, 'buyer8', None)
    assert not _is_linked(a, 'buyer8', b2)
    if hasattr(b2, 'property9'):
...
def test_assoc_Property_Seller_link_reassign_clear():
    ...
    if hasattr(b1, 'seller4'):
        assert _is_linked(b1, 'seller4', a)
    _safe_set(a, 'property5', {b2})
    assert _is_linked(a, 'property5', b2)
    if hasattr(b1, 'seller4'):
        assert not _is_linked(b1, 'seller4', a)
    if hasattr(b2, 'seller4'):
        assert _is_linked(b2, 'seller4', a)
    _safe_set(a, 'property5', set())
    assert not _is_linked(a, 'property5', b2)
    if hasattr(b2, 'seller4'):
        assert not _is_linked(b2, 'seller4', a)
...
def test_assoc_Reg_User_Requirement_link_reassign_clear():
    ...
    _safe_set(a, 'reg_User11', {b1})
    assert _is_linked(a, 'reg_User11', b1)
    if hasattr(b1, 'requirement10'):
        assert _is_linked(b1, 'requirement10', a)
    _safe_set(a, 'reg_User11', {b2})
    assert _is_linked(a, 'reg_User11', b2)
    if hasattr(b1, 'requirement10'):
        assert not _is_linked(b1, 'requirement10', a)
    if hasattr(b2, 'requirement10'):
        assert _is_linked(b2, 'requirement10', a)
    _safe_set(a, 'reg_User11', set())
    assert not _is_linked(a, 'reg_User11', b2)
    if hasattr(b2, 'requirement10'):
