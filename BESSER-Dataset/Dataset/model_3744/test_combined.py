...
def test_hyp_ordb4ora_restriction_NameColumns_setter(instance):
...
@given(instance=ORDB4ORA_Restriction_strategy)
def test_hyp_ordb4ora_restriction_Name_setter(instance):
    original = instance.Name
    instance.Name = original
    assert instance.Name == original
...
def test_ORDB4ORA_Check_Condition_value_roundtrip():
    instance = ORDB4ORA_Check(Condition="sample_text")
...
def test_ORDB4ORA_ForeignKey_OnDelete_value_roundtrip():
    instance = ORDB4ORA_ForeignKey(OnDelete="sample_text")
...
def test_ORDB4ORA_Restriction_Name_value_roundtrip():
    instance = ORDB4ORA_Restriction(Name="sample_text", NameColumns="sample_text")
    assert instance.Name == "sample_text"
    instance.Name = "sample_text_2"
    assert instance.Name == "sample_text_2"


def test_ORDB4ORA_Restriction_NameColumns_value_roundtrip():
    instance = ORDB4ORA_Restriction(Name="sample_text", NameColumns="sample_text")
...
def test_assoc_Type79_link_reassign_clear():
    ...
    if hasattr(b1, 'ORDB4ORA_Datatype80'):
        assert _is_linked(b1, 'ORDB4ORA_Datatype80', a)
    _safe_set(a, 'ORDB4ORA_Varray', b2)
    assert _is_linked(a, 'ORDB4ORA_Varray', b2)
    if hasattr(b1, 'ORDB4ORA_Datatype80'):
        assert not _is_linked(b1, 'ORDB4ORA_Datatype80', a)
    if hasattr(b2, 'ORDB4ORA_Datatype80'):
        assert _is_linked(b2, 'ORDB4ORA_Datatype80', a)
    _safe_set(a, 'ORDB4ORA_Varray', None)
    assert not _is_linked(a, 'ORDB4ORA_Varray', b2)
    if hasattr(b2, 'ORDB4ORA_Datatype80'):
        assert not _is_linked(b2, 'ORDB4ORA_Datatype80', a)


def test_assoc_attribute44_link_reassign_clear():
    a = ORDB4ORA_StoredNestedTable(Name="sample_text")
    b1 = ORDB4ORA_Attribute()
    b2 = ORDB4ORA_Attribute()
...
def test_assoc_attributes41_link_reassign_clear():
    a = ORDB4ORA_StructuralComponent(Name="sample_text")
    b1 = ORDB4ORA_Restriction(Name="sample_text", NameColumns="sample_text")
    b2 = ORDB4ORA_Restriction(Name="sample_text_2", NameColumns="sample_text_2")
...
def test_assoc_columns63_link_reassign_clear():
    ...
    b1 = ORDB4ORA_Column()
    b2 = ORDB4ORA_Column()
    _safe_set(a, 'table64', {b1})
    assert _is_linked(a, 'table64', b1)
    if hasattr(b1, 'Column'):
        assert _is_linked(b1, 'Column', a)
    _safe_set(a, 'table64', {b2})
    assert _is_linked(a, 'table64', b2)
    if hasattr(b1, 'Column'):
        assert not _is_linked(b1, 'Column', a)
    if hasattr(b2, 'Column'):
        assert _is_linked(b2, 'Column', a)
    _safe_set(a, 'table64', set())
    assert not _is_linked(a, 'table64', b2)
    if hasattr(b2, 'Column'):
        assert not _is_linked(b2, 'Column', a)


def test_assoc_components83_link_reassign_clear():
    a = ORDB4ORA_StructuralComponent(Name="sample_text")
    b1 = ORDB4ORA_View()
    b2 = ORDB4ORA_View()
    _safe_set(a, 'ORDB4ORA_StructuralComponent84', b1)
    assert _is_linked(a, 'ORDB4ORA_StructuralComponent84', b1)
    if hasattr(b1, 'ORDB4ORA_View'):
        assert _is_linked(b1, 'ORDB4ORA_View', a)
    _safe_set(a, 'ORDB4ORA_StructuralComponent84', b2)
    assert _is_linked(a, 'ORDB4ORA_StructuralComponent84', b2)
    if hasattr(b1, 'ORDB4ORA_View'):
        assert not _is_linked(b1, 'ORDB4ORA_View', a)
    if hasattr(b2, 'ORDB4ORA_View'):
        assert _is_linked(b2, 'ORDB4ORA_View', a)
    _safe_set(a, 'ORDB4ORA_StructuralComponent84', None)
    assert not _is_linked(a, 'ORDB4ORA_StructuralComponent84', b2)
    if hasattr(b2, 'ORDB4ORA_View'):
        assert not _is_linked(b2, 'ORDB4ORA_View', a)


def test_assoc_datatype0_link_reassign_clear():
...
def test_assoc_model65_link_reassign_clear():
    a = ORDB4ORA_Table(Name="sample_text")
    b1 = ORDB4ORA_Model(Name="sample_text")
    b2 = ORDB4ORA_Model(Name="sample_text_2")
    _safe_set(a, 'table66', b1)
    assert _is_linked(a, 'table66', b1)
    if hasattr(b1, 'Model67'):
        assert _is_linked(b1, 'Model67', a)
    _safe_set(a, 'table66', b2)
    assert _is_linked(a, 'table66', b2)
    if hasattr(b1, 'Model67'):
        assert not _is_linked(b1, 'Model67', a)
    if hasattr(b2, 'Model67'):
        assert _is_linked(b2, 'Model67', a)
    _safe_set(a, 'table66', None)
    assert not _is_linked(a, 'table66', b2)
    if hasattr(b2, 'Model67'):
        assert not _is_linked(b2, 'Model67', a)
...
def test_assoc_operation3_link_reassign_clear():
    ...
    if hasattr(b1, 'model4'):
        assert _is_linked(b1, 'model4', a)
    _safe_set(a, 'Operation', b2)
    assert _is_linked(a, 'Operation', b2)
    if hasattr(b1, 'model4'):
        assert not _is_linked(b1, 'model4', a)
    if hasattr(b2, 'model4'):
        assert _is_linked(b2, 'model4', a)
    _safe_set(a, 'Operation', None)
    assert not _is_linked(a, 'Operation', b2)
    if hasattr(b2, 'model4'):
        assert not _is_linked(b2, 'model4', a)
...
def test_assoc_override14_link_reassign_clear():
    a = ORDB4ORA_Method(Body="sample_text", Name="sample_text")
    b1 = ORDB4ORA_Method(Body="sample_text", Name="sample_text")
    b2 = ORDB4ORA_Method(Body="sample_text_2", Name="sample_text_2")
    _safe_set(a, 'ORDB4ORA_Method', b1)
    assert _is_linked(a, 'ORDB4ORA_Method', b1)
    if hasattr(b1, 'ORDB4ORA_Method13'):
        assert _is_linked(b1, 'ORDB4ORA_Method13', a)
    _safe_set(a, 'ORDB4ORA_Method', b2)
    assert _is_linked(a, 'ORDB4ORA_Method', b2)
    if hasattr(b1, 'ORDB4ORA_Method13'):
        assert not _is_linked(b1, 'ORDB4ORA_Method13', a)
    if hasattr(b2, 'ORDB4ORA_Method13'):
        assert _is_linked(b2, 'ORDB4ORA_Method13', a)
    _safe_set(a, 'ORDB4ORA_Method', None)
    assert not _is_linked(a, 'ORDB4ORA_Method', b2)
    if hasattr(b2, 'ORDB4ORA_Method13'):
        assert not _is_linked(b2, 'ORDB4ORA_Method13', a)
...
def test_assoc_package5_link_reassign_clear():
    a = ORDB4ORA_Package(Name="sample_text")
    b1 = ORDB4ORA_Model(Name="sample_text")
    b2 = ORDB4ORA_Model(Name="sample_text_2")
    _safe_set(a, 'Package', b1)
    assert _is_linked(a, 'Package', b1)
    if hasattr(b1, 'model6'):
        assert _is_linked(b1, 'model6', a)
    _safe_set(a, 'Package', b2)
    assert _is_linked(a, 'Package', b2)
    if hasattr(b1, 'model6'):
        assert not _is_linked(b1, 'model6', a)
    if hasattr(b2, 'model6'):
        assert _is_linked(b2, 'model6', a)
    _safe_set(a, 'Package', None)
    assert not _is_linked(a, 'Package', b2)
    if hasattr(b2, 'model6'):
        assert not _is_linked(b2, 'model6', a)
...
def test_assoc_reference11_link_reassign_clear():
    a = ORDB4ORA_Table(Name="sample_text")
    b1 = ORDB4ORA_ForeignKey(OnDelete="sample_text")
    b2 = ORDB4ORA_ForeignKey(OnDelete="sample_text_2")
...
def test_assoc_restriction61_link_reassign_clear():
    a = ORDB4ORA_Table(Name="sample_text")
    b1 = ORDB4ORA_Restriction(Name="sample_text", NameColumns="sample_text")
    b2 = ORDB4ORA_Restriction(Name="sample_text_2", NameColumns="sample_text_2")
    _safe_set(a, 'table', {b1})
    assert _is_linked(a, 'table', b1)
    if hasattr(b1, 'Restriction62'):
        assert _is_linked(b1, 'Restriction62', a)
    _safe_set(a, 'table', {b2})
    assert _is_linked(a, 'table', b2)
    if hasattr(b1, 'Restriction62'):
        assert not _is_linked(b1, 'Restriction62', a)
    if hasattr(b2, 'Restriction62'):
        assert _is_linked(b2, 'Restriction62', a)
    _safe_set(a, 'table', set())
    assert not _is_linked(a, 'table', b2)
    if hasattr(b2, 'Restriction62'):
        assert not _is_linked(b2, 'Restriction62', a)
...
def test_assoc_returnType19_link_reassign_clear():
    a = ORDB4ORA_Method(Body="sample_text", Name="sample_text")
    b1 = ORDB4ORA_Datatype()
    b2 = ORDB4ORA_Datatype()
    _safe_set(a, 'ORDB4ORA_Method20', b1)
    assert _is_linked(a, 'ORDB4ORA_Method20', b1)
    if hasattr(b1, 'ORDB4ORA_Datatype21'):
        assert _is_linked(b1, 'ORDB4ORA_Datatype21', a)
    _safe_set(a, 'ORDB4ORA_Method20', b2)
    assert _is_linked(a, 'ORDB4ORA_Method20', b2)
    if hasattr(b1, 'ORDB4ORA_Datatype21'):
        assert not _is_linked(b1, 'ORDB4ORA_Datatype21', a)
    if hasattr(b2, 'ORDB4ORA_Datatype21'):
        assert _is_linked(b2, 'ORDB4ORA_Datatype21', a)
    _safe_set(a, 'ORDB4ORA_Method20', None)
    assert not _is_linked(a, 'ORDB4ORA_Method20', b2)
    if hasattr(b2, 'ORDB4ORA_Datatype21'):
        assert not _is_linked(b2, 'ORDB4ORA_Datatype21', a)


def test_assoc_storedNested75_link_reassign_clear():
    a = ORDB4ORA_StoredNestedTable(Name="sample_text")
...
def test_assoc_structured15_link_reassign_clear():
    a = ORDB4ORA_StructuredType(Name="sample_text", is_final=True, is_instantiable=True)
    b1 = ORDB4ORA_Method(Body="sample_text", Name="sample_text")
    b2 = ORDB4ORA_Method(Body="sample_text_2", Name="sample_text_2")
    _safe_set(a, 'StructuredType16', b1)
    assert _is_linked(a, 'StructuredType16', b1)
    if hasattr(b1, 'method'):
        assert _is_linked(b1, 'method', a)
    _safe_set(a, 'StructuredType16', b2)
    assert _is_linked(a, 'StructuredType16', b2)
    if hasattr(b1, 'method'):
        assert not _is_linked(b1, 'method', a)
    if hasattr(b2, 'method'):
        assert _is_linked(b2, 'method', a)
    _safe_set(a, 'StructuredType16', None)
    assert not _is_linked(a, 'StructuredType16', b2)
    if hasattr(b2, 'method'):
...
def test_assoc_structured76_link_reassign_clear():
    a = ORDB4ORA_StructuredType(Name="sample_text", is_final=True, is_instantiable=True)
    b1 = ORDB4ORA_TypedTable()
    b2 = ORDB4ORA_TypedTable()
    _safe_set(a, 'StructuredType78', b1)
    assert _is_linked(a, 'StructuredType78', b1)
    if hasattr(b1, 'typed77'):
        assert _is_linked(b1, 'typed77', a)
    _safe_set(a, 'StructuredType78', b2)
    assert _is_linked(a, 'StructuredType78', b2)
    if hasattr(b1, 'typed77'):
        assert not _is_linked(b1, 'typed77', a)
    if hasattr(b2, 'typed77'):
        assert _is_linked(b2, 'typed77', a)
    _safe_set(a, 'StructuredType78', None)
    assert not _is_linked(a, 'StructuredType78', b2)
    if hasattr(b2, 'typed77'):
        assert not _is_linked(b2, 'typed77', a)


def test_assoc_supertype56_link_reassign_clear():
    a = ORDB4ORA_StructuredType(Name="sample_text", is_final=True, is_instantiable=True)
    ...
    _safe_set(a, 'ORDB4ORA_StructuredType55', b1)
    assert _is_linked(a, 'ORDB4ORA_StructuredType55', b1)
    if hasattr(b1, 'ORDB4ORA_StructuredType57'):
        assert _is_linked(b1, 'ORDB4ORA_StructuredType57', a)
    _safe_set(a, 'ORDB4ORA_StructuredType55', b2)
    assert _is_linked(a, 'ORDB4ORA_StructuredType55', b2)
    if hasattr(b1, 'ORDB4ORA_StructuredType57'):
        assert not _is_linked(b1, 'ORDB4ORA_StructuredType57', a)
    if hasattr(b2, 'ORDB4ORA_StructuredType57'):
        assert _is_linked(b2, 'ORDB4ORA_StructuredType57', a)
    _safe_set(a, 'ORDB4ORA_StructuredType55', None)
    assert not _is_linked(a, 'ORDB4ORA_StructuredType55', b2)
    if hasattr(b2, 'ORDB4ORA_StructuredType57'):
        assert not _is_linked(b2, 'ORDB4ORA_StructuredType57', a)
...
def test_assoc_table73_link_reassign_clear():
    ...
    if hasattr(b1, 'Table74'):
        assert _is_linked(b1, 'Table74', a)
    _safe_set(a, 'triggers', b2)
    assert _is_linked(a, 'triggers', b2)
    if hasattr(b1, 'Table74'):
        assert not _is_linked(b1, 'Table74', a)
    if hasattr(b2, 'Table74'):
        assert _is_linked(b2, 'Table74', a)
    _safe_set(a, 'triggers', None)
    assert not _is_linked(a, 'triggers', b2)
    if hasattr(b2, 'Table74'):
        assert not _is_linked(b2, 'Table74', a)
...
def test_assoc_table8_link_reassign_clear():
    ...
    b1 = ORDB4ORA_Column()
    b2 = ORDB4ORA_Column()
    _safe_set(a, 'Table9', b1)
    assert _is_linked(a, 'Table9', b1)
    if hasattr(b1, 'columns'):
        assert _is_linked(b1, 'columns', a)
    _safe_set(a, 'Table9', b2)
    assert _is_linked(a, 'Table9', b2)
    if hasattr(b1, 'columns'):
        assert not _is_linked(b1, 'columns', a)
    if hasattr(b2, 'columns'):
        assert _is_linked(b2, 'columns', a)
    _safe_set(a, 'Table9', None)
    assert not _is_linked(a, 'Table9', b2)
    if hasattr(b2, 'columns'):
        assert not _is_linked(b2, 'columns', a)


def test_assoc_tables81_link_reassign_clear():
    a = ORDB4ORA_Table(Name="sample_text")
    b1 = ORDB4ORA_View()
    b2 = ORDB4ORA_View()
    _safe_set(a, 'Table82', b1)
    assert _is_linked(a, 'Table82', b1)
    if hasattr(b1, 'views'):
        assert _is_linked(b1, 'views', a)
    _safe_set(a, 'Table82', b2)
    assert _is_linked(a, 'Table82', b2)
    if hasattr(b1, 'views'):
        assert not _is_linked(b1, 'views', a)
    if hasattr(b2, 'views'):
        assert _is_linked(b2, 'views', a)
    _safe_set(a, 'Table82', None)
    assert not _is_linked(a, 'Table82', b2)
    if hasattr(b2, 'views'):
...
def test_assoc_triggers68_link_reassign_clear():
    ...
    if hasattr(b1, 'table69'):
        assert _is_linked(b1, 'table69', a)
    _safe_set(a, 'Trigger', b2)
    assert _is_linked(a, 'Trigger', b2)
    if hasattr(b1, 'table69'):
        assert not _is_linked(b1, 'table69', a)
    if hasattr(b2, 'table69'):
        assert _is_linked(b2, 'table69', a)
    _safe_set(a, 'Trigger', None)
    assert not _is_linked(a, 'Trigger', b2)
    if hasattr(b2, 'table69'):
        assert not _is_linked(b2, 'table69', a)
...
def test_assoc_typed58_link_reassign_clear():
    a = ORDB4ORA_StructuredType(Name="sample_text", is_final=True, is_instantiable=True)
    b1 = ORDB4ORA_TypedTable()
    b2 = ORDB4ORA_TypedTable()
    _safe_set(a, 'structured59', {b1})
    assert _is_linked(a, 'structured59', b1)
    if hasattr(b1, 'TypedTable60'):
        assert _is_linked(b1, 'TypedTable60', a)
    _safe_set(a, 'structured59', {b2})
    assert _is_linked(a, 'structured59', b2)
    if hasattr(b1, 'TypedTable60'):
        assert not _is_linked(b1, 'TypedTable60', a)
    if hasattr(b2, 'TypedTable60'):
        assert _is_linked(b2, 'TypedTable60', a)
    _safe_set(a, 'structured59', set())
    assert not _is_linked(a, 'structured59', b2)
    if hasattr(b2, 'TypedTable60'):
        assert not _is_linked(b2, 'TypedTable60', a)


def test_assoc_updateColumns71_link_reassign_clear():
    a = ORDB4ORA_Trigger(Action="sample_text", Body="sample_text", Event="sample_text", Name="sample_text")
    ...
    if hasattr(b1, 'ORDB4ORA_StructuralComponent72'):
        assert _is_linked(b1, 'ORDB4ORA_StructuralComponent72', a)
    _safe_set(a, 'ORDB4ORA_Trigger', {b2})
    assert _is_linked(a, 'ORDB4ORA_Trigger', b2)
    if hasattr(b1, 'ORDB4ORA_StructuralComponent72'):
        assert not _is_linked(b1, 'ORDB4ORA_StructuralComponent72', a)
    if hasattr(b2, 'ORDB4ORA_StructuralComponent72'):
        assert _is_linked(b2, 'ORDB4ORA_StructuralComponent72', a)
    _safe_set(a, 'ORDB4ORA_Trigger', set())
    assert not _is_linked(a, 'ORDB4ORA_Trigger', b2)
    if hasattr(b2, 'ORDB4ORA_StructuralComponent72'):
        assert not _is_linked(b2, 'ORDB4ORA_StructuralComponent72', a)


def test_assoc_views70_link_reassign_clear():
    a = ORDB4ORA_Table(Name="sample_text")
    b1 = ORDB4ORA_View()
    b2 = ORDB4ORA_View()
...
ORDB4ORA_Attribute_strategy = st.builds(ORDB4ORA_Attribute)
...
ORDB4ORA_Check_strategy = st.builds(ORDB4ORA_Check, Condition=safe_text)
@given(instance=ORDB4ORA_Check_strategy)
@settings(max_examples=25)
...
ORDB4ORA_ForeignKey_strategy = st.builds(ORDB4ORA_ForeignKey, OnDelete=safe_text)
@given(instance=ORDB4ORA_ForeignKey_strategy)
@settings(max_examples=25)
...
ORDB4ORA_NotNull_strategy = st.builds(ORDB4ORA_NotNull)
...
ORDB4ORA_PrimaryKey_strategy = st.builds(ORDB4ORA_PrimaryKey)
...
ORDB4ORA_Restriction_strategy = st.builds(ORDB4ORA_Restriction, Name=safe_text, NameColumns=safe_text)
@given(instance=ORDB4ORA_Restriction_strategy)
@settings(max_examples=25)
...
ORDB4ORA_Unique_strategy = st.builds(ORDB4ORA_Unique)
