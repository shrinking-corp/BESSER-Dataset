...
def test_hyp_mypackage2_orderstatus_exists():
    # Check that the Enumeration exists
    assert mypackage2_OrderStatus is not None

def test_hyp_mypackage2_orderstatus_has_all_literals():
    # Collect the names of literals in this Enumeration
    enum_literals = [lit.name for lit in mypackage2_OrderStatus]
    expected_literals = [
    ]
    # Check that all expected literals exist
    for lit_name in expected_literals:
        assert lit_name in enum_literals, f"Literal '' missing in mypackage2_OrderStatus"

def test_hyp_mypackage_userstate_exists():
    # Check that the Enumeration exists
    assert mypackage_UserState is not None

def test_hyp_mypackage_userstate_has_all_literals():
    # Collect the names of literals in this Enumeration
    enum_literals = [lit.name for lit in mypackage_UserState]
    expected_literals = [
    ]
    # Check that all expected literals exist
    for lit_name in expected_literals:
        assert lit_name in enum_literals, f"Literal '' missing in mypackage_UserState"
