...
class hw3_Passenger:
    ...
    def floor10(self, value):
        ...
        # Remove self from old opposite end
        if old_value is not None:
            if hasattr(old_value, "downward_Bound11"):
                opp_val = getattr(old_value, "downward_Bound11", None)
                if isinstance(opp_val, set):
                    opp_val.discard(self)
                
        # Add self to new opposite end
        if value is not None:
            if hasattr(value, "downward_Bound11"):
                opp_val = getattr(value, "downward_Bound11", None)
                if opp_val is None:
                    setattr(value, "downward_Bound11", set([self]))
                elif isinstance(opp_val, set):
                    opp_val.add(self)
...
class hw3_Floor:
    ...
    def __init__(self, passengersWaiting: int, myFloorNumber: int, upwardBound1: set["hw3_Passenger"] = None, residents9: set["hw3_Passenger"] = None, downward_Bound11: set["hw3_Passenger"] = None):
        ...
        self.downward_Bound11 = downward_Bound11 if downward_Bound11 is not None else set()
    ...
    @property
    def downward_Bound11(self):
        return self.__downward_Bound11
    @downward_Bound11.setter
    def downward_Bound11(self, value):
        # Bidirectional consistency
        old_value = getattr(self, f"_hw3_Floor__downward_Bound11", None)
        self.__downward_Bound11 = value if value is not None else set()
