...
def test_services_is_not_abstract():
    assert not inspect.isabstract(services)


def test_services_constructor_exists():
    assert callable(services.__init__)


def test_services_constructor_args():
    sig = inspect.signature(services.__init__)
    params = list(sig.parameters.keys())
    assert "location" in params, "Missing parameter 'location'"
    assert "database" in params, "Missing parameter 'database'"

def test_services_has_location():
    assert hasattr(services, "location")
    descriptor = None
    for klass in services.__mro__:
        if "location" in klass.__dict__:
            descriptor = klass.__dict__["location"]
            break
    assert isinstance(descriptor, property)

def test_services_has_database():
...
@given(instance=services_strategy)
def test_services_location_setter(instance):
...
@given(instance=services_strategy)
def test_services_database_setter(instance):
...
@given(instance=company_strategy)
def test_company_type_setter(instance):
