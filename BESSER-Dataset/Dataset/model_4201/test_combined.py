...
def test_hyp_typea_is_not_abstract():
    assert not inspect.isabstract(TypeA)


def test_hyp_typea_constructor_exists():
    assert callable(TypeA.__init__)


def test_hyp_typea_constructor_args():
    sig = inspect.signature(TypeA.__init__)
    params = list(sig.parameters.keys())



def test_hyp_mydsl_typeb_is_not_abstract():
    assert not inspect.isabstract(myDsl_TypeB)


def test_hyp_mydsl_typeb_constructor_exists():
    assert callable(myDsl_TypeB.__init__)


def test_hyp_mydsl_typeb_constructor_args():
...
def test_hyp_mydsl_typea_is_not_abstract():
    assert not inspect.isabstract(myDsl_TypeA)


def test_hyp_mydsl_typea_constructor_exists():
    assert callable(myDsl_TypeA.__init__)


def test_hyp_mydsl_typea_constructor_args():
...
def test_hyp_mydsl_typeb_fullname_setter(instance):
...
def test_hyp_mydsl_typea_name_setter(instance):
...
def test_myDsl_TypeA_name_value_roundtrip():
    instance = myDsl_TypeA(name="sample_text")
    assert instance.name == "sample_text"
    instance.name = "sample_text_2"
    assert instance.name == "sample_text_2"


def test_myDsl_TypeB_fullname_value_roundtrip():
    instance = myDsl_TypeB(fullname="sample_text")
    assert instance.fullname == "sample_text"
    instance.fullname = "sample_text_2"
    assert instance.fullname == "sample_text_2"


def test_myDsl_TypeB_isa_TypeA():
    instance = myDsl_TypeB(fullname="sample_text")
...
TypeA_strategy = st.builds(TypeA)
...
myDsl_TypeA_strategy = st.builds(myDsl_TypeA, name=safe_text)
@given(instance=myDsl_TypeA_strategy)
@settings(max_examples=25)
...
myDsl_TypeB_strategy = st.builds(myDsl_TypeB, fullname=safe_text)
@given(instance=myDsl_TypeB_strategy)
@settings(max_examples=25)
