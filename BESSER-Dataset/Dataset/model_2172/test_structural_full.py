...
def test_assoc_childClasses3_link_reassign_clear():
    ...
    _safe_set(a, 'TClass4', b1)
    assert _is_linked(a, 'TClass4', b1)
    if hasattr(b1, 'parentClass'):
        assert _is_linked(b1, 'parentClass', a)
    _safe_set(a, 'TClass4', b2)
    assert _is_linked(a, 'TClass4', b2)
    if hasattr(b1, 'parentClass'):
        assert not _is_linked(b1, 'parentClass', a)
    if hasattr(b2, 'parentClass'):
        assert _is_linked(b2, 'parentClass', a)
    _safe_set(a, 'TClass4', None)
    assert not _is_linked(a, 'TClass4', b2)
    if hasattr(b2, 'parentClass'):
        assert not _is_linked(b2, 'parentClass', a)


def test_assoc_classes62_link_reassign_clear():
    a = TypeGraphBasic_TypeGraph(tName="sample_text")
    b1 = TypeGraphBasic_TClass(tName="sample_text")
    b2 = TypeGraphBasic_TClass(tName="sample_text_2")
    _safe_set(a, 'TypeGraphBasic_TypeGraph63', {b1})
    assert _is_linked(a, 'TypeGraphBasic_TypeGraph63', b1)
    if hasattr(b1, 'TypeGraphBasic_TClass64'):
        assert _is_linked(b1, 'TypeGraphBasic_TClass64', a)
    _safe_set(a, 'TypeGraphBasic_TypeGraph63', {b2})
    assert _is_linked(a, 'TypeGraphBasic_TypeGraph63', b2)
    if hasattr(b1, 'TypeGraphBasic_TClass64'):
        assert not _is_linked(b1, 'TypeGraphBasic_TClass64', a)
    if hasattr(b2, 'TypeGraphBasic_TClass64'):
        assert _is_linked(b2, 'TypeGraphBasic_TClass64', a)
    _safe_set(a, 'TypeGraphBasic_TypeGraph63', set())
    assert not _is_linked(a, 'TypeGraphBasic_TypeGraph63', b2)
    if hasattr(b2, 'TypeGraphBasic_TClass64'):
        assert not _is_linked(b2, 'TypeGraphBasic_TClass64', a)


def test_assoc_containedClasses49_link_reassign_clear():
    a = TypeGraphBasic_TPackage(tName="sample_text")
    b1 = TypeGraphBasic_TClass(tName="sample_text")
    b2 = TypeGraphBasic_TClass(tName="sample_text_2")
    _safe_set(a, 'package', {b1})
    assert _is_linked(a, 'package', b1)
    if hasattr(b1, 'TClass50'):
        assert _is_linked(b1, 'TClass50', a)
    _safe_set(a, 'package', {b2})
    assert _is_linked(a, 'package', b2)
    if hasattr(b1, 'TClass50'):
        assert not _is_linked(b1, 'TClass50', a)
    if hasattr(b2, 'TClass50'):
        assert _is_linked(b2, 'TClass50', a)
    _safe_set(a, 'package', set())
    assert not _is_linked(a, 'package', b2)
    if hasattr(b2, 'TClass50'):
        assert not _is_linked(b2, 'TClass50', a)


def test_assoc_defines7_link_reassign_clear():
    a = TypeGraphBasic_TClass(tName="sample_text")
    b1 = TypeGraphBasic_TMember()
    b2 = TypeGraphBasic_TMember()
    _safe_set(a, 'TypeGraphBasic_TClass8', {b1})
    assert _is_linked(a, 'TypeGraphBasic_TClass8', b1)
    if hasattr(b1, 'TypeGraphBasic_TMember'):
        assert _is_linked(b1, 'TypeGraphBasic_TMember', a)
    _safe_set(a, 'TypeGraphBasic_TClass8', {b2})
    assert _is_linked(a, 'TypeGraphBasic_TClass8', b2)
    if hasattr(b1, 'TypeGraphBasic_TMember'):
    ...
    _safe_set(a, 'TypeGraphBasic_TClass8', set())
    assert not _is_linked(a, 'TypeGraphBasic_TClass8', b2)
    if hasattr(b2, 'TypeGraphBasic_TMember'):
...
def test_assoc_fields60_link_reassign_clear():
    a = TypeGraphBasic_TypeGraph(tName="sample_text")
    b1 = TypeGraphBasic_TField(tName="sample_text")
    b2 = TypeGraphBasic_TField(tName="sample_text_2")
    _safe_set(a, 'TypeGraphBasic_TypeGraph61', {b1})
    assert _is_linked(a, 'TypeGraphBasic_TypeGraph61', b1)
    if hasattr(b1, 'TypeGraphBasic_TField'):
        assert _is_linked(b1, 'TypeGraphBasic_TField', a)
    _safe_set(a, 'TypeGraphBasic_TypeGraph61', {b2})
    assert _is_linked(a, 'TypeGraphBasic_TypeGraph61', b2)
    if hasattr(b1, 'TypeGraphBasic_TField'):
    ...
    _safe_set(a, 'TypeGraphBasic_TypeGraph61', set())
    assert not _is_linked(a, 'TypeGraphBasic_TypeGraph61', b2)
    if hasattr(b2, 'TypeGraphBasic_TField'):
...
def test_assoc_methods58_link_reassign_clear():
    a = TypeGraphBasic_TypeGraph(tName="sample_text")
    b1 = TypeGraphBasic_TMethod(tName="sample_text")
    b2 = TypeGraphBasic_TMethod(tName="sample_text_2")
    _safe_set(a, 'TypeGraphBasic_TypeGraph59', {b1})
    assert _is_linked(a, 'TypeGraphBasic_TypeGraph59', b1)
    if hasattr(b1, 'TypeGraphBasic_TMethod'):
        assert _is_linked(b1, 'TypeGraphBasic_TMethod', a)
    _safe_set(a, 'TypeGraphBasic_TypeGraph59', {b2})
    assert _is_linked(a, 'TypeGraphBasic_TypeGraph59', b2)
    if hasattr(b1, 'TypeGraphBasic_TMethod'):
    ...
    _safe_set(a, 'TypeGraphBasic_TypeGraph59', set())
    assert not _is_linked(a, 'TypeGraphBasic_TypeGraph59', b2)
    if hasattr(b2, 'TypeGraphBasic_TMethod'):
        assert not _is_linked(b2, 'TypeGraphBasic_TMethod', a)


def test_assoc_package6_link_reassign_clear():
    a = TypeGraphBasic_TPackage(tName="sample_text")
    b1 = TypeGraphBasic_TClass(tName="sample_text")
    b2 = TypeGraphBasic_TClass(tName="sample_text_2")
...
def test_assoc_packages57_link_reassign_clear():
    a = TypeGraphBasic_TypeGraph(tName="sample_text")
...
def test_assoc_paramList47_link_reassign_clear():
    a = TypeGraphBasic_TClass(tName="sample_text")
    b1 = TypeGraphBasic_TMethodSignature()
    b2 = TypeGraphBasic_TMethodSignature()
    _safe_set(a, 'TypeGraphBasic_TClass48', b1)
    assert _is_linked(a, 'TypeGraphBasic_TClass48', b1)
    if hasattr(b1, 'TypeGraphBasic_TMethodSignature'):
        assert _is_linked(b1, 'TypeGraphBasic_TMethodSignature', a)
    _safe_set(a, 'TypeGraphBasic_TClass48', b2)
    assert _is_linked(a, 'TypeGraphBasic_TClass48', b2)
    if hasattr(b1, 'TypeGraphBasic_TMethodSignature'):
        assert not _is_linked(b1, 'TypeGraphBasic_TMethodSignature', a)
    if hasattr(b2, 'TypeGraphBasic_TMethodSignature'):
        assert _is_linked(b2, 'TypeGraphBasic_TMethodSignature', a)
    _safe_set(a, 'TypeGraphBasic_TClass48', None)
    assert not _is_linked(a, 'TypeGraphBasic_TClass48', b2)
    if hasattr(b2, 'TypeGraphBasic_TMethodSignature'):
        assert not _is_linked(b2, 'TypeGraphBasic_TMethodSignature', a)
...
def test_assoc_parent55_link_reassign_clear():
    ...
    b1 = TypeGraphBasic_TPackage(tName="sample_text")
    b2 = TypeGraphBasic_TPackage(tName="sample_text_2")
    _safe_set(a, 'TPackage56', b1)
    assert _is_linked(a, 'TPackage56', b1)
    if hasattr(b1, 'subpackage'):
        assert _is_linked(b1, 'subpackage', a)
    _safe_set(a, 'TPackage56', b2)
    assert _is_linked(a, 'TPackage56', b2)
    if hasattr(b1, 'subpackage'):
        assert not _is_linked(b1, 'subpackage', a)
    if hasattr(b2, 'subpackage'):
        assert _is_linked(b2, 'subpackage', a)
    _safe_set(a, 'TPackage56', None)
    assert not _is_linked(a, 'TPackage56', b2)
    if hasattr(b2, 'subpackage'):
        assert not _is_linked(b2, 'subpackage', a)


def test_assoc_parentClass1_link_reassign_clear():
    a = TypeGraphBasic_TClass(tName="sample_text")
    b1 = TypeGraphBasic_TClass(tName="sample_text")
    b2 = TypeGraphBasic_TClass(tName="sample_text_2")
...
def test_assoc_signature5_link_reassign_clear():
    a = TypeGraphBasic_TClass(tName="sample_text")
...
def test_assoc_subpackage52_link_reassign_clear():
    a = TypeGraphBasic_TPackage(tName="sample_text")
    b1 = TypeGraphBasic_TPackage(tName="sample_text")
    b2 = TypeGraphBasic_TPackage(tName="sample_text_2")
    _safe_set(a, 'TPackage53', b1)
    assert _is_linked(a, 'TPackage53', b1)
    if hasattr(b1, 'parent'):
        assert _is_linked(b1, 'parent', a)
    _safe_set(a, 'TPackage53', b2)
    assert _is_linked(a, 'TPackage53', b2)
    if hasattr(b1, 'parent'):
        assert not _is_linked(b1, 'parent', a)
    if hasattr(b2, 'parent'):
        assert _is_linked(b2, 'parent', a)
    _safe_set(a, 'TPackage53', None)
    assert not _is_linked(a, 'TPackage53', b2)
    if hasattr(b2, 'parent'):
