...
def test_hyp_database_component_is_not_abstract():
    assert not inspect.isabstract(DataBase_Component)


def test_hyp_database_component_constructor_exists():
    assert callable(DataBase_Component.__init__)


def test_hyp_database_component_constructor_args():
    sig = inspect.signature(DataBase_Component.__init__)
    params = list(sig.parameters.keys())



def test_hyp_availability_component_is_not_abstract():
    assert not inspect.isabstract(Availability_Component)


def test_hyp_availability_component_constructor_exists():
    assert callable(Availability_Component.__init__)


def test_hyp_availability_component_constructor_args():
    sig = inspect.signature(Availability_Component.__init__)
    params = list(sig.parameters.keys())



def test_hyp_login_component_is_not_abstract():
    assert not inspect.isabstract(Login_Component)


def test_hyp_login_component_constructor_exists():
    assert callable(Login_Component.__init__)


def test_hyp_login_component_constructor_args():
...
@given(instance=Admin_strategy)
def test_hyp_admin_doctorid_setter(instance):
...
def test_Admin_doctorid_value_roundtrip():
    instance = Admin(doctorid=7, id=7, name="sample_text")
...
def test_assoc_Department_Patient_link_reassign_clear():
    a = Patient(address="sample_text", age=7, id=7, name="sample_text", roomno=7, sex="sample_text", telno=7)
    b1 = Admin(doctorid=7, id=7, name="sample_text")
    b2 = Admin(doctorid=13, id=13, name="sample_text_2")
...
Admin_strategy = st.builds(Admin, doctorid=st.integers(), id=st.integers(), name=safe_text)
@given(instance=Admin_strategy)
...
Availability_Component_strategy = st.builds(Availability_Component)
...
DataBase_Component_strategy = st.builds(DataBase_Component)
...
Login_Component_strategy = st.builds(Login_Component)
