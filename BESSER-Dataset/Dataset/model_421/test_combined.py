...
def test_assoc_association2_link_reassign_clear():
    ...
    b1 = UML2_Association()
    b2 = UML2_Association()
    _safe_set(a, 'UML2_Property3', b1)
    assert _is_linked(a, 'UML2_Property3', b1)
    if hasattr(b1, 'UML2_Association'):
        assert _is_linked(b1, 'UML2_Association', a)
    _safe_set(a, 'UML2_Property3', b2)
    assert _is_linked(a, 'UML2_Property3', b2)
    if hasattr(b1, 'UML2_Association'):
        assert not _is_linked(b1, 'UML2_Association', a)
    if hasattr(b2, 'UML2_Association'):
        assert _is_linked(b2, 'UML2_Association', a)
    _safe_set(a, 'UML2_Property3', None)
    assert not _is_linked(a, 'UML2_Property3', b2)
    if hasattr(b2, 'UML2_Association'):
        assert not _is_linked(b2, 'UML2_Association', a)


def test_assoc_bodyCondition17_link_reassign_clear():
    a = UML2_Operation(isQuery=True)
    b1 = UML2_Constraint()
    b2 = UML2_Constraint()
    _safe_set(a, 'UML2_Operation18', b1)
    assert _is_linked(a, 'UML2_Operation18', b1)
    if hasattr(b1, 'UML2_Constraint'):
        assert _is_linked(b1, 'UML2_Constraint', a)
    _safe_set(a, 'UML2_Operation18', b2)
    assert _is_linked(a, 'UML2_Operation18', b2)
    if hasattr(b1, 'UML2_Constraint'):
        assert not _is_linked(b1, 'UML2_Constraint', a)
    if hasattr(b2, 'UML2_Constraint'):
        assert _is_linked(b2, 'UML2_Constraint', a)
    _safe_set(a, 'UML2_Operation18', None)
    assert not _is_linked(a, 'UML2_Operation18', b2)
    if hasattr(b2, 'UML2_Constraint'):
        assert not _is_linked(b2, 'UML2_Constraint', a)


def test_assoc_classifier37_link_reassign_clear():
    a = UML2_Classifier(isAbstract=True)
    b1 = UML2_CreateObjectAction()
    b2 = UML2_CreateObjectAction()
    _safe_set(a, 'UML2_Classifier38', b1)
    assert _is_linked(a, 'UML2_Classifier38', b1)
    if hasattr(b1, 'UML2_CreateObjectAction'):
        assert _is_linked(b1, 'UML2_CreateObjectAction', a)
    _safe_set(a, 'UML2_Classifier38', b2)
    assert _is_linked(a, 'UML2_Classifier38', b2)
    if hasattr(b1, 'UML2_CreateObjectAction'):
    ...
    _safe_set(a, 'UML2_Classifier38', None)
    assert not _is_linked(a, 'UML2_Classifier38', b2)
    if hasattr(b2, 'UML2_CreateObjectAction'):
        assert not _is_linked(b2, 'UML2_CreateObjectAction', a)


def test_assoc_classifier47_link_reassign_clear():
    a = UML2_Classifier(isAbstract=True)
    b1 = UML2_InstanceSpecification()
    b2 = UML2_InstanceSpecification()
    _safe_set(a, 'UML2_Classifier49', b1)
    assert _is_linked(a, 'UML2_Classifier49', b1)
    if hasattr(b1, 'UML2_InstanceSpecification48'):
        assert _is_linked(b1, 'UML2_InstanceSpecification48', a)
    _safe_set(a, 'UML2_Classifier49', b2)
    assert _is_linked(a, 'UML2_Classifier49', b2)
    if hasattr(b1, 'UML2_InstanceSpecification48'):
        assert not _is_linked(b1, 'UML2_InstanceSpecification48', a)
    if hasattr(b2, 'UML2_InstanceSpecification48'):
        assert _is_linked(b2, 'UML2_InstanceSpecification48', a)
    _safe_set(a, 'UML2_Classifier49', None)
    assert not _is_linked(a, 'UML2_Classifier49', b2)
    if hasattr(b2, 'UML2_InstanceSpecification48'):
        assert not _is_linked(b2, 'UML2_InstanceSpecification48', a)


def test_assoc_context_14_link_reassign_clear():
    a = UML2_Classifier(isAbstract=True)
    b1 = UML2_Action()
    b2 = UML2_Action()
    _safe_set(a, 'UML2_Classifier15', b1)
    assert _is_linked(a, 'UML2_Classifier15', b1)
    if hasattr(b1, 'UML2_Action'):
        assert _is_linked(b1, 'UML2_Action', a)
    _safe_set(a, 'UML2_Classifier15', b2)
    assert _is_linked(a, 'UML2_Classifier15', b2)
    if hasattr(b1, 'UML2_Action'):
        assert not _is_linked(b1, 'UML2_Action', a)
    if hasattr(b2, 'UML2_Action'):
        assert _is_linked(b2, 'UML2_Action', a)
    _safe_set(a, 'UML2_Classifier15', None)
    assert not _is_linked(a, 'UML2_Classifier15', b2)
    if hasattr(b2, 'UML2_Action'):
        assert not _is_linked(b2, 'UML2_Action', a)


def test_assoc_definingFeature13_link_reassign_clear():
...
def test_assoc_feature52_link_reassign_clear():
    ...
    _safe_set(a, 'UML2_Feature54', b1)
    assert _is_linked(a, 'UML2_Feature54', b1)
    if hasattr(b1, 'UML2_Classifier53'):
        assert _is_linked(b1, 'UML2_Classifier53', a)
    _safe_set(a, 'UML2_Feature54', b2)
    assert _is_linked(a, 'UML2_Feature54', b2)
    if hasattr(b1, 'UML2_Classifier53'):
        assert not _is_linked(b1, 'UML2_Classifier53', a)
    if hasattr(b2, 'UML2_Classifier53'):
        assert _is_linked(b2, 'UML2_Classifier53', a)
    _safe_set(a, 'UML2_Feature54', None)
    assert not _is_linked(a, 'UML2_Feature54', b2)
    if hasattr(b2, 'UML2_Classifier53'):
        assert not _is_linked(b2, 'UML2_Classifier53', a)


def test_assoc_featuringClassifier11_link_reassign_clear():
    a = UML2_Feature(isStatic=True)
    b1 = UML2_Classifier(isAbstract=True)
    b2 = UML2_Classifier(isAbstract=False)
    _safe_set(a, 'UML2_Feature', {b1})
    assert _is_linked(a, 'UML2_Feature', b1)
    if hasattr(b1, 'UML2_Classifier12'):
        assert _is_linked(b1, 'UML2_Classifier12', a)
    _safe_set(a, 'UML2_Feature', {b2})
    assert _is_linked(a, 'UML2_Feature', b2)
    if hasattr(b1, 'UML2_Classifier12'):
        assert not _is_linked(b1, 'UML2_Classifier12', a)
    if hasattr(b2, 'UML2_Classifier12'):
        assert _is_linked(b2, 'UML2_Classifier12', a)
    _safe_set(a, 'UML2_Feature', set())
    assert not _is_linked(a, 'UML2_Feature', b2)
    if hasattr(b2, 'UML2_Classifier12'):
        assert not _is_linked(b2, 'UML2_Classifier12', a)
...
def test_assoc_general9_link_reassign_clear():
    ...
    if hasattr(b1, 'UML2_Generalization10'):
        assert _is_linked(b1, 'UML2_Generalization10', a)
    _safe_set(a, 'UML2_Classifier', b2)
    assert _is_linked(a, 'UML2_Classifier', b2)
    if hasattr(b1, 'UML2_Generalization10'):
        assert not _is_linked(b1, 'UML2_Generalization10', a)
    if hasattr(b2, 'UML2_Generalization10'):
        assert _is_linked(b2, 'UML2_Generalization10', a)
    _safe_set(a, 'UML2_Classifier', None)
    assert not _is_linked(a, 'UML2_Classifier', b2)
    if hasattr(b2, 'UML2_Generalization10'):
        assert not _is_linked(b2, 'UML2_Generalization10', a)


def test_assoc_generalization58_link_reassign_clear():
    a = UML2_Classifier(isAbstract=True)
    b1 = UML2_Generalization()
    b2 = UML2_Generalization()
    _safe_set(a, 'UML2_Classifier59', {b1})
    assert _is_linked(a, 'UML2_Classifier59', b1)
    if hasattr(b1, 'UML2_Generalization60'):
        assert _is_linked(b1, 'UML2_Generalization60', a)
    _safe_set(a, 'UML2_Classifier59', {b2})
    assert _is_linked(a, 'UML2_Classifier59', b2)
    if hasattr(b1, 'UML2_Generalization60'):
    ...
    _safe_set(a, 'UML2_Classifier59', set())
    assert not _is_linked(a, 'UML2_Classifier59', b2)
    if hasattr(b2, 'UML2_Generalization60'):
        assert not _is_linked(b2, 'UML2_Generalization60', a)


def test_assoc_inheritedMember55_link_reassign_clear():
    a = UML2_NamedElement(name="sample_text", visibility="sample_text")
    b1 = UML2_Classifier(isAbstract=True)
    b2 = UML2_Classifier(isAbstract=False)
    _safe_set(a, 'UML2_NamedElement57', b1)
    assert _is_linked(a, 'UML2_NamedElement57', b1)
    if hasattr(b1, 'UML2_Classifier56'):
        assert _is_linked(b1, 'UML2_Classifier56', a)
    _safe_set(a, 'UML2_NamedElement57', b2)
    assert _is_linked(a, 'UML2_NamedElement57', b2)
    if hasattr(b1, 'UML2_Classifier56'):
        assert not _is_linked(b1, 'UML2_Classifier56', a)
    if hasattr(b2, 'UML2_Classifier56'):
        assert _is_linked(b2, 'UML2_Classifier56', a)
    _safe_set(a, 'UML2_NamedElement57', None)
    assert not _is_linked(a, 'UML2_NamedElement57', b2)
    if hasattr(b2, 'UML2_Classifier56'):
        assert not _is_linked(b2, 'UML2_Classifier56', a)


def test_assoc_member51_link_reassign_clear():
    a = UML2_NamedElement(name="sample_text", visibility="sample_text")
    b1 = UML2_Namespace()
    b2 = UML2_Namespace()
...
def test_assoc_nestedClassifier31_link_reassign_clear():
    a = UML2_Classifier(isAbstract=True)
    b1 = UML2_Class(isActive=True)
    b2 = UML2_Class(isActive=False)
    _safe_set(a, 'UML2_Classifier32', b1)
    assert _is_linked(a, 'UML2_Classifier32', b1)
    if hasattr(b1, 'UML2_Class'):
        assert _is_linked(b1, 'UML2_Class', a)
    _safe_set(a, 'UML2_Classifier32', b2)
    assert _is_linked(a, 'UML2_Classifier32', b2)
    if hasattr(b1, 'UML2_Class'):
        assert not _is_linked(b1, 'UML2_Class', a)
    if hasattr(b2, 'UML2_Class'):
        assert _is_linked(b2, 'UML2_Class', a)
    _safe_set(a, 'UML2_Classifier32', None)
    assert not _is_linked(a, 'UML2_Classifier32', b2)
    if hasattr(b2, 'UML2_Class'):
        assert not _is_linked(b2, 'UML2_Class', a)


def test_assoc_ownedEnd25_link_reassign_clear():
    a = UML2_Property(aggregation="sample_text", isComposite=True, isDerived=True, isDerivedUnion=True)
    b1 = UML2_Association()
    b2 = UML2_Association()
    _safe_set(a, 'UML2_Property27', b1)
    assert _is_linked(a, 'UML2_Property27', b1)
    if hasattr(b1, 'UML2_Association26'):
        assert _is_linked(b1, 'UML2_Association26', a)
    _safe_set(a, 'UML2_Property27', b2)
    assert _is_linked(a, 'UML2_Property27', b2)
    if hasattr(b1, 'UML2_Association26'):
        assert not _is_linked(b1, 'UML2_Association26', a)
    if hasattr(b2, 'UML2_Association26'):
        assert _is_linked(b2, 'UML2_Association26', a)
    _safe_set(a, 'UML2_Property27', None)
    assert not _is_linked(a, 'UML2_Property27', b2)
    if hasattr(b2, 'UML2_Association26'):
        assert not _is_linked(b2, 'UML2_Association26', a)


def test_assoc_ownedParameter16_link_reassign_clear():
...
def test_assoc_ownedReception33_link_reassign_clear():
    a = UML2_Class(isActive=True)
    b1 = UML2_Reception()
    b2 = UML2_Reception()
    _safe_set(a, 'UML2_Class34', {b1})
    assert _is_linked(a, 'UML2_Class34', b1)
    if hasattr(b1, 'UML2_Reception'):
        assert _is_linked(b1, 'UML2_Reception', a)
    _safe_set(a, 'UML2_Class34', {b2})
    assert _is_linked(a, 'UML2_Class34', b2)
    if hasattr(b1, 'UML2_Reception'):
        assert not _is_linked(b1, 'UML2_Reception', a)
    if hasattr(b2, 'UML2_Reception'):
        assert _is_linked(b2, 'UML2_Reception', a)
    _safe_set(a, 'UML2_Class34', set())
    assert not _is_linked(a, 'UML2_Class34', b2)
    if hasattr(b2, 'UML2_Reception'):
        assert not _is_linked(b2, 'UML2_Reception', a)


def test_assoc_qualifier19_link_reassign_clear():
    a = UML2_Property(aggregation="sample_text", isComposite=True, isDerived=True, isDerivedUnion=True)
    b1 = UML2_QualifierValue()
    b2 = UML2_QualifierValue()
    _safe_set(a, 'UML2_Property20', b1)
    assert _is_linked(a, 'UML2_Property20', b1)
    if hasattr(b1, 'UML2_QualifierValue'):
        assert _is_linked(b1, 'UML2_QualifierValue', a)
    _safe_set(a, 'UML2_Property20', b2)
    assert _is_linked(a, 'UML2_Property20', b2)
    if hasattr(b1, 'UML2_QualifierValue'):
    ...
    _safe_set(a, 'UML2_Property20', None)
    assert not _is_linked(a, 'UML2_Property20', b2)
    if hasattr(b2, 'UML2_QualifierValue'):
        assert not _is_linked(b2, 'UML2_QualifierValue', a)


def test_assoc_qualifier5_link_reassign_clear():
    a = UML2_Property(aggregation="sample_text", isComposite=True, isDerived=True, isDerivedUnion=True)
    ...
    if hasattr(b1, 'UML2_Property6'):
        assert _is_linked(b1, 'UML2_Property6', a)
    _safe_set(a, 'UML2_Property4', {b2})
    assert _is_linked(a, 'UML2_Property4', b2)
    if hasattr(b1, 'UML2_Property6'):
        assert not _is_linked(b1, 'UML2_Property6', a)
    if hasattr(b2, 'UML2_Property6'):
        assert _is_linked(b2, 'UML2_Property6', a)
    _safe_set(a, 'UML2_Property4', set())
    assert not _is_linked(a, 'UML2_Property4', b2)
    if hasattr(b2, 'UML2_Property6'):
        assert not _is_linked(b2, 'UML2_Property6', a)


def test_assoc_structuralFeature67_link_reassign_clear():
    a = UML2_StructuralFeature(isReadOnly=True)
...
def test_assoc_subsettedProperty1_link_reassign_clear():
    a = UML2_Property(aggregation="sample_text", isComposite=True, isDerived=True, isDerivedUnion=True)
    b1 = UML2_Property(aggregation="sample_text", isComposite=True, isDerived=True, isDerivedUnion=True)
    b2 = UML2_Property(aggregation="sample_text_2", isComposite=False, isDerived=False, isDerivedUnion=False)
    _safe_set(a, 'UML2_Property', b1)
    assert _is_linked(a, 'UML2_Property', b1)
    if hasattr(b1, 'UML2_Property0'):
        assert _is_linked(b1, 'UML2_Property0', a)
    _safe_set(a, 'UML2_Property', b2)
    assert _is_linked(a, 'UML2_Property', b2)
    if hasattr(b1, 'UML2_Property0'):
        assert not _is_linked(b1, 'UML2_Property0', a)
    if hasattr(b2, 'UML2_Property0'):
        assert _is_linked(b2, 'UML2_Property0', a)
    _safe_set(a, 'UML2_Property', None)
    assert not _is_linked(a, 'UML2_Property', b2)
    if hasattr(b2, 'UML2_Property0'):
        assert not _is_linked(b2, 'UML2_Property0', a)
