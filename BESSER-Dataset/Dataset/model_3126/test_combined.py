...
def test_hyp_umlmm_dummy_is_not_abstract():
    assert not inspect.isabstract(umlMM_dummy)


def test_hyp_umlmm_dummy_constructor_exists():
    assert callable(umlMM_dummy.__init__)


def test_hyp_umlmm_dummy_constructor_args():
...
def test_hyp_umlmm_association_name_setter(instance):
    original = instance.name
    instance.name = original
    assert instance.name == original
