...
def test_hyp_afamilyregistry_is_not_abstract():
    assert not inspect.isabstract(aFamilyRegistry)


def test_hyp_afamilyregistry_constructor_exists():
    assert callable(aFamilyRegistry.__init__)


def test_hyp_afamilyregistry_constructor_args():
    sig = inspect.signature(aFamilyRegistry.__init__)
    params = list(sig.parameters.keys())
...
aFamilyRegistry_strategy = st.builds(
    aFamilyRegistry,
)
...
def test_assoc_links10_link_reassign_clear():
    a = Families_Family(address="sample_text", lastName="sample_text")
    b1 = aFamily()
    b2 = aFamily()
...
aFamilyRegistry_strategy = st.builds(aFamilyRegistry)
@given(instance=aFamilyRegistry_strategy)
@settings(max_examples=25)
def test_aFamilyRegistry_instantiation(instance):
    assert isinstance(instance, aFamilyRegistry)
