...
def test_ecore_memoclass_is_not_abstract():
    assert not inspect.isabstract(ecore_MemoClass)


def test_ecore_memoclass_constructor_exists():
    assert callable(ecore_MemoClass.__init__)


def test_ecore_memoclass_constructor_args():
    sig = inspect.signature(ecore_MemoClass.__init__)
    params = list(sig.parameters.keys())
    assert "instance" in params, "Missing parameter 'instance'"

def test_ecore_memoclass_has_instance():
    assert hasattr(ecore_MemoClass, "instance")
    descriptor = None
    for klass in ecore_MemoClass.__mro__:
...
@given(instance=ecore_MemoClass_strategy)
def test_ecore_memoclass_instance_setter(instance):
