...
def test_assoc_attribute69_link_reassign_clear():
    a = OCL_OclType(name="sample_text")
    b1 = Attribute()
    b2 = Attribute()
    _safe_set(a, 'type70', b1)
    assert _is_linked(a, 'type70', b1)
    if hasattr(b1, 'Attribute71'):
        assert _is_linked(b1, 'Attribute71', a)
    _safe_set(a, 'type70', b2)
    assert _is_linked(a, 'type70', b2)
    if hasattr(b1, 'Attribute71'):
        assert not _is_linked(b1, 'Attribute71', a)
    if hasattr(b2, 'Attribute71'):
        assert _is_linked(b2, 'Attribute71', a)
    _safe_set(a, 'type70', None)
    assert not _is_linked(a, 'type70', b2)
    if hasattr(b2, 'Attribute71'):
        assert not _is_linked(b2, 'Attribute71', a)


def test_assoc_baseExp55_link_reassign_clear():
...
def test_assoc_body109_link_reassign_clear():
    a = OCL_Operation(name="sample_text")
    b1 = OclExpression()
    b2 = OclExpression()
    _safe_set(a, 'owningOperation', b1)
    assert _is_linked(a, 'owningOperation', b1)
    if hasattr(b1, 'OclExpression110'):
        assert _is_linked(b1, 'OclExpression110', a)
    _safe_set(a, 'owningOperation', b2)
    assert _is_linked(a, 'owningOperation', b2)
    if hasattr(b1, 'OclExpression110'):
        assert not _is_linked(b1, 'OclExpression110', a)
    if hasattr(b2, 'OclExpression110'):
        assert _is_linked(b2, 'OclExpression110', a)
    _safe_set(a, 'owningOperation', None)
    assert not _is_linked(a, 'owningOperation', b2)
    if hasattr(b2, 'OclExpression110'):
        assert not _is_linked(b2, 'OclExpression110', a)


def test_assoc_collectionTypes74_link_reassign_clear():
    a = OCL_OclType(name="sample_text")
    b1 = CollectionType()
    b2 = CollectionType()
...
def test_assoc_definitions63_link_reassign_clear():
    a = OCL_OclType(name="sample_text")
...
def test_assoc_initExpression101_link_reassign_clear():
    a = OCL_Attribute(name="sample_text")
    b1 = OclExpression()
    b2 = OclExpression()
    _safe_set(a, 'owningAttribute', b1)
    assert _is_linked(a, 'owningAttribute', b1)
    if hasattr(b1, 'OclExpression102'):
        assert _is_linked(b1, 'OclExpression102', a)
    _safe_set(a, 'owningAttribute', b2)
    assert _is_linked(a, 'owningAttribute', b2)
    if hasattr(b1, 'OclExpression102'):
        assert not _is_linked(b1, 'OclExpression102', a)
    if hasattr(b2, 'OclExpression102'):
        assert _is_linked(b2, 'OclExpression102', a)
    _safe_set(a, 'owningAttribute', None)
    assert not _is_linked(a, 'owningAttribute', b2)
    if hasattr(b2, 'OclExpression102'):
        assert not _is_linked(b2, 'OclExpression102', a)


def test_assoc_initExpression51_link_reassign_clear():
    a = OCL_VariableDeclaration(id="sample_text", varName="sample_text")
    b1 = OclExpression()
    b2 = OclExpression()
...
def test_assoc_letExp53_link_reassign_clear():
    a = OCL_VariableDeclaration(id="sample_text", varName="sample_text")
    b1 = LetExp()
    b2 = LetExp()
    _safe_set(a, 'variable', b1)
    assert _is_linked(a, 'variable', b1)
    if hasattr(b1, 'LetExp54'):
        assert _is_linked(b1, 'LetExp54', a)
    _safe_set(a, 'variable', b2)
    assert _is_linked(a, 'variable', b2)
    if hasattr(b1, 'LetExp54'):
        assert not _is_linked(b1, 'LetExp54', a)
    if hasattr(b2, 'LetExp54'):
        assert _is_linked(b2, 'LetExp54', a)
    _safe_set(a, 'variable', None)
    assert not _is_linked(a, 'variable', b2)
    if hasattr(b2, 'LetExp54'):
        assert not _is_linked(b2, 'LetExp54', a)


def test_assoc_mapType268_link_reassign_clear():
    a = OCL_OclType(name="sample_text")
    b1 = MapType()
    b2 = MapType()
...
def test_assoc_mapType72_link_reassign_clear():
    a = OCL_OclType(name="sample_text")
    b1 = MapType()
    b2 = MapType()
    _safe_set(a, 'keyType', b1)
    assert _is_linked(a, 'keyType', b1)
    if hasattr(b1, 'MapType73'):
        assert _is_linked(b1, 'MapType73', a)
    _safe_set(a, 'keyType', b2)
    assert _is_linked(a, 'keyType', b2)
    if hasattr(b1, 'MapType73'):
        assert not _is_linked(b1, 'MapType73', a)
    if hasattr(b2, 'MapType73'):
        assert _is_linked(b2, 'MapType73', a)
    _safe_set(a, 'keyType', None)
    assert not _is_linked(a, 'keyType', b2)
    if hasattr(b2, 'MapType73'):
        assert not _is_linked(b2, 'MapType73', a)
...
def test_assoc_metamodel111_link_reassign_clear():
    ...
    if hasattr(b1, 'OclModel112'):
        assert _is_linked(b1, 'OclModel112', a)
    _safe_set(a, 'model', b2)
    assert _is_linked(a, 'model', b2)
    if hasattr(b1, 'OclModel112'):
        assert not _is_linked(b1, 'OclModel112', a)
    if hasattr(b2, 'OclModel112'):
        assert _is_linked(b2, 'OclModel112', a)
    _safe_set(a, 'model', None)
    assert not _is_linked(a, 'model', b2)
    if hasattr(b2, 'OclModel112'):
        assert not _is_linked(b2, 'OclModel112', a)


def test_assoc_model115_link_reassign_clear():
    a = OCL_OclModel(name="sample_text")
    b1 = OclModel()
    b2 = OclModel()
    _safe_set(a, 'metamodel', {b1})
    assert _is_linked(a, 'metamodel', b1)
    if hasattr(b1, 'OclModel116'):
        assert _is_linked(b1, 'OclModel116', a)
    _safe_set(a, 'metamodel', {b2})
    assert _is_linked(a, 'metamodel', b2)
    if hasattr(b1, 'OclModel116'):
        assert not _is_linked(b1, 'OclModel116', a)
    if hasattr(b2, 'OclModel116'):
        assert _is_linked(b2, 'OclModel116', a)
    _safe_set(a, 'metamodel', set())
    assert not _is_linked(a, 'metamodel', b2)
    if hasattr(b2, 'OclModel116'):
        assert not _is_linked(b2, 'OclModel116', a)


def test_assoc_oclExpression64_link_reassign_clear():
    a = OCL_OclType(name="sample_text")
    b1 = OclExpression()
    b2 = OclExpression()
    _safe_set(a, 'type', b1)
...
def test_assoc_operation66_link_reassign_clear():
    a = OCL_OclType(name="sample_text")
    b1 = Operation()
    b2 = Operation()
    _safe_set(a, 'returnType', b1)
    assert _is_linked(a, 'returnType', b1)
    if hasattr(b1, 'Operation67'):
        assert _is_linked(b1, 'Operation67', a)
    _safe_set(a, 'returnType', b2)
    assert _is_linked(a, 'returnType', b2)
    if hasattr(b1, 'Operation67'):
        assert not _is_linked(b1, 'Operation67', a)
    if hasattr(b2, 'Operation67'):
        assert _is_linked(b2, 'Operation67', a)
    _safe_set(a, 'returnType', None)
    assert not _is_linked(a, 'returnType', b2)
    if hasattr(b2, 'Operation67'):
        assert not _is_linked(b2, 'Operation67', a)


def test_assoc_parameters105_link_reassign_clear():
    a = OCL_Operation(name="sample_text")
    b1 = Parameter()
    b2 = Parameter()
...
def test_assoc_returnType106_link_reassign_clear():
    ...
    b1 = OclType()
    b2 = OclType()
    _safe_set(a, 'operation107', b1)
    assert _is_linked(a, 'operation107', b1)
    if hasattr(b1, 'OclType108'):
        assert _is_linked(b1, 'OclType108', a)
    _safe_set(a, 'operation107', b2)
    assert _is_linked(a, 'operation107', b2)
    if hasattr(b1, 'OclType108'):
        assert not _is_linked(b1, 'OclType108', a)
    if hasattr(b2, 'OclType108'):
        assert _is_linked(b2, 'OclType108', a)
    _safe_set(a, 'operation107', None)
    assert not _is_linked(a, 'operation107', b2)
    if hasattr(b2, 'OclType108'):
        assert not _is_linked(b2, 'OclType108', a)


def test_assoc_tupleType84_link_reassign_clear():
...
def test_assoc_type103_link_reassign_clear():
    ...
    if hasattr(b1, 'OclType104'):
        assert _is_linked(b1, 'OclType104', a)
    _safe_set(a, 'attribute', b2)
    assert _is_linked(a, 'attribute', b2)
    if hasattr(b1, 'OclType104'):
        assert not _is_linked(b1, 'OclType104', a)
    if hasattr(b2, 'OclType104'):
        assert _is_linked(b2, 'OclType104', a)
    _safe_set(a, 'attribute', None)
    assert not _is_linked(a, 'attribute', b2)
    if hasattr(b2, 'OclType104'):
        assert not _is_linked(b2, 'OclType104', a)
...
def test_assoc_type49_link_reassign_clear():
    ...
    if hasattr(b1, 'OclType50'):
        assert _is_linked(b1, 'OclType50', a)
    _safe_set(a, 'variableDeclaration', b2)
    assert _is_linked(a, 'variableDeclaration', b2)
    if hasattr(b1, 'OclType50'):
        assert not _is_linked(b1, 'OclType50', a)
    if hasattr(b2, 'OclType50'):
        assert _is_linked(b2, 'OclType50', a)
    _safe_set(a, 'variableDeclaration', None)
    assert not _is_linked(a, 'variableDeclaration', b2)
    if hasattr(b2, 'OclType50'):
        assert not _is_linked(b2, 'OclType50', a)
...
def test_assoc_type82_link_reassign_clear():
    ...
    if hasattr(b1, 'OclType83'):
        assert _is_linked(b1, 'OclType83', a)
    _safe_set(a, 'tupleTypeAttribute', b2)
    assert _is_linked(a, 'tupleTypeAttribute', b2)
    if hasattr(b1, 'OclType83'):
        assert not _is_linked(b1, 'OclType83', a)
    if hasattr(b2, 'OclType83'):
        assert _is_linked(b2, 'OclType83', a)
    _safe_set(a, 'tupleTypeAttribute', None)
    assert not _is_linked(a, 'tupleTypeAttribute', b2)
    if hasattr(b2, 'OclType83'):
        assert not _is_linked(b2, 'OclType83', a)


def test_assoc_variableDeclaration77_link_reassign_clear():
    a = OCL_OclType(name="sample_text")
    b1 = VariableDeclaration()
    b2 = VariableDeclaration()
    _safe_set(a, 'type78', b1)
    assert _is_linked(a, 'type78', b1)
    if hasattr(b1, 'VariableDeclaration79'):
        assert _is_linked(b1, 'VariableDeclaration79', a)
    _safe_set(a, 'type78', b2)
    assert _is_linked(a, 'type78', b2)
    if hasattr(b1, 'VariableDeclaration79'):
        assert not _is_linked(b1, 'VariableDeclaration79', a)
    if hasattr(b2, 'VariableDeclaration79'):
        assert _is_linked(b2, 'VariableDeclaration79', a)
    _safe_set(a, 'type78', None)
    assert not _is_linked(a, 'type78', b2)
    if hasattr(b2, 'VariableDeclaration79'):
        assert not _is_linked(b2, 'VariableDeclaration79', a)


def test_assoc_variableExp56_link_reassign_clear():
    a = OCL_VariableDeclaration(id="sample_text", varName="sample_text")
    b1 = VariableExp()
    b2 = VariableExp()
