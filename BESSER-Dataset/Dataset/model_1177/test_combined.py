...
def test_hyp_qualitymetamodel_listvalue_is_not_abstract():
    assert not inspect.isabstract(QualityMetamodel_ListValue)


def test_hyp_qualitymetamodel_listvalue_constructor_exists():
    assert callable(QualityMetamodel_ListValue.__init__)


def test_hyp_qualitymetamodel_listvalue_constructor_args():
...
def test_QualityMetamodel_ListValue_isa_ValueType():
    instance = QualityMetamodel_ListValue()
    assert isinstance(instance, ValueType)
...
def test_assoc_argument86_link_reassign_clear():
...
def test_assoc_arguments84_link_reassign_clear():
...
def test_assoc_attribute131_link_reassign_clear():
    a = QualityMetamodel_QMM_OCL_OclType(name="sample_text")
    b1 = Attribute()
    b2 = Attribute()
    _safe_set(a, 'type132', b1)
    assert _is_linked(a, 'type132', b1)
    if hasattr(b1, 'Attribute133'):
        assert _is_linked(b1, 'Attribute133', a)
    _safe_set(a, 'type132', b2)
    assert _is_linked(a, 'type132', b2)
    if hasattr(b1, 'Attribute133'):
        assert not _is_linked(b1, 'Attribute133', a)
    if hasattr(b2, 'Attribute133'):
        assert _is_linked(b2, 'Attribute133', a)
    _safe_set(a, 'type132', None)
    assert not _is_linked(a, 'type132', b2)
    if hasattr(b2, 'Attribute133'):
        assert not _is_linked(b2, 'Attribute133', a)


def test_assoc_baseExp117_link_reassign_clear():
...
def test_assoc_collectionTypes136_link_reassign_clear():
    a = QualityMetamodel_QMM_OCL_OclType(name="sample_text")
    b1 = CollectionType()
    b2 = CollectionType()
...
def test_assoc_context_167_link_reassign_clear():
    a = QualityMetamodel_QMM_OCL_OclFeatureDefinition(static="sample_text")
    b1 = OclContextDefinition()
    b2 = OclContextDefinition()
    _safe_set(a, 'definition168', b1)
    assert _is_linked(a, 'definition168', b1)
    if hasattr(b1, 'OclContextDefinition169'):
        assert _is_linked(b1, 'OclContextDefinition169', a)
    _safe_set(a, 'definition168', b2)
    assert _is_linked(a, 'definition168', b2)
    if hasattr(b1, 'OclContextDefinition169'):
        assert not _is_linked(b1, 'OclContextDefinition169', a)
    if hasattr(b2, 'OclContextDefinition169'):
        assert _is_linked(b2, 'OclContextDefinition169', a)
    _safe_set(a, 'definition168', None)
    assert not _is_linked(a, 'definition168', b2)
    if hasattr(b2, 'OclContextDefinition169'):
        assert not _is_linked(b2, 'OclContextDefinition169', a)


def test_assoc_definition174_link_reassign_clear():
    ...
    if hasattr(b1, 'OclFeatureDefinition175'):
        assert _is_linked(b1, 'OclFeatureDefinition175', a)
    _safe_set(a, 'feature', b2)
    assert _is_linked(a, 'feature', b2)
    if hasattr(b1, 'OclFeatureDefinition175'):
        assert not _is_linked(b1, 'OclFeatureDefinition175', a)
    if hasattr(b2, 'OclFeatureDefinition175'):
        assert _is_linked(b2, 'OclFeatureDefinition175', a)
    _safe_set(a, 'feature', None)
    assert not _is_linked(a, 'feature', b2)
    if hasattr(b2, 'OclFeatureDefinition175'):
        assert not _is_linked(b2, 'OclFeatureDefinition175', a)


def test_assoc_definitions124_link_reassign_clear():
    a = QualityMetamodel_QMM_OCL_OclType(name="sample_text")
    b1 = OclContextDefinition()
    b2 = OclContextDefinition()
...
def test_assoc_feature166_link_reassign_clear():
    a = QualityMetamodel_QMM_OCL_OclFeatureDefinition(static="sample_text")
    b1 = OclFeature()
    b2 = OclFeature()
...
def test_assoc_initExpression115_link_reassign_clear():
    a = QualityMetamodel_QMM_OCL_LocalVariable(eq="sample_text")
    b1 = OclExpression()
    b2 = OclExpression()
    _safe_set(a, 'initializedVariable', b1)
    assert _is_linked(a, 'initializedVariable', b1)
    if hasattr(b1, 'OclExpression116'):
        assert _is_linked(b1, 'OclExpression116', a)
    _safe_set(a, 'initializedVariable', b2)
    assert _is_linked(a, 'initializedVariable', b2)
    if hasattr(b1, 'OclExpression116'):
        assert not _is_linked(b1, 'OclExpression116', a)
    if hasattr(b2, 'OclExpression116'):
        assert _is_linked(b2, 'OclExpression116', a)
    _safe_set(a, 'initializedVariable', None)
    assert not _is_linked(a, 'initializedVariable', b2)
    if hasattr(b2, 'OclExpression116'):
        assert not _is_linked(b2, 'OclExpression116', a)


def test_assoc_lambdaArgType144_link_reassign_clear():
    a = QualityMetamodel_QMM_OCL_OclType(name="sample_text")
    b1 = LambdaType()
    b2 = LambdaType()
    _safe_set(a, 'argumentTypes', b1)
    assert _is_linked(a, 'argumentTypes', b1)
    if hasattr(b1, 'LambdaType145'):
        assert _is_linked(b1, 'LambdaType145', a)
    _safe_set(a, 'argumentTypes', b2)
    assert _is_linked(a, 'argumentTypes', b2)
    if hasattr(b1, 'LambdaType145'):
        assert not _is_linked(b1, 'LambdaType145', a)
    if hasattr(b2, 'LambdaType145'):
        assert _is_linked(b2, 'LambdaType145', a)
    _safe_set(a, 'argumentTypes', None)
    assert not _is_linked(a, 'argumentTypes', b2)
    if hasattr(b2, 'LambdaType145'):
        assert not _is_linked(b2, 'LambdaType145', a)


def test_assoc_lambdaReturnType142_link_reassign_clear():
    a = QualityMetamodel_QMM_OCL_OclType(name="sample_text")
    b1 = LambdaType()
    b2 = LambdaType()
    _safe_set(a, 'returnType143', b1)
    assert _is_linked(a, 'returnType143', b1)
    if hasattr(b1, 'LambdaType'):
        assert _is_linked(b1, 'LambdaType', a)
    _safe_set(a, 'returnType143', b2)
    assert _is_linked(a, 'returnType143', b2)
    if hasattr(b1, 'LambdaType'):
        assert not _is_linked(b1, 'LambdaType', a)
    if hasattr(b2, 'LambdaType'):
        assert _is_linked(b2, 'LambdaType', a)
    _safe_set(a, 'returnType143', None)
    assert not _is_linked(a, 'returnType143', b2)
    if hasattr(b2, 'LambdaType'):
        assert not _is_linked(b2, 'LambdaType', a)


def test_assoc_letExp113_link_reassign_clear():
    a = QualityMetamodel_QMM_OCL_LocalVariable(eq="sample_text")
    b1 = LetExp()
    b2 = LetExp()
    _safe_set(a, 'variable', b1)
    assert _is_linked(a, 'variable', b1)
    if hasattr(b1, 'LetExp114'):
        assert _is_linked(b1, 'LetExp114', a)
    _safe_set(a, 'variable', b2)
    assert _is_linked(a, 'variable', b2)
    if hasattr(b1, 'LetExp114'):
        assert not _is_linked(b1, 'LetExp114', a)
    if hasattr(b2, 'LetExp114'):
        assert _is_linked(b2, 'LetExp114', a)
    _safe_set(a, 'variable', None)
    assert not _is_linked(a, 'variable', b2)
    if hasattr(b2, 'LetExp114'):
        assert not _is_linked(b2, 'LetExp114', a)


def test_assoc_mapType134_link_reassign_clear():
    a = QualityMetamodel_QMM_OCL_OclType(name="sample_text")
    b1 = MapType()
    b2 = MapType()
    _safe_set(a, 'keyType', b1)
    assert _is_linked(a, 'keyType', b1)
    if hasattr(b1, 'MapType135'):
        assert _is_linked(b1, 'MapType135', a)
    _safe_set(a, 'keyType', b2)
    assert _is_linked(a, 'keyType', b2)
    if hasattr(b1, 'MapType135'):
        assert not _is_linked(b1, 'MapType135', a)
    if hasattr(b2, 'MapType135'):
        assert _is_linked(b2, 'MapType135', a)
    _safe_set(a, 'keyType', None)
    assert not _is_linked(a, 'keyType', b2)
    if hasattr(b2, 'MapType135'):
        assert not _is_linked(b2, 'MapType135', a)


def test_assoc_mapType2129_link_reassign_clear():
    a = QualityMetamodel_QMM_OCL_OclType(name="sample_text")
    b1 = MapType()
    b2 = MapType()
    _safe_set(a, 'valueType130', b1)
    assert _is_linked(a, 'valueType130', b1)
    if hasattr(b1, 'MapType'):
        assert _is_linked(b1, 'MapType', a)
    _safe_set(a, 'valueType130', b2)
    assert _is_linked(a, 'valueType130', b2)
    if hasattr(b1, 'MapType'):
        assert not _is_linked(b1, 'MapType', a)
    if hasattr(b2, 'MapType'):
        assert _is_linked(b2, 'MapType', a)
    _safe_set(a, 'valueType130', None)
    assert not _is_linked(a, 'valueType130', b2)
    if hasattr(b2, 'MapType'):
...
def test_assoc_model187_link_reassign_clear():
...
def test_assoc_oclExpression125_link_reassign_clear():
    a = QualityMetamodel_QMM_OCL_OclType(name="sample_text")
    b1 = OclExpression()
    b2 = OclExpression()
    _safe_set(a, 'type', b1)
...
def test_assoc_operation127_link_reassign_clear():
    a = QualityMetamodel_QMM_OCL_OclType(name="sample_text")
    b1 = Operation()
    b2 = Operation()
...
def test_assoc_source88_link_reassign_clear():
    a = QualityMetamodel_QMM_OCL_OperatorCallExp(operationName="sample_text")
    b1 = OclExpression()
    b2 = OclExpression()
    _safe_set(a, 'appliedOperator', b1)
    assert _is_linked(a, 'appliedOperator', b1)
    if hasattr(b1, 'OclExpression89'):
        assert _is_linked(b1, 'OclExpression89', a)
    _safe_set(a, 'appliedOperator', b2)
    assert _is_linked(a, 'appliedOperator', b2)
    if hasattr(b1, 'OclExpression89'):
        assert not _is_linked(b1, 'OclExpression89', a)
    if hasattr(b2, 'OclExpression89'):
        assert _is_linked(b2, 'OclExpression89', a)
    _safe_set(a, 'appliedOperator', None)
    assert not _is_linked(a, 'appliedOperator', b2)
    if hasattr(b2, 'OclExpression89'):
        assert not _is_linked(b2, 'OclExpression89', a)


def test_assoc_staticPropertyCall146_link_reassign_clear():
    a = QualityMetamodel_QMM_OCL_OclType(name="sample_text")
    b1 = StaticPropertyCallExp()
    b2 = StaticPropertyCallExp()
    _safe_set(a, 'source147', b1)
    assert _is_linked(a, 'source147', b1)
    if hasattr(b1, 'StaticPropertyCallExp148'):
        assert _is_linked(b1, 'StaticPropertyCallExp148', a)
    _safe_set(a, 'source147', b2)
    assert _is_linked(a, 'source147', b2)
    if hasattr(b1, 'StaticPropertyCallExp148'):
        assert not _is_linked(b1, 'StaticPropertyCallExp148', a)
    if hasattr(b2, 'StaticPropertyCallExp148'):
        assert _is_linked(b2, 'StaticPropertyCallExp148', a)
    _safe_set(a, 'source147', None)
    assert not _is_linked(a, 'source147', b2)
    if hasattr(b2, 'StaticPropertyCallExp148'):
        assert not _is_linked(b2, 'StaticPropertyCallExp148', a)


def test_assoc_tupleType154_link_reassign_clear():
...
def test_assoc_tupleTypeAttribute137_link_reassign_clear():
    a = QualityMetamodel_QMM_OCL_OclType(name="sample_text")
...
def test_assoc_type110_link_reassign_clear():
    ...
    if hasattr(b1, 'OclType111'):
        assert _is_linked(b1, 'OclType111', a)
    _safe_set(a, 'variableDeclaration', b2)
    assert _is_linked(a, 'variableDeclaration', b2)
    if hasattr(b1, 'OclType111'):
        assert not _is_linked(b1, 'OclType111', a)
    if hasattr(b2, 'OclType111'):
        assert _is_linked(b2, 'OclType111', a)
    _safe_set(a, 'variableDeclaration', None)
    assert not _is_linked(a, 'variableDeclaration', b2)
    if hasattr(b2, 'OclType111'):
        assert not _is_linked(b2, 'OclType111', a)
...
def test_assoc_type152_link_reassign_clear():
    ...
    if hasattr(b1, 'OclType153'):
        assert _is_linked(b1, 'OclType153', a)
    _safe_set(a, 'tupleTypeAttribute', b2)
    assert _is_linked(a, 'tupleTypeAttribute', b2)
    if hasattr(b1, 'OclType153'):
        assert not _is_linked(b1, 'OclType153', a)
    if hasattr(b2, 'OclType153'):
        assert _is_linked(b2, 'OclType153', a)
    _safe_set(a, 'tupleTypeAttribute', None)
    assert not _is_linked(a, 'tupleTypeAttribute', b2)
    if hasattr(b2, 'OclType153'):
        assert not _is_linked(b2, 'OclType153', a)
...
def test_assoc_variableDeclaration139_link_reassign_clear():
    a = QualityMetamodel_QMM_OCL_OclType(name="sample_text")
    b1 = VariableDeclaration()
    b2 = VariableDeclaration()
    _safe_set(a, 'type140', b1)
    assert _is_linked(a, 'type140', b1)
    if hasattr(b1, 'VariableDeclaration141'):
        assert _is_linked(b1, 'VariableDeclaration141', a)
    _safe_set(a, 'type140', b2)
    assert _is_linked(a, 'type140', b2)
    if hasattr(b1, 'VariableDeclaration141'):
        assert not _is_linked(b1, 'VariableDeclaration141', a)
    if hasattr(b2, 'VariableDeclaration141'):
        assert _is_linked(b2, 'VariableDeclaration141', a)
    _safe_set(a, 'type140', None)
    assert not _is_linked(a, 'type140', b2)
    if hasattr(b2, 'VariableDeclaration141'):
        assert not _is_linked(b2, 'VariableDeclaration141', a)


def test_assoc_variableExp112_link_reassign_clear():
    a = QualityMetamodel_QMM_OCL_VariableDeclaration(varName="sample_text")
    b1 = VariableExp()
    b2 = VariableExp()
...
QualityMetamodel_ListValue_strategy = st.builds(QualityMetamodel_ListValue)
