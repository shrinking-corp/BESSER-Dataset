...
class fsm_State(NamedElement):
    ...
    def __init__(self, initialTime: int, finalTime: int, State: "fsm_StateMachine" = None, source: set["fsm_Transition"] = None, target: set["fsm_Transition"] = None, states: "fsm_StateMachine" = None, fsm_State: "fsm_CompositeState" = None, fsm_State20: "fsm_Region" = None, State10: "fsm_Transition" = None, State12: "fsm_Transition" = None):
    ...
    def fsm_State20(self, value):
        ...
        self.__fsm_State20 = value
        
        # Remove self from old opposite end
        if old_value is not None:
            if hasattr(old_value, "fsm_Region19"):
                opp_val = getattr(old_value, "fsm_Region19", None)
                if isinstance(opp_val, set):
                    opp_val.discard(self)
                
        # Add self to new opposite end
        if value is not None:
            if hasattr(value, "fsm_Region19"):
                opp_val = getattr(value, "fsm_Region19", None)
                if opp_val is None:
                    setattr(value, "fsm_Region19", set([self]))
                elif isinstance(opp_val, set):
                    opp_val.add(self)
...
class fsm_Transition(NamedElement):
    ...
    def __init__(self, initialTime: int, finalTime: int, time: int, Transition: "fsm_StateMachine" = None, Transition4: "fsm_State" = None, Transition6: "fsm_State" = None, incoming: "fsm_State" = None, outgoing: "fsm_State" = None, fsm_Transition: "fsm_Trigger" = None, transitions: "fsm_StateMachine" = None):
        self.initialTime = initialTime
        self.finalTime = finalTime
        self.time = time
    ...
    def transitions(self, value):
        ...
        # Remove self from old opposite end
        if old_value is not None:
            if hasattr(old_value, "StateMachine15"):
                opp_val = getattr(old_value, "StateMachine15", None)
                if opp_val == self:
                    setattr(old_value, "StateMachine15", None)
                
        # Add self to new opposite end
        if value is not None:
            if hasattr(value, "StateMachine15"):
                opp_val = getattr(value, "StateMachine15", None)
                setattr(value, "StateMachine15", self)
