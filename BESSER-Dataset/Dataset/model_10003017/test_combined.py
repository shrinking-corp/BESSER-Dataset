...
def test_hyp_ram1_is_not_abstract():
    assert not inspect.isabstract(RAM1)


def test_hyp_ram1_constructor_exists():
    assert callable(RAM1.__init__)


def test_hyp_ram1_constructor_args():
...
def test_hyp_cpu1_is_not_abstract():
    assert not inspect.isabstract(CPU1)


def test_hyp_cpu1_constructor_exists():
    assert callable(CPU1.__init__)


def test_hyp_cpu1_constructor_args():
...
def test_hyp_ram1_mb_setter(instance):
...
def test_hyp_cpu1_mhz_setter(instance):
...
def test_hyp_hardware_manufacturer_setter(instance):
...
@given(instance=Hardware_strategy)
def test_hyp_hardware_serialNo_setter(instance):
...
def test_CPU1_mhz_value_roundtrip():
    instance = CPU1(mhz=3.14)
    assert instance.mhz == 3.14
    instance.mhz = 9.99
    assert instance.mhz == 9.99
...
def test_Hardware_manufacturer_value_roundtrip():
    instance = Hardware(manufacturer="sample_text", serialNo="sample_text")
    assert instance.manufacturer == "sample_text"
    instance.manufacturer = "sample_text_2"
    assert instance.manufacturer == "sample_text_2"


def test_Hardware_serialNo_value_roundtrip():
    instance = Hardware(manufacturer="sample_text", serialNo="sample_text")
    assert instance.serialNo == "sample_text"
    instance.serialNo = "sample_text_2"
    assert instance.serialNo == "sample_text_2"
...
def test_RAM1_mb_value_roundtrip():
    instance = RAM1(mb=3.14)
    assert instance.mb == 3.14
    instance.mb = 9.99
    assert instance.mb == 9.99
...
CPU1_strategy = st.builds(CPU1, mhz=st.floats(allow_nan=False, allow_infinity=False))
@given(instance=CPU1_strategy)
@settings(max_examples=25)
...
Hardware_strategy = st.builds(Hardware, manufacturer=safe_text, serialNo=safe_text)
@given(instance=Hardware_strategy)
@settings(max_examples=25)
...
RAM1_strategy = st.builds(RAM1, mb=st.floats(allow_nan=False, allow_infinity=False))
@given(instance=RAM1_strategy)
@settings(max_examples=25)
