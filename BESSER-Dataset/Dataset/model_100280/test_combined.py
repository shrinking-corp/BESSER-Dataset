...
def test_hyp_model_hlarctype_1_is_not_abstract():
    assert not inspect.isabstract(model_HLArcType_1)


def test_hyp_model_hlarctype_1_constructor_exists():
    assert callable(model_HLArcType_1.__init__)


def test_hyp_model_hlarctype_1_constructor_args():
    sig = inspect.signature(model_HLArcType_1.__init__)
...
model_HLArcType_1_strategy = st.builds(
    model_HLArcType_1,
)
...
def test_hyp_model_hlarcaddin_type_setter(instance):
...
def test_hyp_model_petrinet_type_setter(instance):
    original = instance.type
    instance.type = original
    assert instance.type == original
...
def test_model_HLArcAddin_type_value_roundtrip():
    instance = model_HLArcAddin(type="sample_text")
...
def test_model_PetriNet_type_value_roundtrip():
    instance = model_PetriNet(type="sample_text")
    assert instance.type == "sample_text"
    instance.type = "sample_text_2"
    assert instance.type == "sample_text_2"
...
def test_model_Type_isa_Annotation():
    instance = model_Type()
    assert isinstance(instance, Annotation)
...
def test_assoc_fusionGroups22_link_reassign_clear():
    a = model_PetriNet(type="sample_text")
...
def test_assoc_hlinscription31_link_reassign_clear():
    a = model_HLArcAddin(type="sample_text")
...
def test_assoc_instance38_link_reassign_clear():
...
def test_assoc_label4_link_reassign_clear():
    a = model_Label()
    b1 = model_HasLabel()
    b2 = model_HasLabel()
...
def test_assoc_parameterAssignment37_link_reassign_clear():
    a = model_ParameterAssignment(parameter="sample_text", value="sample_text")
    b1 = model_Instance(subPageID="sample_text")
    b2 = model_Instance(subPageID="sample_text_2")
    _safe_set(a, 'ParameterAssignment', b1)
...
def test_assoc_petriNet39_link_reassign_clear():
    a = model_PetriNet(type="sample_text")
    b1 = model_FusionGroup()
    b2 = model_FusionGroup()
    ...
    _safe_set(a, 'PetriNet40', None)
    assert not _is_linked(a, 'PetriNet40', b2)
    if hasattr(b2, 'fusionGroups'):
        assert not _is_linked(b2, 'fusionGroups', a)


def test_assoc_toolinfo6_link_reassign_clear():
    a = model_ToolInfo(tool="sample_text", version="sample_text")
    b1 = model_HasToolInfo()
    b2 = model_HasToolInfo()
    _safe_set(a, 'ToolInfo', b1)
    assert _is_linked(a, 'ToolInfo', b1)
    if hasattr(b1, 'parent7'):
        assert _is_linked(b1, 'parent7', a)
    _safe_set(a, 'ToolInfo', b2)
    assert _is_linked(a, 'ToolInfo', b2)
    if hasattr(b1, 'parent7'):
        assert not _is_linked(b1, 'parent7', a)
    if hasattr(b2, 'parent7'):
        assert _is_linked(b2, 'parent7', a)
    _safe_set(a, 'ToolInfo', None)
    assert not _is_linked(a, 'ToolInfo', b2)
    if hasattr(b2, 'parent7'):
        assert not _is_linked(b2, 'parent7', a)
...
model_HLArcAddin_strategy = st.builds(model_HLArcAddin, type=safe_text)
@given(instance=model_HLArcAddin_strategy)
@settings(max_examples=25)
...
model_HLArcType_1_strategy = st.builds(model_HLArcType_1)
@given(instance=model_HLArcType_1_strategy)
@settings(max_examples=25)
def test_model_HLArcType_1_instantiation(instance):
    assert isinstance(instance, model_HLArcType_1)
...
model_PetriNet_strategy = st.builds(model_PetriNet, type=safe_text)
@given(instance=model_PetriNet_strategy)
@settings(max_examples=25)
