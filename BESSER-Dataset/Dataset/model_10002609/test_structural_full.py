...
def test_assoc_Machine_Memory_link_reassign_clear():
    ...
    b1 = Machine()
    b2 = Machine()
    _safe_set(a, 'machine0', {b1})
    assert _is_linked(a, 'machine0', b1)
    if hasattr(b1, 'memory1'):
        assert _is_linked(b1, 'memory1', a)
    _safe_set(a, 'machine0', {b2})
    assert _is_linked(a, 'machine0', b2)
    if hasattr(b1, 'memory1'):
        assert not _is_linked(b1, 'memory1', a)
    if hasattr(b2, 'memory1'):
        assert _is_linked(b2, 'memory1', a)
    _safe_set(a, 'machine0', set())
    assert not _is_linked(a, 'machine0', b2)
    if hasattr(b2, 'memory1'):
        assert not _is_linked(b2, 'memory1', a)


def test_assoc_Processor_Memory_link_reassign_clear():
    a = Cache(chunck="sample_text")
    b1 = Processor()
    b2 = Processor()
    _safe_set(a, 'processor2', {b1})
    assert _is_linked(a, 'processor2', b1)
    if hasattr(b1, 'memory3'):
        assert _is_linked(b1, 'memory3', a)
    _safe_set(a, 'processor2', {b2})
    assert _is_linked(a, 'processor2', b2)
    if hasattr(b1, 'memory3'):
        assert not _is_linked(b1, 'memory3', a)
    if hasattr(b2, 'memory3'):
        assert _is_linked(b2, 'memory3', a)
    _safe_set(a, 'processor2', set())
    assert not _is_linked(a, 'processor2', b2)
    if hasattr(b2, 'memory3'):
        assert not _is_linked(b2, 'memory3', a)
