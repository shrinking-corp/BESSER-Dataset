...
def test_Account_billingAddress_value_roundtrip():
    instance = Account(billingAddress="sample_text", closed=date(2024, 1, 1), open=date(2024, 1, 1))
...
def test_assoc_Account_Payment_link_reassign_clear():
    ...
    b1 = Account(billingAddress="sample_text", closed=date(2024, 1, 1), open=date(2024, 1, 1))
    b2 = Account(billingAddress="sample_text_2", closed=date(2025, 6, 15), open=date(2025, 6, 15))
    _safe_set(a, 'acc1', b1)
...
Account_strategy = st.builds(Account, billingAddress=safe_text, closed=st.dates(), open=st.dates())
