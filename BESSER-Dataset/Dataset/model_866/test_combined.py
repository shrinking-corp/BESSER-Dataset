...
def test_hyp_fsm_initialstate_name_setter(instance):
    original = instance.name
    instance.name = original
    assert instance.name == original
...
def test_fsm_InitialState_name_value_roundtrip():
    instance = fsm_InitialState(name="sample_text")
...
def test_assoc_intialState1_link_reassign_clear():
    a = fsm_InitialState(name="sample_text")
    b1 = fsm_FSM()
    b2 = fsm_FSM()
    _safe_set(a, 'fsm_InitialState', b1)
    assert _is_linked(a, 'fsm_InitialState', b1)
    if hasattr(b1, 'fsm_FSM2'):
        assert _is_linked(b1, 'fsm_FSM2', a)
    _safe_set(a, 'fsm_InitialState', b2)
    assert _is_linked(a, 'fsm_InitialState', b2)
    if hasattr(b1, 'fsm_FSM2'):
        assert not _is_linked(b1, 'fsm_FSM2', a)
    if hasattr(b2, 'fsm_FSM2'):
        assert _is_linked(b2, 'fsm_FSM2', a)
    _safe_set(a, 'fsm_InitialState', None)
    assert not _is_linked(a, 'fsm_InitialState', b2)
    if hasattr(b2, 'fsm_FSM2'):
...
fsm_InitialState_strategy = st.builds(fsm_InitialState, name=safe_text)
@given(instance=fsm_InitialState_strategy)
@settings(max_examples=25)
