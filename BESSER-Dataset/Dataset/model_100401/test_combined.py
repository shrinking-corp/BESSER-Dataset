...
def test_hyp_sgraph_property_is_not_abstract():
    assert not inspect.isabstract(sgraph_Property)


def test_hyp_sgraph_property_constructor_exists():
    assert callable(sgraph_Property.__init__)


def test_hyp_sgraph_property_constructor_args():
...
def test_sgraph_Statechart_isa_DomainElement():
    instance = sgraph_Statechart()
...
def test_assoc_imports10_link_reassign_clear():
    a = sgraph_Import(importedNamespace="sample_text")
    b1 = sgraph_Statechart()
    b2 = sgraph_Statechart()
    _safe_set(a, 'sgraph_Import', b1)
    assert _is_linked(a, 'sgraph_Import', b1)
    if hasattr(b1, 'sgraph_Statechart'):
        assert _is_linked(b1, 'sgraph_Statechart', a)
    _safe_set(a, 'sgraph_Import', b2)
    assert _is_linked(a, 'sgraph_Import', b2)
    if hasattr(b1, 'sgraph_Statechart'):
        assert not _is_linked(b1, 'sgraph_Statechart', a)
    if hasattr(b2, 'sgraph_Statechart'):
        assert _is_linked(b2, 'sgraph_Statechart', a)
    _safe_set(a, 'sgraph_Import', None)
    assert not _is_linked(a, 'sgraph_Import', b2)
    if hasattr(b2, 'sgraph_Statechart'):
...
sgraph_Property_strategy = st.builds(sgraph_Property)
