...
def test_university_Student_MNR_value_roundtrip():
    instance = university_Student(MNR="sample_text")
...
def test_assoc_students5_link_reassign_clear():
    a = university_University(name="sample_text")
    b1 = university_Student(MNR="sample_text")
    b2 = university_Student(MNR="sample_text_2")
    _safe_set(a, 'university_University6', {b1})
    assert _is_linked(a, 'university_University6', b1)
    if hasattr(b1, 'university_Student'):
        assert _is_linked(b1, 'university_Student', a)
    _safe_set(a, 'university_University6', {b2})
    assert _is_linked(a, 'university_University6', b2)
    if hasattr(b1, 'university_Student'):
        assert not _is_linked(b1, 'university_Student', a)
    if hasattr(b2, 'university_Student'):
        assert _is_linked(b2, 'university_Student', a)
    _safe_set(a, 'university_University6', set())
    assert not _is_linked(a, 'university_University6', b2)
    if hasattr(b2, 'university_Student'):
        assert not _is_linked(b2, 'university_Student', a)


def test_assoc_uni7_link_reassign_clear():
    a = university_University(name="sample_text")
    b1 = university_Student(MNR="sample_text")
    b2 = university_Student(MNR="sample_text_2")
    _safe_set(a, 'university_University9', b1)
    assert _is_linked(a, 'university_University9', b1)
    if hasattr(b1, 'university_Student8'):
        assert _is_linked(b1, 'university_Student8', a)
    _safe_set(a, 'university_University9', b2)
    assert _is_linked(a, 'university_University9', b2)
    if hasattr(b1, 'university_Student8'):
        assert not _is_linked(b1, 'university_Student8', a)
    if hasattr(b2, 'university_Student8'):
        assert _is_linked(b2, 'university_Student8', a)
    _safe_set(a, 'university_University9', None)
    assert not _is_linked(a, 'university_University9', b2)
    if hasattr(b2, 'university_Student8'):
        assert not _is_linked(b2, 'university_Student8', a)
...
university_Student_strategy = st.builds(university_Student, MNR=safe_text)
