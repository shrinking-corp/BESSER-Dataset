...
def test_employee_has_yearsHere():
    assert hasattr(Employee, "yearsHere")
    descriptor = None
    for klass in Employee.__mro__:
        if "yearsHere" in klass.__dict__:
            descriptor = klass.__dict__["yearsHere"]
            break
    assert isinstance(descriptor, property)
...
@given(instance=Employee_strategy)
def test_employee_yearsHere_setter(instance):
