...
def test_assoc_additionalCondition370_link_reassign_clear():
    ...
    if hasattr(b1, 'OclExpression371'):
        assert _is_linked(b1, 'OclExpression371', a)
    _safe_set(a, 'QVTOperational_ModelType', {b2})
    assert _is_linked(a, 'QVTOperational_ModelType', b2)
    if hasattr(b1, 'OclExpression371'):
        assert not _is_linked(b1, 'OclExpression371', a)
    if hasattr(b2, 'OclExpression371'):
        assert _is_linked(b2, 'OclExpression371', a)
    _safe_set(a, 'QVTOperational_ModelType', set())
    assert not _is_linked(a, 'QVTOperational_ModelType', b2)
    if hasattr(b2, 'OclExpression371'):
        assert not _is_linked(b2, 'OclExpression371', a)


def test_assoc_annotatedElement4_link_reassign_clear():
...
def test_assoc_assertion241_link_reassign_clear():
    ...
    if hasattr(b1, 'OclExpression242'):
        assert _is_linked(b1, 'OclExpression242', a)
    _safe_set(a, 'ImperativeOCL_AssertExp', b2)
    assert _is_linked(a, 'ImperativeOCL_AssertExp', b2)
    if hasattr(b1, 'OclExpression242'):
        assert not _is_linked(b1, 'OclExpression242', a)
    if hasattr(b2, 'OclExpression242'):
        assert _is_linked(b2, 'OclExpression242', a)
    _safe_set(a, 'ImperativeOCL_AssertExp', None)
    assert not _is_linked(a, 'ImperativeOCL_AssertExp', b2)
    if hasattr(b2, 'OclExpression242'):
        assert not _is_linked(b2, 'OclExpression242', a)
...
def test_assoc_binding388_link_reassign_clear():
    ...
    if hasattr(b1, 'ModelType389'):
        assert _is_linked(b1, 'ModelType389', a)
    _safe_set(a, 'QVTOperational_ModuleImport', {b2})
    assert _is_linked(a, 'QVTOperational_ModuleImport', b2)
    if hasattr(b1, 'ModelType389'):
        assert not _is_linked(b1, 'ModelType389', a)
    if hasattr(b2, 'ModelType389'):
        assert _is_linked(b2, 'ModelType389', a)
    _safe_set(a, 'QVTOperational_ModuleImport', set())
    assert not _is_linked(a, 'QVTOperational_ModuleImport', b2)
    if hasattr(b2, 'ModelType389'):
        assert not _is_linked(b2, 'ModelType389', a)


def test_assoc_body337_link_reassign_clear():
    a = QVTOperational_ImperativeOperation(isBlackbox="sample_text")
    b1 = OperationBody()
    b2 = OperationBody()
    _safe_set(a, 'operation338', b1)
    assert _is_linked(a, 'operation338', b1)
    if hasattr(b1, 'OperationBody'):
        assert _is_linked(b1, 'OperationBody', a)
    _safe_set(a, 'operation338', b2)
    assert _is_linked(a, 'operation338', b2)
    if hasattr(b1, 'OperationBody'):
        assert not _is_linked(b1, 'OperationBody', a)
    if hasattr(b2, 'OperationBody'):
        assert _is_linked(b2, 'OperationBody', a)
    _safe_set(a, 'operation338', None)
    assert not _is_linked(a, 'operation338', b2)
    if hasattr(b2, 'OperationBody'):
        assert not _is_linked(b2, 'OperationBody', a)


def test_assoc_bottomPattern135_link_reassign_clear():
    a = QVTCore_Assignment(isDefault="sample_text")
    b1 = BottomPattern()
    b2 = BottomPattern()
    _safe_set(a, 'assignment', b1)
    assert _is_linked(a, 'assignment', b1)
    if hasattr(b1, 'BottomPattern136'):
        assert _is_linked(b1, 'BottomPattern136', a)
    _safe_set(a, 'assignment', b2)
    assert _is_linked(a, 'assignment', b2)
    if hasattr(b1, 'BottomPattern136'):
        assert not _is_linked(b1, 'BottomPattern136', a)
    if hasattr(b2, 'BottomPattern136'):
        assert _is_linked(b2, 'BottomPattern136', a)
    _safe_set(a, 'assignment', None)
    assert not _is_linked(a, 'assignment', b2)
    if hasattr(b2, 'BottomPattern136'):
        assert not _is_linked(b2, 'BottomPattern136', a)


def test_assoc_bottomPattern148_link_reassign_clear():
    ...
    if hasattr(b1, 'BottomPattern149'):
        assert _is_linked(b1, 'BottomPattern149', a)
    _safe_set(a, 'enforcementOperation', b2)
    assert _is_linked(a, 'enforcementOperation', b2)
    if hasattr(b1, 'BottomPattern149'):
        assert not _is_linked(b1, 'BottomPattern149', a)
    if hasattr(b2, 'BottomPattern149'):
        assert _is_linked(b2, 'BottomPattern149', a)
    _safe_set(a, 'enforcementOperation', None)
    assert not _is_linked(a, 'enforcementOperation', b2)
    if hasattr(b2, 'BottomPattern149'):
        assert not _is_linked(b2, 'BottomPattern149', a)


def test_assoc_class_21_link_reassign_clear():
    ...
    if hasattr(b1, 'Class22'):
        assert _is_linked(b1, 'Class22', a)
    _safe_set(a, 'ownedAttribute', b2)
    assert _is_linked(a, 'ownedAttribute', b2)
    if hasattr(b1, 'Class22'):
        assert not _is_linked(b1, 'Class22', a)
    if hasattr(b2, 'Class22'):
        assert _is_linked(b2, 'Class22', a)
    _safe_set(a, 'ownedAttribute', None)
    assert not _is_linked(a, 'ownedAttribute', b2)
    if hasattr(b2, 'Class22'):
        assert not _is_linked(b2, 'Class22', a)


def test_assoc_condition419_link_reassign_clear():
    ...
    if hasattr(b1, 'OclExpression420'):
        assert _is_linked(b1, 'OclExpression420', a)
    _safe_set(a, 'QVTOperational_ResolveExp', b2)
    assert _is_linked(a, 'QVTOperational_ResolveExp', b2)
    if hasattr(b1, 'OclExpression420'):
        assert not _is_linked(b1, 'OclExpression420', a)
    if hasattr(b2, 'OclExpression420'):
        assert _is_linked(b2, 'OclExpression420', a)
    _safe_set(a, 'QVTOperational_ResolveExp', None)
    assert not _is_linked(a, 'QVTOperational_ResolveExp', b2)
    if hasattr(b2, 'OclExpression420'):
        assert not _is_linked(b2, 'OclExpression420', a)


def test_assoc_configProperty375_link_reassign_clear():
    ...
    if hasattr(b1, 'Property376'):
        assert _is_linked(b1, 'Property376', a)
    _safe_set(a, 'QVTOperational_Module', {b2})
    assert _is_linked(a, 'QVTOperational_Module', b2)
    if hasattr(b1, 'Property376'):
        assert not _is_linked(b1, 'Property376', a)
    if hasattr(b2, 'Property376'):
        assert _is_linked(b2, 'Property376', a)
    _safe_set(a, 'QVTOperational_Module', set())
    assert not _is_linked(a, 'QVTOperational_Module', b2)
    if hasattr(b2, 'Property376'):
        assert not _is_linked(b2, 'Property376', a)


def test_assoc_context339_link_reassign_clear():
    a = QVTOperational_ImperativeOperation(isBlackbox="sample_text")
    b1 = VarParameter()
    b2 = VarParameter()
...
def test_assoc_ctxOwner426_link_reassign_clear():
    a = QVTOperational_VarParameter(kind="sample_text")
    b1 = ImperativeOperation()
    b2 = ImperativeOperation()
    _safe_set(a, 'context427', b1)
    assert _is_linked(a, 'context427', b1)
    if hasattr(b1, 'ImperativeOperation428'):
        assert _is_linked(b1, 'ImperativeOperation428', a)
    _safe_set(a, 'context427', b2)
    assert _is_linked(a, 'context427', b2)
    if hasattr(b1, 'ImperativeOperation428'):
        assert not _is_linked(b1, 'ImperativeOperation428', a)
    if hasattr(b2, 'ImperativeOperation428'):
        assert _is_linked(b2, 'ImperativeOperation428', a)
    _safe_set(a, 'context427', None)
    assert not _is_linked(a, 'context427', b2)
    if hasattr(b2, 'ImperativeOperation428'):
        assert not _is_linked(b2, 'ImperativeOperation428', a)


def test_assoc_defaultValue245_link_reassign_clear():
    ...
    if hasattr(b1, 'OclExpression246'):
        assert _is_linked(b1, 'OclExpression246', a)
    _safe_set(a, 'ImperativeOCL_AssignExp', b2)
    assert _is_linked(a, 'ImperativeOCL_AssignExp', b2)
    if hasattr(b1, 'OclExpression246'):
        assert not _is_linked(b1, 'OclExpression246', a)
    if hasattr(b2, 'OclExpression246'):
        assert _is_linked(b2, 'OclExpression246', a)
    _safe_set(a, 'ImperativeOCL_AssignExp', None)
    assert not _is_linked(a, 'ImperativeOCL_AssignExp', b2)
    if hasattr(b2, 'OclExpression246'):
        assert not _is_linked(b2, 'OclExpression246', a)


def test_assoc_element25_link_reassign_clear():
...
def test_assoc_entry377_link_reassign_clear():
    a = QVTOperational_Module(isBlackbox="sample_text")
    b1 = EntryOperation()
    b2 = EntryOperation()
    _safe_set(a, 'QVTOperational_Module378', b1)
    assert _is_linked(a, 'QVTOperational_Module378', b1)
    if hasattr(b1, 'EntryOperation'):
        assert _is_linked(b1, 'EntryOperation', a)
    _safe_set(a, 'QVTOperational_Module378', b2)
    assert _is_linked(a, 'QVTOperational_Module378', b2)
    if hasattr(b1, 'EntryOperation'):
        assert not _is_linked(b1, 'EntryOperation', a)
    if hasattr(b2, 'EntryOperation'):
        assert _is_linked(b2, 'EntryOperation', a)
    _safe_set(a, 'QVTOperational_Module378', None)
    assert not _is_linked(a, 'QVTOperational_Module378', b2)
    if hasattr(b2, 'EntryOperation'):
        assert not _is_linked(b2, 'EntryOperation', a)


def test_assoc_importedModule390_link_reassign_clear():
    a = QVTOperational_ModuleImport(kind="sample_text")
    b1 = Module()
    b2 = Module()
    _safe_set(a, 'QVTOperational_ModuleImport391', b1)
    assert _is_linked(a, 'QVTOperational_ModuleImport391', b1)
    if hasattr(b1, 'Module'):
        assert _is_linked(b1, 'Module', a)
    _safe_set(a, 'QVTOperational_ModuleImport391', b2)
    assert _is_linked(a, 'QVTOperational_ModuleImport391', b2)
    if hasattr(b1, 'Module'):
        assert not _is_linked(b1, 'Module', a)
    if hasattr(b2, 'Module'):
        assert _is_linked(b2, 'Module', a)
    _safe_set(a, 'QVTOperational_ModuleImport391', None)
    assert not _is_linked(a, 'QVTOperational_ModuleImport391', b2)
    if hasattr(b2, 'Module'):
        assert not _is_linked(b2, 'Module', a)


def test_assoc_left247_link_reassign_clear():
    a = ImperativeOCL_AssignExp(isReset="sample_text")
    b1 = OclExpression()
    b2 = OclExpression()
    _safe_set(a, 'ImperativeOCL_AssignExp248', b1)
    assert _is_linked(a, 'ImperativeOCL_AssignExp248', b1)
    if hasattr(b1, 'OclExpression249'):
        assert _is_linked(b1, 'OclExpression249', a)
    _safe_set(a, 'ImperativeOCL_AssignExp248', b2)
    assert _is_linked(a, 'ImperativeOCL_AssignExp248', b2)
    if hasattr(b1, 'OclExpression249'):
        assert not _is_linked(b1, 'OclExpression249', a)
    if hasattr(b2, 'OclExpression249'):
        assert _is_linked(b2, 'OclExpression249', a)
    _safe_set(a, 'ImperativeOCL_AssignExp248', None)
    assert not _is_linked(a, 'ImperativeOCL_AssignExp248', b2)
    if hasattr(b2, 'OclExpression249'):
        assert not _is_linked(b2, 'OclExpression249', a)


def test_assoc_log243_link_reassign_clear():
    a = ImperativeOCL_AssertExp(severity="sample_text")
    b1 = LogExp()
    b2 = LogExp()
    _safe_set(a, 'ImperativeOCL_AssertExp244', b1)
    assert _is_linked(a, 'ImperativeOCL_AssertExp244', b1)
    if hasattr(b1, 'LogExp'):
        assert _is_linked(b1, 'LogExp', a)
    _safe_set(a, 'ImperativeOCL_AssertExp244', b2)
    assert _is_linked(a, 'ImperativeOCL_AssertExp244', b2)
    if hasattr(b1, 'LogExp'):
        assert not _is_linked(b1, 'LogExp', a)
    if hasattr(b2, 'LogExp'):
        assert _is_linked(b2, 'LogExp', a)
    _safe_set(a, 'ImperativeOCL_AssertExp244', None)
    assert not _is_linked(a, 'ImperativeOCL_AssertExp244', b2)
    if hasattr(b2, 'LogExp'):
        assert not _is_linked(b2, 'LogExp', a)


def test_assoc_metamodel372_link_reassign_clear():
    a = QVTOperational_ModelType(conformanceKind="sample_text")
    b1 = Package()
    b2 = Package()
    _safe_set(a, 'QVTOperational_ModelType373', {b1})
    assert _is_linked(a, 'QVTOperational_ModelType373', b1)
    if hasattr(b1, 'Package374'):
        assert _is_linked(b1, 'Package374', a)
    _safe_set(a, 'QVTOperational_ModelType373', {b2})
    assert _is_linked(a, 'QVTOperational_ModelType373', b2)
    if hasattr(b1, 'Package374'):
        assert not _is_linked(b1, 'Package374', a)
    if hasattr(b2, 'Package374'):
        assert _is_linked(b2, 'Package374', a)
    _safe_set(a, 'QVTOperational_ModelType373', set())
    assert not _is_linked(a, 'QVTOperational_ModelType373', b2)
    if hasattr(b2, 'Package374'):
        assert not _is_linked(b2, 'Package374', a)


def test_assoc_module392_link_reassign_clear():
    a = QVTOperational_ModuleImport(kind="sample_text")
    b1 = Module()
    b2 = Module()
    _safe_set(a, 'moduleImport', b1)
    assert _is_linked(a, 'moduleImport', b1)
    if hasattr(b1, 'Module393'):
        assert _is_linked(b1, 'Module393', a)
    _safe_set(a, 'moduleImport', b2)
    assert _is_linked(a, 'moduleImport', b2)
    if hasattr(b1, 'Module393'):
        assert not _is_linked(b1, 'Module393', a)
    if hasattr(b2, 'Module393'):
        assert _is_linked(b2, 'Module393', a)
    _safe_set(a, 'moduleImport', None)
    assert not _is_linked(a, 'moduleImport', b2)
    if hasattr(b2, 'Module393'):
        assert not _is_linked(b2, 'Module393', a)


def test_assoc_moduleImport379_link_reassign_clear():
    a = QVTOperational_Module(isBlackbox="sample_text")
    b1 = ModuleImport()
    b2 = ModuleImport()
...
def test_assoc_nestedPackage13_link_reassign_clear():
    ...
    if hasattr(b1, 'Package14'):
        assert _is_linked(b1, 'Package14', a)
    _safe_set(a, 'nestingPackage', {b2})
    assert _is_linked(a, 'nestingPackage', b2)
    if hasattr(b1, 'Package14'):
        assert not _is_linked(b1, 'Package14', a)
    if hasattr(b2, 'Package14'):
        assert _is_linked(b2, 'Package14', a)
    _safe_set(a, 'nestingPackage', set())
    assert not _is_linked(a, 'nestingPackage', b2)
    if hasattr(b2, 'Package14'):
        assert not _is_linked(b2, 'Package14', a)
...
def test_assoc_operationalImpl203_link_reassign_clear():
...
def test_assoc_opposite23_link_reassign_clear():
    ...
    if hasattr(b1, 'Property24'):
        assert _is_linked(b1, 'Property24', a)
    _safe_set(a, 'EMOF_Property', b2)
    assert _is_linked(a, 'EMOF_Property', b2)
    if hasattr(b1, 'Property24'):
        assert not _is_linked(b1, 'Property24', a)
    if hasattr(b2, 'Property24'):
        assert _is_linked(b2, 'Property24', a)
    _safe_set(a, 'EMOF_Property', None)
    assert not _is_linked(a, 'EMOF_Property', b2)
    if hasattr(b2, 'Property24'):
        assert not _is_linked(b2, 'Property24', a)


def test_assoc_overridden340_link_reassign_clear():
    a = QVTOperational_ImperativeOperation(isBlackbox="sample_text")
    b1 = ImperativeOperation()
    b2 = ImperativeOperation()
...
def test_assoc_ownedTag380_link_reassign_clear():
    a = QVTOperational_Module(isBlackbox="sample_text")
    b1 = Tag()
    b2 = Tag()
    _safe_set(a, 'QVTOperational_Module381', {b1})
    assert _is_linked(a, 'QVTOperational_Module381', b1)
    if hasattr(b1, 'Tag382'):
        assert _is_linked(b1, 'Tag382', a)
    _safe_set(a, 'QVTOperational_Module381', {b2})
    assert _is_linked(a, 'QVTOperational_Module381', b2)
    if hasattr(b1, 'Tag382'):
        assert not _is_linked(b1, 'Tag382', a)
    if hasattr(b2, 'Tag382'):
        assert _is_linked(b2, 'Tag382', a)
    _safe_set(a, 'QVTOperational_Module381', set())
    assert not _is_linked(a, 'QVTOperational_Module381', b2)
    if hasattr(b2, 'Tag382'):
        assert not _is_linked(b2, 'Tag382', a)
...
def test_assoc_ownedType17_link_reassign_clear():
    ...
    if hasattr(b1, 'Type18'):
        assert _is_linked(b1, 'Type18', a)
    _safe_set(a, 'package', {b2})
    assert _is_linked(a, 'package', b2)
    if hasattr(b1, 'Type18'):
        assert not _is_linked(b1, 'Type18', a)
    if hasattr(b2, 'Type18'):
        assert _is_linked(b2, 'Type18', a)
    _safe_set(a, 'package', set())
    assert not _is_linked(a, 'package', b2)
    if hasattr(b2, 'Type18'):
        assert not _is_linked(b2, 'Type18', a)


def test_assoc_ownedVariable383_link_reassign_clear():
    a = QVTOperational_Module(isBlackbox="sample_text")
    b1 = Variable()
    b2 = Variable()
    _safe_set(a, 'QVTOperational_Module384', {b1})
    assert _is_linked(a, 'QVTOperational_Module384', b1)
    if hasattr(b1, 'Variable385'):
        assert _is_linked(b1, 'Variable385', a)
    _safe_set(a, 'QVTOperational_Module384', {b2})
    assert _is_linked(a, 'QVTOperational_Module384', b2)
    if hasattr(b1, 'Variable385'):
        assert not _is_linked(b1, 'Variable385', a)
    if hasattr(b2, 'Variable385'):
        assert _is_linked(b2, 'Variable385', a)
    _safe_set(a, 'QVTOperational_Module384', set())
    assert not _is_linked(a, 'QVTOperational_Module384', b2)
    if hasattr(b2, 'Variable385'):
        assert not _is_linked(b2, 'Variable385', a)


def test_assoc_package26_link_reassign_clear():
    a = EMOF_Type()
    b1 = Package()
    b2 = Package()
    _safe_set(a, 'ownedType', b1)
    assert _is_linked(a, 'ownedType', b1)
    if hasattr(b1, 'Package27'):
        assert _is_linked(b1, 'Package27', a)
    _safe_set(a, 'ownedType', b2)
    assert _is_linked(a, 'ownedType', b2)
    if hasattr(b1, 'Package27'):
        assert not _is_linked(b1, 'Package27', a)
    if hasattr(b2, 'Package27'):
        assert _is_linked(b2, 'Package27', a)
    _safe_set(a, 'ownedType', None)
    assert not _is_linked(a, 'ownedType', b2)
    if hasattr(b2, 'Package27'):
        assert not _is_linked(b2, 'Package27', a)


def test_assoc_package8_link_reassign_clear():
    a = EMOF_Factory()
    b1 = Package()
    b2 = Package()
...
def test_assoc_part33_link_reassign_clear():
...
def test_assoc_referredVariable322_link_reassign_clear():
    ...
    if hasattr(b1, 'Variable323'):
        assert _is_linked(b1, 'Variable323', a)
    _safe_set(a, 'ImperativeOCL_VariableInitExp', b2)
    assert _is_linked(a, 'ImperativeOCL_VariableInitExp', b2)
    if hasattr(b1, 'Variable323'):
        assert not _is_linked(b1, 'Variable323', a)
    if hasattr(b2, 'Variable323'):
        assert _is_linked(b2, 'Variable323', a)
    _safe_set(a, 'ImperativeOCL_VariableInitExp', None)
    assert not _is_linked(a, 'ImperativeOCL_VariableInitExp', b2)
    if hasattr(b2, 'Variable323'):
        assert not _is_linked(b2, 'Variable323', a)


def test_assoc_resOwner429_link_reassign_clear():
    a = QVTOperational_VarParameter(kind="sample_text")
    b1 = ImperativeOperation()
    b2 = ImperativeOperation()
    _safe_set(a, 'result', b1)
    assert _is_linked(a, 'result', b1)
    if hasattr(b1, 'ImperativeOperation430'):
        assert _is_linked(b1, 'ImperativeOperation430', a)
    _safe_set(a, 'result', b2)
    assert _is_linked(a, 'result', b2)
    if hasattr(b1, 'ImperativeOperation430'):
        assert not _is_linked(b1, 'ImperativeOperation430', a)
    if hasattr(b2, 'ImperativeOperation430'):
        assert _is_linked(b2, 'ImperativeOperation430', a)
    _safe_set(a, 'result', None)
    assert not _is_linked(a, 'result', b2)
    if hasattr(b2, 'ImperativeOperation430'):
        assert not _is_linked(b2, 'ImperativeOperation430', a)


def test_assoc_result341_link_reassign_clear():
    a = QVTOperational_ImperativeOperation(isBlackbox="sample_text")
    b1 = VarParameter()
    b2 = VarParameter()
    _safe_set(a, 'resOwner', {b1})
    assert _is_linked(a, 'resOwner', b1)
    if hasattr(b1, 'VarParameter342'):
        assert _is_linked(b1, 'VarParameter342', a)
    _safe_set(a, 'resOwner', {b2})
    assert _is_linked(a, 'resOwner', b2)
    if hasattr(b1, 'VarParameter342'):
        assert not _is_linked(b1, 'VarParameter342', a)
    if hasattr(b2, 'VarParameter342'):
        assert _is_linked(b2, 'VarParameter342', a)
    _safe_set(a, 'resOwner', set())
    assert not _is_linked(a, 'resOwner', b2)
    if hasattr(b2, 'VarParameter342'):
        assert not _is_linked(b2, 'VarParameter342', a)


def test_assoc_rule101_link_reassign_clear():
...
def test_assoc_target421_link_reassign_clear():
    a = QVTOperational_ResolveExp(isDeferred="sample_text", isInverse="sample_text", one="sample_text")
    b1 = Variable()
    b2 = Variable()
    _safe_set(a, 'QVTOperational_ResolveExp422', b1)
    assert _is_linked(a, 'QVTOperational_ResolveExp422', b1)
    if hasattr(b1, 'Variable423'):
        assert _is_linked(b1, 'Variable423', a)
    _safe_set(a, 'QVTOperational_ResolveExp422', b2)
    assert _is_linked(a, 'QVTOperational_ResolveExp422', b2)
    if hasattr(b1, 'Variable423'):
        assert not _is_linked(b1, 'Variable423', a)
    if hasattr(b2, 'Variable423'):
        assert _is_linked(b2, 'Variable423', a)
    _safe_set(a, 'QVTOperational_ResolveExp422', None)
    assert not _is_linked(a, 'QVTOperational_ResolveExp422', b2)
    if hasattr(b2, 'Variable423'):
        assert not _is_linked(b2, 'Variable423', a)


def test_assoc_typedModel102_link_reassign_clear():
    a = QVTBase_Domain(isCheckable="sample_text", isEnforceable="sample_text")
    b1 = TypedModel()
    b2 = TypedModel()
...
def test_assoc_usedModelType386_link_reassign_clear():
    a = QVTOperational_Module(isBlackbox="sample_text")
    b1 = ModelType()
    b2 = ModelType()
    _safe_set(a, 'QVTOperational_Module387', {b1})
    assert _is_linked(a, 'QVTOperational_Module387', b1)
    if hasattr(b1, 'ModelType'):
        assert _is_linked(b1, 'ModelType', a)
    _safe_set(a, 'QVTOperational_Module387', {b2})
    assert _is_linked(a, 'QVTOperational_Module387', b2)
    if hasattr(b1, 'ModelType'):
        assert not _is_linked(b1, 'ModelType', a)
    if hasattr(b2, 'ModelType'):
        assert _is_linked(b2, 'ModelType', a)
    _safe_set(a, 'QVTOperational_Module387', set())
    assert not _is_linked(a, 'QVTOperational_Module387', b2)
    if hasattr(b2, 'ModelType'):
        assert not _is_linked(b2, 'ModelType', a)


def test_assoc_value137_link_reassign_clear():
    a = QVTCore_Assignment(isDefault="sample_text")
    b1 = OclExpression()
    b2 = OclExpression()
    _safe_set(a, 'QVTCore_Assignment', b1)
    assert _is_linked(a, 'QVTCore_Assignment', b1)
    if hasattr(b1, 'OclExpression138'):
        assert _is_linked(b1, 'OclExpression138', a)
    _safe_set(a, 'QVTCore_Assignment', b2)
    assert _is_linked(a, 'QVTCore_Assignment', b2)
    if hasattr(b1, 'OclExpression138'):
        assert not _is_linked(b1, 'OclExpression138', a)
    if hasattr(b2, 'OclExpression138'):
        assert _is_linked(b2, 'OclExpression138', a)
    _safe_set(a, 'QVTCore_Assignment', None)
    assert not _is_linked(a, 'QVTCore_Assignment', b2)
    if hasattr(b2, 'OclExpression138'):
        assert not _is_linked(b2, 'OclExpression138', a)


def test_assoc_value183_link_reassign_clear():
    a = QVTTemplate_PropertyTemplateItem(isOpposite="sample_text")
    b1 = OclExpression()
    b2 = OclExpression()
    _safe_set(a, 'QVTTemplate_PropertyTemplateItem184', b1)
    assert _is_linked(a, 'QVTTemplate_PropertyTemplateItem184', b1)
    if hasattr(b1, 'OclExpression185'):
        assert _is_linked(b1, 'OclExpression185', a)
    _safe_set(a, 'QVTTemplate_PropertyTemplateItem184', b2)
    assert _is_linked(a, 'QVTTemplate_PropertyTemplateItem184', b2)
    if hasattr(b1, 'OclExpression185'):
        assert not _is_linked(b1, 'OclExpression185', a)
    if hasattr(b2, 'OclExpression185'):
        assert _is_linked(b2, 'OclExpression185', a)
    _safe_set(a, 'QVTTemplate_PropertyTemplateItem184', None)
    assert not _is_linked(a, 'QVTTemplate_PropertyTemplateItem184', b2)
    if hasattr(b2, 'OclExpression185'):
        assert not _is_linked(b2, 'OclExpression185', a)
...
def test_assoc_value250_link_reassign_clear():
    ...
    b1 = OclExpression()
    b2 = OclExpression()
    _safe_set(a, 'ImperativeOCL_AssignExp251', {b1})
    assert _is_linked(a, 'ImperativeOCL_AssignExp251', b1)
    if hasattr(b1, 'OclExpression252'):
        assert _is_linked(b1, 'OclExpression252', a)
    _safe_set(a, 'ImperativeOCL_AssignExp251', {b2})
    assert _is_linked(a, 'ImperativeOCL_AssignExp251', b2)
    if hasattr(b1, 'OclExpression252'):
        assert not _is_linked(b1, 'OclExpression252', a)
    if hasattr(b2, 'OclExpression252'):
        assert _is_linked(b2, 'OclExpression252', a)
    _safe_set(a, 'ImperativeOCL_AssignExp251', set())
    assert not _is_linked(a, 'ImperativeOCL_AssignExp251', b2)
    if hasattr(b2, 'OclExpression252'):
        assert not _is_linked(b2, 'OclExpression252', a)


def test_assoc_variable204_link_reassign_clear():
    a = QVTRelation_Relation(isTopLevel="sample_text")
    b1 = Variable()
    b2 = Variable()
    _safe_set(a, 'QVTRelation_Relation', {b1})
    assert _is_linked(a, 'QVTRelation_Relation', b1)
    if hasattr(b1, 'Variable205'):
        assert _is_linked(b1, 'Variable205', a)
    _safe_set(a, 'QVTRelation_Relation', {b2})
    assert _is_linked(a, 'QVTRelation_Relation', b2)
    if hasattr(b1, 'Variable205'):
        assert not _is_linked(b1, 'Variable205', a)
    if hasattr(b2, 'Variable205'):
        assert _is_linked(b2, 'Variable205', a)
    _safe_set(a, 'QVTRelation_Relation', set())
    assert not _is_linked(a, 'QVTRelation_Relation', b2)
    if hasattr(b2, 'Variable205'):
        assert not _is_linked(b2, 'Variable205', a)


def test_assoc_when206_link_reassign_clear():
    a = QVTRelation_Relation(isTopLevel="sample_text")
    b1 = Pattern()
    b2 = Pattern()
    _safe_set(a, 'QVTRelation_Relation207', b1)
    assert _is_linked(a, 'QVTRelation_Relation207', b1)
    if hasattr(b1, 'Pattern208'):
        assert _is_linked(b1, 'Pattern208', a)
    _safe_set(a, 'QVTRelation_Relation207', b2)
    assert _is_linked(a, 'QVTRelation_Relation207', b2)
    if hasattr(b1, 'Pattern208'):
        assert not _is_linked(b1, 'Pattern208', a)
    if hasattr(b2, 'Pattern208'):
        assert _is_linked(b2, 'Pattern208', a)
    _safe_set(a, 'QVTRelation_Relation207', None)
    assert not _is_linked(a, 'QVTRelation_Relation207', b2)
    if hasattr(b2, 'Pattern208'):
        assert not _is_linked(b2, 'Pattern208', a)


def test_assoc_where209_link_reassign_clear():
    a = QVTRelation_Relation(isTopLevel="sample_text")
    b1 = Pattern()
    b2 = Pattern()
    _safe_set(a, 'QVTRelation_Relation210', b1)
    assert _is_linked(a, 'QVTRelation_Relation210', b1)
    if hasattr(b1, 'Pattern211'):
        assert _is_linked(b1, 'Pattern211', a)
    _safe_set(a, 'QVTRelation_Relation210', b2)
    assert _is_linked(a, 'QVTRelation_Relation210', b2)
    if hasattr(b1, 'Pattern211'):
        assert not _is_linked(b1, 'Pattern211', a)
    if hasattr(b2, 'Pattern211'):
        assert _is_linked(b2, 'Pattern211', a)
    _safe_set(a, 'QVTRelation_Relation210', None)
    assert not _is_linked(a, 'QVTRelation_Relation210', b2)
    if hasattr(b2, 'Pattern211'):
        assert not _is_linked(b2, 'Pattern211', a)
