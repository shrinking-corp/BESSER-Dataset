...
def test_hyp_fkeytype_exists():
    # Check that the Enumeration exists
    assert FKeyType is not None

def test_hyp_fkeytype_has_all_literals():
...
def test_hyp_simplerdbms_fkey_fkeyType_setter(instance):
...
def test_SimpleRDBMS_FKey_fkeyType_value_roundtrip():
    instance = SimpleRDBMS_FKey(fkeyType="sample_text")
    assert instance.fkeyType == "sample_text"
    instance.fkeyType = "sample_text_2"
    assert instance.fkeyType == "sample_text_2"
...
def test_assoc_cols9_link_reassign_clear():
    a = SimpleRDBMS_FKey(fkeyType="sample_text")
    b1 = SimpleRDBMS_Column(id=7, name="sample_text", type="sample_text")
    b2 = SimpleRDBMS_Column(id=13, name="sample_text_2", type="sample_text_2")
    _safe_set(a, 'SimpleRDBMS_FKey10', {b1})
    assert _is_linked(a, 'SimpleRDBMS_FKey10', b1)
    if hasattr(b1, 'SimpleRDBMS_Column11'):
        assert _is_linked(b1, 'SimpleRDBMS_Column11', a)
    _safe_set(a, 'SimpleRDBMS_FKey10', {b2})
    assert _is_linked(a, 'SimpleRDBMS_FKey10', b2)
    if hasattr(b1, 'SimpleRDBMS_Column11'):
        assert not _is_linked(b1, 'SimpleRDBMS_Column11', a)
    if hasattr(b2, 'SimpleRDBMS_Column11'):
        assert _is_linked(b2, 'SimpleRDBMS_Column11', a)
    _safe_set(a, 'SimpleRDBMS_FKey10', set())
    assert not _is_linked(a, 'SimpleRDBMS_FKey10', b2)
    if hasattr(b2, 'SimpleRDBMS_Column11'):
        assert not _is_linked(b2, 'SimpleRDBMS_Column11', a)


def test_assoc_fkeys0_link_reassign_clear():
    a = SimpleRDBMS_Table(id=7, name="sample_text")
    b1 = SimpleRDBMS_FKey(fkeyType="sample_text")
    b2 = SimpleRDBMS_FKey(fkeyType="sample_text_2")
...
SimpleRDBMS_FKey_strategy = st.builds(SimpleRDBMS_FKey, fkeyType=safe_text)
@given(instance=SimpleRDBMS_FKey_strategy)
@settings(max_examples=25)
