...
def test_assoc_E_G_link_reassign_clear():
    a = E(attE="sample_text")
    b1 = G()
    b2 = G()
    _safe_set(a, 'g4', b1)
    assert _is_linked(a, 'g4', b1)
    if hasattr(b1, 'e5'):
        assert _is_linked(b1, 'e5', a)
    _safe_set(a, 'g4', b2)
    assert _is_linked(a, 'g4', b2)
    if hasattr(b1, 'e5'):
        assert not _is_linked(b1, 'e5', a)
    if hasattr(b2, 'e5'):
        assert _is_linked(b2, 'e5', a)
    _safe_set(a, 'g4', None)
    assert not _is_linked(a, 'g4', b2)
    if hasattr(b2, 'e5'):
        assert not _is_linked(b2, 'e5', a)
