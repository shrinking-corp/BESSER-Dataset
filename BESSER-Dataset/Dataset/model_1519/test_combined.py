...
def test_hyp_sm1_state_is_not_abstract():
    assert not inspect.isabstract(sm1_State)


def test_hyp_sm1_state_constructor_exists():
    assert callable(sm1_State.__init__)


def test_hyp_sm1_state_constructor_args():
...
def test_hyp_sm1_statemachine_is_not_abstract():
    assert not inspect.isabstract(sm1_StateMachine)


def test_hyp_sm1_statemachine_constructor_exists():
    assert callable(sm1_StateMachine.__init__)


def test_hyp_sm1_statemachine_constructor_args():
    sig = inspect.signature(sm1_StateMachine.__init__)
    params = list(sig.parameters.keys())



def test_hyp_sm1_transition_is_not_abstract():
    assert not inspect.isabstract(sm1_Transition)


def test_hyp_sm1_transition_constructor_exists():
    assert callable(sm1_Transition.__init__)


def test_hyp_sm1_transition_constructor_args():
...
def test_hyp_sm1_state_name_setter(instance):
...
def test_hyp_sm1_transition_event_setter(instance):
...
def test_sm1_State_name_value_roundtrip():
    instance = sm1_State(name="sample_text")
    assert instance.name == "sample_text"
    instance.name = "sample_text_2"
    assert instance.name == "sample_text_2"


def test_sm1_Transition_event_value_roundtrip():
    instance = sm1_Transition(event="sample_text")
...
def test_assoc_edges2_link_reassign_clear():
    a = sm1_Transition(event="sample_text")
    b1 = sm1_StateMachine()
    b2 = sm1_StateMachine()
    _safe_set(a, 'sm1_Transition', b1)
    assert _is_linked(a, 'sm1_Transition', b1)
    if hasattr(b1, 'sm1_StateMachine3'):
        assert _is_linked(b1, 'sm1_StateMachine3', a)
    _safe_set(a, 'sm1_Transition', b2)
    assert _is_linked(a, 'sm1_Transition', b2)
    if hasattr(b1, 'sm1_StateMachine3'):
        assert not _is_linked(b1, 'sm1_StateMachine3', a)
    if hasattr(b2, 'sm1_StateMachine3'):
        assert _is_linked(b2, 'sm1_StateMachine3', a)
    _safe_set(a, 'sm1_Transition', None)
    assert not _is_linked(a, 'sm1_Transition', b2)
    if hasattr(b2, 'sm1_StateMachine3'):
        assert not _is_linked(b2, 'sm1_StateMachine3', a)


def test_assoc_incoming6_link_reassign_clear():
    a = sm1_Transition(event="sample_text")
    b1 = sm1_State(name="sample_text")
    b2 = sm1_State(name="sample_text_2")
...
def test_assoc_initialState0_link_reassign_clear():
    a = sm1_State(name="sample_text")
    b1 = sm1_StateMachine()
    b2 = sm1_StateMachine()
    _safe_set(a, 'sm1_State', b1)
    assert _is_linked(a, 'sm1_State', b1)
    if hasattr(b1, 'sm1_StateMachine'):
        assert _is_linked(b1, 'sm1_StateMachine', a)
    _safe_set(a, 'sm1_State', b2)
    assert _is_linked(a, 'sm1_State', b2)
    if hasattr(b1, 'sm1_StateMachine'):
        assert not _is_linked(b1, 'sm1_StateMachine', a)
    if hasattr(b2, 'sm1_StateMachine'):
        assert _is_linked(b2, 'sm1_StateMachine', a)
    _safe_set(a, 'sm1_State', None)
    assert not _is_linked(a, 'sm1_State', b2)
    if hasattr(b2, 'sm1_StateMachine'):
        assert not _is_linked(b2, 'sm1_StateMachine', a)
...
sm1_State_strategy = st.builds(sm1_State, name=safe_text)
@given(instance=sm1_State_strategy)
...
sm1_StateMachine_strategy = st.builds(sm1_StateMachine)
...
sm1_Transition_strategy = st.builds(sm1_Transition, event=safe_text)
@given(instance=sm1_Transition_strategy)
@settings(max_examples=25)
