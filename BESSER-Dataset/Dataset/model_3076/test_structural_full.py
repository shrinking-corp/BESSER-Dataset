...
def test_assoc_extendedBy8_link_reassign_clear():
...
def test_assoc_owner11_link_reassign_clear():
    ...
    if hasattr(b1, 'Class12'):
        assert _is_linked(b1, 'Class12', a)
    _safe_set(a, 'features', b2)
    assert _is_linked(a, 'features', b2)
    if hasattr(b1, 'Class12'):
        assert not _is_linked(b1, 'Class12', a)
    if hasattr(b2, 'Class12'):
        assert _is_linked(b2, 'Class12', a)
    _safe_set(a, 'features', None)
    assert not _is_linked(a, 'features', b2)
    if hasattr(b2, 'Class12'):
        assert not _is_linked(b2, 'Class12', a)
