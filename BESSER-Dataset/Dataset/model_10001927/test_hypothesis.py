...
def test_account_has_Test_attr():
    assert hasattr(Account, "Test_attr")
    descriptor = None
    for klass in Account.__mro__:
        if "Test_attr" in klass.__dict__:
            descriptor = klass.__dict__["Test_attr"]
            break
    assert isinstance(descriptor, property)
...
@given(instance=Account_strategy)
def test_account_Test_attr_setter(instance):
