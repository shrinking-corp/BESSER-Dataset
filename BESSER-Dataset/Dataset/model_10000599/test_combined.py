...
@given(instance=Card_strategy)
def test_hyp_card_face_up_setter(instance):
...
def test_Card_face_up_value_roundtrip():
    instance = Card(face_up="sample_text", rank="sample_text", suit="sample_text")
    assert instance.face_up == "sample_text"
    instance.face_up = "sample_text_2"
    assert instance.face_up == "sample_text_2"
...
def test_Player_name_value_roundtrip():
    instance = Player(name="sample_text", score=7)
...
Card_strategy = st.builds(Card, face_up=safe_text, rank=safe_text, suit=safe_text)
@given(instance=Card_strategy)
@settings(max_examples=25)
...
Player_strategy = st.builds(Player, name=safe_text, score=st.integers())
@given(instance=Player_strategy)
@settings(max_examples=25)
