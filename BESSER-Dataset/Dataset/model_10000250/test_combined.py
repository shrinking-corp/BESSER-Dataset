...
def test_hyp_connect4_client_is_not_abstract():
    assert not inspect.isabstract(Connect4_Client)


def test_hyp_connect4_client_constructor_exists():
    assert callable(Connect4_Client.__init__)


def test_hyp_connect4_client_constructor_args():
    sig = inspect.signature(Connect4_Client.__init__)
    params = list(sig.parameters.keys())



def test_hyp_connect4_server_is_not_abstract():
    assert not inspect.isabstract(Connect4_Server)


def test_hyp_connect4_server_constructor_exists():
    assert callable(Connect4_Server.__init__)


def test_hyp_connect4_server_constructor_args():
...
def test_assoc_Board_connect_link_reassign_clear():
    a = Connect4_connect(FRAME_HEIGHT=7, FRAME_WIDTH=7, columnSize=7, label1="sample_text", label2="sample_text", label3="sample_text", label4="sample_text", label5="sample_text", panel="sample_text", rowSize=7, x=7, y=7)
    b1 = Connect4_Board(gameBoard="sample_text", maxColumns=7, maxRows=7)
    b2 = Connect4_Board(gameBoard="sample_text_2", maxColumns=13, maxRows=13)
    _safe_set(a, 'board7', b1)
    assert _is_linked(a, 'board7', b1)
    if hasattr(b1, 'connect6'):
        assert _is_linked(b1, 'connect6', a)
    _safe_set(a, 'board7', b2)
    assert _is_linked(a, 'board7', b2)
    if hasattr(b1, 'connect6'):
        assert not _is_linked(b1, 'connect6', a)
    if hasattr(b2, 'connect6'):
        assert _is_linked(b2, 'connect6', a)
    _safe_set(a, 'board7', None)
    assert not _is_linked(a, 'board7', b2)
    if hasattr(b2, 'connect6'):
        assert not _is_linked(b2, 'connect6', a)


def test_assoc_CirclePanel_connect_link_reassign_clear():
    a = Connect4_connect(FRAME_HEIGHT=7, FRAME_WIDTH=7, columnSize=7, label1="sample_text", label2="sample_text", label3="sample_text", label4="sample_text", label5="sample_text", panel="sample_text", rowSize=7, x=7, y=7)
    b1 = Connect4_CirclePanel(color="sample_text", colorIndex=7)
    b2 = Connect4_CirclePanel(color="sample_text_2", colorIndex=13)
    _safe_set(a, 'circlePanel5', b1)
    assert _is_linked(a, 'circlePanel5', b1)
    if hasattr(b1, 'connect4'):
        assert _is_linked(b1, 'connect4', a)
    _safe_set(a, 'circlePanel5', b2)
    assert _is_linked(a, 'circlePanel5', b2)
    if hasattr(b1, 'connect4'):
        assert not _is_linked(b1, 'connect4', a)
    if hasattr(b2, 'connect4'):
        assert _is_linked(b2, 'connect4', a)
    _safe_set(a, 'circlePanel5', None)
    assert not _is_linked(a, 'circlePanel5', b2)
    if hasattr(b2, 'connect4'):
        assert not _is_linked(b2, 'connect4', a)


def test_assoc_Server_Board_link_reassign_clear():
    a = Connect4_Board(gameBoard="sample_text", maxColumns=7, maxRows=7)
    b1 = Connect4_Server()
    b2 = Connect4_Server()
    _safe_set(a, 'server9', b1)
    assert _is_linked(a, 'server9', b1)
    if hasattr(b1, 'board8'):
        assert _is_linked(b1, 'board8', a)
    _safe_set(a, 'server9', b2)
    assert _is_linked(a, 'server9', b2)
    if hasattr(b1, 'board8'):
        assert not _is_linked(b1, 'board8', a)
    if hasattr(b2, 'board8'):
        assert _is_linked(b2, 'board8', a)
    _safe_set(a, 'server9', None)
    assert not _is_linked(a, 'server9', b2)
    if hasattr(b2, 'board8'):
        assert not _is_linked(b2, 'board8', a)
...
Connect4_Client_strategy = st.builds(Connect4_Client)
...
Connect4_Server_strategy = st.builds(Connect4_Server)
