...
def test_artworkviewmodel_is_not_abstract():
    assert not inspect.isabstract(ArtworkViewModel)


def test_artworkviewmodel_constructor_exists():
    assert callable(ArtworkViewModel.__init__)


def test_artworkviewmodel_constructor_args():
    sig = inspect.signature(ArtworkViewModel.__init__)
    params = list(sig.parameters.keys())
...
ArtworkViewModel_strategy = st.builds(
    ArtworkViewModel,
)
...
@given(instance=ArtworkViewModel_strategy)
@settings(max_examples=50)
def test_artworkviewmodel_instantiation(instance):
    assert isinstance(instance, ArtworkViewModel)
