...
def test_data_Attachment_cachedFileName_value_roundtrip():
    instance = data_Attachment(cachedFileName="sample_text", cachedFileUrl="sample_text", cachedOnly="sample_text", fileExtension="sample_text", fileIdentifier="sample_text", fileUrl="sample_text")
...
def test_data_DataSet_cacheFileAttachements_value_roundtrip():
    instance = data_DataSet(cacheFileAttachements="sample_text", cacheFolder="sample_text", created=date(2024, 1, 1), identCounter="sample_text", identPrefix="sample_text", lastModified=date(2024, 1, 1), logLevel="sample_text")
...
def test_data_InformationObject_name_value_roundtrip():
    instance = data_InformationObject(name="sample_text")
    assert instance.name == "sample_text"
    instance.name = "sample_text_2"
    assert instance.name == "sample_text_2"
...
def test_data_WebAccount_username_value_roundtrip():
    instance = data_WebAccount(username="sample_text")
    assert instance.username == "sample_text"
    instance.username = "sample_text_2"
    assert instance.username == "sample_text_2"
...
def test_data_WebSite_adress_value_roundtrip():
    instance = data_WebSite(adress="sample_text", title="sample_text")
    assert instance.adress == "sample_text"
    instance.adress = "sample_text_2"
    assert instance.adress == "sample_text_2"
...
def test_assoc_binaries20_link_reassign_clear():
    a = data_InformationObject(name="sample_text")
...
def test_assoc_categorized49_link_reassign_clear():
    a = data_InformationObject(name="sample_text")
    b1 = data_Category()
    b2 = data_Category()
...
def test_assoc_dataSet40_link_reassign_clear():
    a = data_Item(created=date(2024, 1, 1), ident="sample_text", lastModified=date(2024, 1, 1), stringValue="sample_text", uri="sample_text")
    b1 = data_DataSet(cacheFileAttachements="sample_text", cacheFolder="sample_text", created=date(2024, 1, 1), identCounter="sample_text", identPrefix="sample_text", lastModified=date(2024, 1, 1), logLevel="sample_text")
    b2 = data_DataSet(cacheFileAttachements="sample_text_2", cacheFolder="sample_text_2", created=date(2025, 6, 15), identCounter="sample_text_2", identPrefix="sample_text_2", lastModified=date(2025, 6, 15), logLevel="sample_text_2")
...
def test_assoc_deleteOnDelete44_link_reassign_clear():
    a = data_Item(created=date(2024, 1, 1), ident="sample_text", lastModified=date(2024, 1, 1), stringValue="sample_text", uri="sample_text")
...
def test_assoc_indoorLocations75_link_reassign_clear():
...
def test_assoc_indoorLocations96_link_reassign_clear():
    a = data_IndoorLocation(name="sample_text")
    b1 = data_IndoorLocation(name="sample_text")
    b2 = data_IndoorLocation(name="sample_text_2")
    _safe_set(a, 'IndoorLocation97', b1)
    assert _is_linked(a, 'IndoorLocation97', b1)
    if hasattr(b1, 'parentIndoorLocation'):
        assert _is_linked(b1, 'parentIndoorLocation', a)
    _safe_set(a, 'IndoorLocation97', b2)
    assert _is_linked(a, 'IndoorLocation97', b2)
    if hasattr(b1, 'parentIndoorLocation'):
    ...
    _safe_set(a, 'IndoorLocation97', None)
    assert not _is_linked(a, 'IndoorLocation97', b2)
    if hasattr(b2, 'parentIndoorLocation'):
        assert not _is_linked(b2, 'parentIndoorLocation', a)


def test_assoc_informationObjects88_link_reassign_clear():
    a = data_InformationObject(name="sample_text")
    b1 = data_MetaInformation()
    b2 = data_MetaInformation()
    _safe_set(a, 'InformationObject89', b1)
    assert _is_linked(a, 'InformationObject89', b1)
    if hasattr(b1, 'metaInformations'):
        assert _is_linked(b1, 'metaInformations', a)
    _safe_set(a, 'InformationObject89', b2)
    assert _is_linked(a, 'InformationObject89', b2)
    if hasattr(b1, 'metaInformations'):
        assert not _is_linked(b1, 'metaInformations', a)
    if hasattr(b2, 'metaInformations'):
        assert _is_linked(b2, 'metaInformations', a)
    _safe_set(a, 'InformationObject89', None)
    assert not _is_linked(a, 'InformationObject89', b2)
    if hasattr(b2, 'metaInformations'):
...
def test_assoc_leader64_link_reassign_clear():
    a = data_Person(dateOfBirth=date(2024, 1, 1), firstname="sample_text", lastname="sample_text", title="sample_text")
    b1 = data_Organisation()
    b2 = data_Organisation()
    _safe_set(a, 'Person65', b1)
    assert _is_linked(a, 'Person65', b1)
    if hasattr(b1, 'leaderOf'):
        assert _is_linked(b1, 'leaderOf', a)
    _safe_set(a, 'Person65', b2)
    assert _is_linked(a, 'Person65', b2)
    if hasattr(b1, 'leaderOf'):
        assert not _is_linked(b1, 'leaderOf', a)
    if hasattr(b2, 'leaderOf'):
        assert _is_linked(b2, 'leaderOf', a)
    _safe_set(a, 'Person65', None)
    assert not _is_linked(a, 'Person65', b2)
    if hasattr(b2, 'leaderOf'):
...
def test_assoc_location90_link_reassign_clear():
    a = data_Location(city="sample_text", country="sample_text", houseNumber="sample_text", latitude="sample_text", longitude="sample_text", state="sample_text", street="sample_text", zipCode="sample_text")
    b1 = data_IndoorLocation(name="sample_text")
    b2 = data_IndoorLocation(name="sample_text_2")
    _safe_set(a, 'Location', b1)
...
def test_assoc_mainCategorized57_link_reassign_clear():
    a = data_InformationObject(name="sample_text")
    b1 = data_Category()
    b2 = data_Category()
    _safe_set(a, 'InformationObject58', b1)
    assert _is_linked(a, 'InformationObject58', b1)
    if hasattr(b1, 'mainCategory'):
        assert _is_linked(b1, 'mainCategory', a)
    _safe_set(a, 'InformationObject58', b2)
    assert _is_linked(a, 'InformationObject58', b2)
    if hasattr(b1, 'mainCategory'):
        assert not _is_linked(b1, 'mainCategory', a)
    if hasattr(b2, 'mainCategory'):
        assert _is_linked(b2, 'mainCategory', a)
    _safe_set(a, 'InformationObject58', None)
    assert not _is_linked(a, 'InformationObject58', b2)
    if hasattr(b2, 'mainCategory'):
...
def test_assoc_metaTagged71_link_reassign_clear():
    a = data_MetaTag(name="sample_text")
    b1 = data_Item(created=date(2024, 1, 1), ident="sample_text", lastModified=date(2024, 1, 1), stringValue="sample_text", uri="sample_text")
    b2 = data_Item(created=date(2025, 6, 15), ident="sample_text_2", lastModified=date(2025, 6, 15), stringValue="sample_text_2", uri="sample_text_2")
    _safe_set(a, 'metaTags', {b1})
    assert _is_linked(a, 'metaTags', b1)
    if hasattr(b1, 'Item72'):
        assert _is_linked(b1, 'Item72', a)
    _safe_set(a, 'metaTags', {b2})
    assert _is_linked(a, 'metaTags', b2)
    if hasattr(b1, 'Item72'):
        assert not _is_linked(b1, 'Item72', a)
    if hasattr(b2, 'Item72'):
        assert _is_linked(b2, 'Item72', a)
    _safe_set(a, 'metaTags', set())
    assert not _is_linked(a, 'metaTags', b2)
    if hasattr(b2, 'Item72'):
        assert not _is_linked(b2, 'Item72', a)


def test_assoc_metaTags41_link_reassign_clear():
    a = data_MetaTag(name="sample_text")
    b1 = data_Item(created=date(2024, 1, 1), ident="sample_text", lastModified=date(2024, 1, 1), stringValue="sample_text", uri="sample_text")
    b2 = data_Item(created=date(2025, 6, 15), ident="sample_text_2", lastModified=date(2025, 6, 15), stringValue="sample_text_2", uri="sample_text_2")
...
def test_assoc_organisations69_link_reassign_clear():
    a = data_Organisation()
    b1 = data_Organisation()
    b2 = data_Organisation()
    _safe_set(a, 'Organisation70', b1)
    assert _is_linked(a, 'Organisation70', b1)
    if hasattr(b1, 'parentOrganisation'):
        assert _is_linked(b1, 'parentOrganisation', a)
    _safe_set(a, 'Organisation70', b2)
    assert _is_linked(a, 'Organisation70', b2)
    if hasattr(b1, 'parentOrganisation'):
        assert not _is_linked(b1, 'parentOrganisation', a)
    if hasattr(b2, 'parentOrganisation'):
        assert _is_linked(b2, 'parentOrganisation', a)
    _safe_set(a, 'Organisation70', None)
    assert not _is_linked(a, 'Organisation70', b2)
    if hasattr(b2, 'parentOrganisation'):
...
def test_assoc_parentIndoorLocation92_link_reassign_clear():
    a = data_IndoorLocation(name="sample_text")
    b1 = data_IndoorLocation(name="sample_text")
    b2 = data_IndoorLocation(name="sample_text_2")
    _safe_set(a, 'IndoorLocation94', b1)
    assert _is_linked(a, 'IndoorLocation94', b1)
    if hasattr(b1, 'indoorLocations93'):
        assert _is_linked(b1, 'indoorLocations93', a)
    _safe_set(a, 'IndoorLocation94', b2)
    assert _is_linked(a, 'IndoorLocation94', b2)
    if hasattr(b1, 'indoorLocations93'):
        assert not _is_linked(b1, 'indoorLocations93', a)
    if hasattr(b2, 'indoorLocations93'):
        assert _is_linked(b2, 'indoorLocations93', a)
    _safe_set(a, 'IndoorLocation94', None)
    assert not _is_linked(a, 'IndoorLocation94', b2)
    if hasattr(b2, 'indoorLocations93'):
        assert not _is_linked(b2, 'indoorLocations93', a)


def test_assoc_parentOrganisation62_link_reassign_clear():
    a = data_Organisation()
    b1 = data_Organisation()
    b2 = data_Organisation()
    _safe_set(a, 'Organisation63', b1)
    assert _is_linked(a, 'Organisation63', b1)
    if hasattr(b1, 'organisations'):
        assert _is_linked(b1, 'organisations', a)
    _safe_set(a, 'Organisation63', b2)
    assert _is_linked(a, 'Organisation63', b2)
    if hasattr(b1, 'organisations'):
        assert not _is_linked(b1, 'organisations', a)
    if hasattr(b2, 'organisations'):
        assert _is_linked(b2, 'organisations', a)
    _safe_set(a, 'Organisation63', None)
    assert not _is_linked(a, 'Organisation63', b2)
    if hasattr(b2, 'organisations'):
        assert not _is_linked(b2, 'organisations', a)


def test_assoc_participants66_link_reassign_clear():
    a = data_Person(dateOfBirth=date(2024, 1, 1), firstname="sample_text", lastname="sample_text", title="sample_text")
    b1 = data_Organisation()
    b2 = data_Organisation()
    _safe_set(a, 'Person67', b1)
    assert _is_linked(a, 'Person67', b1)
    if hasattr(b1, 'participates'):
        assert _is_linked(b1, 'participates', a)
    _safe_set(a, 'Person67', b2)
    assert _is_linked(a, 'Person67', b2)
    if hasattr(b1, 'participates'):
        assert not _is_linked(b1, 'participates', a)
    if hasattr(b2, 'participates'):
        assert _is_linked(b2, 'participates', a)
    _safe_set(a, 'Person67', None)
    assert not _is_linked(a, 'Person67', b2)
    if hasattr(b2, 'participates'):
...
def test_assoc_rankedInformationObject76_link_reassign_clear():
    a = data_StarRanking(normalizedValue="sample_text")
    b1 = data_InformationObject(name="sample_text")
    b2 = data_InformationObject(name="sample_text_2")
    _safe_set(a, 'starRankings', b1)
    assert _is_linked(a, 'starRankings', b1)
    if hasattr(b1, 'InformationObject77'):
        assert _is_linked(b1, 'InformationObject77', a)
    _safe_set(a, 'starRankings', b2)
    assert _is_linked(a, 'starRankings', b2)
    if hasattr(b1, 'InformationObject77'):
        assert not _is_linked(b1, 'InformationObject77', a)
    if hasattr(b2, 'InformationObject77'):
        assert _is_linked(b2, 'InformationObject77', a)
    _safe_set(a, 'starRankings', None)
    assert not _is_linked(a, 'starRankings', b2)
    if hasattr(b2, 'InformationObject77'):
        assert not _is_linked(b2, 'InformationObject77', a)


def test_assoc_rankedInformationObject78_link_reassign_clear():
    a = data_InformationObject(name="sample_text")
    b1 = data_ViewRanking()
    b2 = data_ViewRanking()
    _safe_set(a, 'InformationObject79', b1)
    assert _is_linked(a, 'InformationObject79', b1)
    if hasattr(b1, 'viewRankings'):
        assert _is_linked(b1, 'viewRankings', a)
    _safe_set(a, 'InformationObject79', b2)
    assert _is_linked(a, 'InformationObject79', b2)
    if hasattr(b1, 'viewRankings'):
        assert not _is_linked(b1, 'viewRankings', a)
    if hasattr(b2, 'viewRankings'):
        assert _is_linked(b2, 'viewRankings', a)
    _safe_set(a, 'InformationObject79', None)
    assert not _is_linked(a, 'InformationObject79', b2)
    if hasattr(b2, 'viewRankings'):
        assert not _is_linked(b2, 'viewRankings', a)


def test_assoc_rankedInformationObject80_link_reassign_clear():
    a = data_ThumbRanking()
    b1 = data_InformationObject(name="sample_text")
    b2 = data_InformationObject(name="sample_text_2")
    _safe_set(a, 'thumbRankings', b1)
    assert _is_linked(a, 'thumbRankings', b1)
    if hasattr(b1, 'InformationObject81'):
        assert _is_linked(b1, 'InformationObject81', a)
    _safe_set(a, 'thumbRankings', b2)
    assert _is_linked(a, 'thumbRankings', b2)
    if hasattr(b1, 'InformationObject81'):
        assert not _is_linked(b1, 'InformationObject81', a)
    if hasattr(b2, 'InformationObject81'):
        assert _is_linked(b2, 'InformationObject81', a)
    _safe_set(a, 'thumbRankings', None)
    assert not _is_linked(a, 'thumbRankings', b2)
    if hasattr(b2, 'InformationObject81'):
        assert not _is_linked(b2, 'InformationObject81', a)


def test_assoc_ranker73_link_reassign_clear():
    a = data_Ranking(date=date(2024, 1, 1))
    b1 = data_Person(dateOfBirth=date(2024, 1, 1), firstname="sample_text", lastname="sample_text", title="sample_text")
    b2 = data_Person(dateOfBirth=date(2025, 6, 15), firstname="sample_text_2", lastname="sample_text_2", title="sample_text_2")
    _safe_set(a, 'ranked', b1)
    assert _is_linked(a, 'ranked', b1)
    if hasattr(b1, 'Person74'):
        assert _is_linked(b1, 'Person74', a)
    _safe_set(a, 'ranked', b2)
    assert _is_linked(a, 'ranked', b2)
    if hasattr(b1, 'Person74'):
        assert not _is_linked(b1, 'Person74', a)
    if hasattr(b2, 'Person74'):
        assert _is_linked(b2, 'Person74', a)
    _safe_set(a, 'ranked', None)
    assert not _is_linked(a, 'ranked', b2)
    if hasattr(b2, 'Person74'):
        assert not _is_linked(b2, 'Person74', a)


def test_assoc_setUp39_link_reassign_clear():
    a = data_DataSet(cacheFileAttachements="sample_text", cacheFolder="sample_text", created=date(2024, 1, 1), identCounter="sample_text", identPrefix="sample_text", lastModified=date(2024, 1, 1), logLevel="sample_text")
    b1 = data_Mashup()
    b2 = data_Mashup()
...
def test_assoc_tagged59_link_reassign_clear():
    a = data_InformationObject(name="sample_text")
    b1 = data_Tag()
    b2 = data_Tag()
    _safe_set(a, 'InformationObject60', b1)
    assert _is_linked(a, 'InformationObject60', b1)
    if hasattr(b1, 'tags'):
        assert _is_linked(b1, 'tags', a)
    _safe_set(a, 'InformationObject60', b2)
    assert _is_linked(a, 'InformationObject60', b2)
    if hasattr(b1, 'tags'):
        assert not _is_linked(b1, 'tags', a)
    if hasattr(b2, 'tags'):
        assert _is_linked(b2, 'tags', a)
    _safe_set(a, 'InformationObject60', None)
    assert not _is_linked(a, 'InformationObject60', b2)
    if hasattr(b2, 'tags'):
...
def test_assoc_to86_link_reassign_clear():
    a = data_InformationObject(name="sample_text")
    b1 = data_Connection()
    b2 = data_Connection()
...
def test_assoc_transformed82_link_reassign_clear():
    a = data_Content(locale="sample_text")
    b1 = data_Transformation()
    b2 = data_Transformation()
    _safe_set(a, 'Content83', b1)
    assert _is_linked(a, 'Content83', b1)
    if hasattr(b1, 'transformations'):
        assert _is_linked(b1, 'transformations', a)
    _safe_set(a, 'Content83', b2)
    assert _is_linked(a, 'Content83', b2)
    if hasattr(b1, 'transformations'):
        assert not _is_linked(b1, 'transformations', a)
    if hasattr(b2, 'transformations'):
        assert _is_linked(b2, 'transformations', a)
    _safe_set(a, 'Content83', None)
    assert not _is_linked(a, 'Content83', b2)
    if hasattr(b2, 'transformations'):
...
data_Attachment_strategy = st.builds(data_Attachment, cachedFileName=safe_text, cachedFileUrl=safe_text, cachedOnly=safe_text, fileExtension=safe_text, fileIdentifier=safe_text, fileUrl=safe_text)
...
data_DataSet_strategy = st.builds(data_DataSet, cacheFileAttachements=safe_text, cacheFolder=safe_text, created=st.dates(), identCounter=safe_text, identPrefix=safe_text, lastModified=st.dates(), logLevel=safe_text)
...
data_Image_strategy = st.builds(data_Image)
...
data_InformationObject_strategy = st.builds(data_InformationObject, name=safe_text)
...
data_WebAccount_strategy = st.builds(data_WebAccount, username=safe_text)
...
data_WebSite_strategy = st.builds(data_WebSite, adress=safe_text, title=safe_text)
