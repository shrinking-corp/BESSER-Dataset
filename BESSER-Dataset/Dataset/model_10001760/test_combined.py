...
@given(instance=Sensor_strategy)
def test_hyp_sensor_Status_setter(instance):
...
def test_Sensor_SensorID_value_roundtrip():
    instance = Sensor(SensorID=7, SensorType=7, Status=True)
...
def test_Sensor_Status_value_roundtrip():
    instance = Sensor(SensorID=7, SensorType=7, Status=True)
...
def test_assoc_Sensor_Door_link_reassign_clear():
    a = Sensor(SensorID=7, SensorType=7, Status=True)
    b1 = Door(DoorID=7)
    b2 = Door(DoorID=13)
...
def test_assoc_Sensor_System_link_reassign_clear():
    a = System(Status=True, Update=3.14)
    b1 = Sensor(SensorID=7, SensorType=7, Status=True)
    b2 = Sensor(SensorID=13, SensorType=13, Status=False)
...
Sensor_strategy = st.builds(Sensor, SensorID=st.integers(), SensorType=st.integers(), Status=st.booleans())
@given(instance=Sensor_strategy)
@settings(max_examples=25)
