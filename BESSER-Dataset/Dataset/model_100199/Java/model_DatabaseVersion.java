





import java.util.List;
import java.util.ArrayList;

public class model_DatabaseVersion  {






    private model_Database model_database;




    private TableMapping tablemapping;




    private model_DatabaseVersion model_databaseversion;




    private model_DatabaseVersion model_databaseversion;




    private model_DatabaseVersions model_databaseversions;


    public model_DatabaseVersion(
    ) {
    }



    public model_Database getModel_database() {
        return model_database;
    }

    public void setModel_database(model_Database model_database) {
        this.model_database = model_database;
    }
    public TableMapping getTablemapping() {
        return tablemapping;
    }

    public void setTablemapping(TableMapping tablemapping) {
        this.tablemapping = tablemapping;
    }
    public model_DatabaseVersion getModel_databaseversion() {
        return model_databaseversion;
    }

    public void setModel_databaseversion(model_DatabaseVersion model_databaseversion) {
        this.model_databaseversion = model_databaseversion;
    }
    public model_DatabaseVersion getModel_databaseversion() {
        return model_databaseversion;
    }

    public void setModel_databaseversion(model_DatabaseVersion model_databaseversion) {
        this.model_databaseversion = model_databaseversion;
    }
    public model_DatabaseVersions getModel_databaseversions() {
        return model_databaseversions;
    }

    public void setModel_databaseversions(model_DatabaseVersions model_databaseversions) {
        this.model_databaseversions = model_databaseversions;
    }

}