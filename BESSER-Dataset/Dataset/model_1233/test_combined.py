...
def test_hyp_ocl_cst_enumliteralexpcs_is_not_abstract():
    assert not inspect.isabstract(ocl_cst_EnumLiteralExpCS)


def test_hyp_ocl_cst_enumliteralexpcs_constructor_exists():
    assert callable(ocl_cst_EnumLiteralExpCS.__init__)


def test_hyp_ocl_cst_enumliteralexpcs_constructor_args():
...
def test_hyp_ocl_cst_stateexpcs_is_not_abstract():
    assert not inspect.isabstract(ocl_cst_StateExpCS)


def test_hyp_ocl_cst_stateexpcs_constructor_exists():
    assert callable(ocl_cst_StateExpCS.__init__)


def test_hyp_ocl_cst_stateexpcs_constructor_args():
...
def test_hyp_ocl_cst_invalidliteralexpcs_symbol_setter(instance):
...
def test_hyp_ocl_cst_nullliteralexpcs_symbol_setter(instance):
    original = instance.symbol
    instance.symbol = original
    assert instance.symbol == original
...
def test_hyp_ocl_cst_ismarkedprecs_pre_setter(instance):
...
def test_hyp_ocl_cst_stateexpcs_sequenceOfNames_setter(instance):
...
def test_hyp_ocl_cst_pathnamecs_sequenceOfNames_setter(instance):
    original = instance.sequenceOfNames
    instance.sequenceOfNames = original
    assert instance.sequenceOfNames == original
...
def test_assoc_arguments92_link_reassign_clear():
    ...
    b1 = OCLMessageArgCS()
    b2 = OCLMessageArgCS()
    _safe_set(a, 'ocl_cst_MessageExpCS93', {b1})
    assert _is_linked(a, 'ocl_cst_MessageExpCS93', b1)
    if hasattr(b1, 'OCLMessageArgCS'):
        assert _is_linked(b1, 'OCLMessageArgCS', a)
    _safe_set(a, 'ocl_cst_MessageExpCS93', {b2})
    assert _is_linked(a, 'ocl_cst_MessageExpCS93', b2)
    if hasattr(b1, 'OCLMessageArgCS'):
        assert not _is_linked(b1, 'OCLMessageArgCS', a)
    if hasattr(b2, 'OCLMessageArgCS'):
        assert _is_linked(b2, 'OCLMessageArgCS', a)
    _safe_set(a, 'ocl_cst_MessageExpCS93', set())
    assert not _is_linked(a, 'ocl_cst_MessageExpCS93', b2)
    if hasattr(b2, 'OCLMessageArgCS'):
        assert not _is_linked(b2, 'OCLMessageArgCS', a)


def test_assoc_collectionLiteralParts104_link_reassign_clear():
...
def test_assoc_expressionCS49_link_reassign_clear():
    a = ocl_cst_PrePostOrBodyDeclCS(kind="sample_text")
    b1 = OCLExpressionCS()
    b2 = OCLExpressionCS()
    _safe_set(a, 'ocl_cst_PrePostOrBodyDeclCS50', b1)
    assert _is_linked(a, 'ocl_cst_PrePostOrBodyDeclCS50', b1)
    if hasattr(b1, 'OCLExpressionCS51'):
        assert _is_linked(b1, 'OCLExpressionCS51', a)
    _safe_set(a, 'ocl_cst_PrePostOrBodyDeclCS50', b2)
    assert _is_linked(a, 'ocl_cst_PrePostOrBodyDeclCS50', b2)
    if hasattr(b1, 'OCLExpressionCS51'):
        assert not _is_linked(b1, 'OCLExpressionCS51', a)
    if hasattr(b2, 'OCLExpressionCS51'):
        assert _is_linked(b2, 'OCLExpressionCS51', a)
    _safe_set(a, 'ocl_cst_PrePostOrBodyDeclCS50', None)
    assert not _is_linked(a, 'ocl_cst_PrePostOrBodyDeclCS50', b2)
    if hasattr(b2, 'OCLExpressionCS51'):
        assert not _is_linked(b2, 'OCLExpressionCS51', a)


def test_assoc_initExpression44_link_reassign_clear():
    a = ocl_cst_VariableCS(name="sample_text")
    b1 = OCLExpressionCS()
    b2 = OCLExpressionCS()
    _safe_set(a, 'ocl_cst_VariableCS45', b1)
    assert _is_linked(a, 'ocl_cst_VariableCS45', b1)
    if hasattr(b1, 'OCLExpressionCS46'):
        assert _is_linked(b1, 'OCLExpressionCS46', a)
    _safe_set(a, 'ocl_cst_VariableCS45', b2)
    assert _is_linked(a, 'ocl_cst_VariableCS45', b2)
    if hasattr(b1, 'OCLExpressionCS46'):
        assert not _is_linked(b1, 'OCLExpressionCS46', a)
    if hasattr(b2, 'OCLExpressionCS46'):
        assert _is_linked(b2, 'OCLExpressionCS46', a)
    _safe_set(a, 'ocl_cst_VariableCS45', None)
    assert not _is_linked(a, 'ocl_cst_VariableCS45', b2)
    if hasattr(b2, 'OCLExpressionCS46'):
        assert not _is_linked(b2, 'OCLExpressionCS46', a)


def test_assoc_simpleNameCS113_link_reassign_clear():
    a = ocl_cst_CallExpCS(accessor="sample_text")
    b1 = SimpleNameCS()
    b2 = SimpleNameCS()
    _safe_set(a, 'ocl_cst_CallExpCS114', b1)
    assert _is_linked(a, 'ocl_cst_CallExpCS114', b1)
    if hasattr(b1, 'SimpleNameCS115'):
        assert _is_linked(b1, 'SimpleNameCS115', a)
    _safe_set(a, 'ocl_cst_CallExpCS114', b2)
    assert _is_linked(a, 'ocl_cst_CallExpCS114', b2)
    if hasattr(b1, 'SimpleNameCS115'):
        assert not _is_linked(b1, 'SimpleNameCS115', a)
    if hasattr(b2, 'SimpleNameCS115'):
        assert _is_linked(b2, 'SimpleNameCS115', a)
    _safe_set(a, 'ocl_cst_CallExpCS114', None)
    assert not _is_linked(a, 'ocl_cst_CallExpCS114', b2)
    if hasattr(b2, 'SimpleNameCS115'):
        assert not _is_linked(b2, 'SimpleNameCS115', a)


def test_assoc_simpleNameCS47_link_reassign_clear():
    a = ocl_cst_PrePostOrBodyDeclCS(kind="sample_text")
    b1 = SimpleNameCS()
    b2 = SimpleNameCS()
    _safe_set(a, 'ocl_cst_PrePostOrBodyDeclCS', b1)
    assert _is_linked(a, 'ocl_cst_PrePostOrBodyDeclCS', b1)
    if hasattr(b1, 'SimpleNameCS48'):
        assert _is_linked(b1, 'SimpleNameCS48', a)
    _safe_set(a, 'ocl_cst_PrePostOrBodyDeclCS', b2)
    assert _is_linked(a, 'ocl_cst_PrePostOrBodyDeclCS', b2)
    if hasattr(b1, 'SimpleNameCS48'):
        assert not _is_linked(b1, 'SimpleNameCS48', a)
    if hasattr(b2, 'SimpleNameCS48'):
        assert _is_linked(b2, 'SimpleNameCS48', a)
    _safe_set(a, 'ocl_cst_PrePostOrBodyDeclCS', None)
    assert not _is_linked(a, 'ocl_cst_PrePostOrBodyDeclCS', b2)
    if hasattr(b2, 'SimpleNameCS48'):
        assert not _is_linked(b2, 'SimpleNameCS48', a)


def test_assoc_simpleNameCS89_link_reassign_clear():
    a = ocl_cst_MessageExpCS(kind="sample_text")
    b1 = SimpleNameCS()
    b2 = SimpleNameCS()
    _safe_set(a, 'ocl_cst_MessageExpCS90', b1)
    assert _is_linked(a, 'ocl_cst_MessageExpCS90', b1)
    if hasattr(b1, 'SimpleNameCS91'):
        assert _is_linked(b1, 'SimpleNameCS91', a)
    _safe_set(a, 'ocl_cst_MessageExpCS90', b2)
    assert _is_linked(a, 'ocl_cst_MessageExpCS90', b2)
    if hasattr(b1, 'SimpleNameCS91'):
        assert not _is_linked(b1, 'SimpleNameCS91', a)
    if hasattr(b2, 'SimpleNameCS91'):
        assert _is_linked(b2, 'SimpleNameCS91', a)
    _safe_set(a, 'ocl_cst_MessageExpCS90', None)
    assert not _is_linked(a, 'ocl_cst_MessageExpCS90', b2)
    if hasattr(b2, 'SimpleNameCS91'):
        assert not _is_linked(b2, 'SimpleNameCS91', a)
...
def test_assoc_source111_link_reassign_clear():
    ...
    if hasattr(b1, 'OCLExpressionCS112'):
        assert _is_linked(b1, 'OCLExpressionCS112', a)
    _safe_set(a, 'ocl_cst_CallExpCS', b2)
    assert _is_linked(a, 'ocl_cst_CallExpCS', b2)
    if hasattr(b1, 'OCLExpressionCS112'):
        assert not _is_linked(b1, 'OCLExpressionCS112', a)
    if hasattr(b2, 'OCLExpressionCS112'):
        assert _is_linked(b2, 'OCLExpressionCS112', a)
    _safe_set(a, 'ocl_cst_CallExpCS', None)
    assert not _is_linked(a, 'ocl_cst_CallExpCS', b2)
    if hasattr(b2, 'OCLExpressionCS112'):
        assert not _is_linked(b2, 'OCLExpressionCS112', a)


def test_assoc_target87_link_reassign_clear():
    a = ocl_cst_MessageExpCS(kind="sample_text")
    b1 = OCLExpressionCS()
    b2 = OCLExpressionCS()
    _safe_set(a, 'ocl_cst_MessageExpCS', b1)
    assert _is_linked(a, 'ocl_cst_MessageExpCS', b1)
    if hasattr(b1, 'OCLExpressionCS88'):
        assert _is_linked(b1, 'OCLExpressionCS88', a)
    _safe_set(a, 'ocl_cst_MessageExpCS', b2)
    assert _is_linked(a, 'ocl_cst_MessageExpCS', b2)
    if hasattr(b1, 'OCLExpressionCS88'):
        assert not _is_linked(b1, 'OCLExpressionCS88', a)
    if hasattr(b2, 'OCLExpressionCS88'):
        assert _is_linked(b2, 'OCLExpressionCS88', a)
    _safe_set(a, 'ocl_cst_MessageExpCS', None)
    assert not _is_linked(a, 'ocl_cst_MessageExpCS', b2)
    if hasattr(b2, 'OCLExpressionCS88'):
        assert not _is_linked(b2, 'OCLExpressionCS88', a)


def test_assoc_typeCS42_link_reassign_clear():
    a = ocl_cst_VariableCS(name="sample_text")
    b1 = TypeCS()
    b2 = TypeCS()
    _safe_set(a, 'ocl_cst_VariableCS', b1)
    assert _is_linked(a, 'ocl_cst_VariableCS', b1)
    if hasattr(b1, 'TypeCS43'):
        assert _is_linked(b1, 'TypeCS43', a)
    _safe_set(a, 'ocl_cst_VariableCS', b2)
    assert _is_linked(a, 'ocl_cst_VariableCS', b2)
    if hasattr(b1, 'TypeCS43'):
        assert not _is_linked(b1, 'TypeCS43', a)
    if hasattr(b2, 'TypeCS43'):
        assert _is_linked(b2, 'TypeCS43', a)
    _safe_set(a, 'ocl_cst_VariableCS', None)
    assert not _is_linked(a, 'ocl_cst_VariableCS', b2)
    if hasattr(b2, 'TypeCS43'):
        assert not _is_linked(b2, 'TypeCS43', a)


def test_assoc_typeCS72_link_reassign_clear():
