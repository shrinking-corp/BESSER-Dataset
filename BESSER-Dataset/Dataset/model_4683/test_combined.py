...
def test_assoc_index1_link_reassign_clear():
    ...
    b1 = imp_Expr()
    b2 = imp_Expr()
    _safe_set(a, 'imp_Assign2', b1)
    assert _is_linked(a, 'imp_Assign2', b1)
    if hasattr(b1, 'imp_Expr3'):
        assert _is_linked(b1, 'imp_Expr3', a)
    _safe_set(a, 'imp_Assign2', b2)
    assert _is_linked(a, 'imp_Assign2', b2)
    if hasattr(b1, 'imp_Expr3'):
        assert not _is_linked(b1, 'imp_Expr3', a)
    if hasattr(b2, 'imp_Expr3'):
        assert _is_linked(b2, 'imp_Expr3', a)
    _safe_set(a, 'imp_Assign2', None)
    assert not _is_linked(a, 'imp_Assign2', b2)
    if hasattr(b2, 'imp_Expr3'):
        assert not _is_linked(b2, 'imp_Expr3', a)


def test_assoc_index18_link_reassign_clear():
    a = imp_Var(name="sample_text")
    b1 = imp_Expr()
    b2 = imp_Expr()
    _safe_set(a, 'imp_Var', b1)
    assert _is_linked(a, 'imp_Var', b1)
    if hasattr(b1, 'imp_Expr19'):
        assert _is_linked(b1, 'imp_Expr19', a)
    _safe_set(a, 'imp_Var', b2)
    assert _is_linked(a, 'imp_Var', b2)
    if hasattr(b1, 'imp_Expr19'):
        assert not _is_linked(b1, 'imp_Expr19', a)
    if hasattr(b2, 'imp_Expr19'):
        assert _is_linked(b2, 'imp_Expr19', a)
    _safe_set(a, 'imp_Var', None)
    assert not _is_linked(a, 'imp_Var', b2)
    if hasattr(b2, 'imp_Expr19'):
...
def test_assoc_lhs22_link_reassign_clear():
    ...
    if hasattr(b1, 'imp_Expr23'):
        assert _is_linked(b1, 'imp_Expr23', a)
    _safe_set(a, 'imp_Binary', b2)
    assert _is_linked(a, 'imp_Binary', b2)
    if hasattr(b1, 'imp_Expr23'):
        assert not _is_linked(b1, 'imp_Expr23', a)
    if hasattr(b2, 'imp_Expr23'):
        assert _is_linked(b2, 'imp_Expr23', a)
    _safe_set(a, 'imp_Binary', None)
    assert not _is_linked(a, 'imp_Binary', b2)
    if hasattr(b2, 'imp_Expr23'):
        assert not _is_linked(b2, 'imp_Expr23', a)


def test_assoc_rhs24_link_reassign_clear():
    a = imp_Binary(op="sample_text")
    b1 = imp_Expr()
    b2 = imp_Expr()
    _safe_set(a, 'imp_Binary25', b1)
    assert _is_linked(a, 'imp_Binary25', b1)
    if hasattr(b1, 'imp_Expr26'):
        assert _is_linked(b1, 'imp_Expr26', a)
    _safe_set(a, 'imp_Binary25', b2)
    assert _is_linked(a, 'imp_Binary25', b2)
    if hasattr(b1, 'imp_Expr26'):
        assert not _is_linked(b1, 'imp_Expr26', a)
    if hasattr(b2, 'imp_Expr26'):
        assert _is_linked(b2, 'imp_Expr26', a)
    _safe_set(a, 'imp_Binary25', None)
    assert not _is_linked(a, 'imp_Binary25', b2)
    if hasattr(b2, 'imp_Expr26'):
        assert not _is_linked(b2, 'imp_Expr26', a)
...
def test_assoc_value28_link_reassign_clear():
    ...
    b1 = imp_Value()
    b2 = imp_Value()
    _safe_set(a, 'imp_StringToValueMap29', b1)
    assert _is_linked(a, 'imp_StringToValueMap29', b1)
    if hasattr(b1, 'imp_Value'):
        assert _is_linked(b1, 'imp_Value', a)
    _safe_set(a, 'imp_StringToValueMap29', b2)
    assert _is_linked(a, 'imp_StringToValueMap29', b2)
    if hasattr(b1, 'imp_Value'):
        assert not _is_linked(b1, 'imp_Value', a)
    if hasattr(b2, 'imp_Value'):
        assert _is_linked(b2, 'imp_Value', a)
    _safe_set(a, 'imp_StringToValueMap29', None)
    assert not _is_linked(a, 'imp_StringToValueMap29', b2)
    if hasattr(b2, 'imp_Value'):
        assert not _is_linked(b2, 'imp_Value', a)


def test_assoc_values27_link_reassign_clear():
    a = imp_StringToValueMap(key="sample_text")
    b1 = imp_Store()
    b2 = imp_Store()
