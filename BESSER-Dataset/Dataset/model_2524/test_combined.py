...
def test_hyp_lhs_d_is_not_abstract():
    assert not inspect.isabstract(LHS_D)


def test_hyp_lhs_d_constructor_exists():
    assert callable(LHS_D.__init__)


def test_hyp_lhs_d_constructor_args():
    sig = inspect.signature(LHS_D.__init__)
...
@given(instance=LHS_D_strategy)
def test_hyp_lhs_d_name_setter(instance):
...
def test_hyp_lhs_c_name_setter(instance):
    original = instance.name
    instance.name = original
    assert instance.name == original
...
def test_LHS_C_name_value_roundtrip():
    instance = LHS_C(name="sample_text")
    assert instance.name == "sample_text"
    instance.name = "sample_text_2"
    assert instance.name == "sample_text_2"


def test_LHS_D_name_value_roundtrip():
    instance = LHS_D(name="sample_text")
    assert instance.name == "sample_text"
    instance.name = "sample_text_2"
    assert instance.name == "sample_text_2"
...
def test_assoc_refC1_link_reassign_clear():
    a = LHS_C(name="sample_text")
    b1 = LHS_A(name="sample_text")
    b2 = LHS_A(name="sample_text_2")
...
def test_assoc_refD3_link_reassign_clear():
    a = LHS_D(name="sample_text")
    b1 = LHS_B(name="sample_text")
    b2 = LHS_B(name="sample_text_2")
    _safe_set(a, 'LHS_D', b1)
    assert _is_linked(a, 'LHS_D', b1)
    if hasattr(b1, 'LHS_B4'):
        assert _is_linked(b1, 'LHS_B4', a)
    _safe_set(a, 'LHS_D', b2)
    assert _is_linked(a, 'LHS_D', b2)
    if hasattr(b1, 'LHS_B4'):
        assert not _is_linked(b1, 'LHS_B4', a)
    if hasattr(b2, 'LHS_B4'):
        assert _is_linked(b2, 'LHS_B4', a)
    _safe_set(a, 'LHS_D', None)
    assert not _is_linked(a, 'LHS_D', b2)
    if hasattr(b2, 'LHS_B4'):
        assert not _is_linked(b2, 'LHS_B4', a)
...
LHS_C_strategy = st.builds(LHS_C, name=safe_text)
@given(instance=LHS_C_strategy)
@settings(max_examples=25)
...
LHS_D_strategy = st.builds(LHS_D, name=safe_text)
@given(instance=LHS_D_strategy)
@settings(max_examples=25)
def test_LHS_D_instantiation(instance):
    assert isinstance(instance, LHS_D)
