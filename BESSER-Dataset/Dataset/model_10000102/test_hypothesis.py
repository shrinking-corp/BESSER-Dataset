...
def test_owner_register_is_not_abstract():
    assert not inspect.isabstract(Owner_register)


def test_owner_register_constructor_exists():
    assert callable(Owner_register.__init__)


def test_owner_register_constructor_args():
    sig = inspect.signature(Owner_register.__init__)
    params = list(sig.parameters.keys())
...
Owner_register_strategy = st.builds(
    Owner_register,
)
...
@given(instance=Owner_register_strategy)
@settings(max_examples=50)
def test_owner_register_instantiation(instance):
    assert isinstance(instance, Owner_register)
