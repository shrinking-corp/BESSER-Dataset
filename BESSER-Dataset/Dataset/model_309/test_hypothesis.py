...
def test_library_book_has_borrowed():
    assert hasattr(library_Book, "borrowed")
    descriptor = None
    for klass in library_Book.__mro__:
        if "borrowed" in klass.__dict__:
            descriptor = klass.__dict__["borrowed"]
            break
    assert isinstance(descriptor, property)
...
@given(instance=library_Book_strategy)
def test_library_book_borrowed_setter(instance):
