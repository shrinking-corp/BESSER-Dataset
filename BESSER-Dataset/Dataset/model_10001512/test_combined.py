...
def test_assoc_Customer_Manager_link_reassign_clear():
    ...
    if hasattr(b1, 'manager4'):
        assert _is_linked(b1, 'manager4', a)
    _safe_set(a, 'customer5', b2)
    assert _is_linked(a, 'customer5', b2)
    if hasattr(b1, 'manager4'):
        assert not _is_linked(b1, 'manager4', a)
    if hasattr(b2, 'manager4'):
        assert _is_linked(b2, 'manager4', a)
    _safe_set(a, 'customer5', None)
    assert not _is_linked(a, 'customer5', b2)
    if hasattr(b2, 'manager4'):
        assert not _is_linked(b2, 'manager4', a)
