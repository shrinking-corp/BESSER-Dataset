...
def test_assoc_WorkProduct163_link_reassign_clear():
    a = uma_WorkProductDescriptor(activityEntryState="sample_text", activityExitState="sample_text")
    b1 = uma_WorkProduct()
    b2 = uma_WorkProduct()
    _safe_set(a, 'uma_WorkProductDescriptor164', b1)
    assert _is_linked(a, 'uma_WorkProductDescriptor164', b1)
    if hasattr(b1, 'uma_WorkProduct165'):
        assert _is_linked(b1, 'uma_WorkProduct165', a)
    _safe_set(a, 'uma_WorkProductDescriptor164', b2)
    assert _is_linked(a, 'uma_WorkProductDescriptor164', b2)
    if hasattr(b1, 'uma_WorkProduct165'):
        assert not _is_linked(b1, 'uma_WorkProduct165', a)
    if hasattr(b2, 'uma_WorkProduct165'):
        assert _is_linked(b2, 'uma_WorkProduct165', a)
    _safe_set(a, 'uma_WorkProductDescriptor164', None)
    assert not _is_linked(a, 'uma_WorkProductDescriptor164', b2)
    if hasattr(b2, 'uma_WorkProduct165'):
        assert not _is_linked(b2, 'uma_WorkProduct165', a)


def test_assoc_activityReferences207_link_reassign_clear():
    a = uma_Activity(isEnactable="sample_text")
    b1 = uma_Practice()
    b2 = uma_Practice()
    _safe_set(a, 'uma_Activity209', b1)
    assert _is_linked(a, 'uma_Activity209', b1)
    if hasattr(b1, 'uma_Practice208'):
        assert _is_linked(b1, 'uma_Practice208', a)
    _safe_set(a, 'uma_Activity209', b2)
    assert _is_linked(a, 'uma_Activity209', b2)
    if hasattr(b1, 'uma_Practice208'):
        assert not _is_linked(b1, 'uma_Practice208', a)
    if hasattr(b2, 'uma_Practice208'):
        assert _is_linked(b2, 'uma_Practice208', a)
    _safe_set(a, 'uma_Activity209', None)
    assert not _is_linked(a, 'uma_Activity209', b2)
    if hasattr(b2, 'uma_Practice208'):
        assert not _is_linked(b2, 'uma_Practice208', a)


def test_assoc_bases302_link_reassign_clear():
    ...
    _safe_set(a, 'uma_MethodPlugin301', {b1})
    assert _is_linked(a, 'uma_MethodPlugin301', b1)
    if hasattr(b1, 'uma_MethodPlugin303'):
        assert _is_linked(b1, 'uma_MethodPlugin303', a)
    _safe_set(a, 'uma_MethodPlugin301', {b2})
    assert _is_linked(a, 'uma_MethodPlugin301', b2)
    if hasattr(b1, 'uma_MethodPlugin303'):
        assert not _is_linked(b1, 'uma_MethodPlugin303', a)
    if hasattr(b2, 'uma_MethodPlugin303'):
        assert _is_linked(b2, 'uma_MethodPlugin303', a)
    _safe_set(a, 'uma_MethodPlugin301', set())
    assert not _is_linked(a, 'uma_MethodPlugin301', b2)
    if hasattr(b2, 'uma_MethodPlugin303'):
        assert not _is_linked(b2, 'uma_MethodPlugin303', a)


def test_assoc_breakdownElements117_link_reassign_clear():
...
def test_assoc_categorizedElements266_link_reassign_clear():
    ...
    _safe_set(a, 'uma_DescribableElement267', b1)
    assert _is_linked(a, 'uma_DescribableElement267', b1)
    if hasattr(b1, 'uma_CustomCategory'):
        assert _is_linked(b1, 'uma_CustomCategory', a)
    _safe_set(a, 'uma_DescribableElement267', b2)
    assert _is_linked(a, 'uma_DescribableElement267', b2)
    if hasattr(b1, 'uma_CustomCategory'):
        assert not _is_linked(b1, 'uma_CustomCategory', a)
    if hasattr(b2, 'uma_CustomCategory'):
        assert _is_linked(b2, 'uma_CustomCategory', a)
    _safe_set(a, 'uma_DescribableElement267', None)
    assert not _is_linked(a, 'uma_DescribableElement267', b2)
    if hasattr(b2, 'uma_CustomCategory'):
        assert not _is_linked(b2, 'uma_CustomCategory', a)


def test_assoc_center115_link_reassign_clear():
    ...
    _safe_set(a, 'uma_Point116', b1)
    assert _is_linked(a, 'uma_Point116', b1)
    if hasattr(b1, 'uma_Ellipse'):
        assert _is_linked(b1, 'uma_Ellipse', a)
    _safe_set(a, 'uma_Point116', b2)
    assert _is_linked(a, 'uma_Point116', b2)
    if hasattr(b1, 'uma_Ellipse'):
        assert not _is_linked(b1, 'uma_Ellipse', a)
    if hasattr(b2, 'uma_Ellipse'):
        assert _is_linked(b2, 'uma_Ellipse', a)
    _safe_set(a, 'uma_Point116', None)
    assert not _is_linked(a, 'uma_Point116', b2)
    if hasattr(b2, 'uma_Ellipse'):
        assert not _is_linked(b2, 'uma_Ellipse', a)


def test_assoc_checklists122_link_reassign_clear():
    a = uma_Activity(isEnactable="sample_text")
    b1 = uma_Checklist()
    b2 = uma_Checklist()
    _safe_set(a, 'uma_Activity123', {b1})
    assert _is_linked(a, 'uma_Activity123', b1)
    if hasattr(b1, 'uma_Checklist124'):
        assert _is_linked(b1, 'uma_Checklist124', a)
    _safe_set(a, 'uma_Activity123', {b2})
    assert _is_linked(a, 'uma_Activity123', b2)
    if hasattr(b1, 'uma_Checklist124'):
        assert not _is_linked(b1, 'uma_Checklist124', a)
    if hasattr(b2, 'uma_Checklist124'):
        assert _is_linked(b2, 'uma_Checklist124', a)
    _safe_set(a, 'uma_Activity123', set())
    assert not _is_linked(a, 'uma_Activity123', b2)
    if hasattr(b2, 'uma_Checklist124'):
        assert not _is_linked(b2, 'uma_Checklist124', a)


def test_assoc_childPackages70_link_reassign_clear():
    ...
    _safe_set(a, 'uma_MethodPackage69', {b1})
    assert _is_linked(a, 'uma_MethodPackage69', b1)
    if hasattr(b1, 'uma_MethodPackage71'):
        assert _is_linked(b1, 'uma_MethodPackage71', a)
    _safe_set(a, 'uma_MethodPackage69', {b2})
    assert _is_linked(a, 'uma_MethodPackage69', b2)
    if hasattr(b1, 'uma_MethodPackage71'):
    ...
    _safe_set(a, 'uma_MethodPackage69', set())
    assert not _is_linked(a, 'uma_MethodPackage69', b2)
    if hasattr(b2, 'uma_MethodPackage71'):
        assert not _is_linked(b2, 'uma_MethodPackage71', a)


def test_assoc_concepts125_link_reassign_clear():
    a = uma_Activity(isEnactable="sample_text")
    b1 = uma_Concept()
    b2 = uma_Concept()
    _safe_set(a, 'uma_Activity126', {b1})
    assert _is_linked(a, 'uma_Activity126', b1)
    if hasattr(b1, 'uma_Concept127'):
        assert _is_linked(b1, 'uma_Concept127', a)
    _safe_set(a, 'uma_Activity126', {b2})
    assert _is_linked(a, 'uma_Activity126', b2)
    if hasattr(b1, 'uma_Concept127'):
        assert not _is_linked(b1, 'uma_Concept127', a)
    if hasattr(b2, 'uma_Concept127'):
        assert _is_linked(b2, 'uma_Concept127', a)
    _safe_set(a, 'uma_Activity126', set())
    assert not _is_linked(a, 'uma_Activity126', b2)
    if hasattr(b2, 'uma_Concept127'):
        assert not _is_linked(b2, 'uma_Concept127', a)


def test_assoc_contained75_link_reassign_clear():
...
def test_assoc_container81_link_reassign_clear():
    a = uma_DiagramElement(isVisible="sample_text")
    b1 = uma_GraphElement()
    b2 = uma_GraphElement()
...
def test_assoc_copyrightStatement306_link_reassign_clear():
    ...
    if hasattr(b1, 'uma_SupportingMaterial307'):
        assert _is_linked(b1, 'uma_SupportingMaterial307', a)
    _safe_set(a, 'uma_MethodUnit', b2)
    assert _is_linked(a, 'uma_MethodUnit', b2)
    if hasattr(b1, 'uma_SupportingMaterial307'):
        assert not _is_linked(b1, 'uma_SupportingMaterial307', a)
    if hasattr(b2, 'uma_SupportingMaterial307'):
        assert _is_linked(b2, 'uma_SupportingMaterial307', a)
    _safe_set(a, 'uma_MethodUnit', None)
    assert not _is_linked(a, 'uma_MethodUnit', b2)
    if hasattr(b2, 'uma_SupportingMaterial307'):
        assert not _is_linked(b2, 'uma_SupportingMaterial307', a)


def test_assoc_deliverableParts172_link_reassign_clear():
    a = uma_WorkProductDescriptor(activityEntryState="sample_text", activityExitState="sample_text")
    ...
    _safe_set(a, 'uma_WorkProductDescriptor171', {b1})
    assert _is_linked(a, 'uma_WorkProductDescriptor171', b1)
    if hasattr(b1, 'uma_WorkProductDescriptor173'):
        assert _is_linked(b1, 'uma_WorkProductDescriptor173', a)
    _safe_set(a, 'uma_WorkProductDescriptor171', {b2})
    assert _is_linked(a, 'uma_WorkProductDescriptor171', b2)
    if hasattr(b1, 'uma_WorkProductDescriptor173'):
        assert not _is_linked(b1, 'uma_WorkProductDescriptor173', a)
    if hasattr(b2, 'uma_WorkProductDescriptor173'):
        assert _is_linked(b2, 'uma_WorkProductDescriptor173', a)
    _safe_set(a, 'uma_WorkProductDescriptor171', set())
    assert not _is_linked(a, 'uma_WorkProductDescriptor171', b2)
    if hasattr(b2, 'uma_WorkProductDescriptor173'):
        assert not _is_linked(b2, 'uma_WorkProductDescriptor173', a)


def test_assoc_diagram88_link_reassign_clear():
...
def test_assoc_diagram94_link_reassign_clear():
    a = uma_SemanticModelBridge(presentation="sample_text")
    b1 = uma_Diagram(zoom="sample_text")
    b2 = uma_Diagram(zoom="sample_text_2")
    _safe_set(a, 'namespace', b1)
    assert _is_linked(a, 'namespace', b1)
    if hasattr(b1, 'Diagram95'):
        assert _is_linked(b1, 'Diagram95', a)
    _safe_set(a, 'namespace', b2)
    assert _is_linked(a, 'namespace', b2)
    if hasattr(b1, 'Diagram95'):
        assert not _is_linked(b1, 'Diagram95', a)
    if hasattr(b2, 'Diagram95'):
        assert _is_linked(b2, 'Diagram95', a)
    _safe_set(a, 'namespace', None)
    assert not _is_linked(a, 'namespace', b2)
    if hasattr(b2, 'Diagram95'):
        assert not _is_linked(b2, 'Diagram95', a)


def test_assoc_diagramLink104_link_reassign_clear():
    a = uma_DiagramLink(zoom="sample_text")
    b1 = uma_Diagram(zoom="sample_text")
    b2 = uma_Diagram(zoom="sample_text_2")
    _safe_set(a, 'DiagramLink105', b1)
    assert _is_linked(a, 'DiagramLink105', b1)
    if hasattr(b1, 'diagram'):
        assert _is_linked(b1, 'diagram', a)
    _safe_set(a, 'DiagramLink105', b2)
    assert _is_linked(a, 'DiagramLink105', b2)
    if hasattr(b1, 'diagram'):
        assert not _is_linked(b1, 'diagram', a)
    if hasattr(b2, 'diagram'):
        assert _is_linked(b2, 'diagram', a)
    _safe_set(a, 'DiagramLink105', None)
    assert not _is_linked(a, 'DiagramLink105', b2)
    if hasattr(b2, 'diagram'):
        assert not _is_linked(b2, 'diagram', a)


def test_assoc_diagrams288_link_reassign_clear():
    a = uma_Diagram(zoom="sample_text")
    b1 = uma_ProcessPackage()
    b2 = uma_ProcessPackage()
    _safe_set(a, 'uma_Diagram290', b1)
    assert _is_linked(a, 'uma_Diagram290', b1)
    if hasattr(b1, 'uma_ProcessPackage289'):
        assert _is_linked(b1, 'uma_ProcessPackage289', a)
    _safe_set(a, 'uma_Diagram290', b2)
    assert _is_linked(a, 'uma_Diagram290', b2)
    if hasattr(b1, 'uma_ProcessPackage289'):
        assert not _is_linked(b1, 'uma_ProcessPackage289', a)
    if hasattr(b2, 'uma_ProcessPackage289'):
        assert _is_linked(b2, 'uma_ProcessPackage289', a)
    _safe_set(a, 'uma_Diagram290', None)
    assert not _is_linked(a, 'uma_Diagram290', b2)
    if hasattr(b2, 'uma_ProcessPackage289'):
        assert not _is_linked(b2, 'uma_ProcessPackage289', a)


def test_assoc_element110_link_reassign_clear():
    ...
    _safe_set(a, 'uma_MethodElement111', b1)
    assert _is_linked(a, 'uma_MethodElement111', b1)
    if hasattr(b1, 'uma_UMASemanticModelBridge'):
        assert _is_linked(b1, 'uma_UMASemanticModelBridge', a)
    _safe_set(a, 'uma_MethodElement111', b2)
    assert _is_linked(a, 'uma_MethodElement111', b2)
    if hasattr(b1, 'uma_UMASemanticModelBridge'):
    ...
    _safe_set(a, 'uma_MethodElement111', None)
    assert not _is_linked(a, 'uma_MethodElement111', b2)
    if hasattr(b2, 'uma_UMASemanticModelBridge'):
        assert not _is_linked(b2, 'uma_UMASemanticModelBridge', a)


def test_assoc_examples128_link_reassign_clear():
    a = uma_Activity(isEnactable="sample_text")
    b1 = uma_Example()
    b2 = uma_Example()
    _safe_set(a, 'uma_Activity129', {b1})
    assert _is_linked(a, 'uma_Activity129', b1)
    if hasattr(b1, 'uma_Example130'):
        assert _is_linked(b1, 'uma_Example130', a)
    _safe_set(a, 'uma_Activity129', {b2})
    assert _is_linked(a, 'uma_Activity129', b2)
    if hasattr(b1, 'uma_Example130'):
        assert not _is_linked(b1, 'uma_Example130', a)
    if hasattr(b2, 'uma_Example130'):
        assert _is_linked(b2, 'uma_Example130', a)
    _safe_set(a, 'uma_Activity129', set())
    assert not _is_linked(a, 'uma_Activity129', b2)
    if hasattr(b2, 'uma_Example130'):
        assert not _is_linked(b2, 'uma_Example130', a)


def test_assoc_externalInput182_link_reassign_clear():
    a = uma_WorkProductDescriptor(activityEntryState="sample_text", activityExitState="sample_text")
    b1 = uma_TaskDescriptor()
    b2 = uma_TaskDescriptor()
    _safe_set(a, 'uma_WorkProductDescriptor184', b1)
    assert _is_linked(a, 'uma_WorkProductDescriptor184', b1)
    if hasattr(b1, 'uma_TaskDescriptor183'):
        assert _is_linked(b1, 'uma_TaskDescriptor183', a)
    _safe_set(a, 'uma_WorkProductDescriptor184', b2)
    assert _is_linked(a, 'uma_WorkProductDescriptor184', b2)
    if hasattr(b1, 'uma_TaskDescriptor183'):
        assert not _is_linked(b1, 'uma_TaskDescriptor183', a)
    if hasattr(b2, 'uma_TaskDescriptor183'):
        assert _is_linked(b2, 'uma_TaskDescriptor183', a)
    _safe_set(a, 'uma_WorkProductDescriptor184', None)
    assert not _is_linked(a, 'uma_WorkProductDescriptor184', b2)
    if hasattr(b2, 'uma_TaskDescriptor183'):
        assert not _is_linked(b2, 'uma_TaskDescriptor183', a)


def test_assoc_graphElement86_link_reassign_clear():
    a = uma_DiagramLink(zoom="sample_text")
    b1 = uma_GraphElement()
    b2 = uma_GraphElement()
    _safe_set(a, 'link', b1)
    assert _is_linked(a, 'link', b1)
    if hasattr(b1, 'GraphElement87'):
        assert _is_linked(b1, 'GraphElement87', a)
    _safe_set(a, 'link', b2)
    assert _is_linked(a, 'link', b2)
    if hasattr(b1, 'GraphElement87'):
        assert not _is_linked(b1, 'GraphElement87', a)
    if hasattr(b2, 'GraphElement87'):
        assert _is_linked(b2, 'GraphElement87', a)
    _safe_set(a, 'link', None)
    assert not _is_linked(a, 'link', b2)
    if hasattr(b2, 'GraphElement87'):
        assert not _is_linked(b2, 'GraphElement87', a)


def test_assoc_graphElement92_link_reassign_clear():
    a = uma_SemanticModelBridge(presentation="sample_text")
    b1 = uma_GraphElement()
    b2 = uma_GraphElement()
    _safe_set(a, 'semanticModel', b1)
    assert _is_linked(a, 'semanticModel', b1)
    if hasattr(b1, 'GraphElement93'):
        assert _is_linked(b1, 'GraphElement93', a)
    _safe_set(a, 'semanticModel', b2)
    assert _is_linked(a, 'semanticModel', b2)
    if hasattr(b1, 'GraphElement93'):
        assert not _is_linked(b1, 'GraphElement93', a)
    if hasattr(b2, 'GraphElement93'):
        assert _is_linked(b2, 'GraphElement93', a)
    _safe_set(a, 'semanticModel', None)
    assert not _is_linked(a, 'semanticModel', b2)
    if hasattr(b2, 'GraphElement93'):
        assert not _is_linked(b2, 'GraphElement93', a)


def test_assoc_guidelines131_link_reassign_clear():
    a = uma_Activity(isEnactable="sample_text")
    b1 = uma_Guideline()
    b2 = uma_Guideline()
    _safe_set(a, 'uma_Activity132', {b1})
    assert _is_linked(a, 'uma_Activity132', b1)
    if hasattr(b1, 'uma_Guideline133'):
        assert _is_linked(b1, 'uma_Guideline133', a)
    _safe_set(a, 'uma_Activity132', {b2})
    assert _is_linked(a, 'uma_Activity132', b2)
    if hasattr(b1, 'uma_Guideline133'):
        assert not _is_linked(b1, 'uma_Guideline133', a)
    if hasattr(b2, 'uma_Guideline133'):
        assert _is_linked(b2, 'uma_Guideline133', a)
    _safe_set(a, 'uma_Activity132', set())
    assert not _is_linked(a, 'uma_Activity132', b2)
    if hasattr(b2, 'uma_Guideline133'):
        assert not _is_linked(b2, 'uma_Guideline133', a)


def test_assoc_impactedBy167_link_reassign_clear():
    a = uma_WorkProductDescriptor(activityEntryState="sample_text", activityExitState="sample_text")
    b1 = uma_WorkProductDescriptor(activityEntryState="sample_text", activityExitState="sample_text")
    b2 = uma_WorkProductDescriptor(activityEntryState="sample_text_2", activityExitState="sample_text_2")
...
def test_assoc_impacts169_link_reassign_clear():
    a = uma_WorkProductDescriptor(activityEntryState="sample_text", activityExitState="sample_text")
    b1 = uma_WorkProductDescriptor(activityEntryState="sample_text", activityExitState="sample_text")
    b2 = uma_WorkProductDescriptor(activityEntryState="sample_text_2", activityExitState="sample_text_2")
    _safe_set(a, 'WorkProductDescriptor170', b1)
    assert _is_linked(a, 'WorkProductDescriptor170', b1)
    if hasattr(b1, 'impactedBy'):
        assert _is_linked(b1, 'impactedBy', a)
    _safe_set(a, 'WorkProductDescriptor170', b2)
    assert _is_linked(a, 'WorkProductDescriptor170', b2)
    if hasattr(b1, 'impactedBy'):
        assert not _is_linked(b1, 'impactedBy', a)
    if hasattr(b2, 'impactedBy'):
        assert _is_linked(b2, 'impactedBy', a)
    _safe_set(a, 'WorkProductDescriptor170', None)
    assert not _is_linked(a, 'WorkProductDescriptor170', b2)
    if hasattr(b2, 'impactedBy'):
        assert not _is_linked(b2, 'impactedBy', a)


def test_assoc_interfaceIO294_link_reassign_clear():
    a = uma_WorkProductDescriptor(activityEntryState="sample_text", activityExitState="sample_text")
    b1 = uma_ProcessComponentInterface()
    b2 = uma_ProcessComponentInterface()
    _safe_set(a, 'uma_WorkProductDescriptor296', b1)
    assert _is_linked(a, 'uma_WorkProductDescriptor296', b1)
    if hasattr(b1, 'uma_ProcessComponentInterface295'):
        assert _is_linked(b1, 'uma_ProcessComponentInterface295', a)
    _safe_set(a, 'uma_WorkProductDescriptor296', b2)
    assert _is_linked(a, 'uma_WorkProductDescriptor296', b2)
    if hasattr(b1, 'uma_ProcessComponentInterface295'):
        assert not _is_linked(b1, 'uma_ProcessComponentInterface295', a)
    if hasattr(b2, 'uma_ProcessComponentInterface295'):
        assert _is_linked(b2, 'uma_ProcessComponentInterface295', a)
    _safe_set(a, 'uma_WorkProductDescriptor296', None)
    assert not _is_linked(a, 'uma_WorkProductDescriptor296', b2)
    if hasattr(b2, 'uma_ProcessComponentInterface295'):
        assert not _is_linked(b2, 'uma_ProcessComponentInterface295', a)


def test_assoc_link76_link_reassign_clear():
    a = uma_DiagramLink(zoom="sample_text")
    b1 = uma_GraphElement()
    b2 = uma_GraphElement()
    _safe_set(a, 'DiagramLink', b1)
...
def test_assoc_linkToPredecessor137_link_reassign_clear():
...
def test_assoc_mandatoryInput185_link_reassign_clear():
    a = uma_WorkProductDescriptor(activityEntryState="sample_text", activityExitState="sample_text")
    b1 = uma_TaskDescriptor()
    b2 = uma_TaskDescriptor()
    _safe_set(a, 'uma_WorkProductDescriptor187', b1)
    assert _is_linked(a, 'uma_WorkProductDescriptor187', b1)
    if hasattr(b1, 'uma_TaskDescriptor186'):
        assert _is_linked(b1, 'uma_TaskDescriptor186', a)
    _safe_set(a, 'uma_WorkProductDescriptor187', b2)
    assert _is_linked(a, 'uma_WorkProductDescriptor187', b2)
    if hasattr(b1, 'uma_TaskDescriptor186'):
        assert not _is_linked(b1, 'uma_TaskDescriptor186', a)
    if hasattr(b2, 'uma_TaskDescriptor186'):
        assert _is_linked(b2, 'uma_TaskDescriptor186', a)
    _safe_set(a, 'uma_WorkProductDescriptor187', None)
    assert not _is_linked(a, 'uma_WorkProductDescriptor187', b2)
    if hasattr(b2, 'uma_TaskDescriptor186'):
        assert not _is_linked(b2, 'uma_TaskDescriptor186', a)


def test_assoc_methodPackageSelection311_link_reassign_clear():
    a = uma_MethodPackage(global_="sample_text")
    b1 = uma_MethodConfiguration()
    b2 = uma_MethodConfiguration()
    _safe_set(a, 'uma_MethodPackage313', b1)
    assert _is_linked(a, 'uma_MethodPackage313', b1)
    if hasattr(b1, 'uma_MethodConfiguration312'):
        assert _is_linked(b1, 'uma_MethodConfiguration312', a)
    _safe_set(a, 'uma_MethodPackage313', b2)
    assert _is_linked(a, 'uma_MethodPackage313', b2)
    if hasattr(b1, 'uma_MethodConfiguration312'):
        assert not _is_linked(b1, 'uma_MethodConfiguration312', a)
    if hasattr(b2, 'uma_MethodConfiguration312'):
        assert _is_linked(b2, 'uma_MethodConfiguration312', a)
    _safe_set(a, 'uma_MethodPackage313', None)
    assert not _is_linked(a, 'uma_MethodPackage313', b2)
    if hasattr(b2, 'uma_MethodConfiguration312'):
        assert not _is_linked(b2, 'uma_MethodConfiguration312', a)


def test_assoc_methodPackages299_link_reassign_clear():
    a = uma_MethodPlugin(userChangeable="sample_text")
    b1 = uma_MethodPackage(global_="sample_text")
    b2 = uma_MethodPackage(global_="sample_text_2")
    _safe_set(a, 'uma_MethodPlugin', {b1})
    assert _is_linked(a, 'uma_MethodPlugin', b1)
    if hasattr(b1, 'uma_MethodPackage300'):
        assert _is_linked(b1, 'uma_MethodPackage300', a)
    _safe_set(a, 'uma_MethodPlugin', {b2})
    assert _is_linked(a, 'uma_MethodPlugin', b2)
    if hasattr(b1, 'uma_MethodPackage300'):
        assert not _is_linked(b1, 'uma_MethodPackage300', a)
    if hasattr(b2, 'uma_MethodPackage300'):
        assert _is_linked(b2, 'uma_MethodPackage300', a)
    _safe_set(a, 'uma_MethodPlugin', set())
    assert not _is_linked(a, 'uma_MethodPlugin', b2)
    if hasattr(b2, 'uma_MethodPackage300'):
        assert not _is_linked(b2, 'uma_MethodPackage300', a)


def test_assoc_methodPluginSelection308_link_reassign_clear():
    a = uma_MethodPlugin(userChangeable="sample_text")
    b1 = uma_MethodConfiguration()
    b2 = uma_MethodConfiguration()
    _safe_set(a, 'uma_MethodPlugin310', b1)
    assert _is_linked(a, 'uma_MethodPlugin310', b1)
    if hasattr(b1, 'uma_MethodConfiguration309'):
        assert _is_linked(b1, 'uma_MethodConfiguration309', a)
    _safe_set(a, 'uma_MethodPlugin310', b2)
    assert _is_linked(a, 'uma_MethodPlugin310', b2)
    if hasattr(b1, 'uma_MethodConfiguration309'):
        assert not _is_linked(b1, 'uma_MethodConfiguration309', a)
    if hasattr(b2, 'uma_MethodConfiguration309'):
        assert _is_linked(b2, 'uma_MethodConfiguration309', a)
    _safe_set(a, 'uma_MethodPlugin310', None)
    assert not _is_linked(a, 'uma_MethodPlugin310', b2)
    if hasattr(b2, 'uma_MethodConfiguration309'):
        assert not _is_linked(b2, 'uma_MethodConfiguration309', a)


def test_assoc_methodPlugins325_link_reassign_clear():
    a = uma_MethodPlugin(userChangeable="sample_text")
    b1 = uma_MethodLibrary()
    b2 = uma_MethodLibrary()
    _safe_set(a, 'uma_MethodPlugin326', b1)
    assert _is_linked(a, 'uma_MethodPlugin326', b1)
    if hasattr(b1, 'uma_MethodLibrary'):
        assert _is_linked(b1, 'uma_MethodLibrary', a)
    _safe_set(a, 'uma_MethodPlugin326', b2)
    assert _is_linked(a, 'uma_MethodPlugin326', b2)
    if hasattr(b1, 'uma_MethodLibrary'):
        assert not _is_linked(b1, 'uma_MethodLibrary', a)
    if hasattr(b2, 'uma_MethodLibrary'):
        assert _is_linked(b2, 'uma_MethodLibrary', a)
    _safe_set(a, 'uma_MethodPlugin326', None)
    assert not _is_linked(a, 'uma_MethodPlugin326', b2)
    if hasattr(b2, 'uma_MethodLibrary'):
        assert not _is_linked(b2, 'uma_MethodLibrary', a)


def test_assoc_modifies155_link_reassign_clear():
    a = uma_WorkProductDescriptor(activityEntryState="sample_text", activityExitState="sample_text")
    ...
    if hasattr(b1, 'uma_RoleDescriptor156'):
        assert _is_linked(b1, 'uma_RoleDescriptor156', a)
    _safe_set(a, 'uma_WorkProductDescriptor', b2)
    assert _is_linked(a, 'uma_WorkProductDescriptor', b2)
    if hasattr(b1, 'uma_RoleDescriptor156'):
        assert not _is_linked(b1, 'uma_RoleDescriptor156', a)
    if hasattr(b2, 'uma_RoleDescriptor156'):
        assert _is_linked(b2, 'uma_RoleDescriptor156', a)
    _safe_set(a, 'uma_WorkProductDescriptor', None)
    assert not _is_linked(a, 'uma_WorkProductDescriptor', b2)
    if hasattr(b2, 'uma_RoleDescriptor156'):
        assert not _is_linked(b2, 'uma_RoleDescriptor156', a)


def test_assoc_namespace106_link_reassign_clear():
    a = uma_SemanticModelBridge(presentation="sample_text")
    b1 = uma_Diagram(zoom="sample_text")
    b2 = uma_Diagram(zoom="sample_text_2")
    _safe_set(a, 'SemanticModelBridge108', b1)
    assert _is_linked(a, 'SemanticModelBridge108', b1)
    if hasattr(b1, 'diagram107'):
        assert _is_linked(b1, 'diagram107', a)
    _safe_set(a, 'SemanticModelBridge108', b2)
    assert _is_linked(a, 'SemanticModelBridge108', b2)
    if hasattr(b1, 'diagram107'):
        assert not _is_linked(b1, 'diagram107', a)
    if hasattr(b2, 'diagram107'):
        assert _is_linked(b2, 'diagram107', a)
    _safe_set(a, 'SemanticModelBridge108', None)
    assert not _is_linked(a, 'SemanticModelBridge108', b2)
    if hasattr(b2, 'diagram107'):
        assert not _is_linked(b2, 'diagram107', a)


def test_assoc_optionalInput188_link_reassign_clear():
    a = uma_WorkProductDescriptor(activityEntryState="sample_text", activityExitState="sample_text")
    b1 = uma_TaskDescriptor()
    b2 = uma_TaskDescriptor()
    _safe_set(a, 'uma_WorkProductDescriptor190', b1)
    assert _is_linked(a, 'uma_WorkProductDescriptor190', b1)
    if hasattr(b1, 'uma_TaskDescriptor189'):
        assert _is_linked(b1, 'uma_TaskDescriptor189', a)
    _safe_set(a, 'uma_WorkProductDescriptor190', b2)
    assert _is_linked(a, 'uma_WorkProductDescriptor190', b2)
    if hasattr(b1, 'uma_TaskDescriptor189'):
        assert not _is_linked(b1, 'uma_TaskDescriptor189', a)
    if hasattr(b2, 'uma_TaskDescriptor189'):
        assert _is_linked(b2, 'uma_TaskDescriptor189', a)
    _safe_set(a, 'uma_WorkProductDescriptor190', None)
    assert not _is_linked(a, 'uma_WorkProductDescriptor190', b2)
    if hasattr(b2, 'uma_TaskDescriptor189'):
        assert not _is_linked(b2, 'uma_TaskDescriptor189', a)


def test_assoc_output191_link_reassign_clear():
    a = uma_WorkProductDescriptor(activityEntryState="sample_text", activityExitState="sample_text")
    b1 = uma_TaskDescriptor()
    b2 = uma_TaskDescriptor()
    _safe_set(a, 'uma_WorkProductDescriptor193', b1)
    assert _is_linked(a, 'uma_WorkProductDescriptor193', b1)
    if hasattr(b1, 'uma_TaskDescriptor192'):
        assert _is_linked(b1, 'uma_TaskDescriptor192', a)
    _safe_set(a, 'uma_WorkProductDescriptor193', b2)
    assert _is_linked(a, 'uma_WorkProductDescriptor193', b2)
    if hasattr(b1, 'uma_TaskDescriptor192'):
        assert not _is_linked(b1, 'uma_TaskDescriptor192', a)
    if hasattr(b2, 'uma_TaskDescriptor192'):
        assert _is_linked(b2, 'uma_TaskDescriptor192', a)
    _safe_set(a, 'uma_WorkProductDescriptor193', None)
    assert not _is_linked(a, 'uma_WorkProductDescriptor193', b2)
    if hasattr(b2, 'uma_TaskDescriptor192'):
        assert not _is_linked(b2, 'uma_TaskDescriptor192', a)
...
def test_assoc_planningData143_link_reassign_clear():
...
def test_assoc_position74_link_reassign_clear():
    a = uma_Point(x="sample_text", y="sample_text")
    b1 = uma_GraphElement()
    b2 = uma_GraphElement()
...
def test_assoc_pred160_link_reassign_clear():
    a = uma_WorkOrder(linkType="sample_text")
    b1 = uma_WorkBreakdownElement(isEventDriven="sample_text", isOngoing="sample_text", isRepeatable="sample_text")
    b2 = uma_WorkBreakdownElement(isEventDriven="sample_text_2", isOngoing="sample_text_2", isRepeatable="sample_text_2")
    _safe_set(a, 'uma_WorkOrder161', b1)
    assert _is_linked(a, 'uma_WorkOrder161', b1)
    if hasattr(b1, 'uma_WorkBreakdownElement162'):
        assert _is_linked(b1, 'uma_WorkBreakdownElement162', a)
    _safe_set(a, 'uma_WorkOrder161', b2)
    assert _is_linked(a, 'uma_WorkOrder161', b2)
    if hasattr(b1, 'uma_WorkBreakdownElement162'):
        assert not _is_linked(b1, 'uma_WorkBreakdownElement162', a)
    if hasattr(b2, 'uma_WorkBreakdownElement162'):
        assert _is_linked(b2, 'uma_WorkBreakdownElement162', a)
    _safe_set(a, 'uma_WorkOrder161', None)
    assert not _is_linked(a, 'uma_WorkOrder161', b2)
    if hasattr(b2, 'uma_WorkBreakdownElement162'):
        assert not _is_linked(b2, 'uma_WorkBreakdownElement162', a)
...
def test_assoc_presentedAfter139_link_reassign_clear():
    a = uma_BreakdownElement(hasMultipleOccurrences="sample_text", isOptional="sample_text", isPlanned="sample_text", prefix="sample_text")
    b1 = uma_BreakdownElement(hasMultipleOccurrences="sample_text", isOptional="sample_text", isPlanned="sample_text", prefix="sample_text")
    b2 = uma_BreakdownElement(hasMultipleOccurrences="sample_text_2", isOptional="sample_text_2", isPlanned="sample_text_2", prefix="sample_text_2")
    _safe_set(a, 'uma_BreakdownElement', b1)
    assert _is_linked(a, 'uma_BreakdownElement', b1)
    if hasattr(b1, 'uma_BreakdownElement138'):
        assert _is_linked(b1, 'uma_BreakdownElement138', a)
    _safe_set(a, 'uma_BreakdownElement', b2)
    assert _is_linked(a, 'uma_BreakdownElement', b2)
    if hasattr(b1, 'uma_BreakdownElement138'):
        assert not _is_linked(b1, 'uma_BreakdownElement138', a)
    if hasattr(b2, 'uma_BreakdownElement138'):
        assert _is_linked(b2, 'uma_BreakdownElement138', a)
    _safe_set(a, 'uma_BreakdownElement', None)
    assert not _is_linked(a, 'uma_BreakdownElement', b2)
    if hasattr(b2, 'uma_BreakdownElement138'):
        assert not _is_linked(b2, 'uma_BreakdownElement138', a)


def test_assoc_presentedBefore141_link_reassign_clear():
    a = uma_BreakdownElement(hasMultipleOccurrences="sample_text", isOptional="sample_text", isPlanned="sample_text", prefix="sample_text")
    b1 = uma_BreakdownElement(hasMultipleOccurrences="sample_text", isOptional="sample_text", isPlanned="sample_text", prefix="sample_text")
    b2 = uma_BreakdownElement(hasMultipleOccurrences="sample_text_2", isOptional="sample_text_2", isPlanned="sample_text_2", prefix="sample_text_2")
    _safe_set(a, 'uma_BreakdownElement140', b1)
    assert _is_linked(a, 'uma_BreakdownElement140', b1)
    if hasattr(b1, 'uma_BreakdownElement142'):
        assert _is_linked(b1, 'uma_BreakdownElement142', a)
    _safe_set(a, 'uma_BreakdownElement140', b2)
    assert _is_linked(a, 'uma_BreakdownElement140', b2)
    if hasattr(b1, 'uma_BreakdownElement142'):
    ...
    _safe_set(a, 'uma_BreakdownElement140', None)
    assert not _is_linked(a, 'uma_BreakdownElement140', b2)
    if hasattr(b2, 'uma_BreakdownElement142'):
        assert not _is_linked(b2, 'uma_BreakdownElement142', a)


def test_assoc_property83_link_reassign_clear():
...
def test_assoc_reference82_link_reassign_clear():
...
def test_assoc_referenceWorkflows248_link_reassign_clear():
    a = uma_Activity(isEnactable="sample_text")
    b1 = uma_Discipline()
    b2 = uma_Discipline()
    _safe_set(a, 'uma_Activity250', b1)
    assert _is_linked(a, 'uma_Activity250', b1)
    if hasattr(b1, 'uma_Discipline249'):
        assert _is_linked(b1, 'uma_Discipline249', a)
    _safe_set(a, 'uma_Activity250', b2)
    assert _is_linked(a, 'uma_Activity250', b2)
    if hasattr(b1, 'uma_Discipline249'):
        assert not _is_linked(b1, 'uma_Discipline249', a)
    if hasattr(b2, 'uma_Discipline249'):
        assert _is_linked(b2, 'uma_Discipline249', a)
    _safe_set(a, 'uma_Activity250', None)
    assert not _is_linked(a, 'uma_Activity250', b2)
    if hasattr(b2, 'uma_Discipline249'):
        assert not _is_linked(b2, 'uma_Discipline249', a)


def test_assoc_referenced96_link_reassign_clear():
    a = uma_Reference(isIndividualRepresentation="sample_text")
    b1 = uma_DiagramElement(isVisible="sample_text")
    b2 = uma_DiagramElement(isVisible="sample_text_2")
    _safe_set(a, 'reference', b1)
    assert _is_linked(a, 'reference', b1)
    if hasattr(b1, 'DiagramElement97'):
        assert _is_linked(b1, 'DiagramElement97', a)
    _safe_set(a, 'reference', b2)
    assert _is_linked(a, 'reference', b2)
    if hasattr(b1, 'DiagramElement97'):
        assert not _is_linked(b1, 'DiagramElement97', a)
    if hasattr(b2, 'DiagramElement97'):
        assert _is_linked(b2, 'DiagramElement97', a)
    _safe_set(a, 'reference', None)
    assert not _is_linked(a, 'reference', b2)
    if hasattr(b2, 'DiagramElement97'):
        assert not _is_linked(b2, 'DiagramElement97', a)


def test_assoc_responsibleFor157_link_reassign_clear():
    a = uma_WorkProductDescriptor(activityEntryState="sample_text", activityExitState="sample_text")
    b1 = uma_RoleDescriptor()
    b2 = uma_RoleDescriptor()
    _safe_set(a, 'uma_WorkProductDescriptor159', b1)
    assert _is_linked(a, 'uma_WorkProductDescriptor159', b1)
    if hasattr(b1, 'uma_RoleDescriptor158'):
        assert _is_linked(b1, 'uma_RoleDescriptor158', a)
    _safe_set(a, 'uma_WorkProductDescriptor159', b2)
    assert _is_linked(a, 'uma_WorkProductDescriptor159', b2)
    if hasattr(b1, 'uma_RoleDescriptor158'):
        assert not _is_linked(b1, 'uma_RoleDescriptor158', a)
    if hasattr(b2, 'uma_RoleDescriptor158'):
        assert _is_linked(b2, 'uma_RoleDescriptor158', a)
    _safe_set(a, 'uma_WorkProductDescriptor159', None)
    assert not _is_linked(a, 'uma_WorkProductDescriptor159', b2)
    if hasattr(b2, 'uma_RoleDescriptor158'):
        assert not _is_linked(b2, 'uma_RoleDescriptor158', a)


def test_assoc_reusableAssets134_link_reassign_clear():
    a = uma_Activity(isEnactable="sample_text")
    b1 = uma_ReusableAsset()
    b2 = uma_ReusableAsset()
    _safe_set(a, 'uma_Activity135', {b1})
    assert _is_linked(a, 'uma_Activity135', b1)
    if hasattr(b1, 'uma_ReusableAsset136'):
        assert _is_linked(b1, 'uma_ReusableAsset136', a)
    _safe_set(a, 'uma_Activity135', {b2})
    assert _is_linked(a, 'uma_Activity135', b2)
    if hasattr(b1, 'uma_ReusableAsset136'):
        assert not _is_linked(b1, 'uma_ReusableAsset136', a)
    if hasattr(b2, 'uma_ReusableAsset136'):
        assert _is_linked(b2, 'uma_ReusableAsset136', a)
    _safe_set(a, 'uma_Activity135', set())
    assert not _is_linked(a, 'uma_Activity135', b2)
    if hasattr(b2, 'uma_ReusableAsset136'):
        assert not _is_linked(b2, 'uma_ReusableAsset136', a)
...
def test_assoc_roadmaps118_link_reassign_clear():
    a = uma_Activity(isEnactable="sample_text")
    b1 = uma_Roadmap()
    b2 = uma_Roadmap()
...
def test_assoc_selectedSteps197_link_reassign_clear():
    a = uma_Section(sectionDescription="sample_text", sectionName="sample_text")
    b1 = uma_TaskDescriptor()
    b2 = uma_TaskDescriptor()
    _safe_set(a, 'uma_Section199', b1)
    assert _is_linked(a, 'uma_Section199', b1)
    if hasattr(b1, 'uma_TaskDescriptor198'):
        assert _is_linked(b1, 'uma_TaskDescriptor198', a)
    _safe_set(a, 'uma_Section199', b2)
    assert _is_linked(a, 'uma_Section199', b2)
    if hasattr(b1, 'uma_TaskDescriptor198'):
    ...
    _safe_set(a, 'uma_Section199', None)
    assert not _is_linked(a, 'uma_Section199', b2)
    if hasattr(b2, 'uma_TaskDescriptor198'):
        assert not _is_linked(b2, 'uma_TaskDescriptor198', a)


def test_assoc_semanticModel79_link_reassign_clear():
    a = uma_SemanticModelBridge(presentation="sample_text")
    b1 = uma_GraphElement()
    b2 = uma_GraphElement()
    _safe_set(a, 'SemanticModelBridge', b1)
    assert _is_linked(a, 'SemanticModelBridge', b1)
    if hasattr(b1, 'graphElement80'):
        assert _is_linked(b1, 'graphElement80', a)
    _safe_set(a, 'SemanticModelBridge', b2)
    assert _is_linked(a, 'SemanticModelBridge', b2)
    if hasattr(b1, 'graphElement80'):
        assert not _is_linked(b1, 'graphElement80', a)
    if hasattr(b2, 'graphElement80'):
        assert _is_linked(b2, 'graphElement80', a)
    _safe_set(a, 'SemanticModelBridge', None)
    assert not _is_linked(a, 'SemanticModelBridge', b2)
    if hasattr(b2, 'graphElement80'):
        assert not _is_linked(b2, 'graphElement80', a)


def test_assoc_size109_link_reassign_clear():
...
def test_assoc_superActivities145_link_reassign_clear():
    a = uma_BreakdownElement(hasMultipleOccurrences="sample_text", isOptional="sample_text", isPlanned="sample_text", prefix="sample_text")
    b1 = uma_Activity(isEnactable="sample_text")
    b2 = uma_Activity(isEnactable="sample_text_2")
...
def test_assoc_supportingMaterials119_link_reassign_clear():
    a = uma_Activity(isEnactable="sample_text")
    b1 = uma_SupportingMaterial()
    b2 = uma_SupportingMaterial()
    _safe_set(a, 'uma_Activity120', {b1})
    assert _is_linked(a, 'uma_Activity120', b1)
    if hasattr(b1, 'uma_SupportingMaterial121'):
        assert _is_linked(b1, 'uma_SupportingMaterial121', a)
    _safe_set(a, 'uma_Activity120', {b2})
    assert _is_linked(a, 'uma_Activity120', b2)
    if hasattr(b1, 'uma_SupportingMaterial121'):
        assert not _is_linked(b1, 'uma_SupportingMaterial121', a)
    if hasattr(b2, 'uma_SupportingMaterial121'):
        assert _is_linked(b2, 'uma_SupportingMaterial121', a)
    _safe_set(a, 'uma_Activity120', set())
    assert not _is_linked(a, 'uma_Activity120', b2)
    if hasattr(b2, 'uma_SupportingMaterial121'):
        assert not _is_linked(b2, 'uma_SupportingMaterial121', a)


def test_assoc_variabilityBasedOnElement305_link_reassign_clear():
    ...
    if hasattr(b1, 'uma_VariabilityElement304'):
        assert _is_linked(b1, 'uma_VariabilityElement304', a)
    _safe_set(a, 'uma_VariabilityElement', b2)
    assert _is_linked(a, 'uma_VariabilityElement', b2)
    if hasattr(b1, 'uma_VariabilityElement304'):
        assert not _is_linked(b1, 'uma_VariabilityElement304', a)
    if hasattr(b2, 'uma_VariabilityElement304'):
        assert _is_linked(b2, 'uma_VariabilityElement304', a)
    _safe_set(a, 'uma_VariabilityElement', None)
    assert not _is_linked(a, 'uma_VariabilityElement', b2)
    if hasattr(b2, 'uma_VariabilityElement304'):
        assert not _is_linked(b2, 'uma_VariabilityElement304', a)


def test_assoc_viewpoint102_link_reassign_clear():
    a = uma_Point(x="sample_text", y="sample_text")
    b1 = uma_Diagram(zoom="sample_text")
    b2 = uma_Diagram(zoom="sample_text_2")
...
def test_assoc_viewport84_link_reassign_clear():
    a = uma_Point(x="sample_text", y="sample_text")
    b1 = uma_DiagramLink(zoom="sample_text")
    b2 = uma_DiagramLink(zoom="sample_text_2")
    _safe_set(a, 'uma_Point85', b1)
    assert _is_linked(a, 'uma_Point85', b1)
    if hasattr(b1, 'uma_DiagramLink'):
        assert _is_linked(b1, 'uma_DiagramLink', a)
    _safe_set(a, 'uma_Point85', b2)
    assert _is_linked(a, 'uma_Point85', b2)
    if hasattr(b1, 'uma_DiagramLink'):
        assert not _is_linked(b1, 'uma_DiagramLink', a)
    if hasattr(b2, 'uma_DiagramLink'):
        assert _is_linked(b2, 'uma_DiagramLink', a)
    _safe_set(a, 'uma_Point85', None)
    assert not _is_linked(a, 'uma_Point85', b2)
    if hasattr(b2, 'uma_DiagramLink'):
        assert not _is_linked(b2, 'uma_DiagramLink', a)


def test_assoc_waypoints100_link_reassign_clear():
    a = uma_Point(x="sample_text", y="sample_text")
    b1 = uma_GraphEdge()
    b2 = uma_GraphEdge()
    _safe_set(a, 'uma_Point101', b1)
    assert _is_linked(a, 'uma_Point101', b1)
    if hasattr(b1, 'uma_GraphEdge'):
        assert _is_linked(b1, 'uma_GraphEdge', a)
    _safe_set(a, 'uma_Point101', b2)
    assert _is_linked(a, 'uma_Point101', b2)
    if hasattr(b1, 'uma_GraphEdge'):
        assert not _is_linked(b1, 'uma_GraphEdge', a)
    if hasattr(b2, 'uma_GraphEdge'):
        assert _is_linked(b2, 'uma_GraphEdge', a)
    _safe_set(a, 'uma_Point101', None)
    assert not _is_linked(a, 'uma_Point101', b2)
    if hasattr(b2, 'uma_GraphEdge'):
        assert not _is_linked(b2, 'uma_GraphEdge', a)


def test_assoc_waypoints113_link_reassign_clear():
    ...
    if hasattr(b1, 'uma_Point114'):
        assert _is_linked(b1, 'uma_Point114', a)
    _safe_set(a, 'uma_Polyline', {b2})
    assert _is_linked(a, 'uma_Polyline', b2)
    if hasattr(b1, 'uma_Point114'):
        assert not _is_linked(b1, 'uma_Point114', a)
    if hasattr(b2, 'uma_Point114'):
        assert _is_linked(b2, 'uma_Point114', a)
    _safe_set(a, 'uma_Polyline', set())
    assert not _is_linked(a, 'uma_Polyline', b2)
    if hasattr(b2, 'uma_Point114'):
        assert not _is_linked(b2, 'uma_Point114', a)
