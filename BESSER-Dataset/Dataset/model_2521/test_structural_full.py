...
def test_assoc_right6_link_reassign_clear():
    ...
    b1 = Expression()
    b2 = Expression()
    _safe_set(a, 'codemodel_expressions_BinExp7', b1)
    assert _is_linked(a, 'codemodel_expressions_BinExp7', b1)
    if hasattr(b1, 'Expression8'):
        assert _is_linked(b1, 'Expression8', a)
    _safe_set(a, 'codemodel_expressions_BinExp7', b2)
    assert _is_linked(a, 'codemodel_expressions_BinExp7', b2)
    if hasattr(b1, 'Expression8'):
        assert not _is_linked(b1, 'Expression8', a)
    if hasattr(b2, 'Expression8'):
        assert _is_linked(b2, 'Expression8', a)
    _safe_set(a, 'codemodel_expressions_BinExp7', None)
    assert not _is_linked(a, 'codemodel_expressions_BinExp7', b2)
    if hasattr(b2, 'Expression8'):
