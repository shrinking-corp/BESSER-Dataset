...
def test_hyp_fsm_fsmsystem_is_not_abstract():
    assert not inspect.isabstract(fsm_FSMSystem)


def test_hyp_fsm_fsmsystem_constructor_exists():
    assert callable(fsm_FSMSystem.__init__)


def test_hyp_fsm_fsmsystem_constructor_args():
...
@settings(max_examples=30)
def test_hyp_fsm_fsmsystem_main_changes_state(instance):
    before = copy.deepcopy(instance)
    try:
        # Call operation with dummy parameters
        instance.main()
        ...
        body = tree.body[0].body  # function body
        has_statements = len(body) > 0 and not all(isinstance(stmt, ast.Pass) for stmt in body)
        assert has_statements, f"Function 'main' in fsm_FSMSystem is empty"

        # Check for state change (WARN if no change)
        if instance.__dict__ == before.__dict__:
            warnings.warn(f"Operation 'main' in fsm_FSMSystem did not change state; check implementation")

    except (AttributeError, NotImplementedError, TypeError):
        warnings.warn(f"Operation 'main' in fsm_FSMSystem is not implemented or raised an error")
...
@given(instance=fsm_FSMSystem_strategy)
@settings(max_examples=30)
def test_hyp_fsm_fsmsystem_initialize_changes_state(instance):
    before = copy.deepcopy(instance)
    try:
        # Call operation with dummy parameters
        instance.initialize(
...
@given(instance=fsm_Buffer_strategy)
@settings(max_examples=30)
def test_hyp_fsm_buffer_enqueue_changes_state(instance):
    before = copy.deepcopy(instance)
    try:
        # Call operation with dummy parameters
        instance.enqueue(
...
@given(instance=fsm_Buffer_strategy)
@settings(max_examples=30)
def test_hyp_fsm_buffer_initialize_changes_state(instance):
    before = copy.deepcopy(instance)
    try:
        # Call operation with dummy parameters
        instance.initialize()
...
@given(instance=fsm_Buffer_strategy)
@settings(max_examples=30)
def test_hyp_fsm_buffer_dequeue_changes_state(instance):
    before = copy.deepcopy(instance)
    try:
        # Call operation with dummy parameters
        instance.dequeue()
...
@given(instance=fsm_StateMachine_strategy)
@settings(max_examples=30)
def test_hyp_fsm_statemachine_run_changes_state(instance):
    before = copy.deepcopy(instance)
    try:
        # Call operation with dummy parameters
        instance.run()
...
def test_fsm_Buffer_currentValues_value_roundtrip():
    instance = fsm_Buffer(currentValues="sample_text", initialValue="sample_text")
...
def test_fsm_Buffer_isa_NamedElement():
    instance = fsm_Buffer(currentValues="sample_text", initialValue="sample_text")
    assert isinstance(instance, NamedElement)


def test_fsm_FSMSystem_isa_NamedElement():
    instance = fsm_FSMSystem()
    assert isinstance(instance, NamedElement)
...
def test_assoc_inputs19_link_reassign_clear():
    a = fsm_StateMachine(consummedString="sample_text", producedString="sample_text", unprocessedString="sample_text")
    b1 = fsm_Buffer(currentValues="sample_text", initialValue="sample_text")
    b2 = fsm_Buffer(currentValues="sample_text_2", initialValue="sample_text_2")
    _safe_set(a, 'fsm_StateMachine21', b1)
    assert _is_linked(a, 'fsm_StateMachine21', b1)
    if hasattr(b1, 'fsm_Buffer20'):
        assert _is_linked(b1, 'fsm_Buffer20', a)
    _safe_set(a, 'fsm_StateMachine21', b2)
    assert _is_linked(a, 'fsm_StateMachine21', b2)
    if hasattr(b1, 'fsm_Buffer20'):
        assert not _is_linked(b1, 'fsm_Buffer20', a)
    if hasattr(b2, 'fsm_Buffer20'):
        assert _is_linked(b2, 'fsm_Buffer20', a)
    _safe_set(a, 'fsm_StateMachine21', None)
    assert not _is_linked(a, 'fsm_StateMachine21', b2)
    if hasattr(b2, 'fsm_Buffer20'):
        assert not _is_linked(b2, 'fsm_Buffer20', a)
...
def test_assoc_outputs22_link_reassign_clear():
    a = fsm_StateMachine(consummedString="sample_text", producedString="sample_text", unprocessedString="sample_text")
    b1 = fsm_Buffer(currentValues="sample_text", initialValue="sample_text")
    b2 = fsm_Buffer(currentValues="sample_text_2", initialValue="sample_text_2")
    _safe_set(a, 'fsm_StateMachine24', b1)
    assert _is_linked(a, 'fsm_StateMachine24', b1)
    if hasattr(b1, 'fsm_Buffer23'):
        assert _is_linked(b1, 'fsm_Buffer23', a)
    _safe_set(a, 'fsm_StateMachine24', b2)
    assert _is_linked(a, 'fsm_StateMachine24', b2)
    if hasattr(b1, 'fsm_Buffer23'):
        assert not _is_linked(b1, 'fsm_Buffer23', a)
    if hasattr(b2, 'fsm_Buffer23'):
        assert _is_linked(b2, 'fsm_Buffer23', a)
    _safe_set(a, 'fsm_StateMachine24', None)
    assert not _is_linked(a, 'fsm_StateMachine24', b2)
    if hasattr(b2, 'fsm_Buffer23'):
        assert not _is_linked(b2, 'fsm_Buffer23', a)


def test_assoc_ownedBuffer17_link_reassign_clear():
    a = fsm_FSMSystem()
    b1 = fsm_Buffer(currentValues="sample_text", initialValue="sample_text")
    b2 = fsm_Buffer(currentValues="sample_text_2", initialValue="sample_text_2")
    _safe_set(a, 'fsm_FSMSystem18', {b1})
    assert _is_linked(a, 'fsm_FSMSystem18', b1)
    if hasattr(b1, 'fsm_Buffer'):
        assert _is_linked(b1, 'fsm_Buffer', a)
    _safe_set(a, 'fsm_FSMSystem18', {b2})
    assert _is_linked(a, 'fsm_FSMSystem18', b2)
    if hasattr(b1, 'fsm_Buffer'):
        assert not _is_linked(b1, 'fsm_Buffer', a)
    if hasattr(b2, 'fsm_Buffer'):
        assert _is_linked(b2, 'fsm_Buffer', a)
    _safe_set(a, 'fsm_FSMSystem18', set())
    assert not _is_linked(a, 'fsm_FSMSystem18', b2)
    if hasattr(b2, 'fsm_Buffer'):
        assert not _is_linked(b2, 'fsm_Buffer', a)


def test_assoc_ownedStateMachines15_link_reassign_clear():
    a = fsm_StateMachine(consummedString="sample_text", producedString="sample_text", unprocessedString="sample_text")
    b1 = fsm_FSMSystem()
    b2 = fsm_FSMSystem()
    _safe_set(a, 'fsm_StateMachine16', b1)
    assert _is_linked(a, 'fsm_StateMachine16', b1)
    if hasattr(b1, 'fsm_FSMSystem'):
        assert _is_linked(b1, 'fsm_FSMSystem', a)
    _safe_set(a, 'fsm_StateMachine16', b2)
    assert _is_linked(a, 'fsm_StateMachine16', b2)
    if hasattr(b1, 'fsm_FSMSystem'):
        assert not _is_linked(b1, 'fsm_FSMSystem', a)
    if hasattr(b2, 'fsm_FSMSystem'):
        assert _is_linked(b2, 'fsm_FSMSystem', a)
    _safe_set(a, 'fsm_StateMachine16', None)
    assert not _is_linked(a, 'fsm_StateMachine16', b2)
    if hasattr(b2, 'fsm_FSMSystem'):
        assert not _is_linked(b2, 'fsm_FSMSystem', a)
...
fsm_Buffer_strategy = st.builds(fsm_Buffer, currentValues=safe_text, initialValue=safe_text)
@given(instance=fsm_Buffer_strategy)
@settings(max_examples=25)
...
fsm_FSMSystem_strategy = st.builds(fsm_FSMSystem)
@given(instance=fsm_FSMSystem_strategy)
@settings(max_examples=25)
