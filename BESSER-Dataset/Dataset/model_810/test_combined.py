...
def test_fsm_Buffer_initialValue_value_roundtrip():
    instance = fsm_Buffer(initialValue="sample_text")
...
def test_assoc_inputs16_link_reassign_clear():
    a = fsm_Buffer(initialValue="sample_text")
    b1 = fsm_StateMachine()
    b2 = fsm_StateMachine()
    _safe_set(a, 'fsm_Buffer17', {b1})
    assert _is_linked(a, 'fsm_Buffer17', b1)
    if hasattr(b1, 'fsm_StateMachine18'):
        assert _is_linked(b1, 'fsm_StateMachine18', a)
    _safe_set(a, 'fsm_Buffer17', {b2})
    assert _is_linked(a, 'fsm_Buffer17', b2)
    if hasattr(b1, 'fsm_StateMachine18'):
        assert not _is_linked(b1, 'fsm_StateMachine18', a)
    if hasattr(b2, 'fsm_StateMachine18'):
        assert _is_linked(b2, 'fsm_StateMachine18', a)
    _safe_set(a, 'fsm_Buffer17', set())
    assert not _is_linked(a, 'fsm_Buffer17', b2)
    if hasattr(b2, 'fsm_StateMachine18'):
        assert not _is_linked(b2, 'fsm_StateMachine18', a)
...
def test_assoc_outputs19_link_reassign_clear():
    a = fsm_Buffer(initialValue="sample_text")
    b1 = fsm_StateMachine()
    b2 = fsm_StateMachine()
    _safe_set(a, 'fsm_Buffer20', {b1})
    assert _is_linked(a, 'fsm_Buffer20', b1)
    if hasattr(b1, 'fsm_StateMachine21'):
        assert _is_linked(b1, 'fsm_StateMachine21', a)
    _safe_set(a, 'fsm_Buffer20', {b2})
    assert _is_linked(a, 'fsm_Buffer20', b2)
    if hasattr(b1, 'fsm_StateMachine21'):
        assert not _is_linked(b1, 'fsm_StateMachine21', a)
    if hasattr(b2, 'fsm_StateMachine21'):
        assert _is_linked(b2, 'fsm_StateMachine21', a)
    _safe_set(a, 'fsm_Buffer20', set())
    assert not _is_linked(a, 'fsm_Buffer20', b2)
    if hasattr(b2, 'fsm_StateMachine21'):
        assert not _is_linked(b2, 'fsm_StateMachine21', a)


def test_assoc_ownedBuffer14_link_reassign_clear():
    a = fsm_Buffer(initialValue="sample_text")
    b1 = fsm_FSMSystem()
    b2 = fsm_FSMSystem()
    _safe_set(a, 'fsm_Buffer', b1)
    assert _is_linked(a, 'fsm_Buffer', b1)
    if hasattr(b1, 'fsm_FSMSystem15'):
        assert _is_linked(b1, 'fsm_FSMSystem15', a)
    _safe_set(a, 'fsm_Buffer', b2)
    assert _is_linked(a, 'fsm_Buffer', b2)
    if hasattr(b1, 'fsm_FSMSystem15'):
        assert not _is_linked(b1, 'fsm_FSMSystem15', a)
    if hasattr(b2, 'fsm_FSMSystem15'):
        assert _is_linked(b2, 'fsm_FSMSystem15', a)
    _safe_set(a, 'fsm_Buffer', None)
    assert not _is_linked(a, 'fsm_Buffer', b2)
    if hasattr(b2, 'fsm_FSMSystem15'):
        assert not _is_linked(b2, 'fsm_FSMSystem15', a)
...
fsm_Buffer_strategy = st.builds(fsm_Buffer, initialValue=safe_text)
@given(instance=fsm_Buffer_strategy)
@settings(max_examples=25)
