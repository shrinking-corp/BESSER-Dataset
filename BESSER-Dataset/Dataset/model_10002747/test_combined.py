...
def test_hyp______is_not_abstract():
    assert not inspect.isabstract(____)


def test_hyp______constructor_exists():
    assert callable(____.__init__)


def test_hyp______constructor_args():
    sig = inspect.signature(____.__init__)
    params = list(sig.parameters.keys())
...
_____strategy = st.builds(____)
@given(instance=_____strategy)
@settings(max_examples=25)
def test______instantiation(instance):
    assert isinstance(instance, ____)
