...
def test_library_Author_name_value_roundtrip():
    instance = library_Author(name="sample_text", surname="sample_text")
...
def test_assoc_EReference00_link_reassign_clear():
    ...
    b1 = library_Author(name="sample_text", surname="sample_text")
    b2 = library_Author(name="sample_text_2", surname="sample_text_2")
...
def test_assoc_author1_link_reassign_clear():
    ...
    b1 = library_Author(name="sample_text", surname="sample_text")
    b2 = library_Author(name="sample_text_2", surname="sample_text_2")
    _safe_set(a, 'library_Book2', b1)
    assert _is_linked(a, 'library_Book2', b1)
    if hasattr(b1, 'library_Author3'):
        assert _is_linked(b1, 'library_Author3', a)
    _safe_set(a, 'library_Book2', b2)
    assert _is_linked(a, 'library_Book2', b2)
    if hasattr(b1, 'library_Author3'):
        assert not _is_linked(b1, 'library_Author3', a)
    if hasattr(b2, 'library_Author3'):
        assert _is_linked(b2, 'library_Author3', a)
    _safe_set(a, 'library_Book2', None)
    assert not _is_linked(a, 'library_Book2', b2)
    if hasattr(b2, 'library_Author3'):
        assert not _is_linked(b2, 'library_Author3', a)


def test_assoc_listAuthor4_link_reassign_clear():
    a = library_Author(name="sample_text", surname="sample_text")
    b1 = library_Library()
    b2 = library_Library()
...
def test_assoc_listBook6_link_reassign_clear():
    a = library_Book(title="sample_text")
    b1 = library_Library()
    b2 = library_Library()
    _safe_set(a, 'library_Book8', b1)
    assert _is_linked(a, 'library_Book8', b1)
    if hasattr(b1, 'library_Library7'):
        assert _is_linked(b1, 'library_Library7', a)
    _safe_set(a, 'library_Book8', b2)
    assert _is_linked(a, 'library_Book8', b2)
    if hasattr(b1, 'library_Library7'):
        assert not _is_linked(b1, 'library_Library7', a)
    if hasattr(b2, 'library_Library7'):
        assert _is_linked(b2, 'library_Library7', a)
    _safe_set(a, 'library_Book8', None)
    assert not _is_linked(a, 'library_Book8', b2)
    if hasattr(b2, 'library_Library7'):
        assert not _is_linked(b2, 'library_Library7', a)
...
library_Author_strategy = st.builds(library_Author, name=safe_text, surname=safe_text)
