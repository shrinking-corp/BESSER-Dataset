...
def test_reply_is_not_abstract():
    assert not inspect.isabstract(reply)


def test_reply_constructor_exists():
    assert callable(reply.__init__)


def test_reply_constructor_args():
    sig = inspect.signature(reply.__init__)
...
@given(instance=reply_strategy)
@settings(max_examples=50)
def test_reply_instantiation(instance):
    assert isinstance(instance, reply)
