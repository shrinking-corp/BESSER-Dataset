...
def test_entities_entity_has_name_id():
    assert hasattr(entities_Entity, "name_id")
    descriptor = None
    for klass in entities_Entity.__mro__:
        if "name_id" in klass.__dict__:
            descriptor = klass.__dict__["name_id"]
            break
...
@given(instance=entities_Entity_strategy)
def test_entities_entity_name_id_setter(instance):
