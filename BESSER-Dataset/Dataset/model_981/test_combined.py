...
def test_hyp_henshin_unit_constructor_args():
    ...
    assert "isUsed" in params, "Missing parameter 'isUsed'"
...
def test_hyp_henshin_module_constructor_args():
    ...
    params = list(sig.parameters.keys())
    assert "nullValueMatching" in params, "Missing parameter 'nullValueMatching'"
...
@given(instance=henshin_Unit_strategy)
def test_hyp_henshin_unit_isUsed_setter(instance):
    original = instance.isUsed
    instance.isUsed = original
    assert instance.isUsed == original
...
def test_hyp_henshin_module_nullValueMatching_setter(instance):
    original = instance.nullValueMatching
    instance.nullValueMatching = original
    assert instance.nullValueMatching == original
...
def test_henshin_Module_nullValueMatching_value_roundtrip():
    instance = henshin_Module(nullValueMatching=True)
    assert instance.nullValueMatching == True
    instance.nullValueMatching = False
    assert instance.nullValueMatching == False
...
def test_henshin_Unit_activated_value_roundtrip():
    instance = henshin_Unit(activated=True, isUsed=True)
    assert instance.activated == True
    instance.activated = False
    assert instance.activated == False


def test_henshin_Unit_isUsed_value_roundtrip():
    instance = henshin_Unit(activated=True, isUsed=True)
    assert instance.isUsed == True
    instance.isUsed = False
    assert instance.isUsed == False
...
def test_assoc_else_74_link_reassign_clear():
    a = henshin_Unit(activated=True, isUsed=True)
...
def test_assoc_imports6_link_reassign_clear():
    a = henshin_Module(nullValueMatching=True)
    b1 = henshin_EPackage()
    b2 = henshin_EPackage()
...
def test_assoc_subModules2_link_reassign_clear():
    a = henshin_Module(nullValueMatching=True)
    b1 = henshin_Module(nullValueMatching=True)
    b2 = henshin_Module(nullValueMatching=False)
...
henshin_Module_strategy = st.builds(henshin_Module, nullValueMatching=st.booleans())
@given(instance=henshin_Module_strategy)
@settings(max_examples=25)
...
henshin_Unit_strategy = st.builds(henshin_Unit, activated=st.booleans(), isUsed=st.booleans())
@given(instance=henshin_Unit_strategy)
@settings(max_examples=25)
