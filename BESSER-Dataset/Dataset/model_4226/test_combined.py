...
def test_hyp_company_visitable_is_not_abstract():
    assert not inspect.isabstract(company_Visitable)


def test_hyp_company_visitable_constructor_exists():
    assert callable(company_Visitable.__init__)


def test_hyp_company_visitable_constructor_args():
...
def test_company_Company_isa_Visitable():
    instance = company_Company(name="sample_text")
    assert isinstance(instance, Visitable)


def test_company_Department_isa_Visitable():
    instance = company_Department(name="sample_text")
    assert isinstance(instance, Visitable)


def test_company_Employee_isa_Visitable():
    instance = company_Employee(address="sample_text", name="sample_text", salary=3.14)
    assert isinstance(instance, Visitable)
...
company_Visitable_strategy = st.builds(company_Visitable)
