...
def test_assoc_related12_link_reassign_clear():
    a = Company_Topic(id="sample_text")
    b1 = Company_Project(name="sample_text")
    b2 = Company_Project(name="sample_text_2")
    _safe_set(a, 'Company_Topic', b1)
    assert _is_linked(a, 'Company_Topic', b1)
    if hasattr(b1, 'Company_Project13'):
        assert _is_linked(b1, 'Company_Project13', a)
    _safe_set(a, 'Company_Topic', b2)
    assert _is_linked(a, 'Company_Topic', b2)
    if hasattr(b1, 'Company_Project13'):
        assert not _is_linked(b1, 'Company_Project13', a)
    if hasattr(b2, 'Company_Project13'):
        assert _is_linked(b2, 'Company_Project13', a)
    _safe_set(a, 'Company_Topic', None)
    assert not _is_linked(a, 'Company_Topic', b2)
    if hasattr(b2, 'Company_Project13'):
        assert not _is_linked(b2, 'Company_Project13', a)
...
def test_assoc_topics20_link_reassign_clear():
    a = Company_Topic(id="sample_text")
    b1 = Company_CompanyModel()
    b2 = Company_CompanyModel()
    _safe_set(a, 'Company_Topic22', b1)
    assert _is_linked(a, 'Company_Topic22', b1)
    if hasattr(b1, 'Company_CompanyModel21'):
        assert _is_linked(b1, 'Company_CompanyModel21', a)
    _safe_set(a, 'Company_Topic22', b2)
    assert _is_linked(a, 'Company_Topic22', b2)
    if hasattr(b1, 'Company_CompanyModel21'):
        assert not _is_linked(b1, 'Company_CompanyModel21', a)
    if hasattr(b2, 'Company_CompanyModel21'):
        assert _is_linked(b2, 'Company_CompanyModel21', a)
    _safe_set(a, 'Company_Topic22', None)
    assert not _is_linked(a, 'Company_Topic22', b2)
    if hasattr(b2, 'Company_CompanyModel21'):
        assert not _is_linked(b2, 'Company_CompanyModel21', a)
