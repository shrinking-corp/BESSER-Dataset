...
def test_hyp_department_constructor_args():
    ...
    assert "lecturer__" in params, "Missing parameter 'lecturer__'"
...
def test_hyp_department_has_lecturer__():
    assert hasattr(Department, "lecturer__")
    descriptor = None
    for klass in Department.__mro__:
        if "lecturer__" in klass.__dict__:
            descriptor = klass.__dict__["lecturer__"]
            break
    assert isinstance(descriptor, property)
...
@given(instance=Department_strategy)
def test_hyp_department_lecturer___setter(instance):
    original = instance.lecturer__
    instance.lecturer__ = original
    assert instance.lecturer__ == original
...
def test_assoc_Student_Access_Information_link_reassign_clear():
    ...
    _safe_set(a, 'Having_Attendance6', b1)
    assert _is_linked(a, 'Having_Attendance6', b1)
    if hasattr(b1, 'student7'):
        assert _is_linked(b1, 'student7', a)
    _safe_set(a, 'Having_Attendance6', b2)
    assert _is_linked(a, 'Having_Attendance6', b2)
    if hasattr(b1, 'student7'):
        assert not _is_linked(b1, 'student7', a)
    if hasattr(b2, 'student7'):
        assert _is_linked(b2, 'student7', a)
    _safe_set(a, 'Having_Attendance6', None)
    assert not _is_linked(a, 'Having_Attendance6', b2)
    if hasattr(b2, 'student7'):
