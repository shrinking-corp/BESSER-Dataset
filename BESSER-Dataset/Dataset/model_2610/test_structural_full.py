...
def test_Company_Organisation_name_value_roundtrip():
    instance = Company_Organisation(name="sample_text")
    assert instance.name == "sample_text"
    instance.name = "sample_text_2"
    assert instance.name == "sample_text_2"
...
def test_assoc_address8_link_reassign_clear():
    a = Company_Organisation(name="sample_text")
    ...
    _safe_set(a, 'Company_Organisation9', None)
...
def test_assoc_company16_link_reassign_clear():
    ...
    b1 = Company_CompanyModel()
    b2 = Company_CompanyModel()
    _safe_set(a, 'Company_Organisation17', b1)
    assert _is_linked(a, 'Company_Organisation17', b1)
    if hasattr(b1, 'Company_CompanyModel'):
        assert _is_linked(b1, 'Company_CompanyModel', a)
    _safe_set(a, 'Company_Organisation17', b2)
    assert _is_linked(a, 'Company_Organisation17', b2)
    if hasattr(b1, 'Company_CompanyModel'):
    ...
    _safe_set(a, 'Company_Organisation17', None)
    assert not _is_linked(a, 'Company_Organisation17', b2)
    if hasattr(b2, 'Company_CompanyModel'):
...
def test_assoc_lines10_link_reassign_clear():
    ...
    _safe_set(a, 'Company_Organisation11', {b1})
    assert _is_linked(a, 'Company_Organisation11', b1)
    if hasattr(b1, 'Company_Division'):
        assert _is_linked(b1, 'Company_Division', a)
    _safe_set(a, 'Company_Organisation11', {b2})
    assert _is_linked(a, 'Company_Organisation11', b2)
    if hasattr(b1, 'Company_Division'):
        assert not _is_linked(b1, 'Company_Division', a)
    if hasattr(b2, 'Company_Division'):
        assert _is_linked(b2, 'Company_Division', a)
    _safe_set(a, 'Company_Organisation11', set())
    assert not _is_linked(a, 'Company_Organisation11', b2)
    if hasattr(b2, 'Company_Division'):
...
def test_assoc_persons3_link_reassign_clear():
    a = Company_Person(firstname="sample_text", lastname="sample_text", position="sample_text")
    b1 = Company_Organisation(name="sample_text")
    b2 = Company_Organisation(name="sample_text_2")
...
Company_Organisation_strategy = st.builds(Company_Organisation, name=safe_text)
