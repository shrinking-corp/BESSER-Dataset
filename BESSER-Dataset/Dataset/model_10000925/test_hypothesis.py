...
def test_use_case_diagram_is_not_abstract():
    assert not inspect.isabstract(Use_Case_Diagram)


def test_use_case_diagram_constructor_exists():
    assert callable(Use_Case_Diagram.__init__)


def test_use_case_diagram_constructor_args():
    sig = inspect.signature(Use_Case_Diagram.__init__)
    params = list(sig.parameters.keys())
...
Use_Case_Diagram_strategy = st.builds(
    Use_Case_Diagram,
)
