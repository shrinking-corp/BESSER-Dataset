...
def test_hyp_interface_actor_is_not_abstract():
    assert not inspect.isabstract(Interface_Actor)


def test_hyp_interface_actor_constructor_exists():
    assert callable(Interface_Actor.__init__)


def test_hyp_interface_actor_constructor_args():
    sig = inspect.signature(Interface_Actor.__init__)
...
Interface_Actor_strategy = st.builds(
    Interface_Actor,
)
...
Interface_Actor_strategy = st.builds(Interface_Actor)
@given(instance=Interface_Actor_strategy)
@settings(max_examples=25)
def test_Interface_Actor_instantiation(instance):
    assert isinstance(instance, Interface_Actor)
