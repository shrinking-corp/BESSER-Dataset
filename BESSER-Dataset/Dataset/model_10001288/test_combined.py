...
def test_hyp_gmtpage_is_not_abstract():
    assert not inspect.isabstract(GMTPage)


def test_hyp_gmtpage_constructor_exists():
    assert callable(GMTPage.__init__)


def test_hyp_gmtpage_constructor_args():
    sig = inspect.signature(GMTPage.__init__)
    params = list(sig.parameters.keys())
    assert "scSession_As_Greenway_Session_SessionClient" in params, "Missing parameter 'scSession_As_Greenway_Session_SessionClient'"
...
GMTPage_strategy = st.builds(
    GMTPage,
    scSession_As_Greenway_Session_SessionClient=
        safe_text,
    balance=
        st.floats(min_value=0, max_value=1000,allow_nan=False, allow_infinity=False)
)
...
@given(instance=GMTPage_strategy)
def test_hyp_gmtpage_scSession_As_Greenway_Session_SessionClient_setter(instance):
    original = instance.scSession_As_Greenway_Session_SessionClient
    instance.scSession_As_Greenway_Session_SessionClient = original
    assert instance.scSession_As_Greenway_Session_SessionClient == original



@given(instance=GMTPage_strategy)
def test_hyp_gmtpage_balance_setter(instance):
...
def test_GMTPage_balance_value_roundtrip():
    instance = GMTPage(balance=3.14, scSession_As_Greenway_Session_SessionClient="sample_text")
    assert instance.balance == 3.14
    instance.balance = 9.99
    assert instance.balance == 9.99


def test_GMTPage_scSession_As_Greenway_Session_SessionClient_value_roundtrip():
    instance = GMTPage(balance=3.14, scSession_As_Greenway_Session_SessionClient="sample_text")
    assert instance.scSession_As_Greenway_Session_SessionClient == "sample_text"
    instance.scSession_As_Greenway_Session_SessionClient = "sample_text_2"
    assert instance.scSession_As_Greenway_Session_SessionClient == "sample_text_2"
...
GMTPage_strategy = st.builds(GMTPage, balance=st.floats(allow_nan=False, allow_infinity=False), scSession_As_Greenway_Session_SessionClient=safe_text)
@given(instance=GMTPage_strategy)
@settings(max_examples=25)
def test_GMTPage_instantiation(instance):
    assert isinstance(instance, GMTPage)
