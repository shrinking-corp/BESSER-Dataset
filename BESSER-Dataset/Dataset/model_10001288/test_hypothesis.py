...
def test_gmtpage_is_not_abstract():
    assert not inspect.isabstract(GMTPage)


def test_gmtpage_constructor_exists():
    assert callable(GMTPage.__init__)


def test_gmtpage_constructor_args():
...
def test_gmtpage_has_scSession_As_Greenway_Session_SessionClient():
    assert hasattr(GMTPage, "scSession_As_Greenway_Session_SessionClient")
    descriptor = None
    for klass in GMTPage.__mro__:
        if "scSession_As_Greenway_Session_SessionClient" in klass.__dict__:
            descriptor = klass.__dict__["scSession_As_Greenway_Session_SessionClient"]
            break
    assert isinstance(descriptor, property)

def test_gmtpage_has_balance():
    assert hasattr(GMTPage, "balance")
    descriptor = None
    for klass in GMTPage.__mro__:
...
@given(instance=GMTPage_strategy)
def test_gmtpage_scSession_As_Greenway_Session_SessionClient_setter(instance):
...
@given(instance=GMTPage_strategy)
def test_gmtpage_balance_setter(instance):
