...
def test_hyp_java_astnode_constructor_args():
    ...
    params = list(sig.parameters.keys())
    assert "used" in params, "Missing parameter 'used'"
...
def test_hyp_java_astnode_used_setter(instance):
    original = instance.used
    instance.used = original
    assert instance.used == original
...
def test_java_ASTNode_used_value_roundtrip():
    instance = java_ASTNode(used="sample_text")
    assert instance.used == "sample_text"
    instance.used = "sample_text_2"
    assert instance.used == "sample_text_2"
...
def test_assoc_comments40_link_reassign_clear():
    ...
    b1 = java_ASTNode(used="sample_text")
    b2 = java_ASTNode(used="sample_text_2")
...
def test_assoc_qualifier348_link_reassign_clear():
    a = java_ASTNode(used="sample_text")
    b1 = java_UnresolvedItemAccess()
    b2 = java_UnresolvedItemAccess()
    _safe_set(a, 'java_ASTNode350', b1)
    assert _is_linked(a, 'java_ASTNode350', b1)
    if hasattr(b1, 'java_UnresolvedItemAccess349'):
        assert _is_linked(b1, 'java_UnresolvedItemAccess349', a)
    _safe_set(a, 'java_ASTNode350', b2)
    assert _is_linked(a, 'java_ASTNode350', b2)
    if hasattr(b1, 'java_UnresolvedItemAccess349'):
        assert not _is_linked(b1, 'java_UnresolvedItemAccess349', a)
    if hasattr(b2, 'java_UnresolvedItemAccess349'):
        assert _is_linked(b2, 'java_UnresolvedItemAccess349', a)
    _safe_set(a, 'java_ASTNode350', None)
    assert not _is_linked(a, 'java_ASTNode350', b2)
    if hasattr(b2, 'java_UnresolvedItemAccess349'):
        assert not _is_linked(b2, 'java_UnresolvedItemAccess349', a)
...
java_ASTNode_strategy = st.builds(java_ASTNode, used=safe_text)
@given(instance=java_ASTNode_strategy)
@settings(max_examples=25)
