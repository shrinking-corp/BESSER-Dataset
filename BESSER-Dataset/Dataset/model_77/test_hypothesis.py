...
def test_java_astnode_has_used():
    assert hasattr(java_ASTNode, "used")
    descriptor = None
    for klass in java_ASTNode.__mro__:
        if "used" in klass.__dict__:
            descriptor = klass.__dict__["used"]
            break
    assert isinstance(descriptor, property)
...
@given(instance=java_ASTNode_strategy)
def test_java_astnode_used_setter(instance):
