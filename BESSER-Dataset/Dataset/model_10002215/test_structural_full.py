...
def test_assoc_Personne_Union2_link_reassign_clear():
    ...
    b1 = Personne()
    b2 = Personne()
    _safe_set(a, 'personnes11', {b1})
    assert _is_linked(a, 'personnes11', b1)
    if hasattr(b1, 'union10'):
        assert _is_linked(b1, 'union10', a)
    _safe_set(a, 'personnes11', {b2})
    assert _is_linked(a, 'personnes11', b2)
    if hasattr(b1, 'union10'):
        assert not _is_linked(b1, 'union10', a)
    if hasattr(b2, 'union10'):
        assert _is_linked(b2, 'union10', a)
    _safe_set(a, 'personnes11', set())
    assert not _is_linked(a, 'personnes11', b2)
    if hasattr(b2, 'union10'):
