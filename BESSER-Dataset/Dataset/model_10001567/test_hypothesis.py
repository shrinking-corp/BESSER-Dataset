...
def test_c1_has_atC2():
    assert hasattr(C1, "atC2")
    descriptor = None
    for klass in C1.__mro__:
        if "atC2" in klass.__dict__:
            descriptor = klass.__dict__["atC2"]
            break
    assert isinstance(descriptor, property)
...
@given(instance=C1_strategy)
def test_c1_atC2_setter(instance):
...
@given(instance=C_strategy)
def test_c_atC2_setter(instance):
    original = instance.atC2
    instance.atC2 = original
    assert instance.atC2 == original
