...
def test_hyp_sm_event_is_not_abstract():
    assert not inspect.isabstract(sm_Event)


def test_hyp_sm_event_constructor_exists():
    assert callable(sm_Event.__init__)


def test_hyp_sm_event_constructor_args():
...
def test_hyp_sm_event_name_setter(instance):
    original = instance.name
    instance.name = original
    assert instance.name == original
...
def test_assoc_event12_link_reassign_clear():
    a = sm_Transition(name="sample_text")
    b1 = sm_Event(name="sample_text")
    b2 = sm_Event(name="sample_text_2")
    _safe_set(a, 'sm_Transition13', b1)
    assert _is_linked(a, 'sm_Transition13', b1)
    if hasattr(b1, 'sm_Event'):
        assert _is_linked(b1, 'sm_Event', a)
    _safe_set(a, 'sm_Transition13', b2)
    assert _is_linked(a, 'sm_Transition13', b2)
    if hasattr(b1, 'sm_Event'):
        assert not _is_linked(b1, 'sm_Event', a)
    if hasattr(b2, 'sm_Event'):
        assert _is_linked(b2, 'sm_Event', a)
    _safe_set(a, 'sm_Transition13', None)
    assert not _is_linked(a, 'sm_Transition13', b2)
    if hasattr(b2, 'sm_Event'):
        assert not _is_linked(b2, 'sm_Event', a)
...
def test_assoc_source14_link_reassign_clear():
    ...
    b1 = sm_State(name="sample_text")
    b2 = sm_State(name="sample_text_2")
    _safe_set(a, 'sm_Transition15', b1)
    assert _is_linked(a, 'sm_Transition15', b1)
    if hasattr(b1, 'sm_State16'):
        assert _is_linked(b1, 'sm_State16', a)
    _safe_set(a, 'sm_Transition15', b2)
    assert _is_linked(a, 'sm_Transition15', b2)
    if hasattr(b1, 'sm_State16'):
        assert not _is_linked(b1, 'sm_State16', a)
    if hasattr(b2, 'sm_State16'):
        assert _is_linked(b2, 'sm_State16', a)
    _safe_set(a, 'sm_Transition15', None)
    assert not _is_linked(a, 'sm_Transition15', b2)
    if hasattr(b2, 'sm_State16'):
