...
def test_sr_appbean_is_not_abstract():
    assert not inspect.isabstract(SR_AppBean)


def test_sr_appbean_constructor_exists():
    assert callable(SR_AppBean.__init__)


def test_sr_appbean_constructor_args():
    sig = inspect.signature(SR_AppBean.__init__)
    params = list(sig.parameters.keys())
    assert "time" in params, "Missing parameter 'time'"

def test_sr_appbean_has_time():
    assert hasattr(SR_AppBean, "time")
    descriptor = None
    for klass in SR_AppBean.__mro__:
...
def test_qwsd_is_not_abstract():
    assert not inspect.isabstract(qwsd)


def test_qwsd_constructor_exists():
    assert callable(qwsd.__init__)


def test_qwsd_constructor_args():
    sig = inspect.signature(qwsd.__init__)
    params = list(sig.parameters.keys())
...
SR_AppBean_strategy = st.builds(
    SR_AppBean,
    time=
        st.booleans()
)
qwsd_strategy = st.builds(
    qwsd,
)
...
@given(instance=SR_AppBean_strategy)
def test_sr_appbean_time_setter(instance):
