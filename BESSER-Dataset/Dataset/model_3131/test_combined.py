...
def test_hyp_class_classmodel_is_not_abstract():
    assert not inspect.isabstract(class_ClassModel)


def test_hyp_class_classmodel_constructor_exists():
    assert callable(class_ClassModel.__init__)


def test_hyp_class_classmodel_constructor_args():
    sig = inspect.signature(class_ClassModel.__init__)
...
class_ClassModel_strategy = st.builds(
    class_ClassModel,
)
...
def test_assoc_elems5_link_reassign_clear():
    a = class_NamedElt(name="sample_text")
    b1 = class_ClassModel()
    b2 = class_ClassModel()
    _safe_set(a, 'class_NamedElt', b1)
    assert _is_linked(a, 'class_NamedElt', b1)
    if hasattr(b1, 'class_ClassModel'):
        assert _is_linked(b1, 'class_ClassModel', a)
    _safe_set(a, 'class_NamedElt', b2)
    assert _is_linked(a, 'class_NamedElt', b2)
    if hasattr(b1, 'class_ClassModel'):
        assert not _is_linked(b1, 'class_ClassModel', a)
    if hasattr(b2, 'class_ClassModel'):
        assert _is_linked(b2, 'class_ClassModel', a)
    _safe_set(a, 'class_NamedElt', None)
    assert not _is_linked(a, 'class_NamedElt', b2)
    if hasattr(b2, 'class_ClassModel'):
        assert not _is_linked(b2, 'class_ClassModel', a)
...
class_ClassModel_strategy = st.builds(class_ClassModel)
@given(instance=class_ClassModel_strategy)
@settings(max_examples=25)
def test_class_ClassModel_instantiation(instance):
    assert isinstance(instance, class_ClassModel)
