...
def test_hyp_trace_tracelink_ruleName_setter(instance):
...
def test_trace_TraceLink_ruleName_value_roundtrip():
    instance = trace_TraceLink(ruleName="sample_text")
    assert instance.ruleName == "sample_text"
    instance.ruleName = "sample_text_2"
    assert instance.ruleName == "sample_text_2"


def test_assoc_sourceElements1_link_reassign_clear():
    a = trace_TraceLink(ruleName="sample_text")
    b1 = trace_EObject()
    b2 = trace_EObject()
    _safe_set(a, 'trace_TraceLink2', {b1})
    assert _is_linked(a, 'trace_TraceLink2', b1)
    if hasattr(b1, 'trace_EObject'):
        assert _is_linked(b1, 'trace_EObject', a)
    _safe_set(a, 'trace_TraceLink2', {b2})
    assert _is_linked(a, 'trace_TraceLink2', b2)
    if hasattr(b1, 'trace_EObject'):
        assert not _is_linked(b1, 'trace_EObject', a)
    if hasattr(b2, 'trace_EObject'):
        assert _is_linked(b2, 'trace_EObject', a)
    _safe_set(a, 'trace_TraceLink2', set())
    assert not _is_linked(a, 'trace_TraceLink2', b2)
    if hasattr(b2, 'trace_EObject'):
        assert not _is_linked(b2, 'trace_EObject', a)


def test_assoc_targetElements3_link_reassign_clear():
    a = trace_TraceLink(ruleName="sample_text")
    b1 = trace_EObject()
    b2 = trace_EObject()
    _safe_set(a, 'trace_TraceLink4', {b1})
    assert _is_linked(a, 'trace_TraceLink4', b1)
    if hasattr(b1, 'trace_EObject5'):
        assert _is_linked(b1, 'trace_EObject5', a)
    _safe_set(a, 'trace_TraceLink4', {b2})
    assert _is_linked(a, 'trace_TraceLink4', b2)
    if hasattr(b1, 'trace_EObject5'):
        assert not _is_linked(b1, 'trace_EObject5', a)
    if hasattr(b2, 'trace_EObject5'):
        assert _is_linked(b2, 'trace_EObject5', a)
    _safe_set(a, 'trace_TraceLink4', set())
    assert not _is_linked(a, 'trace_TraceLink4', b2)
    if hasattr(b2, 'trace_EObject5'):
...
trace_TraceLink_strategy = st.builds(trace_TraceLink, ruleName=safe_text)
@given(instance=trace_TraceLink_strategy)
@settings(max_examples=25)
