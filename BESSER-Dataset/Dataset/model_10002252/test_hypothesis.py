...
def test_f1_is_not_abstract():
    assert not inspect.isabstract(F1)


def test_f1_constructor_exists():
    assert callable(F1.__init__)


def test_f1_constructor_args():
    sig = inspect.signature(F1.__init__)
    params = list(sig.parameters.keys())
    assert "balance" in params, "Missing parameter 'balance'"
    assert "ownerName" in params, "Missing parameter 'ownerName'"

def test_f1_has_balance():
    assert hasattr(F1, "balance")
    descriptor = None
    for klass in F1.__mro__:
        if "balance" in klass.__dict__:
            descriptor = klass.__dict__["balance"]
            break
    assert isinstance(descriptor, property)

def test_f1_has_ownerName():
    assert hasattr(F1, "ownerName")
    descriptor = None
    for klass in F1.__mro__:
...
@given(instance=F1_strategy)
def test_f1_balance_setter(instance):
...
@given(instance=F1_strategy)
def test_f1_ownerName_setter(instance):
