...
def test_hyp_a1_constructor_args():
    ...
    params = list(sig.parameters.keys())
    assert "allA" in params, "Missing parameter 'allA'"
...
def test_hyp_c1_att2_setter(instance):
...
def test_hyp_a1_allA_setter(instance):
    original = instance.allA
    instance.allA = original
    assert instance.allA == original
...
def test_A1_allA_value_roundtrip():
    instance = A1(allA="sample_text")
    assert instance.allA == "sample_text"
    instance.allA = "sample_text_2"
    assert instance.allA == "sample_text_2"
...
def test_C1_att2_value_roundtrip():
    instance = C1(att2=True, attC1=7)
...
def test_assoc_A_B2_link_reassign_clear():
    a = B1(attB=7)
    b1 = A1(allA="sample_text")
    b2 = A1(allA="sample_text_2")
    _safe_set(a, 'a7', b1)
...
A1_strategy = st.builds(A1, allA=safe_text)
@given(instance=A1_strategy)
@settings(max_examples=25)
...
C1_strategy = st.builds(C1, att2=st.booleans(), attC1=st.integers())
@given(instance=C1_strategy)
@settings(max_examples=25)
