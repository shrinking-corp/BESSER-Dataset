...
def test_assoc_events12_link_reassign_clear():
    ...
    if hasattr(b1, 'statemachine_Condition13'):
        assert _is_linked(b1, 'statemachine_Condition13', a)
    _safe_set(a, 'statemachine_Event', b2)
    assert _is_linked(a, 'statemachine_Event', b2)
    if hasattr(b1, 'statemachine_Condition13'):
        assert not _is_linked(b1, 'statemachine_Condition13', a)
    if hasattr(b2, 'statemachine_Condition13'):
        assert _is_linked(b2, 'statemachine_Condition13', a)
    _safe_set(a, 'statemachine_Event', None)
    assert not _is_linked(a, 'statemachine_Event', b2)
    if hasattr(b2, 'statemachine_Condition13'):
        assert not _is_linked(b2, 'statemachine_Condition13', a)


def test_assoc_signal14_link_reassign_clear():
    a = statemachine_Signal(name="sample_text")
    b1 = statemachine_Event(value=True)
    b2 = statemachine_Event(value=False)
    _safe_set(a, 'statemachine_Signal16', b1)
    assert _is_linked(a, 'statemachine_Signal16', b1)
    if hasattr(b1, 'statemachine_Event15'):
        assert _is_linked(b1, 'statemachine_Event15', a)
    _safe_set(a, 'statemachine_Signal16', b2)
    assert _is_linked(a, 'statemachine_Signal16', b2)
    if hasattr(b1, 'statemachine_Event15'):
        assert not _is_linked(b1, 'statemachine_Event15', a)
    if hasattr(b2, 'statemachine_Event15'):
        assert _is_linked(b2, 'statemachine_Event15', a)
    _safe_set(a, 'statemachine_Signal16', None)
    assert not _is_linked(a, 'statemachine_Signal16', b2)
    if hasattr(b2, 'statemachine_Event15'):
        assert not _is_linked(b2, 'statemachine_Event15', a)
...
def test_assoc_signal17_link_reassign_clear():
    ...
    if hasattr(b1, 'statemachine_Command18'):
        assert _is_linked(b1, 'statemachine_Command18', a)
    _safe_set(a, 'statemachine_Signal19', b2)
    assert _is_linked(a, 'statemachine_Signal19', b2)
    if hasattr(b1, 'statemachine_Command18'):
        assert not _is_linked(b1, 'statemachine_Command18', a)
    if hasattr(b2, 'statemachine_Command18'):
        assert _is_linked(b2, 'statemachine_Command18', a)
    _safe_set(a, 'statemachine_Signal19', None)
    assert not _is_linked(a, 'statemachine_Signal19', b2)
    if hasattr(b2, 'statemachine_Command18'):
        assert not _is_linked(b2, 'statemachine_Command18', a)
