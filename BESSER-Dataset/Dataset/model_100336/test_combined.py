...
def test_hyp_confidentiality_exists():
    # Check that the Enumeration exists
    assert Confidentiality is not None

def test_hyp_confidentiality_has_all_literals():
    # Collect the names of literals in this Enumeration
...
@given(instance=ptnetLoLA_TransitionExt_strategy)
def test_hyp_ptnetlola_transitionext_confidentiality_setter(instance):
...
def test_ptnetLoLA_TransitionExt_confidentiality_value_roundtrip():
    instance = ptnetLoLA_TransitionExt(confidentiality="sample_text", cost=3.14, maxTime=7, minTime=7, probability=3.14)
    assert instance.confidentiality == "sample_text"
    instance.confidentiality = "sample_text_2"
    assert instance.confidentiality == "sample_text_2"
...
ptnetLoLA_TransitionExt_strategy = st.builds(ptnetLoLA_TransitionExt, confidentiality=safe_text, cost=st.floats(allow_nan=False, allow_infinity=False), maxTime=st.integers(), minTime=st.integers(), probability=st.floats(allow_nan=False, allow_infinity=False))
@given(instance=ptnetLoLA_TransitionExt_strategy)
@settings(max_examples=25)
