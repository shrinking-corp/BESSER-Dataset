...
def test_assoc_Profile_Attendance_link_reassign_clear():
    ...
    _safe_set(a, 'Profile_Attendance_08', b1)
    assert _is_linked(a, 'Profile_Attendance_08', b1)
    if hasattr(b1, 'profile9'):
        assert _is_linked(b1, 'profile9', a)
    _safe_set(a, 'Profile_Attendance_08', b2)
    assert _is_linked(a, 'Profile_Attendance_08', b2)
    if hasattr(b1, 'profile9'):
        assert not _is_linked(b1, 'profile9', a)
    if hasattr(b2, 'profile9'):
        assert _is_linked(b2, 'profile9', a)
    _safe_set(a, 'Profile_Attendance_08', None)
    assert not _is_linked(a, 'Profile_Attendance_08', b2)
    if hasattr(b2, 'profile9'):
