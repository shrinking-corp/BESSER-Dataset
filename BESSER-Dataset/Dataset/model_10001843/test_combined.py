...
def test_hyp_cbis_is_not_abstract():
    assert not inspect.isabstract(Cbis)


def test_hyp_cbis_constructor_exists():
    assert callable(Cbis.__init__)


def test_hyp_cbis_constructor_args():
    sig = inspect.signature(Cbis.__init__)
    params = list(sig.parameters.keys())
...
def test_hyp_bbis_is_not_abstract():
    assert not inspect.isabstract(Bbis)


def test_hyp_bbis_constructor_exists():
    assert callable(Bbis.__init__)


def test_hyp_bbis_constructor_args():
    sig = inspect.signature(Bbis.__init__)
    params = list(sig.parameters.keys())
...
def test_hyp_abis_is_not_abstract():
    assert not inspect.isabstract(Abis)


def test_hyp_abis_constructor_exists():
    assert callable(Abis.__init__)


def test_hyp_abis_constructor_args():
    sig = inspect.signature(Abis.__init__)
    params = list(sig.parameters.keys())
...
Bbis_strategy = st.builds(
    Bbis,
    attB=
        st.integers()
)
Abis_strategy = st.builds(
    Abis,
    attA=
        safe_text
)
...
@given(instance=Cbis_strategy)
def test_hyp_cbis_attC1_setter(instance):
...
@given(instance=Cbis_strategy)
def test_hyp_cbis_attC2_setter(instance):
...
@given(instance=Bbis_strategy)
def test_hyp_bbis_attB_setter(instance):
...
@given(instance=Abis_strategy)
def test_hyp_abis_attA_setter(instance):
...
def test_Abis_attA_value_roundtrip():
    instance = Abis(attA="sample_text")
    assert instance.attA == "sample_text"
    instance.attA = "sample_text_2"
    assert instance.attA == "sample_text_2"
...
def test_Bbis_attB_value_roundtrip():
    instance = Bbis(attB=7)
    assert instance.attB == 7
    instance.attB = 13
    assert instance.attB == 13
...
def test_Cbis_attC1_value_roundtrip():
    instance = Cbis(attC1=7, attC2=True)
    assert instance.attC1 == 7
    instance.attC1 = 13
    assert instance.attC1 == 13


def test_Cbis_attC2_value_roundtrip():
    instance = Cbis(attC1=7, attC2=True)
    assert instance.attC2 == True
    instance.attC2 = False
    assert instance.attC2 == False
...
def test_assoc_A_B_link_reassign_clear():
    a = Bbis(attB=7)
    b1 = Abis(attA="sample_text")
    b2 = Abis(attA="sample_text_2")
    _safe_set(a, 'a5', b1)
...
def test_assoc_B_A_link_reassign_clear():
    ...
    _safe_set(a, 'a0', b1)
    assert _is_linked(a, 'a0', b1)
    if hasattr(b1, 'b1'):
        assert _is_linked(b1, 'b1', a)
    _safe_set(a, 'a0', b2)
    assert _is_linked(a, 'a0', b2)
    if hasattr(b1, 'b1'):
        assert not _is_linked(b1, 'b1', a)
    if hasattr(b2, 'b1'):
        assert _is_linked(b2, 'b1', a)
    _safe_set(a, 'a0', None)
    assert not _is_linked(a, 'a0', b2)
    if hasattr(b2, 'b1'):
        assert not _is_linked(b2, 'b1', a)
...
Abis_strategy = st.builds(Abis, attA=safe_text)
@given(instance=Abis_strategy)
@settings(max_examples=25)
def test_Abis_instantiation(instance):
    assert isinstance(instance, Abis)
...
Bbis_strategy = st.builds(Bbis, attB=st.integers())
@given(instance=Bbis_strategy)
@settings(max_examples=25)
def test_Bbis_instantiation(instance):
    assert isinstance(instance, Bbis)
...
Cbis_strategy = st.builds(Cbis, attC1=st.integers(), attC2=st.booleans())
@given(instance=Cbis_strategy)
@settings(max_examples=25)
def test_Cbis_instantiation(instance):
    assert isinstance(instance, Cbis)
