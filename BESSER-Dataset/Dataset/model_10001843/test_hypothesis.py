...
def test_cbis_is_not_abstract():
    assert not inspect.isabstract(Cbis)


def test_cbis_constructor_exists():
    assert callable(Cbis.__init__)


def test_cbis_constructor_args():
...
def test_cbis_has_attC1():
    assert hasattr(Cbis, "attC1")
    descriptor = None
    for klass in Cbis.__mro__:
        if "attC1" in klass.__dict__:
            descriptor = klass.__dict__["attC1"]
            break
    assert isinstance(descriptor, property)

def test_cbis_has_attC2():
    assert hasattr(Cbis, "attC2")
    descriptor = None
    for klass in Cbis.__mro__:
...
def test_bbis_is_not_abstract():
    assert not inspect.isabstract(Bbis)


def test_bbis_constructor_exists():
    assert callable(Bbis.__init__)


def test_bbis_constructor_args():
    sig = inspect.signature(Bbis.__init__)
    params = list(sig.parameters.keys())
    assert "attB" in params, "Missing parameter 'attB'"

def test_bbis_has_attB():
    assert hasattr(Bbis, "attB")
    descriptor = None
    for klass in Bbis.__mro__:
...
def test_abis_is_not_abstract():
    assert not inspect.isabstract(Abis)


def test_abis_constructor_exists():
    assert callable(Abis.__init__)


def test_abis_constructor_args():
    sig = inspect.signature(Abis.__init__)
    params = list(sig.parameters.keys())
    assert "attA" in params, "Missing parameter 'attA'"

def test_abis_has_attA():
    assert hasattr(Abis, "attA")
    descriptor = None
    for klass in Abis.__mro__:
...
@given(instance=Cbis_strategy)
def test_cbis_attC1_setter(instance):
...
@given(instance=Cbis_strategy)
def test_cbis_attC2_setter(instance):
...
@given(instance=Bbis_strategy)
def test_bbis_attB_setter(instance):
...
@given(instance=Abis_strategy)
def test_abis_attA_setter(instance):
