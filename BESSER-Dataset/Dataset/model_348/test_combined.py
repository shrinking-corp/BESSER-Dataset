...
def test_hyp_extlibrary_connection_is_not_abstract():
    assert not inspect.isabstract(extlibrary_Connection)


def test_hyp_extlibrary_connection_constructor_exists():
    assert callable(extlibrary_Connection.__init__)


def test_hyp_extlibrary_connection_constructor_args():
    sig = inspect.signature(extlibrary_Connection.__init__)
    params = list(sig.parameters.keys())
...
extlibrary_Connection_strategy = st.builds(
    extlibrary_Connection,
)
...
def test_assoc_author20_link_reassign_clear():
    ...
    _safe_set(a, 'extlibrary_Writer22', b1)
    assert _is_linked(a, 'extlibrary_Writer22', b1)
    if hasattr(b1, 'extlibrary_BookOnTape21'):
        assert _is_linked(b1, 'extlibrary_BookOnTape21', a)
    _safe_set(a, 'extlibrary_Writer22', b2)
    assert _is_linked(a, 'extlibrary_Writer22', b2)
    if hasattr(b1, 'extlibrary_BookOnTape21'):
        assert not _is_linked(b1, 'extlibrary_BookOnTape21', a)
    if hasattr(b2, 'extlibrary_BookOnTape21'):
        assert _is_linked(b2, 'extlibrary_BookOnTape21', a)
    _safe_set(a, 'extlibrary_Writer22', None)
    assert not _is_linked(a, 'extlibrary_Writer22', b2)
    if hasattr(b2, 'extlibrary_BookOnTape21'):
        assert not _is_linked(b2, 'extlibrary_BookOnTape21', a)


def test_assoc_books17_link_reassign_clear():
    a = extlibrary_Writer(name="sample_text")
    b1 = extlibrary_Book(category="sample_text", pages=7, title="sample_text")
    b2 = extlibrary_Book(category="sample_text_2", pages=13, title="sample_text_2")
...
def test_assoc_borrowed25_link_reassign_clear():
...
def test_assoc_cast23_link_reassign_clear():
    ...
    _safe_set(a, 'extlibrary_Person24', None)
    assert not _is_linked(a, 'extlibrary_Person24', b2)
    if hasattr(b2, 'extlibrary_VideoCassette'):
        assert not _is_linked(b2, 'extlibrary_VideoCassette', a)


def test_assoc_con15_link_reassign_clear():
    a = extlibrary_Library(name="sample_text", people="sample_text")
    b1 = extlibrary_Connection()
    b2 = extlibrary_Connection()
    _safe_set(a, 'extlibrary_Library16', {b1})
    assert _is_linked(a, 'extlibrary_Library16', b1)
    if hasattr(b1, 'extlibrary_Connection'):
        assert _is_linked(b1, 'extlibrary_Connection', a)
    _safe_set(a, 'extlibrary_Library16', {b2})
    assert _is_linked(a, 'extlibrary_Library16', b2)
    if hasattr(b1, 'extlibrary_Connection'):
        assert not _is_linked(b1, 'extlibrary_Connection', a)
    if hasattr(b2, 'extlibrary_Connection'):
        assert _is_linked(b2, 'extlibrary_Connection', a)
    _safe_set(a, 'extlibrary_Library16', set())
    assert not _is_linked(a, 'extlibrary_Library16', b2)
    if hasattr(b2, 'extlibrary_Connection'):
        assert not _is_linked(b2, 'extlibrary_Connection', a)
...
def test_assoc_reader19_link_reassign_clear():
    a = extlibrary_Person(firstName="sample_text", lastName="sample_text")
    b1 = extlibrary_BookOnTape()
    b2 = extlibrary_BookOnTape()
...
def test_assoc_src32_link_reassign_clear():
    a = extlibrary_Book(category="sample_text", pages=7, title="sample_text")
    b1 = extlibrary_Connection()
    b2 = extlibrary_Connection()
    _safe_set(a, 'extlibrary_Book34', b1)
    assert _is_linked(a, 'extlibrary_Book34', b1)
    if hasattr(b1, 'extlibrary_Connection33'):
        assert _is_linked(b1, 'extlibrary_Connection33', a)
    _safe_set(a, 'extlibrary_Book34', b2)
    assert _is_linked(a, 'extlibrary_Book34', b2)
    if hasattr(b1, 'extlibrary_Connection33'):
        assert not _is_linked(b1, 'extlibrary_Connection33', a)
    if hasattr(b2, 'extlibrary_Connection33'):
        assert _is_linked(b2, 'extlibrary_Connection33', a)
    _safe_set(a, 'extlibrary_Book34', None)
    assert not _is_linked(a, 'extlibrary_Book34', b2)
    if hasattr(b2, 'extlibrary_Connection33'):
        assert not _is_linked(b2, 'extlibrary_Connection33', a)
...
def test_assoc_tar29_link_reassign_clear():
    a = extlibrary_Book(category="sample_text", pages=7, title="sample_text")
    b1 = extlibrary_Connection()
    b2 = extlibrary_Connection()
    _safe_set(a, 'extlibrary_Book31', b1)
    assert _is_linked(a, 'extlibrary_Book31', b1)
    if hasattr(b1, 'extlibrary_Connection30'):
        assert _is_linked(b1, 'extlibrary_Connection30', a)
    _safe_set(a, 'extlibrary_Book31', b2)
    assert _is_linked(a, 'extlibrary_Book31', b2)
    if hasattr(b1, 'extlibrary_Connection30'):
        assert not _is_linked(b1, 'extlibrary_Connection30', a)
    if hasattr(b2, 'extlibrary_Connection30'):
        assert _is_linked(b2, 'extlibrary_Connection30', a)
    _safe_set(a, 'extlibrary_Book31', None)
    assert not _is_linked(a, 'extlibrary_Book31', b2)
    if hasattr(b2, 'extlibrary_Connection30'):
        assert not _is_linked(b2, 'extlibrary_Connection30', a)
...
extlibrary_Connection_strategy = st.builds(extlibrary_Connection)
@given(instance=extlibrary_Connection_strategy)
@settings(max_examples=25)
def test_extlibrary_Connection_instantiation(instance):
    assert isinstance(instance, extlibrary_Connection)
