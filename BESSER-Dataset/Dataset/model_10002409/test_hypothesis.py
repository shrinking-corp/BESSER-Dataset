...
def test_customer_constructor_args():
    ...
    params = list(sig.parameters.keys())
    assert "Booking_Date" in params, "Missing parameter 'Booking_Date'"
    assert "phoneNumber" in params, "Missing parameter 'phoneNumber'"
    assert "FirstName" in params, "Missing parameter 'FirstName'"
    assert "LeaveDate" in params, "Missing parameter 'LeaveDate'"
    assert "LastName" in params, "Missing parameter 'LastName'"

def test_customer_has_Booking_Date():
    assert hasattr(Customer, "Booking_Date")
    descriptor = None
    for klass in Customer.__mro__:
        if "Booking_Date" in klass.__dict__:
            descriptor = klass.__dict__["Booking_Date"]
            break
    assert isinstance(descriptor, property)
...
def test_customer_has_LeaveDate():
    assert hasattr(Customer, "LeaveDate")
    descriptor = None
    for klass in Customer.__mro__:
...
@given(instance=Customer_strategy)
def test_customer_Booking_Date_setter(instance):
    original = instance.Booking_Date
    instance.Booking_Date = original
    assert instance.Booking_Date == original
...
@given(instance=Customer_strategy)
def test_customer_LeaveDate_setter(instance):
