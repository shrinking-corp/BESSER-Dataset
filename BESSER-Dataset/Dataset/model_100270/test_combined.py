...
def test_hyp_ptnet_capacity_is_not_abstract():
    assert not inspect.isabstract(ptnet_Capacity)


def test_hyp_ptnet_capacity_constructor_exists():
    assert callable(ptnet_Capacity.__init__)


def test_hyp_ptnet_capacity_constructor_args():
...
def test_hyp_ptnet_arcnature_is_not_abstract():
    assert not inspect.isabstract(ptnet_ArcNature)


def test_hyp_ptnet_arcnature_constructor_exists():
    assert callable(ptnet_ArcNature.__init__)


def test_hyp_ptnet_arcnature_constructor_args():
...
def test_assoc_containerAnnotationGraphics94_link_reassign_clear():
    ...
    if hasattr(b1, 'AnnotationGraphics95'):
        assert _is_linked(b1, 'AnnotationGraphics95', a)
    _safe_set(a, 'font', b2)
    assert _is_linked(a, 'font', b2)
    if hasattr(b1, 'AnnotationGraphics95'):
        assert not _is_linked(b1, 'AnnotationGraphics95', a)
    if hasattr(b2, 'AnnotationGraphics95'):
        assert _is_linked(b2, 'AnnotationGraphics95', a)
    _safe_set(a, 'font', None)
    assert not _is_linked(a, 'font', b2)
    if hasattr(b2, 'AnnotationGraphics95'):
        assert not _is_linked(b2, 'AnnotationGraphics95', a)
...
def test_assoc_initialMarking99_link_reassign_clear():
    a = ptnet_PTMarking(text="sample_text")
    b1 = ptnet_Place()
    b2 = ptnet_Place()
...
ptnet_ArcNature_strategy = st.builds(ptnet_ArcNature)
...
ptnet_Capacity_strategy = st.builds(ptnet_Capacity)
