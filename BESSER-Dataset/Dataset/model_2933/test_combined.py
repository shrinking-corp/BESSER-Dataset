...
def test_hyp_domainmodel_limits_is_not_abstract():
    assert not inspect.isabstract(domainmodel_Limits)


def test_hyp_domainmodel_limits_constructor_exists():
    assert callable(domainmodel_Limits.__init__)


def test_hyp_domainmodel_limits_constructor_args():
...
def test_hyp_domainmodel_limits_lowerBound_setter(instance):
...
@given(instance=domainmodel_Limits_strategy)
def test_hyp_domainmodel_limits_upperBound_setter(instance):
...
def test_hyp_domainmodel_feature_required_setter(instance):
...
def test_domainmodel_Feature_many_value_roundtrip():
    instance = domainmodel_Feature(many=True, name="sample_text", required=True)
...
def test_domainmodel_Feature_required_value_roundtrip():
    instance = domainmodel_Feature(many=True, name="sample_text", required=True)
...
def test_domainmodel_Limits_lowerBound_value_roundtrip():
    instance = domainmodel_Limits(lowerBound=3.14, upperBound=3.14)
    assert instance.lowerBound == 3.14
    instance.lowerBound = 9.99
    assert instance.lowerBound == 9.99


def test_domainmodel_Limits_upperBound_value_roundtrip():
    instance = domainmodel_Limits(lowerBound=3.14, upperBound=3.14)
...
def test_assoc_features5_link_reassign_clear():
    a = domainmodel_Feature(many=True, name="sample_text", required=True)
...
def test_assoc_limit9_link_reassign_clear():
    a = domainmodel_Limits(lowerBound=3.14, upperBound=3.14)
    b1 = domainmodel_Feature(many=True, name="sample_text", required=True)
    b2 = domainmodel_Feature(many=False, name="sample_text_2", required=False)
    _safe_set(a, 'domainmodel_Limits', b1)
    assert _is_linked(a, 'domainmodel_Limits', b1)
    if hasattr(b1, 'domainmodel_Feature10'):
        assert _is_linked(b1, 'domainmodel_Feature10', a)
    _safe_set(a, 'domainmodel_Limits', b2)
    assert _is_linked(a, 'domainmodel_Limits', b2)
    if hasattr(b1, 'domainmodel_Feature10'):
    ...
    _safe_set(a, 'domainmodel_Limits', None)
    assert not _is_linked(a, 'domainmodel_Limits', b2)
    if hasattr(b2, 'domainmodel_Feature10'):
...
domainmodel_Feature_strategy = st.builds(domainmodel_Feature, many=st.booleans(), name=safe_text, required=st.booleans())
@given(instance=domainmodel_Feature_strategy)
@settings(max_examples=25)
...
domainmodel_Limits_strategy = st.builds(domainmodel_Limits, lowerBound=st.floats(allow_nan=False, allow_infinity=False), upperBound=st.floats(allow_nan=False, allow_infinity=False))
@given(instance=domainmodel_Limits_strategy)
@settings(max_examples=25)
