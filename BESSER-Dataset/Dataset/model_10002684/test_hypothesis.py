...
def test_hod_has_id():
    assert hasattr(HOD, "id")
    descriptor = None
    for klass in HOD.__mro__:
        if "id" in klass.__dict__:
            descriptor = klass.__dict__["id"]
            break
    assert isinstance(descriptor, property)

def test_hod_has_password():
    assert hasattr(HOD, "password")
    descriptor = None
    for klass in HOD.__mro__:
...
def test_counsellor_is_not_abstract():
    assert not inspect.isabstract(COUNSELLOR)


def test_counsellor_constructor_exists():
    assert callable(COUNSELLOR.__init__)


def test_counsellor_constructor_args():
    sig = inspect.signature(COUNSELLOR.__init__)
    params = list(sig.parameters.keys())
    assert "password" in params, "Missing parameter 'password'"
    assert "id" in params, "Missing parameter 'id'"

def test_counsellor_has_password():
    assert hasattr(COUNSELLOR, "password")
    descriptor = None
    for klass in COUNSELLOR.__mro__:
        if "password" in klass.__dict__:
            descriptor = klass.__dict__["password"]
            break
    assert isinstance(descriptor, property)

def test_counsellor_has_id():
    assert hasattr(COUNSELLOR, "id")
    descriptor = None
    for klass in COUNSELLOR.__mro__:
        if "id" in klass.__dict__:
            descriptor = klass.__dict__["id"]
...
@given(instance=HOD_strategy)
def test_hod_id_setter(instance):
...
@given(instance=HOD_strategy)
def test_hod_password_setter(instance):
...
@given(instance=COUNSELLOR_strategy)
def test_counsellor_password_setter(instance):
    original = instance.password
    instance.password = original
    assert instance.password == original
...
@given(instance=COUNSELLOR_strategy)
def test_counsellor_id_setter(instance):
    original = instance.id
    instance.id = original
    assert instance.id == original
