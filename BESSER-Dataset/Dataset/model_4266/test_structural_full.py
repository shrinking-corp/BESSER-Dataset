...
def test_assoc_Game2_link_reassign_clear():
...
def test_assoc_Matchup1_link_reassign_clear():
...
def test_assoc_Matchup3_link_reassign_clear():
    ...
    if hasattr(b1, 'Matchup5'):
        assert _is_linked(b1, 'Matchup5', a)
    _safe_set(a, 'Game4', b2)
    assert _is_linked(a, 'Game4', b2)
    if hasattr(b1, 'Matchup5'):
        assert not _is_linked(b1, 'Matchup5', a)
    if hasattr(b2, 'Matchup5'):
        assert _is_linked(b2, 'Matchup5', a)
    _safe_set(a, 'Game4', None)
    assert not _is_linked(a, 'Game4', b2)
    if hasattr(b2, 'Matchup5'):
        assert not _is_linked(b2, 'Matchup5', a)
...
def test_assoc_player6_link_reassign_clear():
    ...
    _safe_set(a, 'bowling_Player7', b1)
    assert _is_linked(a, 'bowling_Player7', b1)
    if hasattr(b1, 'bowling_Game'):
        assert _is_linked(b1, 'bowling_Game', a)
    _safe_set(a, 'bowling_Player7', b2)
    assert _is_linked(a, 'bowling_Player7', b2)
    if hasattr(b1, 'bowling_Game'):
        assert not _is_linked(b1, 'bowling_Game', a)
    if hasattr(b2, 'bowling_Game'):
        assert _is_linked(b2, 'bowling_Game', a)
    _safe_set(a, 'bowling_Player7', None)
    assert not _is_linked(a, 'bowling_Player7', b2)
    if hasattr(b2, 'bowling_Game'):
