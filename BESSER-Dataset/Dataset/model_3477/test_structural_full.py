...
def test_assoc_childFeatures19_link_reassign_clear():
    ...
    if hasattr(b1, 'Feature20'):
        assert _is_linked(b1, 'Feature20', a)
    _safe_set(a, 'parentGroup', {b2})
    assert _is_linked(a, 'parentGroup', b2)
    if hasattr(b1, 'Feature20'):
        assert not _is_linked(b1, 'Feature20', a)
    if hasattr(b2, 'Feature20'):
        assert _is_linked(b2, 'Feature20', a)
    _safe_set(a, 'parentGroup', set())
    assert not _is_linked(a, 'parentGroup', b2)
    if hasattr(b2, 'Feature20'):
        assert not _is_linked(b2, 'Feature20', a)
