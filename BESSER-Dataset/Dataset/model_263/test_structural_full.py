...
def test_assoc_eAnnotations46_link_reassign_clear():
...
def test_assoc_eClassifier98_link_reassign_clear():
    ...
    b1 = ecore_EGenericType()
    b2 = ecore_EGenericType()
    _safe_set(a, 'ecore_EClassifier100', b1)
    assert _is_linked(a, 'ecore_EClassifier100', b1)
    if hasattr(b1, 'ecore_EGenericType99'):
        assert _is_linked(b1, 'ecore_EGenericType99', a)
    _safe_set(a, 'ecore_EClassifier100', b2)
    assert _is_linked(a, 'ecore_EClassifier100', b2)
    if hasattr(b1, 'ecore_EGenericType99'):
    ...
    _safe_set(a, 'ecore_EClassifier100', None)
    assert not _is_linked(a, 'ecore_EClassifier100', b2)
    if hasattr(b2, 'ecore_EGenericType99'):
        assert not _is_linked(b2, 'ecore_EGenericType99', a)


def test_assoc_eClassifiers58_link_reassign_clear():
...
def test_assoc_eContainingClass47_link_reassign_clear():
    a = ecore_EOperation()
    b1 = ecore_EClass(abstract=True, interface=True)
    b2 = ecore_EClass(abstract=False, interface=False)
...
def test_assoc_eExceptions52_link_reassign_clear():
    a = ecore_EOperation()
    b1 = ecore_EClassifier(defaultValue="sample_text", instanceClass="sample_text", instanceClassName="sample_text", instanceTypeName="sample_text")
    b2 = ecore_EClassifier(defaultValue="sample_text_2", instanceClass="sample_text_2", instanceClassName="sample_text_2", instanceTypeName="sample_text_2")
    _safe_set(a, 'ecore_EOperation53', {b1})
    assert _is_linked(a, 'ecore_EOperation53', b1)
    if hasattr(b1, 'ecore_EClassifier54'):
        assert _is_linked(b1, 'ecore_EClassifier54', a)
    _safe_set(a, 'ecore_EOperation53', {b2})
    assert _is_linked(a, 'ecore_EOperation53', b2)
    if hasattr(b1, 'ecore_EClassifier54'):
        assert not _is_linked(b1, 'ecore_EClassifier54', a)
    if hasattr(b2, 'ecore_EClassifier54'):
        assert _is_linked(b2, 'ecore_EClassifier54', a)
    _safe_set(a, 'ecore_EOperation53', set())
    assert not _is_linked(a, 'ecore_EOperation53', b2)
    if hasattr(b2, 'ecore_EClassifier54'):
        assert not _is_linked(b2, 'ecore_EClassifier54', a)


def test_assoc_eGenericExceptions55_link_reassign_clear():
    a = ecore_EOperation()
    b1 = ecore_EGenericType()
    b2 = ecore_EGenericType()
    _safe_set(a, 'ecore_EOperation56', {b1})
    assert _is_linked(a, 'ecore_EOperation56', b1)
    if hasattr(b1, 'ecore_EGenericType57'):
        assert _is_linked(b1, 'ecore_EGenericType57', a)
    _safe_set(a, 'ecore_EOperation56', {b2})
    assert _is_linked(a, 'ecore_EOperation56', b2)
    if hasattr(b1, 'ecore_EGenericType57'):
        assert not _is_linked(b1, 'ecore_EGenericType57', a)
    if hasattr(b2, 'ecore_EGenericType57'):
        assert _is_linked(b2, 'ecore_EGenericType57', a)
    _safe_set(a, 'ecore_EOperation56', set())
    assert not _is_linked(a, 'ecore_EOperation56', b2)
    if hasattr(b2, 'ecore_EGenericType57'):
        assert not _is_linked(b2, 'ecore_EGenericType57', a)
...
def test_assoc_eGenericType80_link_reassign_clear():
    a = ecore_ETypedElement(lowerBound=7, many=True, ordered=True, required=True, unique=True, upperBound=7)
    b1 = ecore_EGenericType()
    b2 = ecore_EGenericType()
    _safe_set(a, 'ecore_ETypedElement81', b1)
    assert _is_linked(a, 'ecore_ETypedElement81', b1)
    if hasattr(b1, 'ecore_EGenericType82'):
        assert _is_linked(b1, 'ecore_EGenericType82', a)
    _safe_set(a, 'ecore_ETypedElement81', b2)
    assert _is_linked(a, 'ecore_ETypedElement81', b2)
    if hasattr(b1, 'ecore_EGenericType82'):
        assert not _is_linked(b1, 'ecore_EGenericType82', a)
    if hasattr(b2, 'ecore_EGenericType82'):
        assert _is_linked(b2, 'ecore_EGenericType82', a)
    _safe_set(a, 'ecore_ETypedElement81', None)
    assert not _is_linked(a, 'ecore_ETypedElement81', b2)
    if hasattr(b2, 'ecore_EGenericType82'):
        assert not _is_linked(b2, 'ecore_EGenericType82', a)
...
def test_assoc_eKeys73_link_reassign_clear():
    a = ecore_EReference(container=True, containment=True, resolveProxies=True)
    b1 = ecore_EAttribute(iD=True)
    b2 = ecore_EAttribute(iD=False)
    _safe_set(a, 'ecore_EReference74', {b1})
    assert _is_linked(a, 'ecore_EReference74', b1)
    if hasattr(b1, 'ecore_EAttribute75'):
        assert _is_linked(b1, 'ecore_EAttribute75', a)
    _safe_set(a, 'ecore_EReference74', {b2})
    assert _is_linked(a, 'ecore_EReference74', b2)
    if hasattr(b1, 'ecore_EAttribute75'):
        assert not _is_linked(b1, 'ecore_EAttribute75', a)
    if hasattr(b2, 'ecore_EAttribute75'):
        assert _is_linked(b2, 'ecore_EAttribute75', a)
    _safe_set(a, 'ecore_EReference74', set())
    assert not _is_linked(a, 'ecore_EReference74', b2)
    if hasattr(b2, 'ecore_EAttribute75'):
        assert not _is_linked(b2, 'ecore_EAttribute75', a)
...
def test_assoc_eOperation65_link_reassign_clear():
    a = ecore_EOperation()
    b1 = ecore_EParameter()
    b2 = ecore_EParameter()
    _safe_set(a, 'EOperation66', b1)
    assert _is_linked(a, 'EOperation66', b1)
    if hasattr(b1, 'eParameters'):
        assert _is_linked(b1, 'eParameters', a)
    _safe_set(a, 'EOperation66', b2)
    assert _is_linked(a, 'EOperation66', b2)
    if hasattr(b1, 'eParameters'):
        assert not _is_linked(b1, 'eParameters', a)
    if hasattr(b2, 'eParameters'):
        assert _is_linked(b2, 'eParameters', a)
    _safe_set(a, 'EOperation66', None)
    assert not _is_linked(a, 'EOperation66', b2)
    if hasattr(b2, 'eParameters'):
...
def test_assoc_eOpposite68_link_reassign_clear():
    a = ecore_EReference(container=True, containment=True, resolveProxies=True)
    ...
    _safe_set(a, 'ecore_EReference67', b1)
    assert _is_linked(a, 'ecore_EReference67', b1)
    if hasattr(b1, 'ecore_EReference69'):
        assert _is_linked(b1, 'ecore_EReference69', a)
    _safe_set(a, 'ecore_EReference67', b2)
    assert _is_linked(a, 'ecore_EReference67', b2)
    if hasattr(b1, 'ecore_EReference69'):
        assert not _is_linked(b1, 'ecore_EReference69', a)
    if hasattr(b2, 'ecore_EReference69'):
        assert _is_linked(b2, 'ecore_EReference69', a)
    _safe_set(a, 'ecore_EReference67', None)
    assert not _is_linked(a, 'ecore_EReference67', b2)
    if hasattr(b2, 'ecore_EReference69'):
        assert not _is_linked(b2, 'ecore_EReference69', a)
...
def test_assoc_eParameters51_link_reassign_clear():
    a = ecore_EOperation()
    b1 = ecore_EParameter()
    b2 = ecore_EParameter()
    _safe_set(a, 'eOperation', {b1})
...
def test_assoc_eRawType89_link_reassign_clear():
    ...
    b1 = ecore_EGenericType()
    b2 = ecore_EGenericType()
    _safe_set(a, 'ecore_EClassifier91', b1)
    assert _is_linked(a, 'ecore_EClassifier91', b1)
    if hasattr(b1, 'ecore_EGenericType90'):
        assert _is_linked(b1, 'ecore_EGenericType90', a)
    _safe_set(a, 'ecore_EClassifier91', b2)
    assert _is_linked(a, 'ecore_EClassifier91', b2)
    if hasattr(b1, 'ecore_EGenericType90'):
    ...
    _safe_set(a, 'ecore_EClassifier91', None)
    assert not _is_linked(a, 'ecore_EClassifier91', b2)
    if hasattr(b2, 'ecore_EGenericType90'):
        assert not _is_linked(b2, 'ecore_EGenericType90', a)


def test_assoc_eReferenceType70_link_reassign_clear():
    a = ecore_EReference(container=True, containment=True, resolveProxies=True)
    b1 = ecore_EClass(abstract=True, interface=True)
    b2 = ecore_EClass(abstract=False, interface=False)
    _safe_set(a, 'ecore_EReference71', b1)
    assert _is_linked(a, 'ecore_EReference71', b1)
    if hasattr(b1, 'ecore_EClass72'):
        assert _is_linked(b1, 'ecore_EClass72', a)
    _safe_set(a, 'ecore_EReference71', b2)
    assert _is_linked(a, 'ecore_EReference71', b2)
    if hasattr(b1, 'ecore_EClass72'):
        assert not _is_linked(b1, 'ecore_EClass72', a)
    if hasattr(b2, 'ecore_EClass72'):
        assert _is_linked(b2, 'ecore_EClass72', a)
    _safe_set(a, 'ecore_EReference71', None)
    assert not _is_linked(a, 'ecore_EReference71', b2)
    if hasattr(b2, 'ecore_EClass72'):
        assert not _is_linked(b2, 'ecore_EClass72', a)
...
def test_assoc_eSubpackages60_link_reassign_clear():
    a = ecore_EPackage(nsPrefix="sample_text", nsURI="sample_text")
...
def test_assoc_eSuperPackage63_link_reassign_clear():
    a = ecore_EPackage(nsPrefix="sample_text", nsURI="sample_text")
    b1 = ecore_EPackage(nsPrefix="sample_text", nsURI="sample_text")
    b2 = ecore_EPackage(nsPrefix="sample_text_2", nsURI="sample_text_2")
...
def test_assoc_eType78_link_reassign_clear():
    a = ecore_ETypedElement(lowerBound=7, many=True, ordered=True, required=True, unique=True, upperBound=7)
    b1 = ecore_EClassifier(defaultValue="sample_text", instanceClass="sample_text", instanceClassName="sample_text", instanceTypeName="sample_text")
    b2 = ecore_EClassifier(defaultValue="sample_text_2", instanceClass="sample_text_2", instanceClassName="sample_text_2", instanceTypeName="sample_text_2")
    _safe_set(a, 'ecore_ETypedElement', b1)
    assert _is_linked(a, 'ecore_ETypedElement', b1)
    if hasattr(b1, 'ecore_EClassifier79'):
        assert _is_linked(b1, 'ecore_EClassifier79', a)
    _safe_set(a, 'ecore_ETypedElement', b2)
    assert _is_linked(a, 'ecore_ETypedElement', b2)
    if hasattr(b1, 'ecore_EClassifier79'):
        assert not _is_linked(b1, 'ecore_EClassifier79', a)
    if hasattr(b2, 'ecore_EClassifier79'):
        assert _is_linked(b2, 'ecore_EClassifier79', a)
    _safe_set(a, 'ecore_ETypedElement', None)
    assert not _is_linked(a, 'ecore_ETypedElement', b2)
    if hasattr(b2, 'ecore_EClassifier79'):
        assert not _is_linked(b2, 'ecore_EClassifier79', a)
...
def test_assoc_eTypeParameters48_link_reassign_clear():
    a = ecore_EOperation()
    b1 = ecore_ETypeParameter()
    b2 = ecore_ETypeParameter()
    _safe_set(a, 'ecore_EOperation49', {b1})
    assert _is_linked(a, 'ecore_EOperation49', b1)
    if hasattr(b1, 'ecore_ETypeParameter50'):
        assert _is_linked(b1, 'ecore_ETypeParameter50', a)
    _safe_set(a, 'ecore_EOperation49', {b2})
    assert _is_linked(a, 'ecore_EOperation49', b2)
    if hasattr(b1, 'ecore_ETypeParameter50'):
        assert not _is_linked(b1, 'ecore_ETypeParameter50', a)
    if hasattr(b2, 'ecore_ETypeParameter50'):
        assert _is_linked(b2, 'ecore_ETypeParameter50', a)
    _safe_set(a, 'ecore_EOperation49', set())
    assert not _is_linked(a, 'ecore_EOperation49', b2)
    if hasattr(b2, 'ecore_ETypeParameter50'):
        assert not _is_linked(b2, 'ecore_ETypeParameter50', a)
