...
def test_hyp_simplepdl_ressourceconfig_is_not_abstract():
    assert not inspect.isabstract(simplepdl_RessourceConfig)


def test_hyp_simplepdl_ressourceconfig_constructor_exists():
    assert callable(simplepdl_RessourceConfig.__init__)


def test_hyp_simplepdl_ressourceconfig_constructor_args():
...
def test_hyp_simplepdl_ressourceinstance_is_not_abstract():
    assert not inspect.isabstract(simplepdl_RessourceInstance)


def test_hyp_simplepdl_ressourceinstance_constructor_exists():
    assert callable(simplepdl_RessourceInstance.__init__)


def test_hyp_simplepdl_ressourceinstance_constructor_args():
...
def test_hyp_simplepdl_ressourcedefinition_is_not_abstract():
    assert not inspect.isabstract(simplepdl_RessourceDefinition)


def test_hyp_simplepdl_ressourcedefinition_constructor_exists():
    assert callable(simplepdl_RessourceDefinition.__init__)


def test_hyp_simplepdl_ressourcedefinition_constructor_args():
...
def test_hyp_simplepdl_ressourceconfig_name_setter(instance):
...
def test_hyp_simplepdl_ressourceinstance_instances_setter(instance):
...
def test_hyp_simplepdl_ressourcedefinition_number_setter(instance):
...
@given(instance=simplepdl_RessourceDefinition_strategy)
def test_hyp_simplepdl_ressourcedefinition_name_setter(instance):
    original = instance.name
    instance.name = original
    assert instance.name == original
...
def test_simplepdl_RessourceConfig_name_value_roundtrip():
    instance = simplepdl_RessourceConfig(name="sample_text")
    assert instance.name == "sample_text"
    instance.name = "sample_text_2"
    assert instance.name == "sample_text_2"
...
def test_simplepdl_RessourceConfig_isa_ProcessElement():
    instance = simplepdl_RessourceConfig(name="sample_text")
    assert isinstance(instance, ProcessElement)
...
def test_assoc_activity13_link_reassign_clear():
    a = simplepdl_WorkDefinition(max_time=7, min_time=7, name="sample_text")
    b1 = simplepdl_RessourceConfig(name="sample_text")
    b2 = simplepdl_RessourceConfig(name="sample_text_2")
...
def test_assoc_config11_link_reassign_clear():
    a = simplepdl_RessourceInstance(instances=7)
    b1 = simplepdl_RessourceConfig(name="sample_text")
    b2 = simplepdl_RessourceConfig(name="sample_text_2")
    _safe_set(a, 'ressources', b1)
    assert _is_linked(a, 'ressources', b1)
    if hasattr(b1, 'RessourceConfig12'):
        assert _is_linked(b1, 'RessourceConfig12', a)
    _safe_set(a, 'ressources', b2)
    assert _is_linked(a, 'ressources', b2)
    if hasattr(b1, 'RessourceConfig12'):
        assert not _is_linked(b1, 'RessourceConfig12', a)
    if hasattr(b2, 'RessourceConfig12'):
        assert _is_linked(b2, 'RessourceConfig12', a)
    _safe_set(a, 'ressources', None)
    assert not _is_linked(a, 'ressources', b2)
    if hasattr(b2, 'RessourceConfig12'):
        assert not _is_linked(b2, 'RessourceConfig12', a)
...
def test_assoc_linksToRessources4_link_reassign_clear():
    ...
    if hasattr(b1, 'RessourceConfig'):
        assert _is_linked(b1, 'RessourceConfig', a)
    _safe_set(a, 'activity', {b2})
    assert _is_linked(a, 'activity', b2)
    if hasattr(b1, 'RessourceConfig'):
        assert not _is_linked(b1, 'RessourceConfig', a)
    if hasattr(b2, 'RessourceConfig'):
        assert _is_linked(b2, 'RessourceConfig', a)
    _safe_set(a, 'activity', set())
    assert not _is_linked(a, 'activity', b2)
    if hasattr(b2, 'RessourceConfig'):
        assert not _is_linked(b2, 'RessourceConfig', a)
...
def test_assoc_ressources15_link_reassign_clear():
    a = simplepdl_RessourceInstance(instances=7)
    b1 = simplepdl_RessourceConfig(name="sample_text")
    b2 = simplepdl_RessourceConfig(name="sample_text_2")
    _safe_set(a, 'RessourceInstance', b1)
    assert _is_linked(a, 'RessourceInstance', b1)
    if hasattr(b1, 'config'):
        assert _is_linked(b1, 'config', a)
    _safe_set(a, 'RessourceInstance', b2)
    assert _is_linked(a, 'RessourceInstance', b2)
    if hasattr(b1, 'config'):
        assert not _is_linked(b1, 'config', a)
    if hasattr(b2, 'config'):
        assert _is_linked(b2, 'config', a)
    _safe_set(a, 'RessourceInstance', None)
    assert not _is_linked(a, 'RessourceInstance', b2)
    if hasattr(b2, 'config'):
...
simplepdl_RessourceConfig_strategy = st.builds(simplepdl_RessourceConfig, name=safe_text)
@given(instance=simplepdl_RessourceConfig_strategy)
@settings(max_examples=25)
