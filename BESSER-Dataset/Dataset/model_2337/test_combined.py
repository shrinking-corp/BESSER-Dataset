...
def test_hyp_persons_persongroup_is_not_abstract():
    assert not inspect.isabstract(persons_PersonGroup)


def test_hyp_persons_persongroup_constructor_exists():
    assert callable(persons_PersonGroup.__init__)


def test_hyp_persons_persongroup_constructor_args():
...
def test_persons_Person_name_value_roundtrip():
    instance = persons_Person(name="sample_text")
...
def test_assoc_persons0_link_reassign_clear():
    a = persons_Person(name="sample_text")
    b1 = persons_PersonGroup()
    b2 = persons_PersonGroup()
    _safe_set(a, 'persons_Person', b1)
    assert _is_linked(a, 'persons_Person', b1)
    if hasattr(b1, 'persons_PersonGroup'):
        assert _is_linked(b1, 'persons_PersonGroup', a)
    _safe_set(a, 'persons_Person', b2)
    assert _is_linked(a, 'persons_Person', b2)
    if hasattr(b1, 'persons_PersonGroup'):
        assert not _is_linked(b1, 'persons_PersonGroup', a)
    if hasattr(b2, 'persons_PersonGroup'):
        assert _is_linked(b2, 'persons_PersonGroup', a)
    _safe_set(a, 'persons_Person', None)
    assert not _is_linked(a, 'persons_Person', b2)
    if hasattr(b2, 'persons_PersonGroup'):
        assert not _is_linked(b2, 'persons_PersonGroup', a)
...
persons_Person_strategy = st.builds(persons_Person, name=safe_text)
@given(instance=persons_Person_strategy)
...
persons_PersonGroup_strategy = st.builds(persons_PersonGroup)
