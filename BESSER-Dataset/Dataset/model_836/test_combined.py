...
def test_tfsm_FSMClock_numberOfTicks_value_roundtrip():
    instance = tfsm_FSMClock(numberOfTicks=True)
    assert instance.numberOfTicks == True
    instance.numberOfTicks = False
    assert instance.numberOfTicks == False
...
def test_tfsm_TemporalGuard_afterDuration_value_roundtrip():
    instance = tfsm_TemporalGuard(afterDuration="sample_text")
    assert instance.afterDuration == "sample_text"
    instance.afterDuration = "sample_text_2"
    assert instance.afterDuration == "sample_text_2"
...
def test_assoc_globalClocks33_link_reassign_clear():
    a = tfsm_TimedSystem()
    b1 = tfsm_FSMClock(numberOfTicks=True)
    b2 = tfsm_FSMClock(numberOfTicks=False)
...
def test_assoc_onClock24_link_reassign_clear():
    a = tfsm_TemporalGuard(afterDuration="sample_text")
    b1 = tfsm_FSMClock(numberOfTicks=True)
    b2 = tfsm_FSMClock(numberOfTicks=False)
...
tfsm_FSMClock_strategy = st.builds(tfsm_FSMClock, numberOfTicks=st.booleans())
@given(instance=tfsm_FSMClock_strategy)
@settings(max_examples=25)
...
tfsm_TemporalGuard_strategy = st.builds(tfsm_TemporalGuard, afterDuration=safe_text)
@given(instance=tfsm_TemporalGuard_strategy)
@settings(max_examples=25)
