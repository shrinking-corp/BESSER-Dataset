...
def test_hyp_merge_eobject_is_not_abstract():
    assert not inspect.isabstract(merge_EObject)


def test_hyp_merge_eobject_constructor_exists():
    assert callable(merge_EObject.__init__)


def test_hyp_merge_eobject_constructor_args():
    sig = inspect.signature(merge_EObject.__init__)
...
merge_EObject_strategy = st.builds(
    merge_EObject,
)
...
def test_merge_Clazz_attribute_value_roundtrip():
    instance = merge_Clazz(attribute=3.14)
    assert instance.attribute == 3.14
    instance.attribute = 9.99
    assert instance.attribute == 9.99


def test_assoc_reference0_link_reassign_clear():
    a = merge_Clazz(attribute=3.14)
    b1 = merge_EObject()
    b2 = merge_EObject()
    _safe_set(a, 'merge_Clazz', b1)
    assert _is_linked(a, 'merge_Clazz', b1)
    if hasattr(b1, 'merge_EObject'):
        assert _is_linked(b1, 'merge_EObject', a)
    _safe_set(a, 'merge_Clazz', b2)
    assert _is_linked(a, 'merge_Clazz', b2)
    if hasattr(b1, 'merge_EObject'):
        assert not _is_linked(b1, 'merge_EObject', a)
    if hasattr(b2, 'merge_EObject'):
        assert _is_linked(b2, 'merge_EObject', a)
    _safe_set(a, 'merge_Clazz', None)
    assert not _is_linked(a, 'merge_Clazz', b2)
    if hasattr(b2, 'merge_EObject'):
        assert not _is_linked(b2, 'merge_EObject', a)
...
merge_Clazz_strategy = st.builds(merge_Clazz, attribute=st.floats(allow_nan=False, allow_infinity=False))
@given(instance=merge_Clazz_strategy)
...
merge_EObject_strategy = st.builds(merge_EObject)
@given(instance=merge_EObject_strategy)
@settings(max_examples=25)
def test_merge_EObject_instantiation(instance):
    assert isinstance(instance, merge_EObject)
