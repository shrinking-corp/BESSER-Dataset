...
def test_hyp_account_myclass_is_not_abstract():
    assert not inspect.isabstract(account_MyClass)


def test_hyp_account_myclass_constructor_exists():
    assert callable(account_MyClass.__init__)


def test_hyp_account_myclass_constructor_args():
    sig = inspect.signature(account_MyClass.__init__)
...
account_MyClass_strategy = st.builds(
    account_MyClass,
)
...
account_MyClass_strategy = st.builds(account_MyClass)
@given(instance=account_MyClass_strategy)
@settings(max_examples=25)
def test_account_MyClass_instantiation(instance):
    assert isinstance(instance, account_MyClass)
