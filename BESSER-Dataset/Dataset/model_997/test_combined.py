...
def test_assoc_graph8_link_reassign_clear():
    ...
    if hasattr(b1, 'Graph9'):
        assert _is_linked(b1, 'Graph9', a)
    _safe_set(a, 'edges', b2)
    assert _is_linked(a, 'edges', b2)
    if hasattr(b1, 'Graph9'):
        assert not _is_linked(b1, 'Graph9', a)
    if hasattr(b2, 'Graph9'):
        assert _is_linked(b2, 'Graph9', a)
    _safe_set(a, 'edges', None)
    assert not _is_linked(a, 'edges', b2)
    if hasattr(b2, 'Graph9'):
        assert not _is_linked(b2, 'Graph9', a)
...
def test_assoc_trg5_link_reassign_clear():
    a = graph_Node(name="sample_text")
    b1 = graph_Edge(name="sample_text")
    b2 = graph_Edge(name="sample_text_2")
    _safe_set(a, 'graph_Node7', b1)
    assert _is_linked(a, 'graph_Node7', b1)
    if hasattr(b1, 'graph_Edge6'):
        assert _is_linked(b1, 'graph_Edge6', a)
    _safe_set(a, 'graph_Node7', b2)
    assert _is_linked(a, 'graph_Node7', b2)
    if hasattr(b1, 'graph_Edge6'):
        assert not _is_linked(b1, 'graph_Edge6', a)
    if hasattr(b2, 'graph_Edge6'):
        assert _is_linked(b2, 'graph_Edge6', a)
    _safe_set(a, 'graph_Node7', None)
    assert not _is_linked(a, 'graph_Node7', b2)
    if hasattr(b2, 'graph_Edge6'):
        assert not _is_linked(b2, 'graph_Edge6', a)
