...
def test_modify_attendance_external_is_not_abstract():
    assert not inspect.isabstract(Modify_attendance_external)


def test_modify_attendance_external_constructor_exists():
    assert callable(Modify_attendance_external.__init__)


def test_modify_attendance_external_constructor_args():
    sig = inspect.signature(Modify_attendance_external.__init__)
    params = list(sig.parameters.keys())
...
Modify_attendance_external_strategy = st.builds(
    Modify_attendance_external,
)
...
@given(instance=Modify_attendance_external_strategy)
@settings(max_examples=50)
def test_modify_attendance_external_instantiation(instance):
    assert isinstance(instance, Modify_attendance_external)
