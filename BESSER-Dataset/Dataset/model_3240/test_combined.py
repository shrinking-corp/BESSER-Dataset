...
def test_hyp_remes_writeedge_is_not_abstract():
    assert not inspect.isabstract(remes_WriteEdge)


def test_hyp_remes_writeedge_constructor_exists():
    assert callable(remes_WriteEdge.__init__)


def test_hyp_remes_writeedge_constructor_args():
...
def test_remes_Mode_initialization_value_roundtrip():
    instance = remes_Mode(initialization="sample_text")
    assert instance.initialization == "sample_text"
    instance.initialization = "sample_text_2"
    assert instance.initialization == "sample_text_2"
...
def test_remes_InitPoint_isa_Point():
    instance = remes_InitPoint()
    assert isinstance(instance, Point)


def test_remes_WritePoint_isa_Point():
...
def test_assoc_connectFrom39_link_reassign_clear():
    a = remes_Edge(actionBody="sample_text", actionGuard="sample_text")
    b1 = remes_ExitPoint()
    b2 = remes_ExitPoint()
    _safe_set(a, 'exitEdges', b1)
    assert _is_linked(a, 'exitEdges', b1)
    if hasattr(b1, 'ExitPoint40'):
        assert _is_linked(b1, 'ExitPoint40', a)
    _safe_set(a, 'exitEdges', b2)
    assert _is_linked(a, 'exitEdges', b2)
    if hasattr(b1, 'ExitPoint40'):
        assert not _is_linked(b1, 'ExitPoint40', a)
    if hasattr(b2, 'ExitPoint40'):
        assert _is_linked(b2, 'ExitPoint40', a)
    _safe_set(a, 'exitEdges', None)
    assert not _is_linked(a, 'exitEdges', b2)
    if hasattr(b2, 'ExitPoint40'):
        assert not _is_linked(b2, 'ExitPoint40', a)


def test_assoc_connectFrom45_link_reassign_clear():
    a = remes_InitEdge(initialization="sample_text")
    b1 = remes_InitPoint()
    b2 = remes_InitPoint()
    _safe_set(a, 'initEdge', b1)
    assert _is_linked(a, 'initEdge', b1)
    if hasattr(b1, 'InitPoint46'):
        assert _is_linked(b1, 'InitPoint46', a)
    _safe_set(a, 'initEdge', b2)
    assert _is_linked(a, 'initEdge', b2)
    if hasattr(b1, 'InitPoint46'):
        assert not _is_linked(b1, 'InitPoint46', a)
    if hasattr(b2, 'InitPoint46'):
        assert _is_linked(b2, 'InitPoint46', a)
    _safe_set(a, 'initEdge', None)
    assert not _is_linked(a, 'initEdge', b2)
    if hasattr(b2, 'InitPoint46'):
        assert not _is_linked(b2, 'InitPoint46', a)


def test_assoc_connectTo41_link_reassign_clear():
    a = remes_Edge(actionBody="sample_text", actionGuard="sample_text")
    b1 = remes_EntryPoint()
    b2 = remes_EntryPoint()
    _safe_set(a, 'entryEdges', b1)
    assert _is_linked(a, 'entryEdges', b1)
    if hasattr(b1, 'EntryPoint42'):
        assert _is_linked(b1, 'EntryPoint42', a)
    _safe_set(a, 'entryEdges', b2)
    assert _is_linked(a, 'entryEdges', b2)
    if hasattr(b1, 'EntryPoint42'):
        assert not _is_linked(b1, 'EntryPoint42', a)
    if hasattr(b2, 'EntryPoint42'):
        assert _is_linked(b2, 'EntryPoint42', a)
    _safe_set(a, 'entryEdges', None)
    assert not _is_linked(a, 'entryEdges', b2)
    if hasattr(b2, 'EntryPoint42'):
        assert not _is_linked(b2, 'EntryPoint42', a)


def test_assoc_connectTo47_link_reassign_clear():
    a = remes_InitEdge(initialization="sample_text")
    b1 = remes_EntryPoint()
    b2 = remes_EntryPoint()
    _safe_set(a, 'remes_InitEdge', b1)
    assert _is_linked(a, 'remes_InitEdge', b1)
    if hasattr(b1, 'remes_EntryPoint'):
        assert _is_linked(b1, 'remes_EntryPoint', a)
    _safe_set(a, 'remes_InitEdge', b2)
    assert _is_linked(a, 'remes_InitEdge', b2)
    if hasattr(b1, 'remes_EntryPoint'):
        assert not _is_linked(b1, 'remes_EntryPoint', a)
    if hasattr(b2, 'remes_EntryPoint'):
        assert _is_linked(b2, 'remes_EntryPoint', a)
    _safe_set(a, 'remes_InitEdge', None)
    assert not _is_linked(a, 'remes_InitEdge', b2)
    if hasattr(b2, 'remes_EntryPoint'):
        assert not _is_linked(b2, 'remes_EntryPoint', a)


def test_assoc_constants7_link_reassign_clear():
    a = remes_Mode(initialization="sample_text")
...
def test_assoc_container31_link_reassign_clear():
    a = remes_ControlPath(name="sample_text")
    b1 = remes_ExitPoint()
    b2 = remes_ExitPoint()
    _safe_set(a, 'ControlPath32', b1)
    assert _is_linked(a, 'ControlPath32', b1)
    if hasattr(b1, 'exitPoint'):
        assert _is_linked(b1, 'exitPoint', a)
    _safe_set(a, 'ControlPath32', b2)
    assert _is_linked(a, 'ControlPath32', b2)
    if hasattr(b1, 'exitPoint'):
        assert not _is_linked(b1, 'exitPoint', a)
    if hasattr(b2, 'exitPoint'):
        assert _is_linked(b2, 'exitPoint', a)
    _safe_set(a, 'ControlPath32', None)
    assert not _is_linked(a, 'ControlPath32', b2)
    if hasattr(b2, 'exitPoint'):
        assert not _is_linked(b2, 'exitPoint', a)


def test_assoc_entryEdges26_link_reassign_clear():
    a = remes_Edge(actionBody="sample_text", actionGuard="sample_text")
    b1 = remes_EntryPoint()
    b2 = remes_EntryPoint()
    _safe_set(a, 'Edge', b1)
...
def test_assoc_exitEdges28_link_reassign_clear():
    a = remes_Edge(actionBody="sample_text", actionGuard="sample_text")
    b1 = remes_ExitPoint()
    b2 = remes_ExitPoint()
    _safe_set(a, 'Edge30', b1)
    assert _is_linked(a, 'Edge30', b1)
    if hasattr(b1, 'connectFrom29'):
        assert _is_linked(b1, 'connectFrom29', a)
    _safe_set(a, 'Edge30', b2)
    assert _is_linked(a, 'Edge30', b2)
    if hasattr(b1, 'connectFrom29'):
        assert not _is_linked(b1, 'connectFrom29', a)
    if hasattr(b2, 'connectFrom29'):
        assert _is_linked(b2, 'connectFrom29', a)
    _safe_set(a, 'Edge30', None)
    assert not _is_linked(a, 'Edge30', b2)
    if hasattr(b2, 'connectFrom29'):
        assert not _is_linked(b2, 'connectFrom29', a)
...
def test_assoc_initEdge23_link_reassign_clear():
    a = remes_InitEdge(initialization="sample_text")
    b1 = remes_InitPoint()
    b2 = remes_InitPoint()
    _safe_set(a, 'InitEdge', b1)
...
def test_assoc_parsedActionBody43_link_reassign_clear():
    a = remes_Edge(actionBody="sample_text", actionGuard="sample_text")
    b1 = ActionRoot()
    b2 = ActionRoot()
    _safe_set(a, 'remes_Edge44', b1)
    assert _is_linked(a, 'remes_Edge44', b1)
    if hasattr(b1, 'ActionRoot'):
        assert _is_linked(b1, 'ActionRoot', a)
    _safe_set(a, 'remes_Edge44', b2)
    assert _is_linked(a, 'remes_Edge44', b2)
    if hasattr(b1, 'ActionRoot'):
        assert not _is_linked(b1, 'ActionRoot', a)
    if hasattr(b2, 'ActionRoot'):
        assert _is_linked(b2, 'ActionRoot', a)
    _safe_set(a, 'remes_Edge44', None)
    assert not _is_linked(a, 'remes_Edge44', b2)
    if hasattr(b2, 'ActionRoot'):
        assert not _is_linked(b2, 'ActionRoot', a)


def test_assoc_parsedActionGuard37_link_reassign_clear():
    a = remes_Edge(actionBody="sample_text", actionGuard="sample_text")
    b1 = LogicalRoot()
    b2 = LogicalRoot()
    _safe_set(a, 'remes_Edge', b1)
    assert _is_linked(a, 'remes_Edge', b1)
    if hasattr(b1, 'LogicalRoot38'):
        assert _is_linked(b1, 'LogicalRoot38', a)
    _safe_set(a, 'remes_Edge', b2)
    assert _is_linked(a, 'remes_Edge', b2)
    if hasattr(b1, 'LogicalRoot38'):
        assert not _is_linked(b1, 'LogicalRoot38', a)
    if hasattr(b2, 'LogicalRoot38'):
        assert _is_linked(b2, 'LogicalRoot38', a)
    _safe_set(a, 'remes_Edge', None)
    assert not _is_linked(a, 'remes_Edge', b2)
    if hasattr(b2, 'LogicalRoot38'):
        assert not _is_linked(b2, 'LogicalRoot38', a)


def test_assoc_parsedExpression54_link_reassign_clear():
...
def test_assoc_parsedInitialization48_link_reassign_clear():
    a = remes_InitEdge(initialization="sample_text")
    b1 = ActionRoot()
    b2 = ActionRoot()
    _safe_set(a, 'remes_InitEdge49', b1)
    assert _is_linked(a, 'remes_InitEdge49', b1)
    if hasattr(b1, 'ActionRoot50'):
        assert _is_linked(b1, 'ActionRoot50', a)
    _safe_set(a, 'remes_InitEdge49', b2)
    assert _is_linked(a, 'remes_InitEdge49', b2)
    if hasattr(b1, 'ActionRoot50'):
        assert not _is_linked(b1, 'ActionRoot50', a)
    if hasattr(b2, 'ActionRoot50'):
        assert _is_linked(b2, 'ActionRoot50', a)
    _safe_set(a, 'remes_InitEdge49', None)
    assert not _is_linked(a, 'remes_InitEdge49', b2)
    if hasattr(b2, 'ActionRoot50'):
        assert not _is_linked(b2, 'ActionRoot50', a)


def test_assoc_parsedInvariant20_link_reassign_clear():
    a = remes_SubMode(invariant="sample_text", isUrgent=True)
    b1 = LogicalRoot()
    b2 = LogicalRoot()
...
def test_assoc_resources5_link_reassign_clear():
    a = remes_Resource(expression="sample_text", type="sample_text")
    b1 = remes_Mode(initialization="sample_text")
    b2 = remes_Mode(initialization="sample_text_2")
...
def test_assoc_scope51_link_reassign_clear():
...
def test_assoc_scope52_link_reassign_clear():
    a = remes_Resource(expression="sample_text", type="sample_text")
    b1 = remes_Mode(initialization="sample_text")
    b2 = remes_Mode(initialization="sample_text_2")
    _safe_set(a, 'resources', b1)
    assert _is_linked(a, 'resources', b1)
    if hasattr(b1, 'Mode53'):
        assert _is_linked(b1, 'Mode53', a)
    _safe_set(a, 'resources', b2)
    assert _is_linked(a, 'resources', b2)
    if hasattr(b1, 'Mode53'):
        assert not _is_linked(b1, 'Mode53', a)
    if hasattr(b2, 'Mode53'):
        assert _is_linked(b2, 'Mode53', a)
    _safe_set(a, 'resources', None)
    assert not _is_linked(a, 'resources', b2)
    if hasattr(b2, 'Mode53'):
        assert not _is_linked(b2, 'Mode53', a)


def test_assoc_scope55_link_reassign_clear():
    a = remes_Mode(initialization="sample_text")
    b1 = remes_Constant(global_=True, type="sample_text", value="sample_text")
    b2 = remes_Constant(global_=False, type="sample_text_2", value="sample_text_2")
    _safe_set(a, 'Mode56', b1)
    assert _is_linked(a, 'Mode56', b1)
    if hasattr(b1, 'constants'):
        assert _is_linked(b1, 'constants', a)
    _safe_set(a, 'Mode56', b2)
    assert _is_linked(a, 'Mode56', b2)
    if hasattr(b1, 'constants'):
        assert not _is_linked(b1, 'constants', a)
    if hasattr(b2, 'constants'):
        assert _is_linked(b2, 'constants', a)
    _safe_set(a, 'Mode56', None)
    assert not _is_linked(a, 'Mode56', b2)
    if hasattr(b2, 'constants'):
...
remes_Mode_strategy = st.builds(remes_Mode, initialization=safe_text)
@given(instance=remes_Mode_strategy)
@settings(max_examples=25)
...
remes_WriteEdge_strategy = st.builds(remes_WriteEdge)
