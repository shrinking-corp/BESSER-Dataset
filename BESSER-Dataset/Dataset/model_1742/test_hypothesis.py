...
def test_model_actor_is_not_abstract():
    assert not inspect.isabstract(model_Actor)


def test_model_actor_constructor_exists():
    assert callable(model_Actor.__init__)


def test_model_actor_constructor_args():
    sig = inspect.signature(model_Actor.__init__)
    params = list(sig.parameters.keys())
    assert "name" in params, "Missing parameter 'name'"
    assert "id" in params, "Missing parameter 'id'"

def test_model_actor_has_name():
    assert hasattr(model_Actor, "name")
    descriptor = None
    for klass in model_Actor.__mro__:
        if "name" in klass.__dict__:
            descriptor = klass.__dict__["name"]
            break
    assert isinstance(descriptor, property)

def test_model_actor_has_id():
    assert hasattr(model_Actor, "id")
    descriptor = None
    for klass in model_Actor.__mro__:
...
def test_model_movie_is_not_abstract():
    assert not inspect.isabstract(model_Movie)


def test_model_movie_constructor_exists():
    assert callable(model_Movie.__init__)


def test_model_movie_constructor_args():
    sig = inspect.signature(model_Movie.__init__)
    params = list(sig.parameters.keys())
    assert "id" in params, "Missing parameter 'id'"
    assert "title" in params, "Missing parameter 'title'"

def test_model_movie_has_id():
    assert hasattr(model_Movie, "id")
    descriptor = None
    for klass in model_Movie.__mro__:
        if "id" in klass.__dict__:
            descriptor = klass.__dict__["id"]
            break
    assert isinstance(descriptor, property)

def test_model_movie_has_title():
    assert hasattr(model_Movie, "title")
    descriptor = None
    for klass in model_Movie.__mro__:
...
@given(instance=model_Actor_strategy)
def test_model_actor_name_setter(instance):
...
@given(instance=model_Actor_strategy)
def test_model_actor_id_setter(instance):
...
@given(instance=model_Movie_strategy)
def test_model_movie_id_setter(instance):
    original = instance.id
    instance.id = original
    assert instance.id == original
...
@given(instance=model_Movie_strategy)
def test_model_movie_title_setter(instance):
