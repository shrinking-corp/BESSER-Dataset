...
def test_hyp_railway_route_active_setter(instance):
...
def test_hyp_railway_railwayelement_id_setter(instance):
...
def test_railway_RailwayElement_id_value_roundtrip():
    instance = railway_RailwayElement(id=7)
    assert instance.id == 7
    instance.id = 13
    assert instance.id == 13


def test_railway_Route_active_value_roundtrip():
    instance = railway_Route(active=True)
...
def test_assoc_entry11_link_reassign_clear():
    a = railway_Semaphore(signal="sample_text")
    b1 = railway_Route(active=True)
    b2 = railway_Route(active=False)
    _safe_set(a, 'railway_Semaphore', b1)
    assert _is_linked(a, 'railway_Semaphore', b1)
    if hasattr(b1, 'railway_Route12'):
        assert _is_linked(b1, 'railway_Route12', a)
    _safe_set(a, 'railway_Semaphore', b2)
    assert _is_linked(a, 'railway_Semaphore', b2)
    if hasattr(b1, 'railway_Route12'):
        assert not _is_linked(b1, 'railway_Route12', a)
    if hasattr(b2, 'railway_Route12'):
        assert _is_linked(b2, 'railway_Route12', a)
    _safe_set(a, 'railway_Semaphore', None)
    assert not _is_linked(a, 'railway_Semaphore', b2)
    if hasattr(b2, 'railway_Route12'):
        assert not _is_linked(b2, 'railway_Route12', a)


def test_assoc_exit13_link_reassign_clear():
    a = railway_Semaphore(signal="sample_text")
    b1 = railway_Route(active=True)
    b2 = railway_Route(active=False)
    _safe_set(a, 'railway_Semaphore15', b1)
    assert _is_linked(a, 'railway_Semaphore15', b1)
    if hasattr(b1, 'railway_Route14'):
        assert _is_linked(b1, 'railway_Route14', a)
    _safe_set(a, 'railway_Semaphore15', b2)
    assert _is_linked(a, 'railway_Semaphore15', b2)
    if hasattr(b1, 'railway_Route14'):
        assert not _is_linked(b1, 'railway_Route14', a)
    if hasattr(b2, 'railway_Route14'):
        assert _is_linked(b2, 'railway_Route14', a)
    _safe_set(a, 'railway_Semaphore15', None)
    assert not _is_linked(a, 'railway_Semaphore15', b2)
    if hasattr(b2, 'railway_Route14'):
        assert not _is_linked(b2, 'railway_Route14', a)


def test_assoc_follows7_link_reassign_clear():
...
def test_assoc_positions23_link_reassign_clear():
    a = railway_SwitchPosition(position="sample_text")
    b1 = railway_Switch(currentPosition="sample_text")
    b2 = railway_Switch(currentPosition="sample_text_2")
    _safe_set(a, 'SwitchPosition24', b1)
    assert _is_linked(a, 'SwitchPosition24', b1)
    if hasattr(b1, 'target'):
        assert _is_linked(b1, 'target', a)
    _safe_set(a, 'SwitchPosition24', b2)
    assert _is_linked(a, 'SwitchPosition24', b2)
    if hasattr(b1, 'target'):
        assert not _is_linked(b1, 'target', a)
    if hasattr(b2, 'target'):
        assert _is_linked(b2, 'target', a)
    _safe_set(a, 'SwitchPosition24', None)
    assert not _is_linked(a, 'SwitchPosition24', b2)
    if hasattr(b2, 'target'):
        assert not _is_linked(b2, 'target', a)


def test_assoc_requires8_link_reassign_clear():
    a = railway_Route(active=True)
    b1 = railway_Sensor()
    b2 = railway_Sensor()
    _safe_set(a, 'railway_Route9', {b1})
    assert _is_linked(a, 'railway_Route9', b1)
    if hasattr(b1, 'railway_Sensor10'):
        assert _is_linked(b1, 'railway_Sensor10', a)
    _safe_set(a, 'railway_Route9', {b2})
    assert _is_linked(a, 'railway_Route9', b2)
    if hasattr(b1, 'railway_Sensor10'):
        assert not _is_linked(b1, 'railway_Sensor10', a)
    if hasattr(b2, 'railway_Sensor10'):
        assert _is_linked(b2, 'railway_Sensor10', a)
    _safe_set(a, 'railway_Route9', set())
    assert not _is_linked(a, 'railway_Route9', b2)
    if hasattr(b2, 'railway_Sensor10'):
        assert not _is_linked(b2, 'railway_Sensor10', a)


def test_assoc_route25_link_reassign_clear():
    a = railway_SwitchPosition(position="sample_text")
    b1 = railway_Route(active=True)
    b2 = railway_Route(active=False)
...
def test_assoc_routes0_link_reassign_clear():
    a = railway_Route(active=True)
    b1 = railway_RailwayContainer()
    b2 = railway_RailwayContainer()
    _safe_set(a, 'railway_Route', b1)
    assert _is_linked(a, 'railway_Route', b1)
    if hasattr(b1, 'railway_RailwayContainer'):
        assert _is_linked(b1, 'railway_RailwayContainer', a)
    _safe_set(a, 'railway_Route', b2)
    assert _is_linked(a, 'railway_Route', b2)
    if hasattr(b1, 'railway_RailwayContainer'):
        assert not _is_linked(b1, 'railway_RailwayContainer', a)
    if hasattr(b2, 'railway_RailwayContainer'):
        assert _is_linked(b2, 'railway_RailwayContainer', a)
    _safe_set(a, 'railway_Route', None)
    assert not _is_linked(a, 'railway_Route', b2)
    if hasattr(b2, 'railway_RailwayContainer'):
        assert not _is_linked(b2, 'railway_RailwayContainer', a)


def test_assoc_semaphores21_link_reassign_clear():
    a = railway_Semaphore(signal="sample_text")
...
railway_RailwayElement_strategy = st.builds(railway_RailwayElement, id=st.integers())
@given(instance=railway_RailwayElement_strategy)
@settings(max_examples=25)
...
railway_Route_strategy = st.builds(railway_Route, active=st.booleans())
@given(instance=railway_Route_strategy)
@settings(max_examples=25)
