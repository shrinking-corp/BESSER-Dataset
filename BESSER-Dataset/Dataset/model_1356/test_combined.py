...
def test_hyp_statemachine_composite_is_not_abstract():
    assert not inspect.isabstract(statemachine_Composite)


def test_hyp_statemachine_composite_constructor_exists():
    assert callable(statemachine_Composite.__init__)


def test_hyp_statemachine_composite_constructor_args():
...
@given(instance=statemachine_Transition_strategy)
def test_hyp_statemachine_transition_duration_setter(instance):
...
def test_statemachine_Transition_Id_value_roundtrip():
    instance = statemachine_Transition(Id=7, duration=7)
    assert instance.Id == 7
    instance.Id = 13
    assert instance.Id == 13


def test_statemachine_Transition_duration_value_roundtrip():
    instance = statemachine_Transition(Id=7, duration=7)
...
def test_assoc_es_parte_de14_link_reassign_clear():
    a = statemachine_State(name="sample_text")
    b1 = statemachine_Composite()
    b2 = statemachine_Composite()
    _safe_set(a, 'statemachine_State15', b1)
    assert _is_linked(a, 'statemachine_State15', b1)
    if hasattr(b1, 'statemachine_Composite'):
        assert _is_linked(b1, 'statemachine_Composite', a)
    _safe_set(a, 'statemachine_State15', b2)
    assert _is_linked(a, 'statemachine_State15', b2)
    if hasattr(b1, 'statemachine_Composite'):
        assert not _is_linked(b1, 'statemachine_Composite', a)
    if hasattr(b2, 'statemachine_Composite'):
        assert _is_linked(b2, 'statemachine_Composite', a)
    _safe_set(a, 'statemachine_State15', None)
    assert not _is_linked(a, 'statemachine_State15', b2)
    if hasattr(b2, 'statemachine_Composite'):
        assert not _is_linked(b2, 'statemachine_Composite', a)


def test_assoc_es_parte_de3_link_reassign_clear():
...
def test_assoc_source5_link_reassign_clear():
    a = statemachine_Transition(Id=7, duration=7)
...
def test_assoc_tiene11_link_reassign_clear():
    a = statemachine_State(name="sample_text")
    b1 = statemachine_Resource(name="sample_text")
    b2 = statemachine_Resource(name="sample_text_2")
    _safe_set(a, 'statemachine_State12', {b1})
    assert _is_linked(a, 'statemachine_State12', b1)
    if hasattr(b1, 'statemachine_Resource13'):
        assert _is_linked(b1, 'statemachine_Resource13', a)
    _safe_set(a, 'statemachine_State12', {b2})
    assert _is_linked(a, 'statemachine_State12', b2)
    if hasattr(b1, 'statemachine_Resource13'):
        assert not _is_linked(b1, 'statemachine_Resource13', a)
    if hasattr(b2, 'statemachine_Resource13'):
        assert _is_linked(b2, 'statemachine_Resource13', a)
    _safe_set(a, 'statemachine_State12', set())
    assert not _is_linked(a, 'statemachine_State12', b2)
    if hasattr(b2, 'statemachine_Resource13'):
        assert not _is_linked(b2, 'statemachine_Resource13', a)
...
statemachine_Transition_strategy = st.builds(statemachine_Transition, Id=st.integers(), duration=st.integers())
@given(instance=statemachine_Transition_strategy)
@settings(max_examples=25)
