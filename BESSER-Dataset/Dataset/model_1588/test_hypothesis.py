...
def test_petrinet_petrinet_has_nume():
    assert hasattr(petrinet_petrinet, "nume")
    descriptor = None
    for klass in petrinet_petrinet.__mro__:
        if "nume" in klass.__dict__:
            descriptor = klass.__dict__["nume"]
            break
    assert isinstance(descriptor, property)
...
@given(instance=petrinet_petrinet_strategy)
def test_petrinet_petrinet_nume_setter(instance):
