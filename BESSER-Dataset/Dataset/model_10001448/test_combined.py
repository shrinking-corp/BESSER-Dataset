...
def test_hyp_orderstatus2_exists():
    # Check that the Enumeration exists
    assert OrderStatus2 is not None

def test_hyp_orderstatus2_has_all_literals():
    # Collect the names of literals in this Enumeration
    enum_literals = [lit.name for lit in OrderStatus2]
    expected_literals = [
    ]
    # Check that all expected literals exist
    for lit_name in expected_literals:
        assert lit_name in enum_literals, f"Literal '' missing in OrderStatus2"
...
def test_Event_description_value_roundtrip():
    instance = Event(description="sample_text", name="sample_text")
...
def test_assoc_Product_LineItem_link_reassign_clear():
    ...
    b1 = Event(description="sample_text", name="sample_text")
    b2 = Event(description="sample_text_2", name="sample_text_2")
    _safe_set(a, 'event13', b1)
    assert _is_linked(a, 'event13', b1)
    if hasattr(b1, 'lineItems12'):
        assert _is_linked(b1, 'lineItems12', a)
    _safe_set(a, 'event13', b2)
    assert _is_linked(a, 'event13', b2)
    if hasattr(b1, 'lineItems12'):
        assert not _is_linked(b1, 'lineItems12', a)
    if hasattr(b2, 'lineItems12'):
        assert _is_linked(b2, 'lineItems12', a)
    _safe_set(a, 'event13', None)
    assert not _is_linked(a, 'event13', b2)
    if hasattr(b2, 'lineItems12'):
...
Event_strategy = st.builds(Event, description=safe_text, name=safe_text)
@given(instance=Event_strategy)
@settings(max_examples=25)
