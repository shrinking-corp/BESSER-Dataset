...
class Staff:
    ...
    def __init__(self, staffId: str, name: str, type: str, reservationManagementSystem2: "StaffUI" = None):
...
class Booking:
    ...
    def __init__(self, booking_id: int, date: date, startTime: str, endTime: str, reservedTables: str, customer_name: str, contact_no: int, email_id: str, ReservationManagementSystem_Booking_17: "ReservationManagementSystem" = None, reservedBy11: set["Table"] = None):
...
class ReservationManagementSystem:
    ...
    def __init__(self, bookings: str, Staff_StaffUI_00: set["StaffUI"] = None, interacts5: set["CustomerUI"] = None, booking6: set["Booking"] = None, generates8: set["Report"] = None):
    ...
    def Staff_StaffUI_00(self, value):
        # Bidirectional consistency
        old_value = getattr(self, f"_ReservationManagementSystem__Staff_StaffUI_00", None)
        self.__Staff_StaffUI_00 = value if value is not None else set()
        
        # Remove self from old opposite end
        if old_value is not None:
            for item in old_value:
                if hasattr(item, "interacts1"):
                    opp_val = getattr(item, "interacts1", None)
        ...
        if value is not None:
            for item in value:
                if hasattr(item, "interacts1"):
                    ...
                    if opp_val is None:
                        setattr(item, "interacts1", set([self]))
                    elif isinstance(opp_val, set):
                        opp_val.add(self)
