...
def test_hyp_frontofhouse_is_not_abstract():
    assert not inspect.isabstract(FrontOfHouse)


def test_hyp_frontofhouse_constructor_exists():
    assert callable(FrontOfHouse.__init__)


def test_hyp_frontofhouse_constructor_args():
...
def test_assoc_Staff_StaffUI_link_reassign_clear():
    ...
    if hasattr(b1, 'interacts1'):
        assert _is_linked(b1, 'interacts1', a)
    _safe_set(a, 'Staff_StaffUI_00', {b2})
    assert _is_linked(a, 'Staff_StaffUI_00', b2)
    if hasattr(b1, 'interacts1'):
        assert not _is_linked(b1, 'interacts1', a)
    if hasattr(b2, 'interacts1'):
        assert _is_linked(b2, 'interacts1', a)
    _safe_set(a, 'Staff_StaffUI_00', set())
    assert not _is_linked(a, 'Staff_StaffUI_00', b2)
    if hasattr(b2, 'interacts1'):
        assert not _is_linked(b2, 'interacts1', a)
...
FrontOfHouse_strategy = st.builds(FrontOfHouse)
