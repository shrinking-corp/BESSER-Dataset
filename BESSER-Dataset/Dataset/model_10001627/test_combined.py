...
def test_hyp_a_constructor_args():
    ...
    params = list(sig.parameters.keys())
    assert "attA_" in params, "Missing parameter 'attA_'"
...
def test_hyp_a_attA__setter(instance):
    original = instance.attA_
    instance.attA_ = original
    assert instance.attA_ == original
...
def test_A_attA__value_roundtrip():
    instance = A(attA_="sample_text")
    assert instance.attA_ == "sample_text"
    instance.attA_ = "sample_text_2"
    assert instance.attA_ == "sample_text_2"
...
def test_assoc_A_B_link_reassign_clear():
    a = B(attB=7)
    b1 = A(attA_="sample_text")
    b2 = A(attA_="sample_text_2")
    _safe_set(a, 'a3', b1)
...
A_strategy = st.builds(A, attA_=safe_text)
@given(instance=A_strategy)
