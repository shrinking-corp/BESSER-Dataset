...
def test_hyp_er_element_is_not_abstract():
    assert not inspect.isabstract(ER_Element)


def test_hyp_er_element_constructor_exists():
    assert callable(ER_Element.__init__)


def test_hyp_er_element_constructor_args():
    sig = inspect.signature(ER_Element.__init__)
...
@given(instance=ER_Element_strategy)
def test_hyp_er_element_name_setter(instance):
...
def test_ER_Element_name_value_roundtrip():
    instance = ER_Element(name="sample_text")
    assert instance.name == "sample_text"
    instance.name = "sample_text_2"
    assert instance.name == "sample_text_2"


def test_ER_ERModel_isa_Element():
    instance = ER_ERModel()
    assert isinstance(instance, Element)


def test_ER_EntityType_isa_Element():
    instance = ER_EntityType()
    assert isinstance(instance, Element)


def test_ER_Feature_isa_Element():
    instance = ER_Feature()
    assert isinstance(instance, Element)
...
ER_Element_strategy = st.builds(ER_Element, name=safe_text)
@given(instance=ER_Element_strategy)
@settings(max_examples=25)
def test_ER_Element_instantiation(instance):
    assert isinstance(instance, ER_Element)
