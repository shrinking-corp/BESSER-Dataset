...
def test_er_element_is_not_abstract():
    assert not inspect.isabstract(ER_Element)


def test_er_element_constructor_exists():
    assert callable(ER_Element.__init__)


def test_er_element_constructor_args():
    sig = inspect.signature(ER_Element.__init__)
    params = list(sig.parameters.keys())
    assert "name" in params, "Missing parameter 'name'"

def test_er_element_has_name():
    assert hasattr(ER_Element, "name")
    descriptor = None
    for klass in ER_Element.__mro__:
...
@given(instance=ER_Element_strategy)
def test_er_element_name_setter(instance):
