...
def test_assoc_accessors107_link_reassign_clear():
    ...
    if hasattr(b1, 'owner108'):
        assert _is_linked(b1, 'owner108', a)
    _safe_set(a, 'ChildAccess', b2)
    assert _is_linked(a, 'ChildAccess', b2)
    if hasattr(b1, 'owner108'):
        assert not _is_linked(b1, 'owner108', a)
    if hasattr(b2, 'owner108'):
        assert _is_linked(b2, 'owner108', a)
    _safe_set(a, 'ChildAccess', None)
    assert not _is_linked(a, 'ChildAccess', b2)
    if hasattr(b2, 'owner108'):
        assert not _is_linked(b2, 'owner108', a)
...
def test_assoc_children97_link_reassign_clear():
    a = gmfgraph_RealFigure(name="sample_text")
    b1 = gmfgraph_Figure()
    b2 = gmfgraph_Figure()
    _safe_set(a, 'gmfgraph_RealFigure98', {b1})
    assert _is_linked(a, 'gmfgraph_RealFigure98', b1)
    if hasattr(b1, 'gmfgraph_Figure99'):
        assert _is_linked(b1, 'gmfgraph_Figure99', a)
    _safe_set(a, 'gmfgraph_RealFigure98', {b2})
    assert _is_linked(a, 'gmfgraph_RealFigure98', b2)
    if hasattr(b1, 'gmfgraph_Figure99'):
        assert not _is_linked(b1, 'gmfgraph_Figure99', a)
    if hasattr(b2, 'gmfgraph_Figure99'):
        assert _is_linked(b2, 'gmfgraph_Figure99', a)
    _safe_set(a, 'gmfgraph_RealFigure98', set())
    assert not _is_linked(a, 'gmfgraph_RealFigure98', b2)
    if hasattr(b2, 'gmfgraph_Figure99'):
        assert not _is_linked(b2, 'gmfgraph_Figure99', a)


def test_assoc_color31_link_reassign_clear():
    a = gmfgraph_LineBorder(width=7)
    b1 = gmfgraph_Color()
    b2 = gmfgraph_Color()
    _safe_set(a, 'gmfgraph_LineBorder', b1)
    assert _is_linked(a, 'gmfgraph_LineBorder', b1)
    if hasattr(b1, 'gmfgraph_Color'):
        assert _is_linked(b1, 'gmfgraph_Color', a)
    _safe_set(a, 'gmfgraph_LineBorder', b2)
    assert _is_linked(a, 'gmfgraph_LineBorder', b2)
    if hasattr(b1, 'gmfgraph_Color'):
        assert not _is_linked(b1, 'gmfgraph_Color', a)
    if hasattr(b2, 'gmfgraph_Color'):
        assert _is_linked(b2, 'gmfgraph_Color', a)
    _safe_set(a, 'gmfgraph_LineBorder', None)
    assert not _is_linked(a, 'gmfgraph_LineBorder', b2)
    if hasattr(b2, 'gmfgraph_Color'):
        assert not _is_linked(b2, 'gmfgraph_Color', a)
...
def test_assoc_customChildren81_link_reassign_clear():
    a = gmfgraph_FigureAccessor(accessor="sample_text")
    b1 = gmfgraph_CustomFigure()
    b2 = gmfgraph_CustomFigure()
    _safe_set(a, 'gmfgraph_FigureAccessor82', b1)
    assert _is_linked(a, 'gmfgraph_FigureAccessor82', b1)
    if hasattr(b1, 'gmfgraph_CustomFigure'):
        assert _is_linked(b1, 'gmfgraph_CustomFigure', a)
    _safe_set(a, 'gmfgraph_FigureAccessor82', b2)
    assert _is_linked(a, 'gmfgraph_FigureAccessor82', b2)
    if hasattr(b1, 'gmfgraph_CustomFigure'):
    ...
    _safe_set(a, 'gmfgraph_FigureAccessor82', None)
    assert not _is_linked(a, 'gmfgraph_FigureAccessor82', b2)
    if hasattr(b2, 'gmfgraph_CustomFigure'):
        assert not _is_linked(b2, 'gmfgraph_CustomFigure', a)


def test_assoc_defaultSize95_link_reassign_clear():
    a = gmfgraph_Dimension(dx=7, dy=7)
    b1 = gmfgraph_DefaultSizeFacet()
    b2 = gmfgraph_DefaultSizeFacet()
    _safe_set(a, 'gmfgraph_Dimension96', b1)
    assert _is_linked(a, 'gmfgraph_Dimension96', b1)
    if hasattr(b1, 'gmfgraph_DefaultSizeFacet'):
        assert _is_linked(b1, 'gmfgraph_DefaultSizeFacet', a)
    _safe_set(a, 'gmfgraph_Dimension96', b2)
    assert _is_linked(a, 'gmfgraph_Dimension96', b2)
    if hasattr(b1, 'gmfgraph_DefaultSizeFacet'):
    ...
    _safe_set(a, 'gmfgraph_Dimension96', None)
    assert not _is_linked(a, 'gmfgraph_Dimension96', b2)
    if hasattr(b2, 'gmfgraph_DefaultSizeFacet'):
...
def test_assoc_figure100_link_reassign_clear():
    a = gmfgraph_ChildAccess(accessor="sample_text")
    b1 = gmfgraph_Figure()
    b2 = gmfgraph_Figure()
    _safe_set(a, 'gmfgraph_ChildAccess101', b1)
    assert _is_linked(a, 'gmfgraph_ChildAccess101', b1)
    if hasattr(b1, 'gmfgraph_Figure102'):
        assert _is_linked(b1, 'gmfgraph_Figure102', a)
    _safe_set(a, 'gmfgraph_ChildAccess101', b2)
    assert _is_linked(a, 'gmfgraph_ChildAccess101', b2)
    if hasattr(b1, 'gmfgraph_Figure102'):
        assert not _is_linked(b1, 'gmfgraph_Figure102', a)
    if hasattr(b2, 'gmfgraph_Figure102'):
        assert _is_linked(b2, 'gmfgraph_Figure102', a)
    _safe_set(a, 'gmfgraph_ChildAccess101', None)
    assert not _is_linked(a, 'gmfgraph_ChildAccess101', b2)
    if hasattr(b2, 'gmfgraph_Figure102'):
        assert not _is_linked(b2, 'gmfgraph_Figure102', a)


def test_assoc_figure71_link_reassign_clear():
    a = gmfgraph_RealFigure(name="sample_text")
    b1 = gmfgraph_FigureRef()
    b2 = gmfgraph_FigureRef()
    _safe_set(a, 'gmfgraph_RealFigure72', b1)
    assert _is_linked(a, 'gmfgraph_RealFigure72', b1)
    if hasattr(b1, 'gmfgraph_FigureRef'):
        assert _is_linked(b1, 'gmfgraph_FigureRef', a)
    _safe_set(a, 'gmfgraph_RealFigure72', b2)
    assert _is_linked(a, 'gmfgraph_RealFigure72', b2)
    if hasattr(b1, 'gmfgraph_FigureRef'):
        assert not _is_linked(b1, 'gmfgraph_FigureRef', a)
    if hasattr(b2, 'gmfgraph_FigureRef'):
        assert _is_linked(b2, 'gmfgraph_FigureRef', a)
    _safe_set(a, 'gmfgraph_RealFigure72', None)
    assert not _is_linked(a, 'gmfgraph_RealFigure72', b2)
    if hasattr(b2, 'gmfgraph_FigureRef'):
...
def test_assoc_insets32_link_reassign_clear():
...
def test_assoc_insets57_link_reassign_clear():
    a = gmfgraph_Insets(bottom=7, left=7, right=7, top=7)
    b1 = gmfgraph_Figure()
    b2 = gmfgraph_Figure()
    _safe_set(a, 'gmfgraph_Insets59', b1)
    assert _is_linked(a, 'gmfgraph_Insets59', b1)
    if hasattr(b1, 'gmfgraph_Figure58'):
        assert _is_linked(b1, 'gmfgraph_Figure58', a)
    _safe_set(a, 'gmfgraph_Insets59', b2)
    assert _is_linked(a, 'gmfgraph_Insets59', b2)
    if hasattr(b1, 'gmfgraph_Figure58'):
        assert not _is_linked(b1, 'gmfgraph_Figure58', a)
    if hasattr(b2, 'gmfgraph_Figure58'):
        assert _is_linked(b2, 'gmfgraph_Figure58', a)
    _safe_set(a, 'gmfgraph_Insets59', None)
    assert not _is_linked(a, 'gmfgraph_Insets59', b2)
    if hasattr(b2, 'gmfgraph_Figure58'):
        assert not _is_linked(b2, 'gmfgraph_Figure58', a)
...
def test_assoc_location63_link_reassign_clear():
    a = gmfgraph_Point(x=7, y=7)
    b1 = gmfgraph_Figure()
    b2 = gmfgraph_Figure()
    _safe_set(a, 'gmfgraph_Point', b1)
    assert _is_linked(a, 'gmfgraph_Point', b1)
    if hasattr(b1, 'gmfgraph_Figure64'):
        assert _is_linked(b1, 'gmfgraph_Figure64', a)
    _safe_set(a, 'gmfgraph_Point', b2)
    assert _is_linked(a, 'gmfgraph_Point', b2)
    if hasattr(b1, 'gmfgraph_Figure64'):
        assert not _is_linked(b1, 'gmfgraph_Figure64', a)
    if hasattr(b2, 'gmfgraph_Figure64'):
        assert _is_linked(b2, 'gmfgraph_Figure64', a)
    _safe_set(a, 'gmfgraph_Point', None)
    assert not _is_linked(a, 'gmfgraph_Point', b2)
    if hasattr(b2, 'gmfgraph_Figure64'):
        assert not _is_linked(b2, 'gmfgraph_Figure64', a)


def test_assoc_margins83_link_reassign_clear():
    ...
    if hasattr(b1, 'gmfgraph_Dimension84'):
        assert _is_linked(b1, 'gmfgraph_Dimension84', a)
    _safe_set(a, 'gmfgraph_GridLayout', b2)
    assert _is_linked(a, 'gmfgraph_GridLayout', b2)
    if hasattr(b1, 'gmfgraph_Dimension84'):
        assert not _is_linked(b1, 'gmfgraph_Dimension84', a)
    if hasattr(b2, 'gmfgraph_Dimension84'):
        assert _is_linked(b2, 'gmfgraph_Dimension84', a)
    _safe_set(a, 'gmfgraph_GridLayout', None)
    assert not _is_linked(a, 'gmfgraph_GridLayout', b2)
    if hasattr(b2, 'gmfgraph_Dimension84'):
        assert not _is_linked(b2, 'gmfgraph_Dimension84', a)


def test_assoc_maximumSize46_link_reassign_clear():
    a = gmfgraph_Dimension(dx=7, dy=7)
    b1 = gmfgraph_Figure()
    b2 = gmfgraph_Figure()
...
def test_assoc_minimumSize49_link_reassign_clear():
    a = gmfgraph_Dimension(dx=7, dy=7)
    b1 = gmfgraph_Figure()
    b2 = gmfgraph_Figure()
    _safe_set(a, 'gmfgraph_Dimension51', b1)
    assert _is_linked(a, 'gmfgraph_Dimension51', b1)
    if hasattr(b1, 'gmfgraph_Figure50'):
        assert _is_linked(b1, 'gmfgraph_Figure50', a)
    _safe_set(a, 'gmfgraph_Dimension51', b2)
    assert _is_linked(a, 'gmfgraph_Dimension51', b2)
    if hasattr(b1, 'gmfgraph_Figure50'):
        assert not _is_linked(b1, 'gmfgraph_Figure50', a)
    if hasattr(b2, 'gmfgraph_Figure50'):
        assert _is_linked(b2, 'gmfgraph_Figure50', a)
    _safe_set(a, 'gmfgraph_Dimension51', None)
    assert not _is_linked(a, 'gmfgraph_Dimension51', b2)
    if hasattr(b2, 'gmfgraph_Figure50'):
        assert not _is_linked(b2, 'gmfgraph_Figure50', a)
...
def test_assoc_preferredSize52_link_reassign_clear():
    a = gmfgraph_Dimension(dx=7, dy=7)
    b1 = gmfgraph_Figure()
    b2 = gmfgraph_Figure()
    _safe_set(a, 'gmfgraph_Dimension54', b1)
    assert _is_linked(a, 'gmfgraph_Dimension54', b1)
    if hasattr(b1, 'gmfgraph_Figure53'):
        assert _is_linked(b1, 'gmfgraph_Figure53', a)
    _safe_set(a, 'gmfgraph_Dimension54', b2)
    assert _is_linked(a, 'gmfgraph_Dimension54', b2)
    if hasattr(b1, 'gmfgraph_Figure53'):
        assert not _is_linked(b1, 'gmfgraph_Figure53', a)
    if hasattr(b2, 'gmfgraph_Figure53'):
        assert _is_linked(b2, 'gmfgraph_Figure53', a)
    _safe_set(a, 'gmfgraph_Dimension54', None)
    assert not _is_linked(a, 'gmfgraph_Dimension54', b2)
    if hasattr(b2, 'gmfgraph_Figure53'):
        assert not _is_linked(b2, 'gmfgraph_Figure53', a)


def test_assoc_resolvedChildren73_link_reassign_clear():
    ...
    if hasattr(b1, 'gmfgraph_Figure74'):
        assert _is_linked(b1, 'gmfgraph_Figure74', a)
    _safe_set(a, 'gmfgraph_Shape', {b2})
    assert _is_linked(a, 'gmfgraph_Shape', b2)
    if hasattr(b1, 'gmfgraph_Figure74'):
        assert not _is_linked(b1, 'gmfgraph_Figure74', a)
    if hasattr(b2, 'gmfgraph_Figure74'):
        assert _is_linked(b2, 'gmfgraph_Figure74', a)
    _safe_set(a, 'gmfgraph_Shape', set())
    assert not _is_linked(a, 'gmfgraph_Shape', b2)
    if hasattr(b2, 'gmfgraph_Figure74'):
        assert not _is_linked(b2, 'gmfgraph_Figure74', a)


def test_assoc_size65_link_reassign_clear():
    a = gmfgraph_Point(x=7, y=7)
    b1 = gmfgraph_Figure()
    b2 = gmfgraph_Figure()
    _safe_set(a, 'gmfgraph_Point67', b1)
    assert _is_linked(a, 'gmfgraph_Point67', b1)
    if hasattr(b1, 'gmfgraph_Figure66'):
        assert _is_linked(b1, 'gmfgraph_Figure66', a)
    _safe_set(a, 'gmfgraph_Point67', b2)
    assert _is_linked(a, 'gmfgraph_Point67', b2)
    if hasattr(b1, 'gmfgraph_Figure66'):
        assert not _is_linked(b1, 'gmfgraph_Figure66', a)
    if hasattr(b2, 'gmfgraph_Figure66'):
        assert _is_linked(b2, 'gmfgraph_Figure66', a)
    _safe_set(a, 'gmfgraph_Point67', None)
    assert not _is_linked(a, 'gmfgraph_Point67', b2)
    if hasattr(b2, 'gmfgraph_Figure66'):
        assert not _is_linked(b2, 'gmfgraph_Figure66', a)


def test_assoc_size92_link_reassign_clear():
    a = gmfgraph_Dimension(dx=7, dy=7)
    b1 = gmfgraph_XYLayoutData()
    b2 = gmfgraph_XYLayoutData()
    _safe_set(a, 'gmfgraph_Dimension94', b1)
    assert _is_linked(a, 'gmfgraph_Dimension94', b1)
    if hasattr(b1, 'gmfgraph_XYLayoutData93'):
        assert _is_linked(b1, 'gmfgraph_XYLayoutData93', a)
    _safe_set(a, 'gmfgraph_Dimension94', b2)
    assert _is_linked(a, 'gmfgraph_Dimension94', b2)
    if hasattr(b1, 'gmfgraph_XYLayoutData93'):
        assert not _is_linked(b1, 'gmfgraph_XYLayoutData93', a)
    if hasattr(b2, 'gmfgraph_XYLayoutData93'):
        assert _is_linked(b2, 'gmfgraph_XYLayoutData93', a)
    _safe_set(a, 'gmfgraph_Dimension94', None)
    assert not _is_linked(a, 'gmfgraph_Dimension94', b2)
    if hasattr(b2, 'gmfgraph_XYLayoutData93'):
        assert not _is_linked(b2, 'gmfgraph_XYLayoutData93', a)


def test_assoc_sizeHint38_link_reassign_clear():
    ...
    if hasattr(b1, 'gmfgraph_Dimension'):
        assert _is_linked(b1, 'gmfgraph_Dimension', a)
    _safe_set(a, 'gmfgraph_GridLayoutData', b2)
    assert _is_linked(a, 'gmfgraph_GridLayoutData', b2)
    if hasattr(b1, 'gmfgraph_Dimension'):
        assert not _is_linked(b1, 'gmfgraph_Dimension', a)
    if hasattr(b2, 'gmfgraph_Dimension'):
        assert _is_linked(b2, 'gmfgraph_Dimension', a)
    _safe_set(a, 'gmfgraph_GridLayoutData', None)
    assert not _is_linked(a, 'gmfgraph_GridLayoutData', b2)
    if hasattr(b2, 'gmfgraph_Dimension'):
        assert not _is_linked(b2, 'gmfgraph_Dimension', a)


def test_assoc_spacing85_link_reassign_clear():
    a = gmfgraph_GridLayout(equalWidth=True, numColumns=7)
    b1 = gmfgraph_Dimension(dx=7, dy=7)
    b2 = gmfgraph_Dimension(dx=13, dy=13)
    _safe_set(a, 'gmfgraph_GridLayout86', b1)
    assert _is_linked(a, 'gmfgraph_GridLayout86', b1)
    if hasattr(b1, 'gmfgraph_Dimension87'):
        assert _is_linked(b1, 'gmfgraph_Dimension87', a)
    _safe_set(a, 'gmfgraph_GridLayout86', b2)
    assert _is_linked(a, 'gmfgraph_GridLayout86', b2)
    if hasattr(b1, 'gmfgraph_Dimension87'):
        assert not _is_linked(b1, 'gmfgraph_Dimension87', a)
    if hasattr(b2, 'gmfgraph_Dimension87'):
        assert _is_linked(b2, 'gmfgraph_Dimension87', a)
    _safe_set(a, 'gmfgraph_GridLayout86', None)
    assert not _is_linked(a, 'gmfgraph_GridLayout86', b2)
    if hasattr(b2, 'gmfgraph_Dimension87'):
        assert not _is_linked(b2, 'gmfgraph_Dimension87', a)


def test_assoc_spacing88_link_reassign_clear():
    a = gmfgraph_Dimension(dx=7, dy=7)
    b1 = gmfgraph_BorderLayout()
    b2 = gmfgraph_BorderLayout()
    _safe_set(a, 'gmfgraph_Dimension89', b1)
    assert _is_linked(a, 'gmfgraph_Dimension89', b1)
    if hasattr(b1, 'gmfgraph_BorderLayout'):
        assert _is_linked(b1, 'gmfgraph_BorderLayout', a)
    _safe_set(a, 'gmfgraph_Dimension89', b2)
    assert _is_linked(a, 'gmfgraph_Dimension89', b2)
    if hasattr(b1, 'gmfgraph_BorderLayout'):
    ...
    _safe_set(a, 'gmfgraph_Dimension89', None)
    assert not _is_linked(a, 'gmfgraph_Dimension89', b2)
    if hasattr(b2, 'gmfgraph_BorderLayout'):
        assert not _is_linked(b2, 'gmfgraph_BorderLayout', a)


def test_assoc_template75_link_reassign_clear():
    a = gmfgraph_Point(x=7, y=7)
    b1 = gmfgraph_Polyline()
    b2 = gmfgraph_Polyline()
    _safe_set(a, 'gmfgraph_Point76', b1)
    assert _is_linked(a, 'gmfgraph_Point76', b1)
    if hasattr(b1, 'gmfgraph_Polyline'):
        assert _is_linked(b1, 'gmfgraph_Polyline', a)
    _safe_set(a, 'gmfgraph_Point76', b2)
    assert _is_linked(a, 'gmfgraph_Point76', b2)
    if hasattr(b1, 'gmfgraph_Polyline'):
        assert not _is_linked(b1, 'gmfgraph_Polyline', a)
    if hasattr(b2, 'gmfgraph_Polyline'):
        assert _is_linked(b2, 'gmfgraph_Polyline', a)
    _safe_set(a, 'gmfgraph_Point76', None)
    assert not _is_linked(a, 'gmfgraph_Point76', b2)
    if hasattr(b2, 'gmfgraph_Polyline'):
        assert not _is_linked(b2, 'gmfgraph_Polyline', a)


def test_assoc_topLeft90_link_reassign_clear():
    a = gmfgraph_Point(x=7, y=7)
    b1 = gmfgraph_XYLayoutData()
    b2 = gmfgraph_XYLayoutData()
    _safe_set(a, 'gmfgraph_Point91', b1)
    assert _is_linked(a, 'gmfgraph_Point91', b1)
    if hasattr(b1, 'gmfgraph_XYLayoutData'):
        assert _is_linked(b1, 'gmfgraph_XYLayoutData', a)
    _safe_set(a, 'gmfgraph_Point91', b2)
    assert _is_linked(a, 'gmfgraph_Point91', b2)
    if hasattr(b1, 'gmfgraph_XYLayoutData'):
    ...
    _safe_set(a, 'gmfgraph_Point91', None)
    assert not _is_linked(a, 'gmfgraph_Point91', b2)
    if hasattr(b2, 'gmfgraph_XYLayoutData'):
        assert not _is_linked(b2, 'gmfgraph_XYLayoutData', a)


def test_assoc_typedFigure29_link_reassign_clear():
    a = gmfgraph_RealFigure(name="sample_text")
    b1 = gmfgraph_FigureAccessor(accessor="sample_text")
    b2 = gmfgraph_FigureAccessor(accessor="sample_text_2")
    _safe_set(a, 'gmfgraph_RealFigure30', b1)
    assert _is_linked(a, 'gmfgraph_RealFigure30', b1)
    if hasattr(b1, 'gmfgraph_FigureAccessor'):
        assert _is_linked(b1, 'gmfgraph_FigureAccessor', a)
    _safe_set(a, 'gmfgraph_RealFigure30', b2)
    assert _is_linked(a, 'gmfgraph_RealFigure30', b2)
    if hasattr(b1, 'gmfgraph_FigureAccessor'):
    ...
    _safe_set(a, 'gmfgraph_RealFigure30', None)
    assert not _is_linked(a, 'gmfgraph_RealFigure30', b2)
    if hasattr(b2, 'gmfgraph_FigureAccessor'):
