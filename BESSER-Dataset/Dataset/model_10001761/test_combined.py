...
def test_hyp_sap_pi_actor_is_not_abstract():
    assert not inspect.isabstract(SAP_PI_Actor)


def test_hyp_sap_pi_actor_constructor_exists():
    assert callable(SAP_PI_Actor.__init__)


def test_hyp_sap_pi_actor_constructor_args():
    sig = inspect.signature(SAP_PI_Actor.__init__)
    params = list(sig.parameters.keys())



def test_hyp_tibco_actor_is_not_abstract():
    assert not inspect.isabstract(TIBCO_Actor)


def test_hyp_tibco_actor_constructor_exists():
    assert callable(TIBCO_Actor.__init__)


def test_hyp_tibco_actor_constructor_args():
    sig = inspect.signature(TIBCO_Actor.__init__)
    params = list(sig.parameters.keys())
...
SAP_PI_Actor_strategy = st.builds(
    SAP_PI_Actor,
)
TIBCO_Actor_strategy = st.builds(
    TIBCO_Actor,
)
...
SAP_PI_Actor_strategy = st.builds(SAP_PI_Actor)
@given(instance=SAP_PI_Actor_strategy)
@settings(max_examples=25)
def test_SAP_PI_Actor_instantiation(instance):
    assert isinstance(instance, SAP_PI_Actor)
...
TIBCO_Actor_strategy = st.builds(TIBCO_Actor)
@given(instance=TIBCO_Actor_strategy)
@settings(max_examples=25)
def test_TIBCO_Actor_instantiation(instance):
    assert isinstance(instance, TIBCO_Actor)
