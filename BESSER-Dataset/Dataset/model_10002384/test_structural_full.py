...
def test_assoc_FACULTY_ADMIN_link_reassign_clear():
    ...
    if hasattr(b1, 'fACULTY41'):
        assert _is_linked(b1, 'fACULTY41', a)
    _safe_set(a, 'aDMIN40', b2)
    assert _is_linked(a, 'aDMIN40', b2)
    if hasattr(b1, 'fACULTY41'):
        assert not _is_linked(b1, 'fACULTY41', a)
    if hasattr(b2, 'fACULTY41'):
        assert _is_linked(b2, 'fACULTY41', a)
    _safe_set(a, 'aDMIN40', None)
    assert not _is_linked(a, 'aDMIN40', b2)
    if hasattr(b2, 'fACULTY41'):
        assert not _is_linked(b2, 'fACULTY41', a)
...
def test_assoc_FACULTY_STUDENT_link_reassign_clear():
    ...
    _safe_set(a, 'fACULTY39', {b1})
    assert _is_linked(a, 'fACULTY39', b1)
    if hasattr(b1, 'sTUDENT38'):
        assert _is_linked(b1, 'sTUDENT38', a)
    _safe_set(a, 'fACULTY39', {b2})
    assert _is_linked(a, 'fACULTY39', b2)
    if hasattr(b1, 'sTUDENT38'):
        assert not _is_linked(b1, 'sTUDENT38', a)
    if hasattr(b2, 'sTUDENT38'):
        assert _is_linked(b2, 'sTUDENT38', a)
    _safe_set(a, 'fACULTY39', set())
    assert not _is_linked(a, 'fACULTY39', b2)
    if hasattr(b2, 'sTUDENT38'):
        assert not _is_linked(b2, 'sTUDENT38', a)
...
def test_assoc_PARENT_ADMIN_link_reassign_clear():
    ...
    if hasattr(b1, 'pARENT45'):
        assert _is_linked(b1, 'pARENT45', a)
    _safe_set(a, 'aDMIN44', b2)
    assert _is_linked(a, 'aDMIN44', b2)
    if hasattr(b1, 'pARENT45'):
        assert not _is_linked(b1, 'pARENT45', a)
    if hasattr(b2, 'pARENT45'):
        assert _is_linked(b2, 'pARENT45', a)
    _safe_set(a, 'aDMIN44', None)
    assert not _is_linked(a, 'aDMIN44', b2)
    if hasattr(b2, 'pARENT45'):
        assert not _is_linked(b2, 'pARENT45', a)
...
def test_assoc_STUDENT_ADMIN_link_reassign_clear():
    ...
    if hasattr(b1, 'sTUDENT43'):
        assert _is_linked(b1, 'sTUDENT43', a)
    _safe_set(a, 'aDMIN42', b2)
    assert _is_linked(a, 'aDMIN42', b2)
    if hasattr(b1, 'sTUDENT43'):
        assert not _is_linked(b1, 'sTUDENT43', a)
    if hasattr(b2, 'sTUDENT43'):
        assert _is_linked(b2, 'sTUDENT43', a)
    _safe_set(a, 'aDMIN42', None)
    assert not _is_linked(a, 'aDMIN42', b2)
    if hasattr(b2, 'sTUDENT43'):
        assert not _is_linked(b2, 'sTUDENT43', a)
