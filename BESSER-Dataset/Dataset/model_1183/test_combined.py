...
def test_hyp_simpleocl_collectionitem_is_not_abstract():
    assert not inspect.isabstract(simpleocl_CollectionItem)


def test_hyp_simpleocl_collectionitem_constructor_exists():
    assert callable(simpleocl_CollectionItem.__init__)


def test_hyp_simpleocl_collectionitem_constructor_args():
    sig = inspect.signature(simpleocl_CollectionItem.__init__)
    params = list(sig.parameters.keys())



def test_hyp_simpleocl_collectionrange_is_not_abstract():
    assert not inspect.isabstract(simpleocl_CollectionRange)


def test_hyp_simpleocl_collectionrange_constructor_exists():
    assert callable(simpleocl_CollectionRange.__init__)


def test_hyp_simpleocl_collectionrange_constructor_args():
...
def test_hyp_simpleocl_collectionpart_is_not_abstract():
    assert not inspect.isabstract(simpleocl_CollectionPart)


def test_hyp_simpleocl_collectionpart_constructor_exists():
    assert callable(simpleocl_CollectionPart.__init__)


def test_hyp_simpleocl_collectionpart_constructor_args():
...
def test_simpleocl_CollectionItem_isa_CollectionPart():
    instance = simpleocl_CollectionItem()
    assert isinstance(instance, CollectionPart)


def test_simpleocl_CollectionRange_isa_CollectionPart():
    instance = simpleocl_CollectionRange()
    assert isinstance(instance, CollectionPart)
...
def test_simpleocl_CollectionPart_isa_LocatedElement():
    instance = simpleocl_CollectionPart()
...
def test_assoc_appliedOperator22_link_reassign_clear():
    ...
    if hasattr(b1, 'source23'):
        assert _is_linked(b1, 'source23', a)
    _safe_set(a, 'OperatorCallExp', b2)
    assert _is_linked(a, 'OperatorCallExp', b2)
    if hasattr(b1, 'source23'):
        assert not _is_linked(b1, 'source23', a)
    if hasattr(b2, 'source23'):
        assert _is_linked(b2, 'source23', a)
    _safe_set(a, 'OperatorCallExp', None)
    assert not _is_linked(a, 'OperatorCallExp', b2)
    if hasattr(b2, 'source23'):
        assert not _is_linked(b2, 'source23', a)


def test_assoc_argument55_link_reassign_clear():
    a = simpleocl_OperatorCallExp(operationName="sample_text")
    b1 = simpleocl_OclExpression()
    b2 = simpleocl_OclExpression()
    _safe_set(a, 'simpleocl_OperatorCallExp', b1)
    assert _is_linked(a, 'simpleocl_OperatorCallExp', b1)
    if hasattr(b1, 'simpleocl_OclExpression56'):
        assert _is_linked(b1, 'simpleocl_OclExpression56', a)
    _safe_set(a, 'simpleocl_OperatorCallExp', b2)
    assert _is_linked(a, 'simpleocl_OperatorCallExp', b2)
    if hasattr(b1, 'simpleocl_OclExpression56'):
        assert not _is_linked(b1, 'simpleocl_OclExpression56', a)
    if hasattr(b2, 'simpleocl_OclExpression56'):
        assert _is_linked(b2, 'simpleocl_OclExpression56', a)
    _safe_set(a, 'simpleocl_OperatorCallExp', None)
    assert not _is_linked(a, 'simpleocl_OperatorCallExp', b2)
    if hasattr(b2, 'simpleocl_OclExpression56'):
        assert not _is_linked(b2, 'simpleocl_OclExpression56', a)


def test_assoc_argumentTypes131_link_reassign_clear():
    a = simpleocl_OclType(name="sample_text")
    b1 = simpleocl_LambdaType()
    b2 = simpleocl_LambdaType()
    _safe_set(a, 'OclType132', b1)
    assert _is_linked(a, 'OclType132', b1)
    if hasattr(b1, 'lambdaArgType'):
        assert _is_linked(b1, 'lambdaArgType', a)
    _safe_set(a, 'OclType132', b2)
    assert _is_linked(a, 'OclType132', b2)
    if hasattr(b1, 'lambdaArgType'):
        assert not _is_linked(b1, 'lambdaArgType', a)
    if hasattr(b2, 'lambdaArgType'):
        assert _is_linked(b2, 'lambdaArgType', a)
    _safe_set(a, 'OclType132', None)
    assert not _is_linked(a, 'OclType132', b2)
    if hasattr(b2, 'lambdaArgType'):
...
def test_assoc_arguments47_link_reassign_clear():
    ...
    if hasattr(b1, 'simpleocl_OclExpression48'):
        assert _is_linked(b1, 'simpleocl_OclExpression48', a)
    _safe_set(a, 'simpleocl_StaticOperationCall', {b2})
    assert _is_linked(a, 'simpleocl_StaticOperationCall', b2)
    if hasattr(b1, 'simpleocl_OclExpression48'):
        assert not _is_linked(b1, 'simpleocl_OclExpression48', a)
    if hasattr(b2, 'simpleocl_OclExpression48'):
        assert _is_linked(b2, 'simpleocl_OclExpression48', a)
    _safe_set(a, 'simpleocl_StaticOperationCall', set())
    assert not _is_linked(a, 'simpleocl_StaticOperationCall', b2)
    if hasattr(b2, 'simpleocl_OclExpression48'):
        assert not _is_linked(b2, 'simpleocl_OclExpression48', a)


def test_assoc_arguments53_link_reassign_clear():
...
def test_assoc_attribute99_link_reassign_clear():
    a = simpleocl_OclType(name="sample_text")
    b1 = simpleocl_Attribute()
    b2 = simpleocl_Attribute()
    _safe_set(a, 'type100', b1)
    assert _is_linked(a, 'type100', b1)
    if hasattr(b1, 'Attribute101'):
        assert _is_linked(b1, 'Attribute101', a)
    _safe_set(a, 'type100', b2)
    assert _is_linked(a, 'type100', b2)
    if hasattr(b1, 'Attribute101'):
        assert not _is_linked(b1, 'Attribute101', a)
    if hasattr(b2, 'Attribute101'):
        assert _is_linked(b2, 'Attribute101', a)
    _safe_set(a, 'type100', None)
    assert not _is_linked(a, 'type100', b2)
    if hasattr(b2, 'Attribute101'):
        assert not _is_linked(b2, 'Attribute101', a)


def test_assoc_attributes118_link_reassign_clear():
    a = simpleocl_TupleTypeAttribute(name="sample_text")
    b1 = simpleocl_TupleType()
    b2 = simpleocl_TupleType()
    _safe_set(a, 'TupleTypeAttribute119', b1)
    assert _is_linked(a, 'TupleTypeAttribute119', b1)
    if hasattr(b1, 'tupleType'):
        assert _is_linked(b1, 'tupleType', a)
    _safe_set(a, 'TupleTypeAttribute119', b2)
    assert _is_linked(a, 'TupleTypeAttribute119', b2)
    if hasattr(b1, 'tupleType'):
        assert not _is_linked(b1, 'tupleType', a)
    if hasattr(b2, 'tupleType'):
        assert _is_linked(b2, 'tupleType', a)
    _safe_set(a, 'TupleTypeAttribute119', None)
    assert not _is_linked(a, 'TupleTypeAttribute119', b2)
    if hasattr(b2, 'tupleType'):
        assert not _is_linked(b2, 'tupleType', a)


def test_assoc_baseExp86_link_reassign_clear():
...
def test_assoc_collectionTypes104_link_reassign_clear():
    a = simpleocl_OclType(name="sample_text")
...
def test_assoc_context_134_link_reassign_clear():
    ...
    _safe_set(a, 'definition135', b1)
    assert _is_linked(a, 'definition135', b1)
    if hasattr(b1, 'OclContextDefinition136'):
        assert _is_linked(b1, 'OclContextDefinition136', a)
    _safe_set(a, 'definition135', b2)
    assert _is_linked(a, 'definition135', b2)
    if hasattr(b1, 'OclContextDefinition136'):
        assert not _is_linked(b1, 'OclContextDefinition136', a)
    if hasattr(b2, 'OclContextDefinition136'):
        assert _is_linked(b2, 'OclContextDefinition136', a)
    _safe_set(a, 'definition135', None)
    assert not _is_linked(a, 'definition135', b2)
    if hasattr(b2, 'OclContextDefinition136'):
        assert not _is_linked(b2, 'OclContextDefinition136', a)


def test_assoc_context_139_link_reassign_clear():
    a = simpleocl_OclType(name="sample_text")
    b1 = simpleocl_OclContextDefinition()
    b2 = simpleocl_OclContextDefinition()
    _safe_set(a, 'OclType140', b1)
    assert _is_linked(a, 'OclType140', b1)
    if hasattr(b1, 'definitions'):
        assert _is_linked(b1, 'definitions', a)
    _safe_set(a, 'OclType140', b2)
    assert _is_linked(a, 'OclType140', b2)
    if hasattr(b1, 'definitions'):
        assert not _is_linked(b1, 'definitions', a)
    if hasattr(b2, 'definitions'):
        assert _is_linked(b2, 'definitions', a)
    _safe_set(a, 'OclType140', None)
    assert not _is_linked(a, 'OclType140', b2)
    if hasattr(b2, 'definitions'):
        assert not _is_linked(b2, 'definitions', a)


def test_assoc_definition137_link_reassign_clear():
    a = simpleocl_OclFeatureDefinition(static="sample_text")
    b1 = simpleocl_OclContextDefinition()
    b2 = simpleocl_OclContextDefinition()
    _safe_set(a, 'OclFeatureDefinition', b1)
    assert _is_linked(a, 'OclFeatureDefinition', b1)
    if hasattr(b1, 'context_138'):
        assert _is_linked(b1, 'context_138', a)
    _safe_set(a, 'OclFeatureDefinition', b2)
    assert _is_linked(a, 'OclFeatureDefinition', b2)
    if hasattr(b1, 'context_138'):
        assert not _is_linked(b1, 'context_138', a)
    if hasattr(b2, 'context_138'):
        assert _is_linked(b2, 'context_138', a)
    _safe_set(a, 'OclFeatureDefinition', None)
    assert not _is_linked(a, 'OclFeatureDefinition', b2)
    if hasattr(b2, 'context_138'):
        assert not _is_linked(b2, 'context_138', a)


def test_assoc_definition141_link_reassign_clear():
    a = simpleocl_OclFeatureDefinition(static="sample_text")
    b1 = simpleocl_OclFeature(eq="sample_text")
    b2 = simpleocl_OclFeature(eq="sample_text_2")
    _safe_set(a, 'OclFeatureDefinition142', b1)
    assert _is_linked(a, 'OclFeatureDefinition142', b1)
    if hasattr(b1, 'feature'):
        assert _is_linked(b1, 'feature', a)
    _safe_set(a, 'OclFeatureDefinition142', b2)
    assert _is_linked(a, 'OclFeatureDefinition142', b2)
    if hasattr(b1, 'feature'):
        assert not _is_linked(b1, 'feature', a)
    if hasattr(b2, 'feature'):
        assert _is_linked(b2, 'feature', a)
    _safe_set(a, 'OclFeatureDefinition142', None)
    assert not _is_linked(a, 'OclFeatureDefinition142', b2)
    if hasattr(b2, 'feature'):
        assert not _is_linked(b2, 'feature', a)


def test_assoc_definitions93_link_reassign_clear():
    a = simpleocl_OclType(name="sample_text")
    b1 = simpleocl_OclContextDefinition()
    b2 = simpleocl_OclContextDefinition()
...
def test_assoc_elementType91_link_reassign_clear():
    a = simpleocl_OclType(name="sample_text")
    b1 = simpleocl_CollectionType()
    b2 = simpleocl_CollectionType()
    _safe_set(a, 'OclType92', b1)
    assert _is_linked(a, 'OclType92', b1)
    if hasattr(b1, 'collectionTypes'):
        assert _is_linked(b1, 'collectionTypes', a)
    _safe_set(a, 'OclType92', b2)
    assert _is_linked(a, 'OclType92', b2)
    if hasattr(b1, 'collectionTypes'):
        assert not _is_linked(b1, 'collectionTypes', a)
    if hasattr(b2, 'collectionTypes'):
        assert _is_linked(b2, 'collectionTypes', a)
    _safe_set(a, 'OclType92', None)
    assert not _is_linked(a, 'OclType92', b2)
    if hasattr(b2, 'collectionTypes'):
        assert not _is_linked(b2, 'collectionTypes', a)


def test_assoc_feature133_link_reassign_clear():
    a = simpleocl_OclFeatureDefinition(static="sample_text")
    b1 = simpleocl_OclFeature(eq="sample_text")
    b2 = simpleocl_OclFeature(eq="sample_text_2")
...
def test_assoc_initExpression84_link_reassign_clear():
    a = simpleocl_LocalVariable(eq="sample_text")
    b1 = simpleocl_OclExpression()
    b2 = simpleocl_OclExpression()
...
def test_assoc_initializedVariable13_link_reassign_clear():
    a = simpleocl_LocalVariable(eq="sample_text")
    b1 = simpleocl_OclExpression()
    b2 = simpleocl_OclExpression()
...
def test_assoc_keyType127_link_reassign_clear():
    a = simpleocl_OclType(name="sample_text")
    b1 = simpleocl_MapType()
    b2 = simpleocl_MapType()
    _safe_set(a, 'OclType128', b1)
    assert _is_linked(a, 'OclType128', b1)
    if hasattr(b1, 'mapType'):
        assert _is_linked(b1, 'mapType', a)
    _safe_set(a, 'OclType128', b2)
    assert _is_linked(a, 'OclType128', b2)
    if hasattr(b1, 'mapType'):
        assert not _is_linked(b1, 'mapType', a)
    if hasattr(b2, 'mapType'):
        assert _is_linked(b2, 'mapType', a)
    _safe_set(a, 'OclType128', None)
    assert not _is_linked(a, 'OclType128', b2)
    if hasattr(b2, 'mapType'):
        assert not _is_linked(b2, 'mapType', a)


def test_assoc_lambdaArgType112_link_reassign_clear():
    a = simpleocl_OclType(name="sample_text")
    b1 = simpleocl_LambdaType()
    b2 = simpleocl_LambdaType()
    _safe_set(a, 'argumentTypes', b1)
    assert _is_linked(a, 'argumentTypes', b1)
    if hasattr(b1, 'LambdaType113'):
        assert _is_linked(b1, 'LambdaType113', a)
    _safe_set(a, 'argumentTypes', b2)
    assert _is_linked(a, 'argumentTypes', b2)
    if hasattr(b1, 'LambdaType113'):
        assert not _is_linked(b1, 'LambdaType113', a)
    if hasattr(b2, 'LambdaType113'):
        assert _is_linked(b2, 'LambdaType113', a)
    _safe_set(a, 'argumentTypes', None)
    assert not _is_linked(a, 'argumentTypes', b2)
    if hasattr(b2, 'LambdaType113'):
        assert not _is_linked(b2, 'LambdaType113', a)


def test_assoc_lambdaReturnType110_link_reassign_clear():
    a = simpleocl_OclType(name="sample_text")
    b1 = simpleocl_LambdaType()
    b2 = simpleocl_LambdaType()
    _safe_set(a, 'returnType111', b1)
    assert _is_linked(a, 'returnType111', b1)
    if hasattr(b1, 'LambdaType'):
        assert _is_linked(b1, 'LambdaType', a)
    _safe_set(a, 'returnType111', b2)
    assert _is_linked(a, 'returnType111', b2)
    if hasattr(b1, 'LambdaType'):
        assert not _is_linked(b1, 'LambdaType', a)
    if hasattr(b2, 'LambdaType'):
        assert _is_linked(b2, 'LambdaType', a)
    _safe_set(a, 'returnType111', None)
    assert not _is_linked(a, 'returnType111', b2)
    if hasattr(b2, 'LambdaType'):
...
def test_assoc_mapType102_link_reassign_clear():
    a = simpleocl_OclType(name="sample_text")
    b1 = simpleocl_MapType()
    b2 = simpleocl_MapType()
    _safe_set(a, 'keyType', b1)
    assert _is_linked(a, 'keyType', b1)
    if hasattr(b1, 'MapType103'):
        assert _is_linked(b1, 'MapType103', a)
    _safe_set(a, 'keyType', b2)
    assert _is_linked(a, 'keyType', b2)
    if hasattr(b1, 'MapType103'):
        assert not _is_linked(b1, 'MapType103', a)
    if hasattr(b2, 'MapType103'):
        assert _is_linked(b2, 'MapType103', a)
    _safe_set(a, 'keyType', None)
    assert not _is_linked(a, 'keyType', b2)
    if hasattr(b2, 'MapType103'):
        assert not _is_linked(b2, 'MapType103', a)


def test_assoc_mapType298_link_reassign_clear():
    a = simpleocl_OclType(name="sample_text")
    b1 = simpleocl_MapType()
    b2 = simpleocl_MapType()
...
def test_assoc_metamodel155_link_reassign_clear():
    ...
    if hasattr(b1, 'model156'):
        assert _is_linked(b1, 'model156', a)
    _safe_set(a, 'OclMetamodel', b2)
    assert _is_linked(a, 'OclMetamodel', b2)
    if hasattr(b1, 'model156'):
        assert not _is_linked(b1, 'model156', a)
    if hasattr(b2, 'model156'):
        assert _is_linked(b2, 'model156', a)
    _safe_set(a, 'OclMetamodel', None)
    assert not _is_linked(a, 'OclMetamodel', b2)
    if hasattr(b2, 'model156'):
        assert not _is_linked(b2, 'model156', a)
...
def test_assoc_model117_link_reassign_clear():
...
def test_assoc_model154_link_reassign_clear():
    a = simpleocl_OclMetamodel(uri="sample_text")
    b1 = simpleocl_OclInstanceModel()
    b2 = simpleocl_OclInstanceModel()
...
def test_assoc_oclExpression94_link_reassign_clear():
    a = simpleocl_OclType(name="sample_text")
    b1 = simpleocl_OclExpression()
    b2 = simpleocl_OclExpression()
    _safe_set(a, 'type', b1)
    assert _is_linked(a, 'type', b1)
    if hasattr(b1, 'OclExpression95'):
        assert _is_linked(b1, 'OclExpression95', a)
    _safe_set(a, 'type', b2)
    assert _is_linked(a, 'type', b2)
    if hasattr(b1, 'OclExpression95'):
        assert not _is_linked(b1, 'OclExpression95', a)
    if hasattr(b2, 'OclExpression95'):
        assert _is_linked(b2, 'OclExpression95', a)
    _safe_set(a, 'type', None)
    assert not _is_linked(a, 'type', b2)
    if hasattr(b2, 'OclExpression95'):
        assert not _is_linked(b2, 'OclExpression95', a)


def test_assoc_operation96_link_reassign_clear():
    a = simpleocl_OclType(name="sample_text")
    b1 = simpleocl_Operation()
    b2 = simpleocl_Operation()
    _safe_set(a, 'returnType', b1)
    assert _is_linked(a, 'returnType', b1)
    if hasattr(b1, 'Operation97'):
        assert _is_linked(b1, 'Operation97', a)
    _safe_set(a, 'returnType', b2)
    assert _is_linked(a, 'returnType', b2)
    if hasattr(b1, 'Operation97'):
        assert not _is_linked(b1, 'Operation97', a)
    if hasattr(b2, 'Operation97'):
        assert _is_linked(b2, 'Operation97', a)
    _safe_set(a, 'returnType', None)
    assert not _is_linked(a, 'returnType', b2)
    if hasattr(b2, 'Operation97'):
        assert not _is_linked(b2, 'Operation97', a)


def test_assoc_parentOperation12_link_reassign_clear():
    a = simpleocl_OperationCall(operationName="sample_text")
    b1 = simpleocl_OclExpression()
    b2 = simpleocl_OclExpression()
...
def test_assoc_referredVariable24_link_reassign_clear():
...
def test_assoc_result66_link_reassign_clear():
    a = simpleocl_LocalVariable(eq="sample_text")
    b1 = simpleocl_IterateExp()
    b2 = simpleocl_IterateExp()
    _safe_set(a, 'LocalVariable67', b1)
    assert _is_linked(a, 'LocalVariable67', b1)
    if hasattr(b1, 'baseExp'):
        assert _is_linked(b1, 'baseExp', a)
    _safe_set(a, 'LocalVariable67', b2)
    assert _is_linked(a, 'LocalVariable67', b2)
    if hasattr(b1, 'baseExp'):
        assert not _is_linked(b1, 'baseExp', a)
    if hasattr(b2, 'baseExp'):
        assert _is_linked(b2, 'baseExp', a)
    _safe_set(a, 'LocalVariable67', None)
    assert not _is_linked(a, 'LocalVariable67', b2)
    if hasattr(b2, 'baseExp'):
        assert not _is_linked(b2, 'baseExp', a)


def test_assoc_returnType129_link_reassign_clear():
    a = simpleocl_OclType(name="sample_text")
    b1 = simpleocl_LambdaType()
    b2 = simpleocl_LambdaType()
    _safe_set(a, 'OclType130', b1)
    assert _is_linked(a, 'OclType130', b1)
    if hasattr(b1, 'lambdaReturnType'):
        assert _is_linked(b1, 'lambdaReturnType', a)
    _safe_set(a, 'OclType130', b2)
    assert _is_linked(a, 'OclType130', b2)
    if hasattr(b1, 'lambdaReturnType'):
        assert not _is_linked(b1, 'lambdaReturnType', a)
    if hasattr(b2, 'lambdaReturnType'):
        assert _is_linked(b2, 'lambdaReturnType', a)
    _safe_set(a, 'OclType130', None)
    assert not _is_linked(a, 'OclType130', b2)
    if hasattr(b2, 'lambdaReturnType'):
        assert not _is_linked(b2, 'lambdaReturnType', a)


def test_assoc_returnType148_link_reassign_clear():
    a = simpleocl_OclType(name="sample_text")
    b1 = simpleocl_Operation()
    b2 = simpleocl_Operation()
    _safe_set(a, 'OclType150', b1)
    assert _is_linked(a, 'OclType150', b1)
    if hasattr(b1, 'operation149'):
        assert _is_linked(b1, 'operation149', a)
    _safe_set(a, 'OclType150', b2)
    assert _is_linked(a, 'OclType150', b2)
    if hasattr(b1, 'operation149'):
        assert not _is_linked(b1, 'operation149', a)
    if hasattr(b2, 'operation149'):
        assert _is_linked(b2, 'operation149', a)
    _safe_set(a, 'OclType150', None)
    assert not _is_linked(a, 'OclType150', b2)
    if hasattr(b2, 'operation149'):
        assert not _is_linked(b2, 'operation149', a)
...
def test_assoc_source57_link_reassign_clear():
    ...
    if hasattr(b1, 'OclExpression58'):
        assert _is_linked(b1, 'OclExpression58', a)
    _safe_set(a, 'appliedOperator', b2)
    assert _is_linked(a, 'appliedOperator', b2)
    if hasattr(b1, 'OclExpression58'):
        assert not _is_linked(b1, 'OclExpression58', a)
    if hasattr(b2, 'OclExpression58'):
        assert _is_linked(b2, 'OclExpression58', a)
    _safe_set(a, 'appliedOperator', None)
    assert not _is_linked(a, 'appliedOperator', b2)
    if hasattr(b2, 'OclExpression58'):
        assert not _is_linked(b2, 'OclExpression58', a)


def test_assoc_staticPropertyCall114_link_reassign_clear():
    a = simpleocl_OclType(name="sample_text")
    b1 = simpleocl_StaticPropertyCallExp()
    b2 = simpleocl_StaticPropertyCallExp()
    _safe_set(a, 'source115', b1)
    assert _is_linked(a, 'source115', b1)
    if hasattr(b1, 'StaticPropertyCallExp116'):
        assert _is_linked(b1, 'StaticPropertyCallExp116', a)
    _safe_set(a, 'source115', b2)
    assert _is_linked(a, 'source115', b2)
    if hasattr(b1, 'StaticPropertyCallExp116'):
        assert not _is_linked(b1, 'StaticPropertyCallExp116', a)
    if hasattr(b2, 'StaticPropertyCallExp116'):
        assert _is_linked(b2, 'StaticPropertyCallExp116', a)
    _safe_set(a, 'source115', None)
    assert not _is_linked(a, 'source115', b2)
    if hasattr(b2, 'StaticPropertyCallExp116'):
        assert not _is_linked(b2, 'StaticPropertyCallExp116', a)


def test_assoc_tupleType122_link_reassign_clear():
    a = simpleocl_TupleTypeAttribute(name="sample_text")
    b1 = simpleocl_TupleType()
    b2 = simpleocl_TupleType()
...
def test_assoc_tupleTypeAttribute105_link_reassign_clear():
    a = simpleocl_TupleTypeAttribute(name="sample_text")
...
def test_assoc_type145_link_reassign_clear():
    ...
    b1 = simpleocl_Attribute()
    b2 = simpleocl_Attribute()
    _safe_set(a, 'OclType146', b1)
    assert _is_linked(a, 'OclType146', b1)
    if hasattr(b1, 'attribute'):
        assert _is_linked(b1, 'attribute', a)
    _safe_set(a, 'OclType146', b2)
    assert _is_linked(a, 'OclType146', b2)
    if hasattr(b1, 'attribute'):
        assert not _is_linked(b1, 'attribute', a)
    if hasattr(b2, 'attribute'):
        assert _is_linked(b2, 'attribute', a)
    _safe_set(a, 'OclType146', None)
    assert not _is_linked(a, 'OclType146', b2)
    if hasattr(b2, 'attribute'):
...
def test_assoc_type79_link_reassign_clear():
    ...
    _safe_set(a, 'variableDeclaration', b1)
    assert _is_linked(a, 'variableDeclaration', b1)
    if hasattr(b1, 'OclType80'):
        assert _is_linked(b1, 'OclType80', a)
    _safe_set(a, 'variableDeclaration', b2)
    assert _is_linked(a, 'variableDeclaration', b2)
    if hasattr(b1, 'OclType80'):
        assert not _is_linked(b1, 'OclType80', a)
    if hasattr(b2, 'OclType80'):
        assert _is_linked(b2, 'OclType80', a)
    _safe_set(a, 'variableDeclaration', None)
    assert not _is_linked(a, 'variableDeclaration', b2)
    if hasattr(b2, 'OclType80'):
        assert not _is_linked(b2, 'OclType80', a)


def test_assoc_valueType125_link_reassign_clear():
    a = simpleocl_OclType(name="sample_text")
    b1 = simpleocl_MapType()
    b2 = simpleocl_MapType()
    _safe_set(a, 'OclType126', b1)
    assert _is_linked(a, 'OclType126', b1)
    if hasattr(b1, 'mapType2'):
        assert _is_linked(b1, 'mapType2', a)
    _safe_set(a, 'OclType126', b2)
    assert _is_linked(a, 'OclType126', b2)
    if hasattr(b1, 'mapType2'):
        assert not _is_linked(b1, 'mapType2', a)
    if hasattr(b2, 'mapType2'):
        assert _is_linked(b2, 'mapType2', a)
    _safe_set(a, 'OclType126', None)
    assert not _is_linked(a, 'OclType126', b2)
    if hasattr(b2, 'mapType2'):
...
def test_assoc_variable68_link_reassign_clear():
    ...
    b1 = simpleocl_LetExp()
    b2 = simpleocl_LetExp()
    _safe_set(a, 'LocalVariable69', b1)
    assert _is_linked(a, 'LocalVariable69', b1)
    if hasattr(b1, 'letExp'):
        assert _is_linked(b1, 'letExp', a)
    _safe_set(a, 'LocalVariable69', b2)
    assert _is_linked(a, 'LocalVariable69', b2)
    if hasattr(b1, 'letExp'):
        assert not _is_linked(b1, 'letExp', a)
    if hasattr(b2, 'letExp'):
        assert _is_linked(b2, 'letExp', a)
    _safe_set(a, 'LocalVariable69', None)
    assert not _is_linked(a, 'LocalVariable69', b2)
    if hasattr(b2, 'letExp'):
        assert not _is_linked(b2, 'letExp', a)


def test_assoc_variableDeclaration107_link_reassign_clear():
    a = simpleocl_VariableDeclaration(varName="sample_text")
    b1 = simpleocl_OclType(name="sample_text")
    b2 = simpleocl_OclType(name="sample_text_2")
    _safe_set(a, 'VariableDeclaration109', b1)
    assert _is_linked(a, 'VariableDeclaration109', b1)
    if hasattr(b1, 'type108'):
        assert _is_linked(b1, 'type108', a)
    _safe_set(a, 'VariableDeclaration109', b2)
    assert _is_linked(a, 'VariableDeclaration109', b2)
    if hasattr(b1, 'type108'):
        assert not _is_linked(b1, 'type108', a)
    if hasattr(b2, 'type108'):
        assert _is_linked(b2, 'type108', a)
    _safe_set(a, 'VariableDeclaration109', None)
    assert not _is_linked(a, 'VariableDeclaration109', b2)
    if hasattr(b2, 'type108'):
        assert not _is_linked(b2, 'type108', a)


def test_assoc_variableExp81_link_reassign_clear():
    a = simpleocl_VariableDeclaration(varName="sample_text")
    b1 = simpleocl_VariableExp()
    b2 = simpleocl_VariableExp()
...
simpleocl_CollectionItem_strategy = st.builds(simpleocl_CollectionItem)
...
simpleocl_CollectionPart_strategy = st.builds(simpleocl_CollectionPart)
...
simpleocl_CollectionRange_strategy = st.builds(simpleocl_CollectionRange)
