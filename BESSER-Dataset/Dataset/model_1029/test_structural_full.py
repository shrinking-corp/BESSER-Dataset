...
def test_assoc_left30_link_reassign_clear():
    ...
    b1 = fsm_AndTrigger()
    b2 = fsm_AndTrigger()
    _safe_set(a, 'fsm_Trigger31', b1)
    assert _is_linked(a, 'fsm_Trigger31', b1)
    if hasattr(b1, 'fsm_AndTrigger'):
        assert _is_linked(b1, 'fsm_AndTrigger', a)
    _safe_set(a, 'fsm_Trigger31', b2)
    assert _is_linked(a, 'fsm_Trigger31', b2)
    if hasattr(b1, 'fsm_AndTrigger'):
        assert not _is_linked(b1, 'fsm_AndTrigger', a)
    if hasattr(b2, 'fsm_AndTrigger'):
        assert _is_linked(b2, 'fsm_AndTrigger', a)
    _safe_set(a, 'fsm_Trigger31', None)
    assert not _is_linked(a, 'fsm_Trigger31', b2)
    if hasattr(b2, 'fsm_AndTrigger'):
...
def test_assoc_left35_link_reassign_clear():
    ...
    b1 = fsm_OrTrigger()
    b2 = fsm_OrTrigger()
    _safe_set(a, 'fsm_Trigger36', b1)
    assert _is_linked(a, 'fsm_Trigger36', b1)
    if hasattr(b1, 'fsm_OrTrigger'):
        assert _is_linked(b1, 'fsm_OrTrigger', a)
    _safe_set(a, 'fsm_Trigger36', b2)
    assert _is_linked(a, 'fsm_Trigger36', b2)
    if hasattr(b1, 'fsm_OrTrigger'):
        assert not _is_linked(b1, 'fsm_OrTrigger', a)
    if hasattr(b2, 'fsm_OrTrigger'):
        assert _is_linked(b2, 'fsm_OrTrigger', a)
    _safe_set(a, 'fsm_Trigger36', None)
    assert not _is_linked(a, 'fsm_Trigger36', b2)
    if hasattr(b2, 'fsm_OrTrigger'):
...
def test_assoc_right32_link_reassign_clear():
    ...
    b1 = fsm_AndTrigger()
    b2 = fsm_AndTrigger()
    _safe_set(a, 'fsm_Trigger34', b1)
    assert _is_linked(a, 'fsm_Trigger34', b1)
    if hasattr(b1, 'fsm_AndTrigger33'):
        assert _is_linked(b1, 'fsm_AndTrigger33', a)
    _safe_set(a, 'fsm_Trigger34', b2)
    assert _is_linked(a, 'fsm_Trigger34', b2)
    if hasattr(b1, 'fsm_AndTrigger33'):
        assert not _is_linked(b1, 'fsm_AndTrigger33', a)
    if hasattr(b2, 'fsm_AndTrigger33'):
        assert _is_linked(b2, 'fsm_AndTrigger33', a)
    _safe_set(a, 'fsm_Trigger34', None)
    assert not _is_linked(a, 'fsm_Trigger34', b2)
    if hasattr(b2, 'fsm_AndTrigger33'):
        assert not _is_linked(b2, 'fsm_AndTrigger33', a)
...
def test_assoc_right37_link_reassign_clear():
    ...
    b1 = fsm_OrTrigger()
    b2 = fsm_OrTrigger()
    _safe_set(a, 'fsm_Trigger39', b1)
    assert _is_linked(a, 'fsm_Trigger39', b1)
    if hasattr(b1, 'fsm_OrTrigger38'):
        assert _is_linked(b1, 'fsm_OrTrigger38', a)
    _safe_set(a, 'fsm_Trigger39', b2)
    assert _is_linked(a, 'fsm_Trigger39', b2)
    if hasattr(b1, 'fsm_OrTrigger38'):
        assert not _is_linked(b1, 'fsm_OrTrigger38', a)
    if hasattr(b2, 'fsm_OrTrigger38'):
        assert _is_linked(b2, 'fsm_OrTrigger38', a)
    _safe_set(a, 'fsm_Trigger39', None)
    assert not _is_linked(a, 'fsm_Trigger39', b2)
    if hasattr(b2, 'fsm_OrTrigger38'):
        assert not _is_linked(b2, 'fsm_OrTrigger38', a)
