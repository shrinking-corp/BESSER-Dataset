...
def test_UML2_ElementImport_visibility_value_roundtrip():
    instance = UML2_ElementImport(visibility="sample_text")
...
def test_assoc_association44_link_reassign_clear():
    a = UML2_Property(aggregation="sample_text", isComposite=True, isDerived=True, isDerivedUnion=True)
    b1 = UML2_Association()
    b2 = UML2_Association()
    _safe_set(a, 'UML2_Property45', b1)
    assert _is_linked(a, 'UML2_Property45', b1)
    if hasattr(b1, 'UML2_Association46'):
        assert _is_linked(b1, 'UML2_Association46', a)
    _safe_set(a, 'UML2_Property45', b2)
    assert _is_linked(a, 'UML2_Property45', b2)
    if hasattr(b1, 'UML2_Association46'):
        assert not _is_linked(b1, 'UML2_Association46', a)
    if hasattr(b2, 'UML2_Association46'):
        assert _is_linked(b2, 'UML2_Association46', a)
    _safe_set(a, 'UML2_Property45', None)
    assert not _is_linked(a, 'UML2_Property45', b2)
    if hasattr(b2, 'UML2_Association46'):
        assert not _is_linked(b2, 'UML2_Association46', a)


def test_assoc_behavior58_link_reassign_clear():
    a = UML2_OpaqueExpression(bodies="sample_text", language="sample_text")
    b1 = UML2_Behavior()
    b2 = UML2_Behavior()
    _safe_set(a, 'UML2_OpaqueExpression', b1)
    assert _is_linked(a, 'UML2_OpaqueExpression', b1)
    if hasattr(b1, 'UML2_Behavior59'):
        assert _is_linked(b1, 'UML2_Behavior59', a)
    _safe_set(a, 'UML2_OpaqueExpression', b2)
    assert _is_linked(a, 'UML2_OpaqueExpression', b2)
    if hasattr(b1, 'UML2_Behavior59'):
        assert not _is_linked(b1, 'UML2_Behavior59', a)
    if hasattr(b2, 'UML2_Behavior59'):
        assert _is_linked(b2, 'UML2_Behavior59', a)
    _safe_set(a, 'UML2_OpaqueExpression', None)
    assert not _is_linked(a, 'UML2_OpaqueExpression', b2)
    if hasattr(b2, 'UML2_Behavior59'):
        assert not _is_linked(b2, 'UML2_Behavior59', a)


def test_assoc_bodyCondition24_link_reassign_clear():
    a = UML2_Operation(isQuery=True)
    b1 = UML2_Constraint()
    b2 = UML2_Constraint()
    _safe_set(a, 'UML2_Operation25', b1)
    assert _is_linked(a, 'UML2_Operation25', b1)
    if hasattr(b1, 'UML2_Constraint'):
        assert _is_linked(b1, 'UML2_Constraint', a)
    _safe_set(a, 'UML2_Operation25', b2)
    assert _is_linked(a, 'UML2_Operation25', b2)
    if hasattr(b1, 'UML2_Constraint'):
        assert not _is_linked(b1, 'UML2_Constraint', a)
    if hasattr(b2, 'UML2_Constraint'):
        assert _is_linked(b2, 'UML2_Constraint', a)
    _safe_set(a, 'UML2_Operation25', None)
    assert not _is_linked(a, 'UML2_Operation25', b2)
    if hasattr(b2, 'UML2_Constraint'):
        assert not _is_linked(b2, 'UML2_Constraint', a)


def test_assoc_classifier2_link_reassign_clear():
...
def test_assoc_classifier7_link_reassign_clear():
    a = UML2_Classifier(isAbstract=True)
    b1 = UML2_InstanceSpecification()
    b2 = UML2_InstanceSpecification()
    _safe_set(a, 'UML2_Classifier9', b1)
    assert _is_linked(a, 'UML2_Classifier9', b1)
    if hasattr(b1, 'UML2_InstanceSpecification8'):
        assert _is_linked(b1, 'UML2_InstanceSpecification8', a)
    _safe_set(a, 'UML2_Classifier9', b2)
    assert _is_linked(a, 'UML2_Classifier9', b2)
    if hasattr(b1, 'UML2_InstanceSpecification8'):
        assert not _is_linked(b1, 'UML2_InstanceSpecification8', a)
    if hasattr(b2, 'UML2_InstanceSpecification8'):
        assert _is_linked(b2, 'UML2_InstanceSpecification8', a)
    _safe_set(a, 'UML2_Classifier9', None)
    assert not _is_linked(a, 'UML2_Classifier9', b2)
    if hasattr(b2, 'UML2_InstanceSpecification8'):
        assert not _is_linked(b2, 'UML2_InstanceSpecification8', a)


def test_assoc_context_17_link_reassign_clear():
    a = UML2_Classifier(isAbstract=True)
    b1 = UML2_Action()
    b2 = UML2_Action()
    _safe_set(a, 'UML2_Classifier18', b1)
    assert _is_linked(a, 'UML2_Classifier18', b1)
    if hasattr(b1, 'UML2_Action'):
        assert _is_linked(b1, 'UML2_Action', a)
    _safe_set(a, 'UML2_Classifier18', b2)
    assert _is_linked(a, 'UML2_Classifier18', b2)
    if hasattr(b1, 'UML2_Action'):
        assert not _is_linked(b1, 'UML2_Action', a)
    if hasattr(b2, 'UML2_Action'):
        assert _is_linked(b2, 'UML2_Action', a)
    _safe_set(a, 'UML2_Classifier18', None)
    assert not _is_linked(a, 'UML2_Classifier18', b2)
    if hasattr(b2, 'UML2_Action'):
        assert not _is_linked(b2, 'UML2_Action', a)


def test_assoc_definingFeature37_link_reassign_clear():
    a = UML2_StructuralFeature(isReadOnly=True)
    b1 = UML2_Slot()
    b2 = UML2_Slot()
    _safe_set(a, 'UML2_StructuralFeature39', b1)
    assert _is_linked(a, 'UML2_StructuralFeature39', b1)
    if hasattr(b1, 'UML2_Slot38'):
        assert _is_linked(b1, 'UML2_Slot38', a)
    _safe_set(a, 'UML2_StructuralFeature39', b2)
    assert _is_linked(a, 'UML2_StructuralFeature39', b2)
    if hasattr(b1, 'UML2_Slot38'):
        assert not _is_linked(b1, 'UML2_Slot38', a)
    if hasattr(b2, 'UML2_Slot38'):
        assert _is_linked(b2, 'UML2_Slot38', a)
    _safe_set(a, 'UML2_StructuralFeature39', None)
    assert not _is_linked(a, 'UML2_StructuralFeature39', b2)
    if hasattr(b2, 'UML2_Slot38'):
        assert not _is_linked(b2, 'UML2_Slot38', a)


def test_assoc_end71_link_reassign_clear():
    a = UML2_Property(aggregation="sample_text", isComposite=True, isDerived=True, isDerivedUnion=True)
    b1 = UML2_LinkEndData()
    b2 = UML2_LinkEndData()
    _safe_set(a, 'UML2_Property73', b1)
    assert _is_linked(a, 'UML2_Property73', b1)
    if hasattr(b1, 'UML2_LinkEndData72'):
        assert _is_linked(b1, 'UML2_LinkEndData72', a)
    _safe_set(a, 'UML2_Property73', b2)
    assert _is_linked(a, 'UML2_Property73', b2)
    if hasattr(b1, 'UML2_LinkEndData72'):
        assert not _is_linked(b1, 'UML2_LinkEndData72', a)
    if hasattr(b2, 'UML2_LinkEndData72'):
        assert _is_linked(b2, 'UML2_LinkEndData72', a)
    _safe_set(a, 'UML2_Property73', None)
    assert not _is_linked(a, 'UML2_Property73', b2)
    if hasattr(b2, 'UML2_LinkEndData72'):
        assert not _is_linked(b2, 'UML2_LinkEndData72', a)


def test_assoc_feature77_link_reassign_clear():
    a = UML2_Feature(isStatic=True)
    b1 = UML2_Classifier(isAbstract=True)
    b2 = UML2_Classifier(isAbstract=False)
    _safe_set(a, 'UML2_Feature79', b1)
    assert _is_linked(a, 'UML2_Feature79', b1)
    if hasattr(b1, 'UML2_Classifier78'):
        assert _is_linked(b1, 'UML2_Classifier78', a)
    _safe_set(a, 'UML2_Feature79', b2)
    assert _is_linked(a, 'UML2_Feature79', b2)
    if hasattr(b1, 'UML2_Classifier78'):
        assert not _is_linked(b1, 'UML2_Classifier78', a)
    if hasattr(b2, 'UML2_Classifier78'):
        assert _is_linked(b2, 'UML2_Classifier78', a)
    _safe_set(a, 'UML2_Feature79', None)
    assert not _is_linked(a, 'UML2_Feature79', b2)
    if hasattr(b2, 'UML2_Classifier78'):
...
def test_assoc_featuringClassifier67_link_reassign_clear():
    ...
    if hasattr(b1, 'UML2_Classifier68'):
        assert _is_linked(b1, 'UML2_Classifier68', a)
    _safe_set(a, 'UML2_Feature', {b2})
    assert _is_linked(a, 'UML2_Feature', b2)
    if hasattr(b1, 'UML2_Classifier68'):
        assert not _is_linked(b1, 'UML2_Classifier68', a)
    if hasattr(b2, 'UML2_Classifier68'):
        assert _is_linked(b2, 'UML2_Classifier68', a)
    _safe_set(a, 'UML2_Feature', set())
    assert not _is_linked(a, 'UML2_Feature', b2)
    if hasattr(b2, 'UML2_Classifier68'):
        assert not _is_linked(b2, 'UML2_Classifier68', a)
...
def test_assoc_general15_link_reassign_clear():
    ...
    _safe_set(a, 'UML2_Classifier16', b1)
    assert _is_linked(a, 'UML2_Classifier16', b1)
    if hasattr(b1, 'UML2_Generalization'):
        assert _is_linked(b1, 'UML2_Generalization', a)
    _safe_set(a, 'UML2_Classifier16', b2)
    assert _is_linked(a, 'UML2_Classifier16', b2)
    if hasattr(b1, 'UML2_Generalization'):
        assert not _is_linked(b1, 'UML2_Generalization', a)
    if hasattr(b2, 'UML2_Generalization'):
        assert _is_linked(b2, 'UML2_Generalization', a)
    _safe_set(a, 'UML2_Classifier16', None)
    assert not _is_linked(a, 'UML2_Classifier16', b2)
    if hasattr(b2, 'UML2_Generalization'):
        assert not _is_linked(b2, 'UML2_Generalization', a)


def test_assoc_generalization83_link_reassign_clear():
    a = UML2_Classifier(isAbstract=True)
    b1 = UML2_Generalization()
    b2 = UML2_Generalization()
    _safe_set(a, 'UML2_Classifier84', {b1})
    assert _is_linked(a, 'UML2_Classifier84', b1)
    if hasattr(b1, 'UML2_Generalization85'):
        assert _is_linked(b1, 'UML2_Generalization85', a)
    _safe_set(a, 'UML2_Classifier84', {b2})
    assert _is_linked(a, 'UML2_Classifier84', b2)
    if hasattr(b1, 'UML2_Generalization85'):
        assert not _is_linked(b1, 'UML2_Generalization85', a)
    if hasattr(b2, 'UML2_Generalization85'):
        assert _is_linked(b2, 'UML2_Generalization85', a)
    _safe_set(a, 'UML2_Classifier84', set())
    assert not _is_linked(a, 'UML2_Classifier84', b2)
    if hasattr(b2, 'UML2_Generalization85'):
        assert not _is_linked(b2, 'UML2_Generalization85', a)


def test_assoc_importedElement5_link_reassign_clear():
    a = UML2_ElementImport(visibility="sample_text")
    b1 = UML2_PackageableElement()
    b2 = UML2_PackageableElement()
    _safe_set(a, 'UML2_ElementImport', b1)
    assert _is_linked(a, 'UML2_ElementImport', b1)
    if hasattr(b1, 'UML2_PackageableElement'):
        assert _is_linked(b1, 'UML2_PackageableElement', a)
    _safe_set(a, 'UML2_ElementImport', b2)
    assert _is_linked(a, 'UML2_ElementImport', b2)
    if hasattr(b1, 'UML2_PackageableElement'):
        assert not _is_linked(b1, 'UML2_PackageableElement', a)
    if hasattr(b2, 'UML2_PackageableElement'):
        assert _is_linked(b2, 'UML2_PackageableElement', a)
    _safe_set(a, 'UML2_ElementImport', None)
    assert not _is_linked(a, 'UML2_ElementImport', b2)
    if hasattr(b2, 'UML2_PackageableElement'):
        assert not _is_linked(b2, 'UML2_PackageableElement', a)
...
def test_assoc_inheritedMember80_link_reassign_clear():
    ...
    b1 = UML2_Classifier(isAbstract=True)
    b2 = UML2_Classifier(isAbstract=False)
    _safe_set(a, 'UML2_NamedElement82', b1)
    assert _is_linked(a, 'UML2_NamedElement82', b1)
    if hasattr(b1, 'UML2_Classifier81'):
        assert _is_linked(b1, 'UML2_Classifier81', a)
    _safe_set(a, 'UML2_NamedElement82', b2)
    assert _is_linked(a, 'UML2_NamedElement82', b2)
    if hasattr(b1, 'UML2_Classifier81'):
        assert not _is_linked(b1, 'UML2_Classifier81', a)
    if hasattr(b2, 'UML2_Classifier81'):
        assert _is_linked(b2, 'UML2_Classifier81', a)
    _safe_set(a, 'UML2_NamedElement82', None)
    assert not _is_linked(a, 'UML2_NamedElement82', b2)
    if hasattr(b2, 'UML2_Classifier81'):
        assert not _is_linked(b2, 'UML2_Classifier81', a)


def test_assoc_member57_link_reassign_clear():
    a = UML2_NamedElement(name="sample_text", visibility="sample_text")
    b1 = UML2_Namespace()
    b2 = UML2_Namespace()
...
def test_assoc_memberEnd28_link_reassign_clear():
    a = UML2_Property(aggregation="sample_text", isComposite=True, isDerived=True, isDerivedUnion=True)
    b1 = UML2_Association()
    b2 = UML2_Association()
    _safe_set(a, 'UML2_Property30', b1)
    assert _is_linked(a, 'UML2_Property30', b1)
    if hasattr(b1, 'UML2_Association29'):
        assert _is_linked(b1, 'UML2_Association29', a)
    _safe_set(a, 'UML2_Property30', b2)
    assert _is_linked(a, 'UML2_Property30', b2)
    if hasattr(b1, 'UML2_Association29'):
        assert not _is_linked(b1, 'UML2_Association29', a)
    if hasattr(b2, 'UML2_Association29'):
        assert _is_linked(b2, 'UML2_Association29', a)
    _safe_set(a, 'UML2_Property30', None)
    assert not _is_linked(a, 'UML2_Property30', b2)
    if hasattr(b2, 'UML2_Association29'):
        assert not _is_linked(b2, 'UML2_Association29', a)


def test_assoc_nestedClassifier19_link_reassign_clear():
    a = UML2_Classifier(isAbstract=True)
    b1 = UML2_Class(isActive=True)
    b2 = UML2_Class(isActive=False)
    _safe_set(a, 'UML2_Classifier20', b1)
    assert _is_linked(a, 'UML2_Classifier20', b1)
    if hasattr(b1, 'UML2_Class'):
        assert _is_linked(b1, 'UML2_Class', a)
    _safe_set(a, 'UML2_Classifier20', b2)
    assert _is_linked(a, 'UML2_Classifier20', b2)
    if hasattr(b1, 'UML2_Class'):
        assert not _is_linked(b1, 'UML2_Class', a)
    if hasattr(b2, 'UML2_Class'):
        assert _is_linked(b2, 'UML2_Class', a)
    _safe_set(a, 'UML2_Classifier20', None)
    assert not _is_linked(a, 'UML2_Classifier20', b2)
    if hasattr(b2, 'UML2_Class'):
        assert not _is_linked(b2, 'UML2_Class', a)


def test_assoc_ownedEnd26_link_reassign_clear():
    a = UML2_Property(aggregation="sample_text", isComposite=True, isDerived=True, isDerivedUnion=True)
    b1 = UML2_Association()
    b2 = UML2_Association()
    _safe_set(a, 'UML2_Property27', b1)
    assert _is_linked(a, 'UML2_Property27', b1)
    if hasattr(b1, 'UML2_Association'):
        assert _is_linked(b1, 'UML2_Association', a)
    _safe_set(a, 'UML2_Property27', b2)
    assert _is_linked(a, 'UML2_Property27', b2)
    if hasattr(b1, 'UML2_Association'):
        assert not _is_linked(b1, 'UML2_Association', a)
    if hasattr(b2, 'UML2_Association'):
        assert _is_linked(b2, 'UML2_Association', a)
    _safe_set(a, 'UML2_Property27', None)
    assert not _is_linked(a, 'UML2_Property27', b2)
    if hasattr(b2, 'UML2_Association'):
        assert not _is_linked(b2, 'UML2_Association', a)


def test_assoc_ownedParameter23_link_reassign_clear():
...
def test_assoc_ownedReception21_link_reassign_clear():
    a = UML2_Class(isActive=True)
    b1 = UML2_Reception()
    b2 = UML2_Reception()
    _safe_set(a, 'UML2_Class22', {b1})
    assert _is_linked(a, 'UML2_Class22', b1)
    if hasattr(b1, 'UML2_Reception'):
        assert _is_linked(b1, 'UML2_Reception', a)
    _safe_set(a, 'UML2_Class22', {b2})
    assert _is_linked(a, 'UML2_Class22', b2)
    if hasattr(b1, 'UML2_Reception'):
        assert not _is_linked(b1, 'UML2_Reception', a)
    if hasattr(b2, 'UML2_Reception'):
        assert _is_linked(b2, 'UML2_Reception', a)
    _safe_set(a, 'UML2_Class22', set())
    assert not _is_linked(a, 'UML2_Class22', b2)
    if hasattr(b2, 'UML2_Reception'):
        assert not _is_linked(b2, 'UML2_Reception', a)


def test_assoc_qualifier10_link_reassign_clear():
    a = UML2_Property(aggregation="sample_text", isComposite=True, isDerived=True, isDerivedUnion=True)
...
def test_assoc_qualifier48_link_reassign_clear():
    ...
    _safe_set(a, 'UML2_Property47', {b1})
    assert _is_linked(a, 'UML2_Property47', b1)
    if hasattr(b1, 'UML2_Property49'):
        assert _is_linked(b1, 'UML2_Property49', a)
    _safe_set(a, 'UML2_Property47', {b2})
    assert _is_linked(a, 'UML2_Property47', b2)
    if hasattr(b1, 'UML2_Property49'):
        assert not _is_linked(b1, 'UML2_Property49', a)
    if hasattr(b2, 'UML2_Property49'):
        assert _is_linked(b2, 'UML2_Property49', a)
    _safe_set(a, 'UML2_Property47', set())
    assert not _is_linked(a, 'UML2_Property47', b2)
    if hasattr(b2, 'UML2_Property49'):
        assert not _is_linked(b2, 'UML2_Property49', a)


def test_assoc_structuralFeature31_link_reassign_clear():
    a = UML2_StructuralFeature(isReadOnly=True)
...
def test_assoc_subsettedProperty42_link_reassign_clear():
    a = UML2_Property(aggregation="sample_text", isComposite=True, isDerived=True, isDerivedUnion=True)
    b1 = UML2_Property(aggregation="sample_text", isComposite=True, isDerived=True, isDerivedUnion=True)
    b2 = UML2_Property(aggregation="sample_text_2", isComposite=False, isDerived=False, isDerivedUnion=False)
    _safe_set(a, 'UML2_Property41', {b1})
    assert _is_linked(a, 'UML2_Property41', b1)
    if hasattr(b1, 'UML2_Property43'):
        assert _is_linked(b1, 'UML2_Property43', a)
    _safe_set(a, 'UML2_Property41', {b2})
    assert _is_linked(a, 'UML2_Property41', b2)
    if hasattr(b1, 'UML2_Property43'):
        assert not _is_linked(b1, 'UML2_Property43', a)
    if hasattr(b2, 'UML2_Property43'):
        assert _is_linked(b2, 'UML2_Property43', a)
    _safe_set(a, 'UML2_Property41', set())
    assert not _is_linked(a, 'UML2_Property41', b2)
    if hasattr(b2, 'UML2_Property43'):
        assert not _is_linked(b2, 'UML2_Property43', a)
...
UML2_ElementImport_strategy = st.builds(UML2_ElementImport, visibility=safe_text)
