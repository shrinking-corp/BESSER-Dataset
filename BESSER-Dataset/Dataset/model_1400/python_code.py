...
class fsmcore_Statement(ABC):
    ...
    def fsmcore_Statement(self, value):
        ...
        if old_value is not None:
            if hasattr(old_value, "fsmcore_Program24"):
                opp_val = getattr(old_value, "fsmcore_Program24", None)
                if isinstance(opp_val, set):
                    opp_val.discard(self)
                
        # Add self to new opposite end
        if value is not None:
            if hasattr(value, "fsmcore_Program24"):
                opp_val = getattr(value, "fsmcore_Program24", None)
                if opp_val is None:
                    setattr(value, "fsmcore_Program24", set([self]))
                elif isinstance(opp_val, set):
                    opp_val.add(self)

    def eval(self, fsmcore_context):
...
class fsmcore_Constraint:
    ...
    def evalConstraint(self, fsmcore_context):
...
class fsmcore_Trigger:
    ...
    def fsmcore_Trigger(self, value):
        ...
        # Remove self from old opposite end
        if old_value is not None:
            if hasattr(old_value, "fsmcore_Transition16"):
                opp_val = getattr(old_value, "fsmcore_Transition16", None)
                if opp_val == self:
                    setattr(old_value, "fsmcore_Transition16", None)
                
        # Add self to new opposite end
        if value is not None:
            if hasattr(value, "fsmcore_Transition16"):
                opp_val = getattr(value, "fsmcore_Transition16", None)
                setattr(value, "fsmcore_Transition16", self)

class fsmcore_Program:

    def __init__(self, fsmcore_Program: "fsmcore_State" = None, fsmcore_Program11: "fsmcore_State" = None, fsmcore_Program14: "fsmcore_State" = None, fsmcore_Program24: set["fsmcore_Statement"] = None):
        self.fsmcore_Program = fsmcore_Program
        self.fsmcore_Program11 = fsmcore_Program11
        self.fsmcore_Program14 = fsmcore_Program14
        self.fsmcore_Program24 = fsmcore_Program24 if fsmcore_Program24 is not None else set()
        
        pass
    @property
    def fsmcore_Program11(self):
        return self.__fsmcore_Program11

    @fsmcore_Program11.setter
    def fsmcore_Program11(self, value):
        # Bidirectional consistency
        old_value = getattr(self, f"_fsmcore_Program__fsmcore_Program11", None)
        self.__fsmcore_Program11 = value
        
        # Remove self from old opposite end
        if old_value is not None:
            if hasattr(old_value, "fsmcore_State10"):
                opp_val = getattr(old_value, "fsmcore_State10", None)
                if opp_val == self:
                    setattr(old_value, "fsmcore_State10", None)
                
        # Add self to new opposite end
        if value is not None:
            if hasattr(value, "fsmcore_State10"):
                opp_val = getattr(value, "fsmcore_State10", None)
                setattr(value, "fsmcore_State10", self)
    ...
    @property
    def fsmcore_Program14(self):
        return self.__fsmcore_Program14

    @fsmcore_Program14.setter
    def fsmcore_Program14(self, value):
        # Bidirectional consistency
        old_value = getattr(self, f"_fsmcore_Program__fsmcore_Program14", None)
        self.__fsmcore_Program14 = value
        
        # Remove self from old opposite end
        if old_value is not None:
            if hasattr(old_value, "fsmcore_State13"):
                opp_val = getattr(old_value, "fsmcore_State13", None)
                if opp_val == self:
                    setattr(old_value, "fsmcore_State13", None)
                
        # Add self to new opposite end
        if value is not None:
            if hasattr(value, "fsmcore_State13"):
                opp_val = getattr(value, "fsmcore_State13", None)
                setattr(value, "fsmcore_State13", self)

    @property
    def fsmcore_Program24(self):
        return self.__fsmcore_Program24

    @fsmcore_Program24.setter
    def fsmcore_Program24(self, value):
        # Bidirectional consistency
        old_value = getattr(self, f"_fsmcore_Program__fsmcore_Program24", None)
        self.__fsmcore_Program24 = value if value is not None else set()
