...
def test_customer_has_primaryMemberDateOfBirth():
    assert hasattr(Customer, "primaryMemberDateOfBirth")
    descriptor = None
    for klass in Customer.__mro__:
        if "primaryMemberDateOfBirth" in klass.__dict__:
            descriptor = klass.__dict__["primaryMemberDateOfBirth"]
            break
    assert isinstance(descriptor, property)
...
@given(instance=Customer_strategy)
def test_customer_primaryMemberDateOfBirth_setter(instance):
