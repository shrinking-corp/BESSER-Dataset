...
def test_hyp_customer_constructor_args():
    ...
    params = list(sig.parameters.keys())
    assert "primaryMemberDateOfBirth" in params, "Missing parameter 'primaryMemberDateOfBirth'"
...
def test_hyp_customer_primaryMemberDateOfBirth_setter(instance):
    original = instance.primaryMemberDateOfBirth
    instance.primaryMemberDateOfBirth = original
    assert instance.primaryMemberDateOfBirth == original
...
def test_Customer_primaryMemberDateOfBirth_value_roundtrip():
    instance = Customer(primaryMemberDateOfBirth=date(2024, 1, 1))
    assert instance.primaryMemberDateOfBirth == date(2024, 1, 1)
    instance.primaryMemberDateOfBirth = date(2025, 6, 15)
    assert instance.primaryMemberDateOfBirth == date(2025, 6, 15)
...
Customer_strategy = st.builds(Customer, primaryMemberDateOfBirth=st.dates())
@given(instance=Customer_strategy)
@settings(max_examples=25)
