...
def test_hyp_company_phonebookentry_is_not_abstract():
    assert not inspect.isabstract(company_PhonebookEntry)


def test_hyp_company_phonebookentry_constructor_exists():
    assert callable(company_PhonebookEntry.__init__)


def test_hyp_company_phonebookentry_constructor_args():
    sig = inspect.signature(company_PhonebookEntry.__init__)
    params = list(sig.parameters.keys())
...
company_PhonebookEntry_strategy = st.builds(
    company_PhonebookEntry,
)
...
def test_assoc_departments6_link_reassign_clear():
    a = company_Department(number=7)
    b1 = company_Company(name="sample_text")
    b2 = company_Company(name="sample_text_2")
    _safe_set(a, 'company_Department7', b1)
    assert _is_linked(a, 'company_Department7', b1)
    if hasattr(b1, 'company_Company'):
        assert _is_linked(b1, 'company_Company', a)
    _safe_set(a, 'company_Department7', b2)
    assert _is_linked(a, 'company_Department7', b2)
    if hasattr(b1, 'company_Company'):
        assert not _is_linked(b1, 'company_Company', a)
    if hasattr(b2, 'company_Company'):
        assert _is_linked(b2, 'company_Company', a)
    _safe_set(a, 'company_Department7', None)
    assert not _is_linked(a, 'company_Department7', b2)
    if hasattr(b2, 'company_Company'):
...
def test_assoc_employees1_link_reassign_clear():
    ...
    _safe_set(a, 'company_Employee2', b1)
    assert _is_linked(a, 'company_Employee2', b1)
    if hasattr(b1, 'company_Department'):
        assert _is_linked(b1, 'company_Department', a)
    _safe_set(a, 'company_Employee2', b2)
    assert _is_linked(a, 'company_Employee2', b2)
    if hasattr(b1, 'company_Department'):
        assert not _is_linked(b1, 'company_Department', a)
    if hasattr(b2, 'company_Department'):
        assert _is_linked(b2, 'company_Department', a)
    _safe_set(a, 'company_Employee2', None)
    assert not _is_linked(a, 'company_Employee2', b2)
    if hasattr(b2, 'company_Department'):
...
def test_assoc_phonebookEntry0_link_reassign_clear():
    a = company_Employee(name="sample_text")
    b1 = company_PhonebookEntry()
    b2 = company_PhonebookEntry()
    _safe_set(a, 'company_Employee', b1)
    assert _is_linked(a, 'company_Employee', b1)
    if hasattr(b1, 'company_PhonebookEntry'):
        assert _is_linked(b1, 'company_PhonebookEntry', a)
    _safe_set(a, 'company_Employee', b2)
    assert _is_linked(a, 'company_Employee', b2)
    if hasattr(b1, 'company_PhonebookEntry'):
        assert not _is_linked(b1, 'company_PhonebookEntry', a)
    if hasattr(b2, 'company_PhonebookEntry'):
        assert _is_linked(b2, 'company_PhonebookEntry', a)
    _safe_set(a, 'company_Employee', None)
    assert not _is_linked(a, 'company_Employee', b2)
    if hasattr(b2, 'company_PhonebookEntry'):
        assert not _is_linked(b2, 'company_PhonebookEntry', a)
...
company_PhonebookEntry_strategy = st.builds(company_PhonebookEntry)
@given(instance=company_PhonebookEntry_strategy)
@settings(max_examples=25)
def test_company_PhonebookEntry_instantiation(instance):
    assert isinstance(instance, company_PhonebookEntry)
