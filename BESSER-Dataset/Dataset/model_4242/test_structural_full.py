...
def test_assoc_du10_link_reassign_clear():
    a = company_Dept(name="sample_text")
    b1 = company_Subunit()
    b2 = company_Subunit()
    _safe_set(a, 'company_Dept12', b1)
    assert _is_linked(a, 'company_Dept12', b1)
    if hasattr(b1, 'company_Subunit11'):
        assert _is_linked(b1, 'company_Subunit11', a)
    _safe_set(a, 'company_Dept12', b2)
    assert _is_linked(a, 'company_Dept12', b2)
    if hasattr(b1, 'company_Subunit11'):
        assert not _is_linked(b1, 'company_Subunit11', a)
    if hasattr(b2, 'company_Subunit11'):
        assert _is_linked(b2, 'company_Subunit11', a)
    _safe_set(a, 'company_Dept12', None)
    assert not _is_linked(a, 'company_Dept12', b2)
    if hasattr(b2, 'company_Subunit11'):
        assert not _is_linked(b2, 'company_Subunit11', a)
...
def test_assoc_pu7_link_reassign_clear():
    a = company_Employee(salary=3.14)
    b1 = company_Subunit()
    b2 = company_Subunit()
    _safe_set(a, 'company_Employee9', b1)
    assert _is_linked(a, 'company_Employee9', b1)
    if hasattr(b1, 'company_Subunit8'):
        assert _is_linked(b1, 'company_Subunit8', a)
    _safe_set(a, 'company_Employee9', b2)
    assert _is_linked(a, 'company_Employee9', b2)
    if hasattr(b1, 'company_Subunit8'):
        assert not _is_linked(b1, 'company_Subunit8', a)
    if hasattr(b2, 'company_Subunit8'):
        assert _is_linked(b2, 'company_Subunit8', a)
    _safe_set(a, 'company_Employee9', None)
    assert not _is_linked(a, 'company_Employee9', b2)
    if hasattr(b2, 'company_Subunit8'):
        assert not _is_linked(b2, 'company_Subunit8', a)
