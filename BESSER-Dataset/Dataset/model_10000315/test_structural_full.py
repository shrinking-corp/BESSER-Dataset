...
def test_assoc_Message_Admin_link_reassign_clear():
    a = Message(message="sample_text", reciver="sample_text", sender="sample_text")
    b1 = Admin()
    b2 = Admin()
    _safe_set(a, 'admin20', b1)
    assert _is_linked(a, 'admin20', b1)
    if hasattr(b1, 'message21'):
        assert _is_linked(b1, 'message21', a)
    _safe_set(a, 'admin20', b2)
    assert _is_linked(a, 'admin20', b2)
    if hasattr(b1, 'message21'):
        assert not _is_linked(b1, 'message21', a)
    if hasattr(b2, 'message21'):
        assert _is_linked(b2, 'message21', a)
    _safe_set(a, 'admin20', None)
    assert not _is_linked(a, 'admin20', b2)
    if hasattr(b2, 'message21'):
        assert not _is_linked(b2, 'message21', a)
