...
def test_hyp_a_constructor_args():
    ...
    params = list(sig.parameters.keys())
    assert "atttA" in params, "Missing parameter 'atttA'"
...
def test_hyp_a_atttA_setter(instance):
    original = instance.atttA
    instance.atttA = original
    assert instance.atttA == original
...
def test_A_atttA_value_roundtrip():
    instance = A(atttA="sample_text")
    assert instance.atttA == "sample_text"
    instance.atttA = "sample_text_2"
    assert instance.atttA == "sample_text_2"
...
def test_B1_attB_value_roundtrip():
    instance = B1(attB=True)
    assert instance.attB == True
    instance.attB = False
    assert instance.attB == False
...
def test_C1_attC1_value_roundtrip():
    instance = C1(attC1=True, attC2=True)
    assert instance.attC1 == True
    instance.attC1 = False
    assert instance.attC1 == False
...
def test_assoc_A_B_link_reassign_clear():
    a = B(attB=7)
    b1 = A(atttA="sample_text")
    b2 = A(atttA="sample_text_2")
    _safe_set(a, 'a3', b1)
...
A_strategy = st.builds(A, atttA=safe_text)
@given(instance=A_strategy)
...
B1_strategy = st.builds(B1, attB=st.booleans())
@given(instance=B1_strategy)
@settings(max_examples=25)
...
C1_strategy = st.builds(C1, attC1=st.booleans(), attC2=st.booleans())
@given(instance=C1_strategy)
@settings(max_examples=25)
