...
def test_assoc_arcs2_link_reassign_clear():
    a = PetriNet_PetriNet(name="sample_text")
    b1 = Arc()
    b2 = Arc()
    _safe_set(a, 'PetriNet_PetriNet3', {b1})
    assert _is_linked(a, 'PetriNet_PetriNet3', b1)
    if hasattr(b1, 'Arc'):
        assert _is_linked(b1, 'Arc', a)
    _safe_set(a, 'PetriNet_PetriNet3', {b2})
    assert _is_linked(a, 'PetriNet_PetriNet3', b2)
    if hasattr(b1, 'Arc'):
        assert not _is_linked(b1, 'Arc', a)
    if hasattr(b2, 'Arc'):
        assert _is_linked(b2, 'Arc', a)
    _safe_set(a, 'PetriNet_PetriNet3', set())
    assert not _is_linked(a, 'PetriNet_PetriNet3', b2)
    if hasattr(b2, 'Arc'):
...
def test_assoc_incoming7_link_reassign_clear():
    ...
    b1 = PlaceToTransArc()
    b2 = PlaceToTransArc()
    _safe_set(a, 'target8', {b1})
    assert _is_linked(a, 'target8', b1)
    if hasattr(b1, 'PlaceToTransArc9'):
        assert _is_linked(b1, 'PlaceToTransArc9', a)
    _safe_set(a, 'target8', {b2})
    assert _is_linked(a, 'target8', b2)
    if hasattr(b1, 'PlaceToTransArc9'):
        assert not _is_linked(b1, 'PlaceToTransArc9', a)
    if hasattr(b2, 'PlaceToTransArc9'):
        assert _is_linked(b2, 'PlaceToTransArc9', a)
    _safe_set(a, 'target8', set())
    assert not _is_linked(a, 'target8', b2)
    if hasattr(b2, 'PlaceToTransArc9'):
        assert not _is_linked(b2, 'PlaceToTransArc9', a)
...
def test_assoc_outgoing10_link_reassign_clear():
    a = PetriNet_Transition(name="sample_text")
    b1 = TransToPlaceArc()
    b2 = TransToPlaceArc()
    _safe_set(a, 'source11', {b1})
    assert _is_linked(a, 'source11', b1)
    if hasattr(b1, 'TransToPlaceArc12'):
        assert _is_linked(b1, 'TransToPlaceArc12', a)
    _safe_set(a, 'source11', {b2})
    assert _is_linked(a, 'source11', b2)
    if hasattr(b1, 'TransToPlaceArc12'):
        assert not _is_linked(b1, 'TransToPlaceArc12', a)
    if hasattr(b2, 'TransToPlaceArc12'):
        assert _is_linked(b2, 'TransToPlaceArc12', a)
    _safe_set(a, 'source11', set())
    assert not _is_linked(a, 'source11', b2)
    if hasattr(b2, 'TransToPlaceArc12'):
        assert not _is_linked(b2, 'TransToPlaceArc12', a)
