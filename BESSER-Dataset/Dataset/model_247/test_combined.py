...
def test_assoc_eAnnotations50_link_reassign_clear():
...
def test_assoc_eBounds109_link_reassign_clear():
    a = ecore_EGenericType()
    b1 = ecore_ETypeParameter()
    b2 = ecore_ETypeParameter()
    _safe_set(a, 'ecore_EGenericType111', b1)
    assert _is_linked(a, 'ecore_EGenericType111', b1)
    if hasattr(b1, 'ecore_ETypeParameter110'):
        assert _is_linked(b1, 'ecore_ETypeParameter110', a)
    _safe_set(a, 'ecore_EGenericType111', b2)
    assert _is_linked(a, 'ecore_EGenericType111', b2)
    if hasattr(b1, 'ecore_ETypeParameter110'):
        assert not _is_linked(b1, 'ecore_ETypeParameter110', a)
    if hasattr(b2, 'ecore_ETypeParameter110'):
        assert _is_linked(b2, 'ecore_ETypeParameter110', a)
    _safe_set(a, 'ecore_EGenericType111', None)
    assert not _is_linked(a, 'ecore_EGenericType111', b2)
    if hasattr(b2, 'ecore_ETypeParameter110'):
        assert not _is_linked(b2, 'ecore_ETypeParameter110', a)
...
def test_assoc_eClassifier106_link_reassign_clear():
    ...
    if hasattr(b1, 'ecore_EClassifier108'):
        assert _is_linked(b1, 'ecore_EClassifier108', a)
    _safe_set(a, 'ecore_EGenericType107', b2)
    assert _is_linked(a, 'ecore_EGenericType107', b2)
    if hasattr(b1, 'ecore_EClassifier108'):
        assert not _is_linked(b1, 'ecore_EClassifier108', a)
    if hasattr(b2, 'ecore_EClassifier108'):
        assert _is_linked(b2, 'ecore_EClassifier108', a)
    _safe_set(a, 'ecore_EGenericType107', None)
    assert not _is_linked(a, 'ecore_EGenericType107', b2)
    if hasattr(b2, 'ecore_EClassifier108'):
        assert not _is_linked(b2, 'ecore_EClassifier108', a)
...
def test_assoc_eContainingClass51_link_reassign_clear():
    a = ecore_EOperation()
    b1 = ecore_EClass(abstract=True, interface=True)
    b2 = ecore_EClass(abstract=False, interface=False)
...
def test_assoc_eEnum47_link_reassign_clear():
...
def test_assoc_eGenericExceptions59_link_reassign_clear():
    ...
    _safe_set(a, 'ecore_EOperation60', {b1})
    assert _is_linked(a, 'ecore_EOperation60', b1)
    if hasattr(b1, 'ecore_EGenericType61'):
        assert _is_linked(b1, 'ecore_EGenericType61', a)
    _safe_set(a, 'ecore_EOperation60', {b2})
    assert _is_linked(a, 'ecore_EOperation60', b2)
    if hasattr(b1, 'ecore_EGenericType61'):
    ...
    _safe_set(a, 'ecore_EOperation60', set())
    assert not _is_linked(a, 'ecore_EOperation60', b2)
    if hasattr(b2, 'ecore_EGenericType61'):
...
def test_assoc_eLiterals46_link_reassign_clear():
    a = ecore_EEnumLiteral(instance="sample_text", literal="sample_text", value=7)
    b1 = ecore_EEnum()
    b2 = ecore_EEnum()
...
def test_assoc_eLowerBound101_link_reassign_clear():
    a = ecore_EGenericType()
    b1 = ecore_EGenericType()
    b2 = ecore_EGenericType()
    _safe_set(a, 'ecore_EGenericType100', b1)
    assert _is_linked(a, 'ecore_EGenericType100', b1)
    if hasattr(b1, 'ecore_EGenericType102'):
        assert _is_linked(b1, 'ecore_EGenericType102', a)
    _safe_set(a, 'ecore_EGenericType100', b2)
    assert _is_linked(a, 'ecore_EGenericType100', b2)
    if hasattr(b1, 'ecore_EGenericType102'):
        assert not _is_linked(b1, 'ecore_EGenericType102', a)
    if hasattr(b2, 'ecore_EGenericType102'):
        assert _is_linked(b2, 'ecore_EGenericType102', a)
    _safe_set(a, 'ecore_EGenericType100', None)
    assert not _is_linked(a, 'ecore_EGenericType100', b2)
    if hasattr(b2, 'ecore_EGenericType102'):
        assert not _is_linked(b2, 'ecore_EGenericType102', a)
...
def test_assoc_eOperation73_link_reassign_clear():
    a = ecore_EOperation()
    b1 = ecore_EParameter()
    b2 = ecore_EParameter()
    _safe_set(a, 'EOperation74', b1)
    assert _is_linked(a, 'EOperation74', b1)
    if hasattr(b1, 'eParameters'):
        assert _is_linked(b1, 'eParameters', a)
    _safe_set(a, 'EOperation74', b2)
    assert _is_linked(a, 'EOperation74', b2)
    if hasattr(b1, 'eParameters'):
        assert not _is_linked(b1, 'eParameters', a)
    if hasattr(b2, 'eParameters'):
        assert _is_linked(b2, 'eParameters', a)
    _safe_set(a, 'EOperation74', None)
    assert not _is_linked(a, 'EOperation74', b2)
    if hasattr(b2, 'eParameters'):
...
def test_assoc_ePackage43_link_reassign_clear():
    ...
    b1 = ecore_EClassifier(defaultValue="sample_text", instanceClass="sample_text", instanceClassName="sample_text", instanceTypeName="sample_text")
    b2 = ecore_EClassifier(defaultValue="sample_text_2", instanceClass="sample_text_2", instanceClassName="sample_text_2", instanceTypeName="sample_text_2")
    _safe_set(a, 'EPackage44', b1)
    assert _is_linked(a, 'EPackage44', b1)
    if hasattr(b1, 'eClassifiers'):
        assert _is_linked(b1, 'eClassifiers', a)
    _safe_set(a, 'EPackage44', b2)
    assert _is_linked(a, 'EPackage44', b2)
    if hasattr(b1, 'eClassifiers'):
        assert not _is_linked(b1, 'eClassifiers', a)
    if hasattr(b2, 'eClassifiers'):
        assert _is_linked(b2, 'eClassifiers', a)
    _safe_set(a, 'EPackage44', None)
    assert not _is_linked(a, 'EPackage44', b2)
    if hasattr(b2, 'eClassifiers'):
        assert not _is_linked(b2, 'eClassifiers', a)


def test_assoc_ePackage48_link_reassign_clear():
    a = ecore_EPackage(nsPrefix="sample_text", nsURI="sample_text")
    b1 = ecore_EFactory()
    b2 = ecore_EFactory()
...
def test_assoc_eParameters55_link_reassign_clear():
    a = ecore_EOperation()
    b1 = ecore_EParameter()
    b2 = ecore_EParameter()
    _safe_set(a, 'eOperation', {b1})
...
def test_assoc_eRawType97_link_reassign_clear():
    ...
    b1 = ecore_EClassifier(defaultValue="sample_text", instanceClass="sample_text", instanceClassName="sample_text", instanceTypeName="sample_text")
    b2 = ecore_EClassifier(defaultValue="sample_text_2", instanceClass="sample_text_2", instanceClassName="sample_text_2", instanceTypeName="sample_text_2")
    _safe_set(a, 'ecore_EGenericType98', b1)
    assert _is_linked(a, 'ecore_EGenericType98', b1)
    if hasattr(b1, 'ecore_EClassifier99'):
        assert _is_linked(b1, 'ecore_EClassifier99', a)
    _safe_set(a, 'ecore_EGenericType98', b2)
    assert _is_linked(a, 'ecore_EGenericType98', b2)
    if hasattr(b1, 'ecore_EClassifier99'):
        assert not _is_linked(b1, 'ecore_EClassifier99', a)
    if hasattr(b2, 'ecore_EClassifier99'):
        assert _is_linked(b2, 'ecore_EClassifier99', a)
    _safe_set(a, 'ecore_EGenericType98', None)
    assert not _is_linked(a, 'ecore_EGenericType98', b2)
    if hasattr(b2, 'ecore_EClassifier99'):
        assert not _is_linked(b2, 'ecore_EClassifier99', a)
...
def test_assoc_eTypeArguments95_link_reassign_clear():
    a = ecore_EGenericType()
    b1 = ecore_EGenericType()
    b2 = ecore_EGenericType()
    _safe_set(a, 'ecore_EGenericType94', {b1})
    assert _is_linked(a, 'ecore_EGenericType94', b1)
    if hasattr(b1, 'ecore_EGenericType96'):
        assert _is_linked(b1, 'ecore_EGenericType96', a)
    _safe_set(a, 'ecore_EGenericType94', {b2})
    assert _is_linked(a, 'ecore_EGenericType94', b2)
    if hasattr(b1, 'ecore_EGenericType96'):
        assert not _is_linked(b1, 'ecore_EGenericType96', a)
    if hasattr(b2, 'ecore_EGenericType96'):
        assert _is_linked(b2, 'ecore_EGenericType96', a)
    _safe_set(a, 'ecore_EGenericType94', set())
    assert not _is_linked(a, 'ecore_EGenericType94', b2)
    if hasattr(b2, 'ecore_EGenericType96'):
        assert not _is_linked(b2, 'ecore_EGenericType96', a)


def test_assoc_eTypeParameter103_link_reassign_clear():
    a = ecore_EGenericType()
    b1 = ecore_ETypeParameter()
    b2 = ecore_ETypeParameter()
    _safe_set(a, 'ecore_EGenericType104', b1)
    assert _is_linked(a, 'ecore_EGenericType104', b1)
    if hasattr(b1, 'ecore_ETypeParameter105'):
        assert _is_linked(b1, 'ecore_ETypeParameter105', a)
    _safe_set(a, 'ecore_EGenericType104', b2)
    assert _is_linked(a, 'ecore_EGenericType104', b2)
    if hasattr(b1, 'ecore_ETypeParameter105'):
        assert not _is_linked(b1, 'ecore_ETypeParameter105', a)
    if hasattr(b2, 'ecore_ETypeParameter105'):
        assert _is_linked(b2, 'ecore_ETypeParameter105', a)
    _safe_set(a, 'ecore_EGenericType104', None)
    assert not _is_linked(a, 'ecore_EGenericType104', b2)
    if hasattr(b2, 'ecore_ETypeParameter105'):
        assert not _is_linked(b2, 'ecore_ETypeParameter105', a)


def test_assoc_eTypeParameters45_link_reassign_clear():
...
def test_assoc_eTypeParameters52_link_reassign_clear():
    a = ecore_EOperation()
    b1 = ecore_ETypeParameter()
    b2 = ecore_ETypeParameter()
    _safe_set(a, 'ecore_EOperation53', {b1})
    assert _is_linked(a, 'ecore_EOperation53', b1)
    if hasattr(b1, 'ecore_ETypeParameter54'):
        assert _is_linked(b1, 'ecore_ETypeParameter54', a)
    _safe_set(a, 'ecore_EOperation53', {b2})
    assert _is_linked(a, 'ecore_EOperation53', b2)
    if hasattr(b1, 'ecore_ETypeParameter54'):
        assert not _is_linked(b1, 'ecore_ETypeParameter54', a)
    if hasattr(b2, 'ecore_ETypeParameter54'):
        assert _is_linked(b2, 'ecore_ETypeParameter54', a)
    _safe_set(a, 'ecore_EOperation53', set())
    assert not _is_linked(a, 'ecore_EOperation53', b2)
    if hasattr(b2, 'ecore_ETypeParameter54'):
        assert not _is_linked(b2, 'ecore_ETypeParameter54', a)


def test_assoc_eUpperBound92_link_reassign_clear():
    a = ecore_EGenericType()
    b1 = ecore_EGenericType()
    b2 = ecore_EGenericType()
...
def test_assoc_myClasses71_link_reassign_clear():
    a = ecore_EPackage(nsPrefix="sample_text", nsURI="sample_text")
    b1 = ecore_EClass(abstract=True, interface=True)
    b2 = ecore_EClass(abstract=False, interface=False)
    _safe_set(a, 'validOpposite', {b1})
    assert _is_linked(a, 'validOpposite', b1)
    if hasattr(b1, 'EClass72'):
        assert _is_linked(b1, 'EClass72', a)
    _safe_set(a, 'validOpposite', {b2})
    assert _is_linked(a, 'validOpposite', b2)
    if hasattr(b1, 'EClass72'):
        assert not _is_linked(b1, 'EClass72', a)
    if hasattr(b2, 'EClass72'):
        assert _is_linked(b2, 'EClass72', a)
    _safe_set(a, 'validOpposite', set())
    assert not _is_linked(a, 'validOpposite', b2)
    if hasattr(b2, 'EClass72'):
        assert not _is_linked(b2, 'EClass72', a)
...
def test_assoc_validOpposite42_link_reassign_clear():
    a = ecore_EPackage(nsPrefix="sample_text", nsURI="sample_text")
    b1 = ecore_EClass(abstract=True, interface=True)
    b2 = ecore_EClass(abstract=False, interface=False)
    _safe_set(a, 'EPackage', b1)
    assert _is_linked(a, 'EPackage', b1)
    if hasattr(b1, 'myClasses'):
        assert _is_linked(b1, 'myClasses', a)
    _safe_set(a, 'EPackage', b2)
    assert _is_linked(a, 'EPackage', b2)
    if hasattr(b1, 'myClasses'):
        assert not _is_linked(b1, 'myClasses', a)
    if hasattr(b2, 'myClasses'):
        assert _is_linked(b2, 'myClasses', a)
    _safe_set(a, 'EPackage', None)
    assert not _is_linked(a, 'EPackage', b2)
    if hasattr(b2, 'myClasses'):
        assert not _is_linked(b2, 'myClasses', a)
