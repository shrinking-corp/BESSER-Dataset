...
def test_hyp_package_shoppingcart_is_not_abstract():
    assert not inspect.isabstract(Package_ShoppingCart)


def test_hyp_package_shoppingcart_constructor_exists():
    assert callable(Package_ShoppingCart.__init__)


def test_hyp_package_shoppingcart_constructor_args():
    sig = inspect.signature(Package_ShoppingCart.__init__)
...
Package_ShoppingCart_strategy = st.builds(
    Package_ShoppingCart,
    creationDate=
        st.dates()
)
...
@given(instance=Package_ShoppingCart_strategy)
def test_hyp_package_shoppingcart_creationDate_setter(instance):
...
def test_Package_ShoppingCart_creationDate_value_roundtrip():
    instance = Package_ShoppingCart(creationDate=date(2024, 1, 1))
...
def test_assoc_Account_ShoppingCart_link_reassign_clear():
    a = Package_ShoppingCart(creationDate=date(2024, 1, 1))
    b1 = Account(billingAddress="sample_text", closed=date(2024, 1, 1), isClosed=True, open=date(2024, 1, 1))
    b2 = Account(billingAddress="sample_text_2", closed=date(2025, 6, 15), isClosed=False, open=date(2025, 6, 15))
...
Package_ShoppingCart_strategy = st.builds(Package_ShoppingCart, creationDate=st.dates())
@given(instance=Package_ShoppingCart_strategy)
@settings(max_examples=25)
def test_Package_ShoppingCart_instantiation(instance):
    assert isinstance(instance, Package_ShoppingCart)
