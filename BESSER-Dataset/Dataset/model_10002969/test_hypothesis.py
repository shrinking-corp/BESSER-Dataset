...
def test_billing_is_not_abstract():
    assert not inspect.isabstract(billing)


def test_billing_constructor_exists():
    assert callable(billing.__init__)


def test_billing_constructor_args():
    sig = inspect.signature(billing.__init__)
    params = list(sig.parameters.keys())
    assert "patient_name" in params, "Missing parameter 'patient_name'"
    assert "amount" in params, "Missing parameter 'amount'"
    assert "bill_no" in params, "Missing parameter 'bill_no'"

def test_billing_has_patient_name():
    assert hasattr(billing, "patient_name")
    descriptor = None
    for klass in billing.__mro__:
        if "patient_name" in klass.__dict__:
            descriptor = klass.__dict__["patient_name"]
            break
    assert isinstance(descriptor, property)

def test_billing_has_amount():
    assert hasattr(billing, "amount")
    descriptor = None
    for klass in billing.__mro__:
        if "amount" in klass.__dict__:
            descriptor = klass.__dict__["amount"]
...
def test_billing_has_bill_no():
    assert hasattr(billing, "bill_no")
    descriptor = None
    for klass in billing.__mro__:
...
@given(instance=billing_strategy)
def test_billing_patient_name_setter(instance):
    original = instance.patient_name
    instance.patient_name = original
    assert instance.patient_name == original
...
@given(instance=billing_strategy)
def test_billing_amount_setter(instance):
    original = instance.amount
    instance.amount = original
    assert instance.amount == original
...
@given(instance=billing_strategy)
def test_billing_bill_no_setter(instance):
