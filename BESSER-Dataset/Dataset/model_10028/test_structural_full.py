...
def test_SBCS_ControlProgram_mode_value_roundtrip():
    instance = SBCS_ControlProgram(mode="sample_text", pumpControlerFailure=True, pumpFailure=True, smdFailure=True)
...
def test_assoc_AfterTrans27_link_reassign_clear():
    a = SBCS_Snapshot()
    b1 = SBCS_Transition()
    b2 = SBCS_Transition()
    _safe_set(a, 'SBCS_Snapshot29', b1)
    assert _is_linked(a, 'SBCS_Snapshot29', b1)
    if hasattr(b1, 'SBCS_Transition28'):
        assert _is_linked(b1, 'SBCS_Transition28', a)
    _safe_set(a, 'SBCS_Snapshot29', b2)
    assert _is_linked(a, 'SBCS_Snapshot29', b2)
    if hasattr(b1, 'SBCS_Transition28'):
        assert not _is_linked(b1, 'SBCS_Transition28', a)
    if hasattr(b2, 'SBCS_Transition28'):
        assert _is_linked(b2, 'SBCS_Transition28', a)
    _safe_set(a, 'SBCS_Snapshot29', None)
    assert not _is_linked(a, 'SBCS_Snapshot29', b2)
    if hasattr(b2, 'SBCS_Transition28'):
        assert not _is_linked(b2, 'SBCS_Transition28', a)


def test_assoc_AfterTrans5_link_reassign_clear():
    a = SBCS_Snapshot()
    b1 = SBCS_Transition()
    b2 = SBCS_Transition()
    _safe_set(a, 'SBCS_Snapshot6', b1)
    assert _is_linked(a, 'SBCS_Snapshot6', b1)
    if hasattr(b1, 'SBCS_Transition'):
        assert _is_linked(b1, 'SBCS_Transition', a)
    _safe_set(a, 'SBCS_Snapshot6', b2)
    assert _is_linked(a, 'SBCS_Snapshot6', b2)
    if hasattr(b1, 'SBCS_Transition'):
        assert not _is_linked(b1, 'SBCS_Transition', a)
    if hasattr(b2, 'SBCS_Transition'):
        assert _is_linked(b2, 'SBCS_Transition', a)
    _safe_set(a, 'SBCS_Snapshot6', None)
    assert not _is_linked(a, 'SBCS_Snapshot6', b2)
    if hasattr(b2, 'SBCS_Transition'):
        assert not _is_linked(b2, 'SBCS_Transition', a)


def test_assoc_CPPost16_link_reassign_clear():
    a = SBCS_ControlProgram(mode="sample_text", pumpControlerFailure=True, pumpFailure=True, smdFailure=True)
    b1 = SBCS_ControlProgram_Start()
    b2 = SBCS_ControlProgram_Start()
    _safe_set(a, 'SBCS_ControlProgram17', b1)
    assert _is_linked(a, 'SBCS_ControlProgram17', b1)
    if hasattr(b1, 'SBCS_ControlProgram_Start'):
        assert _is_linked(b1, 'SBCS_ControlProgram_Start', a)
    _safe_set(a, 'SBCS_ControlProgram17', b2)
    assert _is_linked(a, 'SBCS_ControlProgram17', b2)
    if hasattr(b1, 'SBCS_ControlProgram_Start'):
    ...
    _safe_set(a, 'SBCS_ControlProgram17', None)
    assert not _is_linked(a, 'SBCS_ControlProgram17', b2)
    if hasattr(b2, 'SBCS_ControlProgram_Start'):
        assert not _is_linked(b2, 'SBCS_ControlProgram_Start', a)


def test_assoc_ControlProgramSnapshot4_link_reassign_clear():
    a = SBCS_Snapshot()
    b1 = SBCS_ControlProgram(mode="sample_text", pumpControlerFailure=True, pumpFailure=True, smdFailure=True)
    b2 = SBCS_ControlProgram(mode="sample_text_2", pumpControlerFailure=False, pumpFailure=False, smdFailure=False)
...
def test_assoc_SBPost20_link_reassign_clear():
    a = SBCS_SteamBoiler(valveOpen="sample_text")
    b1 = SBCS_SteamBoiler_OpenValve()
    b2 = SBCS_SteamBoiler_OpenValve()
    _safe_set(a, 'SBCS_SteamBoiler22', b1)
    assert _is_linked(a, 'SBCS_SteamBoiler22', b1)
    if hasattr(b1, 'SBCS_SteamBoiler_OpenValve21'):
        assert _is_linked(b1, 'SBCS_SteamBoiler_OpenValve21', a)
    _safe_set(a, 'SBCS_SteamBoiler22', b2)
    assert _is_linked(a, 'SBCS_SteamBoiler22', b2)
    if hasattr(b1, 'SBCS_SteamBoiler_OpenValve21'):
        assert not _is_linked(b1, 'SBCS_SteamBoiler_OpenValve21', a)
    if hasattr(b2, 'SBCS_SteamBoiler_OpenValve21'):
        assert _is_linked(b2, 'SBCS_SteamBoiler_OpenValve21', a)
    _safe_set(a, 'SBCS_SteamBoiler22', None)
    assert not _is_linked(a, 'SBCS_SteamBoiler22', b2)
    if hasattr(b2, 'SBCS_SteamBoiler_OpenValve21'):
        assert not _is_linked(b2, 'SBCS_SteamBoiler_OpenValve21', a)


def test_assoc_SBPre18_link_reassign_clear():
    a = SBCS_SteamBoiler(valveOpen="sample_text")
    b1 = SBCS_SteamBoiler_OpenValve()
    b2 = SBCS_SteamBoiler_OpenValve()
    _safe_set(a, 'SBCS_SteamBoiler19', b1)
    assert _is_linked(a, 'SBCS_SteamBoiler19', b1)
    if hasattr(b1, 'SBCS_SteamBoiler_OpenValve'):
        assert _is_linked(b1, 'SBCS_SteamBoiler_OpenValve', a)
    _safe_set(a, 'SBCS_SteamBoiler19', b2)
    assert _is_linked(a, 'SBCS_SteamBoiler19', b2)
    if hasattr(b1, 'SBCS_SteamBoiler_OpenValve'):
    ...
    _safe_set(a, 'SBCS_SteamBoiler19', None)
    assert not _is_linked(a, 'SBCS_SteamBoiler19', b2)
    if hasattr(b2, 'SBCS_SteamBoiler_OpenValve'):
        assert not _is_linked(b2, 'SBCS_SteamBoiler_OpenValve', a)


def test_assoc_SnapshotControlProgram23_link_reassign_clear():
    a = SBCS_Snapshot()
    b1 = SBCS_ControlProgram(mode="sample_text", pumpControlerFailure=True, pumpFailure=True, smdFailure=True)
    b2 = SBCS_ControlProgram(mode="sample_text_2", pumpControlerFailure=False, pumpFailure=False, smdFailure=False)
    _safe_set(a, 'SBCS_Snapshot25', b1)
    assert _is_linked(a, 'SBCS_Snapshot25', b1)
    if hasattr(b1, 'SBCS_ControlProgram24'):
        assert _is_linked(b1, 'SBCS_ControlProgram24', a)
    _safe_set(a, 'SBCS_Snapshot25', b2)
    assert _is_linked(a, 'SBCS_Snapshot25', b2)
    if hasattr(b1, 'SBCS_ControlProgram24'):
        assert not _is_linked(b1, 'SBCS_ControlProgram24', a)
    if hasattr(b2, 'SBCS_ControlProgram24'):
        assert _is_linked(b2, 'SBCS_ControlProgram24', a)
    _safe_set(a, 'SBCS_Snapshot25', None)
    assert not _is_linked(a, 'SBCS_Snapshot25', b2)
    if hasattr(b2, 'SBCS_ControlProgram24'):
        assert not _is_linked(b2, 'SBCS_ControlProgram24', a)


def test_assoc_SteamBoilerControlProgram14_link_reassign_clear():
    a = SBCS_SteamBoiler(valveOpen="sample_text")
    b1 = SBCS_ControlProgram(mode="sample_text", pumpControlerFailure=True, pumpFailure=True, smdFailure=True)
    b2 = SBCS_ControlProgram(mode="sample_text_2", pumpControlerFailure=False, pumpFailure=False, smdFailure=False)
    _safe_set(a, 'SBCS_SteamBoiler', b1)
    assert _is_linked(a, 'SBCS_SteamBoiler', b1)
    if hasattr(b1, 'SBCS_ControlProgram15'):
        assert _is_linked(b1, 'SBCS_ControlProgram15', a)
    _safe_set(a, 'SBCS_SteamBoiler', b2)
    assert _is_linked(a, 'SBCS_SteamBoiler', b2)
    if hasattr(b1, 'SBCS_ControlProgram15'):
        assert not _is_linked(b1, 'SBCS_ControlProgram15', a)
    if hasattr(b2, 'SBCS_ControlProgram15'):
        assert _is_linked(b2, 'SBCS_ControlProgram15', a)
    _safe_set(a, 'SBCS_SteamBoiler', None)
    assert not _is_linked(a, 'SBCS_SteamBoiler', b2)
    if hasattr(b2, 'SBCS_ControlProgram15'):
        assert not _is_linked(b2, 'SBCS_ControlProgram15', a)


def test_assoc_wlmdPost26_link_reassign_clear():
...
SBCS_ControlProgram_strategy = st.builds(SBCS_ControlProgram, mode=safe_text, pumpControlerFailure=st.booleans(), pumpFailure=st.booleans(), smdFailure=st.booleans())
