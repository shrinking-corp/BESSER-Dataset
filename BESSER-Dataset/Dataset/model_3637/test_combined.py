...
def test_hyp_dfaautomaton_symbol_is_not_abstract():
    assert not inspect.isabstract(DFAAutomaton_Symbol)


def test_hyp_dfaautomaton_symbol_constructor_exists():
    assert callable(DFAAutomaton_Symbol.__init__)


def test_hyp_dfaautomaton_symbol_constructor_args():
    sig = inspect.signature(DFAAutomaton_Symbol.__init__)
...
DFAAutomaton_Symbol_strategy = st.builds(
    DFAAutomaton_Symbol,
    symbol=
        safe_text
)
...
@given(instance=DFAAutomaton_Symbol_strategy)
def test_hyp_dfaautomaton_symbol_symbol_setter(instance):
...
def test_DFAAutomaton_Symbol_symbol_value_roundtrip():
    instance = DFAAutomaton_Symbol(symbol="sample_text")
...
def test_assoc_alphabet3_link_reassign_clear():
    a = DFAAutomaton_Symbol(symbol="sample_text")
    b1 = DFAAutomaton_Automaton(name="sample_text")
    b2 = DFAAutomaton_Automaton(name="sample_text_2")
    _safe_set(a, 'DFAAutomaton_Symbol', b1)
    assert _is_linked(a, 'DFAAutomaton_Symbol', b1)
    if hasattr(b1, 'DFAAutomaton_Automaton4'):
        assert _is_linked(b1, 'DFAAutomaton_Automaton4', a)
    _safe_set(a, 'DFAAutomaton_Symbol', b2)
    assert _is_linked(a, 'DFAAutomaton_Symbol', b2)
    if hasattr(b1, 'DFAAutomaton_Automaton4'):
        assert not _is_linked(b1, 'DFAAutomaton_Automaton4', a)
    if hasattr(b2, 'DFAAutomaton_Automaton4'):
        assert _is_linked(b2, 'DFAAutomaton_Automaton4', a)
    _safe_set(a, 'DFAAutomaton_Symbol', None)
    assert not _is_linked(a, 'DFAAutomaton_Symbol', b2)
    if hasattr(b2, 'DFAAutomaton_Automaton4'):
        assert not _is_linked(b2, 'DFAAutomaton_Automaton4', a)
...
def test_assoc_symbol11_link_reassign_clear():
    ...
    b1 = DFAAutomaton_Transition()
    b2 = DFAAutomaton_Transition()
    _safe_set(a, 'DFAAutomaton_Symbol13', b1)
    assert _is_linked(a, 'DFAAutomaton_Symbol13', b1)
    if hasattr(b1, 'DFAAutomaton_Transition12'):
        assert _is_linked(b1, 'DFAAutomaton_Transition12', a)
    _safe_set(a, 'DFAAutomaton_Symbol13', b2)
    assert _is_linked(a, 'DFAAutomaton_Symbol13', b2)
    if hasattr(b1, 'DFAAutomaton_Transition12'):
    ...
    _safe_set(a, 'DFAAutomaton_Symbol13', None)
    assert not _is_linked(a, 'DFAAutomaton_Symbol13', b2)
    if hasattr(b2, 'DFAAutomaton_Transition12'):
...
DFAAutomaton_Symbol_strategy = st.builds(DFAAutomaton_Symbol, symbol=safe_text)
@given(instance=DFAAutomaton_Symbol_strategy)
@settings(max_examples=25)
def test_DFAAutomaton_Symbol_instantiation(instance):
    assert isinstance(instance, DFAAutomaton_Symbol)
