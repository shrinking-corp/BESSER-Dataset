...
def test_assoc_A2_B2_link_reassign_clear():
    a = B2(attB=7)
...
def test_assoc_B2_C2_link_reassign_clear():
    a = C1(attC1=7, attC2=True)
    b1 = B2(attB=7)
    b2 = B2(attB=13)
    _safe_set(a, 'b29', b1)
    assert _is_linked(a, 'b29', b1)
    if hasattr(b1, 'c8'):
        assert _is_linked(b1, 'c8', a)
    _safe_set(a, 'b29', b2)
    assert _is_linked(a, 'b29', b2)
    if hasattr(b1, 'c8'):
        assert not _is_linked(b1, 'c8', a)
    if hasattr(b2, 'c8'):
        assert _is_linked(b2, 'c8', a)
    _safe_set(a, 'b29', None)
    assert not _is_linked(a, 'b29', b2)
    if hasattr(b2, 'c8'):
