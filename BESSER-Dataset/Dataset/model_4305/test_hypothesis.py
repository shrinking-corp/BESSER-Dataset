...
def test_metrics_ruleset_is_not_abstract():
    assert not inspect.isabstract(metrics_RuleSet)


def test_metrics_ruleset_constructor_exists():
    assert callable(metrics_RuleSet.__init__)


def test_metrics_ruleset_constructor_args():
...
def test_metrics_mappingexternal_is_not_abstract():
    assert not inspect.isabstract(metrics_MappingExternal)


def test_metrics_mappingexternal_constructor_exists():
    assert callable(metrics_MappingExternal.__init__)


def test_metrics_mappingexternal_constructor_args():
...
def test_metrics_mappingexternal_has_pluginID():
    assert hasattr(metrics_MappingExternal, "pluginID")
    descriptor = None
    for klass in metrics_MappingExternal.__mro__:
        if "pluginID" in klass.__dict__:
            descriptor = klass.__dict__["pluginID"]
            break
    assert isinstance(descriptor, property)

def test_metrics_mappingexternal_has_classURI():
    assert hasattr(metrics_MappingExternal, "classURI")
    descriptor = None
    for klass in metrics_MappingExternal.__mro__:
        if "classURI" in klass.__dict__:
            descriptor = klass.__dict__["classURI"]
            break
    assert isinstance(descriptor, property)
...
@given(instance=metrics_MappingExternal_strategy)
def test_metrics_mappingexternal_pluginID_setter(instance):
...
@given(instance=metrics_MappingExternal_strategy)
def test_metrics_mappingexternal_classURI_setter(instance):
