...
def test_attendance_system_actor_is_not_abstract():
    assert not inspect.isabstract(Attendance_system_Actor)


def test_attendance_system_actor_constructor_exists():
    assert callable(Attendance_system_Actor.__init__)


def test_attendance_system_actor_constructor_args():
    sig = inspect.signature(Attendance_system_Actor.__init__)
    params = list(sig.parameters.keys())
...
Attendance_system_Actor_strategy = st.builds(
    Attendance_system_Actor,
)
...
@given(instance=Attendance_system_Actor_strategy)
@settings(max_examples=50)
def test_attendance_system_actor_instantiation(instance):
    assert isinstance(instance, Attendance_system_Actor)
